...
enum IVP_Movement_Type {
  ...
  IVP_MT_CALM =
      0x03,  // slow for more than a certain time span (currently 1 second)
  IVP_MT_NOT_SIM = 0x08,  // not simulated, but can be changed to be simulated
  IVP_MT_STATIC_PHANTOM = 0x09,  // like static, but object may be moved by
                                 // setting the matrix directly
  IVP_MT_STATIC = 0x10,          // static object
  IVP_MT_GET_MINDIST = 0x21      // not really a movement type, used to get all
                                 // mindists with recheck_ov_element
};
...
class IVP_Real_Object_Fast_Static : public IVP_Object {
 protected:
  ...
  class IVP_Synapse_Real *
      exact_synapses;  // Linked list of all exact synapses, no sphere nor hull.
  ...
  IVP_Real_Object_Fast_Static(IVP_Cluster *father,
      ...
      : IVP_Object(father, templ),
        ...
        shift_core_f_object{std::numeric_limits<float>::quiet_NaN(),
...
class IVP_Real_Object_Fast : public IVP_Real_Object_Fast_Static {
 ...
 public:
  struct {
    IVP_Movement_Type
        object_movement_state : 8;  // moving, not_simulated, static only
    IVP_BOOL
    collision_detection_enabled : 2; /* Collision detection is enabled. If
                                      * it is disabled, not even the
                                      * IVP_Collision_Filter is called */
    IVP_BOOL shift_core_f_object_is_zero : 2;

    unsigned int object_listener_exists : 1;    /* is set to one if an object
                                                   listener exists */
    unsigned int collision_listener_exists : 1; /* all flags of object listeners
                                                   functions */
  ...
  IVP_Real_Object_Fast(IVP_Cluster *father, const IVP_Template_Object *templ)
      : IVP_Real_Object_Fast_Static(father, templ), cache_object(nullptr) {
...
class IVP_Real_Object : public IVP_Real_Object_Fast {
 ...
 protected:
  IVP_SurfaceManager *surface_manager;  // The surface_manager defines the
                                        // surface structure for this object.
  ...
  virtual void set_new_quat_object_f_core(
      ...
      const IVP_U_Point *trans_object_f_core);  // calls set_new_m...
 public:
  virtual void set_new_m_object_f_core(
      const IVP_U_Matrix
          *new_m_object_f_core); /* specifies new objects coordinate system in
                                    core space */
  void init_object_core(IVP_Environment *env,
                        const IVP_Template_Real_Object *tpop);
  void unlink_contact_points_for_object(IVP_Real_Object *other_object);

 protected:
  IVP_Real_Object(IVP_Cluster *father, IVP_SurfaceManager *,
                  const IVP_Template_Real_Object *,
                  const IVP_U_Quat *q_world_f_obj, const IVP_U_Point *position);

  virtual ~IVP_Real_Object();  // to delete the object use
                               // delete_and_check_vicinity or delete_silently

  /********************************************************************************
   *	The physics simulation internal public section. Handle with care:
   ********************************************************************************/
 public:
  char nocoll_group_ident
      [IVP_NO_COLL_GROUP_STRING_LEN]; /* Identifier for filtering collisions,
                                       * used only if the
                                       * IVP_Collision_Filter_Coll_Group_Ident
                                       * is the installed collision filter, see
                                       * IVP_Application_Environment. */
  ///////// Simulation internals
  IVP_Material *l_default_material;  // default material of object
  IVP_OV_Element *ov_element;  // used to trace objects movement within the
                               // environment->ov_tree
  IVP_FLOAT extra_radius;      // extra collision radius around the object

  ///////// Core
  IVP_Core *physical_core;  // Coordinate system, mass, speed, etc...
                            // (eventually merged)
  IVP_Core
      *friction_core;  // Long term (merged) core for frictions, when objs are
                       // splitted make sure to call 'recheck_ov_element'
  ...
  IVP_Hull_Manager *get_hull_manager() {
    return (IVP_Hull_Manager *)&this->hull_manager;
  }
  void reset_time(IVP_Time offset);

  void revive_object_for_simulation();  // wake up object immediately ; @@@IK
                                        // don't use this - use
                                        // ensure_in_simulation() instead!

  IVP_SurfaceManager *get_surface_manager() const { return surface_manager; }
  IVP_OV_Element *get_ov_element() { return ov_element; }

  void set_movement_state(IVP_Movement_Type mt) {
    flags.object_movement_state = mt;
  }

  IVP_FLOAT get_extra_radius() const {
    return extra_radius;
  }  // returns an extra collision radius around the object#
  void set_extra_radius(IVP_DOUBLE new_radius);
  inline IVP_Cache_Object *
  get_cache_object_no_lock();  // see get_cache_object, but restricted usage

  IVP_Real_Object *to_nonconst() const {
    return (IVP_Real_Object *)this;
  }  // remove const
  /********************************************************************************
  ...
  /*************************************************************************************
   *	Name:	    	get_geom_xxx
   *	Note:		Geometric center/radius is not the same as mass
   *center/radius!
   ************************************************************************************/
  ...
  /***********************************************************************************
   *	Name:	    	calc_at_matrix
   *	Description:	Get the object's matrix for the display engine at Any
   *Time (at). Note:		The matrix is an interpolation of two PSI
   *matrizes.
   **********************************************************************************/
  void calc_at_matrix(IVP_Time current_time_in,
                      IVP_U_Matrix *m_world_f_object_out) const;
  void calc_at_quaternion(IVP_Time current_time_in,
                          IVP_U_Quat *q_world_f_object_out,
                          IVP_U_Point *position_ws_out) const;
   ...
   *                      optimize_for_repeated_calls: set to true if you are
   *planning to call this function every frame Attention:	If the objects
   *penetrates other objects at the new position, no collisions will be
   *generated !!!
   **********************************************************************************/
  void beam_object_to_new_position(
      const IVP_U_Quat *rotation, const IVP_U_Point *position,
      IVP_BOOL optimize_for_repeated_calls = IVP_FALSE);

  /****************************************************************************************************
   *	Name:	    	get_m_world_f_object_AT
   *	Description:	Same as calc_at_matrix but point of time is taken from
   *environment. Note:		Implementation looks like:
   *			'calc_at_matrix(get_environment()->get_current_time(),
   ...
   *	Description:	Calculates a matrix to transform between core and object
   *space.
   **************************************************************************************/
  ...
  IVP_Core *get_original_core() const {
    return original_core;
  }  // Returns the core that was valid at creation of object.

  IVP_Movement_Type get_movement_state() {
    return (IVP_Movement_Type)flags.object_movement_state;
  }  // See declaration of IVP_Movement_Type.

  //////// Simulation
  IVP_BOOL disable_simulation();  // Freeze object immediately disregarding the
                                  // speed, return IVP_TRUE when success
  ...
  int get_collision_check_reference_count();  // see IVP_Universe_Manager for
                                              // details

  /********************************************************************************
   *	Name:	    	async_add_speed_object_ws and
   *async_add_rot_speed_object_cs Description: 	Easy to use functions used to
   *specify speed and rotation speed changes of an object at any time. Side
   *Effects:	Object is woken up. The speed changes are committed at next PSI.
   ********************************************************************************/
  void async_push_object_ws(
      const IVP_U_Point *position_ws,
      const IVP_U_Float_Point
          *impulse_ws);  // impulse vec's length = impulse strength [kg*m/s]
  void async_add_speed_object_ws(
      const IVP_U_Float_Point
          *speed_ws);  // add a value to the core's speed_change variable
  void async_add_rot_speed_object_cs(
      const IVP_U_Float_Point *rotation_vec_cs);  // add a value to the core's
                                                  // rot_speed_change variable

  IVP_BOOL
  is_collision_detection_enabled() {  // Returns IVP_FALSE if
                                      // IVP_Environment::enable_collision_detection
                                      // was not called
  ...
  void add_listener_collision(
      IVP_Listener_Collision *);  // Adds a private collision callback.
  void remove_listener_collision(
      IVP_Listener_Collision *);  // Removes a private collision callback.

  void add_listener_object(
      IVP_Listener_Object *);  // Adds an private object callback.
  void remove_listener_object(
      IVP_Listener_Object *);  // Removes a private object callback.

  /**************************************************************************************
   *	Name:	    	set_motion_controller
   *	Description:	Specifies an external motion controller.
   * 			Overrides any existing motion controller for this
   *object. NULL removes controller. Version info:   use
   *IVP_Real_Object::create_controller_motion to get same functionality
   **************************************************************************************/
   ...
   *	Description:	to be called when the collisionfilter has a new
   *behaviour
   **************************************************************************************/
  ...
  void change_nocoll_group_ident(
      const char *new_identify_string);  // note: max length
