...
void IVP_Anomaly_Manager::max_velocity_exceeded(
    IVP_Anomaly_Limits *al, IVP_Core *, IVP_U_Float_Point *velocity_in_out) {
  IVP_DOUBLE square_speed = velocity_in_out->quad_length();
  IVP_DOUBLE max_speed = al->get_max_velocity();
  velocity_in_out->mult(0.99f * max_speed /
                        IVP_Inline_Math::ivp_sqrtf(square_speed));
}

void IVP_Anomaly_Manager::max_angular_velocity_exceeded(
    IVP_Anomaly_Limits *al, IVP_Core *core,
    IVP_U_Float_Point *angular_velocity_in_out) {
  IVP_DOUBLE square_rot_speed = angular_velocity_in_out->quad_length();
  IVP_DOUBLE max_rot_speed = al->get_max_angular_velocity_per_psi() *
                             core->environment->get_inv_delta_PSI_time();
  angular_velocity_in_out->mult(0.9f * max_rot_speed /
                                IVP_Inline_Math::ivp_sqrtf(square_rot_speed));
}

void IVP_Anomaly_Manager::solve_inter_penetration_simple(
    IVP_Real_Object *obj0, IVP_Real_Object *obj1, IVP_DOUBLE speed_change) {
  ...
  if (IVP_MTIS_SIMULATED(core1->movement_state) && !core1->pinned) {
    ...
    obj1->async_push_object_ws(
        &m_world_f_core1->vv,
        &p1);  // PiMoN: was m_world_f_core0, but this is a bug in Valve's code
    IVP_U_Float_Point rot_speed_object(d_time, 0, 0);
  ...
  IVP_IF(1) {
    ...
    if (!name1) name1 = "(null)";
    ivp_message(
        "Pushed (simple) '%s' and '%s' at %.3fs away to solve penetration.\n",
        name0, name1, env->get_current_time().get_time());
  }
}

// TODO(nillerusr) in the decompiler output, this function is different, but
// this should not affect the result
void IVP_Anomaly_Manager::inter_penetration(IVP_Mindist *mindist,
                                            IVP_Real_Object *obj0,
                                            IVP_Real_Object *obj1,
                                            IVP_DOUBLE speed_change) {
  ...
  IVP_Cache_Object *unmovable_cache =
      my_objects[swapped]->get_cache_object_no_lock();
  IVP_U_Point obj_pos_moving;
  unmovable_cache->m_world_f_object.vimult4(&world_pos_moving, &obj_pos_moving);

  const IVP_Compact_Ledge *my_compact_ledge =
      mindist->synapse[swapped].get_ledge();
  const IVP_Compact_Triangle *my_compact_triangle =
      my_compact_ledge->get_first_triangle();
  ...
  for (int i = 0; i < my_compact_ledge->get_n_triangles(); i++) {
    ...
    three_points[0].set(
        my_compact_triangle->get_edge(0)->get_start_point(my_compact_ledge));
    three_points[1].set(
        my_compact_triangle->get_edge(1)->get_start_point(my_compact_ledge));
    three_points[2].set(
        my_compact_triangle->get_edge(2)->get_start_point(my_compact_ledge));

    IVP_U_Hesse hesse_plane;
    hesse_plane.calc_hesse(&three_points[0], &three_points[2],
                           &three_points[1]);
    hesse_plane.normize();

    IVP_DOUBLE distance = hesse_plane.get_dist(
        &obj_pos_moving);  // negative distance means surface is looking away
                           // from me (backside)
  ...
  IVP_IF(1) {
    ...
    if (!name1) name1 = "(null)";
    ivp_message(
        "Pushed (medium) '%s' and '%s' at %.3fs away to solve penetration.\n",
        name0, name1, env->get_current_time().get_time());
  }
#endif
}

IVP_FLOAT IVP_Anomaly_Manager::get_push_speed_penetration(IVP_Real_Object *obj0,
                                                          IVP_Real_Object *) {
...
IVP_BOOL IVP_Anomaly_Manager::max_collisions_exceeded_check_freezing(
