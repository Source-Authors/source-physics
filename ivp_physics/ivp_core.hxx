 ...
 *	Attention:	many internal variables are defined here, so don't get
 *confused. Version Info:	this file constantly evolves, handle with care
 ********************************************************************************/
...
class IVP_Vector_of_Objects : public IVP_U_Vector<IVP_Real_Object> {
 ...
 public:
  IVP_Vector_of_Objects()
      : IVP_U_Vector<IVP_Real_Object>((void **)&elem_buffer[0], 1) {
 ...
 *                      single core for more than one object, thus allowing
 *merged objects (e.g. to create breakable objects). Attention:	Use only
 *functions that are declared at the end of this class declaration.
 ********************************************************************************/
...
class IVP_Core_Fast_Static {
 ...
 public:
   ...
   *simulation without problems (don't forget to call calc_calc() afterwards)
   *  Note:		For memory reasons, the flags will be combined to a
   *bit-area soon. So use the corresponding functions to read/set them.
   ********************************************************************************/
  ...
  IVP_FLOAT
  upper_limit_radius;  // the radius of the object around the mass center
  IVP_FLOAT max_surface_deviation;  // for all P of all Surfaces S:
                                    // maximum P * S

  IVP_Environment *environment;  // Backlink

  class IVP_Constraint_Car_Object
      *car_wheel;  // bad bad hack for optimized wheel behaviour;

 protected:
  IVP_U_Float_Hesse
      rot_inertia;  // needed for energy calculation, mass is w part
 public:
  IVP_U_Float_Point
      rot_speed_damp_factor;  // (*) dampening of speed, one value per axis: new
                              // speed *= 1/e**(rot_speed_damp_factor *
                              // delta_time)

  /********************************************************************************
   *	type:		private, public READ ONLY
   *	section:	basic, redundant, long time unchangeable section
   *(updated by calc_calc)
   ********************************************************************************/
 private:
  IVP_U_Float_Hesse
      inv_rot_inertia;  // 1.0f/rot_inertia, 4th value is inverse_mass
 public:
  IVP_FLOAT speed_damp_factor;       // (*) same for speed
  IVP_FLOAT inv_object_diameter;     // 2.0f/upper_limit_radius
  IVP_U_Float_Point *spin_clipping;  // if set, than angular velocities are
                                     // individually clipped against this value
  ...
  const class IVP_U_Point_4 *get_inv_masses() {
    return (class IVP_U_Point_4 *)&inv_rot_inertia;
  }  // combined solution for inertias
...
class IVP_Core_Fast_PSI : public IVP_Core_Fast_Static {
 public:
  ...
  IVP_Time time_of_last_psi;  // Indicates point in simulation time the last
                              // matrizes are valid at.

  /********************************************************************************
   *	type:		private, public READ ONLY
   *	section:	basic simulation part, redundant part, calculated by
   *calc_next_PSI_matrix
   ********************************************************************************/
  IVP_FLOAT i_delta_time;  // 1.0f / (delta_time)

  // #+# check for empty space (due to align(16))
  IVP_U_Float_Point rot_speed_change;  // Accumulates async_rot_pushes, units
                                       // like this->rot_speed.
  IVP_U_Float_Point
      speed_change;  // Accumulates async_pushes, units like this->speed.
  ...
  IVP_U_Quat q_world_f_core_last_psi;  // Rotation of core using quaternions
                                       // (see m_world_f_core_x_PSI)
  IVP_U_Quat q_world_f_core_next_psi;  // is is the quat to use at a psi

  IVP_U_Matrix
      m_world_f_core_last_psi; /* The core matrix valid at PSI (rotation part is
                                  redundant, see q_world_f_core_last_psi),
...
class IVP_Core : public IVP_Core_Fast {  //
 ...
 public:
  IVP_U_Vector<class IVP_Controller>
      controllers_of_core;  // all controllers our core is controlled of
                            // (friction systems, constraints, force fields ...)
  IVP_Core_Merged
      *merged_core_which_replace_this_core;  // Or NULL when core is terminal.
  IVP_Simulation_Unit *sim_unit_of_core;

  /********************************************************************************
   *	type:	  private: internal
   *	section:  reference matrizes used for movement checking, #+# maybe only
   *for moving objects
   ********************************************************************************/
  IVP_Time time_of_calm_reference[2];
  IVP_U_Float_Quat q_world_f_core_calm_reference[2];  // used to check the
                                                      // movement of an objec
  ...
  union {
    IVP_Old_Sync_Rot_Z *old_sync_info;  // is also used as flag ("is core known
                                        // to impact system")
  } tmp_null;  // warning: tmp_null has to be 0 when not used (some values are
               // flags)

  IVP_Time_CODE
      mindist_event_already_done;  // used at the end of a impact_system cmp to
   ...
   *	section:	constructor
   ********************************************************************************/
  IVP_Core(IVP_Real_Object *object, const IVP_U_Quat *q_world_f_object_init,
  ...
  void
  update_exact_mindist_events_of_core();  // sets mindist_event_already_done and
                                          // recalculates exact mindist events
                                          // to all objects with flag not set
  ...
  void set_fast_piling_allowed(IVP_BOOL bool_flag) {
    fast_piling_allowed_flag = bool_flag;
  }
  IVP_BOOL fast_piling_allowed() { return (IVP_BOOL)fast_piling_allowed_flag; }

  /********************************************************************************
   *	type:		private, public handle with care
   *	section:	initializing basic, non redundant, long time
   *unchangeable section
   ********************************************************************************/
  void set_radius(IVP_FLOAT upper_limit, IVP_FLOAT max_dev);
  void transform_PSI_matrizes_core(
      const IVP_U_Matrix
          *m_core_f_core);  // Multiply matrizes with m_core_f_core
  ...
  inline void clip_velocity(
      IVP_U_Float_Point *velocity,
      IVP_U_Float_Point
          *angular_velocity);  // parameters are input and output !!!

  inline void calc_next_PSI_matrix(
      IVP_U_Vector<IVP_Core> *cores_which_needs_calc_next_psi,
      class IVP_Event_Sim
          *) {  // call commit_all_calc_next_PSI_matrix afterwards
  ...
  void synchronize_with_rot_z();  // synchronize m_world_f_core_last_psi and
                                  // time_of_last_psi with interpolation
  ...
  void stop_physical_movement();    // to avoid unwanted inter/extrapolations on
                                    // calm objects
  void fire_event_object_frozen();  // just send events
  void freeze_simulation_core();
  void stop_movement_without_collision_recheck();
  IVP_BOOL
  revive_simulation_core();  // returns true when datastructures are changed
  void ensure_core_in_simulation_delayed() {
    environment->add_revive_core(this);
  }
  ...
  void create_collision_merged_core_with(
      IVP_Core *other_core);  // create a merged core
  void set_matrizes_and_speed(
      IVP_Core_Merged *template_core,
      IVP_U_Matrix *m_CORE_f_core_out);  // for splitting

  void reset_time(IVP_Time offset);
  void damp_object(IVP_DOUBLE virt_delta_time,
   ...
   *
   *	 test_		In some cases the user does not want to modify the core
   *but wants to get the result in his own speed and rot_speed variables THIS
   *FUNCTIONS HAVE THE test_ PREFIX
   *
   *	 _ws		xxx_ws means function/variable works in world space
   *	 _cs		xxx_cs means function/variable works in core space
   *			No such suffix means functions needs parameters defined
   *in different spaces, check the variable names for details.
   *
   *	 inline_	All inline functions have got the 'inline_' prefix,
   *include ivp_core_macros.hxx to use them.
   *
   *	 _AT		Means: Any time, not restricted to PSI states
   *			(If not otherwise mentioned, functions work correctly at
   *PSI time only)
   *
   *	 rot_push_  	The 'rot_' prefix means that the function exerts a
   *rotation impulse only.
   *
   *	 center_push_	the 'center_' prefix means, core is pushed in mass
   *center, rot_speed is not changed.
   *
   *	_on_test	In all standard situations, the core's speed and
   *rot_speed variables are taken as source. If these values are parameters to
   *the function, the _on_test suffix is appended to the function name
   ********************************************************************************/

  /********************************************************************************
   *	section:	get the ws speed of any point fixed in core space
   ********************************************************************************/
  void get_surface_speed(const IVP_U_Float_Point *position_cs_in,
                         IVP_U_Float_Point *speed_ws_out) const;
  void get_surface_speed_ws(const IVP_U_Point *position_ws_in,
                            IVP_U_Float_Point *speed_ws_out);
  void get_surface_speed_on_test(const IVP_U_Float_Point *position_cs_in,
                                 const IVP_U_Float_Point *center_speed_in,
                                 const IVP_U_Float_Point *rot_speed_in,
                                 IVP_U_Float_Point *speed_ws_out) const;

  /********************************************************************************
   *	Name:		get_diff_surface_speed_of_two_cores
   *	Description:	core_A->get_surface_speed() -
   *core_B->get_surface_speed()
   ********************************************************************************/
  static void get_diff_surface_speed_of_two_cores(
      const IVP_Core *core_A, const IVP_Core *core_B,
      const IVP_U_Float_Point *position_A_cs,
      const IVP_U_Float_Point *position_B_cs,
  ...
  static void get_diff_surface_speed_of_two_cores_on_test(
      const IVP_Core *core_A, const IVP_Core *core_B,
      const IVP_U_Float_Point *position_A_cs,
      const IVP_U_Float_Point *position_B_cs, const IVP_U_Float_Point *speed_A,
      const IVP_U_Float_Point *rot_speed_A, const IVP_U_Float_Point *speed_B,
      const IVP_U_Float_Point *rot_speed_B,
      IVP_U_Float_Point *speed_ws_A_minus_B_out);

  /********************************************************************************
   *	section:	push the core at a given point and given impulse (#+#
   *will be removed soon)
   ********************************************************************************/
  void push_core(const IVP_U_Float_Point *position_cs_in,
                 const IVP_U_Float_Point *impulse_cs_in,
                 const IVP_U_Float_Point *impulse_ws_in);
  // very fast, but needs redundant input values
  void async_push_core(const IVP_U_Float_Point *position_cs_in,
                       const IVP_U_Float_Point *impulse_cs_in,
                       const IVP_U_Float_Point *impulse_ws_in);

  void test_push_core(const IVP_U_Float_Point *position_cs_in,
                      const IVP_U_Float_Point *impulse_cs_in,
                      const IVP_U_Float_Point *impulse_ws_in,
                      IVP_U_Float_Point *speed_change_out,
                      IVP_U_Float_Point *rot_change_out) const;
  // very fast, but needs redundant input values

  /********************************************************************************
   *	Description:	convinient routines of the above functions
   ********************************************************************************/
  void push_core_ws(const IVP_U_Point *position_ws,
                    const IVP_U_Float_Point *impulse_ws);
  void async_push_core_ws(const IVP_U_Point *position_ws,
                          const IVP_U_Float_Point *impulse_ws);

  /********************************************************************************
   *	Description:	apply an impulse at the center of mass
   ********************************************************************************/
  void center_push_core_multiple_ws(const IVP_U_Float_Point *impulse_ws,
                                    IVP_DOUBLE factor = 1.0f);
  void async_center_push_core_multiple_ws(const IVP_U_Float_Point *impulse_ws,
                                          IVP_DOUBLE factor = 1.0f);

  /********************************************************************************
   *	Description:	apply an angular impulse at the center of mass
   ********************************************************************************/
  void rot_push_core_multiple_ws(const IVP_U_Float_Point *angular_impulse_ws,
                                 IVP_DOUBLE factor = 1.0f);
  void async_rot_push_core_multiple_ws(
      const IVP_U_Float_Point *angular_impulse_ws, IVP_DOUBLE factor = 1.0f);

  /*******************************************************************************
   *	Name:	Rot Pushes
   *	Description: Exerts a rotation impulse
   *	Note:	The length of the axis * [factor] is the power of rotation
   *impulse
   ********************************************************************************/
  void rot_push_core_cs(const IVP_U_Float_Point *angular_impulse_cs);
  void rot_push_core_multiple_cs(const IVP_U_Float_Point *angular_impulse_cs,
                                 IVP_DOUBLE factor = 1.0f);
  void async_rot_push_core_multiple_cs(
      const IVP_U_Float_Point *angular_impulse_cs, IVP_DOUBLE factor = 1.0f);
  void test_rot_push_core_multiple_cs(
      const IVP_U_Float_Point *angular_impulse_cs, IVP_DOUBLE factor,
   ...
   ********************************************************************************/
  IVP_DOUBLE get_energy_on_test(const IVP_U_Float_Point *speed_vec,
                                const IVP_U_Float_Point *rot_speed_vec);

  /********************************************************************************
   *	Name:	      	get_rot_inertia_cs
   *	Description:	Get the virtual projected rotation inertia for one cs
   *axis
   ********************************************************************************/
   ...
   *			given a core direction and a world direction (e.g. of an
   *assumed force vector). Works like calc_virt_mass but slower and more
   *accurately.
   ********************************************************************************/
  IVP_DOUBLE calc_correct_virt_mass(
      const IVP_U_Float_Point *position_cs,
   ...
   *			given a core direction (e.g. of an assumed force
   *vector).
   ********************************************************************************/
  IVP_DOUBLE calc_virt_mass(const IVP_U_Float_Point *core_point,
                            const IVP_U_Float_Point *direction)
      const;  // calc the virtual mass of a point
  ...
  /*******************************************************************************************
   *	section:	Calculate core matrix/quaternion/position at a user
   *defined time between last and next PSI
   *******************************************************************************************/
  void calc_at_matrix(IVP_Time current_time, IVP_U_Matrix *m_world_f_core_out)
      const;  // calc a matrix at any time
  inline void inline_calc_at_quaternion(IVP_Time time,
                                        IVP_U_Quat *quat_out) const;
  inline void inline_calc_at_position(IVP_Time time,
   ...
   *	Description:	The mass, rotation inertia, or material of associated
   *object changed. Recalculate some internal variables. Pretty fast. Call at
   *any time.
   ********************************************************************************/
