    ...
    if (!this->valid_instance) {                                           \
      IVP_IF(1) {                                                          \
        IVP_IFDEBUG(IVP_DM_CLUSTERING_SHORTRANGE_VISUALIZER) {             \
          ivp_debugmanager.dprint(IVP_DM_CLUSTERING_SHORTRANGE_VISUALIZER, \
                                  errortext);                              \
        }                                                                  \
      }                                                                    \
      return;                                                              \
...
void IVP_BetterStatisticsmanager_Data_Entity::set_double_value(
    IVP_DOUBLE value) {
...
void IVP_BetterStatisticsmanager_Data_Entity::set_array_size(int array_size) {
  switch (this->type) {
    ...
    case DOUBLE_ARRAY:
      ...
      this->data.double_array.array =
          (IVP_DOUBLE *)p_calloc(array_size + 1, sizeof(IVP_DOUBLE));
...
void IVP_BetterStatisticsmanager_Data_Entity::set_int_array_latest_value(
    int value) {
...
void IVP_BetterStatisticsmanager_Data_Entity::set_double_array_latest_value(
    IVP_DOUBLE value) {
...
IVP_BetterStatisticsmanager_Data_Entity::
    IVP_BetterStatisticsmanager_Data_Entity(
        IVP_BETTERSTATISTICSMANAGER_DATA_ENTITY_TYPE type_in) {
  this->enabled = IVP_TRUE;
  this->type = type_in;
  ...
  switch (this->type) {
    ...
    case DOUBLE_ARRAY:
      ...
      this->data.double_array.array =
          (IVP_DOUBLE *)p_calloc(11, sizeof(IVP_DOUBLE));
  ...
  for (i = 0; i < this->output_callbacks.len(); i++) {
    ...
    for (k = 0; k < this->data_entities.len(); k++) {
      IVP_BetterStatisticsmanager_Data_Entity *entity =
          this->data_entities.element_at(k);
...
void IVP_BetterStatisticsmanager::install_data_entity(
    IVP_BetterStatisticsmanager_Data_Entity *entity) {
  this->data_entities.add(entity);
  return;
}

void IVP_BetterStatisticsmanager::remove_data_entity(
    IVP_BetterStatisticsmanager_Data_Entity *entity) {
...
void IVP_BetterStatisticsmanager::install_output_callback(
    IVP_BetterStatisticsmanager_Callback_Interface *callback) {
  this->output_callbacks.add(callback);
  return;
}

void IVP_BetterStatisticsmanager::remove_output_callback(
    IVP_BetterStatisticsmanager_Callback_Interface *callback) {
...
IVP_BOOL IVP_BetterStatisticsmanager::get_state() { return (this->enabled); }
...
IVP_BetterStatisticsmanager::~IVP_BetterStatisticsmanager() { return; }
