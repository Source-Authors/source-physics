...
extern void p_demo_register_texture_server(P_Hardware *hw,
                                           P_Texture_Server *server);
...
extern void p_demo_update_graphics_for_frozen_object(
...
//
#if !defined(_XBOX) && (!defined(__MWERKS__) || !defined(__POWERPC__)) && \
    !defined(GEKKO)
extern void p_demo_replace_graphical_surface(
    const IVP_Compact_Surface *graphical_surface);
#else  // Xbox or Mac or GameCube
extern void p_demo_replace_graphical_surface(
    IVP_Real_Object *obj, const IVP_Compact_Surface *graphical_surface);
#endif
//
extern void p_graphlib_render_scene_splitscreen(P_Hardware *hw);

//
extern void p_graphlib_init_splitscreen(P_Hardware *hw,
                                        IVP_Environment *environment);

//
extern void p_graphlib_set_camera_matrix_split_screen(IVP_U_Matrix *mat,
                                                      int frame_nr);

//
extern void p_demo_create_ambientlight(IVP_U_Point light_color);
extern void p_demo_create_pointlight(IVP_U_Point light_color,
...
extern int p_mousepointer_raycast(P_Hardware *hardware,
                                  IVP_Environment *environment,
                                  IVP_U_Matrix *camera,
                                  IVP_U_Point *ray_direction,
...
p_demo_get_statisticsmanager_callback(P_Hardware *hardware);
...
// get a active IVP_FLOAT manager which already manages some keyboard driven
// active floats
...
extern int p_graphlib_convert_directx_object_to_concave_data(
    P_Hardware *hw, const char *filename,
    IVP_Concave_Polyhedron *concave_polyhedron);
extern int p_graphlib_robust_convert_directx_object_to_compact_ledges(
    P_Hardware *hw, const char *filename,
...
// basic class used as an interface between the main() function and the example
// codes
class IVP_Example_Base {
 public:
  // garbage collection
  ...
  const char *my_level_name;  // the predifened world to load (see directory
                              // IVP_Sample_Data/LEVELS/
  const char *my_helptext;    // optional help text when hitting SPACE (NULL: no
                              // helptext);
  const char *my_introtext;   // optional intro text before simulation (NULL: no
                              // introtext)
  IVP_EXAMPLE_MENU_CLASS my_class;

  IVP_BOOL import_external_environment;  // should the demo_level environment be
                                         // physicalized
  IVP_Environment
      *environment;  // set only if import_external_environment==TRUE
  P_Hardware *hardware;
  ...
  // main is called by any program that wants to give up controll untill end of
  // example but if you want to have more interactive controll of the example,
  // eg us GLUT which has its own render loop, call setup(), step() multiple
  // times ,and then cleanup()
...
class P_Texture_Server {
 public:
  IVP_BOOL return_absolute_positions;  // set this flag to IVP_TRUE if your
                                       // texture callback will return absolute
                                       // texture positions
  ...
  virtual void texture_callback_detail(
      const IVP_Real_Object * /*real_object_in*/,
      const IVP_U_Point * /*p1_os_in*/, const IVP_U_Point * /*p2_os_in*/,
      const IVP_U_Point * /*p3_os_in*/, const IVP_U_Point * /*normal_os_in*/,
      const char ** /*texture_name_out*/, IVP_FLOAT * /*u1_out*/,
      IVP_FLOAT * /*v1_out*/, IVP_FLOAT * /*u2_out*/, IVP_FLOAT * /*v2_out*/,
      IVP_FLOAT * /*u3_out*/, IVP_FLOAT * /*v3_out*/
  ) {}
  ...
  P_Texture_Server() { this->return_absolute_positions = IVP_FALSE; }
};

enum IVP_EXAMPLE_MOVING_CAMERA_MODE { IVP_EMCM_ROTATION, IVP_EMCM_SPACEFLIGHT };

class Example_Moving_Camera {  //
 private:
  void cameramatrix_workaround();
  void convert_position_and_focus_to_angles_and_distance(
      const IVP_U_Point &center);

  // -----------------------------------------------------------------------------------
  // copy the following class to your class (private!) if you want to set up a
  // callback. then call camera.set_callback_relative(&relative_default); after
 ...
 public:
  ...
  void show_picture(const IVP_U_Point &cameraposition,
                    const IVP_U_Point &direction);
  void show_picture(const IVP_U_Point &cameraposition,
                    const IVP_U_Point &direction, const IVP_U_Point &up);

  // position camera, given camera position and a visible point and optionally a
  // vector where "up" is
  void show_worldpoint(const IVP_U_Point &cameraposition,
                       const IVP_U_Point &worldpoint,
                       const IVP_U_Point *up_direction = NULL);

  // a camera rotating around a given point
  void rotation(const IVP_U_Point &center, IVP_DOUBLE d_time,
                IVP_BOOL recalc_values = IVP_TRUE);
  ...
  void space_flight_original(IVP_DOUBLE d_time,
                             IVP_BOOL recalc_values = IVP_TRUE);
  ...
  Example_Moving_Camera(
      IVP_Environment &env,
      const IVP_U_Point &position);  // init where the camera is.
};
