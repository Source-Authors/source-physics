...
class IVP_Hull_Manager : public IVP_Hull_Manager_Base {
 ...
 public:
  ...
  inline void increase_hull_by_x(IVP_Time t_now, IVP_FLOAT delta_time,
                                 IVP_FLOAT gradient,
                                 IVP_FLOAT center_gradient_);
  ...
  inline void check_for_reset() {
    if (last_vpsi_time.get_seconds() > time_of_next_reset) {
      reset_times();
      time_of_next_reset =
          int(last_vpsi_time.get_seconds() + IVP_MAX_TIME_WITHOUT_RESET);
    }
  }
  ...
  IVP_FLOAT get_angular_hull_time(IVP_Time current_time) {
    return (current_time - last_vpsi_time) * (gradient - center_gradient) +
           (hull_value_last_vpsi - hull_center_value_last_vpsi);
  }
...
IVP_FLOAT IVP_Hull_Manager::insert_synapse(IVP_Listener_Hull *syn,
                                           IVP_Time t_now,
                                           IVP_DOUBLE delta_valid_hull_time) {
  IVP_FLOAT dt = t_now - last_vpsi_time;
  IVP_FLOAT val = hull_value_last_vpsi + gradient * dt;
  syn->minlist_index = sorted_synapses.add(syn, val + delta_valid_hull_time);

  // sorted_synapses.check();
  return (hull_value_last_vpsi - hull_center_value_last_vpsi) +
         (gradient - center_gradient) * dt;
}

IVP_FLOAT IVP_Hull_Manager::insert_lazy_synapse(
...
void IVP_Hull_Manager::update_synapse(IVP_Listener_Hull *syn, IVP_Time t_now,
                                      IVP_DOUBLE delta_valid_hull_time) {
  sorted_synapses.remove_minlist_elem(syn->minlist_index);
...
void IVP_Hull_Manager::update_lazy_synapse(IVP_Listener_Hull *syn,
                                           IVP_Time /*t_now*/,
                                           IVP_DOUBLE delta_valid_hull_time) {
  sorted_synapses.remove_minlist_elem(syn->minlist_index);
  IVP_FLOAT val = hull_value_next_psi;
