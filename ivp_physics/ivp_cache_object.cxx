...
IVP_Cache_Object *IVP_Cache_Object_Manager::get_cache_object(
...
void IVP_Cache_Object_Manager::check() {
  ...
  for (int i = 0; i < n_cache_objects; i++) {
    ...
    IVP_IF(co->object) { IVP_ASSERT(co->object->cache_object == co); }
  }
}
#endif
...
void IVP_Cache_Object::update_cache_object() {
  ...
  if (current_time - core->time_of_last_psi == 0.0f) {
    ...
    object->get_core()->inline_calc_at_quaternion(current_time,
                                                  &q_world_f_object);
    object->get_core()->inline_calc_at_position(current_time,
                                                &m_world_f_object.vv);
  ...
  if (object->q_core_f_object) {
    q_world_f_object.inline_set_mult_quat(&q_world_f_object,
                                          object->q_core_f_object);
    q_world_f_object.set_matrix(&m_world_f_object);
  }
}

void IVP_Cache_Object::transform_position_to_object_coords(
    const IVP_U_Point *P_world, IVP_U_Point *P_object_out) const {
...
void IVP_Cache_Object::transform_position_to_world_coords(
    const IVP_U_Float_Point *P_object, IVP_U_Point *P_world_out) const {
  this->m_world_f_object.inline_vmult4(P_object, P_world_out);
}

void IVP_Cache_Object::transform_vector_to_object_coords(
    const IVP_U_Point *P_world, IVP_U_Point *P_object_out) const {
  this->m_world_f_object.inline_vimult3(P_world, P_object_out);
}

void IVP_Cache_Object::transform_vector_to_object_coords(
    const IVP_U_Float_Point *P_world, IVP_U_Float_Point *P_object_out) const {
  this->m_world_f_object.inline_vimult3(P_world, P_object_out);
}

void IVP_Cache_Object::transform_vector_to_world_coords(
    const IVP_U_Point *P_object, IVP_U_Point *P_world_out) const {
  this->m_world_f_object.inline_vmult3(P_object, P_world_out);
}

void IVP_Cache_Object::transform_vector_to_world_coords(
    const IVP_U_Float_Point *P_object, IVP_U_Float_Point *P_world_out) const {
  this->m_world_f_object.inline_vmult3(P_object, P_world_out);
}
