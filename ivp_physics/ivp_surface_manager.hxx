 ...
 ********************************************************************************/
enum IVP_SURMAN_TYPE { IVP_SURMAN_POLYGON, IVP_SURMAN_BALL };
...
class IVP_Vector_of_Ledges_16 : public IVP_U_BigVector<IVP_Compact_Ledge> {
 ...
 public:
  IVP_Vector_of_Ledges_16()
      : IVP_U_BigVector<IVP_Compact_Ledge>((void **)&elem_buffer[0], 16) {
...
class IVP_Vector_of_Ledges_256 : public IVP_U_BigVector<IVP_Compact_Ledge> {
 ...
 public:
  IVP_Vector_of_Ledges_256()
      : IVP_U_BigVector<IVP_Compact_Ledge>((void **)&elem_buffer[0], 256) {
    memset(elem_buffer, 0, sizeof(elem_buffer));
  }
};
 ...
 *	Note:		Different object types require different
 *IVP_SurfaceManager implementations. See:
 *IVP_Environment::create_object IVP_SurfaceBuilder_Ledge_Soup
 ********************************************************************************/
class IVP_SurfaceManager {
 ...
 public:
   ...
   *  Description:	Allows the user to set and get a ledge specific
   *client_data Note:		Range of values
   *[0..IVP_SURMAN_POLYGON_MAX_VALUE_CLIENTDATA[ Attention:      Only works for
   *terminal ledges
   *****************************************************************************/
  static void set_ledge_specific_client_data(
      IVP_Compact_Ledge *,
      unsigned int
          value);  // assert value < IVP_SURMAN_POLYGON_MAX_VALUE_CLIENTDATA
  static unsigned int get_ledge_specific_client_data(const IVP_Compact_Ledge *);

  /********************************************************************************
   *	Name:	     	get_single_convex
   *	Description:	returns the only compact ledge if surface_manager is a
   *single convex polyhedron or a ball else NULL
   ********************************************************************************/
  virtual const IVP_Compact_Ledge *get_single_convex() const = 0;

  /********************************************************************************
   *	Name:	     	get_radius_and_radius_dev_to_given_center
   *	Description:	gets the radius and the max( length( surface_normal
   *cross_product surface_point )) if radius_deviation can not be calculated,
   *set it to radius
   ********************************************************************************/
  ...
  virtual void get_radius_and_radius_dev_to_given_center(
      ...
      IVP_FLOAT *radius_deviation) const = 0;
  virtual void get_rotation_inertia(
      IVP_U_Float_Point *rotation_inertia_out) const = 0;

  /********************************************************************************
   *	Name:	     	get_all_ledges_within_radius
   *	Description:	the main function which a surface_manager manager has to
   *implement!!!: returns all ledges which intrude into a sphere around the
   *observer's position Note:		Polygons only
   ********************************************************************************/
  virtual void get_all_ledges_within_radius(
      const IVP_U_Point *observer_position_object, IVP_DOUBLE radius,
   ...
   *                  Fills all ledges which are hit by the specified ray into
   *the ray solver Note:		For polygons only
   ********************************************************************************/
   ...
   *	Description:	Tells the surface manager that a synapse will use (stop
   *use) a ledge Note:		Polygons only
   ********************************************************************************/
