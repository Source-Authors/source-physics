...
void IVP_Core::inline_calc_at_quaternion(IVP_Time current_time,
                                         IVP_U_Quat *quat) const {
  IVP_DOUBLE d_time = current_time - time_of_last_psi;
  quat->set_interpolate_smoothly(&q_world_f_core_last_psi,
                                 &q_world_f_core_next_psi,
                                 d_time * i_delta_time);
}

void IVP_Core::inline_calc_at_position(IVP_Time current_time,
                                       IVP_U_Point *pos) const {
  IVP_DOUBLE d_time = current_time - time_of_last_psi;
  pos->add_multiple(&pos_world_f_core_last_psi, &delta_world_f_core_psis,
                    d_time);
}

void IVP_Core::clip_velocity(IVP_U_Float_Point *velocity,
  ...
  if (this->spin_clipping) {
...
#endif
    {
      ...
      IVP_DOUBLE max_rot_speed = al->get_max_angular_velocity_per_psi() *
                                 this->environment->get_inv_delta_PSI_time();
