...
IVP_BOOL IVP_BetterDebugmanager::is_debug_enabled(
    IVP_DEBUG_CLASS class_id) const {
  if (class_id >= IVP_DEBUG_MAX_N_CLASSES) {
    return (IVP_FALSE);
  }
  if (class_id < IVP_DM_DUMMY || this->flag_list[class_id] == 0 ||
      !this->initialized) {
    return (IVP_FALSE);
  }
  return (IVP_TRUE);
...
void IVP_BetterDebugmanager::dprint(IVP_DEBUG_CLASS class_id,
...
void IVP_BetterDebugmanager::output_function(IVP_DEBUG_CLASS,
                                             const char *string) {
...
IVP_BetterDebugmanager::~IVP_BetterDebugmanager() { return; }
