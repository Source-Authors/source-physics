...
class IVP_Solver_Core_Reaction {
 private:
  void init_trans_ws(const IVP_U_Point *pos_ws, IVP_Core *core,
                     IVP_U_Point_4 cross_direction_position_cs[3],
                     IVP_U_Point_4 cr_mult_inv[3], IVP_FLOAT sign);
  void init_rot_ws(IVP_Core *core, IVP_U_Point_4 cross_direction_position_cs[3],
                   IVP_U_Point_4 cr_mult_inv[3], IVP_FLOAT sign);
 ...
 public:
  ...
  IVP_U_Float_Point
      delta_velocity_ds;  // based on current rot_speeds only speeds

  /* init with normized axis, if direction_X_ws == 0 than not used, if core == 0
   * than static */
  ...
  void init_reaction_solver_rotation_ws(
      ...
      IVP_U_Float_Point
          *direction_0_ws,  // the directions define the r_space (_ds)
      IVP_U_Float_Point *direction_1_ws,
      IVP_U_Float_Point *direction_2_ws);  // if <3, m_velocity_ds_f_impulse_ds
                                           // is not filled completely
  // invertion
  IVP_RETURN_TYPE
  invert_3x3_matrix();  // real inverts the m_velocity_ds_f_impulse_ds matrix
                        // (result is full 3x3
  ...
  void exert_impulse_dim1(IVP_Core *core_0, IVP_Core *core_1,
                          IVP_U_Float_Point &impulse_ds);  // 1 dimension only
  void exert_impulse_dim2(IVP_Core *core_0, IVP_Core *core_1,
                          IVP_U_Float_Point &impulse_ds);  // 2 dimensions
  void exert_impulse_dim3(IVP_Core *core_0, IVP_Core *core_1,
                          IVP_U_Float_Point &impulse_ds);  // 3 dimensions

  void exert_angular_impulse_dim1(
      IVP_Core *core_0, IVP_Core *core_1,
      IVP_U_Float_Point &impulse_rs);  // 1 dimensions
  void exert_angular_impulse_dim2(
      IVP_Core *core_0, IVP_Core *core_1,
      IVP_U_Float_Point &impulse_rs);  // 2 dimensions
  void exert_angular_impulse_dim3(
      IVP_Core *core_0, IVP_Core *core_1,
      IVP_U_Float_Point &impulse_rs);  // 3 dimensions
