...
class IVP_Template_Phantom {
 public:
   ...
   *	Default:	IVP_FALSE
   *	Description:    if set to IVP_TRUE, phantom manages a set of intruding
   *objects, see IVP_Controller_Phantom::get_intruding_objects()
   *********************************************************************/
  IVP_BOOL
  manage_intruding_objects;  // default IVP_FALSE, if set to IVP_TRUE,
                             // phantom manages a set of intruding objects

  /*********************************************************************
   *	Name:	    	manage_intruding_cores
   *	Default:	IVP_FALSE
   *	Description:    if set to IVP_TRUE, phantom manages a set of intruding
   *cores of objects, see IVP_Controller_Phantom::get_intruding_cores()
   *********************************************************************/
  IVP_BOOL
  manage_intruding_cores;  // default IVP_FALSE, if set to IVP_TRUE, phantom
                           // manages a set of intruding objects
   ...
   *			the callback object_left_volume() is only called if the
   *object leaves the phantom plus this extra radius. Note:           Set this
   *value to higher values to speed up the collision detection engine
   *********************************************************************/
   ...
   *	Default:	0.5f[sec]
   *	Description:    This is the time interval used by the collision
   *detection engine to check for intruded objects to leave the phantom volume
   *    Note:           Set this value to higher values to speed up the
   *collision detection engine Version Info:   Not implemented yet
   *********************************************************************/
...
class IVP_Listener_Phantom {
 public:
  virtual void mindist_entered_volume(class IVP_Controller_Phantom *controller,
                                      class IVP_Mindist_Base *mindist) = 0;
  virtual void mindist_left_volume(class IVP_Controller_Phantom *controller,
                                   class IVP_Mindist_Base *mindist) = 0;
  virtual void core_entered_volume(class IVP_Controller_Phantom *controller,
                                   class IVP_Core *pCore) = 0;
  virtual void core_left_volume(class IVP_Controller_Phantom *controller,
                                class IVP_Core *pCore) = 0;
  virtual void phantom_is_going_to_be_deleted_event(
      class IVP_Controller_Phantom
          *controller) = 0;  // indicates either object is deleted or phantom is
                             // converted to object
...
class IVP_Controller_Phantom : public IVP_Listener_Object {
 ...
 protected:
  ...
  IVP_Controller_Phantom(
      IVP_Real_Object *object,
      const IVP_Template_Phantom
          *templat);  // use IVP_Real_Object::convert_to_phantom to create
                      // phantoms
 ...
 public:
  ...
  }  // returns NULL if manage_set was IVP_FALSE
  ...
  void add_sleeping_object(IVP_Real_Object *);
  void remove_sleeping_object(IVP_Real_Object *);
  void wake_all_sleeping_objects();
  void fire_event_core_entered(IVP_Core *);
  void fire_event_core_left(IVP_Core *);

  ~IVP_Controller_Phantom();  // Note: Has side effect, switches phantom object
                              // to real object

  virtual void event_object_deleted(IVP_Event_Object *obj) {
    event_object_revived(obj);
  }
  virtual void event_object_created(IVP_Event_Object *obj) {}
  virtual void event_object_revived(IVP_Event_Object *obj);
  virtual void event_object_frozen(IVP_Event_Object *obj) {}
