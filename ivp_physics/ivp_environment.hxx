...
class IVP_Statistic_Manager {
 public:
  ...
  int impact_delayed_counter;  // number of impacts, where one object gets a
  ...
  int range_intra_exceeded;  // number of events: sphere for checking two
                             // objects recalculated
  int range_world_exceeded;  // number of events: sphere for checking object -
                             // world recalculated
...
class IVP_Environment {  // the environment
  ...
  IVP_DOUBLE delta_PSI_time;  // delta time between two PSIs ( default 1/66
                              // seconds. range: IVP_MAX_DELTA_PSI_TIME -
                              // IVP_MIN_DELTA_PSI_TIME seconds )
  ...
  IVP_Environment(IVP_Environment_Manager *manager,
 ...
 public:
  ...
  void add_draw_vector(const IVP_U_Point *start_ws,
                       const IVP_U_Float_Point *vec, const char *debug_text,
                       int color);
  ...
  IVP_Material_Manager *get_material_manager() const {
    return l_material_manager;
  }
  IVP_Collision_Filter *get_collision_filter() const {
  ...
  IVP_Universe_Manager *get_universe_manager() const {
  ...
  IVP_Real_Object *get_static_object() const {
    return static_object;
  }  // a env. global static ball
  IVP_Freeze_Manager *get_freeze_manager() { return &freeze_manager; }
  //////////// time management
  IVP_Time get_current_time() { return current_time; }
  void set_delta_PSI_time(
      IVP_DOUBLE new_delta_PSI_time);  // range: IVP_MIN_DELTA_PSI_TIME -
                                       // IVP_MAX_DELTA_PSI_TIME

  static void set_global_collision_tolerance(
      IVP_DOUBLE tolerance = 0.01f,
      IVP_DOUBLE gravity_length =
          0.01f);  // set the collision tolerance (try to go for higher values
                   // if possible)
  static IVP_FLOAT get_global_collision_tolerance();
  ////////// world creation

  // this is preleminary: allows to define breakable objects.
  // to create fixed complex objects, see the IVP_SurfaceBuilder_Ledge_Soup
  // classes Note: merge objects before collision detection is enabled and
  // objects are revived !!!
   ...
   *			Objects are not simulated when created. Simulation is
   *started when objects collide or IVP_Real_Object::ensure_in_simulation() is
   *called
   ********************************************************************************/
  IVP_Polygon *create_polygon(
      IVP_SurfaceManager *surface_manager,  // a surface_manager manager
      const IVP_Template_Real_Object
          *templ_obj,  // used to set mass center and rotation inertia directly
                       // (no automatism)
      const IVP_U_Quat *quat_world_f_object,  // the rotation of the object
      const IVP_U_Point *position);           // the position

  IVP_Ball *create_ball(
      const IVP_Template_Ball *templ_ball,  // the radius
      const IVP_Template_Real_Object
          *templ_obj,  // used to set mass center and rotation inertia directly
                       // (no automatism)
      const IVP_U_Quat *quat_world_f_object,  // the rotation of the object
  ...
  IVP_Actuator_Spring *create_spring(
      IVP_Template_Spring *templ);  // creates a damped spring, Note
                                    // IVP_Controller_Stiff_Spring are better
  IVP_Actuator_Suspension *create_suspension(
      IVP_Template_Suspension *templ);  // creates a special vehicle spring

  IVP_Actuator_Force *create_force(
      IVP_Template_Force *templ);  // creates a force actuator
  IVP_Actuator_Torque *create_torque(
      IVP_Template_Torque *templ);  // creates a torque actuator
  IVP_Actuator_Rot_Mot *create_rotmot(
      IVP_Template_Rot_Mot *templ);  // creates a motor

  IVP_Actuator_Stabilizer *create_stabilizer(
      IVP_Template_Stabilizer *templ);  // used for cars
  IVP_Actuator_Check_Dist *create_check_dist(
      IVP_Template_Check_Dist *templ);  // creates a distance checker

  ////////// create constraints
  // for mor info on constraints look at ivp_template_constraint.hxx
  IVP_Constraint *create_constraint(
      const IVP_Template_Constraint *template_constraint);

  IVP_Controller_Motion *create_controller_motion(
      IVP_Real_Object *, const class IVP_Template_Controller_Motion *);

  const IVP_U_Point *get_gravity() {
    return &gravity;
  }  // current gravity vector, default (0,9.83f,0);
  void set_gravity(IVP_U_Point *gravity);  // sets gravity vector (can be
                                           // changed at any time)
  class IVP_Standard_Gravity_Controller *get_gravity_controller(void) {
    return standard_gravity_controller;
  }

  /////////// global listening (private listening: see IVP_Real_Object)
  void add_listener_object_global(
      IVP_Listener_Object
          *);  // adds a callback which listens to events of all objects
  void install_listener_object_global(
      IVP_Listener_Object *);  // adds a callback which listens to events of all
                               // objects if its not in the list already
  void remove_listener_object_global(IVP_Listener_Object *);

  void add_listener_object_private(
      IVP_Real_Object *,
      IVP_Listener_Object
          *);  // adds a callback which listens to object events of one object
  void remove_listener_object_private(
      IVP_Real_Object *,
      IVP_Listener_Object
          *);  // (same as IVP_Real_Object::add_listener_object())

  void add_listener_collision_global(
      IVP_Listener_Collision
          *);  // adds a callback which listens to all collisions
  void remove_listener_collision_global(IVP_Listener_Collision *);

  void add_listener_collision_private(
      IVP_Real_Object *,
      IVP_Listener_Collision
          *);  // adds a callback which listens to just one object
  void remove_listener_collision_private(
      IVP_Real_Object *,
      IVP_Listener_Collision
          *);  // (note: same as IVP_Real_Object::add_listener_collision())

  void add_listener_PSI(IVP_Listener_PSI *);  // adds a callback called at the
                                              // beginning of every PSI
  ...
  /////////// simulation
  void simulate_until(
      IVP_Time time);  // the MAIN function: simulates the entire physical
                       // environment until 'time'
  void simulate_dtime(
      IVP_DOUBLE deta_time);  // the MAIN function: simulates the entire
                              // physical for a given time
  void simulate_time_step(
      IVP_FLOAT sub_psi_pos =
          1.99f);  // simulate exactly one time step (for synchronous use of the
                   // physics engine (e.g. playstation 2).

  void simulate_variable_time_step(
      IVP_FLOAT
          delta_time);  // simulate exactly one time step with a variable length
  ...
  int environment_magic_number;  // to detect if environment is valid (debug
                                 // reasons)
};
...
class IVP_Application_Environment {
 public:
  int n_cache_object;  // == 2**x number of caches for moving objects, should be
                       // larger than typical number of moving objects

  char *scratchpad_addr;  // scratch pad: a fast area which is temporarily used
                          // and is already cached in
  int scratchpad_size;

  /********************************************************************************
   *	Name:	     	Several custom managers
   *	Description:	To override basic functionality of the Ipion Physics
   *Engine you may replace some of it's internal managers by your own version
   ********************************************************************************/
  IVP_Material_Manager *material_manager;  // a manager which serves friction
...
class IVP_Vector_of_Hulls_128 : public IVP_U_Vector<IVP_Hull_Manager_Base> {
 ...
 public:
  IVP_Vector_of_Hulls_128()
      : IVP_U_Vector<IVP_Hull_Manager_Base>((void **)&elem_buffer[0], 128) {
