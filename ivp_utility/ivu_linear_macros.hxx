...
inline void IVP_U_Float_Point::subtract(
    const IVP_U_Float_Point *v2) {  // vector subtraction
  this->subtract(this, v2);
}

inline void IVP_U_Float_Point::add(
    const IVP_U_Float_Point *v2) {  // vector subtraction
  this->add(this, v2);
}

inline void IVP_U_Float_Point::add_multiple(const IVP_U_Float_Point *v2,
                                            IVP_DOUBLE f) {  // vector
  this->add_multiple(this, v2, f);
}

inline void IVP_U_Float_Point::set_multiple(const IVP_U_Quat *q_source,
                                            IVP_DOUBLE f) {
  // dimhotepus: Fix UB on cast IVP_U_Quat -> IVP_U_Float_Point.
  const IVP_U_Float_Point p{q_source->x, q_source->y, q_source->z};
...
// an acos approximation whose absolute value is always less than
// abs(asin(angle)) it differs not more than 0.0005 when abs(angle) < 0.5f and
// it differs with 0.2 when abs(angle) == 1.0f
inline IVP_FLOAT IVP_Inline_Math::fast_approx_asin(IVP_FLOAT angle) {
  ...
  return angle + (1.0f / 6.0f) * triple_angle +
         (3.0f / 40.0f) * quint_angle;  // see mathematics book
}
...
inline IVP_FLOAT IVP_Inline_Math::approx5_cos(IVP_FLOAT angle) {
  IVP_FLOAT quad_angle = angle * angle;
  return 1.0f - (1.0f / 2.0f) * quad_angle +
         (1.0f / 24.0f) * quad_angle * quad_angle;
}
...
inline IVP_FLOAT IVP_Inline_Math::upper_limit_asin(IVP_FLOAT angle) {
  ...
  IVP_FLOAT quint_angle = triple_angle * quad_angle;
  return angle + (1.0f / 6.0f) * triple_angle +
         0.40414f * quint_angle;  // see mathematics book
}

// an acos approximation that differs not more than 0.0016 when abs(angle) <
// 0.57 and that differs not more than 0.0521 in the whole range
...
inline void IVP_U_Matrix3::get_row(IVP_COORDINATE_INDEX row,
                                   IVP_U_Point *row_out) const {
...
inline void IVP_U_Matrix3::get_col(IVP_COORDINATE_INDEX col,
                                   IVP_U_Point *col_out) const {
...
inline void IVP_U_Matrix3::set_row(IVP_COORDINATE_INDEX row,
                                   const IVP_U_Point *row_in) {
  rows[static_cast<int>(row)].set(row_in);
}

inline void IVP_U_Matrix3::set_row(IVP_COORDINATE_INDEX row,
                                   const IVP_U_Float_Point *row_in) {
  rows[static_cast<int>(row)].set(row_in);
}

inline void IVP_U_Matrix3::set_col(IVP_COORDINATE_INDEX col,
                                   const IVP_U_Point *col_in) {
...
inline void IVP_U_Matrix3::set_col(IVP_COORDINATE_INDEX col,
                                   const IVP_U_Float_Point *col_in) {
  set_elem(0, static_cast<int>(col), col_in->k[0]);
  set_elem(1, static_cast<int>(col), col_in->k[1]);
  set_elem(2, static_cast<int>(col), col_in->k[2]);
}

inline IVP_U_Point::IVP_U_Point(const IVP_U_Float_Point &p) { this->set(&p); };
