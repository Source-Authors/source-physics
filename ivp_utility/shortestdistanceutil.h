...
class ShortestDistanceUtil {
 public:
   ...
   */
  static inline void PointToEdge(const IVP_U_Point* p, const IVP_U_Point* s1,
                                 const IVP_U_Point* s2, IVP_U_Point* C) {
    IVP_U_Point l(s1), d;
    l.subtract(p);
    d.subtract(s2, s1);
   ...
   */
  static inline void PointToTriangle(const IVP_U_Point* p,
                                     const IVP_U_Point* t1,
                                     const IVP_U_Point* t2,
                                     const IVP_U_Point* t3, IVP_U_Point* C) {
    // calculate edges
    ...
    for (int i = 0; i < 1; i++) {  // ray triangle intersection
      // ray direction
      IVP_U_Point dir;
      dir.inline_calc_cross_product_and_normize(&edge2, &edge1);

      IVP_U_Point h;
      h.inline_calc_cross_product(&dir, &edge2);
      ...
      IVP_U_Point s;
      s.add_multiple(p, t1, -1.0f);
      IVP_DOUBLE u = inv_det * s.dot_product(&h);

      IVP_U_Point q;
      q.inline_calc_cross_product(&s, &edge1);
    ...
    {  // contact point is triangle edge, deal with that accordingly
      ...
      IVP_U_Point dv1;
      dv1.add_multiple(C, &cp1, -1.0f);
      IVP_U_Point dv2;
      dv2.add_multiple(C, &cp2, -1.0f);
      IVP_U_Point dv3;
      dv3.add_multiple(C, &cp3, -1.0f);
