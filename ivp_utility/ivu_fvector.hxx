 ...
 *			operation,
 *			The vector elements can only be inserted into N
 *(currently set to the fixed value of 2 FVectors and needs to implement int
 *get_fvector_index(int pos) (where pos = 0,1) and void set_fvector_index(int
...
class IVP_U_FVector : public IVP_U_Vector_Base {
 ...
 public:
  ...
  int index_of(T *elem) {
    ...
    IVP_ASSERT(elem->get_fvector_index(1) < 0 ||
               elem->get_fvector_index(1) >= n_elems ||
               element_at(elem->get_fvector_index(1)) == elem);
