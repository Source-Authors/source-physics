...
void *IVP_VHash::remove_elem(const void *elem, unsigned int hash_index) {
  ...
  for (pos = (pos + 1) & size_mm;; pos = (pos + 1) & size_mm) {
    ...
    if (pos > last_pos) {  // not wrapped situation
      if (ideal_pos <
          ideal_pos_of_last_element) {  // shift wrapped elements also
        if (ideal_pos > last_pos)
          continue;  // no way to shift elements left to their ideal position
      } else if (ideal_pos ==
                 ideal_pos_of_last_element) {  // further checks needed: are we
                                               // at the start or at the end of
                                               // the hash ??
        if (ideal_pos <= pos) {  // element is not wrapped -> we are at the end
                                 // of the hash table
          if (ideal_pos > last_pos)
            continue;  // no way to shift elements left to their ideal position
 ...
 *                works only with pointers as key elements and pointers as
 *additional associated elements
 *******************************************************************************************************************/
...
IVP_VHash_Store::IVP_VHash_Store(int size_i) {
  ...
  dont_free = 0;
  elems_store =
      (IVP_VHash_Store_Elem *)p_calloc(size_i, sizeof(IVP_VHash_Store_Elem));
}
...
void IVP_VHash_Store::rehash(int new_size) {
  ...
  size_mm = new_size - 1;
  elems_store =
      (IVP_VHash_Store_Elem *)p_calloc(new_size, sizeof(IVP_VHash_Store_Elem));
