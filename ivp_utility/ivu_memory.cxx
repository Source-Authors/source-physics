...
void *ivp_malloc_aligned(size_t size, unsigned short alignment) {
  ...
  if (data) {
    ...
    void *ret = (void *)((((hk_uintp)data) + alignment +
                          sizeof(IVP_Aligned_Memory) - 1) &
                         (-static_cast<hk_intp>(alignment)));
...
void ivp_free_aligned(void *data) {
  ...
  IVP_IF(1) { am->magic_number = 0; }
  p_free(am);
#endif
}

IVP_U_Memory::~IVP_U_Memory() { free_mem(); }
...
void IVP_U_Memory::free_mem_transaction() {
  ...
  speicherbeginn =
      static_cast<char *>(align_to_next_adress(&first_elem->data[0]));

  size_t ng =
      size_of_external_mem ? size_of_external_mem : IVU_MEMORY_BLOCK_SIZE;
...
IVP_U_Memory::IVP_U_Memory() { init_mem(); }

void IVP_U_Memory::init_mem() {
  transaction_in_use =
      3;  // for transactions: first start init_mem_transaction_usage
  size_of_external_mem = 0;
#if defined(MEMTEST)
#else
  speicherbeginn = 0;
