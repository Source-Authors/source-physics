...
inline void IVP_U_Float_Point::set_pairwise_mult(
    const IVP_U_Float_Point *v1,
    const IVP_U_Float_Point *v2) {  // pairwise multiple
  IVP_DOUBLE a, b, c;
  a = v1->k[0] * v2->k[0];
  b = v1->k[1] * v2->k[1];
  c = v1->k[2] * v2->k[2];
...
inline void IVP_U_Float_Point::add(const IVP_U_Float_Point *v1,
...
inline void IVP_U_Float_Point::subtract(const IVP_U_Float_Point *v1,
                                        const IVP_U_Float_Point *v2) {
  IVP_DOUBLE a, b, c;
  a = v1->k[0] - v2->k[0];
  b = v1->k[1] - v2->k[1];
...
inline void IVP_U_Float_Point::subtract(const IVP_U_Float_Point *v1,const IVP_U_Float_Point *v2){
  asm __volatile__("
                   lqc2 vf4,
                   0x0(% 1) lqc2 vf5, 0x0(% 2) vsub.xyz vf6, vf4, vf5 sqc2 vf6,
                   0x0(% 0) "
                   : /*no output*/
                   : "r"(this), "r"(v1), "r"(v2)
                   : "memory");
...
inline IVP_DOUBLE IVP_U_Float_Point::quad_distance_to(
    const IVP_U_Float_Point *p) const {
#if 1
  ...
  asm __volatile__("
                   lqc2 vf4,
                   0x0(% 1) lqc2 vf5, 0x0(% 2) vsub.xyz vf5, vf4,
                   vf4 vmul.xyz vf5, vf5, vf5 vaddy.x vf5, vf5, vf5 vaddz.x vf5,
                   vf5, vf5 qmfc2 $2, vf5 mtc1 $2, % 0 "
                   : "=r"(ret)
                   : "r"(this), "r"(p)
                   : "$2", "memory");
  return ret;
...
inline void IVP_U_Float_Point::add_multiple(
    const IVP_U_Float_Point *v1, const IVP_U_Float_Point *v2,
    IVP_DOUBLE factor2) {  // vektor addition
#if 0
    IVP_DOUBLE a,b,c;
  ...
  asm __volatile__("
                   lqc2 vf4,
                   0x0(% 2) mfc1 $8, % 3 qmtc2 $8, vf5 vmulx.xyz vf6, vf4,
                   vf5

                       lqc2 vf4,
                   0x0(% 1) vadd.xyz vf6, vf4, vf6 sqc2 vf6, 0x0(% 0) "
                   : /* no output */
...
inline IVP_DOUBLE IVP_U_Float_Point::dot_product(
    const IVP_U_Float_Point *v2) const {
#if 1
...
inline void IVP_U_Float_Point::set_multiple(const IVP_U_Float_Point *v,
                                            IVP_DOUBLE f) {
#if 0
  ...
  asm __volatile__("
                   lqc2 vf4,
                   0x0(% 1) mfc1 $8, % 2 qmtc2 $8, vf5 vmulx.xyz vf6, vf4,
                   vf5 sqc2 vf6, 0x0(% 0) "
                   : /* no output */
                   : "r"(this), "r"(v), "f"(f)
                   : "$8", "memory");
...
inline void IVP_U_Float_Point::inline_subtract_and_mult(
    const IVP_U_Float_Point *v1, const IVP_U_Float_Point *v2,
    IVP_DOUBLE factor) {  // vektor addition
#if 0
    IVP_DOUBLE a,b,c;
    a = (v1->k[0] - v2->k[0]);
  ...
  asm __volatile__("
                   lqc2 vf4,
                   0x0(% 1) lqc2 vf6, 0x0(% 2) vsub.xyz vf4, vf4, vf6 mfc1 $8,
                   % 3 qmtc2 $8, vf5 vmulx.xyz vf6, vf4, vf5 sqc2 vf6,
                   0x0(% 0) "
                   : /* no output */
...
inline IVP_DOUBLE IVP_U_Float_Hesse::get_dist(
    const IVP_U_Float_Point *p) const {
  return this->dot_product(p) + hesse_val;
}

void IVP_U_Matrix3::inline_vimult3(const IVP_U_Float_Point *p_in,
  ...
  asm __volatile__("
                   lqc2 vf8,
                   0x0(% 2) lqc2 vf4, 0x0(% 1) vmulax.xyz ACC, vf4,
                   vf8 lqc2 vf5, 0x10(% 1) vmadday.xyz ACC, vf5, vf8 lqc2 vf6,
                   0x20(% 1) vmaddz.xyz vf12, vf6, vf8 sqc2 vf12, 0x0(% 0) "
                   : /* no output */
...
inline void IVP_U_Matrix::inline_vimult4(const IVP_U_Float_Point *p_in,
                                         IVP_U_Float_Point *p_out) const {
#if 0
  ...
  asm __volatile__("
                   lqc2 vf7,
                   0x30(% 1) lqc2 vf8, 0x0(% 2) vsub.xyz vf8, vf8, vf7 lqc2 vf4,
                   0x0(% 1) lqc2 vf5, 0x10(% 1) vmulax.xyz ACC, vf4,
                   vf8 lqc2 vf6, 0x20(% 1) vmadday.xyz ACC, vf5,
                   vf8 vmaddz.xyz vf12, vf6, vf8 sqc2 vf12, 0x0(% 0) "
                   : /*no output*/
                   : "r"(p_out), "r"(this), "r"(p_in)
                   : "memory");
#endif
...
void IVP_U_Matrix3::inline_vmult3(const IVP_U_Float_Point *p_in,
                                  IVP_U_Float_Point *p_out) const {
#if 0
  ...
  asm __volatile__("
                   ...
                   0x20(% 1)
                       ...
                       vmul.xyz vf4,
                   vf4, vf8 vmul.xyz vf5, vf5, vf8 vmul.xyz vf6, vf6,
                   vf8

                       vaddy.x vf4,
                   vf4, vf4 vaddx.y vf5, vf5, vf5 vaddy.z vf6, vf6,
                   vf6

                       vaddz.x vf12,
                   vf4, vf4 vaddz.y vf12, vf5, vf5 vaddx.z vf12, vf6,
                   vf6
...
inline void IVP_U_Matrix::inline_vmult4(const IVP_U_Float_Point *p_in,
                                        IVP_U_Float_Point *p_out) const {
#if 0
...
void IVP_U_Float_Point::inline_calc_cross_product(const IVP_U_Float_Point *v1,
                                                  const IVP_U_Float_Point *v2) {
#if 0
  ...
  asm __volatile__("
                   lqc2 vf4,
                   0x0(% 1) lqc2 vf5, 0x0(% 2) vopmula.xyz ACC, vf4,
                   vf5 vopmsub.xyz vf6, vf5, vf4 sqc2 vf6, 0x0(% 0) "
                   : /*no output */
                   : "r"(this), "r"(v1), "r"(v2)
                   : "memory");
#endif
...
void IVP_U_Float_Point::inline_calc_cross_product_and_normize(
    const IVP_U_Float_Point *v1, const IVP_U_Float_Point *v2) {
#if 0
    IVP_DOUBLE a  = v1->k[1] * v2->k[2];
    IVP_DOUBLE a2 = v2->k[1] * v1->k[2];
  ...
  asm __volatile__("
                   lqc2 vf4,
                   0x0(% 1) lqc2 vf5, 0x0(% 2) vopmula.xyz ACC, vf4,
                   vf5 vopmsub.xyz vf6, vf5, vf4 vmul.xyz vf5, vf6,
                   vf6 vaddy.x vf5, vf5, vf5 vaddz.x vf5, vf5,
                   vf5

                       vrsqrt Q,
                   vf0w, vf5x vwaitq vmulq.xyz vf7, vf6,
                   Q
...
void IVP_U_Float_Point::inline_set_vert_to_area_defined_by_three_points(
  ...
  asm __volatile__("
                   lqc2 vf3,
                   0x0(% 1) lqc2 vf5, 0x0(% 2) lqc2 vf4, 0x0(% 3) vsub.xyz vf4,
                   vf4, vf3 vsub.xyz vf5, vf5, vf3 vopmula.xyz ACC, vf4,
                   vf5 vopmsub.xyz vf6, vf5, vf4 sqc2 vf6, 0x0(% 0) "
                   : /*no output */
...
inline IVP_DOUBLE IVP_U_Quat::acos_quat(
    const IVP_U_Quat *q) const {  // acosinus between two quats
...
inline void IVP_U_Quat::inline_set_mult_quat(const IVP_U_Quat *q1,
  ...
  asm __volatile__("
                   ...
                   vmul.xyzw vf12,
                   vf4, vf5 #vf12 x1x2 y1y2 z1z2 w1w2 vmulw.xyz vf8, vf4,
                   vf5 #vf8 x1w2 y1w2 z1w2 w1w2 vmulw.xyz vf9, vf5,
                   vf4 #vf9 w1x2 w1y2 w1z2 w1w2
#nop
                       vsubx.w vf12,
                   vf12, vf12 #vf12.w = ww - xx vopmula.xyz ACC, vf4,
                   vf5 vopmsub.xyz vf6, vf5,
                   vf4

                       vadd.xyz vf8,
                   vf8, vf9 vsuby.w vf12, vf12,
                   vf12 #vf12.w = ww - xx -
                                  yy
#nop nop
                                      vadd.xyz vf8,
                   vf8, vf6 vsubz.w vf8, vf12,
                   vf12 #vf8.w = ww - xx - yy -
                                 zz
#nop nop nop
                                     sqc2 vf8,
...
void IVP_U_Quat::normize_correct_step(int steps) {
  ...
  IVP_DOUBLE square = quat->x * quat->x + quat->y * quat->y +
