...
class IVP_Fast_Math {
 public:
  ...
  static IVP_DOUBLE sqrt(IVP_DOUBLE x) { return IVP_Inline_Math::ivp_sqrtf(x); }
  ...
  static double isqrt(double square, int resolution_steps) {
    ...
    if (resolution_steps > 1)
      inv_sqrt += inv_sqrt * (0.5f - (inv_sqrt * inv_sqrt * squareh));
    if (resolution_steps > 2)
      inv_sqrt += inv_sqrt * (0.5f - (inv_sqrt * inv_sqrt * squareh));

    IVP_ASSERT(IVP_Inline_Math::fabsd(1.0f - inv_sqrt * inv_sqrt * square) <
               0.001f);
    return inv_sqrt;
  }
  static IVP_DOUBLE sqrt(IVP_DOUBLE x) { return ::sqrt(x); }
#endif
};
