...
#include <string.h>
#elif defined(LINUX) || defined(WIN32) || defined(SGI) || \
...
IVP_DOUBLE get_float() { return p_atof(p_str_tok(NULL, IVP_WHITESPACE)); }
...
void IVP_U_Point::set_interpolate(const IVP_U_Point *p0, const IVP_U_Point *p1,
                                  IVP_DOUBLE s) {
  IVP_DOUBLE is = 1.0f - s;
  IVP_DOUBLE a = p0->k[0] * is;
  IVP_DOUBLE a2 = p1->k[0] * s;
  IVP_DOUBLE b = p0->k[1] * is;
  IVP_DOUBLE b2 = p1->k[1] * s;
  a += a2;
  IVP_DOUBLE c = p0->k[2] * is;
  IVP_DOUBLE c2 = p1->k[2] * s;
  b += b2;
...
void IVP_U_Point::set_interpolate(const IVP_U_Float_Point *p0,
                                  const IVP_U_Float_Point *p1, IVP_DOUBLE s) {
  IVP_DOUBLE is = 1.0f - s;
  IVP_DOUBLE a = p0->k[0] * is;
  IVP_DOUBLE a2 = p1->k[0] * s;
  IVP_DOUBLE b = p0->k[1] * is;
...
void IVP_U_Float_Point::set_interpolate(const IVP_U_Float_Point *p0,
...
void IVP_U_Point::set_orthogonal_part(const IVP_U_Point *vector,
                                      const IVP_U_Point *normal_v) {
...
void IVP_U_Float_Point::set_orthogonal_part(const IVP_U_Float_Point *vector,
                                            const IVP_U_Float_Point *normal_v) {
  IVP_DOUBLE part_direction = vector->dot_product(normal_v);
  this->add_multiple(vector, normal_v, -part_direction);
}
...
#if !defined(IVP_NO_DOUBLE)
void IVP_U_Point::calc_cross_product(const IVP_U_Point *v1,
                                     const IVP_U_Point *v2) {
  inline_calc_cross_product(v1, v2);
}
#endif

void IVP_U_Float_Point::calc_cross_product(const IVP_U_Float_Point *v1,
                                           const IVP_U_Float_Point *v2) {
  inline_calc_cross_product(v1, v2);
}

void IVP_U_Point::solve_quadratic_equation_fast(
    const IVP_U_Point
        *p) {  // points stores quadratic equ. k[2] + x*k[1] * x^2 * k[0]
...
void IVP_U_Point::solve_quadratic_equation_accurate(
    const IVP_U_Point
        *p) {  // points stores quadratic equ. k[2] + x*k[1] * x^2 * k[0]
  if (IVP_Inline_Math::fabsd(p->k[0]) < P_DOUBLE_EPS) {  // linear equation
    k[0] = 0;
...
IVP_BOOL IVP_U_Point::is_parallel(const IVP_U_Point *v_in,
                                  IVP_DOUBLE eps) const {
  IVP_U_Point v1, v2, c;
  v1.set(this);
  v2.set(v_in);
  c.calc_cross_product(&v1, &v2);
  return ((IVP_BOOL)(c.quad_length() <=
                     eps * eps * v1.quad_length() * v2.quad_length()));
}
...
void IVP_U_Point::print(const char *comment) {
  if (comment) {
    ivp_message("%s Point %f %f %f\n", comment, k[0], k[1], k[2]);
  } else {
    ivp_message("Point %f %f %f\n", k[0], k[1], k[2]);
  }
}
...
IVP_RETURN_TYPE IVP_U_Point::set_crossing(IVP_U_Hesse *h0, IVP_U_Hesse *h1,
                                          IVP_U_Hesse *h2) {
...
IVP_RETURN_TYPE IVP_U_Matrix3::get_angles(IVP_FLOAT *alpha, IVP_FLOAT *beta,
                                          IVP_FLOAT *gamma) {
  ...
  if (!mat.orthonormize()) {
    ivp_message("No valid matrix in get_angles!\n");
  ...
  // calc alpha
  v.set(mat_no_g_no_b.get_elem(0, 0), mat_no_g_no_b.get_elem(1, 0),
        0.0f);  // OS ?? or:  1 and 4
  if (!v.normize()) {
    *alpha = 0.0f;
    ivp_message("very strange: no alpha vec in get_angles!\n");
  } else {
    *alpha =
        (IVP_FLOAT)IVP_Inline_Math::atan2d(v.k[1], v.k[0]);  // or vice versa...
  }
...
void IVP_U_Matrix3::orthogonize() {
  ...
  vz.set(get_elem(0, 2), get_elem(1, 2),
         get_elem(2, 2));  // z-vector shall remain (= v1)
...
int IVP_U_Matrix3::calc_eigen_vector(IVP_DOUBLE eigen_value,
                                     IVP_U_Point *eigen_vector_out) {
  ...
  {                          // find maximum value in all values
    ...
    row_index[0] =
        max_row;  // exchange columns to shift pivot element into m[0][0]
    row_index[max_row] = 0;
  ...
  // get rid of second column values;
  {
    m[2][2] -=
        f2 *
        m[2][1];  // this value should be zero, as we know we have an eigenvalue
  }
...
void IVP_U_Hesse::proj_on_plane(const IVP_U_Point *p,
                                IVP_U_Point *result) const {
...
void IVP_U_Hesse::calc_hesse(const IVP_U_Point *tp0, const IVP_U_Point *tp1,
  ...
  this->k[1] = c1;
  this->k[2] = c2;
  this->hesse_val = -(this->k[0] * fp0 + this->k[1] * fp1 + this->k[2] * fp2);
};
...
void IVP_U_Float_Hesse::proj_on_plane(const IVP_U_Float_Point *p,
                                      IVP_U_Float_Point *result) const {
  result->add_multiple(p, this, -get_dist(p) / quad_length());
  // div unnecessary, if hesse is normized
}

void IVP_U_Float_Hesse::calc_hesse(const IVP_U_Float_Point *tp0,
                                   const IVP_U_Float_Point *tp1,
                                   const IVP_U_Float_Point *tp2) {
  IVP_DOUBLE a0, a1, a2, b0, b1, b2;
  ...
  this->hesse_val =
      (IVP_FLOAT) - (this->k[0] * fp0 + this->k[1] * fp1 + this->k[2] * fp2);
};
...
IVP_RETURN_TYPE IVP_U_Matrix::real_invert(IVP_DOUBLE epsilon) {
  ...
  IVP_U_Point r0;
  r0.inline_calc_cross_product(&rows[1], &rows[2]);
  IVP_U_Point r1;
  r1.inline_calc_cross_product(&rows[2], &rows[0]);
  IVP_U_Point r2;
  r2.inline_calc_cross_product(&rows[0], &rows[1]);
...
IVP_RETURN_TYPE IVP_U_Matrix::real_invert(const IVP_U_Matrix *m,
                                          IVP_DOUBLE epsilon) {
  // overloaded
  this->set_matrix(m);
...
void IVP_U_Matrix::init_rows4(const IVP_U_Point *v0, const IVP_U_Point *v1,
...
void IVP_U_Matrix3::init_normized3_col(const IVP_U_Point *vp,
...
// result is a matrix m_Bs_f_As
void IVP_U_Matrix3::init_normized3_row(const IVP_U_Point *vp,
                                       IVP_COORDINATE_INDEX coordinate) {
  IVP_U_Point v0 = *vp;
  v0.normize();
  IVP_U_Point v2(v0.k[1], v0.k[2] - v0.k[0], -v0.k[1]);  // orthogonal vector
...
// result is a matrix m_Bs_f_As
void IVP_U_Matrix3::init_normized3_col(const IVP_U_Point *vb,
                                       IVP_COORDINATE_INDEX index_b,
                                       const IVP_U_Point *vc) {
  IVP_U_Point v0 = *vb;
  v0.normize();
  IVP_U_Point v2;
  v2.inline_calc_cross_product_and_normize(&v0, vc);
  IVP_U_Point v1;
  v1.inline_calc_cross_product(&v2, &v0);
...
// init orthornomized matrix given an As-axis in Bs coords
// result is a matrix m_Bs_f_As
void IVP_U_Matrix3::init_normized3_row(const IVP_U_Point *vb,
                                       IVP_COORDINATE_INDEX index_b,
                                       const IVP_U_Point *vc) {
  IVP_U_Point v0 = *vb;
  v0.normize();
...
void IVP_U_Matrix3::init_rows3(const IVP_U_Point *v0, const IVP_U_Point *v1,
                               const IVP_U_Point *v2) {
  rows[0].set(v0);
  rows[1].set(v1);
  rows[2].set(v2);
...
void IVP_U_Matrix3::init_columns3(const IVP_U_Point *v0, const IVP_U_Point *v1,
                                  const IVP_U_Point *v2) {
  set_elem(0, 0, v0->k[0]);
  set_elem(1, 0, v0->k[1]);
  set_elem(2, 0, v0->k[2]);
...
void IVP_U_Matrix::init_columns4(const IVP_U_Point *v0, const IVP_U_Point *v1,
                                 const IVP_U_Point *v2,
...
void IVP_U_Matrix::init_rot_multiple(
    const IVP_U_Point *angles,
    IVP_DOUBLE factor) {  // angles.k[0] = beta, [1] = gamma [2] = alpha,
...
void IVP_U_Matrix3::mmult3(const IVP_U_Matrix3 *mb,
                           IVP_U_Matrix3 *m_out) const {
  inline_mmult3(mb, m_out);
}

void IVP_U_Matrix3::mimult3(const IVP_U_Matrix3 *mb,
                            IVP_U_Matrix3 *m_out) const {
...
void IVP_U_Matrix3::mi2mult3(const IVP_U_Matrix3 *mb,
                             IVP_U_Matrix3 *m_out) const {
  IVP_U_Point row[3];
  for (int r = 0; r < 3; r++) {
...
#if !defined(IVP_NO_DOUBLE)
void IVP_U_Matrix::vimult4(const IVP_U_Point *p_in,
                           IVP_U_Float_Point *p_out) const {
...
void IVP_U_Matrix::vimult4(const IVP_U_Float_Point *p_in,
                           IVP_U_Float_Point *p_out) const {
  inline_vimult4(p_in, p_out);
};

void IVP_U_Matrix::vmult4(const IVP_U_Float_Point *p_in,
                          IVP_U_Float_Point *p_out) const {
...
void IVP_U_Matrix::vmult4(const IVP_U_Float_Point *p_in,
                          IVP_U_Point *p_out) const {
  inline_vmult4(p_in, p_out);
};
#endif

void IVP_U_Matrix3::vmult3(const IVP_U_Float_Point *p_in,
                           IVP_U_Float_Point *p_out) const {
...
void IVP_U_Matrix3::vimult3(const IVP_U_Float_Point *p_in,
                            IVP_U_Float_Point *p_out) const {
  inline_vimult3(p_in, p_out);
};

void IVP_U_Matrix::shift_ws(const IVP_U_Point *v_in) { this->vv.add(v_in); };
...
void IVP_U_Matrix::print(const char *headline) {
  if (headline) ivp_message("%s\n", headline);
  ivp_message("mm\t%g %g %g\n\t%g %g %g\n\t%g %g %g\n", this->get_elem(0, 0),
              this->get_elem(0, 1), this->get_elem(0, 2), this->get_elem(1, 0),
              this->get_elem(1, 1), this->get_elem(1, 2), this->get_elem(2, 0),
              this->get_elem(2, 1), this->get_elem(2, 2));

  ivp_message("vv\t%g %g %g\n", this->vv.k[0], this->vv.k[1], this->vv.k[2]);
}
...
IVP_ERROR_STRING IVP_U_Matrix::read_from_file(FILE *fp) {
  ...
  while ((cmd = p_read_first_token(fp))) {
    ...
    ivp_message("Load IVP_U_Matrix: Unknown Command\n'%s'!\n", cmd);
    CORE;
