 ...
 *	Description:	Use these enums everywhere where you want to specify a
 *coodinate.
 ********************************************************************************/
...
class IVP_U_Float_Point {
 public:
  ...
  inline void set_negative(
      const IVP_U_Float_Point *p_source);  // this = p_source * -1.0
  inline void set_to_zero();               // i.e. set(0.0, 0.0, 0.0)

  inline IVP_DOUBLE quad_length()
      const;  // length of vector square: k[0]*k[0] + k[1]*k[1] + k[2]*k[2]

  inline void mult(IVP_DOUBLE factor);  // this *= factor

  inline void add(const IVP_U_Float_Point *v2);  // this += v2
  inline void add(const IVP_U_Float_Point *v1,
                  const IVP_U_Float_Point *v2);  // this = v1 + v2;

  inline void add_multiple(const IVP_U_Float_Point *v,
                           IVP_DOUBLE factor);  // this += v1 * factor
  inline void add_multiple(const IVP_U_Float_Point *v1,
                           const IVP_U_Float_Point *v2, IVP_DOUBLE factor2);

  inline void set_multiple(const IVP_U_Quat *q_source,
                           IVP_DOUBLE f);  // this = q_source.xyz * f
#if !defined(IVP_NO_DOUBLE)
  inline void set_multiple(const IVP_U_Point *v,
                           IVP_DOUBLE f);  // this = q_source.xyz * f
#endif
  inline void set_multiple(const IVP_U_Float_Point *v,
                           IVP_DOUBLE f);  // this = q_source.xyz * f
  inline void set_pairwise_mult(
      const IVP_U_Float_Point *v1,
      const IVP_U_Float_Point *v2);  // this->k[i] = v1->k[i] * v2->k[i]

#if !defined(IVP_NO_DOUBLE)
  inline void subtract(const IVP_U_Point *v1,
                       const IVP_U_Point *v2);  // this = v1 - v2
#endif
  inline void subtract(const IVP_U_Float_Point *v1,
                       const IVP_U_Float_Point *v2);  // this = v1 - v2
  inline void subtract(const IVP_U_Float_Point *v2);  // this = this - v2;

  inline void inline_subtract_and_mult(
      const IVP_U_Float_Point *v1, const IVP_U_Float_Point *v2,
      IVP_DOUBLE factor);  // this = (v1-v2) * factor
#if !defined(IVP_NO_DOUBLE)
  inline void inline_subtract_and_mult(
      const IVP_U_Point *v1, const IVP_U_Point *v2,
      IVP_DOUBLE factor);  // this = (v1-v2) * factor
#endif
  inline IVP_DOUBLE dot_product(const IVP_U_Float_Point *v2) const;

  inline void inline_calc_cross_product(
      const IVP_U_Float_Point *v1,
      const IVP_U_Float_Point *v2);  // this = v1 x v2
  inline void inline_calc_cross_product_and_normize(
      const IVP_U_Float_Point *v1,
      const IVP_U_Float_Point *v2);  // this = v1 x v2

  inline void inline_set_vert_to_area_defined_by_three_points(
      const IVP_U_Float_Point *tp0, const IVP_U_Float_Point *tp1,
      const IVP_U_Float_Point *tp2);
  void calc_cross_product(const IVP_U_Float_Point *v1,
                          const IVP_U_Float_Point *v2);  // this = v1 x v2

  void line_sqrt();  // Sqrt of each element: k[i] = sqrt(k[i])

  void set_interpolate(const IVP_U_Float_Point *p0, const IVP_U_Float_Point *p1,
                       IVP_DOUBLE s);
  void set_orthogonal_part(
      const IVP_U_Float_Point *vector,
      const IVP_U_Float_Point
          *normal_v);  // project vector on surface given by normal_v

  void rotate(IVP_COORDINATE_INDEX axis,
              IVP_FLOAT angle);  // rotate the point around origin

  IVP_DOUBLE real_length() const;  // real length = sqrt(quad_length)
  IVP_DOUBLE fast_real_length()
      const;  // real length with 0.1f% error, really fast
  IVP_DOUBLE
  real_length_plus_normize();  // Normize vector and return former real length.
  inline IVP_DOUBLE quad_distance_to(const IVP_U_Float_Point *p)
      const;  // quad distance from this point to another
...
class IVP_U_Point {
 public:
  ...
  inline void inline_calc_cross_product(
      const IVP_U_Point *v1, const IVP_U_Point *v2);  // this = v1 x v2
  inline void inline_calc_cross_product_and_normize(
      const IVP_U_Point *v1, const IVP_U_Point *v2);  // this = v1 x v2
  void calc_cross_product(const IVP_U_Point *v1,
                          const IVP_U_Point *v2);  // this = v1 x v2

  inline void set_to_zero();                     // == set(0.0, 0.0, 0.0)
  inline void set(const IVP_U_Point *p_source);  // this = p_source
  inline void set(
      const IVP_U_Float_Point *p_source);       // this = (IVP_DOUBLE)p_source
  ...
  inline IVP_DOUBLE quad_length()
      const;  // Square length of vector: k[0]*k[0] + k[1]*k[1] + k[2]*k[2].
  inline IVP_DOUBLE quad_distance_to(const IVP_U_Point *p)
      const;  // Square distance from this point to another
  inline IVP_DOUBLE quad_distance_to(const IVP_U_Float_Point *p)
      const;  // Square distance from this point to another.
  ...
  IVP_DOUBLE
  real_length_plus_normize();  // Normize vector and return former real length.
  IVP_DOUBLE fast_real_length() const;  // Returns real length with max. 0.1f%
                                        // error, really fast if in cache.
  IVP_RETURN_TYPE normize();  // Normize vector (-> real length == 1.0f), return
                              // IVP_FALSE if length too small.
  IVP_RETURN_TYPE fast_normize();  // Normize vector (0.1f% error)

  inline void add(const IVP_U_Point *v2);        // this = this + v2;
  inline void add(const IVP_U_Float_Point *v2);  // this = this + v2;
  inline void add(const IVP_U_Point *v1,
                  const IVP_U_Point *v2);  // this = v1 + v2;
  inline void add(const IVP_U_Float_Point *v1,
                  const IVP_U_Float_Point *v2);  // this = v1 + v2;
  inline void add_multiple(const IVP_U_Point *v,
                           IVP_DOUBLE factor);  // this += v1 * factor
  inline void add_multiple(const IVP_U_Float_Point *v,
                           IVP_DOUBLE factor);  // this += v1 * factor
  inline void add_multiple(const IVP_U_Point *v1, const IVP_U_Point *v2,
                           IVP_DOUBLE factor2);  // this = v1 + factor2*v2  .
  inline void add_multiple(const IVP_U_Point *v1, const IVP_U_Float_Point *v2,
                           IVP_DOUBLE factor2);  // this = v1 + factor2*v2  .
  ...
  inline void inline_subtract_and_mult(
      ...
      IVP_DOUBLE factor);  // this = (v1-v2) * factor
  inline void set_negative(
      const IVP_U_Point *p_source);  // this = p_source * -1.0

  inline void mult(IVP_DOUBLE factor);  // this *= factor
  inline void set_multiple(const IVP_U_Point *p, IVP_DOUBLE f);  // this = p * f
  inline void set_multiple(const IVP_U_Float_Point *p,
                           IVP_DOUBLE f);  // this = p * f
  inline void set_multiple(const IVP_U_Quat *q_source,
                           IVP_DOUBLE f);  // this = q_source.xyz * f
  inline void set_pairwise_mult(
      const IVP_U_Point *v1,
      const IVP_U_Point *v2);  // this->k[i] = v1->k[i] * v2->k[i]
  inline void set_pairwise_mult(
      const IVP_U_Point *v1,
      const IVP_U_Float_Point *v2);  // this->k[i] = v1->k[i] * v2->k[i]

  inline void inline_set_vert_to_area_defined_by_three_points(
      const IVP_U_Point *tp0, const IVP_U_Point *tp1, const IVP_U_Point *tp2);
  inline void inline_set_vert_to_area_defined_by_three_points(
      const IVP_U_Float_Point *tp0, const IVP_U_Float_Point *tp1,
      const IVP_U_Point *tp2);
  inline void inline_set_vert_to_area_defined_by_three_points(
      const IVP_U_Float_Point *tp0, const IVP_U_Float_Point *tp1,
      const IVP_U_Float_Point *tp2);

  void set_interpolate(const IVP_U_Point *p0, const IVP_U_Point *p1,
                       IVP_DOUBLE s); /* linear interpolation between p0 and p1:
                                       * this = p1 * s + p0 * (1.0f-s) */
  void set_interpolate(const IVP_U_Float_Point *p0, const IVP_U_Float_Point *p1,
                       IVP_DOUBLE s); /* linear interpolation between p0 and p1:
                                       * this = p1 * s + p0 * (1.0f-s) */
  ...
  // INTERN_START
  IVP_BOOL is_parallel(const IVP_U_Point *v2, IVP_DOUBLE eps)
      const;  // checks length of crossproduct against eps

  void calc_an_orthogonal(const IVP_U_Point *ip);

  void solve_quadratic_equation_fast(
      const IVP_U_Point
          *a); /* Solve a->k[0]*x*x + a->k[1]*x + a->k[2] = 0,
                * this->k[0] = term under squareroot, this->k[1] = smaller value
                * this->k[2] = greater value,
                * if this->k[0] < 0: no result.
                * Uses fast squareroot */
  void solve_quadratic_equation_accurate(
      const IVP_U_Point
          *a); /* Solve a->k[0]*x*x + a->k[1]*x + a->k[2] = 0,
                * this->k[0] = term under squareroot, this->k[1] = smaller value
                * this->k[2] = greater value,
  ...
  void set_orthogonal_part(
      ...
      const IVP_U_Float_Point
          *normal_v);  // project vector on surface given by normal_v
  IVP_RETURN_TYPE set_crossing(
      IVP_U_Hesse *h0, IVP_U_Hesse *h1,
      IVP_U_Hesse *h2);  // set this to the crossing of three areas
  ...
  inline void byte_swap() {
    IVP_ASSERT(0 && "No byte swap for doubles yet");
    CORE;
  }
...
class IVP_Inline_Math {
 public:
  static inline IVP_RETURN_TYPE invert_2x2_matrix(
      const IVP_DOUBLE a1_in, const IVP_DOUBLE b1_in, const IVP_DOUBLE a2_in,
      const IVP_DOUBLE b2_in, IVP_DOUBLE *i_a1_out, IVP_DOUBLE *i_b1_out,
  ...
  static inline IVP_FLOAT approx5_sin(
      IVP_FLOAT angle);  // fifth order approximation
  static inline IVP_FLOAT approx5_cos(
      IVP_FLOAT angle);  // fifth order approximation of cos

  static inline IVP_FLOAT save_acosf(IVP_FLOAT x);  // slow and save acos

  static inline IVP_FLOAT fast_asin(
      IVP_FLOAT x);  // for documentation see ivu_linear_macros.hxx
  static inline IVP_FLOAT upper_limit_asin(
      IVP_FLOAT angle);  // for documentation see ivu_linear_macros.hxx
  ...
  static inline int int_div_2(
      int a);  // workaround for compiler bug in CodeWarrior1.6
  ...
  static IVP_DOUBLE atan2d(IVP_DOUBLE f1, IVP_DOUBLE f2) {
    return atan2f(f1, f2);
  };
#else
...
class IVP_U_Hesse : public IVP_U_Point {
 public:
  ...
  IVP_DOUBLE hesse_val;
#endif
  void calc_hesse(const IVP_U_Point *p0, const IVP_U_Point *p1,
                  const IVP_U_Point *p2);
  void calc_hesse(const IVP_U_Float_Point *p0, const IVP_U_Float_Point *p1,
                  const IVP_U_Float_Point *p2);
  void calc_hesse_val(const IVP_U_Point *p0);
  void proj_on_plane(const IVP_U_Point *p, IVP_U_Point *result)
      const;  // sets result to a point on the plane and nearest to p
  void mult_hesse(IVP_DOUBLE factor);  // scale the hesse
  void normize();  // normize hesse (->super.calc_quad_length() == 1.0f)
#if !defined(IVP_NO_DOUBLE)
  inline IVP_DOUBLE get_dist(const IVP_U_Point *p)
      const;  // get the distance between a point and the plane
#endif
  inline IVP_DOUBLE get_dist(const IVP_U_Float_Point *p)
      const;  // get the distance between a point and the plane
  IVP_RETURN_TYPE calc_intersect_with(
      const IVP_U_Straight
          *straight,  // calc the intersection between a straight and the plane
...
class IVP_U_Float_Hesse : public IVP_U_Float_Point {
 public:
  ...
  IVP_FLOAT hesse_val;
#endif
  void set4(const IVP_U_Float_Hesse *h) {
    this->set(h);
    this->hesse_val = h->hesse_val;
  }
  void calc_hesse(const IVP_U_Float_Point *p0, const IVP_U_Float_Point *p1,
                  const IVP_U_Float_Point *p2);
  void calc_hesse_val(const IVP_U_Float_Point *p0);
  void proj_on_plane(const IVP_U_Float_Point *p, IVP_U_Float_Point *result)
      const;  // sets result to a point on the plane and nearest to p
  void mult_hesse(IVP_DOUBLE factor);  // scale the hesse
  void normize();  // normize hesse (->super.calc_quad_length() == 1.0f)
...
class IVP_U_Matrix3 {
 ...
 public:
  inline IVP_DOUBLE get_elem(int row, int col) const {
    return rows[row].k[col];
  }
  inline void set_elem(int row, int col, IVP_DOUBLE val) {
    rows[row].k[col] = val;
  }

  inline void inline_mmult3(const IVP_U_Matrix3 *mb,
                            IVP_U_Matrix3 *m_out) const;
  inline void inline_mimult3(const IVP_U_Matrix3 *mb,
                             IVP_U_Matrix3 *m_out) const;

  void mmult3(const IVP_U_Matrix3 *mb, IVP_U_Matrix3 *m_out)
      const;  // m_out = this * mb, note: note: in place operation allowed
  void mimult3(const IVP_U_Matrix3 *mb, IVP_U_Matrix3 *m_out)
      const;  // m_out = (this^-1) * mb, note: in place operation allowed
  void mi2mult3(const IVP_U_Matrix3 *mb, IVP_U_Matrix3 *m_out)
      const;  // m_out = this * (mb^-1), note: in place operation allowed

#if !defined(IVP_NO_DOUBLE)
  inline void inline_vmult3(const IVP_U_Point *p_in,
                            IVP_U_Point *p_out) const;  // p_out = this * p_in
  void vmult3(const IVP_U_Point *p_in,
              IVP_U_Point *p_out) const;  // p_out = this * p_in
#endif

  inline void inline_vmult3(
      const IVP_U_Float_Point *p_in,
      IVP_U_Float_Point *p_out) const;  // p_out = this * p_in
  void vmult3(const IVP_U_Float_Point *p_in,
              IVP_U_Float_Point *p_out) const;  // p_out = this * p_in

#if !defined(IVP_NO_DOUBLE)
  inline void inline_vimult3(const IVP_U_Point *p_in, IVP_U_Point *p_out)
      const;  // p_out = (this^-1) * p_in
  void vimult3(const IVP_U_Point *p_in,
               IVP_U_Point *p_out) const;  // p_out = (this^-1) * p_in
#endif

  inline void inline_vimult3(
      const IVP_U_Float_Point *p_in,
      IVP_U_Float_Point *p_out) const;  // assembler version
  void vimult3(const IVP_U_Float_Point *p_in,
               IVP_U_Float_Point *p_out) const;  // p_out = (this^-1) * p_in
  ...
  IVP_DOUBLE quad_rot_distance_to(
      const IVP_U_Matrix3 *m2);  // sum of quad diff of mm
  ...
  void init_columns3(const IVP_U_Point *v0, const IVP_U_Point *v1,
                     const IVP_U_Point *v2);  // sets this columns to v0,v1,v2,
  void init_rows3(const IVP_U_Point *v0, const IVP_U_Point *v1,
                  const IVP_U_Point *v2);  // sets this rows    to v0,v1,v2
   ...
   ********************************************************************************/
  void init_normized3_col(
      const IVP_U_Point *v,
      IVP_COORDINATE_INDEX
          coordinate_index);  // init orthornomized matrix given any axis
  void init_normized3_col(
      const IVP_U_Point *vb, IVP_COORDINATE_INDEX coordinate_index,
      const IVP_U_Point *vc);  // init orthornomized matrix given any two axes
  void init_normized3_row(
      const IVP_U_Point *v,
      IVP_COORDINATE_INDEX
          coordinate_index);  // init orthornomized matrix given any axis
  void init_normized3_row(
  ...
  void init_rotated3(
      ...
      IVP_FLOAT angle);  // init this matrix as a rotation around the given axis

  inline void get_row(IVP_COORDINATE_INDEX row, IVP_U_Point *row_out) const;
  inline void get_row(IVP_COORDINATE_INDEX row,
                      IVP_U_Float_Point *row_out) const;
  inline void get_col(IVP_COORDINATE_INDEX column, IVP_U_Point *col_out) const;
  inline void get_col(IVP_COORDINATE_INDEX column,
                      IVP_U_Float_Point *col_out) const;
  ...
  IVP_RETURN_TYPE real_invert(
      IVP_DOUBLE epsilon = P_DOUBLE_EPS);  // Using determinant algorithm
  ...
  // INTERN_START
  IVP_RETURN_TYPE get_angles(
      IVP_FLOAT *alpha, IVP_FLOAT *beta,
      IVP_FLOAT *gamma);  // get the euler angles (real slow implementation)
                          // INTERN_END
...
class IVP_U_Matrix : public IVP_U_Matrix3 {
 public:
  ...
  IVP_U_Point *get_position() {
    return &vv;
  }  // get the position of an object matrix
  const IVP_U_Point *get_position() const {
    return &vv;
  }  // get the position of an object matrix

  inline void inline_mmult4(const IVP_U_Matrix *mb, IVP_U_Matrix *m_out)
      const;  // matrix multiplication
  inline void inline_mimult4(const IVP_U_Matrix *mb, IVP_U_Matrix *m_out)
      const;  // this is transposed !!
  void mmult4(const IVP_U_Matrix *mb,
              IVP_U_Matrix *m_out) const;  // m_out = this*mb
  void mimult4(const IVP_U_Matrix *mb,
               IVP_U_Matrix *m_out) const;  // m_out = (this^-1) * mb

  void mi2mult4(const IVP_U_Matrix *mb,
                IVP_U_Matrix *m_out) const;  // m_out = this * (mb^-1)
  void rotate(IVP_COORDINATE_INDEX axis, IVP_FLOAT angle,
              IVP_U_Matrix *m_out);  // m_out = this * I.init_rotated()
  void rotate_invers(IVP_COORDINATE_INDEX axis, IVP_FLOAT angle,
                     IVP_U_Matrix *m_out);

#if !defined(IVP_NO_DOUBLE)
  inline void inline_vmult4(const IVP_U_Point *p_in,
                            IVP_U_Point *p_out) const;  // p_out = this * p_in
  inline void inline_vmult4(const IVP_U_Float_Point *p_in,
                            IVP_U_Point *p_out) const;  // p_out = this * p_in
  void vmult4(const IVP_U_Point *p_in,
              IVP_U_Point *p_out) const;  // p_out = this * p_in
  void vmult4(const IVP_U_Float_Point *p_in,
              IVP_U_Point *p_out) const;  // p_out = this * p_in
#endif
  inline void inline_vmult4(const IVP_U_Float_Point *p_in,
                            IVP_U_Float_Point *p_out)
      const;  // p_out = this * p_in (e.g. object space to core space)
  void vmult4(const IVP_U_Float_Point *p_in,
              IVP_U_Float_Point *p_out) const;  // p_out = this * p_in

#if !defined(IVP_NO_DOUBLE)
  inline void inline_vimult4(const IVP_U_Point *p_in, IVP_U_Point *p_out)
      const;  // p_out = (this^-1) * p_in
  inline void inline_vimult4(const IVP_U_Point *p_in, IVP_U_Float_Point *p_out)
      const;  // p_out = (this^-1) * p_in
  void vimult4(const IVP_U_Point *p_in,
               IVP_U_Float_Point *p_out) const;  // p_out = (this^-1) * p_in
  void vimult4(const IVP_U_Point *p_in,
               IVP_U_Point *p_out) const;  // p_out = (this^-1) * p_in
#endif
  inline void inline_vimult4(
      const IVP_U_Float_Point *p_in,
      IVP_U_Float_Point *p_out) const;  // p_out = (this^-1) * p_in
  void vimult4(const IVP_U_Float_Point *p_in,
               IVP_U_Float_Point *p_out) const;  // p_out = (this^-1) * p_in
  ...
  IVP_RETURN_TYPE real_invert(
      IVP_DOUBLE epsilon = P_DOUBLE_EPS);  // Using determinant algorithm
  IVP_RETURN_TYPE real_invert(const IVP_U_Matrix *m,
                              IVP_DOUBLE epsilon = P_DOUBLE_EPS);  // Overload
  ...
  void init_rot_multiple(
      const IVP_U_Point *angles,
      IVP_DOUBLE factor = 1.0f);  // sets this rotated (using euler angles) and
                                  // position to 0,0,0

  void init_columns4(const IVP_U_Point *v0, const IVP_U_Point *v1,
                     const IVP_U_Point *v2, const IVP_U_Point *shift);
  void init_rows4(const IVP_U_Point *v0, const IVP_U_Point *v1,
                  const IVP_U_Point *v2, const IVP_U_Point *shift);
  void interpolate(
      const IVP_U_Matrix *m1, const IVP_U_Matrix *m2,
      IVP_DOUBLE i_factor);  // this = m1 * (1.0f-factor) + m2 * factor
...
class IVP_U_Quat {
 public:
  ...
  void set_interpolate_smoothly(const IVP_U_Quat *from, const IVP_U_Quat *to,
                                IVP_DOUBLE t);
  void set_interpolate_linear(const IVP_U_Quat *from, const IVP_U_Quat *to,
                              IVP_DOUBLE t);
  void normize_quat();
  ...
  void set_from_rotation_vectors(IVP_DOUBLE x1, IVP_DOUBLE y1, IVP_DOUBLE z1,
                                 IVP_DOUBLE x2, IVP_DOUBLE y2, IVP_DOUBLE z2);
