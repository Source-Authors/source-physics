...
IVP_Template_Constraint::IVP_Template_Constraint() {
  ...
  borderleft_Rfs[0] = borderleft_Rfs[1] = borderleft_Rfs[2] =
      borderleft_Rfs[3] = borderleft_Rfs[4] = borderleft_Rfs[5] = 0.0;
  borderright_Rfs[0] = borderright_Rfs[1] = borderright_Rfs[2] =
      borderright_Rfs[3] = borderright_Rfs[4] = borderright_Rfs[5] = 0.0;
  maximpulse[0] = maximpulse[1] = maximpulse[2] = maximpulse[3] =
      maximpulse[4] = maximpulse[5] = 0.0;
  maximpulse_type[0] = maximpulse_type[1] = maximpulse_type[2] =
      maximpulse_type[3] = maximpulse_type[4] = maximpulse_type[5] =
          IVP_CFE_NONE;
  limited_axis_stiffness = .3f;
}

void IVP_Template_Constraint::set_constraint_Ros(
    IVP_Real_Object *_objR, const IVP_U_Point *_anchor_Ros,
    const IVP_U_Point *_known_axis_Ros, unsigned _transdim, unsigned _rotdim,
    IVP_Real_Object *_objA, const IVP_U_Matrix *m_Rfs_f_Afs) {
  ...
  IVP_U_Matrix m_Aos_f_Ros;
  m_ws_f_Aos.mimult4(&m_ws_f_Ros, &m_Aos_f_Ros);
  ...
  int rotdim = _rotdim;
  axis_type[0] =
      transdim-- > 0 ? IVP_CONSTRAINT_AXIS_FIXED : IVP_CONSTRAINT_AXIS_FREE;
  axis_type[1] =
      transdim-- > 0 ? IVP_CONSTRAINT_AXIS_FIXED : IVP_CONSTRAINT_AXIS_FREE;
  axis_type[2] =
      transdim-- > 0 ? IVP_CONSTRAINT_AXIS_FIXED : IVP_CONSTRAINT_AXIS_FREE;
  axis_type[3] =
      rotdim-- > 0 ? IVP_CONSTRAINT_AXIS_FIXED : IVP_CONSTRAINT_AXIS_FREE;
  axis_type[4] =
      rotdim-- > 0 ? IVP_CONSTRAINT_AXIS_FIXED : IVP_CONSTRAINT_AXIS_FREE;
  axis_type[5] =
      rotdim-- > 0 ? IVP_CONSTRAINT_AXIS_FIXED : IVP_CONSTRAINT_AXIS_FREE;
...
void IVP_Template_Constraint::set_constraint_ws(
    IVP_Real_Object *_objR, const IVP_U_Point *_anchor_ws,
    const IVP_U_Point *_known_axis_ws, unsigned _transdim, unsigned _rotdim,
    IVP_Real_Object *_objA, const IVP_U_Matrix *m_Rfs_f_Afs) {
  IVP_U_Matrix m_ws_f_Ros;
  if (_objR) {
    _objR->get_m_world_f_object_AT(&m_ws_f_Ros);
  ...
  set_constraint_Ros(_objR, &point_Ros, &known_axis_Ros, _transdim, _rotdim,
                     _objA, m_Rfs_f_Afs);
}

void IVP_Template_Constraint::set_stiffness_for_limited_axis(
    IVP_FLOAT stiffness) {
  limited_axis_stiffness = stiffness;
}

// sort the coordinate-systems from tmpl into this.
void IVP_Template_Constraint::sort_coordinates(
  ...
  // Translation...
  for (i = 0; i < 3; i++)
    if (tmpl.axis_type[i] == IVP_CONSTRAINT_AXIS_FIXED) {
      if (tmpl.m_Ros_f_Rfs) {
        IVP_U_Point p;
        tmpl.m_Ros_f_Rfs->get_col(IVP_COORDINATE_INDEX(i), &p);
        m_Ros_f_Rfs->set_col(IVP_COORDINATE_INDEX(pos), &p);
      }
      if (tmpl.m_Aos_f_Afs) {
        IVP_U_Point p;
        tmpl.m_Aos_f_Afs->get_col(IVP_COORDINATE_INDEX(i), &p);
  ...
  for (i = 0; i < 3; i++)
    if (tmpl.axis_type[i] == IVP_CONSTRAINT_AXIS_LIMITED) {
      if (tmpl.m_Ros_f_Rfs) {
        IVP_U_Point p;
        tmpl.m_Ros_f_Rfs->get_col(IVP_COORDINATE_INDEX(i), &p);
        m_Ros_f_Rfs->set_col(IVP_COORDINATE_INDEX(pos), &p);
  ...
  for (i = 0; i < 3; i++)
    if (tmpl.axis_type[i] == IVP_CONSTRAINT_AXIS_FREE) {
      if (tmpl.m_Ros_f_Rfs) {
        IVP_U_Point p;
        tmpl.m_Ros_f_Rfs->get_col(IVP_COORDINATE_INDEX(i), &p);
        m_Ros_f_Rfs->set_col(IVP_COORDINATE_INDEX(pos), &p);
  ...
  if (tmpl.m_Ros_f_Rfs) {
    IVP_U_Point col0;
    m_Ros_f_Rfs->get_col(IVP_INDEX_X, &col0);
    IVP_U_Point col1;
    m_Ros_f_Rfs->get_col(IVP_INDEX_Y, &col1);
    IVP_U_Point cross;
    cross.calc_cross_product(&col0, &col1);
  ...
  if (tmpl.m_Aos_f_Afs) {
    IVP_U_Point col0;
    m_Aos_f_Afs->get_col(IVP_INDEX_X, &col0);
    IVP_U_Point col1;
    m_Aos_f_Afs->get_col(IVP_INDEX_Y, &col1);
    IVP_U_Point cross;
    cross.calc_cross_product(&col0, &col1);
  ...
  if (tmpl.m_Ros_f_Rrs) {
    pos = 0;
    for (i = 0; i < 3; i++)
      if (tmpl.axis_type[i + 3] == IVP_CONSTRAINT_AXIS_FIXED) {
        IVP_U_Point p;
        tmpl.m_Ros_f_Rrs->get_col(IVP_COORDINATE_INDEX(i), &p);
    ...
    for (i = 0; i < 3; i++)
      if (tmpl.axis_type[i + 3] == IVP_CONSTRAINT_AXIS_FREE) {
        IVP_U_Point p;
        tmpl.m_Ros_f_Rrs->get_col(IVP_COORDINATE_INDEX(i), &p);
        m_Ros_f_Rrs->set_col(IVP_COORDINATE_INDEX(pos), &p);
        borderleft_Rfs[pos + 3] = tmpl.borderleft_Rfs[i + 3];
    ...
    {
      IVP_U_Point col0;
      m_Ros_f_Rrs->get_col(IVP_INDEX_X, &col0);
      IVP_U_Point col1;
      m_Ros_f_Rrs->get_col(IVP_INDEX_Y, &col1);
      IVP_U_Point cross;
      cross.calc_cross_product(&col0, &col1);
...
void IVP_Template_Constraint::set_fixing_point_Ros(
    const IVP_U_Point *anchor) {  // point_Ros_f_uRfs
  m_Ros_f_Rfs = &mm_Ros_f_Rfs;
  m_Ros_f_Rfs->vv.set(anchor);
}

void IVP_Template_Constraint::set_translation_axes_Ros(
    const IVP_U_Matrix3 *trans_axes) {  // m_Ros_f_uRfs
  m_Ros_f_Rfs = &mm_Ros_f_Rfs;
...
void IVP_Template_Constraint::free_translation_axis(
    IVP_COORDINATE_INDEX which) {
  axis_type[which] = IVP_CONSTRAINT_AXIS_FREE;
}

void IVP_Template_Constraint::limit_translation_axis(IVP_COORDINATE_INDEX which,
...
void IVP_Template_Constraint::set_max_translation_impulse(
...
void IVP_Template_Constraint::set_max_translation_impulse(
    IVP_COORDINATE_INDEX coord, IVP_CONSTRAINT_FORCE_EXCEED impulsetype,
    IVP_FLOAT impulse) {
  maximpulse[coord] = impulse;
  maximpulse_type[coord] = impulsetype;
}

void IVP_Template_Constraint::set_rotation_axes_Ros(
...
void IVP_Template_Constraint::limit_rotation_axis(IVP_COORDINATE_INDEX which,
                                                  IVP_FLOAT border_left,
                                                  IVP_FLOAT border_right) {
...
void IVP_Template_Constraint::set_max_rotation_impulse(
    IVP_CONSTRAINT_FORCE_EXCEED impulsetype, IVP_FLOAT impulse) {
...
void IVP_Template_Constraint::set_attached_translation_axes_Aos(
    const IVP_U_Matrix3 *rot_attached) {
  m_Aos_f_Afs = &mm_Aos_f_Afs;
  *(IVP_U_Matrix3 *)&mm_Aos_f_Afs = *rot_attached;
}

void IVP_Template_Constraint::set_attached_fixing_point_Aos(
    const IVP_U_Point *trans_attached) {
  m_Aos_f_Afs = &mm_Aos_f_Afs;
...
IVP_Constraint *IVP_Environment::create_constraint(
    const IVP_Template_Constraint *tmpl) {
  if (!tmpl->objectR && !tmpl->objectA) return NULL;
