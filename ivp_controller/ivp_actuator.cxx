...
void IVP_Template_Anchor::set_anchor_position_os(
    IVP_Real_Object *obj, const IVP_U_Float_Point *coords_os) {
  this->object = obj;
...
#if 1
void IVP_Template_Anchor::set_anchor_position_cs(
    IVP_Real_Object *obj, const IVP_U_Float_Point *coords_cs) {
  this->object = obj;
...
void IVP_Template_Anchor::set_anchor_position_cs(IVP_Real_Object *obj,
                                                 const IVP_DOUBLE x,
                                                 const IVP_DOUBLE y,
                                                 const IVP_DOUBLE z) {
  this->object = obj;
...
IVP_Actuator_Two_Point::IVP_Actuator_Two_Point(
    IVP_Environment *env, IVP_Template_Two_Point *two_point_templ,
    IVP_ACTUATOR_TYPE)
    : IVP_Actuator(env) {
  ...
  core0->environment->get_controller_manager()
...
IVP_Actuator_Two_Point::~IVP_Actuator_Two_Point() {
  ...
  core0->environment->get_controller_manager()
      ->remove_controller_from_environment(this, IVP_TRUE);  // silently
}

///////////// FOUR POINT //////////////

IVP_Actuator_Four_Point::IVP_Actuator_Four_Point(
    IVP_Environment *env, IVP_Template_Four_Point *four_point_templ,
    IVP_ACTUATOR_TYPE)
    : IVP_Actuator(env) {
...
void IVP_Anchor_Check_Dist::init_anchor_check_dist(
    IVP_Real_Object *object, IVP_U_Point *position_world_space,
    IVP_Actuator_Check_Dist *i_act_check_dist) {
...
void IVP_Anchor_Check_Dist::hull_limit_exceeded_event(IVP_Hull_Manager *,
                                                      IVP_HTIME) {
...
IVP_Actuator_Check_Dist::IVP_Actuator_Check_Dist(
    IVP_Environment *, IVP_Template_Check_Dist *templ) {
  this->client_data = templ->client_data;
  this->mod_is_outside = templ->mod_is_outside;
  this->range = templ->range;

  this->anchors[0].init_anchor_check_dist(
      templ->objects[0], &templ->position_world_space[0], this);
  this->anchors[1].init_anchor_check_dist(
      templ->objects[1], &templ->position_world_space[1], this);
...
void IVP_Actuator_Two_Point::ensure_actuator_in_simulation() {
  get_actuator_anchor(0)
      ->l_anchor_object->get_environment()
...
void IVP_Actuator_Check_Dist::fire_check_dist_event(IVP_BOOL distance_shorter) {
  for (int i = listeners_check_dist_event.len() - 1; i >= 0; i--) {
    listeners_check_dist_event.element_at(i)->check_dist_event(
        this, distance_shorter);
  }
}

void IVP_Actuator_Check_Dist::fire_check_dist_is_going_to_be_deleted_event() {
  for (int i = listeners_check_dist_event.len() - 1; i >= 0; i--) {
    listeners_check_dist_event.element_at(i)
...
void IVP_Actuator_Check_Dist::hull_limit_exceeded_event() {
  ...
  if (this->mod_is_outside)
    mod_is_outside->set_int(this->is_outside, IVP_TRUE);  // delayed!

  /** re-insert in hull managers **/
  IVP_DOUBLE new_delta_hull =
      IVP_Inline_Math::fabsd(len - this->range) *
      0.5f;  // @@@ maybe unequal hull vals, dependant on cur sur speed
...
void IVP_Anchor_Check_Dist::hull_manager_is_going_to_be_deleted_event(
    IVP_Hull_Manager *) {
...
IVP_Actuator_Force::IVP_Actuator_Force(IVP_Environment *env,
                                       IVP_Template_Force *templ)
...
IVP_Actuator_Force_Active::IVP_Actuator_Force_Active(IVP_Environment *env,
                                                     IVP_Template_Force *templ)
...
IVP_Actuator_Extra::IVP_Actuator_Extra(IVP_Environment *env,
    ...
    : IVP_Actuator_Two_Point(env, templ, IVP_ACTUATOR_TYPE_ETC) {
  this->info = templ->info;

  if (this->info.active_float_force)
    info.active_float_force->add_dependency(this);
  if (this->info.active_float_bomb)
    info.active_float_bomb->add_dependency(this);

  if (this->info.mod_fc_height) info.mod_fc_height->add_dependency(this);
  if (this->info.mod_fc_target_height)
    info.mod_fc_target_height->add_dependency(this);
...
IVP_U_Point *IVP_Actuator_Extra::get_float_cam_props(
    IVP_DOUBLE *fc_height_out, IVP_DOUBLE *fc_target_height_out,
    IVP_DOUBLE *fc_dist_out, IVP_DOUBLE *fc_speed_out) {
...
IVP_Actuator_Extra::~IVP_Actuator_Extra() {
  ...
  if (info.mod_fc_target_height)
    info.mod_fc_target_height->remove_dependency(this);
...
IVP_Actuator_Rot_Mot::IVP_Actuator_Rot_Mot(IVP_Environment *env,
    ...
    : IVP_Actuator_Two_Point(env, templ, IVP_ACTUATOR_TYPE_ROT_MOT) {
  ...
  this->active_float_rotation_speed_out =
      templ->active_float_rotation_speed_out;
  ...
  axis_in_core_coord_system.subtract(&get_actuator_anchor(1)->core_pos,
                                     &get_actuator_anchor(0)->core_pos);
  axis_in_core_coord_system.normize();
}

IVP_Actuator_Rot_Mot_Active::IVP_Actuator_Rot_Mot_Active(
    IVP_Environment *env, IVP_Template_Rot_Mot *templ)
...
void IVP_Actuator_Rot_Mot_Active::active_float_changed(IVP_U_Active_Float *af) {
  if (af == active_float_max_rotation_speed)
    set_max_rotation_speed(active_float_max_rotation_speed->get_float_value());
  if (af == active_float_power)
    set_power(active_float_power->get_float_value());
  if (af == active_float_max_torque)
    set_max_torque(active_float_max_torque->get_float_value());
}
...
void IVP_Actuator_Rot_Mot::do_simulation_controller(
  ...
  if (core == orig_core) {
    ...
    const IVP_U_Matrix *m_w_f_c0 = get_actuator_anchor(0)
                                       ->l_anchor_object->get_core()
                                       ->get_m_world_f_core_PSI();
    const IVP_U_Matrix *m_w_f_c1 = get_actuator_anchor(1)
                                       ->l_anchor_object->get_core()
                                       ->get_m_world_f_core_PSI();
    IVP_U_Point pos1_ws;
  ...
  if (domega > max_rotation_speed) {
    IVP_IF(0) { ivp_message("Rot Mot rot speed threshold reached!\n"); }
    return;  // too fast
  }
...
IVP_Actuator_Torque::IVP_Actuator_Torque(IVP_Environment *env,
                                         IVP_Template_Torque *templ)
    : IVP_Actuator_Two_Point(env, templ, IVP_ACTUATOR_TYPE_TORQUE) {
  this->max_rotation_speed = templ->max_rotation_speed;
  ...
  if (obj0->get_original_core() != obj1->get_original_core()) {
    ivp_message(
        "Both Anchors of a Torque_Actuator must be attached to just one "
        "object.\n");
    CORE;
  }
  axis_in_core_coord_system.subtract(&get_actuator_anchor(1)->core_pos,
...
IVP_Actuator_Torque_Active::IVP_Actuator_Torque_Active(
    IVP_Environment *env, IVP_Template_Torque *templ)
...
void IVP_Actuator_Torque_Active::active_float_changed(IVP_U_Active_Float *af) {
  if (af == active_float_max_rotation_speed)
    set_max_rotation_speed(af->get_float_value());
...
void IVP_Actuator_Torque::do_simulation_controller(
    IVP_Event_Sim *es, IVP_U_Vector<IVP_Core> * /*core_list*/) {
  ...
  if (!IVP_MTIS_SIMULATED(core->movement_state) ||
      core->pinned) {  // only rotate simulated cores
    return;
  }
  ...
  if (core == orig_core) {
    ...
    h.subtract(&get_actuator_anchor(1)->core_pos,
               &get_actuator_anchor(0)->core_pos);
    h.normize();
    axis = &h;
  }
...
IVP_Anchor::~IVP_Anchor() { anchor_get_real_object()->remove_anchor(this); }
...
void IVP_Actuator_Force::do_simulation_controller(
    IVP_Event_Sim *es, IVP_U_Vector<IVP_Core> * /*core_list*/) {
...
void IVP_Actuator_Extra::do_simulation_controller(
    IVP_Event_Sim *es, IVP_U_Vector<IVP_Core> * /*core_list*/) {
...
void IVP_Actuator_Extra::do_puck_force(IVP_DOUBLE dtime) {
  ...
  {
    IVP_DOUBLE force =
        0; /* (dlen - this->info.spring_len) * this->info.spring_constant; */
...
void IVP_Actuator_Extra::calc_float_cam_matrix(IVP_U_Matrix *cam_matrix_out) {
  ...
  cam_matrix_out->init_columns4(&v0, &v1, &cam_dir,
                                &this->current_float_cam_pos);
}
...
void IVP_Actuator_Extra::do_float_cam(IVP_DOUBLE d_time) {
  ...
  IVP_U_Point *fc_pos = this->get_float_cam_props(&fc_height, &fc_target_height,
                                                  &fc_dist, &fc_speed);
  ...
  this->current_float_cam_pos.set_interpolate(fc_pos, &cur_pos,
                                              fc_speed * d_time);
  ...
  if (this->current_vals_are_set == IVP_FALSE) {
    ...
    this->current_look_point.set_interpolate(
        &current_look_point, &look_point,
        fc_speed);  // @@@ or other (fix) speed ?
  }
}
...
IVP_Actuator_Stabilizer::IVP_Actuator_Stabilizer(IVP_Environment *env,
    ...
    : IVP_Actuator_Four_Point(env, templ, IVP_ACTUATOR_TYPE_STABILIZER) {
  l_environment = env;
...
void IVP_Actuator_Stabilizer::do_simulation_controller(
    IVP_Event_Sim *es, IVP_U_Vector<IVP_Core> * /*core_list*/) {
  IVP_Core *core[4];
  int i;
  for (i = 0; i < 4; i++) {
    core[i] = get_actuator_anchor(i)
                  ->l_anchor_object->friction_core;  // @@@OG friction_core???
  }
