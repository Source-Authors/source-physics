...
void ivp_print_mi_matrix(int n, int m, IVP_MI_Vector **A) {
  ...
  for (i = 0; i < m; i++) {
    ivp_message("     [ ");
    for (j = 0; j <= n; j++) {
      ivp_message("% 1.3e ", A[j]->element[i]);
    }
    ivp_message("] \n");
  }
  ivp_message("\n");
 ...
 ************************************************************************/
IVP_RETURN_TYPE IVP_Multidimensional_Interpolator::linfit(int m, int n,
                                                          IVP_MI_Vector **A,
                                                          IVP_FLOAT *X,
                                                          IVP_FLOAT *res) {
  ...
  for (int j = 0; j < n; j++) {
    for (int i = j + 1; i < m; i++) {
      if (IVP_Inline_Math::fabsd(A[j]->element[i]) > MI_eps /*P_DOUBLE_RES*/) {
        ...
        IVP_FLOAT q = (IVP_FLOAT)IVP_Inline_Math::ivp_sqrtf(
            A[j]->element[i] * A[j]->element[i] +
            A[j]->element[j] * A[j]->element[j]);
        IVP_ASSERT(q != 0.0f);
    ...
    if (IVP_Inline_Math::fabsd(A[j]->element[j]) <= f)
      return (IVP_FAULT);  // matrix is not invertible (it is singularian), so
                           // we cannot substitute back later
  }
 ...
 *                most cases already presorted, so bubble sort is efficient
 *enough
 ***********************************************************************************/
void IVP_Multidimensional_Interpolator::sort_vectors(int nr_to_be_sorted) {
  ...
  while (true) {
    ...
    if (previous_inputs[first]->weight_statistic < second_weight) {
      ...
      while (
          (test_pos > 0) &&
          (previous_inputs[test_pos - 1]->weight_statistic < second_weight)) {
        // exchange input vectors
 ...
 ******************************************************************************/
IVP_RETURN_TYPE IVP_Multidimensional_Interpolator::check_interpolation(
    const IVP_MI_Vector *new_input, const int max_tries_nr_of_vectors_involved,
    const IVP_FLOAT max_res, IVP_MI_Vector *output) {
  if (nr_occupied < 2) {  // solver has to compute the solution for itself,
                          // because there is not enough data yet
    return (IVP_FAULT);
  }

  // at this point it is already provided for enough data from previous solver
  // runs

  // try to interpolate a new solution via resolving the linear compensation
  // problem with an increasing number of input vectors
  IVP_MI_Vector **scratchboard_A = (IVP_MI_Vector **)alloca(
      sizeof(void *) *
      (nr_occupied + 1));  // add one element because scratchboard_A also has to
                           // hold the new_input
  for (int i = 0; i <= nr_occupied; i++) {
    IVP_MI_VECTOR_ALLOCA(scratchboard_A[i], nr_of_elements_input);
  }

  IVP_FLOAT *interpolation_weights =
      (IVP_FLOAT *)alloca(sizeof(IVP_FLOAT) * nr_occupied);
  IVP_FLOAT interpolation_weigh_first =
      0.0f;  // interpolation weigh of the first vector, which is (1.0f - (all
             // other weights))
  IVP_FLOAT res;
  ...
  // make sure that after some time we try the interpolation again with a
  // smaller number of vectors
  if (counter_tries_nr_of_vectors_involved++ >=
      max_tries_nr_of_vectors_involved) {
  ...
  if (new_input_difference->length() < MI_eps) {
    ...
    if ((nr_of_vectors_involved < 2) ||
      ...
      // in the previous interpolation run one vector was sufficient (but not
      // now)
      nr_of_vectors_involved = 2;
    }
    ...
    while (nr_of_vectors_involved <= nr_occupied) {
      ...
      scratchboard_A[nr_of_vectors_involved - 1]->set(
          new_input_difference);  // now the matrix [A|b] is ready for LINFIT

      // run the linfit algorithm
      success = linfit(nr_of_elements_input, nr_of_vectors_involved - 1,
                       scratchboard_A, interpolation_weights, &res);

#ifdef WITH_DEBUG_OUTPUT
      ivp_message(
          "nr_of_vectors_involved = %d, nr_occupied = %d, res = %1.4e\n",
      ...
      if (success) {
        ...
        if (res > max_res) {
          success = IVP_FAULT;
#ifdef WITH_DEBUG_OUTPUT
          ivp_message("res is over limit!\n");
          nr_res_over_limit[nr_of_vectors_involved - 1]++;
#endif
        } else {
          // ensure that the interpolation_weights are in the interval
          // [-0.2f; 1.2f]
          for (int ix = 0; ix < (nr_of_vectors_involved - 1); ix++) {
            if (IVP_Inline_Math::fabsd(interpolation_weights[ix] - 0.5f) >
                0.7f) {
              success = IVP_FAULT;
#ifdef WITH_DEBUG_OUTPUT
              ivp_message("interpolation_weigh is over limit!\n");
          ...
          if ((IVP_Inline_Math::fabsd(interpolation_weigh_first - 0.5f) >
               0.7f) &&
              success) {
            success = IVP_FAULT;
#ifdef WITH_DEBUG_OUTPUT
            ivp_message("interpolation_weigh_first is over limit!\n");
            nr_int_weight_over_limit[nr_of_vectors_involved - 1]++;
#endif
          }
        }
...
#ifdef WITH_DEBUG_OUTPUT
        ivp_message("int_w[0] = %e ", interpolation_weigh_first);
        for (int g = 0; g < nr_of_vectors_involved - 1; g++) {
          ivp_message("int_w[%d] = %e ", g + 1, interpolation_weights[g]);
        }
        ivp_message("\n");
#endif
  ...
  if (success) {  // new_input was successfully interpolated, the weights of the
                  // old input vectors involved can be found in
    ...
    // calc the output as result of the linear combination of the solution
    // vectors
    for (int i = 1; i < nr_of_vectors_involved; i++) {
      previous_inputs[i]->weight_statistic =
          previous_inputs[i]->weight_statistic * influence_of_old_weight +
          interpolation_weights[i - 1];

      for (int k = 0; k < nr_of_elements_solution; k++) {
        previous_solutions[scratch_area_index]->set(
            k, previous_solutions[scratch_area_index]->element[k] +
                   interpolation_weights[i - 1] *
                       previous_solutions[i]->element[k]);
      }
    }

    // calc weigh statistics for the first vector and add first vector's share
    // to the output
    previous_inputs[0]->weight_statistic =
        (previous_inputs[0]->weight_statistic * influence_of_old_weight) +
        interpolation_weigh_first;
    for (int k = 0; k < nr_of_elements_solution; k++) {
      previous_solutions[scratch_area_index]->set(
          k, previous_solutions[scratch_area_index]->element[k] +
                 interpolation_weigh_first * previous_solutions[0]->element[k]);
    }
...
#ifdef WITH_DEBUG_OUTPUT
    {
      ...
      for (int i = 1; i < nr_of_vectors_involved; i++) {
        for (int k = 0; k < nr_of_elements_input; k++) {
          test_vector->set(
              k, test_vector->element[k] + interpolation_weights[i - 1] *
                                               previous_inputs[i]->element[k]);
        }
      }

      for (int k = 0; k < nr_of_elements_input; k++) {
        test_vector->set(
            k, test_vector->element[k] +
                   interpolation_weigh_first * previous_inputs[0]->element[k]);
      }

      ivp_message("interpolated_output_vector:\n");
      output->print();
    }
#endif
    // sort the vectors (i.e. index[]) in declining order of their weight
    // statistics
  ...
  for (int y = 0; y < nr_occupied; y++) {
    ivp_message("timestamp = %f  ", previous_inputs[y]->time_stamp);
...
void IVP_Multidimensional_Interpolator::
    add_new_input_solution_combination_conventional(
        const IVP_MI_Vector *new_input, const IVP_MI_Vector *new_solution) {
  // move all vectors one position further, the new vectors will be copied to
  // the first position
  ...
  // give the new input vector a weigh that ensures it will be positioned very
  // high after a sort
  previous_inputs[0]->weight_statistic =
      1.0f / (1.0f - influence_of_old_weight);

  if (nr_occupied < nr_of_vectors) nr_occupied++;
}

void IVP_Multidimensional_Interpolator::
    add_new_input_solution_combination_stochastic(
        const IVP_MI_Vector *new_input, const IVP_MI_Vector *new_solution) {
  ...
  // give the new input vector a weigh that ensures it will be positioned very
  // high after a sort
  previous_inputs[solution_slot]->weight_statistic =
      1.0f / (1.0f - influence_of_old_weight);
 ...
 * Constructor
 ************************/
IVP_Multidimensional_Interpolator::IVP_Multidimensional_Interpolator(
    int nr_of_vectors_, int nr_of_elements_input_,
    int nr_of_elements_solution_) {
  int i;
  ...
  // previous_inputs = new IVP_MI_Vector[nr_of_vectors](nr_of_elements_input);
  // //contains vectors of input data from previous runs of a solver
  previous_inputs =
      (IVP_MI_Vector **)p_malloc(sizeof(void *) * (nr_of_vectors));
  ...
  previous_solutions =
      (IVP_MI_Vector **)p_malloc(sizeof(void *) * (nr_of_vectors + 1));
  for (i = 0; i <= nr_of_vectors; i++) {
