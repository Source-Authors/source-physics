...
#include <ivp_physics.hxx>
    ...
    (defined(__MWERKS__) && defined(__POWERPC__))
#include <alloca.h>
#endif
...
IVP_Constraint_Local_Anchor::~IVP_Constraint_Local_Anchor() = default;
...
IVP_Constraint_Local::IVP_Constraint_Local() {
  ...
  for (auto &t : fixed) {
    t = IVP_CONSTRAINT_AXIS_DUMMY;
  }

  memset(borderleft_Rfs, 0, sizeof(borderleft_Rfs));
  memset(borderright_Rfs, 0, sizeof(borderright_Rfs));
...
void IVP_Constraint_Local::sort_translation_mapping() {
  ...
  matrix_size =
      fixedtrans_dim + fixedrot_dim + limitedtrans_dim + limitedrot_dim;
}
...
void IVP_Constraint_Local::init(const IVP_Template_Constraint &tmpl) {
  ...
  if (objectR)
    objectR->get_m_world_f_object_AT(&m_ws_f_Ros);
  else
    m_ws_f_Ros.init();
  IVP_U_Matrix m_ws_f_Aos;
  if (objectA)
    objectA->get_m_world_f_object_AT(&m_ws_f_Aos);
  else
    m_ws_f_Aos.init();
  IVP_U_Matrix m_Ros_f_Aos;
  m_ws_f_Ros.mimult4(&m_ws_f_Aos, &m_Ros_f_Aos);
  IVP_U_Matrix m_Rcs_f_Ros;
  if (objectR)
    objectR->calc_m_core_f_object(&m_Rcs_f_Ros);
  else
    m_Rcs_f_Ros.init();
  IVP_U_Matrix m_Acs_f_Aos;
  if (objectA)
    objectA->calc_m_core_f_object(&m_Acs_f_Aos);
  else
    m_Acs_f_Aos.init();
  IVP_U_Matrix m_Rcs_f_Aos;
  m_Rcs_f_Ros.mmult4(&m_Ros_f_Aos, &m_Rcs_f_Aos);
  IVP_U_Matrix m_Rcs_f_Acs;
  m_Rcs_f_Aos.mi2mult4(&m_Acs_f_Aos, &m_Rcs_f_Acs);

  // m_Ros_f_Rfs->vimult4(&_centerT_Ros, &center_trans_Rfs);
  // m_Ros_f_Rfs->vimult3(&_centerR_Ros, &center_rot_Rfs);

  if (tmpl.m_Ros_f_Rfs) {
    m_Rcs_f_Ros.mmult4(tmpl.m_Ros_f_Rfs, &m_Rfs_f_Rcs);
  ...
  if (tmpl.m_Aos_f_Afs) {
    m_Acs_f_Aos.mmult4(tmpl.m_Aos_f_Afs, &m_Afs_f_Acs);
    m_Afs_f_Acs.real_invert();
  } else {  // Afs is Rfs
    m_Rfs_f_Rcs.mmult4(&m_Rcs_f_Acs, &m_Afs_f_Acs);
  }
  if (tmpl.m_Ros_f_Rrs) {
    m_Rfs_f_Rcs.rot = new IVP_U_Matrix3;
    m_Rcs_f_Ros.mmult3(tmpl.m_Ros_f_Rrs, m_Rfs_f_Rcs.rot);
    m_Rfs_f_Rcs.rot->transpose3();
    IVP_U_Matrix3 m_fs_f_rs;
    m_Rfs_f_Rcs.mi2mult3(m_Rfs_f_Rcs.rot, &m_fs_f_rs);
  ...
  damp_factor_div_force =
      tmpl.damp_factor / force_factor;  // Ausgleichsrate pro PSI-Zeiteinheit
...
// Calculates the impulses which are needed to relaxe the constraint completely
// till next PSI
void IVP_Constraint_Local::do_simulation_controller(
  ...
  IVP_ASSERT(!coreR || coreR->physical_unmoveable ||
             coreR->movement_state != IVP_MT_NOT_SIM);
  IVP_ASSERT(!coreA || coreA->physical_unmoveable ||
             coreA->movement_state != IVP_MT_NOT_SIM);
  ...
  const IVP_U_Matrix &m_ws_f_Rcs =
      coreR ? *coreR->get_m_world_f_core_PSI() : m_identity;
  const IVP_U_Matrix &m_ws_f_Acs =
      coreA ? *coreA->get_m_world_f_core_PSI() : m_identity;

  IVP_U_Matrix3 &m_fs_f_Rcs = m_Rfs_f_Rcs;
  IVP_U_Matrix3 m_fs_f_ws;
  m_fs_f_Rcs.mi2mult3(
      &m_ws_f_Rcs, &m_fs_f_ws);  // #+# wahrscheinlich nur temporaer verwendet
  IVP_U_Matrix3 m_fs_f_Acs;
  m_fs_f_ws.mmult3(&m_ws_f_Acs, &m_fs_f_Acs);
   ...
   *                  drRA_rs
   ********************************************************************************/
  const IVP_U_Matrix3 &m_Rrs_f_Rcs =
      (m_Rfs_f_Rcs.rot)
          ? *(m_Rfs_f_Rcs.rot)
          : (IVP_U_Matrix3)
                m_Rfs_f_Rcs;  // TL: cast (IVP_U_Matrix) needed for sun compiler
  const IVP_U_Matrix3 &m_Ars_f_Acs =
      (m_Afs_f_Acs.rot) ? *m_Afs_f_Acs.rot : (IVP_U_Matrix3)m_Afs_f_Acs;  //
  ...
  if (fixedrot_dim + limitedrot_dim == 1) {  // cardan joint
    ...
    IVP_U_Point fixed_axisArs_Acs;
    m_Ars_f_Acs.get_row(mapping_uRrs_f_Rrs[IVP_INDEX_X],
                        &fixed_axisArs_Acs);  // The fixed axis of objectA ...
    IVP_U_Point fixed_axisArs_ws;
    m_ws_f_Acs.vmult3(&fixed_axisArs_Acs, &fixed_axisArs_ws);
    IVP_U_Point fixed_axisArs_Rcs;
    m_ws_f_Rcs.vimult3(&fixed_axisArs_ws,
                       &fixed_axisArs_Rcs);  // ... transformed into Rcs
    IVP_U_Point fixed_axisRrs_Rcs;
    m_Rrs_f_Rcs.get_row(
        mapping_uRrs_f_Rrs[IVP_INDEX_X],
        &fixed_axisRrs_Rcs);  // The fixed reference axis of objectR in Rcs
    IVP_U_Point vertical_axisRA_Rcs;  // The axis which is vertical to both axes
    vertical_axisRA_Rcs.calc_cross_product(
        &fixed_axisArs_Rcs,
        &fixed_axisRrs_Rcs);  // what if vertical_axisRA_Rcs == nullvec?

    // find rs:
    // If (fixed_axisArs_Rcs, fixed_axisRrs_Rcs, vertical_axisRA_Rcs) is seen as
    // a coordinate space m_rs_f_fixedaxisRcs, then the x-axis of rs is on the
    // line "y == -x" in this "fixedaxis"-system. y and z only orthonormized
    // Therefore, x_freeaxis == (1, 1, 0). => m_rs_f_Rcs * (1,0,0) ==
    // fixed_axisArs_Rfs - fixed_axisRrs_Rfs.

    // Add both axes. rsx_Rcs is parallel to rs_x only because both matrices are
    // _ortho_normized.
    IVP_U_Point rsx_Rcs;
    rsx_Rcs.add(&fixed_axisArs_Rcs, &fixed_axisRrs_Rcs);
    m_rs_f_Rcs_buffer.init_normized3_col(&rsx_Rcs,
                                         mapping_uRrs_f_Rrs[IVP_INDEX_X]);
    m_rs_f_Rcs_buffer.transpose3();  // #+# use init normized3_row or program
                                     // init_normized3_inv
    pm_rs_f_Rcs->mi2mult3(&m_ws_f_Rcs, &m_rs_f_ws);
    m_rs_f_ws.mmult3(&m_ws_f_Acs, &m_rs_f_Acs);

    // Calculate orientation
    IVP_U_Point vertical_axisRA_Rrs;
    m_Rrs_f_Rcs.vmult3(&vertical_axisRA_Rcs, &vertical_axisRA_Rrs);

    // orientation in Rrs of the common Vertical -- Ausrichtung der gemeinsamen
    // Senkrechten im Rrs:
    IVP_DOUBLE vertical_anglezy_Rrs = IVP_Inline_Math::atan2d(
        vertical_axisRA_Rrs.k[mapping_uRrs_f_Rrs[IVP_INDEX_Y]],
        vertical_axisRA_Rrs.k[mapping_uRrs_f_Rrs[IVP_INDEX_Z]]);

    IVP_U_Point vertical_axisRA_Acs;
    IVP_U_Point vertical_axisRA_Ars, vertical_axisRA_ws;
    ...
    // Orientation in Ars of the common Vertical -- Ausrichtung der gemeinsamen
    // Senkrechten im Ars:
    IVP_DOUBLE vertical_anglezy_Ars = IVP_Inline_Math::atan2d(
        vertical_axisRA_Ars.k[mapping_uRrs_f_Rrs[IVP_INDEX_Y]],
        vertical_axisRA_Ars.k[mapping_uRrs_f_Rrs[IVP_INDEX_Z]]);
    ...
    drRA_now_rs.k[mapping_uRrs_f_Rrs[IVP_INDEX_X]] =
        diff_axis;  // hangt von known_axis ab
    drRA_now_rs.k[mapping_uRrs_f_Rrs[IVP_INDEX_Y]] = 0.0f;
    drRA_now_rs.k[mapping_uRrs_f_Rrs[IVP_INDEX_Z]] = 0.0f;
  } else {
    pm_rs_f_Rcs->mi2mult3(
        &m_ws_f_Rcs, &m_rs_f_ws);  //  #+# move this lines into the next ifs
    m_rs_f_ws.mmult3(&m_ws_f_Acs, &m_rs_f_Acs);
  }
  ...
  if (fixedrot_dim + limitedrot_dim == 3) {  // same orientation
    IVP_U_Matrix3 m_rs_f_Ars;
    m_rs_f_Acs.mi2mult3(&m_Ars_f_Acs, &m_rs_f_Ars);

    IVP_U_Quat quaternion_relatedsystem;
    quaternion_relatedsystem.set_quaternion(&m_rs_f_Ars);
    drRA_now_rs.set(
        2.0f * IVP_Inline_Math::fast_asin(quaternion_relatedsystem.x),
  ...
  IVP_IF(0) {
    ...
    if (coreR)
      coreR->environment->add_draw_vector(m_ws_f_Rcs.get_position(), &world_vec,
                                          m_Rfs_f_Rcs.object->get_name(), 3);
    if (coreA)
      coreA->environment->add_draw_vector(m_ws_f_Acs.get_position(), &world_vec,
                                          m_Afs_f_Acs.object->get_name(), 3);
  }

  IVP_U_Point nullvec;
  nullvec.set_to_zero();
  IVP_U_Float_Point pointR_Rcs;
  m_Rfs_f_Rcs.vimult4(&nullvec, &pointR_Rcs);
  IVP_U_Float_Point pointA_Acs;
  m_Afs_f_Acs.vimult4(&nullvec, &pointA_Acs);
  IVP_U_Point pointR_ws;
  m_ws_f_Rcs.vmult4(&pointR_Rcs, &pointR_ws);
  IVP_U_Point pointA_ws;
  m_ws_f_Acs.vmult4(&pointA_Acs, &pointA_ws);
  IVP_U_Point pointR_Acs;
  m_ws_f_Acs.vimult4(&pointR_ws, &pointR_Acs);

  // All calculations will be done in fs and rs
  IVP_DOUBLE inv_dtime = es->i_delta_time;

  // better name it next_dvRA_fs and next_drRA_rs -- eigentlich next_dvRA_fs und
  // next_drRA_rs
  ...
  {                     // Calc desired velocity difference
    ...
    // aktuelle Position += ... hmm, mueszte mit quaternions gehen, die man
    // aufeinander addiert oder indem man die Achse bestimmt, um die sich das
    // Objekt dreht, und dann in der Rotationsebene den neuen Punkt bestimmt
    ...
    if (coreR) {
      coreR->get_surface_speed_on_test(&pointR_Rcs, &coreR->speed,
                                       &coreR->rot_speed, &v_pointR_ws);
    } else {
      v_pointR_ws.set_to_zero();
    }
    if (coreA) {
      coreA->get_surface_speed_on_test(&pointA_Acs, &coreA->speed,
                                       &coreA->rot_speed, &v_pointA_ws);
    } else {
      v_pointA_ws.set_to_zero();
    }
    IVP_U_Float_Point v_pointR_fs;
    m_fs_f_ws.vmult3(&v_pointR_ws, &v_pointR_fs);
    IVP_U_Float_Point v_pointA_fs;
    m_fs_f_ws.vmult3(&v_pointA_ws, &v_pointA_fs);
    // The velocity needed to reduce the position difference in the next PSI is
    // needed. (dvRA) current position == cpR, cpA; current velocity == vR, vA;
    // d_time == t; next position == npR, npA. dvRA == (npA - npR) / t  &&  npX
    // == cpX + vX * t  => dvRA == (cpA - cpR) / t + vA - vR the same
    // calculation for rotation some lines below.
    {
      IVP_U_Point dRA_now_fs;
      IVP_U_Point dvRA_ws;
      dvRA_ws.subtract(&pointA_ws, &pointR_ws);
      m_fs_f_ws.vmult3(&dvRA_ws, &dRA_now_fs);

      IVP_U_Point dv_fs;
      dv_fs.subtract(&v_pointA_fs, &v_pointR_fs);
      ...
      for (int i = 0; i < 3; i++)
        if (fixed[i] & IVP_CONSTRAINT_AXIS_LIMITED) {
          ...
          if (dvRA_fs.k[i] < borderleft_Rfs[i]) {
            dvRA_fs.k[i] =
                (dRA_now_fs.k[i] - borderleft_Rfs[i]) * limited_axis_stiffness +
                dv_fs.k[i] * d_time;

            (int &)fixed[i] |= IVP_CONSTRAINT_AXIS_FIXED;
          } else if (dvRA_fs.k[i] > borderright_Rfs[i]) {
            dvRA_fs.k[i] = (dRA_now_fs.k[i] - borderright_Rfs[i]) *
                               limited_axis_stiffness +
                           dv_fs.k[i] * d_time;
            (int &)fixed[i] |= IVP_CONSTRAINT_AXIS_FIXED;
          } else {
            (int &)fixed[i] &= ~IVP_CONSTRAINT_AXIS_FIXED;
    ...
    if (coreR)
      ...
      rR_rs.set_to_zero();
    if (coreA)
      m_rs_f_Acs.vmult3(&coreA->rot_speed, &rA_rs);
    else
      rA_rs.set_to_zero();
      ...
      for (int i = 0; i < 3; i++) {
        if (fixed[i + 3] & IVP_CONSTRAINT_AXIS_LIMITED) {
          if (drRA_rs.k[i] < borderleft_Rfs[i + 3]) {
            // dpos - rangenear
            drRA_rs.k[i] = (drRA_now_rs.k[i] - borderleft_Rfs[i + 3]) *
                               limited_axis_stiffness +
                           drs_rs.k[i] * d_time;
            ...
            drRA_rs.k[i] = (drRA_now_rs.k[i] - borderright_Rfs[i + 3]) *
                               limited_axis_stiffness +
                           drs_rs.k[i] * d_time;
            (int &)fixed[i + 3] |= IVP_CONSTRAINT_AXIS_FIXED;
          } else {
  ...
  if (fixedrot_dim == 1) {
    ...
    IVP_U_Point fixed_axisArs_Acs;
    m_Ars_f_Acs.get_row(mapping_uRrs_f_Rrs[IVP_INDEX_X],
                        &fixed_axisArs_Acs);  // Die feste Achse von objectA ...
    IVP_U_Point fixed_axisArs_ws;
    m_ws_f_Acs.vmult3(&fixed_axisArs_Acs, &fixed_axisArs_ws);
    IVP_U_Point fixed_axisArs_Rcs;
    m_ws_f_Rcs.vimult3(&fixed_axisArs_ws,
                       &fixed_axisArs_Rcs);  // ... transformiert ins Rcs
    IVP_U_Point fixed_axisRrs_Rcs;
    m_Rrs_f_Rcs.get_row(
        mapping_uRrs_f_Rrs[IVP_INDEX_X],
        &fixed_axisRrs_Rcs);  // Die feste Referenzachse von objectR im Rcs

    IVP_DOUBLE angleRA = IVP_Inline_Math::save_acosf(
        fixed_axisArs_Rcs.dot_product(&fixed_axisRrs_Rcs));  // vector angle
    IVP_DOUBLE slowrange =
        IVP_DOUBLE(IVP_PI - IVP_Inline_Math::fabsd(angleRA));  // 0 .. IVP_PI
    if (slowrange < 1.0f) drRA_rs.mult(slowrange);
  }

  // Testpushes. Vielleicht kann ich sie rausziehen, wenn die Resultate immer
  // gleich sind?
  ...
  // Translation
  {
    for (int i = 0; i < 3; i++)
      if (fixed[i] & IVP_CONSTRAINT_AXIS_FIXED) {
        ...
        testimpulseA_pointR_fs.set_negative(
            &testimpulseR_pointR_fs);  // #+# can be highly optimized, use
                                       // get_row instead of vimult3
        ...
        /* So funktioniert die Berechnung unter der Annahme, da?sowohl coreR als
           auch coreA beweglich sind: coreR->test_push_core(&pointR_Rcs,
           &testimpulseR_pointR_Rcs, &testimpulseR_pointR_ws, &dvR_ws,
           &drR_Rcs); coreR->inline_get_surface_speed_on_test(&pointR_Rcs,
           &dvR_ws, &drR_Rcs, &dv_pointR_ws);

                tempvec.inline_calc_cross_product(pointR_Rcs,
           testimpulseR_pointR_Rcs); drR_Rcs->pairwise_mult(tempvec,
           coreR->rot_inertia);
                dvR_ws->inline_set_multiple(&testimpulseR_pointR_ws,
           coreR->inv_mass); IVP_U_Point_PCore temppc;
                ...
                dv_pointR_ws->add(dvR_ws);
                  ...
                  coreA->test_push_core(&pointR_Acs, &testimpulseA_pointR_Acs,
           &testimpulseA_pointR_ws, &dvA_ws, &drA_Acs);
                coreA->inline_get_surface_speed_on_test(&pointA_Acs, &dvA_ws,
           &drA_Acs, &dv_pointA_ws);
        */
        ...
        if (!coreR || coreR->physical_unmoveable) {
          ...
          coreR->test_push_core(&pointR_Rcs, &testimpulseR_pointR_Rcs,
                                &testimpulseR_pointR_ws, &dvR_ws,
                                &drR_Rcs);  // Push at pointR
        if (!coreA || coreA->physical_unmoveable) {
          ...
          coreA->test_push_core(&pointR_Acs, &testimpulseA_pointR_Acs,
                                &testimpulseA_pointR_ws, &dvA_ws,
                                &drA_Acs);  // Push at pointR
#else
          coreA->test_push_core(&pointA_Acs, &testimpulseA_pointR_Acs,
                                &testimpulseA_pointR_ws, &dvA_ws,
                                &drA_Acs);  // Push at pointA
#endif
        }
        // Calculate the velocity changes of the impulse at the point
        IVP_U_Float_Point dv_pointR_ws, dv_pointA_ws, dv_pointR_fs,
            dv_pointA_fs;
        if (coreR) {
          coreR->get_surface_speed_on_test(&pointR_Rcs, &dvR_ws, &drR_Rcs,
                                           &dv_pointR_ws);
        } else {
          dv_pointR_ws.set_to_zero();
        }
        if (coreA) {
          coreA->get_surface_speed_on_test(&pointA_Acs, &dvA_ws, &drA_Acs,
                                           &dv_pointA_ws);
        } else {
  ...
  // Rotation
  {
    for (int i = 3; i < 6; i++)
      if (fixed[i] & IVP_CONSTRAINT_AXIS_FIXED) {
        ...
        /* So funktioniert die Berechnung unter der Annahme, dasz sowohl coreR
           als auch coreA beweglich sind:
                coreR->test_rot_push_core_multiple_cs(&testimpulseRrs_Rcs, 1.0,
           &drR_rot_Rcs);
                coreR->inline_get_surface_speed_on_rotation_test_cs(&pointR_Rcs,
           &drR_rot_Rcs, &dv_pointR_Rcs);

                drR_rot_Rcs->pairwise_mult(testimpulseRrs_Rcs, inv_rot_inertia);
                dv_pointR_Rcs->inline_calc_cross_product(drR_rot_Rcs,
           pointR_Rcs);
        */
        ...
        if (!coreR || coreR->physical_unmoveable) {
          ...
          coreR->test_rot_push_core_multiple_cs(&testimpulseRrs_Rcs, 1.0f,
                                                &drR_rot_Rcs);
        }
        if (!coreA || coreA->physical_unmoveable) {
          drA_rot_Acs.set_to_zero();
        } else {
          coreA->test_rot_push_core_multiple_cs(&testimpulseArs_Acs, 1.0f,
                                                &drA_rot_Acs);
        }
        // Calculate the resulting surface speed changes of the impulse at the
        // point
        IVP_U_Float_Point dv_pointR_Rcs, dv_pointA_Acs, dv_pointR_fs,
            dv_pointA_fs;
...
#else
  mg_impulse_f_dvRA.matrix_values = (IVP_DOUBLE *)alloca(
      (matrix_size * aligned_row_len + IVP_VECFPU_SIZE - 1) *
      sizeof(IVP_DOUBLE));
  mg_impulse_f_dvRA.result_vector =
      (IVP_DOUBLE *)alloca(aligned_row_len * sizeof(IVP_DOUBLE));
  mg_impulse_f_dvRA.desired_vector =
      (IVP_DOUBLE *)alloca(aligned_row_len * sizeof(IVP_DOUBLE));
#endif
  // align matrix_values
...

  {
    ...
    for (int i = 0; i < 6; i++)
      if (fixed[i] & IVP_CONSTRAINT_AXIS_FIXED) {  // waagrecht durch
        unsigned int pos_j = 0;
        int j;
        // insert Translation results (vertical)
        for (j = 0; j < 3; j++)
          if (fixed[j] & IVP_CONSTRAINT_AXIS_FIXED) {
            mg_impulse_f_dvRA.matrix_values[pos_i + aligned_row_len * pos_j] =
                testnext_dvRA_fs[i].k[j];
            pos_j++;
          }
  ...
  IVP_U_Float_Point impulseR_fs, impulserotR_rs, impulseA_fs, impulserotA_rs;
  {
    ...
    for (int i = 0; i < 3; i++) {
      ...
      if (fixed[i] & IVP_CONSTRAINT_AXIS_FIXED) pos_i++;
    }
    for (int j = 3; j < 6; j++) {
      if (fixed[j] & IVP_CONSTRAINT_AXIS_FIXED)
      ...
      if (fixed[j] & IVP_CONSTRAINT_AXIS_FIXED) pos_i++;
    }
  }
  ...
  if (maxforce) {
    ...
    switch (norm) {
      case IVP_NORM_MINIMUM:
        /// #+# avoid divisionen !!!!!
        in_range = (IVP_Inline_Math::fabsd(impulseR_fs.k[0]) /
                        maxforce->halfimpulse[0] +
                    IVP_Inline_Math::fabsd(impulseR_fs.k[1]) /
                        maxforce->halfimpulse[1] +
                    IVP_Inline_Math::fabsd(impulseR_fs.k[2]) /
                        maxforce->halfimpulse[2]) < 3.0f;
        break;
      case IVP_NORM_EUCLIDIC:
        /// vectoroperationen
        {
          IVP_FLOAT einheitsforce_x = IVP_Inline_Math::fabsd(impulseR_fs.k[0]) /
                                      maxforce->halfimpulse[0];
          IVP_FLOAT einheitsforce_y = IVP_Inline_Math::fabsd(impulseR_fs.k[1]) /
                                      maxforce->halfimpulse[1];
          IVP_FLOAT einheitsforce_z = IVP_Inline_Math::fabsd(impulseR_fs.k[2]) /
                                      maxforce->halfimpulse[2];
          IVP_FLOAT qlen = einheitsforce_x * einheitsforce_x +
                           einheitsforce_y * einheitsforce_y +
                           einheitsforce_z * einheitsforce_z;
      ...
      case IVP_NORM_MAXIMUM:
        in_range = IVP_Inline_Math::fabsd(impulseR_fs.k[0]) <
                   ...
                   IVP_Inline_Math::fabsd(impulseR_fs.k[2]) <
                       maxforce->halfimpulse[2] &&
                   IVP_Inline_Math::fabsd(impulserotR_rs.k[0]) <
                       maxforce->halfimpulse[3] &&
                   IVP_Inline_Math::fabsd(impulserotR_rs.k[1]) <
                       maxforce->halfimpulse[4] &&
                   IVP_Inline_Math::fabsd(impulserotR_rs.k[2]) <
                       maxforce->halfimpulse[5];
        break;
    }
    IVP_IF(0) {
      ivp_message("%f  %f  %f     ", IVP_Inline_Math::fabsd(impulseR_fs.k[0]),
                  IVP_Inline_Math::fabsd(impulseR_fs.k[1]),
                  IVP_Inline_Math::fabsd(impulseR_fs.k[2]));
      ivp_message("%f  %f  %f\n", IVP_Inline_Math::fabsd(impulserotR_rs.k[0]),
                  IVP_Inline_Math::fabsd(impulserotR_rs.k[1]),
    ...
    if (!in_range) {
      ...
      for (int i = 0; i < 3; i++) {
        ...
        switch (norm) {
          case IVP_NORM_MINIMUM:
            simplenorm_transdir = IVP_Inline_Math::fabsd(impulseR_fs.k[0]) +
                                  IVP_Inline_Math::fabsd(impulseR_fs.k[1]) +
                                  IVP_Inline_Math::fabsd(impulseR_fs.k[2]);
            break;
          case IVP_NORM_EUCLIDIC:
            simplenorm_transdir =
                IVP_Inline_Math::fabsd(impulseR_fs.k[map[i]]) +
                IVP_Inline_Math::fabsd(impulseR_fs.k[map[i + 1]]) *
                    (1.0f / 2.41f) +
                IVP_Inline_Math::fabsd(impulseR_fs.k[map[i + 2]]) *
                    (1.0f / 2.41f);
            break;
          case IVP_NORM_MAXIMUM:
        ...
        if (maxforce->halfimpulse[i] < simplenorm_transdir) {
          switch (maxforce->type[i]) {
            ...
            {
              IVP_FLOAT newfactor =
                  d_time * maxforce->halfimpulse[i] / simplenorm_transdir;
              if (newfactor < maxforce_factor) maxforce_factor = newfactor;
            } break;
            case IVP_CFE_BREAK:  // constraint breaks in this axis
              // Oh no, I can not simply set type[i] to IVP_CONSTRAINT_AXIS_FREE
              // :-( Ich brech den Constraint mal ganz Maybe I should call a
              // notify() function
  ...
  IVP_U_Float_Point impulseR_ws, impulseR_Rcs, impulserotR_Rcs, impulseA_ws,
      impulseA_Acs, impulserotA_Acs;
  ...
  IVP_IF(0) {  // check for plausible values
    IVP_U_Float_Point dvR_ws, dvA_ws, drR_Rcs, drA_Acs, drR_rot_Rcs,
        drA_rot_Acs;
    if (coreR && coreR->physical_unmoveable) {
      dvR_ws.set_to_zero();
      drR_Rcs.set_to_zero();
      ...
      coreR->test_push_core(&pointR_Rcs, &impulseR_Rcs, &impulseR_ws, &dvR_ws,
                            &drR_Rcs);
      coreR->test_rot_push_core_multiple_cs(&impulserotR_Rcs, 1.0f,
                                            &drR_rot_Rcs);
    }
    if (coreA && coreA->physical_unmoveable) {
      dvA_ws.set_to_zero();
      ...
      coreA->test_push_core(&pointA_Acs, &impulseA_Acs, &impulseA_ws, &dvA_ws,
                            &drA_Acs);
      coreA->test_rot_push_core_multiple_cs(&impulserotA_Acs, 1.0f,
                                            &drA_rot_Acs);
    }
  ...
  if (coreR && !coreR->physical_unmoveable) {
    ...
    IVP_IF(1) {  // show lines
      ...
      coreR->environment->add_draw_vector(&pointR_ws, &impulseR_ws, "",
                                          2);  // grun
      ...
      coreR->environment->add_draw_vector(&pointR_ws, &impulserotR_ws, "",
                                          5);  // gelb
    }
  }
  if (coreA && !coreA->physical_unmoveable) {
#ifdef PUSH_SAMEPOINT
    coreA->push_core(&pointR_Acs, &impulseA_Acs,
                     &impulseA_ws);  // Push am pointR
#else
    coreA->push_core(&pointA_Acs, &impulseA_Acs,
                     &impulseA_ws);  // Push am pointA
#endif
...
void IVP_Constraint_Local::change_fixing_point_Ros(
    const IVP_U_Point *anchor) {  // point_Ros_f_nRfs, point_nRfs_in_Ros
  if (m_Rfs_f_Rcs.object)
    m_Rfs_f_Rcs.object->ensure_in_simulation();
  else
    m_Afs_f_Acs.object->ensure_in_simulation();
  IVP_U_Matrix m_Rcs_f_Ros;
  if (m_Rfs_f_Rcs.object)
    m_Rfs_f_Rcs.object->calc_m_core_f_object(&m_Rcs_f_Ros);
  else
    m_Rcs_f_Ros.init();
  IVP_U_Point point_Rcs_f_nRfs;
  m_Rcs_f_Ros.vmult4(anchor, &point_Rcs_f_nRfs);
  IVP_U_Point point_Rfs_f_nRfs;
  m_Rfs_f_Rcs.vmult4(&point_Rcs_f_nRfs,
                     &point_Rfs_f_nRfs);  // point_nRfs_in_Rfs
  m_Rfs_f_Rcs.vv.subtract(&point_Rfs_f_nRfs);

  /* if I would have to translate -- wenn ich ubersetzen mubte:
      IVP_U_Point point_Rcs_f_nuRfs; m_Rcs_f_Ros.vmult4(anchor,
     &point_Rcs_f_nuRfs); IVP_U_Matrix3 m_uRfs_f_Rcs;
     mapping_uRfs_f_Rfs.mapply((IVP_U_Matrix3 *) &m_Rfs_f_Rcs, &m_uRfs_f_Rcs);
      IVP_U_Point point_nuRfs_f_Rcs; m_uRfs_f_Rcs.vmult3(&point_Rcs_f_nuRfs,
     &point_nuRfs_f_Rcs); point_nuRfs_f_Rcs.set_negative(&point_nuRfs_f_Rcs);
      mapping_uRfs_f_Rfs.viapply(&point_nuRfs_f_Rcs, &m_Rfs_f_Rcs.vv);
  */
  m_Afs_f_Acs.vv.subtract(
      &point_Rfs_f_nRfs);  // weil die Punkte so verschoben werden mussen, als
                           // lagen Rfs und Afs aufeinander.
}

void IVP_Constraint_Local::change_target_fixing_point_Ros(
    const IVP_U_Point *anchor) {  // point_Ros_f_nRfs
  if (m_Rfs_f_Rcs.object)
    m_Rfs_f_Rcs.object->ensure_in_simulation();
  else
    m_Afs_f_Acs.object->ensure_in_simulation();
...
void IVP_Constraint_Local::change_translation_axes_Ros(
    const IVP_U_Matrix3 *trans_axes) {  // m_Ros_f_nRfs, m_nRfs_in_Ros
  if (m_Rfs_f_Rcs.object)
    m_Rfs_f_Rcs.object->ensure_in_simulation();
  else
    m_Afs_f_Acs.object->ensure_in_simulation();
  ...
  if (m_Rfs_f_Rcs.object)
    ...
    m_Rcs_f_Ros.init();
  IVP_U_Matrix3 m_Rcs_f_nRfs;
  m_Rcs_f_Ros.mmult3(trans_axes, &m_Rcs_f_nRfs);
  IVP_U_Matrix3 m_Rfs_f_nRfs;
  m_Rfs_f_Rcs.mmult3(&m_Rcs_f_nRfs, &m_Rfs_f_nRfs);

  m_Rfs_f_Rcs.set_transpose3(&m_Rcs_f_nRfs);

  IVP_U_Matrix3 m_nAfs_f_Acs;
  m_Rfs_f_nRfs.mimult3(&m_Afs_f_Acs, &m_nAfs_f_Acs);
...
void IVP_Constraint_Local::change_target_translation_axes_Ros(
    const IVP_U_Matrix3 *trans_axes) {  // m_Ros_f_nRfs, m_nRfs_in_Ros
  if (m_Rfs_f_Rcs.object)
    m_Rfs_f_Rcs.object->ensure_in_simulation();
  else
...
void IVP_Constraint_Local::limit_translation_axis(IVP_COORDINATE_INDEX which,
                                                  IVP_FLOAT border_left,
                                                  IVP_FLOAT border_right) {
  if (m_Rfs_f_Rcs.object)
    m_Rfs_f_Rcs.object->ensure_in_simulation();
  else
    m_Afs_f_Acs.object->ensure_in_simulation();
...
void IVP_Constraint_Local::change_max_translation_impulse(
    IVP_CONSTRAINT_FORCE_EXCEED impulsetype, IVP_FLOAT impulse) {
  if (m_Rfs_f_Rcs.object)
    m_Rfs_f_Rcs.object->ensure_in_simulation();
  else
    m_Afs_f_Acs.object->ensure_in_simulation();
  ...
  maxforce->halfimpulse[0] = maxforce->halfimpulse[1] =
      maxforce->halfimpulse[2] = impulse;
  maxforce->type[0] = maxforce->type[1] = maxforce->type[2] = impulsetype;
}

void IVP_Constraint_Local::change_rotation_axes_Ros(
    const IVP_U_Matrix3 *rot_axes) {  // m_Ros_f_nRrs, m_nRrs_in_Ros
  if (m_Rfs_f_Rcs.object)
    m_Rfs_f_Rcs.object->ensure_in_simulation();
  else
    m_Afs_f_Acs.object->ensure_in_simulation();
  ...
  if (m_Rfs_f_Rcs.object)
    ...
    m_Rcs_f_Ros.init();
  IVP_U_Matrix3 m_Rcs_f_nRrs;
  m_Rcs_f_Ros.mmult3(rot_axes, &m_Rcs_f_nRrs);
  IVP_U_Matrix3 m_Rrs_f_nRrs;
  m_Rfs_f_Rcs.rot->mmult3(&m_Rcs_f_nRrs, &m_Rrs_f_nRrs);
  IVP_U_Matrix3 m_nRrs_f_Rcs;
  m_Rrs_f_nRrs.mimult3(m_Rfs_f_Rcs.rot, &m_nRrs_f_Rcs);
  *m_Rfs_f_Rcs.rot = m_nRrs_f_Rcs;

  IVP_U_Matrix3 m_nArs_f_Acs;
  m_Rrs_f_nRrs.mimult3(m_Afs_f_Acs.rot, &m_nArs_f_Acs);
  *m_Afs_f_Acs.rot = m_nArs_f_Acs;
}

void IVP_Constraint_Local::change_target_rotation_axes_Ros(
    const IVP_U_Matrix3 *rot_axes) {  // m_Ros_f_nRrs
  if (m_Rfs_f_Rcs.object)
    m_Rfs_f_Rcs.object->ensure_in_simulation();
  else
    m_Afs_f_Acs.object->ensure_in_simulation();
  ...
  if (m_Rfs_f_Rcs.object)
    ...
    m_Rcs_f_Ros.init();
  IVP_U_Matrix3 m_Rcs_f_nRrs;
  m_Rcs_f_Ros.mmult3(rot_axes,
                     &m_Rcs_f_nRrs);  // m_Rcs_f_nRrs inverted is the result
...
void IVP_Constraint_Local::limit_rotation_axis(IVP_COORDINATE_INDEX which,
                                               IVP_FLOAT border_left,
                                               IVP_FLOAT border_right) {
  if (m_Rfs_f_Rcs.object)
    m_Rfs_f_Rcs.object->ensure_in_simulation();
...
void IVP_Constraint_Local::change_max_rotation_impulse(
    IVP_CONSTRAINT_FORCE_EXCEED impulsetype, IVP_FLOAT impulse) {
  if (m_Rfs_f_Rcs.object)
    m_Rfs_f_Rcs.object->ensure_in_simulation();
  else
  ...
  maxforce->halfimpulse[3] = maxforce->halfimpulse[4] =
      maxforce->halfimpulse[5] = impulse;
...
void IVP_Constraint_Local::change_Aos_to_relaxe_constraint() {
  if (m_Rfs_f_Rcs.object)
    ...
    m_Afs_f_Acs.object->ensure_in_simulation();
  const IVP_U_Matrix *m_ws_f_Rcs =
      m_Rfs_f_Rcs.object->get_core()->get_m_world_f_core_PSI();
  const IVP_U_Matrix *m_ws_f_Acs =
      m_Afs_f_Acs.object->get_core()->get_m_world_f_core_PSI();
  IVP_U_Matrix m_Rcs_f_Acs;
