 ...
 *			To create an anchor, call
 *IVP_Real_Object::create_anchor()
 ********************************************************************************/
class IVP_Template_Anchor {
 ...
 public:
  ...
  void set_anchor_position_ws(IVP_Real_Object *obj,
                              const IVP_U_Point *coords_ws);
  void set_anchor_position_ws(IVP_Real_Object *obj, const IVP_DOUBLE x,
                              const IVP_DOUBLE y, const IVP_DOUBLE z);
  void set_anchor_position_os(IVP_Real_Object *obj,
                              const IVP_U_Float_Point *coords_os);
  void set_anchor_position_os(IVP_Real_Object *obj, const IVP_DOUBLE x,
                              const IVP_DOUBLE y, const IVP_DOUBLE z);
#if 1
  void set_anchor_position_cs(IVP_Real_Object *obj,
                              const IVP_U_Float_Point *coords_cs);
  void set_anchor_position_cs(IVP_Real_Object *obj, const IVP_DOUBLE x,
                              const IVP_DOUBLE y, const IVP_DOUBLE z);
#endif
 ...
 *			the result is stored either in the info block of the
 *Actuator and optionally in the mod_len and mod_is_outside active_floats
 *
 *	Version Info:	its implementation is very, very fast, so several
 *			ten thousands IVP_Actuator_Check_Dist can be used at the
 *same time.
 *
 ********************************************************************************/
 ...
 *	Description:	Simple force, either static or dynamic (using
 *active_floats)
 ********************************************************************************/
 ...
 *	Description:	Simple torque, either static or dynamic (using
 *active_floats)
 ********************************************************************************/
class IVP_Template_Torque : public IVP_Template_Two_Point {
 public:
  ...
  IVP_FLOAT max_rotation_speed;  // max fabs(rotation speed) the torque will be
                                 // applied
  IVP_U_Active_Float
      *active_float_max_rotation_speed;  // optional override of max_rot_speed
...
class IVP_Template_Rot_Mot : public IVP_Template_Two_Point {
 public:
  ...
  IVP_FLOAT power;       // the power which is applied to the object [J] = [N *
                         // m/sec] neg power means going backwards
  IVP_FLOAT max_torque;  // max absolute torque [n/(m*m)]

  IVP_U_Active_Float
      *active_float_max_rotation_speed;    // optional override of static value
  IVP_U_Active_Float *active_float_power;  // optional override of static value
  IVP_U_Active_Float
      *active_float_max_torque;  // optional override of static value
...
class IVP_Template_Stabilizer : public IVP_Template_Four_Point {
 public:
  IVP_FLOAT stabi_constant; /* in Newton/meter or (if stabi_values_are_relative)
...
class IVP_Anchor {
 ...
 public:
  ...
  void init_anchor(IVP_Actuator *,
                   IVP_Template_Anchor *);  // constructor, called by
                                            // IVP_Real_Object::create_anchor
 ...
 *			Normally they are attached to several (mostly two)
 *objects using anchors. All actuators of one type should normally by grouped by
 *an actuator_manager. At every PSI all actuator_managers execute their
 *actuators code
 ********************************************************************************/
class IVP_Actuator : public IVP_Controller_Dependent {
 ...
 public:
  ...
  IVP_U_Vector<IVP_Core> *get_associated_controlled_cores() override {
    return &actuator_controlled_cores;
  }
  ...
  virtual void anchor_will_be_deleted_event(
      IVP_Anchor *del_anchor);  // when an object is deleted it sends events to
                                // its connected actuators
 ...
 *			sufficient. This is the base class for all such
 *actuators Attention: Version Info:
 ********************************************************************************/
class IVP_Actuator_Two_Point : public IVP_Actuator {
 ...
 public:
  IVP_Actuator_Two_Point(IVP_Environment *,
  ...
  IVP_DOUBLE calc_len();  // calc the distance between both anchors, it's only
                          // valid during the PSI
};
...
class IVP_Actuator_Four_Point : public IVP_Actuator {
 ...
 public:
  ...
  IVP_Actuator_Four_Point(IVP_Environment *,
 ...
 *	Attention:	To create a force use the IVP_Environment->create_force
 *function Constantly changing the force value will cause that the involved
 *objects are never taken out of simulation. (What means unnessary CPU
 *consumption when e.g. invisible trees are rattled unnecessesarily.)
 ********************************************************************************/
...
class IVP_Actuator_Force : public IVP_Actuator_Two_Point {
 ...
 protected:
  ...
  // To create a new force actuator, use the method
  // IVP_Environment->create_force
...
class IVP_Actuator_Force_Active : public IVP_Actuator_Force,
                                  IVP_U_Active_Float_Listener {
  IVP_U_Active_Float *active_float_force;  // optional override of force
 protected:
  void active_float_changed(
      IVP_U_Active_Float *calling_mod) override;  // for wakup
  friend class IVP_Environment;
 ...
 *	Note:		Values can be changed on the fly using
 *IVP_U_Active_Floats
 ********************************************************************************/
...
class IVP_Actuator_Rot_Mot_Active : public IVP_Actuator_Rot_Mot,
  ...
  void active_float_changed(
      IVP_U_Active_Float *calling_mod) override;  // for wakup
 public:
  IVP_Actuator_Rot_Mot_Active(IVP_Environment *env,
 ...
 *			axis. (E.g. car wheels, wind mills)
 *	Note:		Values can be changed on the fly [e.g. by using
 *IVP_U_Active_Floats]
 ********************************************************************************/
...
class IVP_Actuator_Torque_Active : public IVP_Actuator_Torque,
                                   public IVP_U_Active_Float_Listener {
  IVP_U_Active_Float *active_float_max_rotation_speed;
...
class IVP_Listener_Check_Dist_Event {
 public:
  virtual void check_dist_event(IVP_Actuator_Check_Dist *,
                                IVP_BOOL distance_shorter_than_range) = 0;
  virtual void check_dist_is_going_to_be_deleted_event(
      IVP_Actuator_Check_Dist
          *) = 0;  // has to remove itself from IVP_Actuator_Check_Dist
};
...
class IVP_Anchor_Check_Dist : public IVP_Listener_Hull {
 public:
  ...
  IVP_Anchor_Check_Dist()
      : real_object(nullptr),
        l_actuator_check_dist(nullptr),
        object_pos{std::numeric_limits<float>::quiet_NaN(),
                   std::numeric_limits<float>::quiet_NaN(),
                   std::numeric_limits<float>::quiet_NaN()} {}
  void init_anchor_check_dist(IVP_Real_Object *object,
                              IVP_U_Point *position_world_space,
                              IVP_Actuator_Check_Dist *my_act_check_dist);
  virtual ~IVP_Anchor_Check_Dist();
};

/********************************************************************************
 *	Name:	       	IVP_Actuator_Check_Dist
 *	Description:	compares the distance of two check anchors against a
 *predefined distance. Note:		the implementation of this check_dist
 *actuator is pretty fast as long as anchors between check dists are shared.
 *	Version Info:	In future releases check dists may not subclass
...
class IVP_Actuator_Check_Dist {
 ...
 protected:
  ...
  friend class IVP_Environment;
  IVP_Actuator_Check_Dist(IVP_Environment *env,
                          IVP_Template_Check_Dist *check_dist);
 ...
 ********************************************************************************/
class IVP_Actuator_Extra : public IVP_Actuator_Two_Point,
                           public IVP_U_Active_Float_Listener {
 ...
 public:
  ...
  IVP_U_Point *get_float_cam_props(
      IVP_DOUBLE *fc_height_out, IVP_DOUBLE *fc_target_height_out,
      IVP_DOUBLE *fc_dist_out,
      IVP_DOUBLE *fc_speed_out);  // returns &current_float_cam_pos
  ...
  void active_float_changed(
      IVP_U_Active_Float *calling_mod) override;  // for bombs
