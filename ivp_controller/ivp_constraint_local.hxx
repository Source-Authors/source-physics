...
// The Anchor is stored in a certain coords system and it describes from its
// view the axles and the center (fs) vv of m_as_f_bs is the origin of as,
// stored in bs
class IVP_Constraint_Local_Anchor : public IVP_U_Matrix {
 public:
  IVP_Real_Object *object;  // remember object (not core) since cores may be
                            // split up in objects
  IVP_U_Matrix3 *rot;
...
class IVP_Constraint_Local : public IVP_Constraint {
 ...
 private:  // functions
  // void set_orientation(const IVP_U_Matrix3 &m_relatedsystem_f_rotatedsystem,
  // IVP_U_Point *orientation_out); // calculates orientation vector for one
  // matrix
 ...
 protected:
  ...
  void do_simulation_controller(IVP_Event_Sim *es,
 ...
 public:  // constraint changing functions
  ...
  void change_target_translation_axes_Ros(
      const IVP_U_Matrix3 *m_Ros_f_Rfs) override;
  ...
  void limit_translation_axis(IVP_COORDINATE_INDEX which, IVP_FLOAT border_left,
                              IVP_FLOAT border_right) override;
  void change_max_translation_impulse(IVP_CONSTRAINT_FORCE_EXCEED impulsetype,
                                      IVP_FLOAT impulse) override;

  // functions that refer to rotation

  void change_rotation_axes_Ros(const IVP_U_Matrix3 *m_Ros_f_Rrs)
      override;  // set a new rotation axis in both object
  void change_target_rotation_axes_Ros(const IVP_U_Matrix3 *m_Ros_f_Rrs)
      override;  // set a new angular target position of attached object
  void fix_rotation_axis(IVP_COORDINATE_INDEX which) override;
  void free_rotation_axis(IVP_COORDINATE_INDEX which) override;
  void limit_rotation_axis(IVP_COORDINATE_INDEX which, IVP_FLOAT border_left,
                           IVP_FLOAT border_right) override;
  void change_max_rotation_impulse(IVP_CONSTRAINT_FORCE_EXCEED impulsetype,
                                   IVP_FLOAT impulse) override;
