...
class IVP_Raycast_Fake_Jetski_Wheel {
 public:
  ...
  IVP_U_Float_Point spring_direction_cs;  // spring direction in "jetski-space"
                                          // (cs = "car-space")
  ...
  IVP_U_Float_Point
      axis_direction_cs;  // axle direction in "jetski-space" (cs = "car-space")
...
//-----------------------------------------------------------------------------
class IVP_Controller_Raycast_Fake_Jetski_Vector_of_Cores_1
    : public IVP_U_Vector<IVP_Core> {
  void *elem_buffer[1];

 public:
  IVP_Controller_Raycast_Fake_Jetski_Vector_of_Cores_1()
...
// Fake Jetski
//-----------------------------------------------------------------------------
class IVP_Controller_Raycast_Fake_Jetski : public IVP_Car_System,
                                           protected IVP_Controller_Dependent

{
 public:
  // Constructor/Deconstructor.
  IVP_Controller_Raycast_Fake_Jetski(IVP_Environment *environment,
                                     const IVP_Template_Car_System *);
  virtual ~IVP_Controller_Raycast_Fake_Jetski();

  // Steering
  void do_steering_wheel(IVP_POS_WHEEL wheel_pos,
                         IVP_FLOAT s_angle);  // called by do_steering()

  // Car Adjustment
  void change_spring_constant(IVP_POS_WHEEL pos,
                              IVP_FLOAT spring_constant);  // [Newton/meter]
  void change_spring_dampening(
      IVP_POS_WHEEL pos,
      IVP_FLOAT spring_dampening);  // when spring is relaxing spring
  void change_spring_dampening_compression(
      IVP_POS_WHEEL pos,
      IVP_FLOAT spring_dampening);  // [Newton/meter] for compressing spring
  void change_max_body_force(IVP_POS_WHEEL, IVP_FLOAT /*mforce*/) { ; };
  void change_spring_pre_tension(IVP_POS_WHEEL pos,
                                 IVP_FLOAT pre_tension_length);
  void change_spring_length(IVP_POS_WHEEL pos, IVP_FLOAT spring_length);

  void change_stabilizer_constant(IVP_POS_AXIS pos,
                                  IVP_FLOAT stabi_constant);  // [Newton/meter]
  void change_fast_turn_factor(
      IVP_FLOAT fast_turn_factor_);  // not implemented for raycasts
  void change_wheel_torque(IVP_POS_WHEEL pos, IVP_FLOAT torque);

  void update_body_countertorque() { ; };

  void change_body_downforce(
      IVP_FLOAT force);  // extra force to keep flipped objects flipped over

  void fix_wheel(
      IVP_POS_WHEEL,
      IVP_BOOL stop_wheel);  // stop wheel completely (e.g. handbrake )
  void change_friction_of_wheel(IVP_POS_WHEEL pos, IVP_FLOAT friction);

  // Car Info
  IVP_DOUBLE get_body_speed(
      IVP_COORDINATE_INDEX idx_z = IVP_INDEX_Z);  // km/h in 'z' direction
  ...
  virtual void do_steering(
      IVP_FLOAT steering_angle_in);  // default implementation updates
                                     // this->steering_angle

  virtual void set_booster_acceleration(IVP_FLOAT acceleration);
  virtual void activate_booster(IVP_FLOAT thrust, IVP_FLOAT duration,
                                IVP_FLOAT delay);
  ...
  virtual const char *get_controller_name() override {
    return "ivp:controller_raycast_fake_jetski";
  };
 ...
 protected:
  ...
  IVP_Raycast_Fake_Jetski_Wheel
      m_aJetskiWheels[IVP_RAYCAST_FAKE_JETSKI_MAX_WHEELS];  // wheel_of_car
  IVP_Raycast_Fake_Jetski_Axle
      m_aJetskiAxles[IVP_RAYCAST_FAKE_JETSKI_MAX_WHEELS / 2];  // axis_of_jetski
 ...
 protected:
  IVP_Raycast_Fake_Jetski_Wheel *get_wheel(IVP_POS_WHEEL i) {
    return &m_aJetskiWheels[i];
  };
  IVP_Raycast_Fake_Jetski_Axle *get_axle(IVP_POS_AXIS i) {
    return &m_aJetskiAxles[i];
  };
  ...
  virtual void do_simulation_controller(IVP_Event_Sim *,
  ...
  // Simulation.
  void SetupWheelRaycasts(IVP_Ray_Solver_Template *pRaySolverTemplates,
                          const IVP_U_Matrix *m_world_f_core,
                          IVP_Raycast_Fake_Jetski_Wheel_Temp *pTempWheels);
  bool DoSimulationWheels(const IVP_U_Matrix *m_world_f_core,
                          IVP_Raycast_Fake_Jetski_Wheel_Temp *pTempWheels,
                          IVP_Ray_Hit *pRayHits, IVP_FLOAT *pFrictions,
                          IVP_Core *pCarCore);
  void DoSimulationStabilizers(IVP_Raycast_Fake_Jetski_Wheel_Temp *pTempWheels);
  void DoSimulationShocks(IVP_Raycast_Fake_Jetski_Wheel_Temp *pTempWheels,
                          IVP_Ray_Hit *pRayHits, IVP_Event_Sim *pEventSim,
                          IVP_Core *pCarCore);
  void DoSimulationBooster(IVP_Event_Sim *pEventSim, IVP_Core *pCarCore);
  void DoSimulationSteering(IVP_Raycast_Fake_Jetski_Wheel_Temp *pTempWheels,
                            IVP_Core *pCarCore, IVP_Event_Sim *pEventSim);

  void CalcSteeringForces(IVP_Raycast_Fake_Jetski_Wheel_Temp *pTempWheels,
                          IVP_Core *pCarCore, IVP_Event_Sim *pEventSim,
                          IVP_FLOAT *pForcesNeededToDriveStraight);
  void ApplySteeringForces(IVP_Raycast_Fake_Jetski_Wheel_Temp *pTempWheels,
                           IVP_Core *pCareCore, IVP_Event_Sim *pEventSim,
                           IVP_FLOAT *pForcesNeededToDriveStraight);
  IVP_DOUBLE GetSteeringForceRotational(
      int iWheel, IVP_Raycast_Fake_Jetski_Wheel_Temp *pTempWheels,
      IVP_Core *pCarCore, IVP_Event_Sim *pEventSim,
      IVP_FLOAT *pForcesNeededToDriveStraight);

  void TurnInPlaceHack(IVP_Raycast_Fake_Jetski_Wheel_Temp *pTempWheels,
                       IVP_Core *pCarCore, IVP_Event_Sim *pEventSim);

 protected:
  // Pure Virtual
  virtual void do_raycasts(IVP_Event_Sim *, int n_wheels,
