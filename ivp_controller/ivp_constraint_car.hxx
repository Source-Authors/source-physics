...
class IVP_Constraint_Car_Object {
 public:
  ...
  IVP_Constraint_Car_Object(IVP_Constraint_Solver_Car *solver_car,
                            IVP_Real_Object *i_real_obj_app,
                            IVP_Real_Object *i_real_obj_body,
                            IVP_U_Float_Point *target_Bos_override = NULL);
...
class IVP_Constraint_Solver_Car : public IVP_Controller_Dependent {
 public:
  // Constructor/Deconstructor
  IVP_Constraint_Solver_Car(IVP_COORDINATE_INDEX right, IVP_COORDINATE_INDEX up,
  ...
  int get_num_of_appending_terminals() {
    return wheel_objects.len();
  }  // e.g. 4 for four wheels, might be more flexible in the future

  virtual const char *get_controller_name() override {
    return "ivp:constraint_solver_car";
  };
 ...
 public:
  ...
  IVP_Great_Matrix_Many_Zero
      co_matrix;  // inverted matrix for delta vals -> pushes (*time), all in B.
 ...
 protected:
  ...
  // Simulation.
  void do_simulation_controller_rotation(IVP_Event_Sim *es, IVP_Core *core_B,
 ...
 protected:
  ...
  IVP_Constraint *
      c_local_ballsocket[IVP_CONSTRAINT_CAR_MAX_WHEELS];  // remember for plan B
  IVP_Environment *environment;
  int psis_left_for_plan_B;   // internal countdown for deactivating plan B
  IVP_FLOAT max_delta_speed;  // max delta speed allowed for constraint system
                              // inner movements
};
...
class IVP_Constraint_Solver_Car_Builder {
 ...
 public:
  ...
  int n_constraints;  // for matrix size, decreases with num of disabled
                      // constraints

 private:
  void calc_pushing_behavior(int A_obj_idx,
                             int push_vec_idx);  // translation, in init
