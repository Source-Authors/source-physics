...
//-----------------------------------------------------------------------------
void IVP_Controller_Raycast_Airboat::do_simulation_controller(
...
//          the "ground."
//-----------------------------------------------------------------------------
void IVP_Controller_Raycast_Airboat::PreRaycasts(
  ...
  for (int iPoint = 0; iPoint < nPontoonPoints; ++iPoint) {
    ...
    if (pPontoonPoint) {
      ...
      matWorldFromCore->vmult4(&pPontoonPoint->raycast_start_cs,
      ...
      matWorldFromCore->vmult3(&pPontoonPoint->raycast_dir_cs,
                               &pTempPontoons[iPoint].raycast_dir_ws);
...
// Purpose:
//-----------------------------------------------------------------------------
bool IVP_Controller_Raycast_Airboat::PostRaycasts(
    IVP_Ray_Solver_Template *pRaySolverTemplates,
    const IVP_U_Matrix *matWorldFromCore,
    IVP_Raycast_Airboat_Pontoon_Temp *pTempPontoons,
  ...
  for (int iPoint = 0; iPoint < nPontoonPoints; ++iPoint) {
    ...
    IVP_Raycast_Airboat_Wheel *pPontoonPoint = get_wheel(IVP_POS_WHEEL(iPoint));
    IVP_Raycast_Airboat_Pontoon_Temp *pTempPontoonPoint =
    ...
    if (pImpact->bInWater) {
      pTempPontoonPoint->raycast_dir_ws.set_multiple(
          &pTempPontoonPoint->raycast_dir_ws, -1);
    }
    ...
    if (pImpact->bImpact) {
      ...
      vecDelta.subtract(&pImpact->vecImpactPointWS,
                        &pRaySolver->ray_start_point);
      ...
      pTempPontoonPoint->inv_normal_dot_dir =
          1.1f /
          (IVP_Inline_Math::fabsd(pTempPontoonPoint->raycast_dir_ws.dot_product(
               &pTempPontoonPoint->ground_normal_ws)) +
           0.1f);

      // Set the wheel friciton - ground friction (if any) + wheel friction.
      pTempPontoonPoint->friction_value =
          pImpact->flFriction * pPontoonPoint->friction_of_wheel;
    }
    // No impact.
    else {
      ...
      pTempPontoonPoint->ground_normal_ws.set_multiple(
          &pTempPontoonPoint->raycast_dir_ws, -1);
    ...
    pTempPontoonPoint->ground_hit_ws.add_multiple(
        &pRaySolver->ray_start_point, &pTempPontoonPoint->raycast_dir_ws,
        pPontoonPoint->raycast_dist);

    // Get the speed (velocity) at the impact point.
    pAirboatCore->get_surface_speed_ws(
        &pTempPontoonPoint->ground_hit_ws,
        &pTempPontoonPoint->surface_speed_wheel_ws);
    pTempPontoonPoint->projected_surface_speed_wheel_ws.set_orthogonal_part(
        &pTempPontoonPoint->surface_speed_wheel_ws,
        &pTempPontoonPoint->ground_normal_ws);

    matWorldFromCore->vmult3(&pPontoonPoint->axis_direction_cs,
                             &pTempPontoonPoint->axis_direction_ws);
    pTempPontoonPoint->projected_axis_direction_ws.set_orthogonal_part(
        &pTempPontoonPoint->axis_direction_ws,
        &pTempPontoonPoint->ground_normal_ws);
    if (pTempPontoonPoint->projected_axis_direction_ws.normize() == IVP_FAULT) {
      ivp_message(
          "IVP_Controller_Raycast_Airboat::do_simulation_controller "
...
// Purpose:
//-----------------------------------------------------------------------------
void IVP_Controller_Raycast_Airboat::DoSimulationPontoons(
    IVP_Raycast_Airboat_Pontoon_Temp *pTempPontoons,
  ...
  for (int iPoint = 0; iPoint < nPontoonPoints; ++iPoint) {
    IVP_Raycast_Airboat_Wheel *pPontoonPoint = get_wheel(IVP_POS_WHEEL(iPoint));
    if (!pPontoonPoint) continue;

    if (pImpacts[iPoint].bImpact) {
      DoSimulationPontoonsGround(pPontoonPoint, &pTempPontoons[iPoint],
                                 &pImpacts[iPoint], pEventSim, pAirboatCore);
    } else if (pImpacts[iPoint].bInWater) {
      IVP_BOOL bFront = (iPoint < 2) ? IVP_TRUE : IVP_FALSE;
      DoSimulationPontoonsWater(pPontoonPoint, &pTempPontoons[iPoint],
                                &pImpacts[iPoint], pEventSim, pAirboatCore,
                                bFront);
    }
  }
...
//-----------------------------------------------------------------------------
void IVP_Controller_Raycast_Airboat::DoSimulationPontoonsGround(
    IVP_Raycast_Airboat_Wheel *pPontoonPoint,
  ...
  vecSpeedDelta.subtract(&pTempPontoon->projected_surface_speed_wheel_ws,
                         &pTempPontoon->surface_speed_wheel_ws);
...
//-----------------------------------------------------------------------------
void IVP_Controller_Raycast_Airboat::DoSimulationPontoonsWater(
    IVP_Raycast_Airboat_Wheel *pPontoonPoint,
    IVP_Raycast_Airboat_Pontoon_Temp *pTempPontoon,
    IVP_Raycast_Airboat_Impact *pImpact, IVP_Event_Sim *pEventSim,
  ...
  IVP_FLOAT flForce = AIRBOAT_BUOYANCY_SCALAR * 0.25f *
                      pAirboatCore->get_mass() * AIRBOAT_GRAVITY *
                      flVolumeScalar;
...
// Purpose: Water drag.
//-----------------------------------------------------------------------------
void IVP_Controller_Raycast_Airboat::DoSimulationDrag(
    IVP_Raycast_Airboat_Pontoon_Temp *pTempPontoons, IVP_Event_Sim *pEventSim,
    IVP_Core *pAirboatCore) {
  ...
  // Increase drag the more perpendicular the airboat gets to the direction of
  // motion.
  //	IVP_FLOAT flDragTurnScale = vecAirboatDir.dot_product( &vecForwardWS );
  //	flDragTurnScale = ( 1.0f - flDragTurnScale ) + 1.0f;

  // Calculate the drag.
  IVP_FLOAT flDragScale = 0.5f * AIRBOAT_DRAG_COEFFICIENT *
                          AIRBOAT_WATER_DENSITY * AIRBOAT_DRAG_SURFACE_AREA;
...
// Purpose:
//-----------------------------------------------------------------------------
void IVP_Controller_Raycast_Airboat::DoSimulationTurbine(
    IVP_Raycast_Airboat_Pontoon_Temp *pTempPontoons, IVP_Event_Sim *pEventSim,
    IVP_Core *pAirboatCore) {
  // Get the forward vector in world-space.
  IVP_U_Float_Point vecForwardWS;
...
// Purpose:
//-----------------------------------------------------------------------------
void IVP_Controller_Raycast_Airboat::DoSimulationSteering(
    IVP_Raycast_Airboat_Pontoon_Temp *pTempPontoons, IVP_Core *pAirboatCore,
  ...
  if (fabsf(m_SteeringAngle) > 0.01) {
    ...
    flForceRotational = IVP_RAYCAST_AIRBOAT_STEERING_RATE *
                        pAirboatCore->get_mass() * pEventSim->i_delta_time;
  ...
  IVP_FLOAT flRotationalDrag = -0.5f * IVP_RAYCAST_AIRBOAT_STEERING_RATE *
                               pAirboatCore->get_mass() *
...
void IVP_Controller_Raycast_Airboat::do_steering_wheel(IVP_POS_WHEEL wheel_nr,
                                                       IVP_FLOAT s_angle) {
...
void IVP_Controller_Raycast_Airboat::change_spring_constant(
...
void IVP_Controller_Raycast_Airboat::change_spring_dampening(
    IVP_POS_WHEEL pos, IVP_FLOAT spring_dampening) {
  IVP_Raycast_Airboat_Wheel *wheel = get_wheel(pos);
  wheel->spring_damp_relax = spring_dampening;
}

void IVP_Controller_Raycast_Airboat::change_spring_dampening_compression(
    IVP_POS_WHEEL pos, IVP_FLOAT spring_dampening) {
  IVP_Raycast_Airboat_Wheel *wheel = get_wheel(pos);
  wheel->spring_damp_compress = spring_dampening;
}

void IVP_Controller_Raycast_Airboat::change_spring_pre_tension(
...
void IVP_Controller_Raycast_Airboat::change_spring_length(
    IVP_POS_WHEEL pos, IVP_FLOAT spring_length) {
  IVP_Raycast_Airboat_Wheel *wheel = get_wheel(pos);
  wheel->spring_len = spring_length;
}

void IVP_Controller_Raycast_Airboat::change_wheel_torque(IVP_POS_WHEEL pos,
                                                         IVP_FLOAT torque) {
  IVP_Raycast_Airboat_Wheel *wheel = get_wheel(pos);
  wheel->torque = torque;

  // Wake the physics object if need be!
  m_pAirboatBody->get_environment()
...
void IVP_Controller_Raycast_Airboat::fix_wheel(IVP_POS_WHEEL pos,
                                               IVP_BOOL stop_wheel) {
  IVP_Raycast_Airboat_Wheel *wheel = get_wheel(pos);
  wheel->wheel_is_fixed = stop_wheel;
}

void IVP_Controller_Raycast_Airboat::change_friction_of_wheel(
    IVP_POS_WHEEL pos, IVP_FLOAT friction) {
  IVP_Raycast_Airboat_Wheel *wheel = get_wheel(pos);
  wheel->friction_of_wheel = friction;
}

void IVP_Controller_Raycast_Airboat::change_stabilizer_constant(
...
void IVP_Controller_Raycast_Airboat::change_fast_turn_factor(
...
IVP_Controller_Raycast_Airboat::get_controller_priority() {
  return IVP_CP_CONSTRAINTS_MAX;
}

void IVP_Controller_Raycast_Airboat::set_booster_acceleration(
    IVP_FLOAT acceleration) {
  booster_force = acceleration;
}

void IVP_Controller_Raycast_Airboat::activate_booster(IVP_FLOAT thrust,
                                                      IVP_FLOAT duration,
                                                      IVP_FLOAT delay) {
...
void IVP_Controller_Raycast_Airboat::do_steering(IVP_FLOAT steering_angle_in,
                                                 bool bAnalog) {
  // Check for a change.
  if (hk_Math::almost_equal(m_SteeringAngle, steering_angle_in)) return;
...
IVP_DOUBLE IVP_Controller_Raycast_Airboat::get_wheel_angular_velocity(
    IVP_POS_WHEEL pos) {
  IVP_Raycast_Airboat_Wheel *wheel = get_wheel(pos);
  return wheel->wheel_angular_velocity;
}

IVP_DOUBLE IVP_Controller_Raycast_Airboat::get_body_speed(
  ...
  const IVP_U_Matrix *mat_ws =
      m_pAirboatBody->get_core()->get_m_world_f_core_PSI();
...
void IVP_Controller_Raycast_Airboat::get_skid_info(
...
// Purpose:
//-----------------------------------------------------------------------------
void IVP_Controller_Raycast_Airboat::InitRaycastCarEnvironment(
  ...
  IVP_Standard_Gravity_Controller *pGravityController =
      new IVP_Standard_Gravity_Controller();
  ...
  //    extra_gravity = -10.0f *
  //    m_pAirboatBody->get_core()->get_mass();//pCarSystemTemplate->extra_gravity_force_value;
...
// Purpose:
//-----------------------------------------------------------------------------
void IVP_Controller_Raycast_Airboat::InitRaycastCarBody(
    const IVP_Template_Car_System *pCarSystemTemplate) {
  // Car body attributes.
...
// Purpose:
//-----------------------------------------------------------------------------
void IVP_Controller_Raycast_Airboat::InitRaycastCarWheels(
    const IVP_Template_Car_System *pCarSystemTemplate) {
...
// Purpose:
//-----------------------------------------------------------------------------
void IVP_Controller_Raycast_Airboat::InitRaycastCarAxes(
    const IVP_Template_Car_System *pCarSystemTemplate) {
...
//-----------------------------------------------------------------------------
void IVP_Controller_Raycast_Airboat::SetCarSystemDebugData(
...
// Purpose: Debug data for use in vphysics and the engine to visualize car data.
//-----------------------------------------------------------------------------
void IVP_Controller_Raycast_Airboat::GetCarSystemDebugData(
...
void IVP_Controller_Raycast_Airboat::update_booster(
    [[maybe_unused]] IVP_FLOAT booster) {}

void IVP_Controller_Raycast_Airboat::change_max_body_force(
    [[maybe_unused]] IVP_POS_WHEEL wheel, [[maybe_unused]] IVP_FLOAT mforce) {}

void IVP_Controller_Raycast_Airboat::update_body_countertorque() {}

IVP_U_Vector<IVP_Core> *
IVP_Controller_Raycast_Airboat::get_associated_controlled_cores(void) {
  return &vector_of_cores;
}

void IVP_Controller_Raycast_Airboat::core_is_going_to_be_deleted_event(
    IVP_Core *core) {
  P_DELETE_THIS(this);
}

IVP_Raycast_Airboat_Axle *IVP_Controller_Raycast_Airboat::get_axle(
    IVP_POS_AXIS i) {
  return &m_aAirboatAxles[i];
}

IVP_Raycast_Airboat_Wheel *IVP_Controller_Raycast_Airboat::get_wheel(
    IVP_POS_WHEEL i) {
