...
enum IVP_SPRING_FORCE_EXCEED {  // Do not use this enum yet.
  ...
  // IVP_CFE_BEND        // bends itself if force is too great -- not yet
  // implemented
};
 ...
 *	Attention:	Too high spring constants result in higher frequency
 *than PSI rate and will cause high unnatural speeds of objects
 ********************************************************************************/
...
class IVP_Template_Spring : public IVP_Template_Two_Point {
 public:
  IVP_FLOAT spring_len; /* the len of the spring (resulting in no force),
                         * unit: meters
                         * hint: to fix in object in another objects space, use
                         * zero spring len and rel_pos_damp */

  IVP_BOOL spring_values_are_relative; /* set this to IVP_TRUE, if spring values
                                        * should be multiplied with the average
                                        * virtual mass of both objects */
  IVP_BOOL spring_force_only_on_stretch;  // set this to IVP_TRUE, if spring
...
class IVP_Actuator_Spring : public IVP_Actuator_Two_Point {
 ...
 protected:
  ...
  IVP_FLOAT spring_values_factor;  // the following four values are multiplied
                                   // with this factor
  IVP_FLOAT spring_constant;       // already multiplied with factor
  IVP_FLOAT spring_damp;           // the spring damp constant
  IVP_FLOAT rel_pos_damp;   // A damp factor which includes dampening of spring
                            // rotation. This factor is useful to minimize the
                            // number of springs
  IVP_FLOAT break_max_len;  // when spring length exceeds this value,
                            // fire_event_spring_broken is called
  IVP_SPRING_FORCE_EXCEED max_len_exceed_type;
  ...
  IVP_Actuator_Spring(IVP_Environment *, IVP_Template_Spring *,
                      IVP_ACTUATOR_TYPE);
 ...
 public:
  ...
  void add_listener_spring(
      IVP_Listener_Spring *listener);  // NOT_IMPLEMENTED_YET
  void remove_listener_spring(
      IVP_Listener_Spring *listener);  // NOT_IMPLEMENTED_YET
...
class IVP_Actuator_Spring_Active : public IVP_Actuator_Spring,
 ...
 protected:
  ...
  IVP_Actuator_Spring_Active(
      IVP_Environment *env,
      IVP_Template_Spring *spring);  // constructor of the spring
 public:
 ...
 *	Description:	Special spring used for vehicle suspension. Derived from
 *IVP_Actuator_Spring.
 ********************************************************************************/
class IVP_Actuator_Suspension : public IVP_Actuator_Spring {
 protected:
  IVP_FLOAT spring_dampening_compression;  // In vehicles, the dampening is less
                                           // when wheels are pushed upwards. is
                                           // multiplied by spring_values_factor
                                           // The inherited damp_factor is used
                                           // for downward movement.
  IVP_FLOAT max_body_force;  // You can restrict the force exerted at one of the
                             // objects This is used to prevent the body from
                             // jumping too high on side walk borders
 public:
  void set_spring_damp_compression(
      IVP_FLOAT value);  // will be multiplied with factor
