...
IVP_Template_Controller_World_Friction::
  ...
  friction_value_rotation.set(friction_val * 0.3, friction_val * 0.3,
                              friction_val * 0.3);
}

IVP_Controller_World_Friction::IVP_Controller_World_Friction(
    IVP_Real_Object *obj, const IVP_Template_Controller_World_Friction *templ) {
...
void IVP_Controller_World_Friction::do_simulation_controller(
  ...
  //*****************
  {
    ...
    max_friction_correction.set_multiple(&friction_value_translation,
                                         es->delta_time);

    // for each axis clip speed
    for (int i = 0; i < 3; i++) {
      // was fabs, which was a sml call
      if (IVP_Inline_Math::fabsd(correction_speed_cs.k[i]) >
          max_friction_correction.k[i]) {
        int sign_val = 1;
    ...
    my_core->center_push_core_multiple_ws(
        &correction_speed_ws, my_core->get_mass());  // apply impulse
  }
  ...
  //******************
  {
    ...
    correction_rot_speed_cs.subtract(&desired_rot_speed_cs,
                                     &current_cs_rot_speed);

    IVP_U_Float_Point max_friction_correction;
    max_friction_correction.set_multiple(&friction_value_rotation,
                                         es->delta_time);

    // for each axis clip speed
    for (int i = 0; i < 3; i++) {
      // was fabs, which was a sml call
      if (IVP_Inline_Math::fabsd(correction_rot_speed_cs.k[i]) >
          max_friction_correction.k[i]) {
        int sign_val = 1;
    ...
    correction_rot_impulse.k[0] =
        correction_rot_speed_cs.k[0] * my_rot_inertias.k[0];
    correction_rot_impulse.k[1] =
        correction_rot_speed_cs.k[1] * my_rot_inertias.k[1];
    correction_rot_impulse.k[2] =
        correction_rot_speed_cs.k[2] * my_rot_inertias.k[2];

    my_core->rot_push_core_cs(&correction_rot_impulse);
  }
}

void IVP_Controller_World_Friction::core_is_going_to_be_deleted_event(
    IVP_Core *core_i) {
...
IVP_Controller_World_Friction::~IVP_Controller_World_Friction() {
  real_obj->get_environment()
      ->get_controller_manager()
      ->remove_controller_from_core(this, real_obj->get_core());
}
