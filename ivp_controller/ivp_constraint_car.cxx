...
IVP_Constraint_Car_Object::IVP_Constraint_Car_Object(
    IVP_Constraint_Solver_Car *solver_car_, IVP_Real_Object *i_real_obj_app,
    IVP_Real_Object *i_real_obj_body, IVP_U_Float_Point *target_Bos_override) {
...
// Purpose:
//-----------------------------------------------------------------------------
IVP_RETURN_TYPE IVP_Constraint_Solver_Car::init_constraint_system(
    IVP_Environment *env, IVP_Real_Object *body,
  ...
  for (int i = 0; i < wheels.len(); i++) {
    wheel_objects.add(new IVP_Constraint_Car_Object(this, wheels.element_at(i),
                                                    body, p_Bos.element_at(i)));
...
// Purpose:
//-----------------------------------------------------------------------------
void IVP_Constraint_Solver_Car::do_simulation_controller_rotation(
    IVP_Event_Sim *es, IVP_Core *core_B, const IVP_U_Matrix *m_world_f_B) {
  ...
  for (int app_cnt = 0; app_cnt < wheel_objects.len(); app_cnt++) {
    ...
    if (1) {  // new version try
...
#else
        IVP_DOUBLE alpha = core_A->rot_speed.k[x_idx] * es->delta_time *
                           0.45f;  // no angle sign needed
        IVP_DOUBLE sina = angle_sign * IVP_Inline_Math::approx5_sin(alpha);
        IVP_DOUBLE cosa = IVP_Inline_Math::approx5_cos(alpha);
        IVP_DOUBLE ny =
            target_vec_as.k[z_idx] * cosa - target_vec_as.k[y_idx] * sina;
        IVP_DOUBLE nx =
            target_vec_as.k[y_idx] * cosa + target_vec_as.k[z_idx] * sina;
#endif

        cur_angles_B[0] =
            angle_sign * -IVP_Inline_Math::atan2d(ny, target_vec_as.k[x_idx]);
        cur_angles_B[1] =
            angle_sign * IVP_Inline_Math::atan2d(nx, target_vec_as.k[x_idx]);
      }
      ...
      tcb.init_reaction_solver_rotation_ws(core_B, core_A, &y_rot_axis_world2,
                                           &z_rot_axis_world2, NULL);
      ...
      r = IVP_Inline_Math::invert_2x2_matrix(tpm00, tpm01, tpm10, tpm11, &ia1,
                                             &ib1, &ia2, &ib2);
      if (r == IVP_FAULT) {
      ...
      IVP_DOUBLE a =
          -es->i_delta_time * cur_angles_B[0] - tcb.delta_velocity_ds.k[0];
      IVP_DOUBLE b =
          -es->i_delta_time * cur_angles_B[1] - tcb.delta_velocity_ds.k[1];
...
// Purpose:
//-----------------------------------------------------------------------------
void IVP_Constraint_Solver_Car::do_simulation_controller(
  ...
  for (int app_cnt = 0; app_cnt < wheel_objects.len(); app_cnt++) {
    ...
    IVP_U_Point delta_speed_vec;
    {
      ...
      m_world_f_B->vimult4(&m_world_f_A->vv,
                           &cur_pos_A_in_B);  // takes center of app
      ...
      m_world_f_B->vimult3(&core_A->speed,
                           &surspeed_A_in_B);  // takes app center speed
      ...
      next_pos_A_in_B.add_multiple(&cur_pos_A_in_B, &surspeed_A_in_B,
                                   delta_time);
      next_pos_B_in_B.add_multiple(&cur_pos_B_in_B, &surspeed_B_in_B,
                                   delta_time);
      ...
      // difference too high?
      {
        ...
        if (quad_speed > (this->max_delta_speed * this->max_delta_speed)) {
          invalid_count++;
          // IVP_DOUBLE factor = this->max_delta_speed /
          // delta_speed_vec.fast_real_length(); delta_speed_vec.mult(factor);
  ...
  if (init_local_translation == IVP_TRUE) {
    IVP_IF(1) { ivp_message("plan B activated.\n"); }

#if 1
    // appendices are too far away -> start plan B (solve problem with local
    // constraints)
    ...
    for (app_nr = 0; app_nr < wheel_objects.len(); app_nr++) {
      ...
      IVP_U_Point anchorA_Aos;
      anchorA_Aos.set_to_zero();

      IVP_Template_Constraint templ;
      templ.set_ballsocket_tense_Ros(app->real_object, &anchorA_Aos,
                                     body->real_object, &anchorB_Aos);
  ...
  if (this->local_translation_in_use == IVP_FALSE) {
    ...
    for (a_cnt = 0; a_cnt < wheel_objects.len(); a_cnt++) {
      ...
      core_A->push_core_ws(&m_world_f_A->vv,
                           &t_impulse_ws);  // @@@OG use center push
    }
    /*** translation done ***/
  }
}
...
//-----------------------------------------------------------------------------
// Purpose: Performs bilateral test pushes in all 3 coord axis and all 3 rot
// axis
//          using coord system of constraint_obj 'co_obj[0]'.  The resulting
//          delta speed changes are written into this->tmp_matrix.
//-----------------------------------------------------------------------------
void IVP_Constraint_Solver_Car_Builder::calc_pushing_behavior(
    int A_obj_idx, int push_vec_idx) {
  // Verify incoming data.
  IVP_ASSERT(A_obj_idx >= 0 && A_obj_idx < n_appends);
  IVP_ASSERT(push_vec_idx >= 0 && push_vec_idx < 6);
  IVP_ASSERT(this->car_solver->constraint_is_disabled[push_vec_idx] ==
             IVP_FALSE);

  IVP_Constraint_Car_Object *A_obj =
      car_solver->wheel_objects.element_at(A_obj_idx);
  ...
  m_world_f_B->mmult4(
      &A_obj->target_position_bs,
      m_world_f_A);  // m_world_f_A refers to ideal position of A
  ...
  m_B_from_A.vmult4(&A_push_pos_in_A,
                    &B_push_pos_in_B);  // means: objects are regarded as placed
  ...
  // Bilateral Translation Test Pushes of the specified impulse vec in core
  // system of B
  if (push_vec_idx <= 2) {
    /** Push A **/
    IVP_U_Float_Point speed_change_A_world, rot_change_A_in_A;

    core_A->test_push_core(&A_push_pos_in_A, &impulse_A, &impulse_world,
                           &speed_change_A_world, &rot_change_A_in_A);
    ...
    rot_change_A_in_B
        .set_to_zero();  // push in center should not result in rotation

    /**** Push B (contrary) ****/
    core_B->test_push_core(&B_push_pos_in_B, &inv_impulse_B, &inv_impulse_world,
                           &speed_change_B_world, &rot_change_B_in_B);

    // calc speed at push pos
    core_B->get_surface_speed_on_test(&B_push_pos_in_B, &speed_change_B_world,
                                      &rot_change_B_in_B, &sur_speed_B_world);
    m_world_f_B->vimult3(&sur_speed_B_world, &sur_speed_B_in_B);
  }

  // Bilateral Rotation Test Pushes of the specified impulse vec in core system
  // of B
  if (push_vec_idx >= 3) {
    /** Rot push A **/
    IVP_U_Float_Point rot_change_A_in_A;

    core_A->test_rot_push_core_multiple_cs(&impulse_A, rot_impulse_strength,
                                           &rot_change_A_in_A);
    m_B_from_A.vmult3(&rot_change_A_in_A, &rot_change_A_in_B);

    speed_change_A_in_B
        .set_to_zero();  // rot of a in center should not result in translation!

    /**** Rot push B (contrary) ****/
    core_B->test_rot_push_core_multiple_cs(&impulse_B, inv_rot_impulse_strength,
                                           &rot_change_B_in_B);
  ...
  for (int fill_app_idx = 0; fill_app_idx < car_solver->wheel_objects.len();
       fill_app_idx++) {
    ...
    if (fill_app_idx == A_obj_idx) {
      ...
      const IVP_U_Point *app_center_world =
          &this->car_solver->wheel_objects.element_at(fill_app_idx)
               ->get_core()
      ...
      core_B->get_surface_speed_on_test(&app_center_B, &speed_change_B_world,
                                        &rot_change_B_in_B,
                                        &B_at_app_speed_world);
...
IVP_RETURN_TYPE IVP_Constraint_Solver_Car_Builder::calc_constraint_matrix() {
  ...
  this->tmp_matrix.matrix_values =
      (IVP_DOUBLE *)p_malloc(gm_size * gm_size * sizeof(IVP_DOUBLE));
  this->tmp_matrix.desired_vector =
      (IVP_DOUBLE *)p_malloc(gm_size * sizeof(IVP_DOUBLE));
  this->tmp_matrix.result_vector =
      (IVP_DOUBLE *)p_malloc(gm_size * sizeof(IVP_DOUBLE));
  ...
  IVP_DOUBLE *double_vec =
      (IVP_DOUBLE *)p_malloc(gm_size * gm_size * sizeof(IVP_DOUBLE));
  this->car_solver->co_matrix.matrix_values = double_vec;
  this->car_solver->co_matrix.columns = gm_size;

  IVP_RETURN_TYPE ret_val =
      this->tmp_matrix.invert(&this->car_solver->co_matrix);
