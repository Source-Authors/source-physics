...
enum IVP_POS_AXIS { IVP_FRONT = 0, IVP_REAR = 1, IVP_CAR_SYSTEM_MAX_AXIS = 5 };
...
class IVP_Template_Car_System {
 public:
   ...
   ********************************************************************************/
  IVP_Real_Object *
      car_wheel[IVP_CAR_SYSTEM_MAX_WHEELS];  // index according to IVP_POS_WHEEL

  IVP_FLOAT friction_of_wheel
      [IVP_CAR_SYSTEM_MAX_WHEELS];  // to be set for IVP_Controller_Raycast_Car

  IVP_FLOAT wheel_radius[IVP_CAR_SYSTEM_MAX_WHEELS];

  IVP_FLOAT
  wheel_reversed_sign[IVP_CAR_SYSTEM_MAX_WHEELS];  // Default: 1.0f. Use
                                                   // -1.0f when wheels are
                                                   // turned by 180 degrees.

  IVP_FLOAT body_counter_torque_factor;  // (e.g. 0.3f) for nose dive etc.
                                         // produced by wheel torque

  IVP_FLOAT extra_gravity_force_value;    // additional gravity force
  IVP_FLOAT extra_gravity_height_offset;  // force anchor height offset relative
                                          // to center of mass

  IVP_FLOAT body_down_force_vertical_offset;  // vertical offset to mass center
                                              // for tilted object
  ...
  IVP_U_Float_Point
      wheel_pos_Bos[IVP_CAR_SYSTEM_MAX_WHEELS];  // standard position of wheel
                                                 // centers in body's object
                                                 // system
  IVP_U_Float_Point
      trace_pos_Bos[IVP_CAR_SYSTEM_MAX_WHEELS];  // standard position of trace
                                                 // centers in body's object
                                                 // system
  ...
  IVP_FLOAT
  spring_dampening[IVP_CAR_SYSTEM_MAX_WHEELS];  // for releasing springs
  IVP_FLOAT spring_dampening_compression
      [IVP_CAR_SYSTEM_MAX_WHEELS];  // for compressing springs
  IVP_FLOAT
  max_body_force[IVP_CAR_SYSTEM_MAX_WHEELS];  // clipping of spring forces.
                                              // Reduces the jumpy behaviour
                                              // of cars with heavy wheels
  IVP_FLOAT spring_pre_tension[IVP_CAR_SYSTEM_MAX_WHEELS];  // [m] used to keep
                                                            // the wheels at the
                                                            // original position
...
class IVP_Wheel_Skid_Info {
 public:
  ...
  IVP_FLOAT last_skid_value;  // 0 means no skidding, values > 0 means skidding,
                              // check yourself for reasonable ranges
  IVP_Time last_skid_time;
};

struct IVP_CarSystemDebugData_t {
  IVP_U_Point wheelRaycasts[IVP_CAR_SYSTEM_MAX_WHEELS]
                           [2];  // wheels, start = 0, end = 1
  IVP_FLOAT
  wheelRaycastImpacts[IVP_CAR_SYSTEM_MAX_WHEELS];  // wheels, impact raycast
                                                   // floating point min
                                                   // distance
...
class IVP_Car_System {
 public:
  ...
  virtual void do_steering_wheel(
      ...
      IVP_FLOAT s_angle) = 0;  // called by do_steering()

  // Car Adjustment
  virtual void change_spring_constant(
      IVP_POS_WHEEL pos, IVP_FLOAT spring_constant) = 0;  // [Newton/meter]
  virtual void change_spring_dampening(
      IVP_POS_WHEEL pos,
      IVP_FLOAT spring_dampening) = 0;  // when spring is relaxing spring
  virtual void change_spring_dampening_compression(
      IVP_POS_WHEEL pos,
      IVP_FLOAT spring_dampening) = 0;  // [Newton/meter] for compressing spring
  virtual void change_max_body_force(IVP_POS_WHEEL wheel_nr,
                                     IVP_FLOAT mforce) = 0;
  virtual void change_spring_pre_tension(IVP_POS_WHEEL pos,
                                         IVP_FLOAT pre_tension_length) = 0;
  virtual void change_spring_length(IVP_POS_WHEEL wheel_nr,
                                    IVP_FLOAT spring_length) = 0;

  virtual void change_stabilizer_constant(
      IVP_POS_AXIS pos, IVP_FLOAT stabi_constant) = 0;  // [Newton/meter]
  ...
  virtual void
  update_body_countertorque() = 0;  // rotate the body in the opposite direction
                                    // of the wheels

  virtual void update_throttle(IVP_FLOAT flThrottle) = 0;

  virtual void change_body_downforce(
      IVP_FLOAT force) = 0;  // extra force to keep flipped objects flipped over

  virtual void fix_wheel(
      IVP_POS_WHEEL,
      IVP_BOOL stop_wheel) = 0;  // stop wheel completely (e.g. handbrake )

  // Car Info
  virtual IVP_DOUBLE get_body_speed(
      IVP_COORDINATE_INDEX idx_z = IVP_INDEX_Z) = 0;  // km/h in 'z' direction
  virtual IVP_DOUBLE get_wheel_angular_velocity(IVP_POS_WHEEL) = 0;
  virtual void
  update_wheel_positions() = 0;  // move graphical wheels to correct position
  ...
  virtual void set_powerslide([[maybe_unused]] IVP_FLOAT front_accel,
                              [[maybe_unused]] IVP_FLOAT rear_accel) = 0;
  // Tools
  static IVP_FLOAT calc_ackerman_angle(
      IVP_FLOAT alpha, IVP_FLOAT dx,
      IVP_FLOAT dz);  // alpha refers to innermost wheel

  /**** Methods: 2nd Level, based on primitives ****/
  /**** Methods: 2nd Level, based on primitives ****/
  virtual void do_steering(
      IVP_FLOAT steering_angle_in,
      bool bAnalog = false) = 0;  // updates this->steering_angle
  virtual void set_booster_acceleration(
      IVP_FLOAT acceleration) = 0;  // set an additional accerleration force
  virtual void activate_booster(
      IVP_FLOAT thrust, IVP_FLOAT duration,
      IVP_FLOAT recharge_time) = 0;  // set a temporary acceleration force as a
                                     // factor of gravity
  virtual void update_booster(
      IVP_FLOAT delta_time) = 0;  // should be called every frame to allow the
                                  // physics system to deactivate a booster
  ...
  virtual void SetCarSystemDebugData(
      const IVP_CarSystemDebugData_t &carSystemDebugData) = 0;
  virtual void GetCarSystemDebugData(
      IVP_CarSystemDebugData_t &carSystemDebugData) = 0;
...
class IVP_Car_System_Real_Wheels : public IVP_Car_System {
 ...
 protected:
  ...
  IVP_FLOAT body_counter_torque_factor;  // response to wheel torque, e.g. 0.3f
                                         // (for nose dive etc.), call
                                         // update_body_countertorque() when all
                                         // wheel torques are changed

  IVP_FLOAT max_speed;
  IVP_FLOAT
  fast_turn_factor;  // factor to angular accelerates the car if the wheels
                     // are steered ( 0.0f no effect 1.0f strong effect )
  // booster
 ...
 public:
  ...
  void do_steering_wheel(IVP_POS_WHEEL wheel_pos, IVP_FLOAT s_angle)
      override;  // called by do_steering()

  // Car Adjustment
  void change_spring_constant(
      IVP_POS_WHEEL pos, IVP_FLOAT spring_constant) override;  // [Newton/meter]
  void change_spring_dampening(IVP_POS_WHEEL pos, IVP_FLOAT spring_dampening)
      override;  // when spring is relaxing spring
  void change_spring_dampening_compression(IVP_POS_WHEEL pos,
                                           IVP_FLOAT spring_dampening)
      override;  // [Newton/meter] for compressing spring
  void change_max_body_force(IVP_POS_WHEEL wheel_nr, IVP_FLOAT mforce) override;
  void change_spring_pre_tension(IVP_POS_WHEEL pos,
                                 IVP_FLOAT pre_tension_length) override;
  void change_spring_length(IVP_POS_WHEEL wheel_nr,
                            IVP_FLOAT spring_length) override;

  void change_stabilizer_constant(
      IVP_POS_AXIS pos, IVP_FLOAT stabi_constant) override;  // [Newton/meter]
  void change_fast_turn_factor(IVP_FLOAT) override;
  void change_wheel_torque(IVP_POS_WHEEL pos, IVP_FLOAT torque) override;
  void change_wheel_speed_dampening(IVP_POS_WHEEL wheel_nr,
                                    IVP_FLOAT dampening);
  void update_body_countertorque()
      override;  // rotate the body in the opposite direction of the wheels

  void update_throttle(IVP_FLOAT /*flThrottle*/) override {}

  void change_body_downforce(IVP_FLOAT force)
      override;  // extra force to keep flipped objects flipped over

  void fix_wheel(IVP_POS_WHEEL, IVP_BOOL stop_wheel)
      override;  // stop wheel completely (e.g. handbrake )

  // Car Info
  IVP_DOUBLE get_body_speed(IVP_COORDINATE_INDEX idx_z =
                                IVP_INDEX_Z) override;  // km/h in 'z' direction
  ...
  /**** Methods: 2nd Level, based on primitives ****/
  /**** Methods: 2nd Level, based on primitives ****/
  void do_steering(IVP_FLOAT steering_angle_in, bool bAnalog = false)
      override;  // updates this->steering_angle

  void set_booster_acceleration(IVP_FLOAT acceleration) override;
  void activate_booster(IVP_FLOAT thrust, IVP_FLOAT duration,
                        IVP_FLOAT recharge_time) override;
  ...
  void SetCarSystemDebugData(
      const IVP_CarSystemDebugData_t &carSystemDebugData) override;
  void GetCarSystemDebugData(
      IVP_CarSystemDebugData_t &carSystemDebugData) override;
};
