...
class IVP_Template_Controller_Floating {
 ...
 public:
  ...
  IVP_U_Point position_os;       // position where force is exerted (and
                                 // current_distance is measured from)
  ...
  IVP_FLOAT current_distance;  // initial value, later to be fed from outside
                               // (e.g. from ray caster)
};
...
class IVP_Controller_Floating : public IVP_Controller_Independent {
 ...
 public:
  IVP_Controller_Floating(IVP_Real_Object *obj,
                          const IVP_Template_Controller_Floating *templ);
 ...
 protected:
  ...
  virtual const char *get_controller_name() override {
    return "ivp:controller_floating";
  };
  ...
  IVP_U_Float_Point position_os;  // position where force is exerted (and
