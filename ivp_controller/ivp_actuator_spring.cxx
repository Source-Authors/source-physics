...
IVP_Template_Spring::IVP_Template_Spring() {
  ...
  active_float_spring_len = active_float_spring_constant =
      active_float_spring_damp = active_float_spring_rel_pos_damp = nullptr;
}
...
IVP_Actuator_Spring::IVP_Actuator_Spring(IVP_Environment *env,
  ...
  if (spring_templ->spring_values_are_relative) {
    ...
    spring_values_factor =
        (min_mass_0 * min_mass_1) / (min_mass_0 + min_mass_1);
  }
  ...
  this->spring_force_only_on_stretch =
      spring_templ->spring_force_only_on_stretch;
}
...
IVP_Actuator_Spring_Active::IVP_Actuator_Spring_Active(
    IVP_Environment *env, IVP_Template_Spring *spring_templ)
  ...
  active_float_spring_rel_pos_damp =
      spring_templ->active_float_spring_rel_pos_damp;
...
void IVP_Actuator_Spring::do_simulation_controller(
  ...
  if (!spring_force_only_on_stretch || dlen > this->spring_len) {
    ...
    IVP_Core::get_diff_surface_speed_of_two_cores(
        pc1, pc0, &anch1->core_pos, &anch0->core_pos, &relative_world_speed);

    IVP_DOUBLE damp_speed = dir_ws.dot_product(
        &relative_world_speed);  // positive means closing each other
    ...
    {  // manhattan damping
      ...
      force_vector_ws.add_multiple(&relative_world_speed,
                                   manhattan_factor);  // manhattan damping
    }
...
IVP_Actuator_Suspension::IVP_Actuator_Suspension(IVP_Environment *env,
  ...
  // set mass adapted spring constant
  this->spring_dampening_compression =
      templ->spring_dampening_compression * spring_values_factor;
...
void IVP_Actuator_Suspension::do_simulation_controller(
    IVP_Event_Sim *es, IVP_U_Vector<IVP_Core> * /*core_list*/) {
  ...
  IVP_Core::get_diff_surface_speed_of_two_cores(
      pc1, pc0, &anch1->core_pos, &anch0->core_pos, &relative_world_speed);
  IVP_DOUBLE damp_speed = relative_world_speed.dot_product(
      &dir_ws);  // positive means closing each other

  if (0) {  // Version with new IVP_Solver_Core_Reaction
    IVP_Solver_Core_Reaction tcb;
    IVP_U_Float_Point direction(&dir_ws);
    tcb.init_reaction_solver_translation_ws(pc1, pc0, pos0_ws, &direction, 0,
                                            0);
    ivp_message("suspension new %f old %f\n", tcb.delta_velocity_ds.k[0],
                damp_speed);
  }
