 ...
 ********************************************************************************/
void IVP_Car_System_Real_Wheels::change_spring_dampening(
    IVP_POS_WHEEL wheel_nr, IVP_FLOAT damp_factor) {
 ...
 *	Description:		object is ensured to be in simulation
 ********************************************************************************/
void IVP_Car_System_Real_Wheels::change_max_body_force(IVP_POS_WHEEL wheel_nr,
                                                       IVP_FLOAT mforce) {
  IVP_Actuator_Suspension *spring = this->car_spring[(int)wheel_nr];
 ...
 *	Description:		object is ensured to be in simulation
 ********************************************************************************/
void IVP_Car_System_Real_Wheels::change_spring_dampening_compression(
    IVP_POS_WHEEL wheel_nr, IVP_FLOAT damp_factor) {
  IVP_Actuator_Suspension *spring = this->car_spring[(int)wheel_nr];
 ...
 *	Description:		object is ensured to be in simulation
 ********************************************************************************/
void IVP_Car_System_Real_Wheels::change_spring_pre_tension(
    IVP_POS_WHEEL wheel_nr, IVP_FLOAT pre_tension_len) {
  IVP_Actuator_Suspension *spring = this->car_spring[(int)wheel_nr];
 ...
 *	Description:		object is ensured to be in simulation
 ********************************************************************************/
void IVP_Car_System_Real_Wheels::change_spring_length(IVP_POS_WHEEL wheel_nr,
 ...
 *	Description:		object is ensured to be in simulation
 ********************************************************************************/
void IVP_Car_System_Real_Wheels::change_stabilizer_constant(
    IVP_POS_AXIS axis_nr, IVP_FLOAT stabi_constant) {
  IVP_Actuator_Stabilizer *stabi = this->car_stabilizer[(int)axis_nr];
  stabi->set_stabi_constant(stabi_constant);
}

void IVP_Car_System_Real_Wheels::change_fast_turn_factor(
 ...
 *	Description:		object is ensured to be in simulation
 ********************************************************************************/
void IVP_Car_System_Real_Wheels::change_wheel_torque(IVP_POS_WHEEL wheel_nr,
                                                     IVP_FLOAT torque) {
 ...
 *	Description:		object is ensured to be in simulation
 ********************************************************************************/
void IVP_Car_System_Real_Wheels::change_wheel_speed_dampening(
    IVP_POS_WHEEL wheel_nr, IVP_FLOAT dampening) {
 ...
 *	Description:		Updates countertorque resulting from the torques
...
void IVP_Car_System_Real_Wheels::do_steering_wheel(IVP_POS_WHEEL wheel_pos,
                                                   IVP_FLOAT s_angle) {
  int wheel_nr = (int)wheel_pos;

  IVP_Constraint_Solver_Car *cs_car = this->car_constraint_solver;

  IVP_U_Matrix *target_mat =
      &cs_car->wheel_objects.element_at(wheel_nr)->target_position_bs;
...
void IVP_Car_System_Real_Wheels::do_steering(IVP_FLOAT s_angle, bool bAnalog) {
  ...
  IVP_DOUBLE angular_spin =
      d_alpha * get_body_speed(IVP_COORDINATE_INDEX(cs_car->z_idx)) / dz_axles;
  car_body->get_core()
      ->rot_speed_change.k[IVP_COORDINATE_INDEX(cs_car->y_idx)] -=
      angular_spin * this->fast_turn_factor;
  ...
  for (int i = 0; i < wheels_per_axis; i++) { /* turn front wheels */
    if ((i & 1) == (s_angle > 0.0f)) {
      ...
      IVP_FLOAT s_angle_2;
      if (n_wheels >= 4) {
        s_angle_2 = this->calc_ackerman_angle(steering_angle, dx_front_wheels,
                                              dz_axles);
      } else {
 ...
 *	Description:		object is ensured to be in simulation
 ********************************************************************************/
void IVP_Car_System_Real_Wheels::change_spring_constant(IVP_POS_WHEEL wheel_nr,
                                                        IVP_FLOAT constant) {
  IVP_Actuator_Suspension *spring = this->car_spring[(int)wheel_nr];
  spring->set_constant(constant);
}

IVP_DOUBLE IVP_Car_System_Real_Wheels::get_body_speed(
...
IVP_DOUBLE IVP_Car_System_Real_Wheels::get_orig_front_wheel_distance() {
  IVP_U_Matrix *m_left_wheel_bs =
      &this->car_constraint_solver->wheel_objects.element_at(0)
           ->target_position_bs;
  IVP_U_Matrix *m_right_wheel_bs =
      &this->car_constraint_solver->wheel_objects.element_at(1)
           ->target_position_bs;

  IVP_DOUBLE dist =
      m_left_wheel_bs->get_position()->k[this->car_constraint_solver->x_idx] -
...
IVP_DOUBLE IVP_Car_System_Real_Wheels::get_orig_axles_distance() {
  IVP_U_Matrix *m_front_wheel_bs =
      &this->car_constraint_solver->wheel_objects.element_at(0)
           ->target_position_bs;
  IVP_U_Matrix *m_rear_wheel_bs =
      &this->car_constraint_solver->wheel_objects.element_at(2)
           ->target_position_bs;

  IVP_DOUBLE dist =
      m_front_wheel_bs->get_position()->k[this->car_constraint_solver->z_idx] -
...
IVP_DOUBLE IVP_Car_System_Real_Wheels::get_wheel_angular_velocity(
    IVP_POS_WHEEL i) {
  return this->car_act_torque[i]->rot_speed_out;
}

IVP_Car_System_Real_Wheels::IVP_Car_System_Real_Wheels(
    IVP_Environment *env, IVP_Template_Car_System *templ) {
  ...
  ////////    CONSTRAINT SYSTEM    //////////////////////////////////
  ///////////////////////////////////////////////////////////////////
  this->car_constraint_solver = new IVP_Constraint_Solver_Car(
      templ->index_x, templ->index_y, templ->index_z, templ->is_left_handed);
  ...
  for (wheel_nr = 0; wheel_nr < n_wheels; wheel_nr++) {
    ...
    spring_template.spring_dampening_compression =
        templ->spring_dampening_compression[wheel_nr];
    ...
    // @@@OG os origin does not need to be the center! Better use core system's
    // origin!
    anchor_wheel_template[wheel_nr].set_anchor_position_os(car_wheel[wheel_nr],
                                                           0.0f, 0.0f, 0.0f);
    ...
    this->car_spring[wheel_nr] =
        environment->create_suspension(&spring_template);
    ...
    // defaults
    torque_template.max_rotation_speed =
        templ->wheel_max_rotation_speed[(wheel_nr & 2) ? 1 : 0];
    ...
    torque_template.torque =
        0.0f;  // This wasn't here, is this why we started with force?

    this->car_act_torque[wheel_nr] =
        environment->create_torque(&torque_template);
  ...
  if (n_wheels != n_axis) {
    for (int ii = 0; ii < 2; ii++) {
      ...
      // set default
      stabi_template.stabi_constant =
          templ->stabilizer_constant[ii];  // Newton/meter
      ...
      this->car_stabilizer[ii] =
          environment->create_stabilizer(&stabi_template);
    }
  }
  ...
  hp.set_to_zero();
  hp.k[cs_car->y_idx] +=
      templ->body_down_force_vertical_offset;  // offset for arbitrary rotation
                                               // (e.g. when in air)
  ...
  anchor_left_template.set_anchor_position_cs(car_body, &hp);

  hp.k[cs_car->x_idx] = 1.0f;
  hp.k[cs_car->z_idx] = 2.0f;

  anchor_right_template.set_anchor_position_cs(car_body, &hp);

  force_powerslide_template.anchors[0] = &anchor_left_template;
  force_powerslide_template.anchors[1] = &anchor_right_template;
  force_powerslide_template.force = 0.0f;

  this->car_act_powerslide_back =
      environment->create_force(&force_powerslide_template);
...
void IVP_Car_System_Real_Wheels::get_skid_info(
...
void IVP_Car_System_Real_Wheels::set_powerslide(IVP_FLOAT front_accel,
                                                IVP_FLOAT rear_accel) {
  car_act_powerslide_back->set_force(car_body->get_core()->get_mass() *
                                     front_accel);
  car_act_powerslide_front->set_force(car_body->get_core()->get_mass() *
                                      rear_accel);
}

// stop wheel completely (e.g. handbrake )
void IVP_Car_System_Real_Wheels::fix_wheel(IVP_POS_WHEEL wheel_nr,
                                           IVP_BOOL stop_wheel) {
  if (!stop_wheel) {
    car_constraint_solver->wheel_objects.element_at(wheel_nr)
        ->fix_wheel_constraint = NULL;
  ...
  // Already activated.
  if (fix_wheel_constraint[wheel_nr]) return;
  ...
  car_constraint_solver->wheel_objects.element_at(wheel_nr)
      ->fix_wheel_constraint = this->fix_wheel_constraint[wheel_nr];
}

/********************************************************************************
 *	Name:	      	activate_booster
 *	Description:	'Rocket' propulsion (in contrary to the standard wheel
 *drive)
 ********************************************************************************/
void IVP_Car_System_Real_Wheels::activate_booster(IVP_FLOAT thrust,
                                                  IVP_FLOAT duration,
                                                  IVP_FLOAT delay) {
  // Do we have an booster?
  if (booster_actuator[0]) return;

  // Booster not ready yet.
  if (booster_seconds_until_ready > 0.0f) return;
...
void IVP_Car_System_Real_Wheels::set_booster_acceleration(
  ...
  if (acceleration) {
    ...
    if (!booster_actuator[0]) {
      ...
      forceTempUp.force = -1.0f * pCarBody->get_core()->get_mass() *
                          environment->get_gravity()->k[pCarConstraint->y_idx];

      booster_actuator[0] = environment->create_force(&forceTempForward);
      booster_actuator[1] = environment->create_force(&forceTempUp);
    } else {
      booster_actuator[0]->set_force(acceleration *
                                     pCarBody->get_core()->get_mass());
    }
  } else {
...
void IVP_Car_System_Real_Wheels::environment_will_be_deleted(
    IVP_Environment *) {
  delete this;
}

IVP_FLOAT IVP_Car_System::calc_ackerman_angle(IVP_FLOAT alpha, IVP_FLOAT dx,
                                              IVP_FLOAT dz) {
  ...
  IVP_FLOAT a =
      IVP_Inline_Math::fabsd(alpha);  // was fabs, which was a sml call
...
//-----------------------------------------------------------------------------
void IVP_Car_System_Real_Wheels::SetCarSystemDebugData(
...
// Purpose: Debug data for use in vphysics and the engine to visualize car data.
//-----------------------------------------------------------------------------
void IVP_Car_System_Real_Wheels::GetCarSystemDebugData(
