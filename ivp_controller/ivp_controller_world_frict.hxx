...
class IVP_Template_Controller_World_Friction {
 public:
  IVP_U_Point desired_speed_ws;  // desired speed greater zero could be used on
                                 // e.g. moving platforms
  ...
  // warning: these friction values are different to friction values of normal
  // objects.
  //          normally a friction value is the tan( alpha ) , were alpha is
  //          degree of steepest angle were object does not slide here the
  //          friction value is the maximum speed (m/s) that is allowed to be
  //          reduced per second 9.81 m/s means that gravity is totally
  //          neutralized
...
class IVP_Controller_World_Friction : public IVP_Controller_Independent {
 protected:
  ...
  virtual const char *get_controller_name() override {
    return "ivp:controller_world_friction";
  };

  friend class IVP_Environment;

  // --------------------------------------------------------
  // The following settings may be changed in the mainloop e.g. using the
 ...
 public:
  ...
  IVP_Controller_World_Friction(
      IVP_Real_Object *obj,
      const IVP_Template_Controller_World_Friction *templ);
