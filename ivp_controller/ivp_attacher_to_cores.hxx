 ...
 *				ATTACH_T::ATTACH_T( IVP_Attacher_To_Cores *,
 *IVP_Core *) and call attachment_is_going_to_be_deleted in it's destructor
 *			ATTACH_T controlles just one core
 *      Note:           ATTACH_T is resonsible for adding itself to the core in
 *the constructor and removing itself from the core at the destructor It also
 *must listen to the core for IVP_Core::~IVP_Core Only one attachment per core
 *is allowed Important Note: The destructor of ATTACH_T     M U S T   call
...
class IVP_Attacher_To_Cores : protected IVP_Listener_Set_Active<IVP_Core> {
 ...
 public:
  ...
  IVP_Attacher_To_Cores(IVP_U_Set_Active<IVP_Core> *set_of_cores_in)
      : core_to_attachment_hash(16) {
