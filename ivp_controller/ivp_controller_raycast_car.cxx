...
//-----------------------------------------------------------------------------
void IVP_Controller_Raycast_Car::do_simulation_controller(
  ...
  const IVP_U_Matrix *m_world_f_core =
      car_body->get_core()->get_m_world_f_core_PSI();
  ...
  // Wheels.
  if (!DoSimulationWheels(rst, m_world_f_core, wt, ray_hits, frictions,
                          car_core))
    return;
  ...
  // Extra gravity.
  {
    car_core->speed.add_multiple(
        &normized_gravity_ws,
        extra_gravity * car_core->get_inv_mass() * es->delta_time);
  }
...
//          the "ground."
//-----------------------------------------------------------------------------
void IVP_Controller_Raycast_Car::SetupWheelRaycasts(
  ...
  for (int iWheel = 0; iWheel < n_wheels; ++iWheel) {
    ...
    if (pWheel) {
      ...
      m_world_f_core->vmult4(&pWheel->hp_cs,
...
// Purpose:
//-----------------------------------------------------------------------------
bool IVP_Controller_Raycast_Car::DoSimulationWheels(
    IVP_Ray_Solver_Template *pRaySolverTemplates,
    const IVP_U_Matrix *m_world_f_core, IVP_Raycast_Car_Wheel_Temp *pTempWheels,
  ...
  for (int iWheel = 0; iWheel < n_wheels; ++iWheel) {
    ...
    if (pRayHit->hit_real_object) {
      ...
      if (pCacheObject) {
        ...
        // IVP_ASSERT( wheel->raycast_dist <= wheel->spring_len +
        // wheel->wheel_radius);
    ...
    pTempWheel->ground_hit_ws.add_multiple(
        &pRaySolverTemplates[iWheel].ray_start_point,
        &pTempWheel->spring_direction_ws, pWheel->raycast_dist);
    ...
    if (pTempWheel->projected_axis_direction_ws.normize() == IVP_FAULT) {
      IVP_IF(1) {
        ivp_message(
            "IVP_Controller_Raycast_Car::do_simulation_controller "
  ...
  if (wheels_per_axis == 2) {
    for (unsigned int iAxis = 0; iAxis < (unsigned int)n_axis; ++iAxis) {
      ...
      pTempWheels[2 * iAxis].stabilizer_force =
          (flDiff1 - flDiff0) *
          (get_axis(IVP_POS_AXIS(iAxis))->stabilizer_constant * 0.5f);
...
// Purpose:
//-----------------------------------------------------------------------------
void IVP_Controller_Raycast_Car::DoSimulationShocks(
    IVP_Raycast_Car_Wheel_Temp *pTempWheels, IVP_Ray_Hit *pRayHits,
...
// Purpose:
//-----------------------------------------------------------------------------
void IVP_Controller_Raycast_Car::DoSimulationBooster(IVP_Event_Sim *pEventSim,
                                                     IVP_Core *pCarCore) {
...
// Purpose:
//-----------------------------------------------------------------------------
void IVP_Controller_Raycast_Car::DoSimulationSteering(
    IVP_Raycast_Car_Wheel_Temp *pTempWheels, IVP_Core *pCarCore,
...
// Purpose:
//-----------------------------------------------------------------------------
void IVP_Controller_Raycast_Car::CalcSteeringForces(
    IVP_Raycast_Car_Wheel_Temp *pTempWheels, IVP_Core *pCarCore,
  ...
  IVP_RETURN_TYPE retValue = IVP_Inline_Math::invert_2x2_matrix(
      mtx2x2_00, mtx2x2_01, mtx2x2_10, mtx2x2_11, &inv_mat2x2[0],
...
// Purpose:
//-----------------------------------------------------------------------------
void IVP_Controller_Raycast_Car::ApplySteeringForces(
    IVP_Raycast_Car_Wheel_Temp *pTempWheels, IVP_Core *pCarCore,
    IVP_Event_Sim *pEventSim, IVP_FLOAT *pForcesNeededToDriveStraight) {
  ...
  for (iWheel = 0; iWheel < n_wheels; ++iWheel) {
    ...
    IVP_FLOAT flQuadSumForce =
        flFrForce * flFrForce + flForceStraight * flForceStraight;
    ...
    impulse.set_multiple(&pTempWheel->projected_axis_direction_ws,
                         flForceStraight * pEventSim->delta_time);
...
void IVP_Controller_Raycast_Car::do_steering_wheel(IVP_POS_WHEEL wheel_nr,
                                                   IVP_FLOAT s_angle) {
...
void IVP_Controller_Raycast_Car::change_spring_constant(
...
void IVP_Controller_Raycast_Car::change_spring_dampening(
    IVP_POS_WHEEL pos, IVP_FLOAT spring_dampening) {
  IVP_Raycast_Car_Wheel *wheel = get_wheel(pos);
  wheel->spring_damp_relax = spring_dampening;
}

void IVP_Controller_Raycast_Car::change_spring_dampening_compression(
    IVP_POS_WHEEL pos, IVP_FLOAT spring_dampening) {
  IVP_Raycast_Car_Wheel *wheel = get_wheel(pos);
  wheel->spring_damp_compress = spring_dampening;
}

void IVP_Controller_Raycast_Car::change_spring_pre_tension(
...
void IVP_Controller_Raycast_Car::change_spring_length(IVP_POS_WHEEL pos,
                                                      IVP_FLOAT spring_length) {
  IVP_Raycast_Car_Wheel *wheel = get_wheel(pos);
  wheel->spring_len = spring_length;
}

void IVP_Controller_Raycast_Car::change_wheel_torque(IVP_POS_WHEEL pos,
                                                     IVP_FLOAT torque) {
  IVP_Raycast_Car_Wheel *wheel = get_wheel(pos);
  wheel->torque = torque;

  // Wake the physics object if need be!
  car_body->get_environment()
      ->get_controller_manager()
      ->ensure_controller_in_simulation(this);
}

void IVP_Controller_Raycast_Car::fix_wheel(IVP_POS_WHEEL pos,
                                           IVP_BOOL stop_wheel) {
  IVP_Raycast_Car_Wheel *wheel = get_wheel(pos);
  wheel->wheel_is_fixed = stop_wheel;
}

void IVP_Controller_Raycast_Car::change_stabilizer_constant(
    IVP_POS_AXIS pos, IVP_FLOAT stabi_constant) {
  IVP_Raycast_Car_Axis *axis = get_axis(pos);
  axis->stabilizer_constant = stabi_constant;
}

void IVP_Controller_Raycast_Car::change_fast_turn_factor(
...
void IVP_Controller_Raycast_Car::set_booster_acceleration(
    IVP_FLOAT acceleration) {
  booster_force = acceleration;
}

void IVP_Controller_Raycast_Car::activate_booster(IVP_FLOAT thrust,
                                                  IVP_FLOAT duration,
                                                  IVP_FLOAT delay) {
...
void IVP_Controller_Raycast_Car::do_steering(IVP_FLOAT steering_angle_in,
  ...
  if (hk_Math::almost_equal(steering_angle, steering_angle_in)) return;
...
IVP_DOUBLE IVP_Controller_Raycast_Car::get_wheel_angular_velocity(
    IVP_POS_WHEEL pos) {
  IVP_Raycast_Car_Wheel *wheel = get_wheel(pos);
  return wheel->wheel_angular_velocity;
}

IVP_DOUBLE IVP_Controller_Raycast_Car::get_body_speed(
...
void IVP_Controller_Raycast_Car::get_skid_info(
...
void IVP_Controller_Raycast_Car::set_powerslide(
    [[maybe_unused]] IVP_FLOAT front_accel,
    [[maybe_unused]] IVP_FLOAT rear_accel) {}
...
// Purpose:
//-----------------------------------------------------------------------------
void IVP_Controller_Raycast_Car::InitRaycastCarEnvironment(
...
// Purpose:
//-----------------------------------------------------------------------------
void IVP_Controller_Raycast_Car::InitRaycastCarBody(
    const IVP_Template_Car_System *pCarSystemTemplate) {
  // Car body attributes.
...
// Purpose:
//-----------------------------------------------------------------------------
void IVP_Controller_Raycast_Car::InitRaycastCarWheels(
    const IVP_Template_Car_System *pCarSystemTemplate) {
...
// Purpose:
//-----------------------------------------------------------------------------
void IVP_Controller_Raycast_Car::InitRaycastCarAxes(
    const IVP_Template_Car_System *pCarSystemTemplate) {
...
//-----------------------------------------------------------------------------
void IVP_Controller_Raycast_Car::SetCarSystemDebugData(
...
// Purpose: Debug data for use in vphysics and the engine to visualize car data.
//-----------------------------------------------------------------------------
void IVP_Controller_Raycast_Car::GetCarSystemDebugData(
