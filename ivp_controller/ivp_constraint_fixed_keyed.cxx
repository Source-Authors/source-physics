...
IVP_Constraint_Fixed_Keyframed::~IVP_Constraint_Fixed_Keyframed() {
  IVP_Controller_Manager::remove_controller_from_environment(
      this, IVP_TRUE);  // delete silently
}

void IVP_Constraint_Fixed_Keyframed::core_is_going_to_be_deleted_event(
    IVP_Core *) {
  P_DELETE_THIS(this);
}
...
void IVP_Constraint_Fixed_Keyframed::ensure_in_simulation() {
  get_environment()->get_controller_manager()->ensure_controller_in_simulation(
      this);
}

/*
IVP_Template_Constraint
&ivp_create_template_for_constraint_local(IVP_Real_Object *reference_object,
                                                                 IVP_Real_Object
*attached_object, const IVP_Template_Constraint_Fixed_Keyframed *tin){ static
IVP_Template_Constraint t; t.set_fixed( reference_object, attached_object);

        IVP_Cache_Object *co_attached =
attached_object->get_cache_object_no_lock(); IVP_U_Point co_pos_ws(
co_attached->m_world_f_object.get_position());

        IVP_U_Point co_pos_Ros;
        IVP_Cache_Object *co_reference =
reference_object->get_cache_object_no_lock();
        co_reference->transform_position_to_object_coords( & co_pos_ws,
&co_pos_Ros);
...
IVP_Constraint_Fixed_Keyframed::IVP_Constraint_Fixed_Keyframed(
    IVP_Real_Object *reference_object, IVP_Real_Object *attached_object,
    const IVP_Template_Constraint_Fixed_Keyframed *t)
//: IVP_Constraint_Local(
//: ivp_create_template_for_constraint_local(reference_object,attached_object,t
//: ))
{
  ...
  co_reference->m_world_f_object.mimult4(&co_attached->m_world_f_object,
                                         &m_Ros_f_Aos);
...
void IVP_Constraint_Fixed_Keyframed::do_simulation_controller(
  ...
  IVP_Core *core_A = (reference_obj->get_core()->physical_unmoveable)
                         ? NULL
                         : reference_obj->get_core();
  IVP_Core *core_B = (attached_obj->get_core()->physical_unmoveable)
                         ? NULL
                         : attached_obj->get_core();
  ...
  // orientation
  {
    ...
    if (angular_velocity_set) {
      new_q_Ros_f_Rrs.set_interpolate_smoothly(
    ...
    new_q_Ros_f_Rrs.set_matrix(&m_Ros_f_Rrs);
    IVP_U_Matrix3 m_Rws_f_Rrs;
    m_Rws_f_Ros->mmult3(&m_Ros_f_Rrs, &m_Rws_f_Rrs);
    IVP_U_Matrix3 m_Rws_f_Aws;
    m_Aws_f_Aos->mi2mult3(&m_Rws_f_Rrs, &m_Rws_f_Aws);
    IVP_U_Quat q;
    q.set_quaternion(&m_Rws_f_Aws);
    ...
    tcb.init_reaction_solver_rotation_ws(core_B, core_A, &x_rot_axis_world,
                                         &y_rot_axis_world, &z_rot_axis_world);
    ...
    delta_angles.add_multiple(&tcb.delta_velocity_ds,
  ...
  // position
  {
    ...
    new_target_position_Ros.add_multiple(&prime_position_Ros, &velocity_Ros,
                                         dt + es->delta_time);

    IVP_U_Point target_pos_Ws;
    m_Rws_f_Ros->vmult4(&new_target_position_Ros, &target_pos_Ws);

    IVP_U_Float_Point delta_position_ws;
    delta_position_ws.inline_subtract_and_mult(
        &target_pos_Ws, m_Aws_f_Aos->get_position(),
        this->force_factor * es->i_delta_time);

    delta_position_ws.add_multiple(&reference_obj->get_core()->speed,
                                   damp_factor);
    delta_position_ws.add_multiple(&attached_obj->get_core()->speed,
                                   -damp_factor);
    ...
    tcb.init_reaction_solver_translation_ws(
        core_B, core_A, target_pos_Ws, &x_rot_axis_world, &y_rot_axis_world,
        &z_rot_axis_world);
...
void IVP_Constraint_Fixed_Keyframed::set_prime_position_Ros(
    const IVP_U_Point *position, const IVP_U_Float_Point *velocity,
    const IVP_Time &time) {
...
void IVP_Constraint_Fixed_Keyframed::set_prime_orientation_Ros(
    const IVP_U_Quat *orientation0, const IVP_Time &time0,
    const IVP_U_Quat *orientation1, const IVP_FLOAT dt) {
