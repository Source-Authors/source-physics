...
class IVP_Template_Buoyancy {
 public:
  IVP_BOOL
  simulate_wing_behavior;  // if set to IVP_TRUE then the dampening forces
                           // of each triangle will be directed in negative
                           // normal direction of this triangle instead of
                           // the direction of the medium's current.

  IVP_FLOAT medium_density;        // density of the medium in 'kg/m^3'
  IVP_FLOAT pressure_damp_factor;  // resistance constant that influences the
                                   // strength of the pressure dampening (caused
                                   // by suction behind object)
  IVP_FLOAT
  friction_damp_factor;  // resistance constant (depending on the object's
                         // shape) that influences the strength of the
                         // friction dampening (caused by push tension)
  IVP_FLOAT
  torque_factor;  // influences how much the point the dampening impulse has
                  // an effect on will be shifted towards the current
  IVP_FLOAT buoyancy_eps;  // denotes an epsilon value

  IVP_FLOAT ball_rot_dampening_factor;  // influences the strength of dampening
                                        // of a ball's rotation, which can
                                        // result e.g. from the ball not falling
                                        // straightly into the medium
  IVP_FLOAT viscosity_factor;  // viscosity_of_medium = 1 - (viscosity_factor *
                               // delta_psi_time)
  IVP_FLOAT viscosity_input_factor;  // defines how much influence the viscosity
                                     // of the fluid will have on the object;
                                     // depends on aerodynamic shape of object

  IVP_BOOL
  use_interpolation;  // defines if the fluid dynamics are calculated anew
                      // in every PSI step or if we try to interpolate them
                      // from values calculated in PSIs before
  int max_interpolation_tries;  // defines how many interpolations will be tried
                                // in controller_buoyancy before the computation
                                // (via the buoyancy_solver) will be forced
  int max_tries_nr_of_vectors_involved;  // defines how many interpolations will
                                         // be tried in controller_buoyancy with
                                         // the same or an increasing number of
                                         // involved vectors
  ...
  insert_extrapol_only;  // defines if in case the MI failed to interpolate
                         ...
                         // based on the calculation of the buoyancy solver
                         // should be inserted into 'previous_solutions' or
                         // if additionally also the buoyancy solver's
                         // calculated solution is inserted into
                         // 'previous_solutions'

  // defines weights that determine the amount of influence of the different
  // input values on the interpolation in Multidimensional_Interpolator
  struct MI_Weights {
  ...
  // maximum value the residuum of the LINFIT algortihm in the Multidimensional
  // Interpolator is allowed to have It describes the maximum deviation of
  // LINFIT's result from the optimum
  IVP_FLOAT max_res;

  // defines how many PSIs a new solution will be extrapolated into the future
  // by the Multidimensional Interpolator if an interpolation from previous
...
#define INPUT_VECTOR_LENGTH                                      \
  ((sizeof(IVP_U_Float_Point) * 2 + sizeof(IVP_U_Float_Hesse)) / \
   sizeof(IVP_FLOAT))
...
#define SOLUTION_VECTOR_LENGTH \
  ((sizeof(IVP_U_Float_Point) * 4 + sizeof(IVP_FLOAT) * 2) / sizeof(IVP_FLOAT))
/*****************************************************************
...
class IVP_Buoyancy_Output : public IVP_MI_Vector_Base {
 public:
  ...
  IVP_Buoyancy_Output() {
    nr_of_elements = SOLUTION_VECTOR_LENGTH;
    weight_statistic = 0.0f;
    volume_under = 0.0f;
 ...
 *	Description:    Manages the calculation of fluid dynamic related forces
 *for a core of objects Note:           One instance of IVP_Controller_Buoyancy
 *is set up for every core for which buoyancy forces are wished to be computed.
...
class IVP_Controller_Buoyancy : protected IVP_Controller_Independent {
 private:
  ...
  IVP_Core *core;  // holds the core for which this controller manages the fluid
                   // dynamics

  friend class IVP_Buoyancy_Solver;
  IVP_U_Float_Point
      relative_speed_of_current_in_objects_vicinity_old;  // holds the speed of
                                                          // the fluid's current
                                                          // in the vicinity of
                                                          // the object from the
                                                          // PSI before
  IVP_FLOAT core_visible_surface_content_under;  // the area of the surface of
                                                 // the core that is dampened
  IVP_FLOAT
  core_visible_surface_content_under_old;  // the area of the surface of the
                                           // core that is dampened from the
                                           // PSI before

  // vector which applies an instance of 'IVP_Multidimensional_Interpolator' to
  // every object in the core of this 'IVP_Controller_Buoyancy'
  typedef struct {
  ...
  // int max_interpolation_tries;  //defines how many interpolations will be
  // tried before the computation of the forces via the buoyancy_solver will be
  // forced
   ...
   * constructor method
   *********************/
  IVP_Controller_Buoyancy(IVP_Attacher_To_Cores<IVP_Controller_Buoyancy> *,
                          IVP_Core *);
   ...
   **************************************************************************/
  IVP_RETURN_TYPE calculate_future_extrapolation(
   ...
   *              'solution_values_out'
   *****************************************************************************/
  IVP_BOOL use_buoyancy_solver(
      const IVP_Buoyancy_Input *b_input, const IVP_Template_Buoyancy *,
   ...
   *solution values into 'previous_inputs' and 'previous_solutions' of the
   *current 'IVP_Multidimensional_Interpolator' instance
   *************************************************************************************/
  void provide_new_input_solution_combination(
      Attacher_Interpolator *attacher_interpolator,
      IVP_Template_Buoyancy *temp_buoyancy, const IVP_MI_Vector *new_input,
 ...
 protected:
  ...
  IVP_CONTROLLER_PRIORITY get_controller_priority() override {
    return IVP_CP_FORCEFIELDS;
  }
  virtual const char *get_controller_name() override {
    return "ivp:controller_buoyancy";
  };
   ...
   ********************************************************************/
  void do_simulation_controller(IVP_Event_Sim *sim_u,
 ...
 *	Description:    Attaches a IVP_Controller_Buoyancy to every core
 *provided by the list "*set_of_cores"
 ********************************************************************************/
class IVP_Attacher_To_Cores_Buoyancy
 ...
 public:
   ...
   ********************************************************************************/
  virtual IVP_Template_Buoyancy *get_parameters_per_core(IVP_Core *) {
   ...
   ********************************************************************************/
  virtual IVP_SurfaceManager *get_buoyancy_surface(IVP_Real_Object *obj) {
    return obj->get_surface_manager();
  }
