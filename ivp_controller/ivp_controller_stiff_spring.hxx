 ...
 *			- dampening [0 .. 1]
 *			It's a always a good idea to keep the spring_damp value
 *in the same range as the spring_constant ( e.g. s_constant 0.1f -> s_dampening
 *[0.05f .. 0.2f] Attention:	- avoid short spring lens (use ball socket
 *constraints instead) (A short spring len is: if the differential speed of the
 *			connected objects is higher than spring_len * 10
 * /PSI_RATE: current settings:  speed [m/s] < spring_len * 6
 ********************************************************************************/
...
class IVP_Listener_Stiff_Spring {
 public:
  virtual void event_stiff_spring_broken(
      class IVP_Controller_Stiff_Spring *) = 0;
};
...
class IVP_Controller_Stiff_Spring : public IVP_Actuator_Two_Point {
 ...
 public:
  ...
  void add_listener_stiff_spring(
      IVP_Listener_Stiff_Spring *listener);  // NOT_IMPLEMENTED_YET
...
class IVP_Controller_Stiff_Spring_Active : public IVP_Controller_Stiff_Spring,
 ...
 public:
  IVP_Controller_Stiff_Spring_Active(
      IVP_Environment *env,
      IVP_Template_Stiff_Spring_Active *spring);  // constructor of the spring
