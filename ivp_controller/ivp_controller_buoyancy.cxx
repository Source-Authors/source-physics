 ...
 **************************************************************************/
IVP_RETURN_TYPE IVP_Controller_Buoyancy::calculate_future_extrapolation(
    const IVP_Controller_Buoyancy::Attacher_Interpolator::Last_IO
        *last_io_vectors,
    const IVP_MI_Vector *new_input, const IVP_MI_Vector *solution_values,
    const IVP_DOUBLE d_time, const IVP_Time current_time,
    IVP_MI_Vector *future_input, IVP_MI_Vector *future_solution) {
  IVP_DOUBLE future_time =
      current_time.get_time() +
      attacher_buoyancy->template_buoyancy.nr_future_psi_for_extrapolation *
          d_time;
  IVP_DOUBLE divisor = current_time - last_io_vectors->last_psi_time;
  if (divisor > P_DOUBLE_RES) {
    IVP_FLOAT time_quotient =
        (IVP_FLOAT)(future_time - last_io_vectors->last_psi_time) / divisor;
  ...
  IVP_IF(1) { ivp_message("No extrapolation possible!\n"); }
  return (IVP_FAULT);
}
 ...
 *              'solution_values_out'
 *****************************************************************************/
IVP_BOOL IVP_Controller_Buoyancy::use_buoyancy_solver(
    const IVP_Buoyancy_Input *b_input,
  ...
  IVP_Buoyancy_Solver bs(core, this, temp_buoyancy,
                         resulting_speed_of_current_ws);

  IVP_U_Float_Point rel_speed_of_current_os_aligned;
  rel_speed_of_current_os_aligned.set(&b_input->rel_speed_of_current_os);
  // IVP_U_Float_Point surface_os_aligned;
  // surface_os_aligned.set(&b_input->surface_os);
  IVP_BOOL in_water = bs.compute_forces(
      &rel_speed_of_current_os_aligned, &b_input->surface_os,
  ...
  if (in_water) {
    ...
    solution_values_out->object_visible_surface_content_under =
        bs.object_visible_surface_content_under;
    solution_values_out->sum_impulse.set(&bs.sum_impulse);
    solution_values_out->sum_impulse_x_point.set(&bs.sum_impulse_x_point);
    solution_values_out->sum_impulse_x_movevector.set(
 ...
 * Description: applies the dampening forces to the object provided as a
 *parameter
 **********************************************************************************/
void IVP_Controller_Buoyancy::apply_dampening(
    IVP_Real_Object *object, IVP_FLOAT object_visible_surface_content_under,
    IVP_DOUBLE delta_time, IVP_U_Float_Point *sum_impulse_x_movevector,
    IVP_U_Float_Point *sum_impulse_x_point, IVP_U_Float_Point *sum_impulse) {
#ifdef DAMPENING_WITH_MOVEVECTOR

  // mult the (user definable) torque_factor with the
  // object_visible_surface_content_under, also turns around the move vector
  // (multiply with -1.0f)
  IVP_FLOAT move_factor =
      -attacher_buoyancy->template_buoyancy.torque_factor *
      IVP_Inline_Math::ivp_sqrtf(object_visible_surface_content_under) * (0.1f);

  // mult sum_impulse_x_movevector with the modified move_factor and
  // delta_PSI_time
  ...
  // push the core center with the sum of impulses => translation (and a
  // rotation component if the object center is not the same as the core system)
  ...
  IVP_IF(0) {
    ivp_message("core->speed_change.real_length = %f\n",
  ...
  IVP_U_Float_Point sum_impulse_x_movevector_cs;
  {
    ...
    m_core_f_object.vmult3(sum_impulse_x_movevector,
                           &sum_impulse_x_movevector_cs);
  }

  {
    // calc new rot_speed
    IVP_U_Float_Point delta_rot_component1_cs;
    delta_rot_component1_cs.set_pairwise_mult(&sum_impulse_x_point_cs,
                                              core->get_inv_rot_inertia());
#ifdef DAMPENING_WITH_MOVEVECTOR
    IVP_U_Float_Point delta_rot_component2_cs;
    delta_rot_component2_cs.set_pairwise_mult(&sum_impulse_x_movevector_cs,
                                              core->get_inv_rot_inertia());
    ...
    IVP_IF(1) { core->core_plausible_check(); }

    // if object is a ball then apply dampening of rotation which can result
    // e.g. from the ball falling not straightly into the medium
...
#if 0
    ivp_message("In apply_dampening:\n");
 ...
 * Description: applies the buoyancy forces to the object provided as a
 *parameter
 *********************************************************************************/
void IVP_Controller_Buoyancy::apply_buoyancy_impulse(
    IVP_Real_Object *object, IVP_Template_Buoyancy *temp_buoyancy,
    IVP_DOUBLE delta_time, IVP_FLOAT volume_under,
    IVP_U_Float_Point *volume_center_under) {
  // vector indicating the direction the forces will push
  IVP_U_Float_Point dir_ws(
      core->environment->get_gravity());  // the buoyancy impulse works in the
  ...
  if (volume_under > temp_buoyancy->buoyancy_eps) {
    ...
    IVP_U_Point volume_center_under_ws;
    {
      IVP_Cache_Object *cache_object =
          object
              ->get_cache_object_no_lock();  // needed to transform coordinates
                                             // into other coord. systems
      cache_object->transform_position_to_world_coords(volume_center_under,
                                                       &volume_center_under_ws);
    }

    IVP_DOUBLE force_under =
        volume_under * (-temp_buoyancy->medium_density) * (delta_time);
...
void ivp_debug_show_real_values(
    const IVP_Buoyancy_Input * /*b_input*/, IVP_Controller_Buoyancy *cntrl,
    const IVP_Template_Buoyancy *temp_buoyancy, IVP_Core *core,
    IVP_Real_Object *object,
    const IVP_U_Float_Point *resulting_speed_of_current_ws) {
  IVP_USE(object);
  IVP_Buoyancy_Output solution_values_out;
  IVP_Buoyancy_Solver bs(core, cntrl, temp_buoyancy,
                         resulting_speed_of_current_ws);
  //    IVP_BOOL in_water = bs.compute_forces(&b_input->rel_speed_of_current_os,
  //    &b_input->surface_os, object);
 ...
 * Description: combines various ways of inserting new vectors of input and
 *solution values into 'previous_inputs' and 'previous_solutions' of the current
 *              'IVP_Multidimensional_Interpolator' instance
 *************************************************************************************/
void IVP_Controller_Buoyancy::provide_new_input_solution_combination(
    Attacher_Interpolator *ai, IVP_Template_Buoyancy *temp_buoyancy,
    const IVP_MI_Vector *weighted_new_input,
    const IVP_MI_Vector *solution_values, const IVP_DOUBLE d_time,
    const IVP_Time current_time) {
  alignas(IVP_MI_Vector) float input_data[INPUT_VECTOR_LENGTH];
  alignas(IVP_MI_Vector) float solution_data[SOLUTION_VECTOR_LENGTH];
    ...
    case IVP_TRUE: {
      ...
      if (ai->mi->get_nr_occupied() < ai->mi->get_nr_of_vectors()) {
        ...
        if (!(temp_buoyancy->insert_extrapol_only)) {
          ai->mi->add_new_input_solution_combination_conventional(
              weighted_new_input, solution_values);
        }

        // extrapolate into the future
        IVP_RETURN_TYPE success = calculate_future_extrapolation(
            &ai->last_io_vectors, weighted_new_input, solution_values, d_time,
            current_time, future_input, future_solution);
        if (success) {
          future_input->set_time_stamp(current_time);
          future_solution->set_time_stamp(current_time);
          ai->mi->add_new_input_solution_combination_conventional(
              future_input, future_solution);
        }
      } else {
        // weighted_new_input->set_time_stamp(current_time);
        // solution_values->set_time_stamp(current_time);
        if (!(temp_buoyancy->insert_extrapol_only)) {
          ai->mi->add_new_input_solution_combination_stochastic(
              weighted_new_input, solution_values);
        }
 ...
 ********************************************************************/
void IVP_Controller_Buoyancy::do_simulation_controller(
  ...
  attacher_buoyancy->liquid_surface_descriptor->calc_liquid_surface(
      environment, core, &surface_hesse_ws, &abs_speed_of_current_ws);

  // if area is increased, reduce mean velocity of water
  if (core_visible_surface_content_under >
      core_visible_surface_content_under_old) {
    relative_speed_of_current_in_objects_vicinity_old.mult(
        core_visible_surface_content_under_old /
        core_visible_surface_content_under);
  }
  ...
  resulting_speed_of_current_ws.add(
      &relative_speed_of_current_in_objects_vicinity_old);
#else
  resulting_speed_of_current_ws.set(&abs_speed_of_current_ws);
#endif
  IVP_U_Float_Point rel_speed_of_current_ws;
  rel_speed_of_current_ws.subtract(&resulting_speed_of_current_ws,
                                   &core->speed);

  IVP_Template_Buoyancy *temp_buoyancy =
      attacher_buoyancy->get_parameters_per_core(core);
  ...
  for (int i = 0; i < nr_of_objects; i++) {
    ...
    IVP_U_Float_Hesse surface_os;  // surface in object's coordinate system
    {
      ...
      IVP_Cache_Object *cache_object =
          attacher_interpolator[i].object->get_cache_object_no_lock();
      cache_object->transform_vector_to_object_coords(&rel_speed_of_current_ws,
                                                      &rel_speed_of_current_os);
      cache_object->transform_vector_to_object_coords(&surface_hesse_ws,
                                                      &surface_os);
      IVP_U_Matrix m_world_f_object;
      attacher_interpolator[i].object->get_m_world_f_object_AT(
          &m_world_f_object);
      IVP_DOUBLE dist =
          m_world_f_object.get_position()->dot_product(&surface_hesse_ws);
      surface_os.hesse_val = surface_hesse_ws.hesse_val + dist;
    }

    // create new input vector without using weights, needed for the buoyancy
    // solver
...
#endif
    weighted_b_input.rel_speed_of_current_os.set_multiple(
        &rel_speed_of_current_os,
        temp_buoyancy->mi_weights.weight_current_speed);
    weighted_b_input.surface_os.set_multiple(
        &surface_os, temp_buoyancy->mi_weights.weight_surface);
    weighted_b_input.surface_os.hesse_val =
        surface_os.hesse_val * temp_buoyancy->mi_weights.weight_surface;
    weighted_b_input.rot_speed.set_multiple(
        &core->rot_speed, temp_buoyancy->mi_weights.weight_rot_speed);
    ...
    if (temp_buoyancy->use_interpolation) {  // use interpolation
      ...
      // force a new buoyancy solver run if the interpolation_counter has
      ...
      if (interpolation_counter++ >= temp_buoyancy->max_interpolation_tries) {
        ...
        result = attacher_interpolator[i].mi->check_interpolation(
            weighted_new_input, temp_buoyancy->max_tries_nr_of_vectors_involved,
            temp_buoyancy->max_res, solution_values);
      }
      ...
      if (result) {
        ...
        ivp_message("Interpolated!\n");
        ivp_debug_show_real_values(&b_input, temp_buoyancy, core,
...
#ifdef WITH_DEBUG_OUTPUT
        ivp_message("Not Interpolated!\n");
        attacher_interpolator[i].nr_not_interpolated++;
#endif
        IVP_BOOL in_water =
            use_buoyancy_solver(&b_input, temp_buoyancy, &b_output,
                                &resulting_speed_of_current_ws, i);
        if (in_water) {
          // if the buoyancy solver did compute values (if object is in water)
          // then copy the new input/solution-combination into the interpolator
          provide_new_input_solution_combination(
              &attacher_interpolator[i], temp_buoyancy, weighted_new_input,
              solution_values, es->delta_time, environment->get_current_time());
        }
#ifdef WITH_DEBUG_OUTPUT
        else {
          ivp_message("Not yet in water!\n");
        }
#endif
      }
#ifdef WITH_DEBUG_OUTPUT
      ivp_message("times_interpolated : times_calculated = %d : %d\n",
                  attacher_interpolator[i].nr_interpolated,
                  attacher_interpolator[i].nr_not_interpolated);
      ivp_message("# of one vector already sufficient = %d\n",
                  attacher_interpolator[i].mi->nr_one_vector_sufficient);
      for (int q = 0; q < attacher_interpolator[i].mi->get_nr_of_vectors();
           q++) {
        ivp_message("%d vectors involved:  # of res over limit = %d     ",
                    q + 1, attacher_interpolator[i].mi->nr_res_over_limit[q]);
        ivp_message("# of interpol. weigh over limit = %d     ",
                    attacher_interpolator[i].mi->nr_int_weight_over_limit[q]);
        ivp_message("# of times LINFIT failed = %d     ",
                    attacher_interpolator[i].mi->nr_of_linfit_failure[q]);
        ivp_message("# of interpolation successes = %d\n",
                    attacher_interpolator[i].mi->nr_of_success[q]);
      }
      ivp_message("\n\n");
#endif

      // update last_input structure, is needed in
      // 'calculate_future_extrapolation'
      attacher_interpolator[i].last_io_vectors.last_input_vector->set(
          weighted_new_input);
      attacher_interpolator[i].last_io_vectors.last_solution_vector->set(
          solution_values);
      //	    attacher_interpolator[i].last_io_vectors.last_psi_time =
      // environment->get_current_time();
      attacher_interpolator[i].last_io_vectors.last_psi_time =
          environment->get_current_time().get_time();
      ...
      use_buoyancy_solver(&b_input, temp_buoyancy, &b_output,
                          &resulting_speed_of_current_ws, i);
#ifdef WITH_DEBUG_OUTPUT
      ivp_message("nr_not_interpolated = %d\n", ++nr_not_interpolated);
#endif
#if !defined(IVP_NO_MD_INTERPOLATION)
    }
#endif
    ...
    apply_buoyancy_impulse(attacher_interpolator[i].object, temp_buoyancy,
                           es->delta_time, b_output.volume_under,
                           &b_output.volume_center_under);

    apply_dampening(
        attacher_interpolator[i].object,
        IVP_Inline_Math::fabsd(b_output.object_visible_surface_content_under),
        es->delta_time, &b_output.sum_impulse_x_movevector,
        &b_output.sum_impulse_x_point, &b_output.sum_impulse);

    // add 'object_visible_surface_content_under' to
    // 'core_visible_surface_content_under' 'fabs' is taken in case the
    // interpolation's result was negative
    core_visible_surface_content_under +=
        IVP_Inline_Math::fabsd(b_output.object_visible_surface_content_under);
  }

#ifdef DAMPENING_WITH_PARTICLE_ACCELERATION
  IVP_FLOAT inverse_viscosity =
      1.0f - (temp_buoyancy->viscosity_factor * es->delta_time);
  IVP_U_Float_Point speed_of_object_relative_to_medium_ws;
  speed_of_object_relative_to_medium_ws.set(&core->speed);
  speed_of_object_relative_to_medium_ws.subtract(
      &resulting_speed_of_current_ws);

  relative_speed_of_current_in_objects_vicinity_old.mult(inverse_viscosity);
  relative_speed_of_current_in_objects_vicinity_old.add_multiple(
      &speed_of_object_relative_to_medium_ws,
      temp_buoyancy->viscosity_input_factor * es->delta_time);
#endif
}
...
IVP_Controller_Buoyancy::IVP_Controller_Buoyancy(
    IVP_Attacher_To_Cores<IVP_Controller_Buoyancy> *attacher_buoyancy_,
    IVP_Core *core_) {
  attacher_buoyancy = (IVP_Attacher_To_Cores_Buoyancy *)attacher_buoyancy_;
  core = core_;
  core->environment->get_controller_manager()->add_controller_to_core(this,
                                                                      core);
  ...
  // initialize vector which applies a multidimensional interpolator to every
  // object in the core
  int nr_of_objects = core->objects.len();
...
#if !defined(IVP_NO_MD_INTERPOLATION)
    attacher_interpolator[i].mi = new IVP_Multidimensional_Interpolator(
        5, INPUT_VECTOR_LENGTH, SOLUTION_VECTOR_LENGTH);
    attacher_interpolator[i].last_io_vectors.last_input_vector =
        IVP_MI_Vector::malloc_mi_vector(INPUT_VECTOR_LENGTH);
    attacher_interpolator[i].last_io_vectors.last_solution_vector =
        IVP_MI_Vector::malloc_mi_vector(SOLUTION_VECTOR_LENGTH);
#endif
...
#pragma message("this code should not be called")
  ...
  core->environment->get_controller_manager()->remove_controller_from_core(
      this, core);
}

IVP_Attacher_To_Cores_Buoyancy::IVP_Attacher_To_Cores_Buoyancy(
    IVP_Template_Buoyancy &templ, IVP_U_Set_Active<IVP_Core> *set_of_cores_,
