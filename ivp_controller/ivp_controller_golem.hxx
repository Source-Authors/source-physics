...
class IVP_Controller_Golem : public IVP_Controller_Motion {
 ...
 protected:
  ...
  virtual const char *get_controller_name() override {
    return "ivp:controller_golem";
  };

  void beam_object_to_target_position(
      IVP_Event_Sim
          *es);  // may be called from resolve_for_problem to beam object
 public:
  virtual IVP_RETURN_TYPE resolve_for_problem(IVP_Event_Sim *event_sim,
                                              IVP_GOLEM_PROBLEM problem) = 0;

  void set_prime_position(const IVP_U_Point *position,
                          const IVP_U_Float_Point *velocity,
                          const IVP_Time &time);
  void set_prime_orientation(const IVP_U_Quat *orientation0,
                             const IVP_Time &time0,
                             const IVP_U_Quat *orientation1 = NULL,
                             IVP_FLOAT dt = 0.0f);
  IVP_Controller_Golem(IVP_Real_Object *,
                       const IVP_Template_Controller_Golem *);
