 ...
 *    they are sorted, the controller with the highest priority manipulates the
 *object first
 *
 *	Note:		Only values between >IVP_CP_STATIC_FRICTION, and
 *<IVP_CP_ENERGY_FRICTION are allowed (Note: >= and <= is not allowed)
 *
 *			All controllers with priority > IVP_CP_GRAVITY must do
 *async pushes all controllers with priority < IVP_CP_GRAVITY do real pushes
 *
 *			The order of controllers with priority > IVP_CP_GRAVITY
 *is not important
 **********************************************************************************************/
...
class IVP_Controller {
 public:
   ...
   *	Description:	Returns the minimum simulation frequency of this
   *controller The result of this function is heavily cached, so changing it's
   *return value might not have any effect
   ********************************************************************************/
   ...
   *  Note:           This vector is used only temporarily, so you may return a
   *pointer to a thread global vector. Note:           Must return only movable
   *cores Version Info:   In future releases IVP_U_Vector might be replaced by
   *IVP_U_Set.
   ********************************************************************************/
  ...
  /********************************************************************************
   *	Name:	  	reset_time
   *	Description:	called when the time of the whole physics system is
   *resetted offset is old last_PSI_time
   ********************************************************************************/
  virtual void reset_time(IVP_Time /*offset*/) {}

  /********************************************************************************
   *	Name:	  	do_simulation_controller
   *	Description:	Perform the simulation, apply the forces or change the
   *speed, rot_speed
   ********************************************************************************/
  virtual void do_simulation_controller(IVP_Event_Sim *,
                                        IVP_U_Vector<IVP_Core> *core_list) = 0;

  /********************************************************************************
   *	Name:	  	get_controller_priority
   *	Description:	Get the priority of the controller, see
...
class IVP_Standard_Gravity_Controller : public IVP_Controller_Independent {
 public:
  IVP_Standard_Gravity_Controller()
      : grav_vec(std::numeric_limits<float>::quiet_NaN(),
 ...
 *	Note:		To get an instance of this class call
 *IVP_Environment::get_controller_manager()
 ********************************************************************************/
class IVP_Controller_Manager {
 public:
   ...
   *	Description:	inform the system that a new controller is now
   *controlling some cores
   ********************************************************************************/
   ...
   *	Description:	inform the system that a new controller is now
   *controlling an additional single core
   ********************************************************************************/
  static void add_controller_to_core(IVP_Controller_Independent *cntrl,
                                     IVP_Core *core);
   ...
   ********************************************************************************/
  static void remove_controller_from_core(IVP_Controller_Independent *cntrl,
                                          IVP_Core *core);
