...
IVP_Controller_Motion::IVP_Controller_Motion(
  ...
  this->l_environment->get_controller_manager()->add_controller_to_core(this,
                                                                        core);
}
...
void IVP_Controller_Motion::set_max_translation_force(
    const IVP_U_Float_Point *max_tf) {
  max_translation_force = max_tf;
}

void IVP_Controller_Motion::do_simulation_controller(
  ...
  // Little entry checks
  IVP_ASSERT(core_list->len() ==
             1);  // Multiple cores don't make much sense here, do they?
  ...
  delta_position.inline_subtract_and_mult(&this->target_pos_ws, cur_pos_ws,
                                          this->force_factor);
  delta_position.add_multiple(&core->speed,
                              -es->delta_time * this->damp_factor);
  ...
  IVP_DOUBLE inv_d_time = es->i_delta_time;
  {
    ...
    for (int i = 2; i >= 0; i--) {
      ...
      if (IVP_Inline_Math::fabsd(axis_force) < max_translation_force.k[i])
        continue;

      if (delta_position.k[i] < 0) {
        delta_position.k[i] = -max_translation_force.k[i] *
                              core->get_inv_mass() * es->delta_time *
                              es->delta_time;
      } else {
        delta_position.k[i] = max_translation_force.k[i] *
                              core->get_inv_mass() * es->delta_time *
  ...
  // ---------------------------------------------------------
  {
    IVP_U_Quat target_q_core_f_world;
    target_q_core_f_world.set_invert_unit_quat(&target_q_world_f_core);
    IVP_U_Quat delta_quat_core_f_core;
    delta_quat_core_f_core.set_mult_quat(&target_q_core_f_world,
    ...
    rot_speed_change_cs.add_multiple(&delta_angles_now_cs, &core->rot_speed,
                                     -this->angular_damp_factor);

    for (int i = 2; i >= 0; --i) {
      IVP_FLOAT rot_force =
          rot_speed_change_cs.k[i] * inv_d_time * core->get_rot_inertia()->k[i];
      if (IVP_Inline_Math::fabsd(rot_force) < max_torque.k[i]) continue;

      // clip rot change so that resulting rot force doesn't exceed max rot
      // force
      if (rot_force < 0) {
        rot_speed_change_cs.k[i] = -max_torque.k[i] *
                                   core->get_inv_rot_inertia()->k[i] *
                                   es->delta_time;
      } else {
        rot_speed_change_cs.k[i] = max_torque.k[i] *
                                   core->get_inv_rot_inertia()->k[i] *
...
void IVP_Controller_Motion::core_is_going_to_be_deleted_event(
    IVP_Core *core_i) {
...
IVP_Controller_Motion::~IVP_Controller_Motion() {
  this->l_environment->get_controller_manager()->remove_controller_from_core(
      this, core);
}

void IVP_Controller_Motion::set_target_position_ws(
...
void IVP_Controller_Motion::set_target_object_position_ws(
    IVP_Real_Object *ro, const IVP_U_Quat *desired_orientation,
    const IVP_U_Point *position_of_object_space_center_ws) {
  if (!real_object->flags.shift_core_f_object_is_zero) {
    ...
    IVP_U_Point pos_ws;
    pos_ws.subtract(position_of_object_space_center_ws, &shift_wo);
...
void IVP_Controller_Motion::set_target_q_world_f_core(
    const IVP_U_Quat *orientation) {
  if (IVP_Inline_Math::fabsd(orientation->acos_quat(&target_q_world_f_core) -
                             1.0f) < P_DOUBLE_EPS)
    return;
...
IVP_Controller_Golem::IVP_Controller_Golem(
    IVP_Real_Object *o, const IVP_Template_Controller_Golem *t)
...
void IVP_Controller_Golem::beam_object_to_target_position(IVP_Event_Sim *es) {
  real_object->beam_object_to_new_position(&target_q_world_f_core,
                                           &target_pos_ws, IVP_TRUE);
}

void IVP_Controller_Golem::do_simulation_controller(
    IVP_Event_Sim *es, IVP_U_Vector<IVP_Core> *cores) {
  // Little entry checks
  IVP_ASSERT(cores->len() ==
             1);  // Multiple cores don't make much sense here, do they?
  ...
  if (!angular_velocity_set) {
    ...
    target_q_world_f_core.set_interpolate_smoothly(
        ...
        dt * i_delta_prime_orientation_time);
  }
  ...
  IVP_U_Float_Point delta_position;
  delta_position.subtract(&this->target_pos_ws, cur_pos_ws);

  if (!real_object->flags.shift_core_f_object_is_zero) {
    IVP_U_Float_Point shift_cs_os_ws;
    m_world_f_core->vmult3(real_object->get_shift_core_f_object(),
                           &shift_cs_os_ws);
  ...
  // ---------------------------------------------------------
  {
    ...
    if (delta_angles_now_cs.quad_length() >
        max_delta_orientation * max_delta_orientation) {
...
void IVP_Controller_Golem::set_prime_position(const IVP_U_Point *position,
...
void IVP_Controller_Golem::set_prime_orientation(const IVP_U_Quat *orientation0,
                                                 const IVP_Time &time0,
                                                 const IVP_U_Quat *orientation1,
                                                 IVP_FLOAT dt) {
