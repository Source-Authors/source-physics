...
class IVP_Constraint : public IVP_Controller_Dependent {
 ...
 public:
  ...
  /******************************************************************************************
   *  Feature:        deactivate()
   *  Description:    You can deactivate this constraint temporarily if you call
   *this function. The constraint is removed from simulation until you
   *activate() it again. Attention:		If a constraint is deactivated,
   *it will not get deleted when you delete the environment
   *****************************************************************************************/
  void deactivate();
  void activate();
  ...
  /******************************************************************************************
   *  Feature:        Destruktor
   *  Description:    You can delete a constraint whenever you want to
   *permanently remove it from simulation.
   *****************************************************************************************/
  virtual ~IVP_Constraint();

  /******************************************************************************************
   *  Feature:        Constraint changing functions
   *  Description:    You can change this constraint's parameter by using these
   *functions. The constraint changing functions are used exactly the same way
   *as the constraint setting functions in class IVP_Template_Constraint. Look
   *there for a complete description. Note:           Constraint changing
   *functions are rather expensive. Try not to use them some thousand times per
   *second. But creating a new constraint is much more expensive than changing
   *one parameter of an existing one. Spaces: R Reference Object A Attached
   *Object os object system fs system for translation rs system for rotation
   *(may be equal to fs)
   *
   *  Attention:      The constraint changing functions are not yet fully
   *tested.
   *****************************************************************************************/

  // constraint changing functions that refer to translation
  virtual void change_fixing_point_Ros(
      const IVP_U_Point
          *anchor_Ros);  // just set a new fixing point at both objects
  virtual void change_target_fixing_point_Ros(const IVP_U_Point *anchor_Ros);
  virtual void change_translation_axes_Ros(
      const IVP_U_Matrix3 *m_Ros_f_Rfs);  // just changes the translation axes.
                                          // Normally has no effect
  virtual void change_target_translation_axes_Ros(
      const IVP_U_Matrix3
          *m_Ros_f_Rfs);  // move attached object to a new postion
  ...
  virtual void limit_translation_axis(IVP_COORDINATE_INDEX which,
  ...
  virtual const char *get_controller_name() override {
    return "ivp:constraint";
  };

  // some functions are missing yet.
 public:
  // functions to get the values of the constraint are missing (should be
  // feedable to changing values)
 ...
 public:
  ...
  static IVP_Constraint *create_constraint_any_solver(
      IVP_Template_Constraint *constraint_template);
