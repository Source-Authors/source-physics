...
void hk_Array_Base::alloc_mem(int size, int num) {
  ...
  m_elems =
      hk_allocate<char>(static_cast<size_t>(size) * static_cast<size_t>(num),
                        hk_MEMORY_CLASS::HK_MEMORY_CLASS_ARRAY);
...
void hk_Array_Base::grow_mem(int size) {
  ...
  const hk_uint32 double_memsize{static_cast<hk_uint32>(m_memsize) + m_memsize};
  // dimhotepus: Sometimes m_memsize * 2 doesn't fit unsigned short.
  hk_array_store_index new_memsize = static_cast<hk_array_store_index>(
      double_memsize > USHRT_MAX ? USHRT_MAX : double_memsize);
  if (!new_memsize) {
    new_memsize = 2;
  }

  char *new_array = hk_allocate<char>(new_memsize * static_cast<size_t>(size),
                                      hk_MEMORY_CLASS::HK_MEMORY_CLASS_ARRAY);
  ...
  if (m_elems && (m_elems != reinterpret_cast<char *>(this + 1))) {
    hk_deallocate(m_elems, m_memsize * static_cast<size_t>(size),
                  hk_MEMORY_CLASS::HK_MEMORY_CLASS_ARRAY);
  }
...
void hk_Array_Base::grow_mem(int size, int n_elems) {
  ...
  char *new_array = hk_allocate<char>(
      static_cast<size_t>(new_memsize) * static_cast<size_t>(size),
      hk_MEMORY_CLASS::HK_MEMORY_CLASS_ARRAY);
  if (new_array) {
    if (m_elems) {
      memcpy(new_array, m_elems, m_memsize * static_cast<size_t>(size));
