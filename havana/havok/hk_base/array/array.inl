...
hk_Array<T>::hk_Array(hk_array_store_index initial_size) {
  ...
  if (initial_size) {
    ...
    m_elems = reinterpret_cast<char*>(
        hk_allocate<T>(initial_size, hk_MEMORY_CLASS::HK_MEMORY_CLASS_ARRAY));
  } else {
    m_memsize = 0;
    m_elems = HK_NULL;
  }
...
hk_Array<T>::~hk_Array() {
  if (m_elems) {
    ...
    hk_deallocate(get_elems(), m_memsize,
...
#if !defined(_XBOX)
template <class T>
void hk_Array<T>::remove_element(
    hk_array_index idx,
...
void hk_Array<T>::free_elem_array() {
  if (m_elems &&
      (static_cast<void*>(m_elems) != static_cast<void*>(this + 1))) {
    hk_deallocate(m_elems, m_memsize * sizeof(T),
                  hk_MEMORY_CLASS::HK_MEMORY_CLASS_ARRAY);
  }
  m_n_elems = 0;
  m_memsize = 0;
