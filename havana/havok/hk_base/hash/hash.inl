...
void hk_Hash<KV>::rehash(int new_size) {
  ...
  m_elems = hk_allocate<hk_Hash_Element>(static_cast<size_t>(new_size),
                                         hk_MEMORY_CLASS::HK_MEMORY_CLASS_HASH);
  {
...
//: HK_ASSERT(size = 2,4,8,16,32 ... 2**x)
{
  ...
  m_size_mm = size - 1;
  m_nelems = 0;
  m_elems = hk_allocate<hk_Hash_Element>(static_cast<size_t>(size),
                                         hk_MEMORY_CLASS::HK_MEMORY_CLASS_HASH);
  hk_Hash_Element *e = m_elems;
  for (int i = m_size_mm; i >= 0; i--) {
    e->m_hash_index = 0;
...
template <class KV>
hk_Hash<KV>::~hk_Hash() {
  hk_deallocate(m_elems, static_cast<size_t>(m_size_mm) + 1,
                hk_MEMORY_CLASS::HK_MEMORY_CLASS_HASH);
}
