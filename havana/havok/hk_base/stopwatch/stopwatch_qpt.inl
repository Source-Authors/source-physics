...
inline hk_uint64 hk_query_cpu_current_frequency() {
  ...
  hk_uint64 freq = (endTicks - startTicks - hk_measure_cpu_timer_overhead())
                   << scale;
  if (freq == 0) {
...
inline hk_real hk_Stopwatch_qpt::get_elapsed_time() const {
  return static_cast<hk_real>(m_ticks_total - s_measure_overhead) /
         static_cast<hk_real>(s_ticks_per_second);
}
...
inline hk_real hk_Stopwatch_qpt::get_split_time() {
  ...
  return static_cast<hk_real>(sticks) /
         static_cast<hk_real>(s_ticks_per_second);
}
