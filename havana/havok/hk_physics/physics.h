...
#define HK_TRANSFORM_TO_CORE_SPACE(body, vec)                 \
  body->get_core()->get_m_world_f_core_PSI()->inline_vimult3( \
      (IVP_U_Float_Point *)&vec, (IVP_U_Float_Point *)&vec)
...
class hk_Rigid_Body : public IVP_Real_Object {
 public:
  ...
  hk_Rigid_Body(IVP_Cluster *father, IVP_SurfaceManager *surface,
                const IVP_Template_Real_Object *obj,
                const IVP_U_Quat *q_world_f_obj, const IVP_U_Point *position)
      : IVP_Real_Object(father, surface, obj, q_world_f_obj, position) {}
  ...
  inline hk_Transform get_transform_next_PSI(float deltaTime) {
    ...
    q_core_f_core.set_fast_multiple_with_clip(&get_core()->rot_speed,
                                              deltaTime);
    q_next_PSI.inline_set_mult_quat(
        (IVP_U_Quat *)&this->get_core()->q_world_f_core_next_psi,
        &q_core_f_core);
    q_next_PSI.fast_normize_quat();
    q_next_PSI.set_matrix(&next_m_world_f_core);
    /*
                    next_m_world_f_core.vv.k[0] = this->get_core()->speed.k[0] *
       deltaTime + this->get_core()->m_world_f_core_last_psi.vv.k[0];
                    next_m_world_f_core.vv.k[1] = this->get_core()->speed.k[1] *
       deltaTime + this->get_core()->m_world_f_core_last_psi.vv.k[1];
                    next_m_world_f_core.vv.k[2] = this->get_core()->speed.k[2] *
       deltaTime + this->get_core()->m_world_f_core_last_psi.vv.k[2];
    */
    next_m_world_f_core.vv.add_multiple(
        get_core()->m_world_f_core_last_psi.get_position(), &get_core()->speed,
        deltaTime);

    if (!this->flags.shift_core_f_object_is_zero)
      next_m_world_f_core.vmult4(get_shift_core_f_object(),
                                 &next_m_world_f_core.vv);
  ...
  inline hk_Transform get_cached_transform() {
    ...
    if (this->flags.shift_core_f_object_is_zero) {
      ...
      coreShiftMatrix.vmult4(this->get_shift_core_f_object(),
                             &coreShiftMatrix.vv);
      coreShiftMatrix.get_4x4_column_major(&t);
    }
    return t;
  }
  inline hk_Entity_Core_ID get_entity_core_id() { return 0; }
...
class hk_Rigid_Body_Core : protected IVP_Core {
 public:
  ...
  hk_Vector3 &_get_spin() { return *(hk_Vector3 *)&rot_speed; }

  hk_Vector3 &_get_linear_velocity() { return *(hk_Vector3 *)&speed; }

  inline hk_real get_mass() { return IVP_Core::get_mass(); }

  inline hk_real get_inv_mass() { return IVP_Core::get_inv_mass(); }
  ...
  void add_to_mass_matrix_inv(hk_Core_VMQ_Input &input,
                              hk_Dense_Matrix &matrix_out,
                              hk_real velocities_out[]);

  inline void add_to_velocities(hk_Core_VMQ_Input &input,
                                hk_real velocities_out[]) {
  ...
  void apply_impulses(hk_Core_VMQ_Input &input,
...
class hk_Rigid_Body_Binary_EF {
 ...
 protected:
  hk_Rigid_Body_Binary_EF(hk_Environment *, hk_Rigid_Body *, hk_Rigid_Body *,
                          hk_effector_priority);
 ...
 public:
  ...
  inline hk_Rigid_Body *get_rigid_body(int i) { return m_entities[i]; }
...
class hk_Link_EF : public IVP_Controller_Dependent {
 ...
 public:
  ...
  IVP_CONTROLLER_PRIORITY get_controller_priority() override {
    return IVP_CP_CONSTRAINTS_MIN;
  }

  virtual void apply_effector_PSI(class hk_PSI_Info &pi,
                                  hk_Array<hk_Entity *> *) = 0;

  void do_simulation_controller(
      IVP_Event_Sim *es, IVP_U_Vector<IVP_Core> * /*core_list*/) override {
  ...
  inline hk_Environment *get_environment() const { return m_environment; }

  virtual const char *get_controller_name() override { return "hk:link_ef"; };
};
class hk_Limited_Ball_Socket_BP *ivp_create_limited_ball_socket_bp(
    IVP_Template_Constraint *tmpl);
class hk_Constraint *ivp_create_ragdoll_constraint_from_local_constraint_system(
    class hk_Local_Constraint_System *, IVP_Template_Constraint *);
