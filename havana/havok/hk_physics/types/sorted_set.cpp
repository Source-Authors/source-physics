...
hk_Sorted_Set::hk_Sorted_Set(int start_size, hk_Memory *mem) {
  ...
  m_elems =
      hk_allocate(hk_SS_Element, m_malloced_size, HK_MEMORY_CLASS_SORTED_SET);
...
hk_Sorted_Set::~hk_Sorted_Set() {
  hk_deallocate(hk_SS_Element, m_elems, m_malloced_size,
                HK_MEMORY_CLASS_SORTED_SET);
}

hk_Sorted_Set::hk_ss_index hk_Sorted_Set::add_ss_element(void *elem,
                                                         hk_ss_real value,
                                                         hk_Memory *mem) {
  ...
  if (m_free_list != HK_SORTED_SET_UNUSED) {
    ...
    // increase array
    unsigned int i;
    hk_SS_Element *new_elems = hk_allocate(
        hk_SS_Element, 2 * (m_malloced_size + 1), HK_MEMORY_CLASS_SORTED_SET);
  ...
  if (count_cmp > max_cmp_len) {
    ...
    if (next_of_first !=
        HK_SORTED_SET_LONG_UNUSED) {  // real long as first element
