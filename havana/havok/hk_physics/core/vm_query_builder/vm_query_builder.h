...
class hk_VMQ_Storage {
 public:
  ...
  inline hk_Dense_Matrix &get_dense_matrix() { return m_dense_matrix; }
 ...
 protected:
  ...
  HK_ALIGNED_VARIABLE(hk_real m_impulse_info[2][sizeof(hk_Virtual_Mass_Query) *
                                                N / sizeof(hk_real)],
                      16);
...
//
enum hk_Body_Index { HK_BODY_A = 0, HK_BODY_B = 1 };
...
class hk_VM_Query_Builder {
 public:
  ...
  inline T &get_vmq_storage() { return m_vmq_storage; }

  inline void begin_entries([[maybe_unused]] int size) {}

  inline void add_angular(int index_offset, hk_Body_Index body_index,
                          hk_Rigid_Body *rb, const hk_Vector3 &axis_ws,
                          hk_real sign) {
    HK_ASSERT(int(index_offset + m_vmq_offset / sizeof(hk_Virtual_Mass_Query)) <
              m_vmq_storage.length());

    hk_Virtual_Mass_Query &vmq = *offset_vmq(
        m_vmq_storage.get_vmq(static_cast<int>(body_index)), index_offset);
  ...
  inline void add_linear(int index_offset, hk_Body_Index body_index,
                         hk_Rigid_Body *rb, const hk_Vector3 &position_ws,
                         const hk_Vector3 &direction_ws, hk_real signum) {
    HK_ASSERT(int(index_offset + m_vmq_offset / sizeof(hk_Virtual_Mass_Query)) <
              m_vmq_storage.length());
    ...
    hk_Vector3 mass_center_relative;
    mass_center_relative.set_sub(position_ws, rb->get_center_of_mass());
  ...
  inline void add_linear(int index_offset, hk_Body_Index body_index,
                         hk_Rigid_Body *rb,
                         const hk_Mass_Relative_Vector3 &pos_rel,
                         const hk_Vector3 &direction_ws, hk_real signum) {
    HK_ASSERT(int(index_offset + m_vmq_offset / sizeof(hk_Virtual_Mass_Query)) <
              m_vmq_storage.length());
  ...
  inline void add_linear_xyz(
      int index_offset, hk_Axis_Index axis, hk_Body_Index body_index,
      hk_Rigid_Body *rb, const hk_Mass_Relative_Vector3 &mass_center_relative,
      hk_real signum) {
    HK_ASSERT(int(index_offset + m_vmq_offset / sizeof(hk_Virtual_Mass_Query)) <
              m_vmq_storage.length());
...
#if 1
    if (axis == HK_X_DIRECTION) {
      vmq.m_angular.set(0.0f, signum * mcr.z, -mcr.y * signum);
    } else if (axis == HK_Y_DIRECTION) {
      vmq.m_angular.set(-mcr.z * signum, 0.0f, mcr.x * signum);
    } else {
      vmq.m_angular.set(mcr.y * signum, -mcr.x * signum, 0.0f);
    }
#else
  ...
  inline void commit(hk_Body_Index body_index, hk_Rigid_Body *rb) {
    ...
    core->add_to_mass_matrix_inv(m_input[static_cast<int>(body_index)], matrix,
                                 velocities);
  }
  ...
  inline void update_velocities(hk_Body_Index body_index, hk_Rigid_Body *rb) {
    rb->get_rigid_body_core()->add_to_velocities(
        m_input[static_cast<int>(body_index)], m_vmq_storage.get_velocities());
  }
  ...
  inline void apply_impulses(hk_Body_Index body_index, hk_Rigid_Body *rb,
    ...
    rb->get_rigid_body_core()->apply_impulses(
        m_input[static_cast<int>(body_index)], impulses);
  }
 ...
 protected:
  ...
  inline hk_Virtual_Mass_Query *offset_vmq(hk_Virtual_Mass_Query *base,
                                           int index) {
 ...
 *		[VMQB.add_linear ( second_body ...  )  ] only if second body
 *exists VMQB.commit_entry()
 *	}
