...
void hk_Rigid_Body_Core::add_to_mass_matrix_inv(hk_Core_VMQ_Input &input,
                                                hk_Dense_Matrix &matrix_out,
                                                hk_real velocities_out[]) {
  if (physical_unmoveable || pinned) return;
  hk_Single_Rigid_Body_CFAD *delta_spins =
      (hk_Single_Rigid_Body_CFAD *)input.m_buffer;

  {  // first step multiply all angular forces with the inertia tensor and
    ...
    do {
      ...
      hk_real linear_projected_vel =
          mq_a->m_linear.dot(_get_linear_velocity());  // IPION

      ds_a->m_delta_spin.set_mul3(_get_inv_body_inertia(), mq_a->m_angular);

      int i_dest_index = mq_a->m_matrix_index;
      vel[i_dest_index] +=
          spin_projected_vel + linear_projected_vel;  // Note: a double dot will
                                                      // perform faster on PS2

      hk_Virtual_Mass_Query *mq_b = input.m_vmq;

      for (int j = i - 1; j >= 0; j--) {
        hk_real spin_part = ds_a->m_delta_spin.dot(
            mq_b->m_angular);  // Note double dot will be faster on PS2
        hk_real linear_part =
            get_inv_mass() * mq_a->m_linear.dot(mq_b->m_linear);
      ...
      hk_real spin_part = ds_a->m_delta_spin.dot(
          mq_a->m_angular);  // Note double dot will be faster on PS2
...
void hk_Rigid_Body_Core::apply_impulses(hk_Core_VMQ_Input &input,
  ...
  do {
    ...
    _get_linear_velocity().add_mul(
        impulse_strength[mq->m_matrix_index] * this->get_inv_mass(),
        mq->m_linear);
    mq++;
    ds++;
  } while (--i >= 0);
