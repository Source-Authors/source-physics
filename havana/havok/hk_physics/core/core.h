...
class hk_Core : public hk_Object {
 public:
  ...
  enum HK_CORE_MOVEMENT_STATE { HK_CORE_MOVING, HK_CORE_SLOW, HK_CORE_STILL };
 ...
 public:
  ...
  //: change the velocities to reflect the simplified interpolated movement used
  ...
  //: called to update internal caches to optimize the hk_Synchronized_Core
  //: function calls

  virtual void update_body_transforms_and_positions(hk_timestamp time_stamp,
                                                    hk_time) = 0;
  //: updates all transforms of all controlled rigid bodies

  virtual void async_apply_impulse(hk_Impulse_Info &impulse,
                                   hk_real impulse_strength) = 0;
  //: apply an impulse which will become effective at the next
  //: commit_async_impulses() call

  // virtual void async_apply_impulses( int n_impulses, hk_Impulse_Info axis[],
  // hk_real impulse_strength[] );

  /* for collision events that are (possibly) inside the PSI */
  virtual void get_worst_case_values(hk_Entity *entity,
                                     hk_Vector3 &separating_plane_norm,
                                     hk_Worst_Case &wc) = 0;
  //: this method will give back the worst case vel. and accel. tangents (to the
  //: separating plane).
...
//: the next functions require that the core is synchronized, either by
//: explicitely calling synchronize core
...
class hk_Synchronized_Core : public hk_Core {
 public:
  ...
  virtual void speculative_step(hk_PSI_Info &, hk_PSI_Event_Collector &) = 0;
  ...
  virtual void add_to_mass_matrix_inv(hk_Core_VMQ_Input &input,
                                      hk_Dense_Matrix &matrix_out,
                                      hk_real velocities_out[]) {
    ;
  };
  virtual void add_to_velocities(hk_Core_VMQ_Input &input,
                                 hk_real velocities_out[]) {
    ;
  };
  ...
  // virtual void add_to_sparse_mass_matrix_inv( int n_axis, hk_Impulse_Info
  // axis[], hk_sparse_matrix &matrix_out) = 0;

  // virtual void build_3x3_mass_matrix_inv( hk_Vector3 &position_ws, hk_Matrix3
  // &inv_mass_matrix_out ) const = 0;
  //: build a 3x3 mass matrix for the 3 world space directions at a given
  //: position

  // virtual void get_projected_velocities( int n_vel, hk_Impulse_Info axis[],
  // hk_real velocities_out[]) const = 0;

  // virtual void get_velocity( hk_Body &body, hk_Vector3 &position_ws,
  // hk_Vector3 &velocity_out) =0;
  //: get the vecocity at a given point

  virtual void apply_impulses(int n_impulses, hk_Impulse_Info axis[],
                              const hk_real impulse_strength[]) {}
