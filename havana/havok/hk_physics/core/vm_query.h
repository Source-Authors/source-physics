...
#define HK_MAX_SIZEOF_CACHED_FORCE_AXIS_DESCRIPTION \
  (sizeof(hk_Cached_Force_Axis_Description))
...
class hk_Impulse_Info {
 public:
  ...
  /* is usually the
  (direction	of impulse) cross (point_of_contact -
  pivot_point(==center_of_mass))
  */
...
struct hk_Core_VMQ_Input {
  ...
  //: the sice of the buffer must be at least
  //: HK_MAX_SIZEOF_CACHED_FORCE_AXIS_DESCRIPTION * m_n_queries
};
