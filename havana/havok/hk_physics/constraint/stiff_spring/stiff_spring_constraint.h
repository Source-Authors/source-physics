#ifndef HK_STIFF_SPRING_CONSTRAINT_H
#define HK_STIFF_SPRING_CONSTRAINT_H

class hk_Stiff_Spring_BP;

class hk_Stiff_Spring_Constraint : public hk_Constraint {
 public:
  void apply_effector_PSI(hk_PSI_Info &, hk_Array<hk_Entity *> *);

  hk_Stiff_Spring_Constraint(hk_Environment *, const hk_Stiff_Spring_BP *,
                             hk_Rigid_Body *a, hk_Rigid_Body *b);
  hk_Stiff_Spring_Constraint(hk_Local_Constraint_System *,
                             const hk_Stiff_Spring_BP *, hk_Rigid_Body *a,
                             hk_Rigid_Body *b);

  inline ~hk_Stiff_Spring_Constraint() {}

  void set_length(hk_real);

  void init_constraint(const void *) override;
  void write_to_blueprint(hk_Stiff_Spring_BP *);

  const char *get_constraint_type() override { return "stiff_spring"; }

  int get_constraint_dof() override { return 5; }

 protected:
  void init_stiff_spring_constraint(const hk_Stiff_Spring_BP *);

  void step_constraint(hk_PSI_Info &pi, void *mem, hk_real tau_factor,
                       hk_real damp_factor) override;

  int get_vmq_storage_size() override;

  hk_Vector3 m_translation_os_ks[2];  // 32

  hk_real m_tau;                  // 4
  hk_real m_strength;             // 4
  hk_real m_stiff_spring_length;  // 4
  hk_real m_min_length;

 private:
  int setup_and_step_constraint(hk_PSI_Info &pi, void *mem, hk_real tau_factor,
                                hk_real damp_factor) override;
};

#endif  // HK_STIFF_SPRING_CONSTRAINT_H
