...
void hk_Stiff_Spring_Constraint::init_stiff_spring_constraint(
...
hk_Stiff_Spring_Constraint::hk_Stiff_Spring_Constraint(
    hk_Environment *env, const hk_Stiff_Spring_BP *bp, hk_Rigid_Body *a,
    hk_Rigid_Body *b)
...
hk_Stiff_Spring_Constraint::hk_Stiff_Spring_Constraint(
    hk_Local_Constraint_System *constraint_system, const hk_Stiff_Spring_BP *bp,
    hk_Rigid_Body *a, hk_Rigid_Body *b)
    : hk_Constraint(constraint_system, a, b, HK_PRIORITY_LOCAL_CONSTRAINT,
                    HK_NEXT_MULTIPLE_OF(
                        16, static_cast<int>(sizeof(hk_Stiff_Spring_Work)))) {
  init_stiff_spring_constraint(bp);
}
...
int hk_Stiff_Spring_Constraint::get_vmq_storage_size() {
  return HK_NEXT_MULTIPLE_OF(16,
                             static_cast<int>(sizeof(hk_Stiff_Spring_Work)));
}
...
int hk_Stiff_Spring_Constraint::setup_and_step_constraint(hk_PSI_Info &pi,
                                                          void *mem,
                                                          hk_real tau_factor,
                                                          hk_real damp_factor) {
  ...
  translation_ws_ks[0]._set_transformed_pos(b0->get_cached_transform(),
                                            m_translation_os_ks[0]);
  translation_ws_ks[1]._set_transformed_pos(b1->get_cached_transform(),
                                            m_translation_os_ks[1]);
  ...
  work.current_dist =
      IntervalDistance(norm_length, m_min_length, m_stiff_spring_length);

  if (hk_Math::almost_equal(this->m_min_length, this->m_stiff_spring_length) ||
      work.current_dist != 0.f)
    work.skip_solve = false;
  else {
    hk_Vector3 next_translation_ws_ks[2];
    next_translation_ws_ks[0]._set_transformed_pos(
        b0->get_transform_next_PSI(pi.get_delta_time()),
        m_translation_os_ks[0]);
    next_translation_ws_ks[1]._set_transformed_pos(
        b1->get_transform_next_PSI(pi.get_delta_time()),
        m_translation_os_ks[1]);
    ...
    hk_real next_dist =
        IntervalDistance(next_norm_length, m_min_length, m_stiff_spring_length);
  ...
  query_engine.begin(1);
  {
    ...
    {
      query_engine.add_linear(0, HK_BODY_A, b0, translation_ws_ks[0], dir,
                              1.0f);
      query_engine.add_linear(0, HK_BODY_B, b1, translation_ws_ks[0], dir,
                              -1.0f);
    }
  ...
  {  // step
    ...
    hk_real delta_dist =
        tau_factor * m_tau * pi.get_inv_delta_time() * work.current_dist -
...
void hk_Stiff_Spring_Constraint::step_constraint(hk_PSI_Info &pi, void *mem,
                                                 hk_real tau_factor,
                                                 hk_real damp_factor) {
...
void hk_Stiff_Spring_Constraint::apply_effector_PSI(hk_PSI_Info &pi,
                                                    hk_Array<hk_Entity *> *) {
  hk_Stiff_Spring_Work work_mem;
  hk_Stiff_Spring_Constraint::setup_and_step_constraint(pi, (void *)&work_mem,
                                                        1.0f, 1.0f);
}
