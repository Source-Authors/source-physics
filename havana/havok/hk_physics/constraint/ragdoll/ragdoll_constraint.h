...
class hk_Ragdoll_Constraint : public hk_Constraint {
 public:
  hk_Ragdoll_Constraint(hk_Environment *, const hk_Ragdoll_Constraint_BP *,
                        hk_Rigid_Body *a, hk_Rigid_Body *b);
  hk_Ragdoll_Constraint(hk_Local_Constraint_System *,
                        const hk_Ragdoll_Constraint_BP *, hk_Rigid_Body *a,
                        hk_Rigid_Body *b);
  ...
  void init_ragdoll_constraint(const hk_Ragdoll_Constraint_BP *,
                               hk_Local_Constraint_System *sys = HK_NULL);
  //: update all parameters of the ragdoll constraint,

  void write_to_blueprint(hk_Ragdoll_Constraint_BP *);

  const char *get_constraint_type() override { return "ragdoll"; }

  int get_constraint_dof() override { return 3; }

  inline hk_Transform get_transform(int x) const {
    return m_transform_os_ks[x];
  }

  void update_transforms(const hk_Transform &os_ks_0,
