...
class hk_Ragdoll_Constraint_Work {
 public:
#ifdef HK_ARCH_PPC
  static inline void *operator new(size_t, void *addr) { return addr; }
#else
  static inline void *operator new(size_t, void *addr) { return addr; }
  ...
  inline hk_real get_twist_axis_length() { return twist_axis_ws.w; }
};
...
void hk_Ragdoll_Constraint::init_constraint(const void *vbp) {
  const hk_Ragdoll_Constraint_BP *bp =
      static_cast<const hk_Ragdoll_Constraint_BP *>(vbp);
  this->init_ragdoll_constraint(bp);
}

void hk_Ragdoll_Constraint::init_ragdoll_constraint(
    const hk_Ragdoll_Constraint_BP *bp, hk_Local_Constraint_System *sys) {
  ...
  hk_real center_cone = (bp->m_limits[HK_LIMIT_CONE].m_limit_max +
                         bp->m_limits[HK_LIMIT_CONE].m_limit_min) *
                        0.5f;
  hk_real diff_cone = bp->m_limits[HK_LIMIT_CONE].m_limit_max -
                      bp->m_limits[HK_LIMIT_CONE].m_limit_min;
  ...
  m_limits[HK_LIMIT_PLANES].m_limit_min =
      -hk_Math::sin(bp->m_limits[HK_LIMIT_PLANES].m_limit_max);
  m_limits[HK_LIMIT_PLANES].m_limit_max =
      -hk_Math::sin(bp->m_limits[HK_LIMIT_PLANES].m_limit_min);
...
hk_Ragdoll_Constraint::hk_Ragdoll_Constraint(hk_Environment *env,
                                             const hk_Ragdoll_Constraint_BP *bp,
...
hk_Ragdoll_Constraint::hk_Ragdoll_Constraint(
    ...
    const hk_Ragdoll_Constraint_BP *bp, hk_Rigid_Body *a, hk_Rigid_Body *b)
    : hk_Constraint(
          constraint_system, a, b, HK_PRIORITY_LOCAL_CONSTRAINT,
...
void hk_Ragdoll_Constraint::apply_angular_part(hk_PSI_Info &pi,
  ...
  hk_Constraint_Limit_Util::do_angular_limit(
      pi, b0, work.twist_axis_ws, work.joint_angles(HK_LIMIT_TWIST), b1,
      m_limits[HK_LIMIT_TWIST], tau_factor * twist_factor,
      strength_factor * twist_factor);
  ...
  do {
    ...
    hk_Vector3 &plane_dir_ws =
        (i == HK_LIMIT_CONE) ? work.twist_axis_Ref_ws : work.plane_Axis_Ref_ws;

    hk_Vector3 &dir_ws = work.twist_axis_Att_ws;

    hk_Constraint_Limit_Util::do_angular_plane_limit(
        pi, b0, b1, dir_ws, plane_dir_ws, current_pos, limit, tau_factor,
        strength_factor);
  } while (--i >= 1);
}
...
int hk_Ragdoll_Constraint::setup_and_step_constraint(hk_PSI_Info &pi, void *mem,
                                                     hk_real tau_factor,
                                                     hk_real strength_factor) {
  ...
  { /* setup and step ANGULAR PARTS */
    ...
    {  // twist setup
      work.twist_axis_ws.set_add(work.twist_axis_Ref_ws,
                                 work.twist_axis_Att_ws);
      ...
      m_ws_us.get_column(1).set_cross(work.twist_axis_ws,
                                      work.plane_Axis_Ref_ws);
      m_ws_us.get_column(1).normalize();
      m_ws_us.get_column(2).set_cross(m_ws_us.get_column(1),
                                      m_ws_us.get_column(0));

      hk_Vector3 perp_axes_Att_ws;
      perp_axes_Att_ws.set_rotated_dir(t1, m_transform_os_ks[1].get_column(2));

      hk_Vector3 a_us;
      a_us.set_rotated_inv_dir(m_ws_us, perp_axes_Att_ws);

      work.joint_angles(HK_LIMIT_TWIST) = -hk_Math::atan2(a_us.y, a_us.z);
    }
    // planes setup
    work.joint_angles(HK_LIMIT_CONE) =
        work.twist_axis_Ref_ws.dot(work.twist_axis_Att_ws);
    work.joint_angles(HK_LIMIT_PLANES) =
        work.plane_Axis_Ref_ws.dot(work.twist_axis_Att_ws);

    this->apply_angular_part(pi, work, tau_factor, strength_factor);
  }

  if (m_constrainTranslation) { /* setup and step LINEAR PART */
    hk_VM_Query_Builder<hk_VMQ_Storage<3>> &query_engine =
        work.query_engine_linear;

    hk_Vector3 position_ws[2];
    position_ws[0]._set_transformed_pos(t0,
                                        m_transform_os_ks[0].get_translation());
    position_ws[1]._set_transformed_pos(t1,
    ...
    if (lcs) lcs->report_square_error(dir.length_squared());
    ...
    query_engine.begin(3);
    {
      ...
      query_engine.add_linear_xyz(0, HK_X_DIRECTION, HK_BODY_A, b0, mcr_0,
                                  1.0f);
      query_engine.add_linear_xyz(0, HK_X_DIRECTION, HK_BODY_B, b1, mcr_1,
                                  -1.0f);

      query_engine.add_linear_xyz(1, HK_Y_DIRECTION, HK_BODY_A, b0, mcr_0,
                                  1.0f);
      query_engine.add_linear_xyz(1, HK_Y_DIRECTION, HK_BODY_B, b1, mcr_1,
                                  -1.0f);

      query_engine.add_linear_xyz(2, HK_Z_DIRECTION, HK_BODY_A, b0, mcr_0,
                                  1.0f);
      query_engine.add_linear_xyz(2, HK_Z_DIRECTION, HK_BODY_B, b1, mcr_1,
                                  -1.0f);
    ...
    hk_real *approaching_velocity =
        query_engine.get_vmq_storage().get_velocities();

    hk_Vector3 delta_dist_3;
    delta_dist_3.set_mul(tau_factor * m_tau * pi.get_inv_delta_time(), dir);
    // RaphaellIT7: Fix for ragdoll jittering from ZenMax.
    ...
    hk_Fixed_Dense_Matrix<3> &mass_matrix =
        query_engine.get_vmq_storage().get_fixed_dense_matrix();
...
void hk_Ragdoll_Constraint::step_constraint(hk_PSI_Info &pi, void *mem,
                                            hk_real tau_factor,
                                            hk_real strength_factor) {
  hk_Rigid_Body *b0 = get_rigid_body(0);
  hk_Rigid_Body *b1 = get_rigid_body(1);
  ...
  if (m_constrainTranslation) { /* step LINEAR */
    hk_Vector3 &dir = work.dir;
    hk_VM_Query_Builder<hk_VMQ_Storage<3>> &query_engine =
        work.query_engine_linear;
    ...
    hk_Vector3 delta_dist_3;
    hk_Preallocated_Dense_Vector delta(delta_dist_3.get_real_pointer(), 3,
                                       4);  //-V112

    hk_real *approaching_velocity =
        query_engine.get_vmq_storage().get_velocities();
    delta_dist_3.set_mul(tau_factor * m_tau * pi.get_inv_delta_time(), dir);
    delta_dist_3.add_mul(-1.0f * strength_factor * m_strength,
                         *(hk_Vector3 *)approaching_velocity);
...
void hk_Ragdoll_Constraint::apply_effector_PSI(hk_PSI_Info &pi,
                                               hk_Array<hk_Entity *> *) {
  hk_Ragdoll_Constraint_Work work_mem;
  hk_Ragdoll_Constraint::setup_and_step_constraint(pi, (void *)&work_mem, 1.0f,
                                                   1.0f);
}
...
void hk_Ragdoll_Constraint::update_transforms(const hk_Transform &os_ks_0,
