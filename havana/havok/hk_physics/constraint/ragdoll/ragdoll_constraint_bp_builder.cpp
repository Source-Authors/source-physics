...
hk_Ragdoll_Constraint_BP_Builder::initialize_from_limited_ball_socket_bp(
  ...
  switch (number_of_freedom) {
    ...
    case 1: {  // limited hinge
      ...
      r.m_limits[HK_LIMIT_CONE].set_limits(-limit_diff[axis] * 0.5f,
                                           limit_diff[axis] * 0.5f);
    ...
    case 2:
    case 3: {
      {  // first search the axis with the lowest rot inertia, which is should
         // be used for the twist axis in Aos
        ...
        // get the pivot point
        hk_Vector3 pos_Ref_ws;
        pos_Ref_ws.set_transformed_pos(
            t0, r.m_transform_os_ks[0].get_translation());
        hk_Vector3 pos_Att_ws;
        pos_Att_ws.set_transformed_pos(
            t1, r.m_transform_os_ks[1].get_translation());
        ...
        for (int i = 0; i < 3; i++) {
          // get the axis
          hk_Vector3 axis_Ref_ws;
          axis_Ref_ws.set_rotated_dir(t0, r.m_transform_os_ks[0].get_column(i));
          hk_Vector3 axis_Att_ws;
          axis_Att_ws.set_rotated_dir(t1, r.m_transform_os_ks[1].get_column(i));
          ...
          hk_real inv_virt_mass =
              query.get_vmq_storage().get_dense_matrix()(0, 0);

          hk_Vector3 cross_a;
          cross_a.set_cross(pos_Ref_ws, axis_Ref_ws);
          hk_Vector3 cross_b;
          cross_b.set_cross(pos_Att_ws, axis_Att_ws);
        ...
        int l_axis = (axis_of_min_inertia == 2)
                         ? 0
                         : axis_of_min_inertia + 1;  // axis with the lower diff
        int u_axis =
            (axis_of_min_inertia == 0)
                ? 2
                : axis_of_min_inertia - 1;  // axis with the higher diff
        ...
        // resort matrix and limits
        r.m_transform_os_ks[0].get_column(0) =
            m0.get_column(axis_of_min_inertia);
        r.m_transform_os_ks[1].get_column(0) =
            m1.get_column(axis_of_min_inertia);
        ...
        r.m_limits[HK_LIMIT_TWIST].set_limits(
            bp->m_angular_limits[axis_of_min_inertia].m_min,
            bp->m_angular_limits[axis_of_min_inertia].m_max);
        r.m_limits[HK_LIMIT_CONE].set_limits(-limit_diff[u_axis] * 0.5f,
                                             limit_diff[u_axis] * 0.5f);
        r.m_limits[HK_LIMIT_PLANES].set_limits(
            bp->m_angular_limits[l_axis].m_min,
            bp->m_angular_limits[l_axis].m_max);
...
void hk_Ragdoll_Constraint_BP_Builder::set_ragdoll_constraint(
    hk_Rigid_Body *a, hk_Rigid_Body *b, const hk_Vector3 &pivot_point_ws,
    ...
    const hk_Interval<hk_real> &plane_limits, bool constrainTranslation) {
  hk_Ragdoll_Constraint_BP &r = this->m_ragdoll_constraint_bp;
  ...
  r.m_limits[HK_LIMIT_PLANES].set_limits(plane_limits.m_min,
                                         plane_limits.m_max);
