...
#ifdef HK_ARCH_PPC
  static inline void* operator new(size_t size, void* addr) { return addr; }
#else
...
void hk_Car_Wheel_Constraint::init_car_wheel_constraint(
...

  {
    ...
    hk_Constraint_Limit_Util::init_linear_limit(steering_axis_ws, delta_pos_ws,
                                                m_suspension_limit);
...
void hk_Car_Wheel_Constraint::set_steering_angle(hk_real angle) {
  ...
  hk_Vector3 sin_axis;
  sin_axis.set_cross(m_initial_hinge_axis_Ros, m_steering_axis_Ros);
...
hk_Car_Wheel_Constraint::hk_Car_Wheel_Constraint(
    hk_Local_Constraint_System* constraint_system, const hk_Car_Wheel_BP* bp,
    hk_Rigid_Body* a, hk_Rigid_Body* b)
    : hk_Constraint(
          constraint_system, a, b, HK_PRIORITY_LOCAL_CONSTRAINT,
          HK_NEXT_MULTIPLE_OF(16, sizeof(hk_Car_Wheel_Constraint_Work))) {
  init_car_wheel_constraint(bp);
}
...
int hk_Car_Wheel_Constraint::setup_and_step_constraint(hk_PSI_Info& pi,
                                                       void* mem,
                                                       hk_real tau_factor,
                                                       hk_real damp_factor) {
  hk_Rigid_Body* b0 = get_rigid_body(0);
  ...
  if (this->m_suspension_limit.m_limit_is_enabled |
      m_suspension_limit.m_friction_is_enabled) {  // joint friction
    hk_Constraint_Limit_Util::do_linear_limit(
        pi, b0, b1, orig_ws[0], steering_axis_ws, delta_pos_ws,
        m_suspension_limit, tau_factor, damp_factor);
  }

  if (this->m_wheel_limit.m_limit_is_enabled |
      m_wheel_limit.m_friction_is_enabled) {  // joint friction
    ...
    hk_Constraint_Limit_Util::do_angular_limit(pi, b0, hinge_axis_ws[0], alpha,
                                               b1, m_wheel_limit, tau_factor,
                                               damp_factor);
  }
...

  {
    ...
    query_engine.add_linear(0, HK_BODY_B, b1, mcr_pt_1, hinge_axis_ws[0],
                            -1.0f);

    query_engine.add_linear(1, HK_BODY_A, b0, mcr_pt_0, perp_steering_axis_ws,
                            1.0f);
    query_engine.add_linear(1, HK_BODY_B, b1, mcr_pt_1, perp_steering_axis_ws,
                            -1.0f);
  ...
  hk_Fixed_Dense_Vector<4> delta;
  {
    ...
    delta.set_mul_add_mul(m_tau * tau_factor, work.m_correction,
                          -1.0f * m_damp * damp_factor, vmq_velocity);
  }

  hk_Fixed_Dense_Matrix<4>& mass_matrix =
...
void hk_Car_Wheel_Constraint::step_constraint(hk_PSI_Info& pi, void* mem,
                                              hk_real tau_factor,
                                              hk_real damp_factor) {
...
void hk_Car_Wheel_Constraint::apply_effector_PSI(hk_PSI_Info& pinfo,
  ...
  hk_Car_Wheel_Constraint::setup_and_step_constraint(pinfo, (void*)&tmp_mem,
                                                     1.0f, 1.0f);
}
