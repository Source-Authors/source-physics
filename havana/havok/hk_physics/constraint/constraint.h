...
class hk_Constraint : public hk_Rigid_Body_Binary_EF {
 ...
 public:
  inline hk_Constraint(hk_Environment *env, hk_Rigid_Body *a, hk_Rigid_Body *b,
                       hk_effector_priority redundend_prio)
      : hk_Rigid_Body_Binary_EF(env, a, b, redundend_prio),
        m_constraint_system(HK_NULL),
        m_client_data(HK_NULL) {}

  hk_Constraint(hk_Local_Constraint_System *sys, hk_Rigid_Body *a,
                hk_Rigid_Body *b, hk_effector_priority redundend_prio,
                int redundant_storage_size);
  ...
  inline void constraint_system_deleted_event(
      const hk_Local_Constraint_System *pSystem) {
    if (m_constraint_system == pSystem) m_constraint_system = NULL;
  }
  ...
  virtual const char *get_constraint_type() { return "unknown"; }

  virtual int get_constraint_dof() { return 0; }
