...
hk_Pulley_Constraint::hk_Pulley_Constraint(hk_Environment *env,
...
hk_Pulley_Constraint::hk_Pulley_Constraint(
    hk_Local_Constraint_System *constraint_system, const hk_Pulley_BP *bp,
    hk_Rigid_Body *a, hk_Rigid_Body *b)
    : hk_Constraint(
          constraint_system, a, b, HK_PRIORITY_LOCAL_CONSTRAINT,
          HK_NEXT_MULTIPLE_OF(16, static_cast<int>(sizeof(hk_Pulley_Work)))) {
  init_pulley_constraint(bp);
}

void hk_Pulley_Constraint::set_length(hk_real length) { m_length = length; }

void hk_Pulley_Constraint::set_gearing(hk_real gearing) { m_gearing = gearing; }

void hk_Pulley_Constraint::set_worldspace_point(int index,
                                                const hk_Vector3 &ws_point) {
...
int hk_Pulley_Constraint::setup_and_step_constraint(hk_PSI_Info &pi, void *mem,
                                                    hk_real tau_factor,
                                                    hk_real damp_factor) {
  ...
  work.current_length = dir[0].normalize_with_length() +
                        dir[1].normalize_with_length() - m_length;
...

  {
    ...
    {
      query_engine.add_linear(0, HK_BODY_A, b0, translation_ws_ks[0], dir[0],
                              1.0f);
      query_engine.add_linear(0, HK_BODY_B, b1, translation_ws_ks[1], dir[1],
                              1.0f);
    }
  ...
  {  // step
    if (m_is_rigid || work.current_length >= 0) {
      ...
      hk_real delta_dist =
          tau_factor * m_tau * pi.get_inv_delta_time() * work.current_length -
...
void hk_Pulley_Constraint::step_constraint(hk_PSI_Info &pi, void *mem,
                                           hk_real tau_factor,
                                           hk_real damp_factor) {
...
void hk_Pulley_Constraint::apply_effector_PSI(hk_PSI_Info &pi,
                                              hk_Array<hk_Entity *> *) {
  hk_Pulley_Work work_mem;
  hk_Pulley_Constraint::setup_and_step_constraint(pi, (void *)&work_mem, 1.0f,
                                                  1.0f);
}
