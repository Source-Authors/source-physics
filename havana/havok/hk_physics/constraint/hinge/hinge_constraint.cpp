...
void hk_Hinge_Constraint::init_hinge_constraint(const hk_Hinge_BP* bp) {
  ...
  for (int i = 0; i < 2; ++i) {
    if (bp->m_axis_perp_os[i].length() != 0.0f) {
      ...
      m_axis_perp_os[i] =
          hk_Vector3_Util::perp_vec(m_axis_os[i].get_direction());
    }
  }
  ...
  hk_Constraint_Limit_Util::init_angular_limit(perp_x_ws, perp_y_ws,
                                               this->m_limit, perp_x2_ws);
}
...
hk_Hinge_Constraint::hk_Hinge_Constraint(
    hk_Local_Constraint_System* constraint_system, const hk_Hinge_BP* bp,
    hk_Rigid_Body* a, hk_Rigid_Body* b)
    : hk_Constraint(constraint_system, a, b, HK_PRIORITY_LOCAL_CONSTRAINT,
                    HK_NEXT_MULTIPLE_OF(16, sizeof(hk_Hinge_Constraint_Work))) {
  hk_Hinge_Constraint::init_constraint(bp);
}
...
int hk_Hinge_Constraint::setup_and_step_constraint(hk_PSI_Info& pi, void* mem,
                                                   hk_real tau_factor,
  ...
  if (this->m_limit.m_limit_is_enabled |
      m_limit.m_friction_is_enabled)  // joint friction
  {
    hk_Vector3 perp_x2_ws;
    perp_x2_ws.set_rotated_dir(t1, m_axis_perp_os[1]);
    ...
    hk_Constraint_Limit_Util::do_angular_limit(
        pi, b0, line_ws[0], alpha, b1, m_limit, tau_factor, strength_factor);
  }
...

  {
    ...
    // point-to-point
    query_engine.add_linear_xyz(0, HK_X_DIRECTION, HK_BODY_A, b0, mcr_pt_0,
                                1.0f);
    query_engine.add_linear_xyz(0, HK_X_DIRECTION, HK_BODY_B, b1, mcr_pt_1,
                                -1.0f);

    query_engine.add_linear_xyz(1, HK_Y_DIRECTION, HK_BODY_A, b0, mcr_pt_0,
                                1.0f);
    query_engine.add_linear_xyz(1, HK_Y_DIRECTION, HK_BODY_B, b1, mcr_pt_1,
                                -1.0f);

    query_engine.add_linear_xyz(2, HK_Z_DIRECTION, HK_BODY_A, b0, mcr_pt_0,
                                1.0f);
    query_engine.add_linear_xyz(2, HK_Z_DIRECTION, HK_BODY_B, b1, mcr_pt_1,
                                -1.0f);
  ...
  hk_Fixed_Dense_Vector<5> delta;
  {
    ...
    work.m_correction(4) =
        pi.get_inv_delta_time() * -perp_x_ws.dot(line_ws[1]);  //-V112

    delta.set_mul_add_mul(m_tau * tau_factor, work.m_correction,
                          -1.0f * m_strength * strength_factor, vmq_velocity);
  }

  hk_Fixed_Dense_Matrix<5>& mass_matrix =
...
void hk_Hinge_Constraint::step_constraint(hk_PSI_Info& pi, void* mem,
                                          hk_real tau_factor,
                                          hk_real strength_factor) {
  ...
  hk_Dense_Matrix& mass_matrix =
      query_engine.get_vmq_storage().get_dense_matrix();
...
void hk_Hinge_Constraint::apply_effector_PSI(hk_PSI_Info& pinfo,
  ...
  hk_Hinge_Constraint::setup_and_step_constraint(pinfo, (void*)&tmp_mem, 1.0f,
                                                 1.0f);
}
                                                                                                ...
                                                                                                hk_Array<hk_Entity*>* )
{
        ...
        hk_Vector3 orig_ws[2];
        orig_ws[0].set_transformed_pos( b0->get_cached_transform(),
m_axis_os[0].get_origin()); orig_ws[1].set_transformed_pos(
b1->get_cached_transform(), m_axis_os[1].get_origin());

        hk_Vector3 line_ws[2];
        line_ws[0].set_transformed_pos( b0->get_cached_transform(),
                        m_axis_os[0].get_origin() +
m_axis_os[0].get_direction()); line_ws[1].set_transformed_pos(
b1->get_cached_transform(), m_axis_os[1].get_origin() +
m_axis_os[1].get_direction());

        extern void HK_DISPLAY_POINT(const hk_Vector3& p, int c);
        extern void HK_DISPLAY_RAY(const hk_Vector3& p, const hk_Vector3& d, int
c);
...

        {
                ...
                // point-to-point
                query_engine.add_linear_xyz( 0, HK_X_DIRECTION, HK_BODY_A, b0,
mcr_pt_0,  1.0f ); query_engine.add_linear_xyz( 0, HK_X_DIRECTION, HK_BODY_B,
b1, mcr_pt_1, -1.0f );

                query_engine.add_linear_xyz( 1, HK_Y_DIRECTION, HK_BODY_A, b0,
mcr_pt_0,  1.0f ); query_engine.add_linear_xyz( 1, HK_Y_DIRECTION, HK_BODY_B,
b1, mcr_pt_1, -1.0f );

                query_engine.add_linear_xyz( 2, HK_Z_DIRECTION, HK_BODY_A, b0,
mcr_pt_0,  1.0f ); query_engine.add_linear_xyz( 2, HK_Z_DIRECTION, HK_BODY_B,
b1, mcr_pt_1, -1.0f );
                ...
                query_engine.add_linear_xyz( 3, HK_X_DIRECTION, HK_BODY_A, b0,
mcr_ln_0,  1.0f ); query_engine.add_linear_xyz( 3, HK_X_DIRECTION, HK_BODY_B,
b1, mcr_ln_1, -1.0f );

                query_engine.add_linear_xyz( 4, HK_Y_DIRECTION, HK_BODY_A, b0,
mcr_ln_0,  1.0f ); query_engine.add_linear_xyz( 4, HK_Y_DIRECTION, HK_BODY_B,
b1, mcr_ln_1, -1.0f );
        ...
        const hk_real* vmq_velocity =
query_engine.get_vmq_storage().get_velocities();

        hk_Vector3 point_velocity( vmq_velocity[0], vmq_velocity[1],
vmq_velocity[2] ); hk_Vector3 line_velocity( vmq_velocity[3], vmq_velocity[4], 0
);
        ...
        hk_Dense_Matrix& mass_matrix =
query_engine.get_vmq_storage().get_dense_matrix(); hk_result solve_ok =
hk_Dense_Matrix_Util::solve( mass_matrix, impulse, 0.0f);

        HK_DISPLAY_RAY(orig_ws[0],
hk_Vector3(-impulse(0),-impulse(1),-impulse(2)), 0x00ffff);
        HK_DISPLAY_RAY(line_ws[0], hk_Vector3(-impulse(3),-impulse(4),0),
0x00ffff);

        HK_ASSERT(solve_ok == HK_OK);

        query_engine.apply_impulses( HK_BODY_A, b0,
impulse.get_const_real_pointer() ); query_engine.apply_impulses( HK_BODY_B, b1,
impulse.get_const_real_pointer() );
}
*/
