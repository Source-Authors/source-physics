...
void hk_Hinge_BP_Builder::set_position_os(int body_index,
                                          const hk_Vector3 &position) {
  m_hinge_bp.m_axis_os[body_index].m_origin = position;
}

void hk_Hinge_BP_Builder::set_axis_ws(hk_Rigid_Body *a, hk_Rigid_Body *b,
                                      const hk_Vector3 &axis_ws) {
  hk_Vector3 n_axis = axis_ws;
  n_axis.normalize();
  m_hinge_bp.m_axis_os[0].m_direction.set_rotated_inv_dir(
      a->get_cached_transform(), n_axis);
  m_hinge_bp.m_axis_os[1].m_direction.set_rotated_inv_dir(
      b->get_cached_transform(), n_axis);
}

void hk_Hinge_BP_Builder::set_angular_motor(hk_real angular_velocity,
                                            hk_real max_torque) {
...
void hk_Hinge_BP_Builder::set_tau(hk_real tau) { m_hinge_bp.m_tau = tau; }

void hk_Hinge_BP_Builder::set_axis_perp_os(int body,
                                           const hk_Vector3 &axis_os) {
