...
class hk_Hinge_Constraint : public hk_Constraint {
 public:
  hk_Hinge_Constraint(hk_Environment *, const hk_Hinge_BP *, hk_Rigid_Body *a,
                      hk_Rigid_Body *b);
  hk_Hinge_Constraint(hk_Local_Constraint_System *, const hk_Hinge_BP *,
                      hk_Rigid_Body *a, hk_Rigid_Body *b);

  int get_vmq_storage_size() override;

  int setup_and_step_constraint(hk_PSI_Info &pi, void *mem, hk_real tau_factor,
  ...
  const char *get_constraint_type() override { return "hinge"; }

  int get_constraint_dof() override { return 1; }
 ...
 protected:
  ...
  hk_real
      m_tau;  // XXX rename me to m_tau - this is drift stabilization paramater
  hk_real m_strength;  // XXX rename me to m_strength or similar - 1.0 is pure
                       // velocity constraint
