...
class hk_Prismatic_Constraint : public hk_Constraint {
 public:
  hk_Prismatic_Constraint(hk_Environment *, const hk_Prismatic_BP *,
                          hk_Rigid_Body *Ref, hk_Rigid_Body *Att);
  hk_Prismatic_Constraint(hk_Local_Constraint_System *, const hk_Prismatic_BP *,
                          hk_Rigid_Body *Ref, hk_Rigid_Body *Att);
  ...
  const char *get_constraint_type() override { return "prismatic"; }

  int get_constraint_dof() override { return 1; }

 protected:
  void init_prismatic_constraint(const hk_Prismatic_BP *,
                                 hk_Local_Constraint_System *sys);

  hk_real m_strength;
  hk_real m_tau;
  hk_real m_pos_interpolation_value;  // if 0.0f than t0.get_position() is the
                                      // reference pos, 1.0f
