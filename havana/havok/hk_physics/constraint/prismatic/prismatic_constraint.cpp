...
void hk_Prismatic_Constraint::init_prismatic_constraint(
    const hk_Prismatic_BP* bp, hk_Local_Constraint_System* sys) {
  m_tau = bp->m_tau;
  ...
  hk_Vector3 axis_ws;
  axis_ws.set_rotated_dir(t_ws_f_Ros, m_axis_Ros);
  hk_Vector3 pos_ws;
  pos_ws.set_interpolate(t_ws_f_Ros.get_translation(),
                         t_ws_f_Aos.get_translation(),
                         m_pos_interpolation_value);

  hk_Vector3 delta_ws;
  {
    delta_ws.set_sub(t_ws_f_Aos.get_translation(),
                     t_ws_f_Ros.get_translation());
...
hk_Prismatic_Constraint::hk_Prismatic_Constraint(hk_Environment* env,
...
hk_Prismatic_Constraint::hk_Prismatic_Constraint(
    hk_Local_Constraint_System* constraint_system, const hk_Prismatic_BP* bp,
...
int hk_Prismatic_Constraint::setup_and_step_constraint(
    hk_PSI_Info& pi, void* mem, hk_real tau_factor, hk_real strength_factor) {
  ...
  hk_Vector3 axis_ws;
  axis_ws.set_rotated_dir(t_ws_f_Ros, m_axis_Ros);
  hk_Vector3 perp_axis_ws;
  perp_axis_ws.set_rotated_dir(t_ws_f_Ros, m_perp_axis_Ros);
  hk_Vector3 perp_axis2_ws;
  perp_axis2_ws.set_cross(axis_ws, perp_axis_ws);

  hk_Vector3 delta_ws;
  {
    hk_Vector3 shift_ws_Ros;
    shift_ws_Ros.set_rotated_dir(t_ws_f_Ros,
                                 m_transform_Ros_Aos.get_translation());
    delta_ws.set_sub(t_ws_f_Aos.get_translation(),
                     t_ws_f_Ros.get_translation());
    // delta_ws.set_sub( delta_ws, m_transform_Ros_Aos.get_translation());
  ...
  {
    work.m_correction(3) = tau_factor * m_tau * pi.get_inv_delta_time() *
                           delta_ws.dot(perp_axis_ws);
    work.m_correction(4) = tau_factor * m_tau * pi.get_inv_delta_time() *
                           delta_ws.dot(perp_axis2_ws);  //-V112
  }
  ...
  if (this->m_limits[0].m_limit_is_enabled |
      m_limits[0].m_friction_is_enabled) {  // joint friction
    hk_Constraint_Limit_Util::do_linear_limit(pi, b0, b1, pos_ws, axis_ws,
                                              delta_ws, m_limits[0], tau_factor,
                                              strength_factor);
  }
...

  {
    ...
    {
      query_engine.add_angular(0, HK_BODY_A, b0, t_ws_f_Ros.get_column(0),
                               1.0f);
      query_engine.add_angular(0, HK_BODY_B, b1, t_ws_f_Ros.get_column(0),
                               -1.0f);

      query_engine.add_angular(1, HK_BODY_A, b0, t_ws_f_Ros.get_column(1),
                               1.0f);
      query_engine.add_angular(1, HK_BODY_B, b1, t_ws_f_Ros.get_column(1),
                               -1.0f);

      query_engine.add_angular(2, HK_BODY_A, b0, t_ws_f_Ros.get_column(2),
                               1.0f);
      query_engine.add_angular(2, HK_BODY_B, b1, t_ws_f_Ros.get_column(2),
                               -1.0f);
    }
    {
      hk_Vector3 pos;
      pos.set_interpolate(t_ws_f_Ros.get_translation(),
                          t_ws_f_Aos.get_translation(),
                          m_pos_interpolation_value);
      ...
      query_engine.add_linear(4, HK_BODY_A, b0, mcr_0, perp_axis2_ws,
                              1.0f);  //-V112
      query_engine.add_linear(4, HK_BODY_B, b1, mcr_1, perp_axis2_ws,
                              -1.0f);  //-V112
    }
  ...
  hk_Fixed_Dense_Vector<5> delta;
  {
    ...
    delta.set_add_mul(work.m_correction, -1.0f * m_strength * strength_factor,
                      vmq_velocity);
  }

  hk_Fixed_Dense_Matrix<5>& mass_matrix =
      query_engine.get_vmq_storage().get_fixed_dense_matrix();
  [[maybe_unused]] hk_result invert_ok =
...
void hk_Prismatic_Constraint::step_constraint(hk_PSI_Info& pi, void* mem,
                                              hk_real tau_factor,
  ...
  if (this->m_limits[0].m_limit_is_enabled |
    ...
    //   hk_Vector3 delta_ws;
    //   delta_ws.set_sub( t_ws_f_Aos.get_translation(),
    //   t_ws_f_Ros.get_translation()); delta_ws.set_sub( delta_ws,
    //   m_transform_Ros_Aos.get_translation());
    //
    // Rotate m_transform_Ros_Aos.get_translation() into worldspace using the
    // reference object transform (this wasn't happening before - delta_ws
    // wasn't being calculated in worldspace). This code is essentially cribbed
    // from the same section in setup_and_step_constraint, above. Code CB, code
    // review OS.
    hk_Vector3 shift_ws_Ros;
    shift_ws_Ros.set_rotated_dir(t_ws_f_Ros,
                                 m_transform_Ros_Aos.get_translation());
  ...
  hk_Fixed_Dense_Vector<5> delta;
  delta.set_mul_add_mul(tau_factor, work.m_correction,
...
void hk_Prismatic_Constraint::apply_effector_PSI(hk_PSI_Info& pi,
                                                 hk_Array<hk_Entity*>*) {
  hk_Prismatic_Work work_mem;
  hk_Prismatic_Constraint::setup_and_step_constraint(pi, (void*)&work_mem, 1.0f,
                                                     1.0f);
}
...
void hk_Prismatic_Constraint::set_motor(hk_real desired_vel,
                                        hk_real max_force) {
