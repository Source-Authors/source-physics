...
class hk_Constraint_Limit_Util {
 public:
  static inline void do_angular_limit(
      hk_PSI_Info &pi, hk_Rigid_Body *b0,
      const hk_Vector3
          &rot_axis_Ref_ws,  // the axis the objects are rotating around
      hk_real alpha,         // the current angle
      hk_Rigid_Body *b1, hk_Constraint_Limit &limit, hk_real tau_factor,
      hk_real damp_factor) {
    ...
    hk_real virtual_mass =
        1.0f / query.get_vmq_storage().get_dense_matrix()(0, 0);
    ...
    if (limit.m_friction_is_enabled) {
      ...
      if (d_alpha > HK_PI) {
        d_alpha -= HK_PI * 2.0f;
        // don't reset the reference position, and adjust alpha to be in the
        // correct range if it has flipped over
      ...
      hk_real angular_correction_per_psi =
          d_alpha * friction_tau * pi.get_inv_delta_time() -
          friction_damp * query.get_vmq_storage().get_velocities()[0];

      impulse = angular_correction_per_psi * virtual_mass;

      if (hk_Math::fabs(impulse) * pi.get_inv_delta_time() >
          limit.m_joint_friction) {
        hk_real factor = limit.m_joint_friction / hk_Math::fabs(impulse) *
                         pi.get_delta_time();
    ...
    if (limit.m_limit_is_enabled) {  // do the limits
      ...
      hk_real angular_vel =
          query.get_vmq_storage().get_velocities()[0] +
          impulse * query.get_vmq_storage().get_dense_matrix()(0, 0);

      hk_real next_alpha = alpha + angular_vel * pi.get_delta_time();
      if (next_alpha > limit.m_limit_max) {
        hk_real delta = (next_alpha - limit.m_limit_max) * limit.m_limit_tau *
                        tau_factor * pi.get_inv_delta_time();
        impulse -= virtual_mass * delta;
      } else if (next_alpha < limit.m_limit_min) {
        hk_real delta = (next_alpha - limit.m_limit_min) * limit.m_limit_tau *
                        tau_factor * pi.get_inv_delta_time();
        impulse -= virtual_mass * delta;
      }
    }
    if (!hk_Math::almost_zero(impulse)) {
      hk_real impulses[1];
      impulses[0] = impulse;
  ...
  static inline void init_angular_limit(
      const hk_Vector3 &cos_axis_Ref_ws,  // the axis of Ref object which is
                                          // used to sinus measure the angle
      const hk_Vector3 &sin_axis_Ref_ws,  // the axis of Ref object which is
                                          // used to cosin measure the angle
      hk_Constraint_Limit &limit, const hk_Vector3 &axis_Att_ws) {
  ...
  static inline void init_angular_limit(hk_Constraint_Limit &limit,
                                        hk_real alpha) {
    limit.m_ref_position = alpha;
  }

  static inline void do_linear_limit(
      hk_PSI_Info &pi, hk_Rigid_Body *b0, hk_Rigid_Body *b1,
      const hk_Vector3 &pos_ws,
      const hk_Vector3 &axis_ws,   // the axis the objects are rotating around
      const hk_Vector3 &delta_ws,  // the axis of Att object which is used to
                                   // measure the angle (should be equal to the
                                   // sin_axis to get zero angles
      hk_Constraint_Limit &limit, hk_real tau_factor, hk_real damp_factor) {
    ...
    if (limit.m_limit_is_enabled) {  // do the limits
      ...
      hk_real vel = query.get_vmq_storage().get_velocities()[0] +
                    impulse * query.get_vmq_storage().get_dense_matrix()(0, 0);

      hk_real next_pos = alpha - vel * pi.get_delta_time();
      if (next_pos > limit.m_limit_max) {
        hk_real delta = (next_pos - limit.m_limit_max) * tau_factor *
                        limit.m_limit_tau * pi.get_inv_delta_time();
        impulse += virtual_mass * delta;
      } else if (next_pos < limit.m_limit_min) {
        hk_real delta = (next_pos - limit.m_limit_min) * tau_factor *
                        limit.m_limit_tau * pi.get_inv_delta_time();
        impulse += virtual_mass * delta;
      }
    }
  ...
  static inline void init_linear_limit(const hk_Vector3 &axis_ws,
  ...
  static inline void do_angular_plane_limit(
      // a fake angular constraint based on a linear direction clipping against
      // a plane
      hk_PSI_Info &pi, hk_Rigid_Body *b0, hk_Rigid_Body *b1,
      const hk_Vector3 &axis_ws,  // the axis the objects are rotating around
      const hk_Vector3 &plane_normal_ws,  // the plane we are clipping
      hk_real alpha,  // the current distance (e.g. initialized with
                      // axis_ws.dot( plane_normal_ws )
      hk_Constraint_Limit &limit, hk_real tau_factor, hk_real damp_factor) {
    hk_VM_Query_Builder<hk_VMQ_Storage<1>> query;
    query.begin(1);
    {
      hk_Vector3 cross;
      cross.set_cross(axis_ws, plane_normal_ws);
