...
hk_Fixed_Constraint::hk_Fixed_Constraint(hk_Environment* env,
...
hk_Fixed_Constraint::hk_Fixed_Constraint(
    hk_Local_Constraint_System* constraint_system, const hk_Fixed_BP* bp,
    hk_Rigid_Body* a, hk_Rigid_Body* b)
    : hk_Constraint(constraint_system, a, b, HK_PRIORITY_LOCAL_CONSTRAINT,
                    HK_NEXT_MULTIPLE_OF(16, sizeof(hk_Fixed_Work))) {
  init_fixed_constraint(bp);
}
...
// setup and step
int hk_Fixed_Constraint::setup_and_step_constraint(hk_PSI_Info& pi, void* mem,
                                                   hk_real tau_factor,
...

  {
    qe.begin_entries(6);
    {
      ...
      qe.add_angular(4, HK_BODY_A, b0, joint_axes_ws.get_column(1),
                     1.0f);  //-V112
      qe.add_angular(4, HK_BODY_B, b1, joint_axes_ws.get_column(1),
                     -1.0f);  //-V112
  ...
  hk_Fixed_Dense_Vector<6> delta;
  {
    ...
    work.m_correction(3) = pi.get_inv_delta_time() *
                           t1.get_column(1).dot(joint_axes_ws.get_column(2));
    work.m_correction(4) =
        pi.get_inv_delta_time() *
        -t1.get_column(0).dot(joint_axes_ws.get_column(2));  //-V112
    work.m_correction(5) = pi.get_inv_delta_time() *
                           t1.get_column(0).dot(joint_axes_ws.get_column(1));

    delta.set_mul_add_mul(m_tau * tau_factor, work.m_correction,
                          -1.0f * m_strength * strength_factor, vmq_velocity);
  }

  hk_Fixed_Dense_Matrix<6>& mass_matrix =
      qe.get_vmq_storage().get_fixed_dense_matrix();
...
// step
void hk_Fixed_Constraint::step_constraint(hk_PSI_Info& pi, void* mem,
                                          hk_real tau_factor,
                                          hk_real strength_factor) {
  hk_Rigid_Body* b0 = get_rigid_body(0);
  hk_Rigid_Body* b1 = get_rigid_body(1);
...
void hk_Fixed_Constraint::apply_effector_PSI(hk_PSI_Info& pi,
                                             hk_Array<hk_Entity*>*) {
  hk_Fixed_Work work_mem;
  hk_Fixed_Constraint::setup_and_step_constraint(pi, (void*)&work_mem, 1.0f,
                                                 1.0f);
}
