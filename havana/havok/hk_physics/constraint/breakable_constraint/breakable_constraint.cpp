...
hk_Breakable_Constraint::hk_Breakable_Constraint(
    hk_Environment* env, const hk_Breakable_Constraint_BP* bp)
    : hk_Constraint(env, bp->m_real_constraint->get_rigid_body(0),
...
hk_Breakable_Constraint::hk_Breakable_Constraint(
    ...
    : hk_Constraint(constraint_system, bp->m_real_constraint->get_rigid_body(0),
                    bp->m_real_constraint->get_rigid_body(1),
                    HK_PRIORITY_LOCAL_CONSTRAINT,
...
void hk_Breakable_Constraint::init_constraint(const void* vbp) {
  const hk_Breakable_Constraint_BP* bp =
      static_cast<const hk_Breakable_Constraint_BP*>(vbp);
  init_breakable_constraint(bp);
}

void hk_Breakable_Constraint::init_breakable_constraint(
    const hk_Breakable_Constraint_BP* bp) {
...
void hk_Breakable_Constraint::write_to_blueprint(
...
struct BreakableConstraintHelper {
  ...
  bool breaks(hk_real linear_limit, hk_real angular_limit) {
    ...
    if (m_core->get_mass() * linear.length_squared() <
...
void hk_Breakable_Constraint::FireEventIfBroken() {
  if (m_is_broken && get_constraint_system()->is_active()) {
    IVP_Environment* env = reinterpret_cast<IVP_Environment*>(
        get_constraint_system()->get_environment());
...
int hk_Breakable_Constraint::setup_and_step_constraint(hk_PSI_Info& pi,
                                                       void* mem,
                                                       hk_real tau_factor,
                                                       hk_real damp_factor) {
  /*
   * TODO(crack); this is highly outdated and needs to handle linear and angular
  ...
  m_real_constraint->setup_and_step_constraint(pi, mem, tau_factor,
                                               damp_factor);

  m_is_broken = ss0.breaks(m_linear_strength, m_angular_strength) ||
                ss1.breaks(m_linear_strength, m_angular_strength);
...
void hk_Breakable_Constraint::step_constraint(hk_PSI_Info& pi, void* mem,
                                              hk_real tau_factor,
                                              hk_real damp_factor) {
  /*
   * TODO(crack); this is highly outdated and needs to handle linear and angular
...
void hk_Breakable_Constraint::apply_effector_PSI(hk_PSI_Info&,
                                                 hk_Array<hk_Entity*>*) {
