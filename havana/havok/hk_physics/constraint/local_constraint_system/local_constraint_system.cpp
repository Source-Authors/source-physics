...
hk_Local_Constraint_System::hk_Local_Constraint_System(
    hk_Environment* env, hk_Local_Constraint_System_BP* bp)
    : hk_Link_EF(env) {
  m_environment = env;
  m_errorCount = 0;
  m_client_data = NULL;
...
hk_Local_Constraint_System::~hk_Local_Constraint_System() {
  for (hk_Array<hk_Constraint*>::iterator i = m_constraints.start();
       m_constraints.is_valid(i); i = m_constraints.next(i)) {
    hk_Constraint* constraint = m_constraints.get_element(i);
    constraint->constraint_system_deleted_event(this);
  }

  if (m_is_active) {
    m_environment->get_controller_manager()->remove_controller_from_environment(
        this, IVP_TRUE);  // silently
  }
}

void hk_Local_Constraint_System::get_constraints_in_system(
    hk_Array<hk_Constraint*>& constraints_out) {
  for (hk_Array<hk_Constraint*>::iterator i = m_constraints.start();
       m_constraints.is_valid(i); i = m_constraints.next(i)) {
...
//@@CB
void hk_Local_Constraint_System::core_is_going_to_be_deleted_event(
    IVP_Core* my_core) {
  hk_Rigid_Body* rigid_body;

  if (m_bodies.length()) {
    for (hk_Array<hk_Rigid_Body*>::iterator i = m_bodies.start();
         m_bodies.is_valid(i); i = m_bodies.next(i)) {
...
void hk_Local_Constraint_System::constraint_deletion_event(
...
void hk_Local_Constraint_System::recalc_storage_size() {
  ...
  for (hk_Array<hk_Constraint*>::iterator i = m_constraints.start();
       m_constraints.is_valid(i); i = m_constraints.next(i)) {
    m_size_of_all_vmq_storages +=
        m_constraints.get_element(i)->get_vmq_storage_size();
  }
}

void hk_Local_Constraint_System::add_constraint(hk_Constraint* constraint,
                                                int storage_size) {
...
void hk_Local_Constraint_System::activate() {
  if (!m_is_active && m_bodies.length()) {
    m_environment->get_controller_manager()->announce_controller_to_environment(
        this);
    m_is_active = true;
  }
}
...
void hk_Local_Constraint_System::write_to_blueprint(
    hk_Local_Constraint_System_BP* bpOut) {
  bpOut->m_damp = 1.0f;
  bpOut->m_tau = 1.0f;
...
void hk_Local_Constraint_System::clear_error() { m_errorCount = 0; }

void hk_Local_Constraint_System::report_square_error(float errSq) {
  if (errSq > (m_errorTolerance * m_errorTolerance)) m_errorThisTick = true;
}

void hk_Local_Constraint_System::solve_penetration(IVP_Real_Object* pivp0,
                                                   IVP_Real_Object* pivp1) {
  if (m_penetrationCount >= 4) return;
  ...
  if (m_penetrationPairs[m_penetrationCount].obj0 >= 0 &&
      m_penetrationPairs[m_penetrationCount].obj1 >= 0)
    m_penetrationCount++;
}

void hk_Local_Constraint_System::get_effected_entities(
    hk_Array<hk_Entity*>& ent_out) {
...
static IVP_DOUBLE GetMoveableMass(const IVP_Core* pCore) {
  if (pCore->physical_unmoveable || pCore->pinned) return pCore->get_mass();

  return 0.0;
}

void hk_Local_Constraint_System::apply_effector_PSI(hk_PSI_Info& pi,
                                                    hk_Array<hk_Entity*>*) {
  ...
  // first do the setup
  {
    ...
    for (int i = 0; i < m_constraints.length(); i++) {
      ...
      [[maybe_unused]] hk_Rigid_Body* b0 =
          m_constraints.element_at(i)->get_rigid_body(0);
      [[maybe_unused]] hk_Rigid_Body* b1 =
          m_constraints.element_at(i)->get_rigid_body(1);

      int b_size = m_constraints.element_at(i)->setup_and_step_constraint(
          pi, p_buffer, 1.0f, 1.0f);
      p_buffer += b_size;
    }
  }
  ...
  for (int i = 0; i < m_n_iterations; i++) {
    ...
    for (int x = 0; x < 2 && hk_Math::almost_zero(taus[x]); x++) {
      for (int ii = m_constraints.length() - 1; ii >= 0; ii--) {
        m_constraints.element_at(ii)->step_constraint(pi, vmq_buffers[ii],
                                                      taus[x], damps[x]);
      }
      for (int j = 0; j < m_constraints.length(); j++) {
        m_constraints.element_at(j)->step_constraint(pi, vmq_buffers[j],
                                                     taus[x], damps[x]);
      }
    }
  }
  ...
  if (m_penetrationCount) {
    ...
    for (int i = 0; i < m_penetrationCount; i++) {
      ...
      if (deltaLength <= 0.01) {
        // if the objects are exactly on top of each other just push down X the
        // axis
      ...
      if (IVP_MTIS_SIMULATED(core1->movement_state) && !core1->pinned) {
        obj1->async_push_object_ws(&m_world_f_core1->vv,
                                   &p0);  // PiMoN: was m_world_f_core0, but
                                          // this is a bug in Valve's code
  ...
  if (m_errorCount <= m_minErrorTicks) m_errorCount++;
}

hk_real hk_Local_Constraint_System::get_epsilon() { return 0.2f; }
