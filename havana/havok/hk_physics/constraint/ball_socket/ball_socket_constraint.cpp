...
hk_Ball_Socket_Constraint::hk_Ball_Socket_Constraint(
    hk_Environment *env, const hk_Ball_Socket_BP *bp, hk_Rigid_Body *a,
    hk_Rigid_Body *b)
...
hk_Ball_Socket_Constraint::hk_Ball_Socket_Constraint(
    hk_Local_Constraint_System *constraint_system, const hk_Ball_Socket_BP *bp,
    hk_Rigid_Body *a, hk_Rigid_Body *b)
    : hk_Constraint(constraint_system, a, b, HK_PRIORITY_LOCAL_CONSTRAINT,
                    HK_NEXT_MULTIPLE_OF(16, sizeof(hk_Ball_Socket_Work))) {
  init_ball_socket_constraint(bp);
}
...
void hk_Ball_Socket_Constraint::init_ball_socket_constraint(
...
int hk_Ball_Socket_Constraint::setup_and_step_constraint(hk_PSI_Info &pi,
                                                         void *mem,
                                                         hk_real tau_factor,
                                                         hk_real damp_factor) {
  ...
  delta_dist_3.add_mul(-1.0f * m_strength * damp_factor,
...
void hk_Ball_Socket_Constraint::step_constraint(hk_PSI_Info &pi, void *mem,
                                                hk_real tau_factor,
                                                hk_real damp_factor) {
  ...
  delta_dist_3.set_mul(tau_factor * m_tau * pi.get_inv_delta_time(), dir);
  delta_dist_3.add_mul(-1.0f * damp_factor * m_strength,
...
void hk_Ball_Socket_Constraint::apply_effector_PSI(hk_PSI_Info &pi,
                                                   hk_Array<hk_Entity *> *) {
  hk_Ball_Socket_Work work_mem;
  hk_Ball_Socket_Constraint::setup_and_step_constraint(pi, (void *)&work_mem,
                                                       1.0f, 1.0f);
}
