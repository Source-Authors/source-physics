...
// [ 0 M ]
// [ I 0 ]
void hk_Spatial_Matrix::set_spatial_inertia_tensor(
    const hk_Matrix3& inertia_tensor, const hk_real mass) {
...
// [ -r~R R ]   and r is vector from origin of F to origin of G in G's frame of
// reference
//
void hk_Spatial_Matrix::set_spatial_transform(const hk_Matrix3& R,
                                              const hk_Vector3& r) {
  hk_Matrix3 rR;
...
// reference
//
void hk_Spatial_Matrix::set_spatial_transform(
    [[maybe_unused]] const hk_Transform& cartesian_frame) {
  HK_ASSERT(0);
  /*	hk_Matrix3 rR;
          hk_real raw[16];
...
void hk_Spatial_Matrix::set_mul(const hk_Spatial_Matrix& a,
                                const hk_Spatial_Matrix& b) {
  hk_Matrix3 work;
...
void hk_Spatial_Matrix::set_add(const hk_Spatial_Matrix& a,
                                const hk_Spatial_Matrix& b) {
...
void hk_Spatial_Matrix::set_sub(const hk_Spatial_Matrix& a,
                                const hk_Spatial_Matrix& b) {
  m_Block[0][0] = a.m_Block[0][0];
  m_Block[0][0] -= b.m_Block[0][0];
...
void hk_Spatial_Matrix::set_vector_mul_vector(const hk_Spatial_Vector& a,
                                              const hk_Spatial_Vector& b) {
  int idr, idc;
...
void hk_Spatial_Matrix::gausse_siedel_solve(hk_real A[][6], int dim, hk_real* x,
                                            hk_real* b) {
  static int* order;
  ...
  for (elimcol = 0; elimcol < dim; elimcol++) {
    ...
    // get row with largest value at pivot from set of rows not eliminated
    // already
    for (max_pivot = 0, check_pivot = elimcol; check_pivot < dim;
         check_pivot++) {
    ...
    for (target_row = elimcol + 1; target_row < dim; target_row++) {
      ...
      // do each column for this elimanation step that hasn't been eliminated
      // already
...
void hk_Spatial_Matrix::linear_solve(hk_Spatial_Vector& sx,
                                     const hk_Spatial_Vector& sb) {
  hk_real x[6];
  hk_real b[6];
  hk_real A[6][6];
