...
void hk_Gauss_Elm_Solver::add_multiple_line(int first, int second,
                                            hk_gauss_real factor) {
  ...
  hk_VecFPU::fpu_add_multiple_row(second_line, first_line, factor,
                                  m_n_columns - first, HK_FALSE);
  m_b[second] += m_b[first] * factor;
}

hk_result hk_Gauss_Elm_Solver::solve_gauss_elemination(
    hk_Gauss_Elm_Input &inp, hk_Gauss_Elm_Output &outp) {
  m_A = inp.m_A;
  m_b = inp.m_b;
...
#endif
  hk_VecFPU::fpu_exchange_rows(&m_A[b * m_aligned_row_len],
                               &m_A[a * m_aligned_row_len], m_n_columns,
                               HK_TRUE);
...
hk_result hk_Gauss_Elm_Solver::solve_lower_null_matrix() {
  ...
  for (i = m_n_columns - 1; i >= 0; i--) {
    hk_gauss_real *pointer_in_line =
        &m_A[i * m_aligned_row_len + m_n_columns - 1];
    ...
    if (hk_Math::fabsd(left_side) < m_gauss_eps) {
      ...
      m_b[i] = right_side /
               left_side;  // division left_side is not needed, it was done
                           // already above (make a vector of inverses)
    }
  }
