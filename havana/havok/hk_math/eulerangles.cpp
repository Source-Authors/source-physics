...
//:
void hk_Euler_Angles::set_euler_angles(const hk_Quaternion &q,
                                       hk_EULER_ANGLES_ORDER order) {
 ...
 *		input i1:			the middle axis to rotage
 *(pitch) input i2:			the last axis to rotate (roll)
 *
 *		output taxis_ws_os_out: the constraint axis to be used by the
 *constraint solver output angles_out:		the eulerangle to be used by the
 *constraint solver
 *
 *		Note:	expects the transforms to be in column major order
 *		Note:	whole function can be optimized using assembler in PS2
 *****************************************************************/
void hk_Euler_Angles::create_rot_axis_and_angles(const hk_Rotation &tref_ws_os,
                                                 const hk_Rotation &tatt_ws_os,
                                                 int i0, int i1, int i2,
                                                 // in
                                                 hk_Matrix3 &taxis_ws_os_out,
                                                 hk_Vector3 &angles_out)
// out
{
