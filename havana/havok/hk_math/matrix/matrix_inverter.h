...
class hk_Matrix_Inverter {
 public:
  static inline void invert_no_pivot_search(hk_real *in_matrix, int lda,
                                            int num_rows, hk_real *out_matrix) {
...

    {
      for (int i = num_rows - 1; i >= 0; i--) {
        ...
        for (int row = i - 1; row >= 0; row--) {
          ...
          //	hk_VecFPU::fpu_add_multiple_row( inverted_row, pr_out, -factor,
          // lda, HK_TRUE);
