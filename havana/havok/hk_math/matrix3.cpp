...
void hk_Matrix3::set_rows(const hk_Vector3& r0, const hk_Vector3& r1,
                          const hk_Vector3& r2) {
  _set_rows(r0, r1, r2);
}

void hk_Matrix3::set_cols(const hk_Vector3& c0, const hk_Vector3& c1,
                          const hk_Vector3& c2) {
  get_column(0) = c0;
  get_column(1) = c1;
  get_column(2) = c2;
...
bool hk_Matrix3::is_identity_rotation() const {
  ...
  return hk_Math::almost_equal(m(0, 0), 1) && hk_Math::almost_zero(m(0, 1)) &&
         hk_Math::almost_zero(m(0, 2)) && hk_Math::almost_zero(m(1, 0)) &&
         hk_Math::almost_equal(m(1, 1), 1) && hk_Math::almost_zero(m(1, 2)) &&
         hk_Math::almost_zero(m(2, 0)) && hk_Math::almost_zero(m(2, 1)) &&
         hk_Math::almost_equal(m(2, 2), 1);
}
...
hk_result hk_Matrix3::invert(hk_real epsilon) {
  hk_Vector3 r0;
  r0.set_cross(get_column(1), get_column(2));
  hk_Vector3 r1;
  r1.set_cross(get_column(2), get_column(0));
  hk_Vector3 r2;
  r2.set_cross(get_column(0), get_column(1));
...
void hk_Matrix3::transpose() {
  hk_real swap;
  swap = get_column(0)(1);
  get_column(0)(1) = get_column(1)(0);
  get_column(1)(0) = swap;
  swap = get_column(0)(2);
  get_column(0)(2) = get_column(2)(0);
  get_column(2)(0) = swap;
  swap = get_column(1)(2);
  get_column(1)(2) = get_column(2)(1);
  get_column(2)(1) = swap;
}
...
void hk_Matrix3::set_rotated_diagonal_matrix(
    const hk_Rotation& r, const hk_Vector3& diagonal_matrix3) {
  hk_Matrix3 t;
