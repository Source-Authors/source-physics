...
Bodies", SIGGRAPH '94 The difference is we use here a incremental LU solution
for the active variables In case the LU is invalid - e.g. numerical problems,
Sub-Matrix is singular, ... - we use a gaussian elemination to get delta_x
values of active variables per step (very costly :( )

   Optimization used here is to say what variables should be active at start. We
than try to setup the LU solver and start with an set of actives right from the
start.

   The algorithm tries to solve full_A*full_x >= full_b, where full_x is the
solution constraints: full_x >= 0.0 and if full_x[i]>0 than equation i has to
hold Modification: with n_equations you can specify that the first part of A and
x are treated as a system of equations this means full_A*full_x = full_b for i<=
n_equations. full_x is then allowed to be negative

   During all computations the most important variables are:
     vector int:  active_inactive_ignored (has information about what variables
are active,inactive or not processed yet) integer      r_actives (number of
active variables) integer      ignored_pos (number of next unknown to process)
   Given that information we can calculate all other information during a setup
in case numerical errors accumulate. The most valuable thing to lose in that
setup is the LU information. We can recompute it with
   lu_sub_solver::l_u_decomposition_with_pivoting() but that costs 3X a gaussian
elemination

   lu_sub_solver: here is stored the LU decompostion of the sub Matrix A of
active variables in case an active variable is changed to inactive we call
lu_sub_solver::decrement_l_u in case an inactive/ignored variable becomes active
we call lu_sub_solver::increment_l_u

   accel: stores vector values full_A*full_x - full_b
          has to be zero for the active variables and >zero for inactives and
some known value for ignored variables

   delta_x: during a step holds the changes of x when moving 1.0 with the latest
ignored variable has to be zero for inactive variables (their x values are and
shall stay zero)

   delta_accel: during a step holds the changes of accel when moving 1.0 with
the latest ignored variable has to be zero for active variables (their accels
are and shall stay zero)


   if accel > 0.0 for next ignored variable, we can move that variable to
inactives. Else we have to do a standard step

   standard step:
     step begins with call to 'get_xdirection'. There the delta_x values are
calculated. Delta_x for new ignored variable is 1.0 for the rest of the actives
delta_x is unknown. So A*delta_x = delta_accel A* (? ? .. ? 1.0) = ( 0 0 0 .. 0
alpha )  sub matrix A is (r_actives+1)*(r_actives+1) A* delta_x + ai = ( 0 0 0 0
0 0 0)  sub matrix A is (r_actives*r_actives) and ai is ith column of A we have
to solve: A * delta_x = -ai we solve this with our LU solver or with
gauss-eleminiation

     After we have delta_x we calculate delta_accel: delta_accel = A * delta_x
(and get value alpha)
     ...
     Three possibilites: ignored variable becomes active, one active becomes
inactive, one inactive becomes active Then we have to update accel and full_x
and do next step
    ...
    if (val > HK_LCP_TEST_EPS) {
      HK_LCP_IF(debug_lcs) {
        hkprintf("numerical_unstable %d %.10f should be %.10f\n", ignored_pos,
                 temp[actives_inactives_ignored[k]],
                 accel[actives_inactives_ignored[k]]);
      }
      return HK_FALSE;
    }
...
hk_result hk_LCP_Solver::solve_lcp(hk_LCP_Input &in, hk_LCP_Temp &tmp,
                                   hk_LCP_Output &out) {
  input_struct = &in;
  ...
  // Gauss solver shares all its memory with LU_Solver. Gauss is only needed
  // when LU is invalid
  ...
  m_gauss_inp.m_gauss_eps =
      HK_LCP_SOLVER_EPS;  // for gauss a more liberal EPS is enough (otherwise
                          // it may happen the unilateral solver to fail
                          // completely) sorry, but for xdirection its not
                          // acceptable to have very large values
    ...
    for (int j = 0; j < r_actives; j++) {
      ...
      if (diff > eps) {
        hkprintf("linear_constraint_failed #%d %f == %f (diff %f > %.4f)\n", j,
                 temp[index_full], full_b[index_full], diff, eps);
      }
    }
  }
...
void hk_LCP_Solver::increment_sub_solver() {
  ...
  if (sub_solver_status == 0) {
    ...
    for (k = 0; k < r_actives; k++) {
      lu_sub_solver
          .m_U_matrix[lu_sub_solver.m_n_sub * lu_sub_solver.m_aligned_row_len +
                      k] =
          full_A[new_var_orig * aligned_size + actives_inactives_ignored[k]];
    }
    for (k = 0; k < r_actives - 1; k++) {
      lu_sub_solver.m_inout_vec[k] =
...
void hk_LCP_Solver::do_a_little_random_permutation() {
  ...
  // zero, this "wrong" values are distributed alternatively )
  ...
  exchange_lcs_variables(first_permute_index + n_equations,
                         second_permute_index + n_equations);
...
void hk_LCP_Solver::move_not_necessary_actives_to_inactives() {
  ...
  for (i = n_equations; i < r_actives; i++) {
    if (full_x[actives_inactives_ignored[i]] < HK_LCP_SOLVER_EPS) {
      // hkprintf("juhu_removed_not_necessary_active %d full
...
hk_result hk_LCP_Solver::solve_lc() {
  ...
  while (1) {
    ...
    HK_LCP_IF(debug_lcs) { debug_out_lcs(); }
    ...
    if ((sub_solver_status > 0)) {
      if ((sub_solver_status == 1) && (did_real_step)) {
        ...
        if (setup_l_u_solver() == HK_OK) {
          ...
          HK_LCP_IF(debug_lcs) { hkprintf("setup_lu_notok\n"); }
        }
      } else {
    ...
    if (accel[ignored_full_index] < 0.0f) {
      ...
      if (get_xdirection() == HK_FAULT) {
        HK_LCP_IF(debug_lcs) { hkprintf("lcs_fdirection_total_fail\n"); }
      ...
      if (-accel[ignored_full_index] <
          delta_accel[ignored_full_index] * HK_LCP_MAX_STEP_LEN) {
        smallest_step =
      ...
      // if we increase value of ignored variable, equations will allways hold
      // true whatever step size we choose however the x-value of active
      // variables are not allowed to pass through zero
      for (i = n_equations; i < r_actives; i++) {
        index = actives_inactives_ignored[i];
        hk_real del_f = delta_x[index];
        if (del_f < -solver_eps) {
          hk_real local_step = -full_x[index] / del_f;
          if ((hk_Math::fabs(local_step) < solver_eps) &&
      ...
      HK_LCP_IF(debug_lcs) { hkprintf("smallest_step_is %f\n", smallest_step); }
    ...
    limiting_var_found:
      ...
      if (limiting_var < r_actives) {
        ...
        HK_ASSERT((hk_Math::fabs(full_x[indx]) < 0.001f) ||
                  (hk_Math::fabs(smallest_step) < solver_eps));
        full_x[indx] = 0.0f;
        r_actives--;
        ...
        move_ignored_to_active:
          ...
          HK_LCP_IF(0) { debug_test_all_values(); }
        }
      }
    } else {
...
void hk_LCP_Solver::update_step_vars(hk_real step_size) {
  int i;
  HK_LCP_IF(debug_lcs) { hkprintf("deltaa  "); }
  for (i = 0; i < n_variables; i++) {
    HK_LCP_IF(debug_lcs) { hkprintf("%f ", delta_accel[i]); }
    accel[i] =
        accel[i] + step_size * delta_accel[i];  // Vector Optimization possible
    full_x[i] = full_x[i] + step_size * delta_x[i];  // dito
  }
  HK_LCP_IF(debug_lcs) { hkprintf("\n"); }
...
hk_result hk_LCP_Solver::get_xdirection() {
  ...
  if (sub_solver_status == 0) {
    ...
    HK_LCP_IF((debug_lcs)) { hkprintf("sub_lu_is_off %d\n", r_actives); }
    debug_no_lu_count++;

    m_gauss_inp.m_n_columns = r_actives;
    m_gauss_inp.m_aligned_row_len =
        hk_VecFPU::calc_aligned_row_len(r_actives, m_gauss_inp.m_A);
    ...
    for (i = 0; i < r_actives; i++) {
      ...
      for (j = 0; j < r_actives; j++) {
        int index2 = actives_inactives_ignored[j];
        m_gauss_inp.m_A[i * m_gauss_inp.m_aligned_row_len + j] =
            this->full_A[index * aligned_size + index2];
      }
    }
    ret_val =
        m_gauss_sub_solver.solve_gauss_elemination(m_gauss_inp, m_gauss_out);
    for (i = 0; i < r_actives; i++) {
...
hk_result hk_LCP_Solver::setup_l_u_solver() {
  int i;
  HK_LCP_IF(0) { hkprintf("setting_up_lu\n"); }
  ...
  for (i = 0; i < r_actives; i++) {
    ...
    hk_incrlu_real *dest =
        &lu_sub_solver.m_U_matrix[i * lu_sub_solver.m_aligned_row_len];
    for (int j = 0; j < r_actives; j++) {
      int index2 = actives_inactives_ignored[j];
...
void hk_LCP_Solver::lcs_bubble_sort_x_vals() {
  ...
  while (1) {
    ...
    if (full_x[actives_inactives_ignored[r1]] <
      ...
      int r_t = r1;
      while ((r_t > n_equations) &&
             (full_x[actives_inactives_ignored[r_t - 1]] <
...
void hk_LCP_Solver::get_values_when_setup() {
  ...
  for (i = 0; i < r_actives; i++) {
    accel[actives_inactives_ignored[i]] =
...
void hk_LCP_Solver::startup_setup(int try_actives) {
  ...
  aligned_sub_size =
      hk_VecFPU::calc_aligned_row_len(r_actives, lu_sub_solver.m_L_matrix);
  ...
  if (setup_l_u_solver() == HK_OK) {
    ...
    HK_LCP_IF(debug_lcs) {
      ...
      for (j = 0; j < r_actives; j++) {
        hkprintf(
            "%.3f ",
            lu_sub_solver.m_L_matrix[lu_sub_solver.m_aligned_row_len * 48 + j]);
      }
      hkprintf("\n\n");
    }
...
hk_result hk_LCP_Solver::full_setup() {
  HK_LCP_IF(0) { hkprintf("doing_fulll_setup\n"); }
  ...
  if (setup_l_u_solver() == HK_OK) {
    ...
    sub_solver_status = 2;
    HK_LCP_IF(debug_lcs) { hkprintf("setup_lu_failed\n"); }
    ...
    if (ret_val != HK_OK) {
      HK_LCP_IF(1) { hkprintf("error_setup_gauss_failed_too\n"); }
      return ret_val;
    }
