...
class hk_LCP_Input {
 public:
  ...
  // Warning: at the moment m_A is interpreted as row-wise organized and simply
  // pointer to memory is taken
  //          but actually m_A is column-wise organized. Doesn't matter because
  //          matrix is symetric, but ...
  hk_real *m_b;  // aligned Vector
  // m_A and m_b are sorted in a way first the m_n_equations occur and then the
  // m_n_actives_at_start
...
class hk_LCP_Temp {
 public:
  // warning: hk_incrlu_real type has to be greater or equal hk_real and
  // hk_gauss_real, because memory is shared
...
//  m_result has >= 0.0 and if m_result[i] > 0.0 than ith equation is '==' and
//  not '>' vector m_active gives information about what components in m_result
//  are > 0.0

// Warning: solver uses three different types of floating points (hk_real,
// hk_incrlu_real, hk_gauss_real)
//          if alignment (right now 16Bytes for all three) differs, algorithm
//          will crash !!!
class hk_LCP_Solver {
  int m_max_steps_lcp;
  ...
  hk_Incr_LU_Matrix
      lu_sub_solver;  // two aligned NxN matrices and 4 aligned vectors
