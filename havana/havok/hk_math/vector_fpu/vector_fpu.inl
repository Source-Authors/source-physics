...
inline void hk_VecFPU::fpu_add_multiple_row(hk_real *target_adress,
                                            hk_real *source_adress,
                                            hk_real factor, int size,
                                            hk_bool adress_aligned) {
  if (adress_aligned == HK_FALSE) {
    // we have to calculate the block size and shift adresses to lower aligned
    // adresses
    hk_intp result_adress =
        reinterpret_cast<hk_intp>(source_adress) & hk_VecFPU_MEM_MASK_FLOAT;

    target_adress = reinterpret_cast<hk_real *>(
        reinterpret_cast<hk_intp>(target_adress) & hk_VecFPU_MEM_MASK_FLOAT);
    size += (reinterpret_cast<hk_intp>(source_adress) - result_adress) >>
            hk_VecFPU_MEMSHIFT_FLOAT;
...
inline hk_real hk_VecFPU::fpu_large_dot_product(hk_real *base_a,
                                                hk_real *base_b, int size,
                                                hk_bool adress_aligned) {
  if (adress_aligned == HK_FALSE) {
    // we have to calculate the block size and shift adresses to lower aligned
    // adresses
    hk_intp result_adress =
        reinterpret_cast<hk_intp>(base_a) & hk_VecFPU_MEM_MASK_FLOAT;
    base_b = reinterpret_cast<hk_real *>(reinterpret_cast<hk_intp>(base_b) &
                                         hk_VecFPU_MEM_MASK_FLOAT);
    size += (reinterpret_cast<hk_intp>(base_a) - result_adress) >>
            hk_VecFPU_MEMSHIFT_FLOAT;  // because start changed
...
inline void hk_VecFPU::fpu_multiply_row(hk_real *target_adress, hk_real factor,
                                        int size, hk_bool adress_aligned) {
  if (adress_aligned == HK_FALSE) {
    // we have to calculate the block size and shift adresses to lower aligned
    // adresses
...
// )
inline void hk_VecFPU::fpu_exchange_rows(hk_real *target_adress1,
                                         hk_real *target_adress2, int size,
                                         hk_bool adress_aligned) {
  if (adress_aligned == HK_FALSE) {
    // we have to calculate the block size and shift adresses to lower aligned
    // adresses
...
inline void hk_VecFPU::fpu_copy_rows(hk_real *target_adress,
                                     hk_real *source_adress, int size,
                                     hk_bool adress_aligned) {
  if (adress_aligned == HK_FALSE) {
    // we have to calculate the block size and shift adresses to lower aligned
    // adresses
...
inline void hk_VecFPU::fpu_set_row_to_zero(hk_real *target_adress, int size,
                                           hk_bool adress_aligned) {
  if (adress_aligned == HK_FALSE) {
...
inline int hk_VecFPU::calc_aligned_row_len(
    int unaligned_len, [[maybe_unused]] hk_real
                           *dummy_type) {  // dummy type is used for overloading
  return (unaligned_len + hk_VecFPU_SIZE_FLOAT - 1) & hk_VecFPU_MASK_FLOAT;
}

//----------------------------------------------------------------------------------------------------------------------

inline void hk_VecFPU::fpu_add_multiple_row(hk_double *target_adress,
                                            hk_double *source_adress,
                                            hk_double factor, int size,
                                            hk_bool adress_aligned) {
  if (adress_aligned == HK_FALSE) {
    // we have to calculate the block size and shift adresses to lower aligned
    // adresses
    hk_intp result_adress =
        reinterpret_cast<hk_intp>(source_adress) & hk_VecFPU_MEM_MASK_DOUBLE;
    target_adress = (hk_double *)(reinterpret_cast<hk_intp>(target_adress) &
                                  hk_VecFPU_MEM_MASK_DOUBLE);
    size += (reinterpret_cast<hk_intp>(source_adress) - result_adress) >>
            hk_VecFPU_MEMSHIFT_DOUBLE;
...
inline hk_double hk_VecFPU::fpu_large_dot_product(hk_double *base_a,
                                                  hk_double *base_b, int size,
                                                  hk_bool adress_aligned) {
  if (adress_aligned == HK_FALSE) {
    // we have to calculate the block size and shift adresses to lower aligned
    // adresses
    hk_intp result_adress =
        reinterpret_cast<hk_intp>(base_a) & hk_VecFPU_MEM_MASK_DOUBLE;
    base_b = (hk_double *)(reinterpret_cast<hk_intp>(base_b) &
                           hk_VecFPU_MEM_MASK_DOUBLE);
    size += (reinterpret_cast<hk_intp>(base_a) - result_adress) >>
            hk_VecFPU_MEMSHIFT_DOUBLE;  // because start changed
...
inline void hk_VecFPU::fpu_multiply_row(hk_double *target_adress,
                                        hk_double factor, int size,
                                        hk_bool adress_aligned) {
  if (adress_aligned == HK_FALSE) {
    // we have to calculate the block size and shift adresses to lower aligned
...
// )
inline void hk_VecFPU::fpu_exchange_rows(hk_double *target_adress1,
                                         hk_double *target_adress2, int size,
                                         hk_bool adress_aligned) {
  if (adress_aligned == HK_FALSE) {
    // we have to calculate the block size and shift adresses to lower aligned
    // adresses
...
inline void hk_VecFPU::fpu_copy_rows(hk_double *target_adress,
                                     hk_double *source_adress, int size,
                                     hk_bool adress_aligned) {
  if (adress_aligned == HK_FALSE) {
    // we have to calculate the block size and shift adresses to lower aligned
    // adresses
...
inline void hk_VecFPU::fpu_set_row_to_zero(hk_double *target_adress, int size,
                                           hk_bool adress_aligned) {
  if (adress_aligned == HK_FALSE) {
    hk_intp adress = reinterpret_cast<hk_intp>(target_adress);
    hk_intp result_adress = adress & hk_VecFPU_MEM_MASK_DOUBLE;
...
inline int hk_VecFPU::calc_aligned_row_len(
    int unaligned_len, [[maybe_unused]] hk_double
                           *dummy_type) {  // dummy type is used for overloading
