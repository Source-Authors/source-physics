...
// compute a step.  current state in t0, state after time step t0 -> t1 is put
// in y1 fourth order runga-kutta
void OdeRungaKutta4::calc_step(hk_real y0[], hk_real y1[], unsigned int len,
                               hk_real t0, hk_real t1, dydt_function dydt,
                               void *client_data) {
  unsigned int i;
  // reallocate if necessary
  if (len > state_size) ode_realloc(len);
...
OdeEuler::OdeEuler() { OdeSolver::ode_realloc(ODE_INITIAL_STATE_SIZE); }

void OdeEuler::calc_step(hk_real y0[], hk_real y1[], unsigned int len,
                         hk_real t0, hk_real t1, dydt_function dydt,
                         void *client_data) {
  unsigned int i;
  // reallocate if necessary
...
OdeMidPoint::OdeMidPoint() { OdeSolver::ode_realloc(ODE_INITIAL_STATE_SIZE); }
...
void OdeMidPoint::calc_step(hk_real y0[], hk_real y1[], unsigned int len,
                            hk_real t0, hk_real t1, dydt_function dydt,
                            void *client_data) {
  unsigned int i;
  // reallocate if necessary
