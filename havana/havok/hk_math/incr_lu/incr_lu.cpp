...
hk_result hk_Incr_LU_Matrix::normize_row_L(int row_nr) {
  ...
  hk_VecFPU::fpu_multiply_row(&base[0], inv_val, m_n_sub,
                              HK_FALSE);  // should be TRUE
...
hk_result hk_Incr_LU_Matrix::normize_row(int row_nr) {
  ...
  hk_incrlu_real inv_val = 1.0f / val;
  hk_VecFPU::fpu_multiply_row(&m_L_matrix[row_base], inv_val, m_n_sub,
                              HK_FALSE);
  hk_VecFPU::fpu_multiply_row(&m_U_matrix[row_base + row_nr + 1], inv_val,
                              m_n_sub - row_nr - 1, HK_FALSE);
...
#else
  hk_VecFPU::fpu_exchange_rows(&m_U_matrix[base_a + pivot_col],
                               &m_U_matrix[base_b + pivot_col],
                               m_n_sub - pivot_col, HK_FALSE);
  hk_VecFPU::fpu_exchange_rows(&m_L_matrix[base_a], &m_L_matrix[base_b],
                               m_n_sub, HK_FALSE);
#endif
}
...
void hk_Incr_LU_Matrix::pivot_search_l_u(int col_nr) {
  hk_incrlu_real biggest =
      hk_Math::fabs(m_U_matrix[col_nr * m_aligned_row_len + col_nr]);
  int pivot_row = col_nr;

  for (int i = m_n_sub - 1; i > col_nr; i--) {
    hk_incrlu_real new_val =
        hk_Math::fabs(m_U_matrix[i * m_aligned_row_len + col_nr]);
    if (new_val > biggest) {
      pivot_row = i;
      biggest = new_val;
    }
...
void hk_Incr_LU_Matrix::add_neg_row_upwards_l_u(int row_down, int dest_row,
                                                hk_incrlu_real factor) {
...
#else
  hk_VecFPU::fpu_add_multiple_row(&base_b[row_down + 1], &base_a[row_down + 1],
                                  -factor, m_n_sub - row_down - 1, HK_FALSE);
#endif
...
#else
  hk_VecFPU::fpu_add_multiple_row(&base_b[0], &base_a[0], -factor, m_n_sub,
                                  HK_TRUE);  // should be TRUE
#endif
  base_b[row_down] = 0.0f;
}

// add row to a row more downwards
void hk_Incr_LU_Matrix::add_neg_row_to_row_l_u(int pivot_row, int dest_row,
                                               hk_incrlu_real factor) {
...
#else
  hk_VecFPU::fpu_add_multiple_row(&m_U_matrix[base_b + pivot_row + 1],
                                  &m_U_matrix[base_a + pivot_row + 1], -factor,
                                  m_n_sub - pivot_row - 1, HK_FALSE);
  hk_VecFPU::fpu_add_multiple_row(&m_L_matrix[base_b], &m_L_matrix[base_a],
                                  -factor, m_n_sub, HK_FALSE);  // set to TRUE
#endif
  m_U_matrix[base_b + pivot_row] = 0.0f;
}

void hk_Incr_LU_Matrix::subtract_row_L(int from, int dest,
                                       hk_incrlu_real factor) {
...
hk_result hk_Incr_LU_Matrix::l_u_decomposition_with_pivoting() {
  ...
  for (i = m_n_sub - 1; i >= 0; i--) {
    ...
    hk_VecFPU::fpu_set_row_to_zero(&base[0], m_n_sub,
                                   HK_FALSE);  // should be TRUE
    base[i] = 1.0f;
  }
  ...
  for (rows_to_do = 1; rows_to_do < m_n_sub; rows_to_do++) {
    ...
    for (int rows_become_zero = m_n_sub - 1; rows_become_zero >= rows_to_do;
         rows_become_zero--) {
      hk_incrlu_real factor =
          m_U_matrix[rows_become_zero * m_aligned_row_len + pivot_elem];
...
// incremental l_u_decomposition: matrix is correctly decomposed to
// m_n_sub*m_n_sub last row of new U was already added, row has length m_n_sub+1
...
hk_result hk_Incr_LU_Matrix::increment_l_u() {
  ...
  hk_VecFPU::fpu_set_row_to_zero(&m_L_matrix[m_n_sub * m_aligned_row_len],
                                 m_n_sub, HK_FALSE);  // should be TRUE
...
void hk_Incr_LU_Matrix::add_neg_col_L(int col0, int col_dest,
                                      hk_incrlu_real factor) {
  // Vector FPU not possible
  int i;
  for (i = m_n_sub - 1; i >= 0; i--) {
    m_L_matrix[i * m_aligned_row_len + col_dest] -=
        factor * m_L_matrix[i * m_aligned_row_len + col0];
  }
}
...
void hk_Incr_LU_Matrix::exchange_columns_L(int col0, int col1) {
  ...
  for (i = 0; i < m_n_sub; i++) {
    ...
    m_L_matrix[i * m_aligned_row_len + col0] =
        m_L_matrix[i * m_aligned_row_len + col1];
...
void hk_Incr_LU_Matrix::exchange_columns_U(int col0, int col1) {
  ...
  for (i = 0; i < m_n_sub; i++) {
    ...
    m_U_matrix[i * m_aligned_row_len + col0] =
        m_U_matrix[i * m_aligned_row_len + col1];
...
void hk_Incr_LU_Matrix::mult_vec_with_L() {
  int i;
  for (i = m_n_sub - 1; i >= 0; i--) {
    hk_incrlu_real sum = hk_VecFPU::fpu_large_dot_product(
        &m_L_matrix[i * m_aligned_row_len], &m_inout_vec[0], m_n_sub, HK_TRUE);
    m_temp_vec[i] = sum;
  }
}
...
void hk_Incr_LU_Matrix::add_neg_row_L(int source_row, int dest_row,
                                      hk_incrlu_real factor) {
