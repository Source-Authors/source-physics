...
class hk_Dense_Vector {
 public:
  HK_DECLARE_NONVIRTUAL_CLASS_ALLOCATOR(HK_MEMORY_CLASS_CONSTRAINT,
                                        hk_Dense_Vector)

  inline hk_real& operator()(int i) {
    HK_ASSERT(i < m_size);
    return m_elt[i];
  }
  inline const hk_real& operator()(int i) const {
    HK_ASSERT(i < m_size);
    return m_elt[i];
  }
...
class hk_Fixed_Dense_Vector : public hk_Dense_Vector {
 public:
  ...
  // SK says XXX fix me
  inline void set_add_mul(const hk_Fixed_Dense_Vector<N>& a, hk_real factor,
                          const hk_real b[]);
  inline void set_mul_add_mul(hk_real factor_a,
                              const hk_Fixed_Dense_Vector<N>& a,
                              hk_real factor_b, const hk_real b[]);
