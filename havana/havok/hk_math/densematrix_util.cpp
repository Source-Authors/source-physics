 ...
 * MULT
 */
void hk_Dense_Matrix_Util::mult(const hk_Dense_Matrix &m,
                                const hk_Dense_Vector &in,
                                hk_Dense_Vector &out) {
 ...
 * MULT
 */
void hk_Dense_Matrix_Util::mult(const hk_Dense_Matrix &m, const hk_real *in,
                                hk_real *out) {
  const int rows = m.get_num_rows();
  const int cols = m.get_num_cols();
...
hk_result hk_Dense_Matrix_Util::invert_6x6(hk_Fixed_Dense_Matrix<6> &m,
                                           [[maybe_unused]] hk_real tolerance) {
  hk_real inverted[6 * 8];

  hk_Matrix_Inverter::invert_no_pivot_search(m.get_elems(), m.get_lda(),
                                             m.get_num_rows(), inverted);
...
hk_result hk_Dense_Matrix_Util::invert_5x5(hk_Fixed_Dense_Matrix<5> &m,
                                           [[maybe_unused]] hk_real tolerance) {
...
hk_result hk_Dense_Matrix_Util::invert_4x4(hk_Fixed_Dense_Matrix<4> &m,
                                           [[maybe_unused]] hk_real tolerance) {
...
hk_result hk_Dense_Matrix_Util::invert_3x3_symmetric(hk_Dense_Matrix &m,
                                                     hk_real tolerance) {
  ...
  hk_Vector3 r0;
  r0.set_cross(*reinterpret_cast<const hk_Vector3 *>(co1),
               *reinterpret_cast<const hk_Vector3 *>(co2));
  hk_Vector3 r1;
  r1.set_cross(*reinterpret_cast<const hk_Vector3 *>(co2),
               *reinterpret_cast<const hk_Vector3 *>(co0));
  hk_Vector3 r2;
  r2.set_cross(*reinterpret_cast<const hk_Vector3 *>(co0),
               *reinterpret_cast<const hk_Vector3 *>(co1));

  hk_real D =
      reinterpret_cast<const hk_Vector3 *>(co0)->dot(r0);  // main determinant
  ...
  reinterpret_cast<hk_Vector3 *>(co2)->set_mul(DI, r2);
  return HK_OK;
}

hk_result hk_Dense_Matrix_Util::invert_2x2(const hk_Dense_Matrix &m,
                                           hk_Dense_Matrix &out,
                                           hk_real tolerance) {
 ...
 * SOLVE
 */
hk_result hk_Dense_Matrix_Util::solve(hk_Dense_Matrix &m, hk_Dense_Vector &v,
                                      hk_real tolerance) {
