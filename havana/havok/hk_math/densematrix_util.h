...
class hk_Dense_Matrix_Util {
 public:
  static void mult(const hk_Dense_Matrix& m, const hk_Dense_Vector& in,
                   hk_Dense_Vector& out);
  static void mult(const hk_Dense_Matrix& m, const hk_real* in, hk_real* out);
  static inline void mult_3_symmetric(const hk_Fixed_Dense_Matrix<3>& m,
                                      const hk_Vector3& in, hk_Vector3& out);
  ...
  static hk_result invert_2x2(const hk_Dense_Matrix& in, hk_Dense_Matrix& out,
                              hk_real tolerance);

  static hk_result solve(hk_Dense_Matrix& m, hk_Dense_Vector& v,
                         hk_real tolerance);
