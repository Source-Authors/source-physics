...
void IVP_Compact_Surface::byte_swap() {
  /*
    ...
    int			byte_size:24;		// size of whole structure in
    bytes int			offset_ledgetree_root;	// offset to root node
    of internal ledgetree int			dummy[3];		//
    16byte memory align
  */
...
void IVP_Compact_Surface::byte_swap_all(IVP_BOOL swap_points,
                                        int point_estimate) {
#if defined(__POWERPC__)
  byte_swap();
#endif
  ...
  if (ltn) {
    if (swap_points) {
      ...
      const IVP_Compact_Ledge* ledge =
          ltn->get_compact_hull();  // returns const only?!
      ...
      //
      const_cast<IVP_Compact_Ledgetree_Node*>(ltn)->byte_swap_all(
          &swapped_points);
