...
IVP_Template_Compact_Grid::IVP_Template_Compact_Grid() { P_MEM_CLEAR(this); }

IVP_GridBuilder_Array::IVP_GridBuilder_Array(
    IVP_U_Memory *mm_, const IVP_Template_Compact_Grid *gp,
  ...
  height_points = (IVP_Compact_Poly_Point *)mm->get_mem(
      sizeof(IVP_Compact_Poly_Point) * n_rows * n_cols);
  unsigned int buffer_size =
      sizeof(IVP_Compact_Poly_Point) * n_rows * n_cols * 3;
  compact_poly_point_buffer = (IVP_Compact_Poly_Point *)mm->get_mem(
      buffer_size);  // two extra points per square worst case
  ledge_reference_field = (IVP_Compact_Grid_Element *)mm->get_memc(
      sizeof(IVP_Compact_Grid_Element) * n_rows *
      n_cols);  // last row and col not used

  grid_point_to_ledge_point_array =
      (int *)mm->get_mem(sizeof(int) * n_rows * n_cols);
  ...
  IVP_DOUBLE dx =
      (gp->row_info.invert_axis) ? -gp->grid_field_size : gp->grid_field_size;
  IVP_DOUBLE dy = (gp->column_info.invert_axis) ? -gp->grid_field_size
                                                : gp->grid_field_size;
...
int IVP_GridBuilder_Array::install_grid_point(int grid_point_index) {
  ...
  IVP_IF(1) {
    [[maybe_unused]] IVP_Compact_Poly_Point *cpp =
        &compact_poly_point_buffer[index];
...
void IVP_GridBuilder_Array::add_triangle(int s_p0_off, int s_p1_off,
  ...
  IVP_IF(1) {
    ...
    for (int e = 0; e < 3; e++) {
      ...
      if ((opp[e] < 0) && (opp[e] > -1000) &&
          (oppo->get_opposite_index() > -1000)) {
...
void IVP_GridBuilder_Array::insert_opposite_index(const IVP_Compact_Edge *edge,
                                                  int index) {
  ...
  IVP_IF(1) {
    ...
    oppo = edge->get_opposite();
    if (oppo->get_triangle() - c_ledge->get_first_triangle() >= triangle_count)
      return;  // not in list yet
    //	if((index < 0)&&(index > -1000) && (oppo->get_opposite_index() >
    //-1000)){
    IVP_ASSERT(edge == oppo->get_opposite());
...
IVP_Compact_Ledge *
IVP_GridBuilder_Array::convert_convex_triangle_to_compact_ledge(
    int strip_points[]) {
  int num_triangles = 2;
  ...
  int mem_size =
      sizeof(IVP_Compact_Ledge) + num_triangles * sizeof(IVP_Compact_Triangle);
  ...
  {  // set point arrays
    ...
    c_ledge->set_offset_ledge_points(
        (int)((uchar *)c_points -
              (uchar *)
                  c_ledge));  // byte offset from 'this' to (ledge) point array
  }
...
IVP_Compact_Ledge *
IVP_GridBuilder_Array::convert_convex_square_to_compact_ledge(
    int strip_points[], IVP_BOOL is_right_starter) {
  int num_triangles = 4;
  ...
  // pierce info
  IVP_Compact_Triangle *tri;
  tri = c_ledge->get_first_triangle();
  tri->set_pierce_index(2);
  tri = tri->get_next_tri();
  tri->set_pierce_index(2);
  tri = tri->get_next_tri();
  tri->set_pierce_index(0);
  tri = tri->get_next_tri();
  tri->set_pierce_index(0);
...
IVP_Compact_Ledge *
IVP_GridBuilder_Array::convert_convex_stripe_to_compact_ledge_fast(
    int strip_points[], int n_points, IVP_U_Float_Point ref_point[2],
  ...
  {  // set point arrays
    ...
    c_ledge->set_offset_ledge_points(
        (uchar *)c_points -
        (uchar *)c_ledge);  // byte offset from 'this' to (ledge) point array
  }

  c_ledge->set_is_compact(IVP_FALSE);
  c_ledge->set_size(mem_size);  // if size < 0 than compact_edge is not compact
                                // and shares point arrays
  c_ledge->n_triangles = num_triangles;
  c_ledge->has_chilren_flag = IVP_FALSE;
  ...
  int left_off, mid_off,
      right_off;  // used to override the upper edge opposite indices

  //// START TRIANGLES
  //// START TRIANGLES
  {
    IVP_U_Hesse hesse;
    hesse.calc_hesse(get_point_at_strip_pos(-1), get_point_at_strip_pos(-2),
                     get_point_at_strip_pos(0));
  ...
  while (1) {
    ...
    if (cp < end_point_idx) {
      ...
      IVP_U_Hesse hesse;
      hesse.calc_hesse(get_point_at_strip_pos(-2), get_point_at_strip_pos(-1),
                       get_point_at_strip_pos(cp + 1 - 2));
      IVP_DOUBLE dist = hesse.get_dist(get_point_at_strip_pos(cp /*+2-2*/));
      if (dist < 0.0f) {  // sign.
        const IVP_Compact_Triangle *tri =
            &c_ledge->get_first_triangle()[this->triangle_count - 1];
  ...
  //// INSERT PIERCING INFO
  {
    ...
    for (int t = num_triangles - 1; t >= 0; t--, tri = tri->get_next_tri()) {
      ...
      for (int u = num_triangles - 1; u >= 0;
           u--, tri2 = tri2->get_next_tri()) {
        if (t == u) continue;
...
void IVP_GridBuilder_Array::convert_strip_to_compact_ledges(
    int row, IVP_U_Vector<IVP_Compact_Ledge> *ledges) {
  int strip_field_offset[IVP_GRID_MAX_COLUMNS *
                         2];  // jumps alternately between the two rows of
                              // points defining row
  int dest_stripe[IVP_GRID_MAX_COLUMNS * 2];  // the compacted strip to be later
                                              // translated into compact ledge
  ...
  for (start_pos = 0; start_pos < strip_size - 2; start_pos = end_pos - 1) {
    ...
    dest_stripe[dest_index] =
        strip_field_offset[start_pos];  // insert first line
  ...
  check_flat_triangles:
    for (ep = start_pos; ep < strip_size - 3; ep += 2) {
      ...
      for (p = ep; p < ep + 3 && p < strip_size - 1; p++) {
        IVP_DOUBLE diff = height_field[strip_field_offset[p + 0]] +
                          height_field[strip_field_offset[p - 1]] -
                          height_field[strip_field_offset[p - 2]] -
                          height_field[strip_field_offset[p + 1]];
    ...
    if (ep < strip_size - 1) {  // check the next triangle for convex piece
      IVP_DOUBLE diff = height_field[strip_field_offset[ep + 0]] +
                        height_field[strip_field_offset[ep - 1]] -
                        height_field[strip_field_offset[ep - 2]] -
                        height_field[strip_field_offset[ep + 1]];
    ...
    while (1) {
      ...
      IVP_BOOL is_right_starter;
      if ((strip_field_offset[first_point] <
           strip_field_offset[first_point + 1]) ^
      ...
      if (dest_index == 4) {
        cl = convert_convex_square_to_compact_ledge(&dest_stripe[0],
                                                    is_right_starter);
        break;
      }
      {  // else
        ...
        {  // search for minimum value
          ...
          IVP_FLOAT col_pos = 0.5f * (p0a->k[templ->column_info.maps_to] +
                                      p0b->k[templ->column_info.maps_to]);
          ...
          z = p0b->k[templ->height_maps_to] * dz;
          if (z < minz) minz = z;
          z = p1a->k[templ->height_maps_to] * dz;
          if (z < minz) minz = z;
          z = p1b->k[templ->height_maps_to] * dz;
          if (z < minz) minz = z;
        ...
        cl = convert_convex_stripe_to_compact_ledge_fast(
            &dest_stripe[0], dest_index, ref_points, is_right_starter);
#ifdef DEBUG
    ...
    for (int p = first_point; p < ep - 1 /* == end_pos-1*/; p++) {
      ...
      if (a_offset < b_offset) {  // forward strip
        ...
        ledge_reference_field[b_offset - 1].compact_ledge_index[1] =
            ledge_index;
      }
    }
  }
...
void IVP_GridBuilder_Array::convert_array_to_compact_ledges(
    const IVP_Template_Compact_Grid *gp,
    IVP_U_Vector<IVP_Compact_Ledge> *ledges) {
  IVP_U_Matrix m_object_f_grid;
  {
    IVP_U_Point col_axle;
    col_axle.set_to_zero();
    IVP_U_Point row_axle;
    row_axle.set_to_zero();
    IVP_U_Point h_axle;
    h_axle.set_to_zero();
...
IVP_Compact_Grid *IVP_GridBuilder_Array::compile_ledges_into_compact_grid(
    const IVP_Template_Compact_Grid *gp,
    IVP_U_Vector<IVP_Compact_Ledge> *ledges) {
  IVP_Compact_Grid *cg = NULL;
  hk_intp buffer_size =
      offsetof(IVP_Compact_Grid,
               offset_compact_ledge_array);  // size for base compact ledge

  buffer_size +=
      ledges->len() * sizeof(int);  // add buffersize for ledge index array
  buffer_size +=
      (n_rows - 1) * (n_cols - 1) *
      sizeof(
          IVP_Compact_Grid_Element);  // the grid referencing ledge index array
  ...
  cg->offset_grid_elements =
      ((char *)(&cg->offset_compact_ledge_array[0])) - ((char *)cg) +
      sizeof(int) * ledges->len();  // skip the ledge offsets

  // TL: align offset to next 16 Byte Alignment (I didn't write the GridBuilder,
  // so I have no idea wether the memory will be enough, lets hope ... - the
  // SUN-Debugger will find it)
  ...
  {  // fill in the compact ledges
    ...
    char *dest = ((char *)cg) + cg->offset_grid_elements +
                 (n_rows - 1) * (n_cols - 1) * sizeof(IVP_Compact_Grid_Element);

    char *compact_points = dest;
    {  // all points
      int size_of_points =
          sizeof(IVP_Compact_Poly_Point) * n_compact_poly_points_used;
    ...
    for (int ledge_index = 0; ledge_index < ledges->len(); ledge_index++) {
      ...
      cl_dest->set_offset_ledge_points((char *)cl->get_point_array() -
                                       (char *)compact_poly_point_buffer +
                                       compact_points - (char *)cl_dest);

      IVP_ASSERT(cl_dest->get_point_array()->quad_distance_to(
                     cl->get_point_array()) == 0.0f);
  ...
  {  // fill in the grid referencing offset_compact_ledge_array
    IVP_Compact_Grid_Element *ge =
        (IVP_Compact_Grid_Element *)cg->get_grid_elements();  // discard const
...
IVP_Compact_Grid *IVP_GridBuilder_Array::convert_array_to_compact_grid(
    IVP_Environment *env, const IVP_Template_Compact_Grid *gp,
