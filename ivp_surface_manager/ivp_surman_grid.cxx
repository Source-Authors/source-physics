...
void IVP_SurfaceManager_Grid::traverse_grid(
  ...
  compact_grid->m_grid_f_object.vmult4(&visitor_position_object_space,
                                       &visitor_position_grid_space);
  ...
  if (fmax_row >= IVP_DOUBLE(compact_grid->n_rows))
    fmax_row = compact_grid->n_rows - 0.5f;
  if (fmax_col >= IVP_DOUBLE(compact_grid->n_columns))
    fmax_col = compact_grid->n_columns - 0.5f;
  ...
  char flag_buffer[8192];  // flags are needed to flag compact ledges already in
                           // vector
  char *flags;
    ...
    for (int col = min_col; col <= max_col; col++) {
      ...
      for (int i = 0; i < 2; i++) {
        ...
        const IVP_Compact_Ledge *ledge =
            compact_grid->get_compact_ledge_at(ledge_id);
#ifdef DEBUG
...
void IVP_SurfaceManager_Grid::get_all_ledges_within_radius(
    const IVP_U_Point *visitor_position_object_space, IVP_DOUBLE radius,
...
void IVP_SurfaceManager_Grid::insert_all_ledges_hitting_ray(
  ...
  get_all_ledges_within_radius(&center, ray_solver_os.ray_length * 0.5f, NULL,
                               NULL, NULL, &ledges);
...
void IVP_SurfaceManager_Grid::get_all_terminal_ledges(
    IVP_U_BigVector<IVP_Compact_Ledge> *resulting_ledges) {
  IVP_U_Point visitor_position_object_space;
  visitor_position_object_space.set_to_zero();
  traverse_grid(visitor_position_object_space, P_DOUBLE_MAX, resulting_ledges);
}

void IVP_SurfaceManager_Grid::get_mass_center(
    IVP_U_Float_Point *mass_center_out) const {
  mass_center_out->set(compact_grid->center.k);
}

void IVP_SurfaceManager_Grid::get_rotation_inertia(
...
void IVP_SurfaceManager_Grid::add_reference_to_ledge(
    const IVP_Compact_Ledge * /*ledge*/) {}

void IVP_SurfaceManager_Grid::remove_reference_to_ledge(
    const IVP_Compact_Ledge * /*ledge*/) {}
