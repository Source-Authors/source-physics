...
class IVP_Template_Grid_Axle_Descript {
 public:
  int n_points;  // number of points (not fields) for a certain direction (NOTE:
                 // for a grid consisting of 8x8 fields this value will be 9 for
                 // rows and columns)
  IVP_COORDINATE_INDEX maps_to;  // the coordinate index (IVP axis) the array's
                                 // row or column is translated onto
  IVP_BOOL invert_axis;  // set to true if the grid's "axis" translates to
                         // negative IVP axis direction
};
...
class IVP_GridBuilder_Array {
 private:
  ...
  void convert_strip_to_compact_ledges(int row,
                                       IVP_U_Vector<IVP_Compact_Ledge> *ledges);
  void convert_array_to_compact_ledges(const IVP_Template_Compact_Grid *,
                                       IVP_U_Vector<IVP_Compact_Ledge> *ledges);

  IVP_Compact_Ledge *convert_convex_triangle_to_compact_ledge(
      int strip_points[]);
  IVP_Compact_Ledge *convert_convex_square_to_compact_ledge(
      int strip_points[], IVP_BOOL is_right_starter);

  IVP_Compact_Grid *compile_ledges_into_compact_grid(
      const IVP_Template_Compact_Grid *gp,
      IVP_U_Vector<IVP_Compact_Ledge> *ledges);
  IVP_GridBuilder_Array(IVP_U_Memory *mm, const IVP_Template_Compact_Grid *gp,
                        IVP_FLOAT *height_field);
  // converts a convex strip to a compact ledge, the reference points are needed
  // for guaranteed convexity
  IVP_Compact_Ledge *convert_convex_stripe_to_compact_ledge_fast(
      int strip_points[], int n_points, IVP_U_Float_Point reference_points[2],
      IVP_BOOL is_single_starter);

  void add_triangle(int p0_off, int p1_off, int p2_off, int opp0, int opp1,
                    int opp2);
 ...
 public:
   ...
   *****************************************************************************/
  static IVP_Compact_Grid *IVP_CDECL convert_array_to_compact_grid(
      IVP_Environment *environment, const IVP_Template_Compact_Grid *parameters,
      IVP_FLOAT *height_field);
