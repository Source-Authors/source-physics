...
class IVP_SurfaceManager_Polygon_Solver {
 public:
  ...
  void traverse_cluster(const IVP_Compact_Ledgetree_Node *node,
                        const IVP_U_Point *visitor_position_object_space,
                        IVP_DOUBLE radius,
                        IVP_U_BigVector<IVP_Compact_Ledge> *resulting_ledges);
};

void IVP_SurfaceManager_Polygon_Solver::traverse_cluster(
    const IVP_Compact_Ledgetree_Node *node,
    const IVP_U_Point *visitor_position_object_space, IVP_DOUBLE radius,
    IVP_U_BigVector<IVP_Compact_Ledge> *resulting_ledges) {
  const IVP_DOUBLE work = IVP_COMPACT_BOUNDINGBOX_STEP_SIZE;

  // this->traversion_depth++;
  // if ( this->traversion_depth > this->max_traversion_depth )
  // this->max_traversion_depth = this->traversion_depth;
  ...
  if (IVP_Inline_Math::fabsd(dist_vec.k[0]) >=
      (node->box_sizes[0] * work2 + radius)) {
    IVP_IF(0) { ivp_message("Box\n"); }
    return;
  }
  if (IVP_Inline_Math::fabsd(dist_vec.k[1]) >=
      (node->box_sizes[1] * work2 + radius)) {
    IVP_IF(0) { ivp_message("Box\n"); }
    return;
  }
  if (IVP_Inline_Math::fabsd(dist_vec.k[2]) >=
      (node->box_sizes[2] * work2 + radius)) {
    IVP_IF(0) { ivp_message("Box\n"); }
    return;
  }

  // ivp_message("Node    : %f - %f *** %f - %f *** %f - %f\n", min_x, max_x,
  // min_y, max_y, min_z, max_z); ivp_message("Intruder: %f - %f *** %f - %f ***
  // %f - %f\n", s_bb_min_x, s_bb_max_x, s_bb_min_y, s_bb_max_y, s_bb_min_z,
  // s_bb_max_z);
#endif
  ...
  if (node->is_terminal()) {
    ...
    if (ledge->get_n_triangles() ==
        2) {  // IVP_DOUBLE triangle allows deeper search
      const IVP_Compact_Triangle *triangle = ledge->get_first_triangle();
      IVP_DOUBLE qdist = IVP_CLS.calc_qlen_PF_F_space(
          ledge, triangle, visitor_position_object_space);
  ...
  traverse_cluster(node->left_son(), visitor_position_object_space, radius,
                   resulting_ledges);
  // this->traversion_depth--;
  traverse_cluster(node->right_son(), visitor_position_object_space, radius,
                   resulting_ledges);
  // this->traversion_depth--;
...
void IVP_SurfaceManager_Polygon::insert_all_ledges_hitting_ray(
    IVP_Ray_Solver *ray_solver, IVP_Real_Object *object) {
...
void IVP_SurfaceManager_Polygon::get_all_ledges_within_radius(
    const IVP_U_Point *visitor_position_object_space, IVP_DOUBLE radius,
    const IVP_Compact_Ledge *root_ledge, IVP_Real_Object * /*other_object*/,
  ...
  if (!root_ledge) {
    ...
    sps.traverse_cluster(root_node, visitor_position_object_space, radius,
                         resulting_ledges);
  } else {
    const IVP_Compact_Ledgetree_Node *root_node =
        root_ledge->get_ledgetree_node();
    sps.traverse_cluster(root_node->left_son(), visitor_position_object_space,
                         radius, resulting_ledges);
    sps.traverse_cluster(root_node->right_son(), visitor_position_object_space,
                         radius, resulting_ledges);
  }
  return;
}

void IVP_SurfaceManager_Polygon::get_all_terminal_ledges(
    IVP_U_BigVector<IVP_Compact_Ledge> *resulting_ledges) {
  IVP_Compact_Ledge_Solver::get_all_ledges(compact_surface, resulting_ledges);
}

void IVP_SurfaceManager_Polygon::get_mass_center(
    IVP_U_Float_Point *mass_center_out) const {
  mass_center_out->set(compact_surface->mass_center.k);
}

void IVP_SurfaceManager_Polygon::get_rotation_inertia(
    IVP_U_Float_Point *rotation_inertia_out) const {
  rotation_inertia_out->set(compact_surface->rotation_inertia.k);
}

void IVP_SurfaceManager_Polygon::get_radius_and_radius_dev_to_given_center(
  ...
  *radius_deviation = compact_surface->max_factor_surface_deviation *
                          IVP_COMPACT_SURFACE_DEVIATION_STEP_SIZE *
                          compact_surface->upper_limit_radius +
                      center_shift;
}

const IVP_Compact_Ledge *IVP_SurfaceManager_Polygon::get_single_convex() const {
  const IVP_Compact_Ledgetree_Node *root =
      compact_surface->get_compact_ledge_tree_root();
...
void IVP_SurfaceManager_Polygon::add_reference_to_ledge(
    const IVP_Compact_Ledge *ledge) {
  IVP_USE(ledge);
  return;
}

void IVP_SurfaceManager_Polygon::remove_reference_to_ledge(
    const IVP_Compact_Ledge *ledge) {
  IVP_USE(ledge);
  return;
}
