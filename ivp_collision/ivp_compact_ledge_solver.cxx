...
void IVP_Compact_Ledge_Solver::calc_bounding_box(
    const IVP_Compact_Ledge *c_ledge_in, IVP_U_Point *min_extents_out,
    IVP_U_Point *max_extents_out) {
  // calc object axis aligned bounding box

  // @@@ for point array might be shared, we walk through all points of all
  // triangles!
  ...
  {  // set min max to any point
    ...
    const IVP_U_Float_Point *any_ledge_point =
        give_object_coords(first_edge, c_ledge_in);
  ...
  for (; tri_cnt >= 0; tri_cnt--, tri = tri->get_next_tri()) {
    ...
    for (edge_cnt = 2, edge = tri->get_first_edge(); edge_cnt >= 0;
...
void IVP_Compact_Ledge_Solver::calc_geom_center_and_radius(
    const IVP_Compact_Ledge *c_ledge_in, IVP_U_Point *geom_center_out,
    IVP_DOUBLE *geom_radius_out) {
  ...
  *geom_radius_out =
      0.5f * IVP_Inline_Math::sqrtd(min_extents.quad_distance_to(&max_extents));
};
...
void IVP_Compact_Ledge_Solver::calc_radius_to_given_center(
    const IVP_Compact_Ledge *ledge, const IVP_U_Point *center_in,
    IVP_DOUBLE *radius_out, IVP_DOUBLE *radius_dev_out) {
...
#ifdef HAVOK_MOPP
void IVP_Compact_Ledge_Solver::calc_radius_to_given_center(
    const IVP_Compact_Mopp *c_mopp_in, const IVP_U_Point *center_in,
    IVP_DOUBLE *radius_out, IVP_DOUBLE *radius_dev_out) {
  *radius_out = 0;
  *radius_dev_out = 0;
...
void IVP_Compact_Ledge_Solver::calc_radius_to_given_center(
    const IVP_Compact_Surface *c_surface_in, const IVP_U_Point *center_in,
    IVP_DOUBLE *radius_out, IVP_DOUBLE *radius_dev_out) {
  *radius_out = 0;
  *radius_dev_out = 0;
  IVP_U_BigVector<IVP_Compact_Ledge> all_ledges(1024);
...
void IVP_Compact_Ledge_Solver::calc_pos_other_space(
    const IVP_Compact_Edge *P, IVP_Cache_Ledge_Point *m_cache_P,
  ...
  m_cache_other_space->clp_cache_object->m_world_f_object.inline_vimult4(&p_ws,
                                                                         res);
}

void IVP_Compact_Ledge_Solver::transform_vec_other_space(
    const IVP_U_Point *dir_os, IVP_Cache_Ledge_Point *m_cache_dir,
    IVP_Cache_Ledge_Point *m_cache_other_space, IVP_U_Point *res) {
  static IVP_U_Point dir_ws;
  m_cache_dir->clp_cache_object->m_world_f_object.inline_vmult3(dir_os,
                                                                &dir_ws);
  m_cache_other_space->clp_cache_object->m_world_f_object.inline_vimult3(
      &dir_ws, res);
}

void IVP_Compact_Ledge_Solver::transform_pos_other_space(
    const IVP_U_Float_Point *pos_in_os, IVP_Cache_Ledge_Point *m_cache_dir,
    IVP_Cache_Ledge_Point *m_cache_other_space, IVP_U_Point *res) {
  IVP_U_Point dir_ws;
  IVP_U_Point pos_os(pos_in_os);
  m_cache_dir->clp_cache_object->m_world_f_object.inline_vmult4(&pos_os,
                                                                &dir_ws);
  m_cache_other_space->clp_cache_object->m_world_f_object.inline_vimult4(
      &dir_ws, res);
}
#ifdef DEBUG
/********************************************************************************
 *	Names:	       	calc s_vals
 *	Description:	find the point on an edge with the shortest distance to
 *a given other point and return the position of that point on the edge as a
 *relative value between 0 and 1:   0 means projected point is identical to
 *first point of edge 1 means projected point is identical to second point od
 *the edge
 ********************************************************************************/
IVP_DOUBLE IVP_Compact_Ledge_Solver::calc_s_val(
    const IVP_Compact_Edge *edge, const IVP_U_Point *p_world,
    IVP_Cache_Ledge_Point *m_cache_edge) {
  // calcs intersect pos
  // von lot von p auf this (rel. zu this)
  IVP_U_Point tp;
  give_world_coords_AT(edge, m_cache_edge, &tp);
  IVP_U_Point tp_next;
  give_world_coords_AT(edge->get_next(), m_cache_edge, &tp_next);
...
void IVP_Compact_Ledge_Solver::calc_unscaled_s_val_K_space(
    const IVP_Compact_Ledge *c_ledge, const IVP_Compact_Edge *edge,
    const IVP_U_Point *p_object, IVP_Unscaled_S_Result *result) {
  // calcs intersect pos
  // von lot von p auf this (rel. zu this)
  ...
  d.subtract(p1, p0);
  IVP_U_Point d0;
  d0.subtract(p0, p_object);
  IVP_U_Point d1;
  d1.subtract(p1, p_object);
...
// for debugging only
#ifdef DEBUG
void IVP_Compact_Ledge_Solver::calc_qr_vals(
    const IVP_Compact_Edge *e_tri, const IVP_U_Point *p_world,
    IVP_DOUBLE *out_q, IVP_DOUBLE *out_r,
    IVP_Cache_Ledge_Point *m_cache_e_tri) {
  IVP_U_Point tp;
  IVP_CLS.give_world_coords_AT(e_tri, m_cache_e_tri, &tp);
  IVP_U_Point tp_next;
  IVP_CLS.give_world_coords_AT(e_tri->get_next(), m_cache_e_tri, &tp_next);
  IVP_U_Point tp_prev;
  IVP_CLS.give_world_coords_AT(e_tri->get_prev(), m_cache_e_tri, &tp_prev);
...
#ifdef DEBUG
void IVP_Compact_Ledge_Solver::calc_qr_vals(
    const IVP_Compact_Edge *e_tri, const IVP_Compact_Edge *p, IVP_DOUBLE *out_q,
    IVP_DOUBLE *out_r, IVP_Cache_Ledge_Point *m_cache_e_tri,
    IVP_Cache_Ledge_Point *m_cache_p) {
  IVP_U_Point pp;
  IVP_CLS.give_world_coords_AT(p, m_cache_p, &pp);
  calc_qr_vals(e_tri, &pp, out_q, out_r, m_cache_e_tri);
}
#endif

/* rounding error of calc_unscaled_qr_vals_F_space:
   input:  p_object    error (P_MAX_WORLD_DOUBLE * P_DOUBLE_RES * ( 1 + x ) ||
   inf) tp, tp_next, tp_prv 0 epsilon
 */

static char ivp_uqr_mod_table[] = {
    0, 2 * sizeof(IVP_FLOAT), 1 * sizeof(IVP_FLOAT), 0, 2 * sizeof(IVP_FLOAT)};

void IVP_Compact_Ledge_Solver::calc_unscaled_qr_vals_F_space(
    const IVP_Compact_Ledge *c_ledge, const IVP_Compact_Edge *tri,
    const IVP_U_Point *p_object, IVP_Unscaled_QR_Result *result) {
  ...
  const IVP_U_Float_Point *tp =
      give_object_coords(triangle->get_edge(0), c_ledge);
  const IVP_U_Float_Point *tp_next =
      give_object_coords(triangle->get_edge(1), c_ledge);
  const IVP_U_Float_Point *tp_prev =
      give_object_coords(triangle->get_edge(2), c_ledge);
  ...
  ((IVP_FLOAT *)(res + ivp_uqr_mod_table[this_edge_index + 1]))[0] =
      Det - q - r;
  ((IVP_FLOAT *)(res + ivp_uqr_mod_table[this_edge_index + 2]))[0] = q;
}

IVP_KK_Input::IVP_KK_Input(const IVP_Compact_Edge *K_in,
  ...
  const IVP_U_Float_Point *L_next_os =
...
IVP_RETURN_TYPE IVP_Compact_Ledge_Solver::calc_unscaled_KK_vals(
    const IVP_KK_Input &in, IVP_Unscaled_KK_Result *result) {
  // returns IVP_FAULT when edges are parallel, in this case sk and sl are
  // estimated calcs common point of two straights K and L (as Tri_Edges) and
  // returns both intersect proportions
  const IVP_U_Point &norm = in.cross_KL_Los;
  IVP_DOUBLE quad_dist = norm.quad_length();

  if (quad_dist > (P_DOUBLE_RES * P_DOUBLE_RES * P_MAX_OBJECT_SIZE *
                   P_MAX_OBJECT_SIZE)) {  // not parallel
  ...
  IVP_IF(0) { ivp_message("calc_unscaled_KK_vals: parallel edges\n"); }
...
#define IVP_KK_CHECK_1 0.000001f
  ...
  // check K positions
  {
    ...
    for (int i = 0; i < 11; i++) {
      ...
      IVP_U_Point pK_Los;
      pK_Los.set_interpolate(&in.K_Los[0], &in.K_Los[1], pos);
      IVP_DOUBLE qdist =
          quad_dist_edge_to_point_K_space(ledge_L, in.L, &pK_Los);
      if (qdist < min_qdist) {
        min_qdist = qdist;
  ...
  }
  {
    ...
    for (int i = 0; i < 9; i++) {
      IVP_FLOAT pos = check_pos[i];
      IVP_U_Point pL_Kos;
      pL_Kos.set_interpolate(&L_Kos[0], &L_Kos[1], pos);
      IVP_DOUBLE qdist =
          quad_dist_edge_to_point_K_space(ledge_K, in.K, &pL_Kos);
      if (qdist < min_qdist) {
        min_qdist = qdist;
...
#ifdef DEBUG
// debugging only
void IVP_Compact_Ledge_Solver::calc_hesse_normized_AT(
    const IVP_Compact_Edge *edge, IVP_Cache_Ledge_Point *clp,
    IVP_U_Hesse *hesse_out_ws) {
  // normized
  IVP_U_Point p0;
  IVP_CLS.give_world_coords_AT(edge, clp, &p0);
  IVP_U_Point p1;
  IVP_CLS.give_world_coords_AT(edge->get_next(), clp, &p1);
  IVP_U_Point p2;
  IVP_CLS.give_world_coords_AT(edge->get_prev(), clp, &p2);
  hesse_out_ws->calc_hesse(&p0, &p2, &p1);
  hesse_out_ws->normize();
}
#endif

void IVP_Compact_Ledge_Solver::calc_hesse_object(const IVP_Compact_Edge *edge,
  ...
  const IVP_U_Float_Point *p1 =
      IVP_CLS.give_object_coords(edge->get_next(), ledge);
  const IVP_U_Float_Point *p2 =
      IVP_CLS.give_object_coords(edge->get_prev(), ledge);
  hesse_out_os->calc_hesse(p0, p2, p1);
  hesse_out_os->normize();
}

void IVP_Compact_Ledge_Solver::calc_hesse_vec_object_not_normized(
...
IVP_DOUBLE IVP_Compact_Ledge_Solver::quad_dist_edge_to_point_K_space(
    const IVP_Compact_Ledge *ledge_K, const IVP_Compact_Edge *K,
    const IVP_U_Point *object_pos) {
  const IVP_U_Float_Point *p0 = IVP_CLS.give_object_coords(K, ledge_K);
  const IVP_U_Float_Point *p1 =
      IVP_CLS.give_object_coords(K->get_next(), ledge_K);
...
IVP_DOUBLE IVP_Compact_Ledge_Solver::calc_qlen_PF_F_space(
    const IVP_Compact_Ledge *ledge, const IVP_Compact_Triangle *tri,
    const IVP_U_Point *object_pos) {
  ...
  if (!qr.is_outside()) {
    ...
    IVP_DOUBLE res =
        normal.dot_product(object_pos) - normal.dot_product(start_point);
  ...
  for (int i = 2; i >= 0; i--) {
    ...
    if (sr.checks[1] < 0)
      continue;  // no need to check this case as this point wil be touched with
                 // the next edge
    IVP_DOUBLE qdist =
        IVP_CLS.quad_dist_edge_to_point_K_space(ledge, e, object_pos);
    if (qdist < qres) qres = qdist;
  }
  return qres;
};

IVP_DOUBLE IVP_Compact_Ledge_Solver::calc_qlen_PP_P_space(
    const IVP_Compact_Ledge *P_ledge, const IVP_Compact_Edge *P,
    const IVP_U_Point *P_Pos) {
  const IVP_U_Float_Point *P_os = give_object_coords(P, P_ledge);
  return P_Pos->quad_distance_to(P_os);
}

IVP_DOUBLE IVP_Compact_Ledge_Solver::calc_qlen_PK_K_space(
    const IVP_U_Point *P_in_K_space, const IVP_Compact_Ledge *K_ledge,
...
void IVP_Compact_Ledge_Solver::give_world_coords_AT(
    const IVP_Compact_Edge *edge, IVP_Cache_Ledge_Point *clp,
...
IVP_DOUBLE IVP_Compact_Ledge_Solver::calc_qlen_KK(
  ...
  if (!kkr.is_outside_L()) {
    if (kkr.checks_K[0] <= 0.0f) {
      return calc_qlen_PK_K_space(&kkin.K_Los[0],
                                  m_cache_L->get_compact_ledge(), L);
    }
    if (kkr.checks_K[1] <= 0.0f) {
      return calc_qlen_PK_K_space(&kkin.K_Los[1],
                                  m_cache_L->get_compact_ledge(), L);
    }
  ...
  for (i = 0; i <= 1; i++) {
    ...
    IVP_DOUBLE h_len =
        calc_qlen_PK_K_space(&L_Kos, m_cache_K->get_compact_ledge(), K);
    if (h_len < min_len) {
      min_len = h_len;
      min_Kp = pL[i];
    }
    IVP_DOUBLE h_len_2 =
        calc_qlen_PK_K_space(&kkin.K_Los[i], m_cache_L->get_compact_ledge(), L);
    if (h_len_2 < min_len) {
      min_len = h_len_2;
      min_Kp = pK[i];
    }
...
IVP_BOOL IVP_Compact_Ledge_Solver::check_ledge(const IVP_Compact_Ledge *cl) {
  ...
  for (int i = 0; i < cl->get_n_triangles(); i++) {
    ...
    for (int j = 0; j < 3; j++) {
      ...
      IVP_ASSERT(edge->get_start_point_index() ==
                 oppo->get_next()->get_start_point_index());
      ...
      int max_c = 0;
      for (const IVP_Compact_Edge *e = edge->get_opposite();;
           e = e->get_next()->get_opposite()) {
        if (max_c++ > 1000) CORE;
