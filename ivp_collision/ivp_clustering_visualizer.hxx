...
enum IVP_CLUSTERING_VISUALIZER_FILTER { ACCEPT = 1, REJECT = 2 };
...
class IVP_Clustering_Visualizer_Longrange {
 private:
  // global data
  IVP_BOOL enabled;
  IVP_U_Vector<IVP_Clustering_Visualizer_Longrange_Callback>
      graphics_callbacks;  // list of user-definable callbacks
 ...
 public:
  ...
  void add_object_to_filter(IVP_Real_Object *obj,
                            IVP_CLUSTERING_VISUALIZER_FILTER filter);
  void remove_object_from_filter(IVP_Real_Object *obj,
                                 IVP_CLUSTERING_VISUALIZER_FILTER filter);

  void add_object(IVP_Real_Object *real_object, IVP_U_Float_Point *center,
                  IVP_DOUBLE radius);
   ...
   * Description:	Used to install or remove a certain visualization
   *callback. Input:		<callback>  callback to install/remove
   *****************************************************************************/
  void install_visualize_callback(
      IVP_Clustering_Visualizer_Longrange_Callback *callback);
  void remove_visualize_callback(
      IVP_Clustering_Visualizer_Longrange_Callback *callback);
...
class IVP_Clustering_Visualizer_Shortrange_Callback {
 public:
  ...
  IVP_BOOL
  reported_ledge;  // IVP_TRUE if node is terminal and the corresponding
                   // ledge has been passed back from hierarchy check!
  void *user_data;
...
class IVP_Clustering_Visualizer_Shortrange {
 private:
  ...
  IVP_Clustering_Visualizer_Shortrange_Objectdata
      *pipelined_private_property_data;

  void add_node_to_pipeline(const IVP_Compact_Ledgetree_Node *node,
 ...
 public:
  ...
  IVP_BOOL
  remove_longrange_visualization;  // do not display longrange visualization
                                   // if object hierarchy is traversed!
  ...
  void analyze_collision(IVP_Real_Object *object0, IVP_DOUBLE radius0,
                         IVP_Real_Object *object1, IVP_DOUBLE radius1);

  void add_object_to_intruder_filter(IVP_Real_Object *obj,
                                     IVP_CLUSTERING_VISUALIZER_FILTER filter);
  void add_object_to_private_property_filter(
      IVP_Real_Object *obj, IVP_CLUSTERING_VISUALIZER_FILTER filter);
  void remove_object_from_intruder_filter(
      IVP_Real_Object *obj, IVP_CLUSTERING_VISUALIZER_FILTER filter);
  void remove_object_from_private_property_filter(
      IVP_Real_Object *obj, IVP_CLUSTERING_VISUALIZER_FILTER filter);
