...
IVP_Ray_Solver::IVP_Ray_Solver(const IVP_Ray_Solver_Template *templ) {
  ...
  this->ray_center_point.add_multiple(&this->ray_direction,
                                      this->ray_length * 0.5f);

  this->ray_end_point.add_multiple(&ray_start_point, &ray_direction,
                                   ray_length);
}

void IVP_Ray_Solver_Min_Hash::add_hit_object(
    IVP_Real_Object *object, const IVP_Compact_Ledge *compact_ledge,
    const IVP_Compact_Triangle *compact_triangle, IVP_DOUBLE hit_dist,
  ...
  if (this->output_min_hash.counter >= IVP_MAX_NUM_RAY_HITS) {
    IVP_IF(1) {
      ivp_message(
          "IVP_Ray_Solver::add_hit_object - max ray hits exceeded -> hit list "
          "truncated.\n");
    }
    // todo: if hit_dist < current hash minimum: replace any min hash entry with
    // this hit
    // ...
    return;
  }
...
IVP_BOOL IVP_Ray_Solver::check_ray_against_sphere(
    const IVP_U_Float_Point *sphere_center_ws, IVP_FLOAT sphere_radius) {
...
IVP_BOOL IVP_Ray_Solver_Group::check_ray_group_against_sphere(
    const IVP_U_Float_Point *sphere_center_ws, IVP_FLOAT sphere_radius) {
  // returns IVP_FALSE if sphere is not hit by ray
  IVP_U_Float_Point delta_vec;
...
IVP_BOOL IVP_Ray_Solver_Os::check_ray_against_sphere_os(
    const IVP_U_Float_Point *sphere_center_os, IVP_FLOAT sphere_radius) {
  // returns IVP_FALSE if sphere is not hit by ray
...
IVP_BOOL IVP_Ray_Solver::check_ray_against_square(
    IVP_FLOAT pos_dist, IVP_FLOAT pos_axis_len,
    const IVP_U_Float_Point *min_coords, const IVP_U_Float_Point *max_coords,
  ...
  for (i = 1; i >= 0; --i) {
    ...
    if ((comp_val <
         (min_coords->k[coord] - ray_start_point.k[coord]) * pos_axis_len) ||
        (comp_val >
         (max_coords->k[coord] - ray_start_point.k[coord]) * pos_axis_len)) {
      // miss
      return IVP_FALSE;
    }
  }
...
IVP_BOOL IVP_Ray_Solver::check_ray_against_cube(
    const IVP_U_Float_Point *luf_point, const IVP_U_Float_Point *rlb_point) {
  ...
  if (ray_start_point.k[0] < luf_point->k[0])
    start_point_flags = IVP_LEFT;
  else if (ray_start_point.k[0] > rlb_point->k[0])
    start_point_flags = IVP_RIGHT;

  if (ray_start_point.k[1] < luf_point->k[1])
    start_point_flags |= IVP_ABOVE;
  else if (ray_start_point.k[1] > rlb_point->k[1])
    start_point_flags |= IVP_BELOW;

  if (ray_start_point.k[2] < luf_point->k[2])
    start_point_flags |= IVP_FRONT;
  else if (ray_start_point.k[2] > rlb_point->k[2])
    start_point_flags |= IVP_BEHIND;
  ...
  if (ray_end_point.k[0] < luf_point->k[0])
    end_point_flags = IVP_LEFT;
  else if (ray_end_point.k[0] > rlb_point->k[0])
    end_point_flags = IVP_RIGHT;

  if (ray_end_point.k[1] < luf_point->k[1])
    end_point_flags |= IVP_ABOVE;
  else if (ray_end_point.k[1] > rlb_point->k[1])
    end_point_flags |= IVP_BELOW;

  if (ray_end_point.k[2] < luf_point->k[2])
    end_point_flags |= IVP_FRONT;
  else if (ray_end_point.k[2] > rlb_point->k[2])
    end_point_flags |= IVP_BEHIND;
  ...
  if ((start_point_flags & IVP_FRONT) && !(end_point_flags & IVP_FRONT)) {
    ...
    IVP_U_Float_Point max_coords(rlb_point->k[0], rlb_point->k[1],
                                 luf_point->k[2]);
    IVP_BOOL res = check_ray_against_square(pos_dist, pos_axis_len, luf_point,
                                            &max_coords, 0, 1);
  ...
  if ((start_point_flags & IVP_RIGHT) && !(end_point_flags & IVP_RIGHT)) {
    ...
    IVP_ASSERT(pos_dist >= 0.0f);
    IVP_ASSERT(pos_axis_len >= 0.0f);
    IVP_U_Float_Point min_coords(rlb_point->k[0], luf_point->k[1],
                                 luf_point->k[2]);
    IVP_BOOL res = check_ray_against_square(pos_dist, pos_axis_len, &min_coords,
                                            rlb_point, 1, 2);
    if (res) return IVP_TRUE;
  }
  // facet 2 (back)
  if ((start_point_flags & IVP_BEHIND) && !(end_point_flags & IVP_BEHIND)) {
    ...
    IVP_ASSERT(pos_dist >= 0.0f);
    IVP_ASSERT(pos_axis_len >= 0.0f);
    IVP_U_Float_Point min_coords(luf_point->k[0], luf_point->k[1],
                                 rlb_point->k[2]);
    IVP_BOOL res = check_ray_against_square(pos_dist, pos_axis_len, &min_coords,
                                            rlb_point, 0, 1);
    if (res) return IVP_TRUE;
  ...
  if ((start_point_flags & IVP_LEFT) && !(end_point_flags & IVP_LEFT)) {
    ...
    IVP_ASSERT(pos_dist >= 0.0f);
    IVP_ASSERT(pos_axis_len >= 0.0f);
    IVP_U_Float_Point max_coords(luf_point->k[0], rlb_point->k[1],
                                 rlb_point->k[2]);
    IVP_BOOL res = check_ray_against_square(pos_dist, pos_axis_len, luf_point,
                                            &max_coords, 1, 2);
    if (res) return IVP_TRUE;
  ...
  if ((start_point_flags & IVP_ABOVE) && !(end_point_flags & IVP_ABOVE)) {
    ...
    IVP_ASSERT(pos_dist >= 0.0f);
    IVP_ASSERT(pos_axis_len >= 0.0f);
    IVP_U_Float_Point max_coords(rlb_point->k[0], luf_point->k[1],
                                 rlb_point->k[2]);
    IVP_BOOL res = check_ray_against_square(pos_dist, pos_axis_len, luf_point,
                                            &max_coords, 0, 2);
    if (res) return IVP_TRUE;
  ...
  if ((start_point_flags & IVP_BELOW) && !(end_point_flags & IVP_BELOW)) {
    ...
    IVP_ASSERT(pos_dist >= 0.0f);
    IVP_ASSERT(pos_axis_len >= 0.0f);
    IVP_U_Float_Point min_coords(luf_point->k[0], rlb_point->k[1],
                                 luf_point->k[2]);
    IVP_BOOL res = check_ray_against_square(pos_dist, pos_axis_len, &min_coords,
                                            rlb_point, 0, 2);
    if (res) return IVP_TRUE;
...
IVP_BOOL IVP_Ray_Solver_Group::check_ray_group_against_cube(
    const IVP_U_Float_Point *cube_center_ws, IVP_FLOAT cube_size) {
  IVP_U_Float_Point dist;
  dist.subtract(cube_center_ws, &center_ws);
  dist.set(IVP_Inline_Math::fabsd(dist.k[0]), IVP_Inline_Math::fabsd(dist.k[1]),
...
IVP_BOOL IVP_Ray_Solver_Os::check_ray_against_compact_ledge_os(
  ...
  // Get hold of a surface that is pointing into the opposite of the ray
  // direction.
  ...
  if (n_triangles == 2) {  // super fast version for triangle ledges
    ...
    IVP_CLS.calc_hesse_vec_object_not_normized(edge, ledge_to_compare,
                                               &hesse_vec_os);

    const IVP_U_Float_Point *p0 =
        IVP_CLS.give_object_coords(edge, ledge_to_compare);
    ...
    intersect_point.set_interpolate(&ray_start_point, &ray_end_point, hit_dist);

    IVP_Unscaled_QR_Result qr;
    IVP_CLS.calc_unscaled_qr_vals_F_space(ledge_to_compare, edge,
                                          &intersect_point, &qr);
    ...
    hit_listener->add_hit_object(object, ledge_to_compare, tri,
                                 hit_dist * ray_length, &hesse_vec_os);
    return IVP_TRUE;
  }
  ...
  if (!take_edge) {
    IVP_IF(0) {
      ivp_message(
          "IVP_Ray_Solver::check_ray_against_compact_ledge - no correct face "
          "found.\n");
    }
    return IVP_FALSE;
  }

  {
    ...
    for (; --tri_cnt >= 0;) {
      ...
      IVP_DOUBLE hit_dist = a / (a - b);
      IVP_U_Point intersect_point;
      intersect_point.set_interpolate(&ray_start_point, &ray_end_point,
                                      hit_dist);
      ...
      hit_dist *= ray_length;
      hesse_vec_os.IVP_U_Point::normize();
      hit_listener->add_hit_object(object, ledge_to_compare,
                                   edge->get_triangle(), hit_dist,
                                   &hesse_vec_os);
      return IVP_TRUE;
    ...
    // ...
    IVP_IF(1) { ivp_message("check_ray_against_compact_ledge: endl loop.\n"); }
    return IVP_FALSE;
  }
}

void IVP_Ray_Solver_Os::check_ray_against_ledge_tree_node_os(
  ...
  IVP_BOOL ray_hits_sphere =
      this->check_ray_against_sphere_os(&center, node->radius);
...
void IVP_Ray_Solver_Os::check_ray_against_compact_surface_os(
...
IVP_Ray_Solver_Os::IVP_Ray_Solver_Os(class IVP_Ray_Solver *so,
                                     IVP_Real_Object *obj) {
  object = obj;
  hit_listener = so;
  ...
  co->transform_position_to_object_coords(&so->ray_start_point,
                                          &this->ray_start_point);
  co->transform_vector_to_object_coords(&so->ray_direction,
                                        &this->ray_direction);
  this->ray_length = so->ray_length;
  this->ray_end_point.add_multiple(&this->ray_start_point, &this->ray_direction,
                                   this->ray_length);
  this->ray_center_point.set(&this->ray_start_point);
  this->ray_center_point.add_multiple(&this->ray_direction,
                                      this->ray_length * 0.5f);
...
void IVP_Ray_Solver::check_ray_against_object(IVP_Real_Object *object) {
  ...
  if (object) {
    if (ray_flags & IVP_RAY_SOLVER_IGNORE_PHANTOMS) {
      if (object->get_controller_phantom()) return;
    }
    if (ray_flags & IVP_RAY_SOLVER_IGNORE_MOVINGS) {
      if (!IVP_MTIS_IS_STATIC(object->get_movement_state())) return;
    }
...
void IVP_Ray_Solver_Group::check_ray_group_against_object(
...
void IVP_Ray_Solver::check_ray_against_node(IVP_OV_Node *node,
                                            IVP_OV_Tree_Manager *ov_tree_man) {
  // Recursive function

  // Check whether ray hits the long range cluster node.
  IVP_U_Float_Point luf_point,
      rlb_point;  // left upper front / right lower back
  IVP_FLOAT cubesize;
  ov_tree_man->get_luf_coordinates_ws(
      node, &luf_point,
      &cubesize);  // fills vars with left-upper-front corner's coordinates
  rlb_point.set(luf_point.k[0] + cubesize, luf_point.k[1] + cubesize,
                luf_point.k[2] + cubesize);
  ...
  // Check all objects that are contained by this node.
  {
    ...
    for (i = node->elements.len() - 1; i >= 0; --i) {
      ...
      IVP_BOOL ray_hits_sphere =
          check_ray_against_sphere(&object_center_os, core->upper_limit_radius);
      if (ray_hits_sphere) {
...
void IVP_Ray_Solver_Group::check_ray_group_against_node(
    IVP_OV_Node *node, IVP_OV_Tree_Manager *ov_tree_man) {
  // Recursive function

  // Check whether ray hits the long range cluster node.
  IVP_U_Float_Point luf_point,
      cube_center_point;  // left upper front / right lower back
  IVP_FLOAT cubesize;
  ov_tree_man->get_luf_coordinates_ws(
      node, &luf_point,
      &cubesize);  // fills vars with left-upper-front corner's coordinates
  IVP_FLOAT csh = cubesize * 0.5f;
  cube_center_point.set(luf_point.k[0] + csh, luf_point.k[1] + csh,
                        luf_point.k[2] + csh);

  IVP_BOOL ray_hits_cube =
      this->check_ray_group_against_cube(&cube_center_point, cubesize);
  if (!ray_hits_cube) return;  // not hit
  ...
  // Check all objects that are contained by this node.
  {
    ...
    for (i = node->elements.len() - 1; i >= 0; --i) {
      ...
      IVP_BOOL ray_hits_sphere = check_ray_group_against_sphere(
          &object_center_os, core->upper_limit_radius);
      if (ray_hits_sphere) {
...
void IVP_Ray_Solver::check_ray_against_all_objects_in_sim(
...
void IVP_Ray_Solver_Group::check_ray_group_against_all_objects_in_sim(
    const IVP_Environment *environment) {
  // Checks wether any object in long range tree is hit by the specified ray.
  // Returns IVP_TRUE and adds the corresponding objects each time they are hit.
  // Returns IVP_FALSE if the object is not hit by the ray.
...
IVP_Ray_Solver_Group::IVP_Ray_Solver_Group(int n_ray_solvers_,
  ...
  for (int j = 0; j < n_ray_solvers; j++) {
    IVP_DOUBLE qdist =
        ray_solvers[j]->ray_start_point.quad_distance_to(&center_ws);
...
void IVP_Ray_Solver_Min::add_hit_object(
    IVP_Real_Object *object, const IVP_Compact_Ledge *compact_ledge,
    const IVP_Compact_Triangle *compact_triangle, IVP_DOUBLE hit_dist,
    IVP_U_Point *hit_sur_vec_os) {
