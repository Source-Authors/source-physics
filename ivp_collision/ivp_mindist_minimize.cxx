...
#include <ivp_physics.hxx>
    ...
    (defined(__MWERKS__) && __POWERPC__)
#include <alloca.h>
#endif
...
IVP_BOOL IVP_Mindist_Minimize_Solver::check_loop_hash(
    IVP_SYNAPSE_POLYGON_STATUS i_s0, const IVP_Compact_Edge *i_e0,
...
void IVP_Mindist_Minimize_Solver::init_loop_hash() {
  P_DELETE(loop_hash);
  loop_hash =
      new IVP_Hash(IVP_LOOP_HASH_SIZE /*size*/, sizeof(IVP_Loop_Key_Struct));
}
#else
void IVP_Mindist_Minimize_Solver::init_loop_hash() {}
...
void IVP_Mindist::mindist_rescue_push() {
  ...
  IVP_Anomaly_Manager *anomaly_manager =
      obj0->get_environment()->get_anomaly_manager();
  anomaly_manager->inter_penetration(
      this, obj0, obj1,
      anomaly_manager->get_push_speed_penetration(obj0, obj1));
}
...
IVP_MRC_TYPE IVP_Mindist::recalc_mindist() {
  ...
  while (1) {
    ...
    switch (res) {
      case IVP_MRC_BACKSIDE: {
#ifdef IVP_MINDIST_BEHAVIOUR_DEBUG
        IVP_ASSERT(this->detect_collision(
                       synapse[0]->to_poly()->get_ivp_polygon()->tetras,
                       synapse[1]->to_poly()->get_ivp_polygon()->tetras) ==
                   IVP_FALSE);
#endif
        // find best triangle on opposite of convex object
        mms.pierce_mindist();
      ...
      case IVP_MRC_ENDLESS_LOOP: {
        ...
        IVP_IF(1) {
          ...
          if (!name1) name1 = "(null)";
          ivp_message(
              "recalc_mindist: Endless loop. No collision or termination "
              "between '%s' and '%s'.\n",
              name0, name1);
        }
        // MINDIST RESCUE PUSH
...
IVP_MRC_TYPE IVP_Mindist_Minimize_Solver::p_minimize_FF(
  ...
  // check all 9 point point combinations first:
  {
    ...
    for (pA = A, a = 0; a < 3; pA = pA->get_next(), a++) {
      for (pB = B, b = 0; b < 3; pB = pB->get_next(), b++) {
        IVP_U_Point pos_B_ws;
        IVP_CLS.give_world_coords_AT(pB, m_cache_B, &pos_B_ws);
        IVP_U_Point pos_A_ws;
        IVP_CLS.give_world_coords_AT(pA, m_cache_A, &pos_A_ws);
  ...
  IVP_Cache_Ledge_Point *cc_A[2];
  cc_A[0] = m_cache_A;
  cc_A[1] = m_cache_B;
  IVP_Cache_Ledge_Point *cc_B[2];
  cc_B[0] = m_cache_B;
  cc_B[1] = m_cache_A;
  const IVP_Compact_Edge *tabA[2];
  tabA[0] = A;
  tabA[1] = B;
  const IVP_Compact_Edge *tabB[2];
  tabB[0] = B;
  tabB[1] = A;
  {
    for (int i = 0; i < 2; i++) {
      ...
      for (pA = tabA[i], a = 0; a < 3; pA = pA->get_next(), a++) {
        ...
        IVP_CLS.calc_unscaled_qr_vals_F_space(cc_B[i]->get_compact_ledge(), pB,
                                              &pA_Bos, &qr);
        if (!qr.is_outside()) {
    ...
    for (i = 0; i < 2; i++) {
      ...
      for (pB = tabB[i], b = 0; b < 3; pB = pB->get_next(), b++) {
        ...
        for (pA = tabA[i], a = 0; a < 3; pA = pA->get_next(), a++) {
          ...
          IVP_CLS.calc_unscaled_s_val_K_space(cc_A[i]->get_compact_ledge(), pA,
                                              &pB_Aos, &sr);
          if (sr.is_outside()) continue;
          IVP_DOUBLE qdist = IVP_CLS.quad_dist_edge_to_point_K_space(
              cc_A[i]->get_compact_ledge(), pA, &pB_Aos);
          if (qdist * (1 + P_DOUBLE_RES) < min_qdist) {
  ...
  if ((syn1->get_status() == IVP_ST_POINT) &&
      (syn0->get_status() != IVP_ST_POINT)) {
  ...
  switch (syn0->get_status()) {
    case IVP_ST_POINT: {
      switch (syn1->get_status()) {
        ...
        default:
          ivp_message(
              "IVP_Mindist_Minimize_Solver::p_minimize_FF failed(%s:%d)\n",
              __FILE__, __LINE__);
      }
      break;
    };
...
IVP_MRC_TYPE IVP_Mindist_Minimize_Solver::minimize_default_poly_poly(
  ...
  IVP_IF(ivp_check_debug_mindist(mms->mindist)) {
    ivp_message("%32s statii: %i:%i len %f\n", " '' ", syn0->get_status(),
                syn1->get_status(), mms->mindist->get_length());
  }

  return ret_val;
}

IVP_MRC_TYPE IVP_Mindist_Minimize_Solver::minimize_PB(
    IVP_Mindist_Minimize_Solver *mms) {
  CORE;
  mms->swap_synapses();
...
IVP_MRC_TYPE IVP_Mindist_Minimize_Solver::minimize_B_POLY(
    IVP_Mindist_Minimize_Solver *mms) {
...
IVP_MRC_TYPE IVP_Mindist_Minimize_Solver::minimize_KB(
    IVP_Mindist_Minimize_Solver *mms) {
  CORE;
  mms->swap_synapses();
  return minimize_B_POLY(mms);
...
IVP_MRC_TYPE IVP_Mindist_Minimize_Solver::minimize_FB(
    IVP_Mindist_Minimize_Solver *mms) {
  CORE;
  mms->swap_synapses();
  return minimize_B_POLY(mms);
...
IVP_MRC_TYPE IVP_Mindist_Minimize_Solver::minimize_swapped_poly_poly(
    IVP_Mindist_Minimize_Solver *mms) {
  mms->swap_synapses();
  return minimize_default_poly_poly(mms);
}

IVP_MRC_TYPE IVP_Mindist_Minimize_Solver::minimize_BB(
    IVP_Mindist_Minimize_Solver *mms) {
  ...
  IVP_DOUBLE inv_len;
  if (IVP_Inline_Math::fabsd(qlen) >
      P_DOUBLE_EPS) {  // used to be fabs, which was a sml call
  ...
  mms->mindist->contact_dot_diff_center =
      diff_center.dot_product(&mms->mindist->contact_plane);
#endif
...
const IVP_Compact_Edge *IVP_Compact_Ledge_Solver::minimize_on_other_side(
    ...
    const IVP_U_Point *partner_os) {  // manage pierce count
  ...
  memset(pierce_visited_array, 0,
         n_triangles);  // @@@OG could be optimized, depending on how often
                        // piercing takes place

  int pierce_idx = edge->get_triangle()->get_pierce_index();
  const IVP_Compact_Triangle *pierced_tri =
      &c_ledge->get_first_triangle()[pierce_idx];

  const IVP_Compact_Edge *F =
      pierced_tri->get_first_edge();  // precalculated piercing

  while (1) {
    pierce_visited_array[F->get_triangle()->get_tri_index()] =
        1;  // tag triangle as visited
...
IVP_MRC_TYPE IVP_Mindist_Minimize_Solver::minimize_illegal(
...
IVP_MRC_TYPE (
    *IVP_Mindist_Minimize_Solver::mms_function_table[IVP_ST_MAX_LEGAL]
                                                    [IVP_ST_MAX_LEGAL])(
    IVP_Mindist_Minimize_Solver *mms);
...
void IVP_Mindist_Minimize_Solver::init_mms_function_table() {
  ...
  mms_function_table[IVP_ST_POINT][IVP_ST_TRIANGLE] =
      minimize_default_poly_poly;
  ...
  mms_function_table[IVP_ST_TRIANGLE][IVP_ST_POINT] =
      minimize_swapped_poly_poly;
  mms_function_table[IVP_ST_TRIANGLE][IVP_ST_EDGE] = minimize_illegal;
  mms_function_table[IVP_ST_TRIANGLE][IVP_ST_TRIANGLE] =
      minimize_default_poly_poly;
