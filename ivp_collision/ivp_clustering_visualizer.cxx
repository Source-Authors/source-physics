...
void IVP_Clustering_Visualizer_Shortrange::add_node_to_pipeline(
    const IVP_Compact_Ledgetree_Node *node, IVP_BOOL reported_ledge) {
  int x;
  for (x = 0; x < this->pipelined_private_property_data->ledgetree_nodes.len();
       x++) {
    IVP_Clustering_Visualizer_Shortrange_Nodedata *nodedata =
        this->pipelined_private_property_data->ledgetree_nodes.element_at(x);
    if (nodedata->node == node) {
      nodedata->reported_ledge =
          (IVP_BOOL)((int)nodedata->reported_ledge || (int)reported_ledge);
      if (node->is_terminal()) {
  ...
  IVP_Clustering_Visualizer_Shortrange_Nodedata *new_nodedata =
      new IVP_Clustering_Visualizer_Shortrange_Nodedata();
  new_nodedata->node = (IVP_Compact_Ledgetree_Node *)node;
  if (node->is_terminal())
    new_nodedata->n_touches = 1;
  else
    new_nodedata->n_touches =
        0;  // no reference counting for non-terminal nodes!
...
IVP_BOOL IVP_Clustering_Visualizer_Shortrange::recursively_traverse_cluster(
    const IVP_Compact_Ledgetree_Node *node) {
  // returns: IVP_TRUE, whenever node has been aborted (this includes terminal
  // nodes!)
  ...
  distance_between_centers.subtract(
      &center, &this->intruder_position_in_private_property_cs);

  {
    ...
    if (distance_between_centers.quad_length() >= sum_of_radii * sum_of_radii)
      goto abort;
  }
...

  {
    ...
    if (IVP_Inline_Math::fabsd(distance_between_centers.k[0]) >=
        (node->box_sizes[0] * work + this->intruder_radius))
      goto abort;
    if (IVP_Inline_Math::fabsd(distance_between_centers.k[1]) >=
        (node->box_sizes[1] * work + this->intruder_radius))
      goto abort;
    if (IVP_Inline_Math::fabsd(distance_between_centers.k[2]) >=
        (node->box_sizes[2] * work + this->intruder_radius))
      goto abort;
  }
...
abort:
  if (this->boxmode !=
      TERMINALS_ONLY) {  // display for ABORTED_BOXES and TRAVERSED_BOXES
    this->add_node_to_pipeline(node, IVP_FALSE);
  }
  return (IVP_TRUE);
}
...
void IVP_Clustering_Visualizer_Shortrange::visualize_collisions() {
  ...
  for (i = 0; i < this->output_callbacks.len(); i++) {
    IVP_Clustering_Visualizer_Shortrange_Callback *callback =
        this->output_callbacks.element_at(i);

    int x;
    for (x = this->pipelined_objects.len() - 1; x >= 0; x--) {
      IVP_Clustering_Visualizer_Shortrange_Objectdata *objectdata =
          this->pipelined_objects.element_at(x);

      int y;
      for (y = objectdata->ledgetree_nodes.len() - 1; y >= 0; y--) {
        IVP_Clustering_Visualizer_Shortrange_Nodedata *nodedata =
            objectdata->ledgetree_nodes.element_at(y);
        IVP_Compact_Ledgetree_Node *node = nodedata->node;

        callback->private_property =
            objectdata->real_object;  // this->private_property;
        callback->node = node;
...
void IVP_Clustering_Visualizer_Shortrange::analyze_collision(
    IVP_Real_Object *object0, IVP_DOUBLE radius0, IVP_Real_Object *object1,
    IVP_DOUBLE radius1) {
  if (!this->enabled) return;
  ...
  for (x = 0; x < 2; x++) {
    ...
    if (this->use_intruder_accept_filter &&
        !this->accept_filter_intruder->find(this->intruder))
      continue;  // only process intruder objects that are mentioned in the
                 // filter!
    if (this->use_intruder_reject_filter &&
        this->reject_filter_intruder->find(this->intruder))
      continue;  // only process intruder objects that are mentioned in the
                 // filter!
    if (this->use_private_property_accept_filter &&
        !this->accept_filter_private_property->find(this->private_property))
      continue;  // only process private_property objects that are mentioned in
                 // the filter!
    if (this->use_private_property_reject_filter &&
        this->reject_filter_private_property->find(this->private_property))
      continue;  // only process private_property objects that are mentioned in
                 // the filter!

    IVP_SurfaceManager_Polygon *surface_manager =
        (IVP_SurfaceManager_Polygon *)private_property
            ->get_surface_manager();  // because of above verification we KNOW
                                      // that object is of type 'polygon'
    const IVP_Compact_Surface *compact_surface =
        surface_manager->get_compact_surface();
    const IVP_Compact_Ledgetree_Node *private_property_ledgetree_rootnode =
    ...
    object_cache->transform_position_to_object_coords(
        &intruder_matrix.vv, &this->intruder_position_in_private_property_cs);
    object_cache->remove_reference();

    this->pipelined_private_property_data = NULL;
    for (int y = 0; y < this->pipelined_objects.len(); y++) {
      IVP_Clustering_Visualizer_Shortrange_Objectdata *objectdata =
          this->pipelined_objects.element_at(y);
    ...
    if (!this->pipelined_private_property_data) {
      this->pipelined_private_property_data =
          new IVP_Clustering_Visualizer_Shortrange_Objectdata();
      this->pipelined_private_property_data->real_object =
    ...
    // ivp_global_clustering_visualizer.longrange.add_object(this->intruder,
    // &center, this->intruder_radius);
    // *** temporary hack !!!
  }
...
void IVP_Clustering_Visualizer_Shortrange::add_object_to_intruder_filter(
    IVP_Real_Object *obj, IVP_CLUSTERING_VISUALIZER_FILTER filter) {
  switch (filter) {
    case ACCEPT: {
...
void IVP_Clustering_Visualizer_Shortrange::remove_object_from_intruder_filter(
    IVP_Real_Object *obj, IVP_CLUSTERING_VISUALIZER_FILTER filter) {
  switch (filter) {
    case ACCEPT: {
...
void IVP_Clustering_Visualizer_Shortrange::
    add_object_to_private_property_filter(
        IVP_Real_Object *obj, IVP_CLUSTERING_VISUALIZER_FILTER filter) {
  switch (filter) {
    case ACCEPT: {
      if (!this->accept_filter_private_property->find(
              obj)) {  // pair already stored?
        this->accept_filter_private_property->add(
            obj);  // no, then store now...
      }
    } break;
    case REJECT: {
      if (!this->reject_filter_private_property->find(
              obj)) {  // pair already stored?
        this->reject_filter_private_property->add(
            obj);  // no, then store now...
      }
    } break;
  }
...
void IVP_Clustering_Visualizer_Shortrange::
    remove_object_from_private_property_filter(
        IVP_Real_Object *obj, IVP_CLUSTERING_VISUALIZER_FILTER filter) {
  switch (filter) {
    case ACCEPT: {
      if (this->accept_filter_private_property->find(
              obj)) {  // pair stored in hash?
...
void IVP_Clustering_Visualizer_Shortrange::install_visualize_callback(
    IVP_Clustering_Visualizer_Shortrange_Callback *callback) {
  this->output_callbacks.add(callback);
  return;
}

void IVP_Clustering_Visualizer_Shortrange::remove_visualize_callback(
    IVP_Clustering_Visualizer_Shortrange_Callback *callback) {
...
IVP_Clustering_Visualizer_Shortrange::IVP_Clustering_Visualizer_Shortrange() {
  ...
  this->accept_filter_private_property =
      new IVP_Clustering_Visualizer_Object_Hash(64);
  this->reject_filter_private_property =
      new IVP_Clustering_Visualizer_Object_Hash(64);
  ...
  this->boxmode =
      TRAVERSED_BOXES;  // ABORTED_BOXES TERMINALS_ONLY TRAVERSED_BOXES
...
IVP_Clustering_Visualizer_Longrange_Callback::
    ~IVP_Clustering_Visualizer_Longrange_Callback() {
  return;
}
...
IVP_BOOL IVP_Clustering_Visualizer_Longrange_Hash::compare(
    const void *elem0, const void *elem1) const {
...
int IVP_Clustering_Visualizer_Longrange_Hash::obj_to_index(
    IVP_Real_Object *obj) {
...
IVP_Real_Object *IVP_Clustering_Visualizer_Longrange_Hash::remove(
    IVP_Real_Object *obj) {
  return (IVP_Real_Object *)remove_elem(obj, obj_to_index(obj));
}

IVP_Real_Object *IVP_Clustering_Visualizer_Longrange_Hash::find(
    IVP_Real_Object *obj) {
...
void IVP_Clustering_Visualizer_Longrange::visualize() {
  ...
  for (x = this->visualize_data.len() - 1; x >= 0; x--) {
    IVP_Clustering_Visualizer_Longrange_Data *data =
        this->visualize_data.element_at(x);
    IVP_Real_Object *real_object = data->real_object;

    if (real_object->get_core()->physical_unmoveable &&
        !this->visualize_unmoveable_objects)
      return;
    if (!real_object->get_core()->physical_unmoveable &&
        !this->visualize_moveable_objects)
      return;
    ...
    for (i = 0; i < this->graphics_callbacks.len(); i++) {
      IVP_Clustering_Visualizer_Longrange_Callback *callback =
          this->graphics_callbacks.element_at(i);
...
void IVP_Clustering_Visualizer_Longrange::remove_object(
    IVP_Real_Object *real_object) {
  if (!this->enabled) return;
...
void IVP_Clustering_Visualizer_Longrange::add_object(
    IVP_Real_Object *real_object, IVP_U_Float_Point *center,
    IVP_DOUBLE radius) {
  IVP_Clustering_Visualizer_Longrange_Data *data =
      new IVP_Clustering_Visualizer_Longrange_Data();
...
void IVP_Clustering_Visualizer_Longrange::remove_objectdata(
    IVP_Real_Object *real_object) {
  int x;
...
void IVP_Clustering_Visualizer_Longrange::install_visualize_callback(
    IVP_Clustering_Visualizer_Longrange_Callback *callback) {
  this->graphics_callbacks.add(callback);
  return;
}

void IVP_Clustering_Visualizer_Longrange::remove_visualize_callback(
    IVP_Clustering_Visualizer_Longrange_Callback *callback) {
...
void IVP_Clustering_Visualizer_Longrange::add_object_to_filter(
    IVP_Real_Object *obj, IVP_CLUSTERING_VISUALIZER_FILTER filter) {
  switch (filter) {
    case ACCEPT: {
...
void IVP_Clustering_Visualizer_Longrange::remove_object_from_filter(
    IVP_Real_Object *obj, IVP_CLUSTERING_VISUALIZER_FILTER filter) {
  switch (filter) {
    case ACCEPT: {
