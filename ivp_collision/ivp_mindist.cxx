...
void IVP_Mindist_Settings::set_collision_tolerance(IVP_DOUBLE t) {
  ...
  for (int i = IVP_MAX_STEPS_FOR_COLLDIST_DECREASE - 1; i >= 0; i--) {
    coll_dists[i] =
        min_coll_dists + (i / IVP_DOUBLE(IVP_MAX_STEPS_FOR_COLLDIST_DECREASE)) *
                             (minimum_friction_dist - min_coll_dists);
  }
  friction_dist = minimum_friction_dist + 1.0f * t;

  keeper_dist = friction_dist + 0.3f * t;

  speed_after_keeper_dist = IVP_Inline_Math::ivp_sqrtf(
      2.0f * (keeper_dist - min_coll_dists) *
      9.81f);  // speed when falling down from keeper dist to coll dist
  distance_keepers_safety =
      0.01f *
      t;  // safety gap, when surpassed, mindist doesnt appear in complex
...
void IVP_Mindist_Settings::set_event_queue_min_delta_time_base(
    IVP_DOUBLE base) {
...
IVP_Mindist_Base::IVP_Mindist_Base(IVP_Collision_Delegator *del)
    : IVP_Collision(del) {
  coll_type = IVP_COLL_NONE;
  synapse_sort_flag = 0;
  recalc_result = IVP_MDRR_OK;
  is_in_phantom_set =
      IVP_FALSE;  // mindist is in phantom set_of_mindists already
...
IVP_Mindist::IVP_Mindist(IVP_Environment *my_env, IVP_Collision_Delegator *del)
    : IVP_Mindist_Base(del) {
  IVP_ASSERT(del);
  recalc_time_stamp = 0;
  next = 0;
  prev = 0;
...
IVP_Mindist::~IVP_Mindist() {
  ...
  if (g_pCurrentMindist == this) {
    IVP_Collision_Filter *coll_filter =
        get_synapse(0)->l_obj->get_environment()->get_collision_filter();
    coll_filter->check_objects_for_collision_detection(get_synapse(0)->l_obj,
                                                       get_synapse(1)->l_obj);
  }
#endif
  IVP_ASSERT(g_pCurrentMindist != this);
  IVP_Environment *l_environment =
      get_mindist_synapse(0)->get_object()->get_environment();
  ...
  // dimhotepus: remove reference only for polygons as per
  // IVP_Mindist::init_mindist
  IVP_Real_Object *pop0 = md->get_synapse(0)->get_object();
  if (pop0->get_type() == IVP_POLYGON)
    pop0->get_surface_manager()->remove_reference_to_ledge(
        md->get_synapse(0)->get_ledge());

  IVP_Real_Object *pop1 = md->get_synapse(1)->get_object();
  if (pop1->get_type() == IVP_POLYGON)
    pop1->get_surface_manager()->remove_reference_to_ledge(
        md->get_synapse(1)->get_ledge());
...
void IVP_Synapse_Real::check_consistency_of_ledge(
    const IVP_Compact_Edge * /*test_edge*/) const {
  return;
#if 0
...
void IVP_Synapse_Real::print() {
  ...
  // dimhotepus: Correctly print type.
  ivp_message("%s.\n", stat_text);
}
#endif
...
void IVP_Mindist::init_mindist(IVP_Real_Object *pop0, IVP_Real_Object *pop1,
  ...
  switch (pop1->get_type()) {
    case IVP_POLYGON:
      ...
      pop1->get_surface_manager()->add_reference_to_ledge(
          e1->get_compact_ledge());
      break;
    case IVP_BALL:
      if (pop0->get_type() == IVP_BALL) {
        if (pop0->client_data < pop1->client_data) {
          IVP_Synapse_Real *synh;
          synh = syn0;
          syn0 = syn1;
          syn1 = synh;
        }
  ...
  switch (pop0->get_type()) {
    case IVP_POLYGON:
      ...
      pop0->get_surface_manager()->add_reference_to_ledge(
          e0->get_compact_ledge());
      break;
    case IVP_BALL:
  ...
  if (pop0->get_controller_phantom() || pop1->get_controller_phantom()) {
    ...
    mm->insert_and_recalc_exact_mindist(
        this);  // #+# insert only (data cache !!)
  }
}
...
class IVP_MM_CMP {
 public:
  ...
  static inline hk_intp calc_hash_index(IVP_Collision *c,
                                        IVP_MM_CMP_Key * /*ref_key*/) {
    const IVP_Compact_Ledge *ledge[2];
    c->get_ledges(ledge);
 ...
 *	Description:	Insert mindists, between all pairs of ledges of two
 *objects scan_radius:	the radius, the surface_manager is searched mindists:
 *an optional hash of mindists, which is checked and updated returns:	the last
 *collision created
 ********************************************************************************/
void IVP_Mindist_Manager::create_exact_mindists(
    IVP_Real_Object *pop0, IVP_Real_Object *pop1, IVP_DOUBLE scan_radius,
  ...
  if (!single_ledge0) {
    ...
    IVP_U_Point pos1;
    core1->inline_calc_at_position(env->get_current_time(), &pos1);
    search_radius_0 =
        core1->upper_limit_radius + pop0->get_extra_radius() + scan_radius;
    IVP_Cache_Object *cache0 = pop0->get_cache_object_no_lock();
    cache0->m_world_f_object.vimult4(&pos1, &visitor_position_os0);
    pop0->get_surface_manager()->get_all_ledges_within_radius(
        &visitor_position_os0, search_radius_0, root_ledge0, NULL,
        single_ledge1, &ledges0);
  } else {
  ...
  if (!single_ledge1) {
    ...
    IVP_U_Point pos0;
    core0->inline_calc_at_position(env->get_current_time(), &pos0);
    search_radius_1 =
        core0->upper_limit_radius + pop1->get_extra_radius() + scan_radius;
    IVP_Cache_Object *cache1 = pop1->get_cache_object_no_lock();
    cache1->m_world_f_object.vimult4(&pos0, &visitor_position_os1);
    pop1->get_surface_manager()->get_all_ledges_within_radius(
        &visitor_position_os1, search_radius_1, root_ledge1, NULL,
        single_ledge0, &ledges1);
  } else {
  ...
  // IVP_SUPREME_SUPPORT
  ivp_global_clustering_visualizer.shortrange.analyze_collision(
      pop0, search_radius_1, pop1, search_radius_0);
#endif
  ...
  IVP_Diff_Hash<IVP_MM_CMP, IVP_U_FVector<IVP_Collision>, IVP_Collision,
                IVP_MM_CMP_Key>
      diff_hash(mindists, a_buffer, buffer_size, NULL);
  ...
  for (int i = ledges0.len() - 1; i >= 0; i--) {
    ...
    for (int j = ledges1.len() - 1; j >= 0; j--) {
      ...
      new_mindist->init_mindist(pop0, pop1, tri0->get_first_edge(),
                                tri1->get_first_edge());
      IVP_ASSERT(new_collisions.n_elems <= new_collisions.memsize);
...
void IVP_Mindist_Manager::insert_exact_mindist(IVP_Mindist *new_mindist) {
  ...
  if ((syn0->get_core()->car_wheel || syn1->get_core()->car_wheel)) {
    if (new_mindist->mindist_function != IVP_MF_PHANTOM) {
      ...
      // syn0->get_object()->get_environment()->get_current_time().get_seconds());
...
void IVP_Mindist_Manager::insert_and_recalc_exact_mindist(
    IVP_Mindist *new_mindist) {
  IVP_ASSERT(new_mindist->mindist_status == IVP_MD_UNINITIALIZED);
  ...
  // TL : when obj is revived it gets all mindists and builds friction system ->
  // do not move mindist into hull
  ...
  if (new_mindist->recalc_result == IVP_MDRR_OK) {
    ...
    allow_hull_bit =
        (IVP_Movement_Type)((core0->movement_state) | (core1->movement_state));
...
void IVP_Mindist_Manager::insert_and_recalc_phantom_mindist(
    IVP_Mindist *new_mindist) {
  IVP_ASSERT(new_mindist->mindist_status == IVP_MD_UNINITIALIZED);
  IVP_ASSERT(new_mindist->mindist_function == IVP_MF_PHANTOM);
...
class IVP_OO_CMP {
 public:
  ...
  static inline hk_intp calc_hash_index(IVP_Collision *c,
    ...
    hk_intp x = hk_intp(objects[0]) ^ hk_intp(objects[1]) ^
                hk_intp(con);  // take other object (trick to avoid if)
...
class IVP_Vector_of_OV_Elements_128
    : public IVP_U_Vector<class IVP_OV_Element> {
  void *elem_buffer[128];
...
void IVP_Mindist_Manager::recheck_ov_element(IVP_Real_Object *object) {
  ...
  if (!elem) return;  // not collision enabled
  ...
  if (!scanning_universe) {  // check for new objects in the universe manager
    IVP_DOUBLE hull_time = environment->range_manager->get_coll_range_in_world(
        object);  // distance to check
    ...
    if (um) {
      ...
      if (IVP_MTIS_CAN_COLLIDE(mt)) {
        ...
        um->ensure_objects_in_environment(object, &elem->center,
                                          real_check_sphere);
        scanning_universe = IVP_FALSE;  // finished
      }
    }
    radius = environment->ov_tree_manager->insert_ov_element(
        elem, real_check_sphere, real_check_sphere, &colliding_elements);
    ...
    radius = environment->ov_tree_manager->insert_ov_element(
        elem, real_check_sphere, real_check_sphere, NULL);
    IVP_DOUBLE real_hull_time = P_DOUBLE_EPS;  // recheck as soon as possible
                                               // because it's not checked now
    elem->add_to_hull_manager(hm, real_hull_time);  // insert into event queue
    return;  // thats it, IVP_Universe_Manager can only add objects which do not
             // have collision candidates except object
  }
  ...
  // convert old list into hash
  IVP_Diff_Hash<IVP_OO_CMP, IVP_U_FVector<IVP_Collision>, IVP_Collision,
                IVP_Real_Object>
      diff_hash(&elem->collision_fvector, a_buffer, buffer_size, object);
  ...
  {  // compare new list with old list
    for (int i = colliding_elements.len() - 1; i >= 0; i--) {
      ...
      if (!IVP_MTIS_CAN_COLLIDE(obj0->get_movement_state()) &&
          !IVP_MTIS_CAN_COLLIDE(obj1->get_movement_state())) {
        continue;
      }
      if (obj0->friction_core == obj1->friction_core) continue;
      ...
      if (coll_filter->check_objects_for_collision_detection(obj0, obj1) ==
          IVP_FALSE)
        continue;
      // search for existing
  ...
  {  // delete unused collisions
    for (int x = elem->collision_fvector.len() - 1;
         x >= diff_hash.n_found_objects; x--) {
      IVP_Collision *c = elem->collision_fvector.element_at(x);
      //
      // HACKHACK: E3 2003, get around delete of collision object that is
      // calling us up the stack
      //
  ...
  {  // create new ones
    for (int x = new_objects.len() - 1; x >= 0; x--) {
      ...
      for (int k = environment->collision_delegator_roots.len() - 1; k >= 0;
           k--) {
        IVP_Collision_Delegator_Root *cdr =
            environment->collision_delegator_roots.element_at(k);
...
void IVP_Mindist_Manager::enable_collision_detection_for_object(
...
void IVP_Mindist_Manager::recalc_all_exact_wheel_mindist() {
  for (int i = wheel_look_ahead_mindists.len() - 1; i >= 0; i--) {
    ...
    if (syn1->get_edge() == mdist->last_visited_triangle) continue;

    if ((syn1->get_status() == IVP_ST_TRIANGLE) &&
        (syn0->get_status() == IVP_ST_BALL)) {
      ...
      if (md_len < ivp_mindist_settings.max_distance_for_friction) {
        if (syn1->get_ledge()->is_terminal()) {
          ...
          // environment->get_current_time().get_seconds());
...
void IVP_Mindist::update_exact_mindist_events(
    IVP_BOOL allow_hull_conversion, IVP_MINDIST_EVENT_HINT event_hint) {
  IVP_IFDEBUG(
      IVP_TRUE, IVP_ASSERT(mindist_status == IVP_MD_EXACT);
      ...
      if (dm->file_out_impacts) {
        fprintf(dm->out_deb_file, "doing_mindist_events %zi at %f\n",
                ...
                get_environment()->get_current_time().get_time());
      })
  ...
  {
    mim.sum_max_surface_rot_speed =
        core0->max_surface_rot_speed + core1->max_surface_rot_speed;
    mim.worst_case_speed = mim.sum_max_surface_rot_speed +
                           core0->current_speed + core1->current_speed;
  }
...

  {
    ...
    IVP_DOUBLE hull_check_len =
        coll_dist + env->get_delta_PSI_time() * mim.worst_case_speed * 2.1f;
    ...
    if (get_length() > hull_check_len) {
      if (allow_hull_conversion) {
        ...
        my_manager->insert_hull_mindist(
            this, get_length() -
                      coll_dist);  // secdist is subtracted, because hull
                                   // manager is already checked till next PSI
      }
      return;
    }
  }

  {
    mim.projected_center_speed =
        core1->speed.dot_product(&contact_plane) -
        core0->speed.dot_product(&contact_plane);  // inverse to direction

    IVP_DOUBLE rot0_projected =
        contact_plane.dot_product(&core0->rotation_axis_world_space);
    rot0_projected *= rot0_projected;
    rot0_projected = IVP_Inline_Math::ivp_sqrtf(1.001f - rot0_projected) *
                     core0->max_surface_rot_speed;  // make sure sqrt does not
                                                    // crash -> increase result

    IVP_DOUBLE rot1_projected =
        contact_plane.dot_product(&core1->rotation_axis_world_space);
    rot1_projected *= rot1_projected;
    rot1_projected = IVP_Inline_Math::ivp_sqrtf(1.001f - rot1_projected) *
                     core1->max_surface_rot_speed;
    mim.max_coll_speed =
        rot0_projected + rot1_projected + mim.projected_center_speed;
  ...
  IVP_DOUBLE d_len =
      d_time *
      mim.max_coll_speed;  // max length that the system can move till next PSI
  ...
  if (mim.event_type_out != IVP_COLL_NONE) {
    if (next_event - mim.t_now < P_FLOAT_RES) {  // maybe an error
      if (event_hint == IVP_EH_NOW) {
        ...
        if ((event_hint == IVP_EH_BIG_DELAY) ||
            (mim.event_type_out & 0x0f)) {  //
          if (mdist >= P_DOUBLE_RES) {
            next_event =
                env->get_current_time() + mdist / mim.worst_case_speed +
                (ivp_mindist_settings.event_queue_min_delta_time_base * 1e-4f) *
                    env->get_delta_PSI_time();  //@@CB
          } else {
            next_event =
                env->get_current_time() +
                (ivp_mindist_settings.event_queue_min_delta_time_base * 1e-3f) *
                    env->get_delta_PSI_time();  //@@CB
          }
        } else {
          if (mdist >= P_DOUBLE_RES) {
            next_event =
                env->get_current_time() + 0.1f * mdist / mim.worst_case_speed +
                (ivp_mindist_settings.event_queue_min_delta_time_base * 1e-7f) *
                    env->get_delta_PSI_time();  //@@CB
          } else {
            next_event =
                env->get_current_time() +
                (ivp_mindist_settings.event_queue_min_delta_time_base * 1e-5f) *
                    env->get_delta_PSI_time();  //@@CB
          }
        }

        if (next_event - env->get_next_PSI_time() >= 0.0f) return;
      }
    }
...
void IVP_Synapse::hull_manager_is_reset(
    IVP_FLOAT dt, IVP_FLOAT center_dt) {  // hull_manager is reset
  IVP_Mindist *md = (IVP_Mindist *)this->get_synapse_mindist();
  md->hull_manager_is_reset(dt, center_dt);
}

void IVP_Synapse::hull_limit_exceeded_event(IVP_Hull_Manager *,
                                            IVP_HTIME hull_intrusion_value) {
  IVP_Mindist *md = (IVP_Mindist *)this->get_synapse_mindist();
  md->mindist_hull_limit_exceeded_event(hull_intrusion_value);
}

void IVP_Synapse::hull_manager_is_going_to_be_deleted_event(
    IVP_Hull_Manager *) {
  delete this
      ->get_synapse_mindist();  // dont use P_DELETE, because mindist deletes
                                // synapse and P_DELETE sets pointer to NULL
}

void ivp_optimiztion_center_check_successfull() { ; }

void IVP_Mindist::hull_manager_is_reset(
    IVP_FLOAT dt, IVP_FLOAT center_dt) {  // hull_manager is reset
#ifdef IVP_HALFSPACE_OPTIMIZATION_ENABLED
...
void IVP_Mindist::mindist_hull_limit_exceeded_event(
    IVP_HTIME hull_intrusion_value) {
  ...
  IVP_U_Point center0;
  core0->inline_calc_at_position(current_time, &center0);
  IVP_U_Point center1;
  core1->inline_calc_at_position(current_time, &center1);

  IVP_U_Point diff;
  diff.subtract(&center0, &center1);
  IVP_DOUBLE qdistance = diff.quad_length();

  IVP_DOUBLE speed0 =
      core0->current_speed + core0->max_surface_rot_speed + P_DOUBLE_EPS;  //
  IVP_DOUBLE speed1 =
      core1->current_speed + core1->max_surface_rot_speed + P_DOUBLE_EPS;
  IVP_DOUBLE speed = speed0 + speed1;

  IVP_IF(ivp_check_debug_mindist(this)) {
    ivp_message("%32s center_distance %f old_dist %f\n",
                "hull_limit_exceeded_event",
                IVP_Inline_Math::ivp_sqrtf(qdistance), get_length());
  }
  ...
  if (!disable_halfspace_optimization) {
    ...
    IVP_DOUBLE delta_center_distance =
        contact_dot_diff_center - new_contact_dot_diff_center;

    // calc estimated angular movement
    IVP_DOUBLE current_angular_hull_time_0 =
        m0->get_angular_hull_time(current_time);
    IVP_DOUBLE current_angular_hull_time_1 =
        m1->get_angular_hull_time(current_time);

    IVP_DOUBLE sum_angular =
        current_angular_hull_time_0 + current_angular_hull_time_1;
    IVP_DOUBLE angular_movement = sum_angular - this->sum_angular_hull_time;

    IVP_DOUBLE estimated_distance = this->get_length() - angular_movement -
    ...
    IVP_IF(0 && hk_Math::almost_equal(this->contact_plane.k[1], -1.0f) &&
           !this->mindist_function) {
      ivp_message(
          "sum_angular %f   angular_movement %f    sum_angular_hull_time %f\n",
          sum_angular, angular_movement, sum_angular_hull_time);
    }

    if (estimated_distance > speed *
                                 obj0->get_environment()->get_delta_PSI_time() *
                                 look_ahead_factor) {
      // successfull delay
#ifdef LINUX
      ...
      IVP_IF(ivp_check_debug_mindist(this)) {
        ivp_message(
            "%32s estimated_distance into hull dist: %f error %f speed %f\n",
            "hull_limit_exceeded_event", IVP_Inline_Math::ivp_sqrtf(qdistance),
            estimated_distance, speed);
      }
  ...
  if (this->mindist_function !=
      IVP_MF_PHANTOM) {  // #+# just insert in list and wait for delayed update
...
void IVP_Mindist::create_cp_in_advance_pretension(IVP_Real_Object *robject,
                                                  float gap_len) {
  ...
  if (old_fr_syn) {
    ...
    IVP_Contact_Point *new_cp = try_to_generate_managed_friction(
        &associated_fs, &success, sim_unit_not_destroy, call_recalc_svals);

    if (success == IVP_TRUE) {
      IVP_Contact_Point *old_cp = old_fr_syn->get_contact_point();
      IVP_Time old_time = old_cp->last_time_of_recalc_friction_s_vals;
      old_cp->last_time_of_recalc_friction_s_vals =
          robject->environment->get_current_time();
      ...
      world_friction_vec.set_multiple(
          &old_cp->tmp_contact_info->span_friction_v[0],
          old_cp->span_friction_s[0]);
      world_friction_vec.add_multiple(
          &old_cp->tmp_contact_info->span_friction_v[1],
          old_cp->span_friction_s[1]);

      float s1, s2;
      s1 = new_cp->tmp_contact_info->span_friction_v[0].dot_product(
          &world_friction_vec);
      s2 = new_cp->tmp_contact_info->span_friction_v[1].dot_product(
          &world_friction_vec);
...
void IVP_Mindist::simulate_time_event(IVP_Environment *env) {
  ...
  IVP_IF(ivp_check_debug_mindist(l_mindist)) {
    ...
    ivp_message("%32s COLL_TYPE %X: old_len=%g\n", "Time_Event_Mindist_Coll",
                l_mindist->coll_type, md_len);
  }
  ...
  if (l_mindist->recalc_result == IVP_MDRR_OK) {
    if ((l_mindist->coll_type & 0x0f) ==
        0) {  // real collision might have happened
      IVP_ASSERT(l_mindist->mindist_status == IVP_MD_EXACT);

      IVP_FLOAT md_len = l_mindist->get_length();
      IVP_FLOAT sec_dist = this->get_coll_dist() +
                           ivp_mindist_settings.mindist_change_force_dist;
      if (md_len < sec_dist) {
        // mindist shorter than this -> impact
        // ivp_message("do impact %x at %f\n", this,
        // this->get_synapse(0)->get_object()->get_environment()->get_current_time().get_seconds());

        l_mindist->do_impact();
      } else {
        l_mindist->update_exact_mindist_events(
            IVP_FALSE,
            IVP_EH_BIG_DELAY);  // recalculate mindist, no hull conversion
      }
    } else {
      l_mindist->update_exact_mindist_events(
          IVP_FALSE,
          IVP_EH_SMALL_DELAY);  // recalculate mindist, no hull conversion
    }
  }
...
//check ball / triangle
          if(
(syn2->get_status()==IVP_ST_TRIANGLE)&&(syn1->get_status()==IVP_ST_BALL) ) { if(
syn1->l_obj->get_core()->car_wheel ) {
...
void IVP_Mindist_Manager::remove_exact_mindist(IVP_Mindist *del_mindist) {
  ...
  IVP_ASSERT(del_mindist->mindist_status ==
             IVP_MD_EXACT);  // requires friction mode
  IVP_IF(1) { del_mindist->mindist_status = IVP_MD_UNINITIALIZED; }
  ...
  // environment->get_current_time().get_seconds(), i);
  if (i >= 0) {
    wheel_look_ahead_mindists.remove_at_and_allow_resort(i);
  }
}
...
void IVP_Mindist_Manager::remove_invalid_mindist(IVP_Mindist *del_mindist) {
  IVP_ASSERT(del_mindist->mindist_status ==
             IVP_MD_INVALID);  // requires friction mode
...
void IVP_Mindist_Manager::remove_hull_mindist(IVP_Mindist *del_mindist) {
  IVP_ASSERT(del_mindist->mindist_status == IVP_MD_HULL ||
             del_mindist->mindist_status == IVP_MD_HULL_RECURSIVE);
...
void IVP_Mindist_Manager::insert_hull_mindist(IVP_Mindist *md,
                                              IVP_HTIME hull_time0,
                                              IVP_HTIME hull_time1) {
...
void IVP_Mindist_Manager::insert_hull_mindist(IVP_Mindist *md,
  ...
  if (!IVP_MTIS_IS_MOVING(obj0->get_movement_state())) {
    ...
    IVP_FLOAT speed0 =
        core0->max_surface_rot_speed + core0->current_speed + P_FLOAT_EPS;
    IVP_FLOAT speed1 =
        core1->max_surface_rot_speed + core1->current_speed + P_FLOAT_EPS;
...
void IVP_Mindist_Manager::insert_lazy_hull_mindist(IVP_Mindist *md,
                                                   IVP_HTIME hull_time0,
                                                   IVP_HTIME hull_time1) {
  IVP_ASSERT(md->mindist_status == IVP_MD_UNINITIALIZED);
  md->mindist_status = IVP_MD_HULL;
...
void IVP_Mindist_Manager::insert_lazy_hull_mindist(IVP_Mindist *md,
  ...
  if (!IVP_MTIS_IS_MOVING(obj0->get_movement_state())) {
    ...
    IVP_IF(ivp_check_debug_mindist(md)) {
      ivp_message("%32s into hull, htimes %f: %f\n", "insert_lazy_hull_mindist",
                  factor * speed_corr_0, factor * speed_corr_1);
    }
  }
}
...
void IVP_Mindist_Manager::mindist_left_phantom(IVP_Mindist *mdist) {
  if (mdist->is_in_phantom_set) {
    ...
    mdist->is_in_phantom_set = IVP_FALSE;
    {
      ...
      IVP_Controller_Phantom *cp0 =
          syn0->get_object()->get_controller_phantom();
      if (cp0) cp0->mindist_left_volume(mdist);
    }
    {
      IVP_Synapse_Real *syn1 = mdist->get_synapse(1);
      IVP_Controller_Phantom *cp1 =
          syn1->get_object()->get_controller_phantom();
