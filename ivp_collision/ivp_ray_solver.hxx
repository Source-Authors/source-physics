...
class IVP_Ray_Hit {
 ...
 public:
  IVP_U_Float_Point
      hit_surface_direction_os;  // not valid for ball objects! For balls, you
                                 // can easily compute this direction at a later
                                 // point of time.
...
class IVP_Ray_Solver_Os {
 protected:
  ...
  inline IVP_BOOL check_ray_against_sphere_os(
...
class IVP_Ray_Solver : public IVP_Ray_Hit_Listener {
 public:
  ...
  void check_ray_against_compact_ledge_os(
      const IVP_Compact_Ledge *compact_ledge_to_compare,
      IVP_Real_Object *object);
  inline IVP_BOOL check_ray_against_sphere(
 ...
 public:
  ...
  void check_ray_against_node(IVP_OV_Node *node,
  ...
  IVP_BOOL check_ray_against_cube(const IVP_U_Float_Point *luf_point,
                                  const IVP_U_Float_Point *rlb_point);
  IVP_BOOL check_ray_against_square(IVP_FLOAT pos_dist, IVP_FLOAT pos_axis_len,
...
class IVP_Ray_Solver_Group {
  ...
  inline IVP_BOOL check_ray_group_against_sphere(
      const IVP_U_Float_Point *sphere_center_ws, IVP_FLOAT sphere_radius);
 ...
 public:
  ...
  IVP_BOOL check_ray_group_against_cube(const IVP_U_Float_Point *cube_center_ws,
                                        IVP_FLOAT cube_size);

  void check_ray_group_against_node(IVP_OV_Node *node,
                                    IVP_OV_Tree_Manager *ov_tree_man);
...
class IVP_Ray_Solver_Min_Hash : public IVP_Ray_Solver {
 private:
  ...
  void add_hit_object(IVP_Real_Object *object,
                      ...
                      const IVP_Compact_Triangle *compact_triangle,
                      IVP_DOUBLE hit_dist,
                      IVP_U_Point *hit_sur_vec_os) override;

 public:
  IVP_U_Min_Hash *get_result_min_hash() { return &output_min_hash; }

  IVP_Ray_Solver_Min_Hash(const IVP_Ray_Solver_Template *templ)
      : IVP_Ray_Solver(templ), output_min_hash(8) {
...
class IVP_Ray_Solver_Min : public IVP_Ray_Solver {
 ...
 public:
  ...
  IVP_Ray_Solver_Min(const IVP_Ray_Solver_Template *templ)
      : IVP_Ray_Solver(templ), ray_hit{} {  // inits class members
