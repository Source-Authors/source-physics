...
IVP_BOOL ivp_check_debug_mindist(IVP_Mindist *md) {
  ...
  if (!IVP_DEBUG_OBJECT1) return IVP_FALSE; //-V649 //-V779
  ...
  if ((!P_String::string_cmp(name0, search0, IVP_FALSE) &&
       !P_String::string_cmp(name1, search1, IVP_FALSE)) ||
      (!P_String::string_cmp(name0, search1, IVP_FALSE) &&
       !P_String::string_cmp(name1, search0, IVP_FALSE))) {
    const IVP_Time &time =
        md->get_synapse(0)->l_obj->get_environment()->get_current_time();
    if (time - IVP_DEBUG_TIME >= 0.0f) {
      ivp_message("%10s %10s %8f-%X: ", name0, name1, time.get_time(),
                  (int)((hk_intp)md & 0xffff));
      return IVP_TRUE;
    }
  }
  return IVP_FALSE;
...
IVP_DOUBLE IVP_Mindist_Minimize_Solver::p_optimize_FF(
...
IVP_RETURN_TYPE IVP_Mindist_Minimize_Solver::proove_polypoly() {
  ...
  m_cache_0.init_cache_ledge_point(poly_0, ledge0);
  ...
  for (int l0 = ledge0->get_n_triangles() - 1; l0 >= 0; l0--) {
    for (int l1 = ledge1->get_n_triangles() - 1; l1 >= 0; l1--) {
      ...
      IVP_DOUBLE qd = p_optimize_FF(t0->get_first_edge(), t1->get_first_edge(),
                                    &m_cache_0, &m_cache_1, min_qdist);
      if (qd < min_qdist) {
  ...
  //	IVP_DOUBLE qd0 = p_optimize_FF(best_t0->get_first_edge(),
  // best_t1->get_first_edge(), &m_cache_0, &m_cache_1, min_qdist +
  // P_DOUBLE_RES); IVP_DOUBLE qd1 = p_optimize_FF(best_t0->get_first_edge(),
  // best_t1->get_first_edge(), &m_cache_0, &m_cache_1, min_qdist +
  // P_DOUBLE_RES);
...
#ifdef DEBUG
int situation(IVP_Cache_Ledge_Point *tri_cache, IVP_Cache_Ledge_Point *e_cache,
              IVP_Compact_Edge *triangle_e0, IVP_Compact_Edge *edge) {
  ...
  IVP_DOUBLE q = 0, r = 0;
  if (tri_hesse.calc_intersect_with(&edge_straight, &pierce_point_w) ==
      IVP_OK) {
    pierces = IVP_TRUE;
  ...
  IVP_CLS.calc_qr_vals(triangle_e0, &e_0, &q_edge_start, &r_edge_start,
                       tri_cache);
  ...
  if (pierces) {
    ivp_message("pierces %s triangle:  %f %f    q=%f r=%f \n",
                (pierce_inside) ? "inside" : "outside", pierce_point_f.k[0],
                pierce_point_f.k[1], q, r);
  } else {
