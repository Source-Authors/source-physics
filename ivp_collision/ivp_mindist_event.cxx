...
IVP_DOUBLE IVP_3D_Solver_PF_COLL::get_value(IVP_U_Matrix *A_w_f_c,
...
IVP_DOUBLE IVP_3D_Solver_VEC_PARALLEL_AREA::get_value(IVP_U_Matrix *A_w_f_c,
                                                      IVP_U_Matrix *B_w_f_c) {
...
IVP_DOUBLE IVP_3D_Solver_DISTANCE_OF_TWO_POINTS::get_value(
    IVP_U_Matrix *A_w_f_c, IVP_U_Matrix *B_w_f_c) {
  ...
  // check linear
  IVP_DOUBLE linear_dist =
      dir_world.dot_product(&normized_direction_world_at_t0) * linear_factor;

  if (linear_dist * IVP_Inline_Math::fabsd(linear_dist) < res) {
    return linear_dist;
  }

  return IVP_Inline_Math::sqrtd(
      res);  // bad, but allows good estimates for first deviation
}
...
IVP_DOUBLE IVP_3D_Solver_S_VALS::get_value(IVP_U_Matrix *A_w_f_c,
                                           IVP_U_Matrix *B_w_f_c) {
...
// Note: to minimize errors, the lot is constructed using calc_hesse twice
// instead of calc_s_val

IVP_DOUBLE IVP_3D_Solver_PK_KK::get_value(IVP_U_Matrix *A_w_f_c,
                                          IVP_U_Matrix *B_w_f_c) {
...
IVP_DOUBLE IVP_3D_Solver_PK_COLL::get_value(IVP_U_Matrix *A_w_f_c,
                                            IVP_U_Matrix *B_w_f_c) {
...
IVP_DOUBLE IVP_3D_Solver_PF_NPF::get_value(IVP_U_Matrix *A_w_f_c,
                                           IVP_U_Matrix *B_w_f_c) {
...
void IVP_Mindist_Event_Solver::calc_next_event_PF(
    const IVP_Compact_Edge *P, const IVP_Compact_Edge *F,
  ...
  event_time_out =
      t_max;  // flag, set after next psi -> no event within this PSI

  IVP_3D_Solver_PF_COLL pf_coll_solver;

  {
    pf_coll_solver.point_object.set(IVP_CLS.give_object_coords(P, m_cache_P));
    pf_coll_solver.point_of_area_object.set(
        IVP_CLS.give_object_coords(F, m_cache_F));

    // pf_coll_solver.hesse_of_area_object = F->get_triangle()->tmp.gen.hesse;
    IVP_CLS.calc_hesse_vec_object_not_normized(
        F, m_cache_F->get_compact_ledge(),
        &pf_coll_solver.hesse_of_area_object);
    ...
    IVP_DOUBLE sec_dist =
        mindist->get_coll_dist() + radius;  // security distance
    IVP_DOUBLE real_coll = ivp_mindist_settings.real_coll_dist + 0.5f * radius;
    IVP_DOUBLE now_len = mindist->len_numerator + radius;
    IVP_BOOL result = pf_coll_solver.find_first_t_for_value_coll(
        sec_dist, real_coll, t_now, event_time_out, &cache_P, &cache_F,
        &now_len, &event_time_out);
    if (result) {
  ...
  {
    IVP_DOUBLE real_dist =
        P_MIN(mindist->get_coll_dist(), mindist->len_numerator);
    real_dist += mindist->sum_extra_radius *
                 0.1f;  // lazy sector change for extra radius
    sec_dist = -ivp_mindist_settings.mindist_change_force_dist *
               m_cache_F->get_core()->inv_object_diameter;
    sec_dist *= real_dist;
  ...
  {
    m_cache_F->get_object_cache()->m_world_f_object.vmult3(
        &pf_npf_solver.hesse_of_area_object, &wHesseF);
    m_cache_P->get_object_cache()->m_world_f_object.vimult3(&wHesseF,
                                                            &hesse_in_P_object);
  }

  pf_npf_solver.set_max_deviation(max_rot_speed + P_DOUBLE_EPS);
  IVP_DOUBLE max_grad_change =
      pf_npf_solver.max_deviation * (event_time_out - t_now);

  const IVP_U_Float_Point *p_object = IVP_CLS.give_object_coords(P, m_cache_P);
  for (const IVP_Compact_Edge *e = P->get_prev()->get_opposite(); 1;
       e = e->get_prev()->get_opposite()) {
    const IVP_U_Float_Point *p_next_object =
        IVP_CLS.give_object_coords(e->get_next(), m_cache_P);
    IVP_U_Point vec_object;
    ...
    if (grad < max_grad_change) {  // fast check
      ...
      IVP_BOOL found_flag;
      found_flag = pf_npf_solver.find_first_t_for_value_max_dev(
          sec_dist, t_now, event_time_out, 0, &cache_P, &cache_F, &grad,
          &event_time_out);
      if (found_flag) {
...
void IVP_Mindist_Event_Solver::calc_next_event_BF(
    const IVP_Compact_Edge *F, IVP_Cache_Object *m_cache_B,
  ...
  IVP_3D_Solver_PF_COLL pf_coll_solver;
  {
    ...
    IVP_DOUBLE sec_dist =
        mindist->get_coll_dist() + radius;  // security distance
    IVP_DOUBLE real_coll = ivp_mindist_settings.real_coll_dist +
                           0.5f * radius;  // allow penetration
    IVP_DOUBLE now_len = mindist->len_numerator + radius;

    //@@@ optimization for static reference object (linear movement of ball)
    IVP_BOOL result = pf_coll_solver.find_first_t_for_value_coll(
        sec_dist, real_coll, t_now, t_max, &cache_P, &cache_F, &now_len,
        &event_time_out);
    if (result) {
      event_type_out = IVP_COLL_PF_COLL;
    }
...
IVP_DOUBLE IVP_3D_Solver_KK_COLL::get_value(IVP_U_Matrix *K_w_f_c,
  ...
  IVP_DOUBLE res =
      H.dot_product(&k_point_world) - H.dot_product(&l_point_world);
  IVP_DOUBLE iH_len = IVP_Inline_Math::isqrt_float(
      H.quad_length());  // bad, but allows good estimate for first deviation
  res *= iH_len;
  return res * side;
}
...
IVP_DOUBLE IVP_3D_Solver_KK_PARALLEL::get_value(IVP_U_Matrix *K_w_f_c,
                                                IVP_U_Matrix *L_w_f_c) {
  IVP_U_Point k_vec_world;
  IVP_U_Point l_vec_world;
...
void IVP_Mindist_Event_Solver::calc_next_event_KK(
    const IVP_Compact_Edge *K, const IVP_Compact_Edge *L,
  ...
  // which direction does H have ? (we want to know for which point to check
  // collision with which surface)
  int side;

  /** detect collision time, first **/
  {
    const IVP_U_Float_Point *k = IVP_CLS.give_object_coords(K, m_cache_K);
    const IVP_U_Float_Point *kn =
        IVP_CLS.give_object_coords(K->get_next(), m_cache_K);
    const IVP_U_Float_Point *l = IVP_CLS.give_object_coords(L, m_cache_L);
    const IVP_U_Float_Point *ln =
        IVP_CLS.give_object_coords(L->get_next(), m_cache_L);

    solver_kk_coll.k_vec_object.subtract(
        kn, k);  // only rough estimate for KK parallel
    ...
    solver_kk_coll.set_max_deviation(
        this->max_coll_speed);  // thats not correct, but ensures no collision
                                // !!!

    // check side
    {
      IVP_U_Point k_vec_world_now, l_vec_world_now;
      m_cache_L->clp_cache_object->m_world_f_object.vmult3(
          &solver_kk_coll.l_vec_object, &l_vec_world_now);
      m_cache_K->clp_cache_object->m_world_f_object.vmult3(
          &solver_kk_coll.k_vec_object, &k_vec_world_now);
    ...
    IVP_DOUBLE found = solver_kk_coll.find_first_t_for_value_coll(
        mindist->get_coll_dist(), ivp_mindist_settings.real_coll_dist, t_now,
        event_time_out, &cache_K, &cache_L, 0, &event_time_out);
  ...
  IVP_3D_Solver_KK_PARALLEL solver_kk_parallel;
  {
    ...
    IVP_DOUBLE dev =
        sum_abs_omega + sum_abs_omega;  // as kk parallel calcs the quadratic
                                        // length we have to adjust dev:
    // max (f^2') = max ( 2f * f') = max ( 2* 1 * f')
    solver_kk_parallel.set_max_deviation(dev + P_DOUBLE_EPS);
    IVP_DOUBLE found = solver_kk_parallel.find_first_t_for_value_max_dev(
        P_DOUBLE_EPS, t_now, event_time_out, 0, &cache_K, &cache_L, 0,
        &event_time_out);
    if (!hk_Math::almost_zero(found)) {
  ...
  IVP_U_Point
      *edge_object[4];  // edges to check, see P array at p_minimize_Leave_KK
  ...
  for (int i = 0; i <= 3; i++) {
    ...
    IVP_CLS.calc_hesse_vec_object_not_normized(
        f, m_cache_tab[3 - i]->get_compact_ledge(),
        &solver_KK_PF.hesse_of_area_object);
    solver_KK_PF.hesse_of_area_object.fast_normize();

    IVP_ASSERT(m_cache_tab[3 - i]->get_compact_ledge() ==
               f->get_compact_ledge());
    ...
    IVP_DOUBLE sec_dist = -ivp_mindist_settings.mindist_change_force_dist *
                          cache_e->core->inv_object_diameter;
    ivp_u_bool found = solver_KK_PF.find_first_t_for_value_max_dev(
        sec_dist, t_now, event_time_out, 0, cache_e, cache_F, 0,
        &event_time_out);
    if (found) {
...
/***************************** PP ******************************************/
/***************************** PP ******************************************/
void IVP_Mindist_Event_Solver::calc_next_event_PP(
    const IVP_Compact_Edge *P, const IVP_Compact_Edge *P2,
    IVP_Cache_Ledge_Point *m_cache_P, IVP_Cache_Ledge_Point *m_cache_P2) {
  ...
  const IVP_U_Float_Point *p_object = IVP_CLS.give_object_coords(P, m_cache_P);
  const IVP_U_Float_Point *p2_object =
      IVP_CLS.give_object_coords(P2, m_cache_P2);
  ...
  // check collision of both points
  {
    ...
    solver_pp_coll.normized_direction_world_at_t0.set_multiple(
        &mindist->contact_plane, -1);
#if 0
	IVP_IF(1){
    ...
    IVP_FLOAT radius = mindist->sum_extra_radius;
    ivp_u_bool found = solver_pp_coll.find_first_t_for_value_coll(
        mindist->get_coll_dist() + radius,
        ivp_mindist_settings.real_coll_dist + radius * 0.5f, t_now, t_max,
  ...
  {
    IVP_DOUBLE
    deviation[2];  // dev[0] when Point[0] is point and Poit[1] is edge
    IVP_DOUBLE
    neg_maximal_safe_s_value;  // needed to calc exact sector change value
    {                          // deviation calculation
      ...
      IVP_DOUBLE speed_P = p_object->fast_real_length() * solver_P->abs_omega +
                           solver_P->current_speed;
      IVP_DOUBLE speed_P2 =
          p2_object->fast_real_length() * solver_P2->abs_omega +
          solver_P2->current_speed;
      deviation[0] = speed_P + speed_P2 + max_dist_PP * solver_P2->abs_omega;
      deviation[1] = speed_P + speed_P2 + max_dist_PP * solver_P->abs_omega;

      IVP_DOUBLE object_radius =
          P_MAX(m_cache_P->get_core()->upper_limit_radius,
                m_cache_P2->get_core()->upper_limit_radius);
      IVP_DOUBLE quad_coll_dist = mindist->get_coll_dist();
      quad_coll_dist *= quad_coll_dist;
      neg_maximal_safe_s_value =
          -P_MIN(max_dist_PP * max_dist_PP, quad_coll_dist) * 0.5f /
          object_radius;
    }
    ...
    for (int j = 0; j < 2; j++) {
      ...
      for (const IVP_Compact_Edge *e = point2->get_prev()->get_opposite(); 1;
           e = e->get_prev()->get_opposite()) {
        const IVP_U_Float_Point *epn =
            IVP_CLS.give_object_coords(e->get_next(), mcache[1 - j]);
        ...
        if (solver_pp_pk.find_first_t_for_value_max_dev(
                sec_dist, t_now, event_time_out, 0, cache[j], cache[1 - j], 0,
                &event_time_out)) {
...
/***************************** PP ******************************************/
/***************************** PP ******************************************/
void IVP_Mindist_Event_Solver::calc_next_event_BP(
    IVP_Ball *ball, const IVP_Compact_Edge *P2, IVP_Cache_Object *m_cache_B,
  ...
  // check s vals for sector change
  {
    ...
    {                          // deviation calculation
      ...
      IVP_DOUBLE inv_object_diameter =
          m_cache_P2->get_core()->inv_object_diameter;
      IVP_DOUBLE quad_coll_dist = mindist->get_coll_dist();
      quad_coll_dist *= quad_coll_dist;
      neg_maximal_safe_s_value =
          -P_MIN(max_dist_PP * max_dist_PP, quad_coll_dist) *
          inv_object_diameter;
    }

    {
      ...
      for (const IVP_Compact_Edge *e = P2->get_prev()->get_opposite(); 1;
           e = e->get_prev()->get_opposite()) {
        const IVP_U_Float_Point *epn =
            IVP_CLS.give_object_coords(e->get_next(), m_cache_P2);
        ...
        if (solver_pp_pk.find_first_t_for_value_max_dev(
                sec_dist, t_now, event_time_out, 0, &cache_P, &cache_P2, 0,
                &event_time_out)) {
          event_type_out = IVP_COLL_PP_PK;
        }
        if (e == P2) break;
...
/***************************** PK ******************************************/
/***************************** PK ******************************************/
void IVP_Mindist_Event_Solver::calc_next_event_PK(
    const IVP_Compact_Edge *P, const IVP_Compact_Edge *K,
    IVP_Cache_Ledge_Point *m_cache_P, IVP_Cache_Ledge_Point *m_cache_K) {
  IVP_U_Matrix_Cache cache_P(m_cache_P->clp_cache_object);
  IVP_U_Matrix_Cache cache_K(m_cache_K->clp_cache_object);
  ...
  // check collision of point and edge
  {
    ...
    const IVP_U_Float_Point *k_object =
        IVP_CLS.give_object_coords(K, m_cache_K);
    const IVP_U_Float_Point *kn_object =
        IVP_CLS.give_object_coords(K->get_next(), m_cache_K);
    ...
    // find initial virtual collision plane
    {
      ...
      IVP_U_Point K_world;
      IVP_U_Point K_vec_world;  // normized direction
      m_cache_P->get_object_cache()->transform_position_to_world_coords(
          &solver_pk_coll.P_object, &P_world);
      m_cache_K->get_object_cache()->transform_position_to_world_coords(
          &solver_pk_coll.K_object, &K_world);
      m_cache_K->get_object_cache()->transform_vector_to_world_coords(
          &solver_pk_coll.K_vec_object, &K_vec_world);
      ...
      m_cache_K->get_object_cache()->transform_vector_to_object_coords(
          &H, &solver_pk_coll.K_Lot_object);
      solver_pk_coll.K_Lot_object.normize();
    }

    ivp_u_bool found = solver_pk_coll.find_first_t_for_value_coll(
        mindist->get_coll_dist() + radius,
        ivp_mindist_settings.real_coll_dist + radius * 0.9f, t_now, t_max,
  ...
  // both areas F0/F1
  {
    ...
    solver_pk_pf.set_max_deviation(
        this->worst_case_speed);  // when does point P move through lot area
    ...
    for (i = 0; i <= 1; i++) {
      ...
      IVP_U_Point v0;
      const IVP_U_Float_Point *f_object =
          IVP_CLS.give_object_coords(F, m_cache_K);
      const IVP_U_Float_Point *fn_object =
          IVP_CLS.give_object_coords(F->get_next(), m_cache_K);
      const IVP_U_Float_Point *fp_object =
          IVP_CLS.give_object_coords(F->get_prev(), m_cache_K);
      v0.subtract(fn_object, f_object);  // @@@ already calculated
      IVP_U_Point fhesse_vec;
      fhesse_vec.inline_set_vert_to_area_defined_by_three_points(
          f_object, fp_object, fn_object);
      ...
      IVP_DOUBLE sec_dist = -ivp_mindist_settings.mindist_change_force_dist;
      ivp_u_bool found_flag = solver_pk_pf.find_first_t_for_value_max_dev(
          sec_dist, t_now, event_time_out, 0, &cache_P, &cache_K, 0,
          &event_time_out);
      if (found_flag) {
  ...
  IVP_DOUBLE max_dev;
  {
    ...
    // Note: Lot' does not use |omegax|, because it is independent from rotation
    // of objects

    IVP_DOUBLE mindist_PK =
        dist_PK_now - this->worst_case_speed * (event_time_out - t_now);
    if (mindist_PK < P_RES_EPS) mindist_PK = P_RES_EPS;

    const IVP_U_Float_Point *spp = p_object;
    IVP_DOUBLE speed_P = spp->fast_real_length() * solver_P->abs_omega +
                         m_cache_P->get_core()->current_speed;
    IVP_DOUBLE speed_K =
        m_cache_K->get_core()->max_surface_rot_speed * solver_K->abs_omega +
        m_cache_K->get_core()->current_speed;
    max_dev = (speed_P + speed_K) / mindist_PK + sum_abs_omega;
  }

  {
    IVP_3D_Solver_PK_KK
        solver_pk_kk;  // calcs: - s_val * tmp.common.real_len
                       //	const IVP_U_Float_Point *p_object =
    // IVP_CLS.give_object_coords(P,m_cache_P);
    const IVP_U_Float_Point *k_object =
        IVP_CLS.give_object_coords(K, m_cache_K);
    const IVP_U_Float_Point *kn_object =
        IVP_CLS.give_object_coords(K->get_next(), m_cache_K);
    ...
    // exponentially go
    IVP_DOUBLE sec_dist =
        -0.3f * dist * (m_cache_K->get_core()->inv_object_diameter * 2.0f);

    for (const IVP_Compact_Edge *e = P->get_prev()->get_opposite(); 1;
         e = e->get_prev()->get_opposite()) {
      const IVP_U_Float_Point *en_object =
          IVP_CLS.give_object_coords(e->get_next(), m_cache_P);

      solver_pk_kk.P_ne_vec_object.subtract(en_object, p_object);
      solver_pk_kk.P_ne_vec_object.fast_normize();

      ivp_u_bool found_flag = solver_pk_kk.find_first_t_for_value_max_dev(
          sec_dist, t_now, event_time_out, 0, &cache_P, &cache_K, 0,
          &event_time_out);
      if (found_flag) {
...
/***************************** PK ******************************************/
/***************************** PK ******************************************/
void IVP_Mindist_Event_Solver::calc_next_event_BK(
    IVP_Ball *ball, const IVP_Compact_Edge *K, IVP_Cache_Object *m_cache_B,
...
void IVP_Mindist_Event_Solver::next_event_B_POLY(
  ...
  IVP_Cache_Ledge_Point m_cache_1(syn1->get_object()->to_poly(),
...
void IVP_Mindist_Event_Solver::next_event_default_poly_poly(
    IVP_Mindist_Event_Solver *mim) {
  ...
  IVP_Cache_Ledge_Point m_cache_0(
      syn0->get_object()->to_poly(),
      e0->get_compact_ledge());  // @@@ faster way? -OG
  IVP_Cache_Ledge_Point m_cache_1(syn1->get_object()->to_poly(),
                                  e1->get_compact_ledge());
...
void IVP_Mindist_Event_Solver::next_event_illegal(IVP_Mindist_Event_Solver *) {
  // error: inconsistency between IVP_Mindist_Event_Solver and
  // IVP_Mindist_Minimize_Solver
  CORE;
}

void (*IVP_Mindist_Event_Solver::mim_function_table[IVP_ST_MAX_LEGAL]
                                                   [IVP_ST_MAX_LEGAL])(
    IVP_Mindist_Event_Solver *mim);
...
void IVP_Mindist_Event_Solver::init_mim_function_table() {
  ...
  mim_function_table[IVP_ST_POINT][IVP_ST_TRIANGLE] =
      next_event_default_poly_poly;
