...
class IVP_Collision_Delegator {
 public:
   ...
   *			a and b can also be calculated using the get_objects
   *function however it's much faster this way
   ********************************************************************************/
  virtual void collision_is_going_to_be_deleted_event(
      class IVP_Collision
          *t) = 0;  // should remove t from its internal structures
  virtual ~IVP_Collision_Delegator() {}
  // Note: in the destructor, it should not delete it's IVP_Collision children,
  // but call child->delegator_is_going_to_be_deleted_event(this)
 ...
 *			see
 *IVP_Environment::add(/remove)_collision_delegator_root
 ********************************************************************************/
class IVP_Collision_Delegator_Root : public IVP_Collision_Delegator {
 public:
   ...
   *	Description:	create a subclass which checks collisions between the
   *base_object and the colliding_element Return:		NULL if type of
   *objects cannot be handled
   ********************************************************************************/
  virtual IVP_Collision *delegate_collisions_for_object(
      IVP_Real_Object *base_object, IVP_Real_Object *colliding_element) = 0;

  virtual void environment_is_going_to_be_deleted_event(
      IVP_Environment *env) = 0;
};
...
class IVP_Collision : public IVP_Time_Event {
 public:
  ...
  int fvector_index[2];  // for ov_tree backlink ( see
                         // IVP_Mindist::recheck_ov_element())
  ...
  IVP_Collision(IVP_Collision_Delegator *d) {
    delegator = d;
    fvector_index[0] = fvector_index[1] = -1;
  }
};
