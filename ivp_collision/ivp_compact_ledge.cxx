...
int IVP_Compact_Edge::next_table[] = {0, 1 * int(sizeof(IVP_Compact_Edge)),
                                      1 * int(sizeof(IVP_Compact_Edge)),
                                      -2 * int(sizeof(IVP_Compact_Edge))};
int IVP_Compact_Edge::prev_table[] = {0, 2 * int(sizeof(IVP_Compact_Edge)),
                                      -1 * int(sizeof(IVP_Compact_Edge)),
                                      -1 * int(sizeof(IVP_Compact_Edge))};
...
void IVP_Compact_Edge::byte_swap() {
  /*
          ...
          signed   int opposite_index:15;		// rel to this // maybe
     extra array, 3 bits more than tri_index/pierce_index unsigned int
     is_virtual:1;
  */
...
IVP_Compact_Triangle::IVP_Compact_Triangle() { P_MEM_CLEAR(this); }
...
void IVP_Compact_Ledge::c_ledge_init() { P_MEM_CLEAR(this); }
...
void IVP_Compact_Ledge::byte_swap() {
  /*
     ...
     outside this piece of memory unsigned int dummy:4; unsigned int
     size_div_16:24; short n_triangles; short for_future_use;
  */

  ivp_byte_swap4((uint&)c_point_offset);
  ivp_byte_swap4((uint&)ledgetree_node_offset);

  uint bitfields = *(uint*)(&ledgetree_node_offset +
                            1);  // as can't get addr of bitfield member..
...
void IVP_Compact_Ledge::byte_swap_all(
    IVP_U_BigVector<IVP_Compact_Poly_Point>* pre_swapped_points) {
#if defined(__POWERPC__)
  byte_swap();
#endif
...
void IVP_Compact_Ledgetree_Node::byte_swap() {
  /*
      ...
      int offset_compact_ledge; //(if != 0, pointer to hull that contains all
     subelements IVP_U_Float_Point3 center;	// in object_coords IVP_FLOAT
     radius; // size of sphere uchar box_sizes[IVP_CLT_N_DIRECTIONS]; uchar
     free_0;
  */
...
void IVP_Compact_Ledgetree_Node::byte_swap_all(
    IVP_U_BigVector<IVP_Compact_Poly_Point>* pre_swapped_points) {
#if defined(__POWERPC__)
  //
  ...
  if (!is_terminal()) {
    ...
    IVP_ASSERT(l);
    const_cast<IVP_Compact_Ledgetree_Node*>(l)->byte_swap_all(
        pre_swapped_points);

    // right:
    const IVP_Compact_Ledgetree_Node* r = right_son();  // returns const only?!
    IVP_ASSERT(r);
    const_cast<IVP_Compact_Ledgetree_Node*>(r)->byte_swap_all(
        pre_swapped_points);
  }
