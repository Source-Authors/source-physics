...
IVP_OV_Element::~IVP_OV_Element() {
  ...
  this->real_object->get_environment()
      ->fire_object_is_removed_from_collision_detection(real_object);
  IVP_ASSERT(collision_fvector.len() == 0);
  this->real_object->get_environment()
      ->get_ov_tree_manager()
      ->remove_ov_element(this);
}

void IVP_OV_Element::add_to_hull_manager(IVP_Hull_Manager *hm,
                                         IVP_DOUBLE hull_time) {
  const IVP_Time &current_time =
      real_object->get_environment()->get_current_time();
  if (hull_manager) {
...
IVP_HULL_ELEM_TYPE IVP_OV_Element::get_type() {
  return IVP_HULL_ELEM_OO_CONNECTOR;
};

void IVP_OV_Element::hull_manager_is_going_to_be_deleted_event(
    IVP_Hull_Manager *hm) {
  IVP_ASSERT(hm == hull_manager);
  delete this;
}

void IVP_OV_Element::hull_limit_exceeded_event(IVP_Hull_Manager *hm,
                                               IVP_HTIME) {
  IVP_ASSERT(hm == hull_manager);
  real_object->get_environment()->get_mindist_manager()->recheck_ov_element(
      real_object);
}
...
int IVP_OV_Tree_Manager::log_base2(IVP_DOUBLE x) const { return PFM_LD(x); }

IVP_BOOL IVP_OV_Tree_Manager::box_contains_box(
    const struct IVP_OV_Node_Data *master_data, const IVP_OV_Node *sub_node,
  ...
  int master_x1 =
      master_data->x
      << rasterlevel_diff;  // convert master rasterpoints to sub rasterpoints
...
IVP_DOUBLE IVP_OV_Tree_Manager::calc_optimal_box(const IVP_OV_Element *element,
  ...
  // as our selfmade power/log algorithm only works for the range of -40 to 40
  // we have to manually crop any possible underflow (caused by extremely small
  // objects) to the minimum value, i.e. we have a minimum boxsize of 2^-40
  // meters to be used by our tree
  if (raster_level < -40) {
    raster_level = -40;
    IVP_IF(1) {
      ivp_message(
          "IVP_OV_Tree_Manager - WARNING: Object too small! Adjusting raster "
          "size to 2^-40 meters.\n");
    }
  }
...
IVP_BOOL IVP_OV_Tree_Manager::box_overlaps_with_box(
    const IVP_OV_Node *largenode, const IVP_OV_Node *smallnode,
    const int rasterlevel_diff) const
...
IVP_OV_Node *IVP_OV_Tree_Manager::find_smallest_box(
    const IVP_OV_Node *master_node, const IVP_OV_Node *sub_node) const {
  int i;
  int rasterlevel_diff =
      (master_node->data.rasterlevel - 1) - sub_node->data.rasterlevel;
  for (i = 0; i < master_node->children.len(); i++) {
    const IVP_OV_Node *child = master_node->children.element_at(i);
    if (box_contains_box(&child->data, sub_node, rasterlevel_diff) ==
        IVP_TRUE) {
...
void IVP_OV_Tree_Manager::connect_boxes(IVP_OV_Node *node,
                                        IVP_OV_Node *new_node) {
  int rasterlevel_diff = node->data.sizelevel - new_node->data.sizelevel;

  IVP_ASSERT(rasterlevel_diff > 0);

  if (rasterlevel_diff == 1) {
    // new node is exactly one level below -> simply insert it as one of our
    // children
    new_node->parent = node;
    node->children.add(new_node);
#ifdef MY_DEBUG
    if (node->children.n_elems > 27)
      ivp_message("*** ERROR *** Excessive amount of children: %d\n",
  ...
  else if (new_node->data.x >=
           (((node->data.x << 1) + 1) << (rasterlevel_diff - 1))) {
  ...
  // if new node lower than quarter, use middle row
  else if (new_node->data.y >=
           (((node->data.y << 1) + 1) << (rasterlevel_diff - 1))) {
  ...
  // if new node lower than quarter, use middle row
  else if (new_node->data.z >=
           (((node->data.z << 1) + 1) << (rasterlevel_diff - 1))) {
  ...
  DEBUG_IF {
    ivp_indent_output();
    ivp_message(
        "Created new subbox (%+.4f/%+.4f/%+.4f [%d: %+d/%+d/%+d] +++ %d: %f)\n",
        ...
        new_subnode->data.rasterlevel, new_subnode->data.x, new_subnode->data.y,
        new_subnode->data.z, new_subnode->data.sizelevel,
...
void IVP_OV_Tree_Manager::expand_tree(const IVP_OV_Node *new_node) {
  ...
  if (this->root->data.sizelevel > 40)
    ivp_message("*** ERROR *** Excessive sizelevel (%d) for element\n",
  ...
  if (rest_x == -1) {
    ...
    if ((new_node->data.x * rasterpoints_newnode) <
        (rasterpoints * new_root->data.x)) {
#ifdef DEBUG_EXPANDTREE
      DEBUG_IF {
        ivp_indent_output();
  ...
  if (rest_y == -1) {
    ...
    if ((new_node->data.y * rasterpoints_newnode) <
        (rasterpoints * new_root->data.y)) {
#ifdef DEBUG_EXPANDTREE
      DEBUG_IF {
        ivp_indent_output();
  ...
  if (rest_z == -1) {
    ...
    if ((new_node->data.z * rasterpoints_newnode) <
        (rasterpoints * new_root->data.z)) {
#ifdef DEBUG_EXPANDTREE
      DEBUG_IF {
        ivp_indent_output();
...
#ifdef MY_DEBUG
  if (new_root->children.n_elems >= 27)
    ivp_message(
...
void IVP_OV_Tree_Manager::collect_subbox_collision_partners(
    const IVP_OV_Element *elem, const IVP_OV_Node *node) {
  {
    for (int i = node->elements.len() - 1; i >= 0;
         i--) {  // check real distance of spheres
...
void IVP_OV_Tree_Manager::collect_collision_partners(
    const IVP_OV_Element *elem, const IVP_OV_Node *masternode,
  ...
  if ((masternode->data.sizelevel - 1) > new_node->data.sizelevel) {
    int rasterlevel_diff =
        (masternode->data.rasterlevel - 1) - new_node->data.rasterlevel;
    for (int i = 0; i < masternode->children.len(); i++) {
      ...
      if (child == new_node) {
        ...
        if (box_overlaps_with_box(child, new_node, rasterlevel_diff) ==
            IVP_TRUE) {
    ...
    int rasterlevel_diff =
        new_node->data.rasterlevel - (masternode->data.rasterlevel - 1);
    for (int i = 0; i < masternode->children.len(); i++) {
      IVP_OV_Node *child = masternode->children.element_at(i);
      if (child == new_node) {
        collect_subbox_collision_partners(elem, child);
      } else {
        if (box_overlaps_with_box(new_node, child, rasterlevel_diff) ==
            IVP_TRUE) {
          collect_collision_partners(elem, child, new_node);
        }
      }
...
IVP_DOUBLE IVP_OV_Tree_Manager::insert_ov_element(
  ...
  // IVP_SUPREME_SUPPORT
  ivp_global_clustering_visualizer.longrange.add_object(
      element->real_object, &element->center, used_radius);
#endif
  ...
  if (new_node != NULL) {
    ...
    // check whether root box is already large enough for the element to
    // completely fit in...
    for (;;) {
      if (this->root->data.rasterlevel >= new_node->data.rasterlevel) {
        if (!box_contains_box(
                &this->root->data, new_node,
                this->root->data.rasterlevel - new_node->data.rasterlevel)) {
    ...
    if (this->root->data.rasterlevel == new_node->data.rasterlevel) {
      ...
      P_DELETE(new_node);
      new_node = this->root;  // necessary for e.g. finding the collision
                              // partners (see below)
      ...
      // search for the smallest box in one of the branches that completely
      // embraces the new box
  ...
  if (this->collision_partners) {
    ...
    // remove new element again from list (as we do not want our element to
    // collide with itself :)
    // this->collision_partners->remove_allow_resort(element);  checks after are
    // just faster than scanning this whole vector
  }
...
IVP_OV_Node *IVP_OV_Tree_Manager::cleanup_node(IVP_OV_Node *node) {
  if (node->elements.n_elems != 0)
    return (NULL);  // still elements left in node!
...
#ifdef IVP_HOME_BUILD
  // IVP_SUPREME_SUPPORT
  ivp_global_clustering_visualizer.longrange.remove_object(
      element->real_object);
#endif
...
void IVP_OV_Tree_Manager::get_luf_coordinates_ws(const IVP_OV_Node *node,
                                                 IVP_U_Float_Point *p,
                                                 IVP_FLOAT *cubesize) {
...
void IVP_OV_Tree_Manager::get_center_coordinates_ws(const IVP_OV_Node *node,
                                                    IVP_U_Float_Point *p,
                                                    IVP_FLOAT *cubesize) {
  p->k[0] =
      (IVP_FLOAT)(this->power2(node->data.rasterlevel) * (node->data.x + 1));
  p->k[1] =
      (IVP_FLOAT)(this->power2(node->data.rasterlevel) * (node->data.y + 1));
  p->k[2] =
      (IVP_FLOAT)(this->power2(node->data.rasterlevel) * (node->data.z + 1));
  *cubesize = (IVP_FLOAT)(this->power2(node->data.sizelevel));
