...
extern class IVP_Mindist_Settings {
 public:
  ...
  IVP_FLOAT
  coll_dists[IVP_MAX_STEPS_FOR_COLLDIST_DECREASE];  // Minimal distance
                                                    // between objects
                                                    // before collision
                                                    // occurs [meter]
  ...
  IVP_FLOAT speed_after_keeper_dist;  // speed when falling from keeper_dist to
                                      // coll_dist
  IVP_FLOAT distance_keepers_safety;  // safety gap, when surpassed mindist
                                      // doesnt appear in complex
  IVP_FLOAT max_distance_for_friction;  // when to through away friction mindist
  IVP_FLOAT max_distance_for_impact_system;  // check friction mindinst for
                                             // impact when in this range

  // collision
  IVP_FLOAT mindist_change_force_dist;        // in [meter]
  IVP_FLOAT min_vertical_speed_at_collision;  //
  // collision is assumed when distance is < coll_dist +
  // mindist_change_force_dist
 ...
 *	Description:	Simple root collision delegator which generates
 *functions useable for convex subparts
 ********************************************************************************/
class IVP_Collision_Delegator_Root_Mindist
 ...
 public:
  ...
  IVP_Collision *delegate_collisions_for_object(
      IVP_Real_Object *base_object,
      IVP_Real_Object *colliding_element) override;
...
class IVP_Synapse_OO : public IVP_Listener_Hull {
 ...
 public:
  ...
  void hull_limit_exceeded_event(IVP_Hull_Manager *hull_manager,
                                 IVP_HTIME) override;
  void hull_manager_is_going_to_be_deleted_event(IVP_Hull_Manager *)
      override;  // function have to remove itself from hull manager
};
...
class IVP_OO_Watcher : public IVP_Collision, public IVP_Collision_Delegator {
 ...
 public:
  ...
  IVP_OO_Watcher(IVP_Collision_Delegator *del, IVP_Real_Object *obj0,
 ...
 *	Attention:	No additional storage elements allowed in
 *IVP_Synapse_Real
 ********************************************************************************/
class IVP_Synapse_Real : public IVP_Synapse {
 ...
 public:
  inline void update_synapse(const IVP_Compact_Edge *e,
                             IVP_SYNAPSE_POLYGON_STATUS s) {
 ...
 protected:
  IVP_Synapse_Real() {}
  void init_synapse(IVP_Mindist *min, IVP_Real_Object *object_to_link,
                    const IVP_Compact_Edge *e, IVP_SYNAPSE_POLYGON_STATUS s) {
 ...
 public:
  ...
  inline IVP_DOUBLE insert_in_hull_manager(
      IVP_DOUBLE rel_hull_time);  // returns current center pos
...
enum IVP_MINIMAL_DIST_RECALC_RESULT { IVP_MDRR_OK = 0, IVP_MDRR_INTRUSION = 1 };
...
// type of mindist
enum IVP_MINDIST_FUNCTION { IVP_MF_COLLISION = 0, IVP_MF_PHANTOM = 1 };
...
class IVP_Mindist_Base : public IVP_Collision {
 ...
 public:  // Read only public !!!!
  IVP_BOOL
  is_in_phantom_set : 2;  // mindist is in phantom set_of_mindists already
...
#ifdef IVP_HALFSPACE_OPTIMIZATION_ENABLED
  IVP_FLOAT
  contact_dot_diff_center;  // contact plane * ( core0->pos - core1->pos),
  IVP_DOUBLE
  sum_angular_hull_time;  // sum of center hull time, needs IVP_DOUBLE
#endif

  IVP_U_Float_Point
      contact_plane;  //  needed for halfspace optimization, normized surface
                      //  normal, only valid if recalc_result == IVP_MDRR_OK
  ...
  }  // distance, only valid if recalc_result == IVP_MDRR_OK
...
class IVP_Mindist : public IVP_Mindist_Base  //
{
 ...
 public:
   ...
   ********************************************************************************/
  void init_mindist(IVP_Real_Object *pop0, IVP_Real_Object *pop1,
                    const IVP_Compact_Edge *e0, const IVP_Compact_Edge *e1);
   ...
   *	Description:	tries to reactivate invalid mindists (same as
   *recalc_mindist but optimized for invalid baviour
   ********************************************************************************/
  ...
  void update_exact_mindist_events(
      ...
      IVP_MINDIST_EVENT_HINT
          allow_events_at_now);  // after recalc_mindist check for collision

  virtual void do_impact();  // does everything, including recalculation of
                             // everything, PSI synchronization etc ...
  ...
  //     this function is called to create a Contact_Point before a impact
  //     appears. the Contact_Point is fed with the correct friction values of
  //     an already existing Contact_Point
...
class IVP_Mindist_Recursive : public IVP_Mindist,
  ...
  void collision_is_going_to_be_deleted_event(class IVP_Collision *t)
      override;  // should remove t from its internal structures
 ...
 public:
  ...
  void do_impact()
      override;  // recursive check for sub mindists or IVP_Mindist::do_impact
...
class IVP_Mindist_Manager {
 ...
 public:
   ...
   *			class IVP_Mindist_OO_Watcher (needs
   *IVP_Mindist_OO_Watcher);
   ********************************************************************************/
  static void create_exact_mindists(
      IVP_Real_Object *obj0, IVP_Real_Object *obj1, IVP_DOUBLE scan_radius,
      ...
      const IVP_Compact_Ledge *root_ledge1, IVP_Collision_Delegator *del);

  void insert_exact_mindist(
      IVP_Mindist *new_mindist);  // insert in linked list only
  void insert_and_recalc_exact_mindist(
      IVP_Mindist *new_mindist);  // insert no phantoms in linked list and
                                  // recalc and update_events
  void insert_and_recalc_phantom_mindist(
      IVP_Mindist *new_mindist);  // insert phantoms in linked list and recalc
                                  // and update_events
  void remove_exact_mindist(IVP_Mindist *del_mindist);

  void insert_invalid_mindist(
      IVP_Mindist *new_mindist);  // insert in linked list only
  ...
  static void insert_hull_mindist(IVP_Mindist *md, IVP_HTIME hull_time0,
                                  IVP_HTIME hull_time1);
  static void insert_hull_mindist(IVP_Mindist *md, IVP_HTIME hull_time);

  static void insert_lazy_hull_mindist(IVP_Mindist *md, IVP_HTIME hull_time0,
                                       IVP_HTIME hull_time1);
  ...
  void enable_collision_detection_for_object(
      IVP_Real_Object *);  // creates ov element for object
