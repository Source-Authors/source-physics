...
void IVP_Mindist_Recursive::do_impact() {
  ...
  // this->get_spawned_mindist_count());

  if (this->get_spawned_mindist_count() >
      ivp_mindist_settings.max_spawned_mindist_count)
    goto do_impact;
      ...
      switch (syn1->get_status()) {
        ...
        case IVP_ST_EDGE:
          ...
          // now check details
          {
            ...
            const IVP_Compact_Ledge *l0 =
                syn0->get_edge()->get_triangle()->get_compact_ledge();
            const IVP_Compact_Ledge *l1 =
                syn1->get_edge()->get_triangle()->get_compact_ledge();
  ...
  }
  {
    ...
    env->get_range_manager()->get_coll_range_intra_objects(
        syn0->get_object(), syn1->get_object(), &dist0, &dist1);
...
void IVP_Mindist_Recursive::exact_mindist_went_invalid(
    IVP_Mindist_Manager *mm) {
  IVP_Synapse_Real *syn0 = this->get_synapse(0);
  IVP_Synapse_Real *syn1 = this->get_synapse(1);
...
void IVP_Mindist_Recursive::delete_all_children() {
  ...
  // this->get_spawned_mindist_count());//@@CB
  mindists.clear();
}

IVP_Mindist_Recursive::IVP_Mindist_Recursive(IVP_Environment *env,
...
void IVP_Mindist_Recursive::collision_is_going_to_be_deleted_event(
    IVP_Collision *c) {
...
void IVP_Mindist_Recursive::recheck_recursive_childs(IVP_DOUBLE dist_intra) {
  ...
  if (this->get_spawned_mindist_count() >
      ivp_mindist_settings.max_spawned_mindist_count)  //@@CB
    return;

  const IVP_Compact_Ledge *sl[2] = {NULL, NULL};  // single reference ledge
  const IVP_Compact_Ledge *rl[2] = {NULL, NULL};  // single root ledge
  sl[1 - recursive_status] = get_synapse(1 - recursive_status)
                                 ->get_edge()
                                 ->get_triangle()
                                 ->get_compact_ledge();
  rl[recursive_status] = get_synapse(recursive_status)
                             ->get_edge()
                             ->get_triangle()
                             ->get_compact_ledge();
  ...
  int l = mindists.len();  //@@CB
  mm->create_exact_mindists(obj0, obj1, dist_intra, &mindists, sl[0], sl[1],
                            rl[0], rl[1], this);
  l = mindists.len() - l;                 //@@CB
  this->change_spawned_mindist_count(l);  //@@CB
}
...
void IVP_Mindist_Recursive::rec_hull_limit_exceeded_event() {
  ...
  IVP_DOUBLE dist0, dist1;
  IVP_Environment *env = obj0->get_environment();
  env->get_range_manager()->get_coll_range_intra_objects(obj0, obj1, &dist0,
                                                         &dist1);
