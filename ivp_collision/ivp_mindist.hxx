...
/* if IVP_HALFSPACE_OPTIMIZATION_ENABLED is defined, the mindist remembers a
 * virtual plane seperating both ledges. It tries to use that halfspace to
 * optimize the hull */
...
class IVP_Synapse : public IVP_Listener_Hull {  // sizeof() == 32
 ...
 protected:
  ...
  void hull_limit_exceeded_event(IVP_Hull_Manager *hull_manager,
                                 IVP_HTIME hull_intrusion_value) override;
  void hull_manager_is_going_to_be_deleted_event(
      IVP_Hull_Manager *hull_manager) override;
 ...
 public:
  ...
  IVP_Synapse()
      : next{nullptr},
        ...
        l_obj{nullptr},
        edge{nullptr},
        mindist_offset{std::numeric_limits<hk_intp>::max()},
        status{SHRT_MAX} {}
  ...
  void init_synapse_real(IVP_Mindist_Base *min,
                         IVP_Real_Object *object_to_link) {
