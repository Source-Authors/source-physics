...
IVP_Time IVP_3D_Solver::calc_nullstelle(IVP_Time t0, IVP_Time t1,
                                        IVP_DOUBLE value, IVP_DOUBLE v0,
                                        IVP_DOUBLE v1,
  ...
  while (1) {
    ...
    // t0.get_time(),v0, tt.get_time(),vv, t1.get_time(),v1, vv-value);
...
IVP_BOOL IVP_3D_Solver::find_first_t_for_value_max_dev(
    IVP_DOUBLE value, IVP_Time t_now, IVP_Time t_max, int t_now_cache_index,
  ...
  while (1) {
    ...
    int int_t_step = (int)(t_step * (IVP_3D_SOLVER_MAX_STEPS_PER_PSI *
                                     IVP_3D_SOLVER_PSIS_PER_SECOND));
    ...
    IVP_DOUBLE d_tstep = int_t_step * (1.0f / (IVP_3D_SOLVER_MAX_STEPS_PER_PSI *
                                               IVP_3D_SOLVER_PSIS_PER_SECOND));
    IVP_Time t1 = t0;
    t1 += d_tstep;
    ...
    if (v1 <= value) {
      ...
      IVP_Time t =
          calc_nullstelle(t0, t1, value, v0, v1, mc_A->object, mc_B->object);
      if (t - t_max > 0.0f) {
        return IVP_FALSE;
      }
      *t_out = t;
...
IVP_BOOL IVP_3D_Solver::find_first_t_for_value_max_dev2(
    IVP_DOUBLE value, IVP_Time t_now, IVP_Time t_max, int t_now_cache_index,
    IVP_U_Matrix_Cache *mc_A, IVP_U_Matrix_Cache *mc_B,
    IVP_DOUBLE *opt_val_at_t_now, IVP_Time *t_out) {
...
IVP_BOOL IVP_3D_Solver::find_first_t_for_value_coll(
    IVP_DOUBLE value, IVP_DOUBLE value2, IVP_Time t_now, IVP_Time t_max,
    IVP_U_Matrix_Cache *mc_A, IVP_U_Matrix_Cache *mc_B,
    IVP_DOUBLE *opt_val_at_t_now, IVP_Time *t_out) {
  /***** first 'null'-stelle from t_now to t_max *******/
  ...
  // wenn stets value(=coll_dist) > werte > value2(=echte oberflaeche), dann
  // nix. wenn ein wert > value, dann ab dort ganz normal den naechsten
  ...
  if (tnow_val > value) {
    ...
    return find_first_t_for_value_max_dev(
        value, t_now, t_max, 0 /* cache_index*/, mc_A, mc_B, &tnow_val, t_out);
  }
  ...
  while (t0 - t_max < 0.0f) {
    ...
    IVP_U_Matrix *B;
    IVP_Time t1;
    IVP_DOUBLE real_t_step =
        int_t_step * (1.0f / (IVP_3D_SOLVER_MAX_STEPS_PER_PSI *
                              IVP_3D_SOLVER_PSIS_PER_SECOND));
    t1 = t0 + real_t_step;
    cache1_index += int_t_step;
    ...
    if (v1 <= tnow_val) {
      // in t1 we are nearer to object than in t_now -> indicate another
      // collision at t0
      *t_out = t0;
      return IVP_TRUE;
    }
