...
IVP_MRC_TYPE IVP_Mindist_Minimize_Solver::p_minimize_PF(
  ...
  for (e = F, j = 0; j < 3; e = e->get_next(), j++) {
    ...
    IVP_DOUBLE dist =
        IVP_CLS.calc_qlen_PK_K_space(&p_Fos, m_cache_F->get_compact_ledge(), e);
...
IVP_MRC_TYPE IVP_Mindist_Minimize_Solver::p_minimize_BF(
    IVP_Cache_Ball *m_cache_B, const IVP_Compact_Edge *F,
  ...
  IVP_U_Point p_Fos;
  IVP_U_Point *center_B =
      m_cache_B->cache_object->m_world_f_object.get_position();
  m_cache_F->clp_cache_object->transform_position_to_object_coords(center_B,
                                                                   &p_Fos);
  ...
  if (!qr.is_outside()) {
    ...
    IVP_CLS.calc_hesse_vec_object_not_normized(
        ...
        &wHesse_vecF_os);  // should be normized: see below
    wHesse_vecF_os.normize();
    IVP_U_Point contact_plane;
    m_cache_F->get_object_cache()->transform_vector_to_world_coords(
        &wHesse_vecF_os, &contact_plane);
    mindist->contact_plane.set(&contact_plane);

    IVP_DOUBLE dist_plus_radius =
        p_Fos.dot_product(&wHesse_vecF_os) - wHesse_vecF_os.dot_product(F_O_os);
    ...
    diff_center.subtract(&m_cache_B->cache_object->core_pos,
                         &m_cache_F->get_object_cache()->core_pos);
    mindist->contact_dot_diff_center =
        diff_center.dot_product(&mindist->contact_plane);
#endif
    return IVP_MRC_OK;
  }
...
IVP_MRC_TYPE IVP_Mindist_Minimize_Solver::p_minimize_Leave_PF(
    const IVP_Compact_Edge *P, const IVP_U_Point *P_Fos,
    const IVP_Compact_Edge *F, IVP_Cache_Ledge_Point *m_cache_P,
    IVP_Cache_Ledge_Point *m_cache_F) {
  ...
  IVP_DOUBLE back_side_check_distance =
      0.0f;  //-0.5f * ivp_mindist_settings.coll_dist;	// when should we treat
             // surface as backside
  {
    IVP_U_Point wHesse_vecF_Fos;
    IVP_CLS.calc_hesse_vec_object_not_normized(
        F, m_cache_F->get_compact_ledge(),
        &wHesse_vecF_Fos);  // should be normized: see below
    wHesse_vecF_Fos.fast_normize();
    IVP_U_Point hesse_vec_ws;
    m_cache_F->get_object_cache()->transform_vector_to_world_coords(
        &wHesse_vecF_Fos, &hesse_vec_ws);
    m_cache_P->get_object_cache()->transform_vector_to_object_coords(
        &hesse_vec_ws, &hesse_vec_Pos);

    // if backside than reverse hesse
    const IVP_U_Float_Point *F_O_Fos = IVP_CLS.give_object_coords(F, m_cache_F);
    mindist->len_numerator = wHesse_vecF_Fos.dot_product(P_Fos) -
                             wHesse_vecF_Fos.dot_product(F_O_Fos);
...
#ifdef IVP_HALFSPACE_OPTIMIZATION_ENABLED
    IVP_U_Float_Point diff_center;
    diff_center.subtract(&m_cache_P->get_object_cache()->core_pos,
                         &m_cache_F->get_object_cache()->core_pos);
    mindist->contact_dot_diff_center =
        diff_center.dot_product(&mindist->contact_plane);
#endif
...
#ifdef DEBUG_CHECK_LEN
  check_len_PF(P, F, m_cache_P,
               m_cache_F);  // check wether world_pos_hash is afflicted
#endif
  ...
  const IVP_Compact_Edge *min_edge = NULL;
  {
    ...
    for (; 1; e = e->get_opposite()->get_prev()) {
      const IVP_Compact_Poly_Point *wenext =
          IVP_CLS.give_object_coords(e, m_cache_P);
      IVP_U_Point vec_Pos;
  ...
  if (min_edge == NULL) {  // no nearer point found
    ...
    if (mindist->len_numerator + mindist->sum_extra_radius <
        back_side_check_distance) {  // check for backside
      this->pos_opposite_BacksideOs.set(P_Fos);
      m_cache_F->tmp.synapse->update_synapse(F, IVP_ST_BACKSIDE);
      return IVP_MRC_BACKSIDE;
    }
  ...
  IVP_CLS.calc_unscaled_qr_vals_F_space(F->get_compact_ledge(), F, &P2_Fos,
                                        &qr);
  ...
  // now that we are outside, let's search for exactly one edge e of F with
  // these restrictions:
  // 		1. new P is outside e
  //		2. shortest distance between e and P_NP is on e ( 0 < s_val <
  // 1);
...
IVP_MRC_TYPE IVP_Mindist_Minimize_Solver::p_minimize_KK(
  ...
  if (!kkr.is_outside_K()) {
    ...
    if (kkr.checks_L[0] < 0.0f) {  // sl is already checked for interval [0 ..
                                   // 1], now check side only
  ...
  // Now minimal dist is outside of both edges, meaning that the lines have the
  // nearest distance outside the edges before switching to PP case we have to
  // check whether a PK case should be called
  ...
  IVP_CLS.calc_unscaled_s_val_K_space(m_cache_L->get_compact_ledge(), L,
                                      &pkp_Los, &sr_kL);
  ...
  IVP_CLS.calc_unscaled_s_val_K_space(m_cache_K->get_compact_ledge(), K,
                                      &plp_Kos, &sr_lK);
...
IVP_MRC_TYPE IVP_Mindist_Minimize_Solver::p_minimize_Leave_KK(
    const IVP_Compact_Edge *K, const IVP_Compact_Edge *L,
  ...
  // create plane to determine on which side of K the start point of L can be
  // found
  IVP_U_Point H_Kos;
  IVP_U_Point H_Los;
  IVP_U_Point H_ws;
  ...
  int side;  // OSR
  {
    ...
    m_cache_L->get_object_cache()->transform_vector_to_world_coords(&H_Los,
                                                                    &H_ws);
    m_cache_K->get_object_cache()->transform_vector_to_object_coords(&H_ws,
                                                                     &H_Kos);

    mindist->len_numerator =
        IVP_Inline_Math::fabsd(val * iHQ);  // no direction of KK
    mindist->len_numerator -= mindist->sum_extra_radius;
    mindist->contact_plane.set_multiple(&H_ws, (side - 0.5f) * 2.0f * iHQ);

#ifdef IVP_HALFSPACE_OPTIMIZATION_ENABLED
    IVP_U_Float_Point diff_center;
    diff_center.subtract(&m_cache_K->get_object_cache()->core_pos,
                         &m_cache_L->get_object_cache()->core_pos);
    mindist->contact_dot_diff_center =
        diff_center.dot_product(&mindist->contact_plane);
#endif
  }

  IVP_BOOL reverse_side_check[4];  // IVP_TRUE when point sees side from wrong
                                   // direction
  IVP_Leave_KK_Case cases[4];
  IVP_U_Point L_Kos[2];
  IVP_CLS.transform_pos_other_space(kkin.L_Los[0], m_cache_L, m_cache_K,
                                    &L_Kos[0]);
  IVP_CLS.transform_pos_other_space(kkin.L_Los[1], m_cache_L, m_cache_K,
                                    &L_Kos[1]);
  {
  ...
  }
  {
    ...
    for (int i = 0; i <= 3; i++) {
      IVP_CLS.calc_hesse_vec_object_not_normized(
          c->F, c->F->get_compact_ledge(), &c->hesse_Fos);
  ...
  IVP_Unscaled_QR_Result min_qr;
  {
    ...
    for (int i = 0; i <= 3; i++, cF++) {
      ...
      IVP_DOUBLE inv_hesse_real_length =
          IVP_Inline_Math::isqrt_float(cF->hesse_Fos.quad_length());
      IVP_DOUBLE grad =
          delta_h2 * IVP_Inline_Math::isqrt_float(e_dir_Fos.quad_length());
      grad *= inv_hesse_real_length;
      if (grad < min_grad_pos) {
        // check side of triangle

        IVP_Unscaled_QR_Result qr;
        IVP_CLS.calc_unscaled_qr_vals_F_space(
            cF->m_cache_F->get_compact_ledge(), cF->F, ce->P_Fos, &qr);
  ...
  if (min_edge == 0) {
    ...
    if (reverse_side_check[2] + reverse_side_check[3] == IVP_TRUE + IVP_TRUE) {
      ...
      this->pos_opposite_BacksideOs.set_interpolate(&kkin.K_Los[0],
                                                    &kkin.K_Los[1], sk);
...

  {
    if (!min_qr.is_outside()) {  // nearer point is inside new area->PF
      ...
      IVP_CLS.calc_pos_other_space(min_edge, min_edge_m_cache,
                                   min_plane_m_cache, &edge_Fos);
      return p_minimize_Leave_PF(min_edge, &edge_Fos, min_plane,
                                 min_edge_m_cache, min_plane_m_cache);
    }

    if (min_qr.checks[2] >= 0.0f) {  // other side is inside
      return p_minimize_KK(min_edge, min_plane->get_next(), min_edge_m_cache,
                           min_plane_m_cache);  // no ideas about backsides !!!
    }

    if (min_qr.checks[1] >= 0.0f) {
      return p_minimize_KK(min_edge, min_plane->get_prev(), min_edge_m_cache,
                           min_plane_m_cache);  // no ideas about backsides !!!
    }
  }
  ...
  {
    IVP_DOUBLE dist_next = IVP_CLS.calc_qlen_KK(
        min_edge, min_plane->get_next(), min_edge_m_cache, min_plane_m_cache);
    IVP_DOUBLE dist_prev =
        IVP_CLS.calc_qlen_KK(min_edge, min_plane->get_prev()->get_opposite(),
                             min_edge_m_cache, min_plane_m_cache);

    // @@@ slow solution: compare all 3 dists...
#ifdef IVP_MINDIST_BEHAVIOUR_DEBUG
    IVP_DOUBLE dist_plane =
        calc_qlen_KK(min_edge, min_plane, min_edge_m_cache, min_plane_m_cache);
...
IVP_MRC_TYPE IVP_Mindist_Minimize_Solver::p_minimize_BP(
    IVP_Cache_Ball *m_cache_ball, const IVP_Compact_Edge *P,
    IVP_Cache_Ledge_Point *m_cache_P) {
  if (--P_Finish_Counter < 0) {
  ...
  IVP_U_Point p_ws;
  IVP_CLS.give_world_coords_AT(P, m_cache_P, &p_ws);
  center_b_ws = m_cache_ball->cache_object->m_world_f_object.get_position();
  m_cache_P->get_object_cache()->transform_position_to_object_coords(
      center_b_ws, &center_ball_Pmos);
...
#ifdef IVP_HALFSPACE_OPTIMIZATION_ENABLED
  IVP_U_Float_Point diff_center;
  diff_center.subtract(&m_cache_ball->cache_object->core_pos,
                       &m_cache_P->get_object_cache()->core_pos);
  mindist->contact_dot_diff_center =
      diff_center.dot_product(&mindist->contact_plane);
#endif
  ...
  // each vertex P is position, all neighbouring edges of Pm checked
  {
    ...
    const IVP_U_Float_Point *Pm_Pmos =
        IVP_CLS.give_object_coords(Pm, m_cache_P);
    ...
    for (; 1; e = e->get_opposite()->get_prev()) {
      const IVP_U_Float_Point *tp_next_os =
          IVP_CLS.give_object_coords(e, m_cache_P);
      IVP_DOUBLE grad = Pm_P_Pmos.dot_product(tp_next_os) - dist;
      if (grad > 0) {
        IVP_DOUBLE i_len =
            IVP_Inline_Math::isqrt_float(tp_next_os->quad_distance_to(Pm_Pmos));
        grad *= i_len;
  ...
  IVP_CLS.calc_unscaled_s_val_K_space(m_cache_P->get_compact_ledge(), Kmax,
                                      &center_ball_Pmos, &sr);  //
  if (sr.checks[1] <= 0) {
    IVP_IF(1) { ivp_message("BP epsilon problem\n"); }
    goto end_BP;
  }
...
IVP_MRC_TYPE IVP_Mindist_Minimize_Solver::p_minimize_PP(
    const IVP_Compact_Edge *A, const IVP_Compact_Edge *B,
  ...
  m_cache_B->get_object_cache()->transform_position_to_object_coords(
      &points_ws[0], &point_other_os[0]);
  m_cache_A->get_object_cache()->transform_position_to_object_coords(
      &points_ws[1], &point_other_os[1]);

  {
    ...
    if (qlen > P_DOUBLE_RES) {
      ...
      mindist->contact_plane.inline_subtract_and_mult(&points_ws[0],
                                                      &points_ws[1], inv_len);
    } else {
      return IVP_MRC_ENDLESS_LOOP;
    }
...
#ifdef IVP_HALFSPACE_OPTIMIZATION_ENABLED
    IVP_U_Float_Point diff_center;
    diff_center.subtract(&m_cache_A->get_object_cache()->core_pos,
                         &m_cache_B->get_object_cache()->core_pos);
    mindist->contact_dot_diff_center =
        diff_center.dot_product(&mindist->contact_plane);
#endif
  }
  ...
  for (int ip = 1; ip >= 0; ip--) {
    ...
    const IVP_U_Float_Point *Pm_Pmos =
        IVP_CLS.give_object_coords(Pm, m_cache_Pm);
    IVP_U_Point Pm_P_Pmos;
    ...
    for (; 1; e = e->get_opposite()->get_prev()) {
      const IVP_U_Float_Point *tp_next_os =
          IVP_CLS.give_object_coords(e, m_cache_Pm);
      IVP_DOUBLE grad = Pm_P_Pmos.dot_product(tp_next_os) - dist;
      if (grad > 0) {
        IVP_DOUBLE i_len =
            IVP_Inline_Math::isqrt_float(tp_next_os->quad_distance_to(Pm_Pmos));
  ...
  IVP_Unscaled_S_Result sr;
  IVP_CLS.calc_unscaled_s_val_K_space(m_cache_K_max->get_compact_ledge(), Kmax,
                                      P_Pmos_max, &sr);  //
  if (sr.checks[1] <= 0) {
    IVP_IF(1) { ivp_message("PP epsilon problem\n"); }
    goto end_PP;
  }
...
IVP_MRC_TYPE IVP_Mindist_Minimize_Solver::p_minimize_BK(
    IVP_Cache_Ball *m_cache_B, const IVP_Compact_Edge *K,
    IVP_Cache_Ledge_Point *m_cache_K) {
  IVP_U_Point P_Kos;
  {
    IVP_U_Point *wP = m_cache_B->cache_object->m_world_f_object.get_position();
    m_cache_K->get_object_cache()->transform_position_to_object_coords(wP,
                                                                       &P_Kos);
  }

  IVP_Unscaled_S_Result sr;
  IVP_CLS.calc_unscaled_s_val_K_space(m_cache_K->get_compact_ledge(), K, &P_Kos,
                                      &sr);
...
IVP_MRC_TYPE IVP_Mindist_Minimize_Solver::p_minimize_Leave_BK(
    IVP_Cache_Ball *m_cache_ball, const IVP_Compact_Edge *K,
    IVP_Cache_Ledge_Point *m_cache_K) {
  if (--P_Finish_Counter < 0) {
  ...
  {
    IVP_U_Point *p_ws =
        m_cache_ball->cache_object->m_world_f_object.get_position();
    m_cache_K->get_object_cache()->transform_position_to_object_coords(p_ws,
                                                                       &p_ks);
  }

  // get direction of edge K
  const IVP_U_Float_Point *k_start_os =
      IVP_CLS.give_object_coords(K, m_cache_K);  // a point on an edge
  const IVP_U_Float_Point *k_next_os =
      IVP_CLS.give_object_coords(K->get_next(), m_cache_K);  // next point
  const IVP_U_Float_Point *k_tri_os = IVP_CLS.give_object_coords(
      K->get_prev(), m_cache_K);  // prev point == other point of this triange
  const IVP_U_Float_Point *k_oppo_tri_os = IVP_CLS.give_object_coords(
      K->get_opposite()->get_prev(), m_cache_K);  // point of opposite triangle
  ...
  IVP_U_Point vec_K_tri;
  vec_K_tri.subtract(k_tri_os, k_start_os);
  IVP_U_Point vec_K_oppo_tri;
  vec_K_oppo_tri.subtract(k_oppo_tri_os, k_start_os);

  IVP_U_Point hesse_tri_os;
  hesse_tri_os.calc_cross_product(&vec_K_ks,
                                  &vec_K_tri);  // vertical to triangle
  IVP_U_Point hesse_oppo_tri_os;
  hesse_oppo_tri_os.calc_cross_product(&vec_K_oppo_tri,
                                       &vec_K_ks);  // v. to oppo tri
  ...
  IVP_CLS.calc_unscaled_qr_vals_F_space(m_cache_K->get_compact_ledge(), K,
                                        &p_ks, &qr0);
  IVP_CLS.calc_unscaled_qr_vals_F_space(m_cache_K->get_compact_ledge(),
                                        K->get_opposite(), &p_ks, &qr1);
  ...
  IVP_U_Float_Point diff_center;
  diff_center.subtract(&m_cache_ball->cache_object->core_pos,
                       &m_cache_K->get_object_cache()->core_pos);
  mindist->contact_dot_diff_center =
      diff_center.dot_product(&mindist->contact_plane);
#endif
...
IVP_MRC_TYPE IVP_Mindist_Minimize_Solver::p_minimize_PK(
...
IVP_MRC_TYPE IVP_Mindist_Minimize_Solver::p_minimize_Leave_PK(
    const IVP_Compact_Edge *P, const IVP_Compact_Edge *K,
    IVP_Cache_Ledge_Point *m_cache_P, IVP_Cache_Ledge_Point *m_cache_K) {
  if (--P_Finish_Counter < 0) {
  ...
  {  // Lot _ws
    ...
    if (qlen > P_DOUBLE_EPS) {
      ...
      mindist->len_numerator = qlen * inv_len - mindist->sum_extra_radius;
      Lot2.calc_cross_product(&vec_K_ks,
                              &Lot2);  // does not change the length !!!
      m_cache_K->get_object_cache()->transform_vector_to_world_coords(&Lot2,
                                                                      &Lot2);
      mindist->contact_plane.set_multiple(&Lot2, -inv_len * iqK_len);
    } else {
      mindist->len_numerator = -mindist->sum_extra_radius;
      IVP_U_Point orth;
      orth.calc_an_orthogonal(&vec_K_ks);
      orth.normize();
  ...
  // Lot _point_space
  m_cache_P->get_object_cache()->transform_vector_to_object_coords(&Lot2,
                                                                   &Lot2);
