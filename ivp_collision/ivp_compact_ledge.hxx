...
class IVP_Compact_Edge {
 ...
 public:
  ...
  // dimhotepus: unsigned -> bool.
  inline void set_is_virtual(bool val) { is_virtual = val; }
 ...
 public:
  ...
  // see IVP_Compact_Ledge_Solver
  inline const IVP_Compact_Poly_Point *get_start_point(
      const IVP_Compact_Ledge *c_ledge) const;
...
class IVP_Compact_Ledge {
 ...
 public:
  ...
  // get corresponding ledge tree node for recursive compace ledges only ( no
  // grids )
  inline const IVP_Compact_Ledgetree_Node *get_ledgetree_node() const {
    IVP_ASSERT(!is_terminal());
    return (IVP_Compact_Ledgetree_Node *)(((char *)this) +
...
class IVP_Compact_Ledgetree_Node {
 public:
  ...
  const IVP_Compact_Ledgetree_Node *right_son() const {
    IVP_ASSERT(offset_right_node);
    return (IVP_Compact_Ledgetree_Node *)(((char *)this) +
                                          this->offset_right_node);
  }

  IVP_BOOL is_terminal() const {
    if (!offset_right_node)
      return IVP_TRUE;
    else
      return IVP_FALSE;
  }
  ...
  void byte_swap();  // just byte swap this data
  void byte_swap_all(IVP_U_BigVector<IVP_Compact_Poly_Point>
                         *pre_swapped_points);  // byte_swap, and recurse too
                                                // all related child data
};
...
const IVP_Compact_Ledge *IVP_Compact_Edge::get_compact_ledge() const {
  ...
  return (const IVP_Compact_Ledge *)(((const char *)c_tri) -
                                     sizeof(IVP_Compact_Ledge));
}

const IVP_Compact_Poly_Point *IVP_Compact_Edge::get_start_point(
    const IVP_Compact_Ledge *c_ledge) const {
...
const IVP_Compact_Edge *IVP_Compact_Edge::get_next() const {
  ...
  return (IVP_Compact_Edge *)(((char *)this) +
                              ((int *)(((char *)this->next_table) + idx))[0]);
}
...
const IVP_Compact_Edge *IVP_Compact_Edge::get_prev() const {
  hk_intp idx = (hk_intp)(((hk_uintp)this) & 0x0c);
  return (IVP_Compact_Edge *)(((char *)this) +
                              ((int *)(((char *)this->prev_table) + idx))[0]);
}
