...
static inline bool AABB_overlaps_AABB_d(IVP_U_Float_Point* rlb1,
  ...
  if (IVP_Inline_Math::fabsd(pos1->k[0] - pos2->k[0]) >
      (rlb1->k[0] + rlb2->k[0]))
    return false;

  if (IVP_Inline_Math::fabsd(pos1->k[1] - pos2->k[1]) >
      (rlb1->k[1] + rlb2->k[1]))
    return false;

  if (IVP_Inline_Math::fabsd(pos1->k[2] - pos2->k[2]) >
      (rlb1->k[2] + rlb2->k[2]))
    return false;
...
static inline void sphere_against_ledgetree_node(
    const IVP_Compact_Ledgetree_Node* cln, IVP_U_Point* query_pos,
    IVP_DOUBLE query_rad, int current_depth, int max_depth) {
  ...
  if (sphere_overlaps_sphere_d(query_pos, query_rad, &node_pos, node_rad)) {
    if (current_depth < max_depth && !cln->is_terminal()) {
      sphere_against_ledgetree_node(cln->left_son(), query_pos, query_rad,
                                    current_depth + 1, max_depth);
      sphere_against_ledgetree_node(cln->right_son(), query_pos, query_rad,
                                    current_depth + 1, max_depth);
    } else {
 ...
 * for culling against the query sphere, rather than just use the bounding
 * sphere (i.e. sphere at the root of the sphere tree). This should give a
 * better approximation of the object being queried.
 */
static inline IVP_BOOL sphere_against_object(IVP_Real_Object* obj,
                                             IVP_U_Point* query_pos,
                                             IVP_DOUBLE query_rad,
                                             int max_depth) {
  found_valid_sphere_node = IVP_FALSE;

  if (obj->get_type() == IVP_POLYGON) {
    const IVP_Compact_Surface* cs =
        static_cast<IVP_SurfaceManager_Polygon*>(obj->get_surface_manager())
            ->get_compact_surface();
    ...
    return sphere_overlaps_sphere_d(query_pos, query_rad,
                                    obj->get_core()->get_position_PSI(),
                                    obj->get_core()->upper_limit_radius);
  }
...
IVP_BOOL IVP_Sphere_Solver::check_sphere_against_object(
    const IVP_Sphere_Solver_Template* tmpl, IVP_Real_Object* obj) {
  ...
  return sphere_against_object(obj, &obj_center, m_radius,
                               m_max_traversal_depth);
}

void IVP_Sphere_Solver::check_sphere_against_environment(
    const IVP_Sphere_Solver_Template* tmpl, IVP_Environment* env) {
  m_center.set(&tmpl->center);
  m_radius = tmpl->radius;
  m_max_traversal_depth = tmpl->max_traversal_depth;
...
void IVP_Sphere_Solver::recursively_collect_intruding_objects(
    IVP_OV_Node* node, IVP_U_Float_Point* query_rlb,
    IVP_U_Float_Point* query_pos) {
  ...
  node_pos.set(node_luf.k[0] + (node_size / 2.0f),
               node_luf.k[1] + (node_size / 2.0f),
               node_luf.k[2] + (node_size / 2.0f));

  {
    if (AABB_overlaps_AABB_d(&node_rlb, &node_pos, query_rlb, query_pos)) {
      for (int i = 0; i < node->elements.len(); i++) {
        IVP_Real_Object* query_object =
            node->elements.element_at(i)->real_object;
        ...
        if (sphere_overlaps_sphere_d(&m_center, m_radius, pos, rad)) {
          ...
          co->transform_position_to_object_coords(&m_center,
                                                  &query_center_object_space);

          if (sphere_against_object(query_object, &query_center_object_space,
                                    m_radius, m_max_traversal_depth)) {
...

      {
        for (int i = 0; i < node->children.len(); i++) {
          recursively_collect_intruding_objects(node->children.element_at(i),
                                                query_rlb, query_pos);
        }
      }
    }
