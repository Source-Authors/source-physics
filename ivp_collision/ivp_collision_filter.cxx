...
IVP_Collision_Filter::~IVP_Collision_Filter() { return; }

// ***************************************************************************

IVP_BOOL
IVP_Collision_Filter_Coll_Group_Ident::check_objects_for_collision_detection(
...
void IVP_Collision_Filter_Coll_Group_Ident::environment_will_be_deleted(
    IVP_Environment *) {
 ...
 *	Description:	help-class for "Exclusive Pair" collision filter hash
 *table
 ********************************************************************************/
class IVP_CFEP_Objectpair {
 public:
 ...
 *	Description:	"Exclusive Pair" collision filter; stores pairs of
 *object within a fast hash; collision detection will be disabled for these
 *pairs
 ********************************************************************************/
void IVP_Collision_Filter_Exclusive_Pair::generate_hash_entry(
    IVP_Real_Object *obj0, IVP_Real_Object *obj1, IVP_CFEP_Objectpair *entry) {
...
void IVP_Collision_Filter_Exclusive_Pair::disable_collision_between_objects(
    IVP_Real_Object *obj0, IVP_Real_Object *obj1) {
...
void IVP_Collision_Filter_Exclusive_Pair::enable_collision_between_objects(
    IVP_Real_Object *obj0, IVP_Real_Object *obj1) {
  ...
  if (this->hash_table->find(&old_pair)) {  // pair stored in hash?
    IVP_CFEP_Objectpair *removed_pair =
        this->hash_table->remove(&old_pair);  // yes, remove...
...
IVP_BOOL
IVP_Collision_Filter_Exclusive_Pair::check_objects_for_collision_detection(
    IVP_Real_Object *obj0, IVP_Real_Object *obj1) {
...
void IVP_Collision_Filter_Exclusive_Pair::environment_will_be_deleted(
    IVP_Environment *) {
  delete (this);
  return;
}
...
void IVP_Meta_Collision_Filter::add_collision_filter(
    IVP_Collision_Filter *filter) {
  filter_set.add(filter);
}

void IVP_Meta_Collision_Filter::remove_collision_filter(
    IVP_Collision_Filter *filter) {
  filter_set.remove(filter);
}

IVP_BOOL IVP_Meta_Collision_Filter::check_objects_for_collision_detection(
    IVP_Real_Object *object0, IVP_Real_Object *object1) {
  int do_collision = 1;
  ...
  for (i = filter_set.len() - 1; i >= 0; i--) {
    ...
    do_collision =
        do_collision & (int)coll_filter->check_objects_for_collision_detection(
                           object0, object1);
  }
...
IVP_Meta_Collision_Filter::~IVP_Meta_Collision_Filter() { return; }
...
void IVP_Meta_Collision_Filter::environment_will_be_deleted(
    IVP_Environment *env) {
  int i;
  for (i = filter_set.len() - 1; i >= 0; i--) {
    IVP_Collision_Filter *coll_filter = filter_set.element_at(i);
