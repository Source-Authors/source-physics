...
struct IVP_OV_Node_Data {
  ...
  int z;  // -> (2^rasterlevel) * x/y/z = world coordinates of upper-left-front
          // corner of cube
...
class IVP_OV_Tree_Manager {
  ...
  inline IVP_DOUBLE power2(int x) const { return (this->powerlist[x + 40]); }

  IVP_DOUBLE calc_optimal_box(const IVP_OV_Element *element,
                              IVP_DOUBLE min_radius, IVP_DOUBLE max_radius);
  IVP_BOOL box_contains_box(const struct IVP_OV_Node_Data *master_data,
                            const IVP_OV_Node *sub_node,
                            int const rasterlevel_diff) const;
  IVP_BOOL box_overlaps_with_box(const IVP_OV_Node *largenode,
                                 const IVP_OV_Node *smallnode,
                                 int const rasterlevel_diff) const;
  IVP_OV_Node *find_smallest_box(const IVP_OV_Node *master_node,
                                 const IVP_OV_Node *sub_node) const;
  void connect_boxes(IVP_OV_Node *node, IVP_OV_Node *new_node);
  void expand_tree(const IVP_OV_Node *new_node);
  void collect_subbox_collision_partners(const IVP_OV_Element *center_element,
                                         const IVP_OV_Node *node);
  void collect_collision_partners(const IVP_OV_Element *center_element,
