...
class IVP_U_Matrix_Cache {
 ...
 public:
  ...
  IVP_U_Matrix *calc_matrix_at(IVP_Time t, int index) {
    ...
    IVP_ASSERT(((int)((t - base_time) * (IVP_3D_SOLVER_MAX_STEPS_PER_PSI *
 ...
 public:
  IVP_U_Matrix_Cache(IVP_Cache_Object *co) { p_init(co); }
...
class IVP_3D_Solver {
 protected:
  ...
  virtual IVP_DOUBLE get_value(IVP_U_Matrix *A_w_f_c,
                               IVP_U_Matrix *B_w_f_c) = 0;
 ...
 public:
  ...
  void set_max_deviation(IVP_DOUBLE dev) {
    max_deviation = dev;
    inv_max_deviation = 1.0f / max_deviation;
  }
  ...
  IVP_BOOL find_first_t_for_value_no_zero_dev(
      ...
      int t_now_cache_index,  // set to 0 if cache base is set to t_now
      IVP_U_Matrix_Cache *mc_A, IVP_U_Matrix_Cache *mc_B,
      IVP_DOUBLE *opt_val_at_t_now, IVP_DOUBLE *opt_val_at_next_psi,
      IVP_Time *t_out);
  ...
  IVP_BOOL find_first_t_for_value_coll(
      ...
      IVP_Time t_now, IVP_Time t_max, IVP_U_Matrix_Cache *mc_A,
      IVP_U_Matrix_Cache *mc_B, IVP_DOUBLE *opt_val_at_t_now, IVP_Time *t_out);
};
