...
class IVP_Contact_Point;  // a kind a (void *) pointer to identify friction
                          // points
...
class IVP_Contact_Point_API {
 public:
   ...
   *	Description:	returns the integrated kinetic energy, which is
   *destroyed by this friction handle. The value may jitter, as the PSI and
   ...
   *	Description:	the integrated destroyed energy is stored only as a
   *float So from time to time you have to reset this value in order not to
   *loose precision.
   ********************************************************************************/
  ...
  /********************************************************************************
   *	Name:	       	get_vert_force
   *	Description:	returns the vertical force between the two involved
  ...
  static void get_surface_normal_ws(const IVP_Contact_Point* friction_handle,
...
class IVP_Listener_Collision {
 ...
 public:
  ...
  // collision: set the IVP_LISTENER_COLLISION_CALLBACK_COLLISION bit in the
  // constructor if you want to use the next callback.
  ...
  // friction: set the IVP_LISTENER_COLLISION_CALLBACK_FRICTION bit in the
  // constructor if you want to use the next callback.
  ...
  IVP_Listener_Collision(
      int /* IVP_LISTENER_COLLISION_CALLBACKS */ enable_callbacks = 1) {
    enabled_callbacks =
        enable_callbacks;  // set flags for implemented callbacks
  }
