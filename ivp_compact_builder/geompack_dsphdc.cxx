...
void IVP_Geompack::dsphdc_() {
  ...
  /*        FACESDATA(1,1:NFACE+1) - head pointer to vertex indices in
   * FACEVERTICESLIST for */
  /*              each face; 1 = FACESDATA(1,1) < ... < FACESDATA(1,NFACE+1) */
  /*        FACEVERTICESLIST(1,1:*) - vertex indices; those for Ith face are in
   * FACEVERTICESLIST(1,J) */
  /*              for J = FACESDATA(1,I),...,FACESDATA(1,I+1)-1 */
  /*        POLYHEDRONFIRSTFACEOFFSET(1:NPOLH+1) - head pointer to face indices
   * in POLYHEDRONFACEINDICES for each */
  /*              polyhedron; 1 = POLYHEDRONFIRSTFACEOFFSET(1) <
   * POLYHEDRONFIRSTFACEOFFSET(2) < ... < POLYHEDRONFIRSTFACEOFFSET(NPOLH+1) */
  /*        POLYHEDRONFACEINDICES(1,1:*) - signed face indices; those for Ith
   * polyh are in */
  /*              POLYHEDRONFACEINDICES(1,J) for J =
   * POLYHEDRONFIRSTFACEOFFSET(I),...,POLYHEDRONFIRSTFACEOFFSET(I+1)-1; the face
   * index */
  /*              must be negated if the ordering of vertices for the face */
  /*              in FACEVERTICESLIST is in CW order when viewed from outside
   * Ith polyh */
  /*        HASHTABLE_SIZE - size of hash table HT; should be a prime number
   * which */
  /*              is >= NVC+2 */
  ...
  /*     Output parameters: */
  /*        FACESDATA(1:3,1:NFACE) - FACESDATA(1,F) same as input;
   * FACESDATA(2,F) and */
  ...
  /*              (positive) or CW (negative) in FACEVERTICESLIST when viewed
   * from */
  /*              outside polyh; if interior face, 2 signs are different */
  /*        NORMALS(1:3,1:NFACE) - normals at faces; NORMALS(*,F) is unit
   * outward */
  /*              normal of face F with its vertices oriented CCW when */
  /*              viewed from outside polyhedron |FACESDATA(2,F)| */
  /*        FACEVERTICESLIST(1:6,1:NVERT) - face vertex list where NVERT =
   * FACESDATA(1, */
  ...
  /*              V = FACEVERTICESLIST(SUCC,U). Let LU =
   * FACEVERTICESLIST(LOC,U), LV = FACEVERTICESLIST(LOC,V), */
  ...
  /*              X = FACEVERTICESLIST(SUCC,W). If (LV-LU)*SF > 0, then
   * FACEVERTICESLIST(EDGC,U) = W, */
  /*              FACEVERTICESLIST(EDGA,W) = U, and EDGE_ANGLES(U) is angle at
   * UV between the */
  /*              2 faces inside P; else FACEVERTICESLIST(EDGA,U) = W,
   * FACEVERTICESLIST(EDGC,W) = U, */
   ...
   */
  /*              EDGE_ANGLES(J) corresponds to FACEVERTICESLIST(*,J) and is
   * determined by */
  /*              EDGC field */
  /*        POLYHEDRONFACEINDICES(1:2,1:NPF) - row 1 same as input and row 2
   * used for link, */
  /*              where NPF = POLYHEDRONFIRSTFACEOFFSET(NPOLH+1)-1 */

  /*     Working parameters: */
  /*        HT(0:HASHTABLE_SIZE-1),EDGE(1:4,1:MAXEDG) - hash table and edge
   * records */
      ...
      IVP_IF(1) {
        IVP_IFDEBUG(IVP_DM_GEOMPACK_LEVEL1, {
          ivp_debugmanager.dprint(
              ...
              "*** GEOMPACK: face oriented same way twice in routine DSPHDC\n");
        })
      }
      return;
    }
    ...
    for (i__ = polyhedronfirstfaceoffset[p]; i__ <= i__2; ++i__) {
      ...
      for (j = facesdata[f * 3 + 1]; j <= i__3; ++j) {
        ...
        if (k <= 0) {
          ...
          dotp = normals[f * 3 + 1] * normals[g * 3 + 1] +
                 normals[f * 3 + 2] * normals[g * 3 + 2] +
                 normals[f * 3 + 3] * normals[g * 3 + 3];
          ...
          for (l = 1; l <= 3; ++l) {
            ac[l - 1] = (vcl[l + lb * 3] - vcl[l + la * 3]) * .5 + en[l - 1];
            /* L100: */
          }
    ...
    if (nht != 0) {
      ...
      IVP_IF(1) {
        IVP_IFDEBUG(IVP_DM_GEOMPACK_LEVEL1, {
          ivp_debugmanager.dprint(
              IVP_DM_GEOMPACK_LEVEL1,
              "*** GEOMPACK: unmatched edge determined by routine DSPHDC\n");
        })
      }
      return;
    }
