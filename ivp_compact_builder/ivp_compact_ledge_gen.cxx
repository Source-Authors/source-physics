...
IVP_Compact_Ledge_Generator::IVP_Compact_Ledge_Generator() {
  ...
  orig_triangles = nullptr;
}

int IVP_Compact_Ledge_Generator::prepare_compact_ledge(
...
IVP_RETURN_TYPE IVP_Compact_Ledge_Generator::validate() {
  ...
  for (i = 0; i < n_triangles; i++) {
    ...
    for (j = 0; j < 3; edge = edge->next, j++) {
      ...
      const IVP_Compact_Poly_Point *c_po =
          IVP_CLS.give_object_coords(c_edge->get_opposite(), compact_ledge);
      ...
      const IVP_Compact_Poly_Point *c_point =
          IVP_CLS.give_object_coords(c_edge, compact_ledge);
