...
void IVP_Poly_Point::print(const char *text) {
  ivp_message("num %i: ", this->point_num());
...
void p_del_double_triangles(IVP_Triangle **tri_a, IVP_Triangle **tri_b = NULL,
                            IVP_Triangle **tri_c = NULL);
void p_del_double_triangles(IVP_Triangle **tri_a, IVP_Triangle **tri_b,
                            IVP_Triangle **tri_c) {
...
int p_check_for_flat(IVP_Tri_Edge *a0, IVP_Tri_Edge *b0, IVP_DOUBLE eps) {
  IVP_DOUBLE dist1 =
      -a0->triangle->tmp.gen.hesse.get_dist(b0->prev->start_point);
  IVP_DOUBLE dist2 =
      -b0->triangle->tmp.gen.hesse.get_dist(a0->prev->start_point);
...
int IVP_Triangle::print(const char *comment) {
  ...
  ivp_message("%s '%s' T(%d %d %d) \n", (comment) ? comment : "", obj_name,
...
int IVP_Object_Polygon_Tetra::check_concavity_and_manage(IVP_Tri_Edge *edge,
                                                         P_CONVEXIFY_STATE) {
...
int IVP_Tri_Edge::check_concavity(IVP_Tri_Edge *other_edge) {
  ...
  IVP_DOUBLE concav =
      -this->triangle->tmp.gen.hesse.get_dist(other_edge->prev->start_point);
  {
    IVP_DOUBLE concav2 =
        -other_edge->triangle->tmp.gen.hesse.get_dist(this->prev->start_point);
    if (concav2 > concav) {
      concav = concav2;
    }
  }

  IVP_DOUBLE scal = this->triangle->tmp.gen.hesse.dot_product(
      &other_edge->triangle->tmp.gen.hesse);

  if (scal > 0.0f) {
    // angle greater than 90 degrees or greater 270

    if (scal < 1.0f - P_Pop_Scal_Eps ||
        IVP_Inline_Math::fabsd(concav) >= P_POP_SURFACE_CONVEX_BLUR) {
...
#ifdef SUR_DEBUG
  ivp_message("TRIANGLE GENERATED: %d, %d, %d\n", pn0, pn1, pn2);
#endif
}
...
#ifdef SUR_DEBUG
  ivp_message("    Line generated: %d, %d\n", sp->point_num, ep->point_num);
#endif
}
...
int P_Sur_2D_Line::point_lies_to_the_left(IVP_U_Point *i_point) {
  ...
  if (hesse_dist <= P_Pop_Too_Flat_Eps)
    return 2;  // not enough distance for triangle
  return 1;
}

int P_Sur_2D_Line::has_points(P_Sur_2D_Point *point_a,
                              P_Sur_2D_Point *point_b) {
  if (!point_b) {
...
IVP_DOUBLE P_Sur_2D_Line::dist_to_point(IVP_U_Point *i_point) {
  ...
  IVP_DOUBLE hesse_dist =
      delta_y * (start_point->k[0] - i_point->k[0]) +
...
int P_Sur_2D_Line::point_lies_in_interval(IVP_U_Point *i_point) {
  ...
  if (line_abs_delta > P_DOUBLE_EPS) {
    ...
    if (point_delta > P_CROSS_EPS &&
        point_delta < line_abs_delta - P_CROSS_EPS) {
...
int P_Sur_2D_Line::is_crossing_line(P_Sur_2D_Line *line_v) {
  ...
  P_Sur_2D_Line *line_u = this;
#ifdef SUR_DEBUG
  ivp_message("    crossing (%d, %d) with (%d, %d): ",
              line_u->start_point->point_num, line_u->end_point->point_num,
              line_v->start_point->point_num, line_v->end_point->point_num);
#endif
...
#define DET_EPS 0.000000001f
...
#ifdef SUR_DEBUG
    ivp_message("det < det_eps: ");
#endif
    // linear dependency: on the line or just parallel?
    if (IVP_Inline_Math::fabsd(line_u->dist_to_point(line_v->start_point)) <
        P_CROSS_EPS_QUAD) {
#ifdef SUR_DEBUG
      ivp_message("on the line: ");
#endif
      if (line_u->overlaps_with_line(line_v)) {
#ifdef SUR_DEBUG
        ivp_message("overlapping (crossing)!\n");
#endif
        return 1;  // lines are overlapping
      } else {
#ifdef SUR_DEBUG
        ivp_message("not overlapping (no crossing)!\n");
#endif
        return 0;
      }
    } else {
#ifdef SUR_DEBUG
      ivp_message("really parallel.\n");
#endif
...
#ifdef SUR_DEBUG
    ivp_message("common end/start point -> no crossing\n");
#endif
  ...
  IVP_DOUBLE u_t =
      v_dx * o_dy - v_dy * o_dx;  // u_sp + u_t*D*(u_ep-u_sp) = crossing
#ifdef SUR_DEBUG
  ivp_message("u_t %g: ", u_t / D);
#endif
  if (u_t < left_border || u_t > right_border) {
#ifdef SUR_DEBUG
    ivp_message("no crossing.\n");
#endif
    return 0;  // crosses beyond line interval
  }
  // check crossing on line_v
  IVP_DOUBLE v_t =
      u_dx * o_dy - o_dx * u_dy;  // v_sp + v_t*D*(v_ep-v_sp) = crossing
#ifdef SUR_DEBUG
  ivp_message("v_t %g: ", v_t / D);
#endif
...
P_Sur_2D::P_Sur_2D(IVP_Object_Polygon_Tetra *tetras_,
...
IVP_ERROR_STRING P_Sur_2D::calc_line_representation() {
  ...
  ivp_message(
      ...
      (int)pop, sur->get_surface_index(), sur->n_lines);
#endif
...
#ifdef SUR_DEBUG
  ivp_message(
      "hesse: %g %g %g; max_koord: %d; take koords %d %d; mirrored: %d\n",
      hesse->k[0], hesse->k[1], hesse->k[2], max_koord, td_k0, td_k1,
      trafo_is_mirrored);
#endif
  if (max_koord == 1) trafo_is_mirrored = 1 - trafo_is_mirrored;

  // transfer line and point info
  P_Sur_2D_Point **point_hash_array =
      (P_Sur_2D_Point **)p_calloc(pop->n_points, sizeof(P_Sur_2D_Point *));
  P_FREE(this->line_array);
  this->line_array =
      (P_Sur_2D_Line **)p_calloc(sur->n_lines, sizeof(P_Sur_2D_Line *));
  P_FREE(this->point_array);
  this->point_array =
      (P_Sur_2D_Point **)p_calloc(sur->n_lines, sizeof(P_Sur_2D_Point *));
  int point_count = 0;
  int i;
...
int p_count_reachable(P_Sur_2D_Point *i_point) {
  ...
  for (point = i_point; point && !point->was_reached;
...
IVP_DOUBLE p_ab_quad_length(P_Sur_2D_Point *pa, P_Sur_2D_Point *pb,
                            P_Sur_2D_Point *pc) {
...
IVP_ERROR_STRING P_Sur_2D::calc_triangle_representation() {
  ...
  int reached_points_counter =
      p_count_reachable(this->lines.first->start_point);
...
#ifdef SUR_DEBUG
  ivp_message("   Having islands would be: %d\n", having_islands);
#endif
  ...
  while ((td_base_line = this->lines.first)) {
#ifdef SUR_DEBUG
    ivp_message("TAKE BASE LINE: (%d %d)\n",
                td_base_line->start_point->point_num,
                td_base_line->end_point->point_num);
#endif
    if (loop_counter++ >
        100) {  //@@@@SF: changed from 100000 to 100 [05.03.2000]
      // ivp_message("Degenerated polygon found:\n");
      //	    P_Sur_2D_Point *point_a = td_base_line->start_point; // a b
      // c form a triangle 	    P_Sur_2D_Point *point_b =
      // td_base_line->end_point; 	    ivp_message("  %f %f %f\n", point_a->k[0],
      //point_a->k[1], point_a->k[2]); 	    ivp_message("  %f %f %f\n",
      // point_b->k[0], point_b->k[1], point_b->k[2]);
      P_Sur_2D_Line *td_point_line;
      for (td_point_line = this->lines.first; td_point_line;
           td_point_line = td_point_line->next) {
        P_Sur_2D_Point *point_c = td_point_line->start_point;
        ivp_message("  %f %f %f\n", point_c->k[0], point_c->k[1],
                    point_c->k[2]);
      }
    ...
    P_Sur_2D_Point *point_a =
        td_base_line->start_point;  // a b c form a triangle
    P_Sur_2D_Point *point_b = td_base_line->end_point;
    P_Sur_2D_Line *td_point_line;
    for (td_point_line = this->lines.first; td_point_line;
         td_point_line = td_point_line->next) {
      P_Sur_2D_Point *point_c = td_point_line->start_point;
      ab_min_hash.add((void *)td_point_line,
                      p_ab_quad_length(point_a, point_b, point_c));
    }

    // now try triangles in ascending side length order
    for (td_point_line = (P_Sur_2D_Line *)ab_min_hash.find_min_elem();
         td_point_line; ab_min_hash.remove_min(),
        td_point_line = (P_Sur_2D_Line *)ab_min_hash.find_min_elem()) {
      if (td_point_line == td_base_line) continue;
      if (td_point_line->start_point == td_base_line->end_point) continue;
#ifdef SUR_DEBUG
      ivp_message("  Trying point %d: ", td_point_line->start_point->point_num);
#endif
...
#ifdef SUR_DEBUG
        if (dist_flag == 2) {
          ivp_message("lies to near for triangle!");
        } else {
          ivp_message("lies on wrong side.\n");
        }
#endif
        continue;
      }
      ...
      for (td_cross_line = this->lines.first; td_cross_line;
      ...
      if (having_islands) {
        ...
        for (td_inside_line = this->lines.first; td_inside_line;
      ...
      for (td_ident_line = this->lines.first; td_ident_line;
           td_ident_line = td_ident_line_next) {
        td_ident_line_next = td_ident_line->next;
        // if( td_ident_line->has_points(point_c, point_a) &&
        // td_ident_line->has_points(point_c, point_b) ){
        //    ivp_message("scheibenkleister!\n");
        // }
        if (td_ident_line->has_points(point_c, point_a)) {
...
void ivp_check_for_opposite(IVP_Hash *hash, IVP_Poly_Point *p0,
                            IVP_Poly_Point *p1, IVP_Tri_Edge *edge) {
...
IVP_Object_Polygon_Tetra::IVP_Object_Polygon_Tetra(
  ...
  this->points =
      (IVP_Poly_Point *)p_calloc(sizeof(IVP_Poly_Point), this->n_points);
  int i;
  ...
  this->surfaces =
      (IVP_Poly_Surface *)p_calloc(sizeof(IVP_Poly_Surface), this->n_surfaces);
...
int IVP_Poly_Surface::get_surface_index() { return this - tetras->surfaces; }
...
IVP_ERROR_STRING IVP_Object_Polygon_Tetra::make_triangles() {
  ...
  IVP_Hash *hash = new IVP_Hash(num_of_edges /*size*/,
                                sizeof(void *) * 2 /*keylen*/, 0 /*notfound*/);
  {
    int i;
    IVP_Tri_Edge *edge;
    for (i = templ->n_surfaces - 1, sur = &templ->surfaces[0]; i >= 0;
         i--, sur++) {
      ...
      for (td_tri = td_sur->triangles.first; td_tri; td_tri = td_tri->next) {
        ...
        // clockwise ?
        {
          ...
          if (scal < 0.0f) {
            ...
            po3 = po2;
            po2 = h;
            // ivp_message("Points swapped for clockwise.\n");
          }
        }
...
IVP_ERROR_STRING IVP_Object_Polygon_Tetra::final_convexify_check() {
  ...
  for (tri = ph->triangles.first; tri; tri = tri->next) {
    ...
    for (i = 0; i < 3; i++, e = e->next) {
      ...
      if (conc_flag == -2) {
        error_flag = "error";
        ivp_message(
            "convex_test: tri->edge: tmp.gen.concav_flag == -2 (partly "
            "overlapping)");
        tri->print("\n");
      }
      if (conc_flag == -1) {
      ...
      if (conc_flag == 0) {
        ...
        if (!tri->flags.is_terminal &&
            (e->opposite->triangle->flags.is_hidden ||
             e->triangle->flags.is_hidden)) {
          ivp_message(
              "convex_test:  warning edge is convex, but neighbours are "
              "hidden.");
          tri->print("\n");
        }
      }
  ...
  if (!error_flag) {
    // ivp_message("final_convexify_check: OK: %i triangles, %i extrapoints.\n",
...
IVP_ERROR_STRING IVP_Object_Polygon_Tetra::check_konsistency_of_triangles() {
  ...
  for (tri = ph->triangles.first; tri; tri = tri->next) {
    ...
    if (tri->calc_areasize() < 1E-6f) {
      error_flag = "error";
      ivp_message("pysic_test: tri->calc_areasize() < 1E-6f in triangle: %g",
                  tri->calc_areasize());
      tri->print("\n");
    }
    ...
    if (tri->flags.is_hidden) {
      ...
      if (!tri->flags.is_terminal && ((e->opposite->prev->start_point !=
                                       e->next->opposite->prev->start_point) ||
                                      (e->opposite->prev->start_point !=
                                       e->prev->opposite->prev->start_point))) {
        error_flag = "error";
        ivp_message(
            "physic_test:  triangle is hidden, but neighbours don't form a "
            "tetraeder.");
        tri->print("\n");
      }
      ...
      if (!(e->opposite->triangle->flags.is_hidden &&
        ...
        error_flag = "error";
        ivp_message(
            "physic_test:  triangle is hidden, but not all neighbours are "
            "hidden.");
        tri->print("\n");
      }
    } else {
    ...
    for (i = 0; i < 3; i++, edge = edge->next) {
      ...
      if (tri->flags.is_hidden && edge->tmp.gen.hash_class) {
        error_flag = "error";
        ivp_message("physic_test: tri->is_hidden && in hash");
        tri->print("\n");
      }
      ...
      if (edge->start_point != edge->opposite->next->start_point) {
        error_flag = "error";
        ivp_message(
            "physic_test: edge->start_point != "
            "edge->opposite->next->startpoint in triangle ");
        edge->print("\n");
      }
      if (edge != edge->other_side()->other_side()) {
        error_flag = "error";
        ivp_message(
            "physic_test: edge != edge->other_side()->other_side() in "
            "triangle ");
        edge->print("\n");
      }
      ...
      if (edge->triangle == edge->opposite->triangle) {
        error_flag = "error";
        ivp_message(
            "physic_test: edge->triangle == edge->opposite->triangle in "
            "triangle ");
        edge->print("\n");
      }
      if (edge->behind->other_side()->behind != edge->other_side()) {
        error_flag = "error";
        ivp_message(
            "physic_test: edge->behind->other_side()->behind = "
            "edge->other_side in triangle ");
        edge->print("\n");
      }
      if (edge->triangle != tri) {
        error_flag = "error";
        ivp_message("physic_test: edge->triangle != tri in triangle ");
        edge->print("\n");
      }
      if ((edge->behind->start_point != edge->start_point) ||
          (edge->behind->next->start_point != edge->next->start_point)) {
        error_flag = "error";
        ivp_message(
            "physic_test: (edge->behind->sp != edge->sp) || "
            "(edge->behind->next->sp != edge->next->sp) in triangle ");
        tri->print("\n");
      }
    }
  }
  ...
  if (ph->triangles.len != n_triangles_check) {
    ivp_message("physik_test: Anzahl der Triangles (%d) != n_triangles (%d)!\n",
  ...
  for (tri = this->triangles.first; tri; tri = tri->next) {
    ...
    for (i = 2; i >= 0; edge = edge->next, i--) {
      if (hk_Math::almost_equal(edge->concavity, uninit) &&
        ...
        int flag =
            this->check_concavity_and_manage(edge, P_CONVEXIFY_STATE_INIT);
        if (flag == -1) {
          ivp_message("Terminal object has identical triangles!!!\n");
          CORE;
        }
      }
    }
...
void IVP_Object_Polygon_Tetra::move_edge_to_problem_hash(IVP_Tri_Edge *edge) {
  ...
  this->tetra_intrude->checkin_edge(
      edge);  // intrusion check with visible edges only

  IVP_DOUBLE rating =
      -edge->start_point->quad_distance_to(edge->next->start_point);  // length
  add_edge_to_min_list(edge, P_HASH_CLASS_PROBLEM, rating);
}

void IVP_Object_Polygon_Tetra::move_edge_to_convex_intrude_hash(
    IVP_Tri_Edge *edge) {
  IVP_Tri_Edge *opp = edge->opposite;
  this->remove_edge_from_min_list(edge);
...
void IVP_Object_Polygon_Tetra::move_edge_to_epsilon_hash(IVP_Tri_Edge *edge) {
  ...
  if (edge->triangle->flags.is_hidden) return;
  this->tetra_intrude->checkin_edge(
      edge);  // intrusion check with convex edges only
...
void IVP_Object_Polygon_Tetra::add_edge_to_min_list(IVP_Tri_Edge *edge,
                                                    P_HASH_CLASS hash_class,
                                                    IVP_DOUBLE rating) {
...
IVP_BOOL IVP_Object_Polygon_Tetra::p_link_edge(IVP_Tri_Edge *edge,
                                               IVP_Tri_Edge *neighb) {
  ...
  IVP_ASSERT(neighb_opp != edge);  // questionable; maybe just do a return?
  {
    ...
    for (test_edge = edge->next; test_edge != edge;
  ...
  [[maybe_unused]] int flag =
      this->check_concavity_and_manage(edge, P_CONVEXIFY_STATE_LINK);
  // if (flag <0) retval = IVP_TRUE;
  flag =
      this->check_concavity_and_manage(orig_edge_oppo, P_CONVEXIFY_STATE_LINK);
  // if (flag <0) retval = IVP_TRUE;
  return retval;
}

int IVP_Object_Polygon_Tetra::link_triangle_couple(IVP_Triangle *triangle,
  ...
  IVP_ASSERT(!flag);
  flag = p_link_edge(triangle->three_edges[0].prev, neighbor_2);  //-V1048
  IVP_ASSERT(!flag);
...
IVP_Tri_Edge *IVP_Object_Polygon_Tetra::get_an_edge_with_points(
    IVP_Poly_Point *p1, IVP_Poly_Point *p2) {
  IVP_Poly_Point *p[2];
  int i = 0;
  if (p1 > p2) i = 1;
  p[i] = p1;
  ...
  IVP_Tri_Edge *found_edge =
      (IVP_Tri_Edge *)this->points_to_edge_hash->find((char *)&p[0]);
  return found_edge;
}

void IVP_Object_Polygon_Tetra::add_edge_into_point_to_edge_hash(
    IVP_Tri_Edge *edge) {
...
IVP_Triangle *IVP_Object_Polygon_Tetra::generate_double_triangle(
    IVP_Poly_Point *p1, IVP_Poly_Point *p2, IVP_Poly_Point *p3) {
  ...
  i_e0->prev = i_e1;
  i_e0->start_point = p2;
  i_e0->triangle = i_tri_a;
  i_e1->next = i_e0;
  i_e1->prev = i_e2;
  i_e1->start_point = p3;
  i_e1->triangle = i_tri_a;
  i_e2->next = i_e1;
  i_e2->prev = i_e0;
  i_e2->start_point = p1;
  i_e2->triangle = i_tri_a;

  tri_a->other_side = i_tri_a;
  i_tri_a->other_side = tri_a;

  p_link_triangle_self(tri_a);
  p_link_triangle_self(i_tri_a);
...
void IVP_Object_Polygon_Tetra::make_double_triangle_permanent(
...
IVP_DOUBLE p_calc_min_intrude_dist(IVP_Triangle *tri,
                                   IVP_Intrusion_Status *stat,
  ...
  for (intersect = stat->intersections; intersect;
       intersect = intersect->next) {
    if (intersect->type != IVP_INTRUSION_CHECK_OVERLAP) {
      IVP_DOUBLE dist =
          tri->tmp.gen.hesse.get_dist(&intersect->intersect_point);
      if (dist < min) min = dist;
    }
  }
  return min;
...
void IVP_Object_Polygon_Tetra::calc_extrusion_point(const IVP_Tri_Edge *edge,
...
void IVP_Object_Polygon_Tetra::pop_problematic_edge(IVP_Tri_Edge *edge) {
  ...
  {
    IVP_DOUBLE dist0 =
        edge->triangle->tmp.gen.hesse.get_dist(oppo->prev->start_point);
    IVP_DOUBLE dist1 =
        oppo->triangle->tmp.gen.hesse.get_dist(edge->prev->start_point);
...
#ifdef CONVEX_DEBUG
    ivp_message(
        "***** Problematic %s,pop: edge %i %i, other_p %i %i, dist %f\n",
        (edge->tmp.gen.concav_flag == 0) ? "convex" : "CONCAV",
        edge->start_point->point_num(), oppo->start_point->point_num(),
        edge->prev->start_point->point_num(),
        oppo->prev->start_point->point_num(), dist0);
#endif
  }
  ...
  if (!i_got_a_real_problem &&
      edge->tmp.gen.concav_flag != 0) {  // check flatness of concav edges
    ...
    IVP_Triangle *tri_a = generate_double_triangle(
        b2->start_point, a2->start_point, a0->start_point);
    IVP_Triangle *tri_b = generate_double_triangle(
        a2->start_point, b2->start_point, a1->start_point);

    // check wether pop becomes too flat
    int pop_too_flat_flag = 0;
    pop_too_flat_flag = p_check_for_flat(
        &tri_a->three_edges[0], &tri_b->three_edges[0], P_Pop_Too_Flat_Eps);
    if (pop_too_flat_flag == 1) {
#ifdef CONVEX_DEBUG
      ivp_message("problematic pop would be too flat.\n");
#endif
  ...
  if (!i_got_a_real_problem) {
    ...
    IVP_Tri_Edge *a2 = a1->next;
    // perform pyramid pop
    IVP_Triangle *tri_pa = generate_double_triangle(
        a0->next->start_point, a0->start_point, extra_point);
    IVP_Triangle *tri_pb = generate_double_triangle(
        a1->next->start_point, a1->start_point, extra_point);
    IVP_Triangle *tri_pc = generate_double_triangle(
        a2->next->start_point, a2->start_point, extra_point);

    // check wether an epsilon problem occurred
    IVP_INTRUSION_CHECK_RESULTS i_flag = this->tetra_intrude->check_intrusion(
        a0, tri_pa->three_edges[0].prev, tri_pb->three_edges[0].next,
        tri_pc->three_edges[0].next, extra_points, 3);
    if (i_flag) {
#ifdef CONVEX_DEBUG
      ivp_message("********* Epsilon problem with extra point. P_Pop_Eps: %f\n",
                  P_Pop_Eps);
#endif
      ...
      this->link_triangle_couple(tri_pc, a2, tri_pb->three_edges[0].prev,
                                 tri_pa->three_edges[0].next);
...
#ifdef CONVEX_DEBUG
    ivp_message(
        "********* Epsilon problem moved to epsilon hash: epsilon = %f\n",
        P_Pop_Eps);
#endif
...
void IVP_Object_Polygon_Tetra::record_intruding_convex_edges(
...
void IVP_Object_Polygon_Tetra::link_existing_pop_edge(IVP_Tri_Edge *pop_edge) {
  ...
  while (1) {
    ...
    if (p_ep == n_ep) {
      ...
      ivp_message(
          "\n**********************************************pop edge already "
          "existed in neighbour region -> linked it.\n");
      e->print("e");
      pop_edge->print("e0");
      ivp_message("\n");
#endif
...
void IVP_Object_Polygon_Tetra::pop_concav_edge(IVP_Tri_Edge *edge) {
...
#ifdef CONVEX_DEBUG
  ivp_message("Try pop: edge %d %d with %d & %d. concavity %g)",
              edge->start_point->point_num(),
              edge->next->start_point->point_num(),
              edge->prev->start_point->point_num(),
              edge->opposite->prev->start_point->point_num(), edge->concavity);
#endif
  ...
  IVP_INTRUSION_CHECK_RESULTS intrusion_val;
  {
    ...
    intrusion_val = this->tetra_intrude->check_intrusion(
        a0, b0, otherside0, otherside1, this->extra_points, n_new_triangles);
  }
  ...
  if ((intrusion_val != IVP_INTRUSION_CHECK_NONE)) {
    ...
    if (intrusion_val != IVP_INTRUSION_CHECK_EPSILON) {
      ...
      IVP_Intrusion_Status *status = this->tetra_intrude->calc_intrusion_status(
          a0, b0, tri_first->three_edges[0].other_side(),
          tri_second->three_edges[0].other_side(), this->extra_points,
          n_new_triangles);
...
#ifdef CONVEX_DEBUG
      ivp_message(
          "\n            epsilon violation. smaller epsilon will work. shifted "
          "to eps hash.\n");
#endif
      this->move_edge_to_epsilon_hash(edge);
    }
...
void IVP_Object_Polygon_Tetra::convexify() {
  ...
  IVP_Tetra_Point *t4_points =
      (IVP_Tetra_Point *)p_calloc(sizeof(IVP_Tetra_Point), n_malloced_points);
  int i;
  ...
  while (1) {
    ...
    ivp_message("%i:", num_of_loops);
    for (i = P_HASH_CLASS_NORMAL; i < P_HASH_CLASS_MAX; i++) {
      ivp_message(" %i", min_hash[i]->counter);
    }
    ivp_message(":   ");
    num_of_loops++;
    if (num_of_loops == n_physical_pops - 1) {
      ivp_message("Nearly Last Loop!\n");
    }

    if (num_of_loops == n_physical_pops) {
      ivp_message("P_CONVEX_LOOPS reached!\n");
      break;
    }
#endif
    ...
    IVP_Tri_Edge *edge =
        (IVP_Tri_Edge *)min_hash[P_HASH_CLASS_NORMAL]->find_min_elem();
    if (edge) {
    ...
    if (min_hash[P_HASH_CLASS_EPSILON]->counter &&
        (P_Pop_Eps >= P_DOUBLE_EPS)) {
      P_Pop_Eps *= P_POP_EPS_REDUCE_FACTOR;
      P_Pop_Too_Flat_Eps = P_Pop_Eps * P_POP_TOO_FLAT_EPS_FACTOR;
      if (P_Pop_Eps < P_DOUBLE_EPS) {
        ivp_message("*** P_Pop_Eps went smaller than P_DOUBLE_EPS!!\n");
        continue;
      }
#ifdef CONVEX_DEBUG
      ivp_message(
          "#############################    Reduced P_Pop_Eps to %f. Hashes "
          "are recalculated\n",
          P_Pop_Eps);
#endif
      // calc_concavities();
    ...
    // ********** take an existing problematic edge
    {
      ...
      IVP_Tri_Edge *edge2 =
          (IVP_Tri_Edge *)min_hash[P_HASH_CLASS_CONVEX_INTRUDE]
              ->find_min_elem();
      if (edge) {
        if (edge2) {
          if (min_hash[P_HASH_CLASS_PROBLEM]->find_min_value() >
              min_hash[P_HASH_CLASS_CONVEX_INTRUDE]->find_min_value()) {
            edge = edge2;
          }
        }
      } else {
    ...
    //	ivp_message("No edges left in all three min hashes.\n");
    break;
  }
...
#ifdef CONVEX_DEBUG
  ivp_message("Convexify finished.\n");
#endif
}
  ...
  // insert all pierce infos
  {
    ...
    for (tri = triangles.first; tri; tri = tri->next) {
      ...
      if (tri->pierced_triangle)
        continue;  // already treated by corresp. triangle
      ...
      for (tri2 = triangles.first; tri2; tri2 = tri2->next) {
        ...
        IVP_DOUBLE scal_val =
            tri->tmp.gen.hesse.dot_product(&tri2->tmp.gen.hesse);
      ...
      found_tri->pierced_triangle = tri;  // problem is pretty good symmetric
                                          // (depending on bewertungsfunktion)
    }
  }
}
