...
void qh_produce_output(void) {
  ...
  if (qh GETarea) qh_getarea(qh facet_list);
  ...
  if (qh PRINTstatistics) {
    ...
    size in bytes: hashentry %d merge %d ridge %d vertex %d facet %d\n\
        ...
        sizeof(hashentryT), sizeof(mergeT), sizeof(ridgeT), sizeof(vertexT),
        sizeof(facetT), qh normal_size, d_1, d_1 + SETelemsize);
  }
  if (qh_setsize((setT *)qhmem.tempstack) != tempsize) {
    ivp_message(
        "qhull internal error (qh_produce_output): temporary sets not empty "
        "(%d)\n",
...
static int qh_compare_facetarea(const void *p1, const void *p2) {
  ...
  if (!a->isarea) return -1;
  if (!b->isarea) return 1;
...
static int qh_compare_facetvisit(const void *p1, const void *p2) {
  ...
  if (!(i = a->visitid)) i = a->id; /* do not convert to int */
  if (!(j = b->visitid)) j = b->id;
  return (i - j);
} /* compare_facetvisit */
#ifdef SUN
}
...
void qh_countfacets(facetT *facetlist, setT *facets, boolT printall,
  ...
  FORALLfacet_(facetlist) {
    if ((facet->visible && qh NEWfacets) || (!printall && qh_skipfacet(facet)))
      facet->visitid = 0;
    else {
      ...
      if (facet->coplanarset) numcoplanars += qh_setsize(facet->coplanarset);
    }
  }
...
*/
pointT *qh_detvnorm(vertexT *vertex, vertexT *vertexA, setT *centers,
                    realT *offsetp) {
  facetT *facet, **facetp;
  ...
  FOREACHfacet_(centers) {
    ...
    if (!facet->visitid)
      ...
      if (!facet->center) facet->center = qh_facetcenter(facet->vertices);
  ...
  if (numcenters > dim) {
    ...
    if (unbounded) qh_setappend(&simplex, midpoint);
    qh_maxsimplex(dim, points, NULL, 0, &simplex);
    qh_setdelnth(simplex, 0);
  } else if (numcenters == dim) {
    if (unbounded) qh_setappend(&points, midpoint);
    simplex = points;
  } else {
    ivp_message(
  ...
  FOREACHpoint_(simplex) {
    if (qh IStracing >= 4)  //-V112
      qh_printmatrix(qh ferr, "qh_detvnorm: Voronoi vertex or midpoint", &point,
                     1, dim);
    if (point != point0) {
      qh gm_row[i++] = gmcoord;
      coord = point0;
      for (k = dim; k--;) *(gmcoord++) = *point++ - *coord++;
    }
  }
  qh gm_row[i] = gmcoord; /* does not overlap midpoint, may be used later for
                             qh_areasimplex */
  normal = gmcoord;
  qh_sethyperplane_gauss(dim, qh gm_row, point0, True, normal, &offset,
                         &nearzero);
  ...
  *offsetp = offset;
  if (simplex != points) qh_settempfree(&simplex);
...
setT *qh_detvridge(vertexT *vertex) {
  ...
  qsort(SETaddr_(centers, facetT), qh_setsize(centers), sizeof(facetT *),
...
setT *qh_detvridge3(vertexT *atvertex, vertexT *vertex) {
  ...
  if (qh CHECKfrequently) {
    FOREACHneighbor_(vertex) {
      if (!neighbor->seen2) {
        fprintf(stderr,
                "qh_detvridge3: neigbors of vertex p%d are not connected at "
                "facet %d\n",
      ...
      for each unvisited vertex
        if atvertex and vertex share more than d-1 neighbors
          ...
          if printvridge defined
            ...
            call printvridge
*/
int qh_eachvoronoi(FILE *fp, printvridgeT printvridge, vertexT *atvertex,
                   boolT visitall, qh_RIDGE innerouter, boolT inorder) {
  boolT unbounded;
  int count;
  ...
  FOREACHneighbor_(atvertex) {
    if (neighbor->visitid < numfacets) neighbor->seen = True;
  }
  FOREACHneighbor_(atvertex) {
    if (neighbor->seen) {
      FOREACHvertex_(neighbor->vertices) {
        if (vertex->visitid != qh vertex_visit && !vertex->seen) {
          ...
          if (count >= qh hull_dim - 1) { /* e.g., 3 for 3-d Voronoi */
            if (firstinf) {
              if (innerouter == qh_RIDGEouter) continue;
              unbounded = False;
            } else {
              if (innerouter == qh_RIDGEinner) continue;
              unbounded = True;
            }
            totridges++;
            trace4((qh ferr,
...
*/
int qh_eachvoronoi_all(FILE *fp, printvridgeT printvridge, boolT isupper,
  ...
  FORALLfacets {
    if (facet->upperdelaunay == isupper) facet->visitid = numcenters++;
  }
  FORALLvertices vertex->seen = False;
  FORALLvertices totridges +=
      qh_eachvoronoi(fp, printvridge, vertex, !qh_ALL, innerouter, inorder);
  return totridges;
} /* eachvoronoi_all */
...
*/
void qh_facet2point(facetT *facet, pointT **point0, pointT **point1,
                    realT *mindist) {
...
setT *qh_facetvertices(facetT *facetlist, setT *facets, boolT allfacets) {
  ...
  if (facetlist == qh facet_list && allfacets && !facets) {
    ...
    FORALLfacet_(facetlist) {
      if (!allfacets && qh_skipfacet(facet)) continue;
...
void qh_geomplanes(facetT *facet, realT *outerplane, realT *innerplane) {
  ...
  if (qh MERGING || qh JOGGLEmax < REALmax / 2) {
    ...
    if (qh JOGGLEmax < REALmax / 2)
      radius -=
          qh JOGGLEmax *
          sqrt(qh hull_dim); /* already accounted for in qh_outerinner() */
    *outerplane += radius;
    *innerplane -= radius;
...
void qh_markkeep(facetT *facetlist) {
  ...
  FORALLfacet_(facetlist) {
    if (!facet->visible && facet->good) qh_setappend(&facets, facet);
  }
  size = qh_setsize(facets);
  if (qh KEEParea) {
    qsort(SETaddr_(facets, facetT), size, sizeof(facetT *),
  ...
  if (qh KEEPminArea < REALmax / 2) {
    FOREACHfacet_(facets) {
      if (!facet->isarea || facet->f.area < qh KEEPminArea) facet->good = False;
    }
  }
...
*/
setT *qh_markvoronoi(facetT *facetlist, setT *facets, boolT printall,
                     boolT *islowerp, int *numcentersp) {
  int numcenters = 0;
  facetT *facet, **facetp;
  setT *vertices;
  boolT islower = False;
  ...
  if (qh ATinfinity) SETelem_(vertices, qh num_points - 1) = NULL;
  qh visit_id++;
  maximize_(qh visit_id, (unsigned)qh num_facets);
  ...
  FORALLfacet_(facetlist) {
    if (printall || !qh_skipfacet(facet)) facet->visitid = numcenters++;
  }
  FOREACHfacet_(facets) {
    if (printall || !qh_skipfacet(facet)) facet->visitid = numcenters++;
  }
...
void qh_order_vertexneighbors(vertexT *vertex) {
  ...
  while (qh_setsize(vertex->neighbors)) {
    ...
    if (!neighbor) {
      ivp_message(
          "qhull internal error (qh_order_vertexneighbors): no neighbor of v%d "
          "for f%d\n",
          vertex->id, facet->id);
...
void qh_printafacet(FILE *fp, int format, facetT *facet, boolT printall) {
  ...
  if (!printall && qh_skipfacet(facet)) return;
  if (facet->visible && qh NEWfacets && format != qh_PRINTfacets) return;
  qh printoutnum++;
  switch (format) {
    case qh_PRINTarea:
      if (facet->isarea) {
    ...
    case qh_PRINTcoplanars:
      ...
      FOREACHpoint_(facet->coplanarset) fprintf(fp, " %d", qh_pointid(point));
      fprintf(fp, "\n");
      break;
    ...
    case qh_PRINTgeom: /* either 2 , 3, or 4-d by qh_printbegin */
      ...
      if (qh PRINTdim != qh hull_dim) qh_normalize2(color, 3, True, NULL, NULL);
      if (qh hull_dim <= 2)
    ...
    LABELprintnorm:
      ...
      if (qh CDDoutput) fprintf(fp, qh_REAL_1, -offset);
    ...
    case qh_PRINTneighbors:
      ...
      FOREACHneighbor_(facet) fprintf(
          fp, " %d", neighbor->visitid ? neighbor->visitid - 1 : neighbor->id);
      fprintf(fp, "\n");
      break;
    case qh_PRINTpointintersect:
      if (!qh feasible_point) {
        fprintf(fp,
                "qhull input error (qh_printafacet): option 'Fp' needs qh "
                "feasible_point\n");
        qh_errexit(qh_ERRinput, NULL, NULL);
      }
      if (facet->offset > 0) goto LABELprintinfinite;
      ...
      break;
    LABELprintinfinite:
      for (k = qh hull_dim; k--;) fprintf(fp, qh_REAL_1, qh_INFINITE);
      fprintf(fp, "\n");
      break;
...
*/
void qh_printbegin(FILE *fp, int format, facetT *facetlist, setT *facets,
                   boolT printall) {
  ...
  switch (format) {
    ...
    case qh_PRINTcoplanars:
      ...
      break;
    case qh_PRINTcentrums:
      if (qh CENTERtype == qh_ASnone) qh_clearcenters(qh_AScentrum);
    ...
    case qh_PRINTgeom:
      ...
      if (qh hull_dim == 4 && (qh PRINTinner || qh PRINTouter ||         //-V112
        ...
        ivp_message(
            "qhull warning: output for outer/inner planes and centrums not "
            "implemented in 4-d\n");
      ...
      if (qh PRINTdim == 2) {
        ...
        FOREACHfacet_(facets) qh_printend4geom(NULL, facet, &num, printall);
        qh ridgeoutnum = num;
      ...
      if (qh PRINTdots) {
        ...
        if (qh DELAUNAY && qh ATinfinity) num--;
        ...
        FORALLpoints {
          if (!qh DELAUNAY || !qh ATinfinity ||
              qh_pointid(point) != qh num_points - 1) {
            if (qh PRINTdim == 4)  //-V112
              qh_printpoint(fp, NULL, point);
            else
      ...
      if (qh PRINTdim != 4 &&  //-V112
        ...
        if (qh PRINTcoplanar || qh PRINTcentrums) {
          ...
          if (qh PRINTcoplanar && !qh PRINTspheres) {
            FOREACHvertex_(vertices) qh_printpointvect2(
                fp, vertex->point, NULL, qh interior_point, qh PRINTradius);
          }
          FORALLfacet_(facetlist) {
            if (!printall && qh_skipfacet(facet)) continue;
            if (!facet->normal) continue;
            if (qh PRINTcentrums && qh PRINTdim <= 3)
              qh_printcentrum(fp, facet, qh PRINTcradius);
            if (!qh PRINTcoplanar) continue;
            FOREACHpoint_(facet->coplanarset) qh_printpointvect2(
                fp, point, facet->normal, NULL, qh PRINTradius);
    ...
    case qh_PRINTincidences:
      if (qh VORONOI)
        ivp_message(
            "qhull warning: writing Delaunay.  Use 'p' or 'o' for Voronoi "
            "centers\n");
    ...
    case qh_PRINToff:
    case qh_PRINTtriangles:
      if (qh VORONOI) goto LABELnoformat;
      num = qh hull_dim;
      if (format == qh_PRINToff || qh hull_dim == 2)
        fprintf(fp, "%d\n%d %d %d\n", num,
                qh num_points + qh_setsize(qh other_points), numfacets,
                totneighbors / 2);
      else { /* qh_PRINTtriangles */
        qh printoutvar =
            qh num_points + qh_setsize(qh other_points); /* first centrum */
        if (qh DELAUNAY) num--; /* drop last dimension */
        fprintf(fp, "%d\n%d %d %d\n", num,
                qh printoutvar + numfacets - numsimplicial,
                numsimplicial + numridges, totneighbors / 2);
      }
      FORALLpoints qh_printpointid(qh fout, NULL, num, point, -1);
      FOREACHpoint_(qh other_points)
    ...
    LABELnoformat:
      ivp_message(
          "qhull internal error (qh_printbegin): can not use this format for "
          "dimension %d\n",
          qh hull_dim);
      qh_errexit(qh_ERRqhull, NULL, NULL);
  }
...
void qh_printcenter(FILE *fp, int format, const char *string, facetT *facet) {
  int k, num;
  if (!fp) return;
  if (qh CENTERtype != qh_ASvoronoi && qh CENTERtype != qh_AScentrum) return;
  if (string) fprintf(fp, string, facet->id);
  if (qh CENTERtype == qh_ASvoronoi) {
    ...
    if (!facet->normal || !facet->upperdelaunay || !qh ATinfinity) {
      ...
      for (k = 0; k < num; k++) fprintf(fp, qh_REAL_1, qh_INFINITE);
    }
  } else /* qh CENTERtype == qh_AScentrum */ {
    num = qh hull_dim;
    if (format == qh_PRINTtriangles && qh DELAUNAY) num--;
...
-0.3  0.3 0.0001     0 0 1 1 } transform { \n",
  ...
  if (tempcentrum) qh_memfree(centrum, qh normal_size);
} /* printcentrum */
...
*/
void qh_printend(FILE *fp, int format, facetT *facetlist, setT *facets,
                 boolT printall) {
  int num;
  facetT *facet, **facetp;
  if (!fp) return;
  if (!qh printoutnum) ivp_message("qhull warning: no facets printed\n");
  switch (format) {
    case qh_PRINTgeom:
      if (qh hull_dim == 4 && qh DROPdim < 0 && !qh PRINTnoplanes) {  //-V112
        qh visit_id++;
        num = 0;
        FORALLfacet_(facetlist) qh_printend4geom(fp, facet, &num, printall);
        FOREACHfacet_(facets) qh_printend4geom(fp, facet, &num, printall);
    ...
    case qh_PRINTnormals:
    case qh_PRINTouter:
      if (qh CDDoutput) fprintf(fp, "end\n");
      break;
    case qh_PRINTmathematica:
      fprintf(fp, "}\n");
      break;
...
void qh_printend4geom(FILE *fp, facetT *facet, int *nump, boolT printall) {
  ...
  if (!fp) return;
  if (!printall && qh_skipfacet(facet)) return;
  if (qh PRINTnoplanes || (facet->visible && qh NEWfacets)) return;
  if (!facet->normal) return;
  if (fp) {
    for (i = 0; i < 3; i++) {
...
*/
void qh_printextremes_2d(FILE *fp, facetT *facetlist, setT *facets,
                         int printall) {
  ...
  if (!fp) return;
  qh_countfacets(facetlist, facets, printall, &numfacets, &numsimplicial,
                 &totneighbors, &numridges,
                 &numcoplanars); /* marks qh visit_id */
  vertices = qh_facetvertices(facetlist, facets, printall);
  fprintf(fp, "%d\n", qh_setsize(vertices));
  qh_settempfree(&vertices);
  if (!numfacets) return;
...
/*-<a                             href="qh-c.htm#io"
  ...
  notes:
    ...
    unordered
*/
void qh_printextremes_d(FILE *fp, facetT *facetlist, setT *facets,
                        int printall) {
  setT *vertices;
  vertexT *vertex, **vertexp;
...
void qh_printfacet(FILE *fp, facetT *facet) {
  qh_printfacetheader(fp, facet);
  if (facet->ridges) qh_printfacetridges(fp, facet);
} /* printfacet */
...
/*-<a                             href="qh-c.htm#io"
  ...
  qh_printfacet2geom( fp, facet, color )
    ...
    notes:
      assume precise calculations in io.c with roundoff covered by
  qh_GEOMepsilon mindist is calculated within io.c.  maxoutside is calculated
  elsewhere so a DISTround error may have occured.
*/
...
void qh_printfacet3geom_nonsimplicial(FILE *fp, facetT *facet, realT color[3]) {
  ...
  FOREACHpoint_(projectedpoints) qh_memfree(point, qh normal_size);
    ...
    FOREACHridge_(facet->ridges) {
      ...
      if (neighbor->visitid != qh visit_id) {
        if (qh DOintersections)
          qh_printhyperplaneintersection(fp, facet, neighbor, ridge->vertices,
                                         black);
        if (qh PRINTridges) {
...
*/
void qh_printfacet3geom_points(FILE *fp, setT *points, facetT *facet,
                               realT offset, realT color[3]) {
  ...
  FOREACHpoint_(printpoints) {
    ...
    if (printpoints != points) qh_memfree(point, qh normal_size);
    fprintf(fp, "\n");
  }
  if (printpoints != points) qh_settempfree(&printpoints);
  fprintf(fp, "%d ", n);
  for (i = 0; i < n; i++) fprintf(fp, "%d ", i);
  fprintf(fp, "%8.4g %8.4g %8.4g 1.0 }\n", color[0], color[1], color[2]);
...
void qh_printfacet3geom_simplicial(FILE *fp, facetT *facet, realT color[3]) {
  ...
  FOREACHvertex_(vertices) qh_setappend(&points, vertex->point);
  if (qh PRINTouter || (!qh PRINTnoplanes && !qh PRINTinner))
  ...
  if ((qh DOintersections || qh PRINTridges) &&
    ...
    FOREACHneighbor_(facet) {
      if (neighbor->visitid != qh visit_id) {
        vertices =
            qh_setnew_delnthsorted(facet->vertices, qh hull_dim,
...
void qh_printfacet3math(FILE *fp, facetT *facet, int notfirst) {
  ...
  FOREACHpoint_(points) qh_memfree(point, qh normal_size);
  qh_settempfree(&points);
  qh_settempfree(&vertices);
  fprintf(fp, "}]");
} /* printfacet3math */
...
void qh_printfacet3vertex(FILE *fp, facetT *facet, int format) {
  ...
  if (!fp) return;
  vertices = qh_facet3vertex(facet);
  if (format == qh_PRINToff) fprintf(fp, "%d ", qh_setsize(vertices));
  FOREACHvertex_(vertices) fprintf(fp, "%d ", qh_pointid(vertex->point));
  fprintf(fp, "\n");
  qh_settempfree(&vertices);
} /* printfacet3vertex */
...
void qh_printfacet4geom_nonsimplicial(FILE *fp, facetT *facet, realT color[3]) {
  ...
  FOREACHridge_(facet->ridges) {
    neighbor = otherfacet_(ridge, facet);
    if (neighbor->visitid == qh visit_id) continue;
    if (qh PRINTtransparent && !neighbor->good) continue;
    if (qh DOintersections)
      qh_printhyperplaneintersection(fp, facet, neighbor, ridge->vertices,
                                     color);
    else {
      ...
      FOREACHvertex_(ridge->vertices) {
        ...
        point = qh_projectpoint(vertex->point, facet, dist);
        for (k = 0; k < qh hull_dim; k++) {
          if (k != qh DROPdim) fprintf(fp, "%8.4g ", point[k]);
        }
        fprintf(fp, "\n");
        qh_memfree(point, qh normal_size);
      }
...
void qh_printfacet4geom_simplicial(FILE *fp, facetT *facet, realT color[3]) {
  ...
  FOREACHneighbor_(facet) {
    ...
    if (qh DOintersections)
      ...
      FOREACHvertex_(vertices) {
        for (k = 0; k < qh hull_dim; k++) {
          if (k != qh DROPdim) fprintf(fp, "%8.4g ", vertex->point[k]);
        }
        fprintf(fp, "\n");
      }
      if (qh DROPdim >= 0)
...
void qh_printfacetNvertex_nonsimplicial(FILE *fp, facetT *facet, int id,
  ...
  if (facet->visible && qh NEWfacets) return;
  FOREACHridge_(facet->ridges) {
    if (format == qh_PRINTtriangles) fprintf(fp, "%d ", qh hull_dim);
    fprintf(fp, "%d ", id);
...
void qh_printfacetheader(FILE *fp, facetT *facet) {
  ...
  if (facet->toporient)
    ...
    fprintf(fp, " bottom");
  if (facet->simplicial) fprintf(fp, " simplicial");
  if (facet->upperdelaunay) fprintf(fp, " upperDelaunay");
  if (facet->visible) fprintf(fp, " visible");
  if (facet->newfacet) fprintf(fp, " new");
  if (facet->tested) fprintf(fp, " tested");
  if (!facet->good) fprintf(fp, " notG");
  if (facet->seen) fprintf(fp, " seen");
  if (facet->coplanar) fprintf(fp, " coplanar");
  if (facet->mergehorizon) fprintf(fp, " mergehorizon");
  if (facet->keepcentrum) fprintf(fp, " keepcentrum");
  if (facet->dupridge) fprintf(fp, " dupridge");
  if (facet->mergeridge && !facet->mergeridge2) fprintf(fp, " mergeridge1");
  if (facet->mergeridge2) fprintf(fp, " mergeridge2");
  if (facet->newmerge) fprintf(fp, " newmerge");
  if (facet->flipped) fprintf(fp, " flipped");
  if (facet->notfurthest) fprintf(fp, " notfurthest");
  if (facet->degenerate) fprintf(fp, " degenerate");
  if (facet->redundant) fprintf(fp, " redundant");
  fprintf(fp, "\n");
  if (facet->isarea)
  ...
  if (facet->nummerge) fprintf(fp, "    - merges: %d\n", facet->nummerge);
  ...
  if (!SETempty_(facet->outsideset)) {
    ...
    if (qh_setsize(facet->outsideset) < 6) {
      ...
      FOREACHpoint_(facet->outsideset) qh_printpoint(fp, "     ", point);
    } else if (qh_setsize(facet->outsideset) < 21) {
      qh_printpoints(fp, "    - outside set:", facet->outsideset);
    } else {
      fprintf(fp, "    - outside set:  %d points.",
              qh_setsize(facet->outsideset));
  ...
  if (!SETempty_(facet->coplanarset)) {
    ...
    if (qh_setsize(facet->coplanarset) < 6) {
      ...
      FOREACHpoint_(facet->coplanarset) qh_printpoint(fp, "     ", point);
    } else if (qh_setsize(facet->coplanarset) < 21) {
      qh_printpoints(fp, "    - coplanar set:", facet->coplanarset);
    } else {
      fprintf(fp, "    - coplanar set:  %d points.",
              qh_setsize(facet->coplanarset));
      qh_printpoint(fp, "  Furthest", furthest);
    }
    zinc_(Zdistio);
...
void qh_printfacetridges(FILE *fp, facetT *facet) {
  ...
  if (facet->visible && qh NEWfacets) {
    ...
    FOREACHridge_(facet->ridges) fprintf(fp, " r%d", ridge->id);
    fprintf(fp, "\n");
  } else {
    fprintf(fp, "    - ridges:\n");
    ...
    FOREACHridge_(facet->ridges) {
      if (!ridge->seen) qh_printridge(fp, ridge);
    }
  }
} /* printfacetridges */
...
*/
void qh_printfacets(FILE *fp, int format, facetT *facetlist, setT *facets,
                    boolT printall) {
  int numfacets, numsimplicial, numridges, totneighbors, numcoplanars;
  facetT *facet, **facetp;
  setT *vertices;
  ...
  if (format == qh_PRINTnone)
    ...
    fprintf(fp, "7 %d %d %d %d %d %d %d\n2 ", qh hull_dim,
            qh num_points + qh_setsize(qh other_points), qh num_vertices,
            qh num_facets - qh num_visible, qh_setsize(vertices), numfacets,
            numcoplanars);
    qh_settempfree(&vertices);
    ...
    qh_printbegin(fp, format, facetlist, facets, printall);
    FORALLfacet_(facetlist) qh_printafacet(fp, format, facet, printall);
    FOREACHfacet_(facets) qh_printafacet(fp, format, facet, printall);
    qh_printend(fp, format, facetlist, facets, printall);
  }
  qh RANDOMdist = qh old_randomdist;
} /* printfacets */
...
the points by randomly rotating the input with 'QR0'.\n");
  ...
  for (k = 0; k < qh hull_dim; k++) {
    min = REALmax;
    max = -REALmin;
    for (i = qh num_points, coord = qh first_point + k; i--;
         coord += qh hull_dim) {
      maximize_(max, *coord);
      minimize_(min, *coord);
    }
...
void qh_printhyperplaneintersection(FILE *fp, facetT *facet1, facetT *facet2,
  ...
  FOREACHvertex_(vertices) {
    ...
    t = qh_divzero(-dist2 + costheta * dist1, denominator, mindenom,
                   &nearzero2);
    if (nearzero1 || nearzero2) s = t = 0.0;
    for (k = qh hull_dim; k--;)
...
/*-<a                             href="qh-c.htm#io"
  ...
  notes:
    if pointA == pointB,
      it's a 1 point VECT
*/
void qh_printline3geom(FILE *fp, pointT *pointA, pointT *pointB,
                       realT color[3]) {
  int k;
  realT pA[4], pB[4];
  ...
  if ((fabs(pA[0] - pB[0]) > 1e-3) || (fabs(pA[1] - pB[1]) > 1e-3) ||
    ...
    for (k = 0; k < 3; k++) fprintf(fp, "%8.4g ", pB[k]);
    fprintf(fp, " # p%d\n", qh_pointid(pointB));
  } else
    fprintf(fp, "VECT 1 1 1 1 1\n");
  for (k = 0; k < 3; k++) fprintf(fp, "%8.4g ", pA[k]);
  fprintf(fp, " # p%d\n", qh_pointid(pointA));
...
/*-<a                             href="qh-c.htm#io"
  ...
  notes:
    ...
    bumps qh.visit_id
*/
void qh_printneighborhood(FILE *fp, int format, facetT *facetA, facetT *facetB,
                          boolT printall) {
  facetT *neighbor, **neighborp, *facet;
  setT *facets;

  if (format == qh_PRINTnone) return;
  qh_findgood_all(qh facet_list);
  if (facetA == facetB) facetB = NULL;
  ...
  for (facet = facetA; facet; facet = ((facet == facetA) ? facetB : 0)) {
    ...
    FOREACHneighbor_(facet) {
      ...
      if (printall || !qh_skipfacet(neighbor)) qh_setappend(&facets, neighbor);
    }
  }
...
void qh_printpointid(FILE *fp, const char *string, int dim, pointT *point,
                     int id) {
  int k;
  realT r; /*bug fix*/
  if (!fp) return;
...
void qh_printpoint3(FILE *fp, pointT *point) {
  ...
  for (k = 0; k < 3; k++) fprintf(fp, "%8.4g ", p[k]);
  fprintf(fp, " # p%d\n", qh_pointid(point));
} /* printpoint3 */
...
*/
void qh_printpoints_out(FILE *fp, facetT *facetlist, setT *facets,
                        int printall) {
  int allpoints = qh num_points + qh_setsize(qh other_points);
  int numpoints = 0, point_i, point_n;
  setT *vertices, *points;
  ...
  FOREACHvertex_(vertices) {
    id = qh_pointid(vertex->point);
    if (id >= 0) SETelem_(points, id) = vertex->point;
  }
  if (qh KEEPinside || qh KEEPcoplanar || qh KEEPnearinside) {
    FORALLfacet_(facetlist) {
      ...
      FOREACHpoint_(facet->coplanarset) {
        id = qh_pointid(point);
        if (id >= 0) SETelem_(points, id) = point;
      }
    }
    FOREACHfacet_(facets) {
      if (!printall && qh_skipfacet(facet)) continue;
...
/*-<a                             href="qh-c.htm#io"
  ...
  center point
*/
void qh_printpointvect(FILE *fp, pointT *point, coordT *normal, pointT *center,
                       realT radius, realT color[3]) {
  ...
  if (center) qh_normalize2(diff, qh hull_dim, True, NULL, NULL);
  for (k = qh hull_dim; k--;) pointA[k] = point[k] + diff[k] * radius;
...
*/
void qh_printpointvect2(FILE *fp, pointT *point, coordT *normal, pointT *center,
                        realT radius) {
...
void qh_printridge(FILE *fp, ridgeT *ridge) {
  ...
  if (ridge->tested) fprintf(fp, " tested");
  if (ridge->nonconvex) fprintf(fp, " nonconvex");
  fprintf(fp, "\n");
...
*/
void qh_printvdiagram(FILE *fp, int format, facetT *facetlist, setT *facets,
                      boolT printall) {
  setT *vertices;
  ...
  fprintf(fp, "%d\n", totcount);
  totcount = qh_printvdiagram2(fp, printvridge, vertices, innerouter,
                               True /* inorder*/);
  qh_settempfree(&vertices);
...
/*-<a                             href="qh-c.htm#io"
  ...
  see:
    qh_eachvoronoi_all()
*/
int qh_printvdiagram2(FILE *fp, printvridgeT printvridge, setT *vertices,
                      qh_RIDGE innerouter, boolT inorder) {
  int totcount = 0;
...
void qh_printvertex(FILE *fp, vertexT *vertex) {
  ...
  if (vertex->deleted) fprintf(fp, " deleted");
  if (vertex->delridge) fprintf(fp, " ridgedeleted");
  fprintf(fp, "\n");
  if (vertex->neighbors) {
    fprintf(fp, "  neighbors:");
    FOREACHneighbor_(vertex) fprintf(fp, " f%d", neighbor->id);
    fprintf(fp, "\n");
  }
} /* printvertex */
...
void qh_printvertexlist(FILE *fp, const char *string, facetT *facetlist,
  ...
  vertices = qh_facetvertices(facetlist, facets, printall);
  fputs(string, fp);
  FOREACHvertex_(vertices) qh_printvertex(fp, vertex);
  qh_settempfree(&vertices);
} /* printvertexlist */
...
*/
void qh_printvneighbors(FILE *fp, facetT *facetlist, setT *facets,
                        boolT printall) {
  ...
  qh_countfacets(facetlist, facets, printall, &numfacets, &numsimplicial,
                 &totneighbors, &numridges,
                 &numcoplanars); /* sets facet->visitid */
  fprintf(fp, "%d\n", numpoints);
  qh_vertexneighbors();
  vertices = qh_facetvertices(facetlist, facets, printall);
  ...
  FOREACHvertex_(vertices) qh_point_add(vertex_points, vertex->point, vertex);
  FORALLfacet_(facetlist) {
    FOREACHpoint_(facet->coplanarset)
  ...
  FOREACHvertex_i_(vertex_points) {
    if (vertex) {
      ...
      FOREACHneighbor_(vertex) fprintf(
          fp, " %d", neighbor->visitid ? neighbor->visitid - 1 : neighbor->id);
      fprintf(fp, "\n");
...
*/
void qh_printvoronoi(FILE *fp, int format, facetT *facetlist, setT *facets,
                     boolT printall) {
  ...
  if (format == qh_PRINTgeom)
    ...
    fprintf(fp, "%d\n%d %d 1\n", qh hull_dim - 1, numcenters,
            qh_setsize(vertices));
  if (format == qh_PRINTgeom) {
    for (k = qh hull_dim - 1; k--;) fprintf(fp, qh_REAL_1, 0.0);
    fprintf(fp, " 0 # infinity not used\n");
  } else {
    for (k = qh hull_dim - 1; k--;) fprintf(fp, qh_REAL_1, qh_INFINITE);
    fprintf(fp, "\n");
  }
  FORALLfacet_(facetlist) {
    if (facet->visitid && facet->visitid < numfacets) {
      if (format == qh_PRINTgeom) fprintf(fp, "# %d f%d\n", vid++, facet->id);
      qh_printcenter(fp, format, NULL, facet);
    }
  }
  FOREACHfacet_(facets) {
    ...
    if (vertex) {
      if (qh hull_dim == 3)
        ...
        qsort(SETaddr_(vertex->neighbors, vertexT),
              qh_setsize(vertex->neighbors), sizeof(facetT *),
              qh_compare_facetvisit);
      FOREACHneighbor_(vertex) {
        if (neighbor->visitid == 0)
          numinf = 1;
...
/*-<a                             href="qh-c.htm#io"
  ...
  see:
    ...
    qh_eachvoronoi()
*/
void qh_printvnorm(FILE *fp, vertexT *vertex, vertexT *vertexA, setT *centers,
                   boolT /*unbounded*/) {
  pointT *normal;
  realT offset;
  int k;
  ...
  fprintf(fp, "%d %d %d ", 2 + qh hull_dim, qh_pointid(vertex->point),
          qh_pointid(vertexA->point));
  for (k = 0; k < qh hull_dim - 1; k++) fprintf(fp, qh_REAL_1, normal[k]);
...
*/
void qh_printvridge(FILE *fp, vertexT *vertex, vertexT *vertexA, setT *centers,
                    boolT /*unbounded*/) {
  facetT *facet, **facetp;
  if (!fp) return;

  fprintf(fp, "%d %d %d", qh_setsize(centers) + 2, qh_pointid(vertex->point),
          qh_pointid(vertexA->point));
  FOREACHfacet_(centers) fprintf(fp, " %d", facet->visitid);
  fprintf(fp, "\n");
} /* printvridge */
...
void qh_projectdim3(pointT *source, pointT *destination) {
  ...
  for (k = 0, i = 0; k < qh hull_dim; k++) {
    if (qh hull_dim == 4) {  //-V112
      if (k != qh DROPdim) destination[i++] = source[k];
    } else if (k == qh DROPdim)
      destination[i++] = 0;
    else
      destination[i++] = source[k];
  }
  while (i < 3) destination[i++] = 0.0;
} /* projectdim3 */
...
int qh_readfeasible(int dim, char *remainder) {
  ...
  if (!qh HALFspace) {
    ivp_message(
        "qhull input error: feasible point (dim 1 coords) is only valid for "
        "halfspace intersection\n");
    qh_errexit(qh_ERRinput, NULL, NULL);
  }
  if (qh feasible_string)
    ivp_message(
        "qhull input warning: feasible point (dim 1 coords) overrides 'Hn,n,n' "
        "feasible point for halfspace intersection\n");
    ...
    while (*s) {
      ...
      *(coords++) = value;
      if (++tokcount == dim) {
        ...
        if (s != t) {
          ivp_message(
              "qhull input error: coordinates for feasible point do not finish "
              "out the line: %s\n",
              s);
          qh_errexit(qh_ERRinput, NULL, NULL);
        }
  ...
  ivp_message(
      "qhull input error: only %d coordinates.  Could not read %d-d feasible "
      "point.\n",
      tokcount, dim);
  qh_errexit(qh_ERRinput, NULL, NULL);
  return 0;
...
coordT *qh_readpoints(int *numpoints, int *dimension, boolT *ismalloc) {
    ...
    while ((s = fgets(firstline, qh_MAXfirst, qh fin))) {
      ...
      if (qh HALFspace && linecount == 1 && isdigit(*s)) {
        ...
        if (qh_strtol(s, &s) == 1)
          ...
          dimfeasible = 0;
      } else if (!memcmp(firstline, "begin", 5) ||
                 !memcmp(firstline, "BEGIN", 5))
        break;
  ...
  while (!numinput && (s = fgets(firstline, qh_MAXfirst, qh fin))) {
    linecount++;
    if (!memcmp(s, "begin", 5) || !memcmp(s, "BEGIN", 5)) wasbegin = True;
    while (*s) {
      while (isspace(*s)) s++;
      if (!*s) break;
      if (!isdigit(*s)) {
  ...
  if (!s) {
    ivp_message(
        "qhull input error: short input file.  Did not find dimension and "
        "number of points\n");
    qh_errexit(qh_ERRinput, NULL, NULL);
  }
  ...
  if (isdelaunay) {
    ...
    *numpoints = numinput;
    if (qh ATinfinity) (*numpoints)++;
  } else if (qh HALFspace) {
    *dimension = diminput - 1;
    *numpoints = numinput;
    if (diminput < 3) {
      ivp_message(
          "qhull input error: dimension %d (first number, includes offset) "
          "should be at least 3 for halfspaces\n",
          diminput);
      qh_errexit(qh_ERRinput, NULL, NULL);
    }
    if (dimfeasible) {
      if (dimfeasible != *dimension) {
        ivp_message(
            "qhull input error: dimension %d of feasible point is not one less "
            "than dimension %d for halfspaces\n",
            dimfeasible, diminput);
        qh_errexit(qh_ERRinput, NULL, NULL);
      }
    } else
  ...
  if (qh HALFspace) {
    qh half_space = coordp =
        (coordT *)p_malloc(qh normal_size + sizeof(coordT));
    if (qh CDDinput) {
  ...
  *ismalloc = True; /* use malloc since memory not setup */
  ...
  if (isdelaunay && qh ATinfinity) {
    ...
    for (k = (*dimension) - 1; k--;) infinity[k] = 0.0;
  }
  ...
  while ((s = (isfirst ? s : fgets(qh line, qh maxline, qh fin)))) {
      ...
      if (*s == 'e' || *s == 'E') {
        if (!memcmp(s, "end", 3) || !memcmp(s, "END", 3)) {
          if (qh CDDinput)
            ...
            ivp_message(
                "qhull input warning: the input appears to be in cdd format.  "
                "If so, use 'Fd'\n");
        }
      }
    }
    ...
    while (*s) {
      ...
      if (s == t) {
        if (!*qh rbox_command)
          strncat(qh rbox_command, s, sizeof(qh rbox_command) - 1);
        if (*s && !firsttext) firsttext = linecount;
        if (!islong && !firstshort && coordcount) firstshort = linecount;
        break;
      }
      if (!firstpoint) firstpoint = linecount;
      s = t;
      if (++tokcount > maxcount) continue;
      if (qh HALFspace) {
        ...
        *(coords++) = value;
        if (qh CDDinput && !coordcount) {
          if (value != 1.0) {
            ivp_message(
                "qhull input error: for cdd format, point at line %d does not "
                "start with '1'\n",
                linecount);
            qh_errexit(qh_ERRinput, NULL, NULL);
          }
      ...
      if (++coordcount == diminput) {
        ...
        if (isdelaunay) {
          *(coords++) = paraboloid;
          ...
          if (!qh_sethalfspace(*dimension, coords, &coords, normalp, offsetp,
                               qh feasible_point)) {
            ivp_message("The halfspace was on line %d\n", linecount);
            if (wasbegin)
              ivp_message(
                  "The input appears to be in cdd format.  If so, you should "
                  "use option 'Fd'\n");
            qh_errexit(qh_ERRinput, NULL, NULL);
          }
        ...
        if (*s) {
          islong = True;
          if (!firstlong) firstlong = linecount;
        }
      }
    }
    if (!islong && !firstshort && coordcount) firstshort = linecount;
...
%d points and %d extra coordinates.  Line %d is the first\npoint",
        ...
        firstpoint);
    if (firsttext) ivp_message(", line %d is the first comment", firsttext);
    if (firstshort)
      ivp_message(", line %d is the first short\nline", firstshort);
    if (firstlong) ivp_message(", line %d is the first long line", firstlong);
    ivp_message(".  Continue with %d points.\n", newnum);
    numinput = newnum;
    if (isdelaunay && qh ATinfinity) {
      for (k = tokcount % diminput; k--;) infinity[k] -= *(--coords);
      *numpoints = newnum + 1;
    } else {
  ...
  if (isdelaunay && qh ATinfinity) {
    for (k = (*dimension) - 1; k--;) infinity[k] /= numinput;
    if (coords == infinity)
      coords += (*dimension) - 1;
    else {
      for (k = 0; k < (*dimension) - 1; k++) *(coords++) = infinity[k];
    }
...
boolT qh_skipfacet(facetT *facet) {
  ...
  if (qh PRINTneighbors) {
    if (facet->good) return !qh PRINTgood;
    FOREACHneighbor_(facet) {
      if (neighbor->good) return False;
    }
    return True;
