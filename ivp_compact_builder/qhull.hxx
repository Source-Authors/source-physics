...
/*-<a                             href="qh-c.htm#poly"
  ...
  for constructing hull:
    f.visible           True if facet on list of visible facets (will be
  deleted) f.newfacet          True if facet on list of newly created facets
...
struct facetT {
  ...
  union {              /* in order of testing */
    ...
    facetT *samecycle; /*  cycle of facets from the same visible/horizon
                          intersection, if ->newfacet */
...
#define qh_MAXnummerge 511 /*     2^9-1 */
  ...
  flagT tested : 1;      /* True if facet convexity has been tested (false after
                            merge */
...
struct qhT {
  ...
  */
  ...
  boolT
      KEEPcoplanar; /* true 'Qc' if keeping nearest facet for coplanar points */
  boolT KEEPinside; /* true 'Qi' if keeping nearest facet for inside points
                        set automatically if 'd Qc' */
  int KEEPmerge;    /* 'PMn' number of facets to keep with most merges */
  realT KEEPminArea; /* 'PFn' minimum facet area to keep */
  realT MAXcoplanar; /* 'Un' max distance below a facet to be coplanar*/
  boolT MERGEexact;  /* true 'Qx' if exact merges (coplanar, degen, dupridge,
                        flipped) */
  boolT MERGEindependent; /* true 'Q2' if merging independent sets */
  boolT MERGING; /* true if exact-, pre- or post-merging, with angle and centrum
                    tests */
  ...
  boolT ONLYgood; /* true 'Qg' if process points with good visible or horizon
                     facets */
  boolT
      ONLYmax; /* true 'Qm' if only process points that increase max_outside */
  ...
  char
      *feasible_string; /* feasible point 'Hn,n,n' for halfspace intersection */
  ...
  pointT *input_points; /* copy of original qh.first_point for input points for
                           qh_joggleinput */
  ...
  */
  ...
  realT outside_err;    /* application's epsilon for coplanar points
                           qh_check_bestdist() qh_check_points() reports error if
                           point outside */
  ...
  */
  ...
  char jmpXtra2[40];   /* extra bytes in case jmp_buf is defined wrong by
                          compiler*/
  ...
  */
  ...
  facetT *facet_next;      /* next facet for buildhull()
                              ...
                              NARROWhull: previous facets may have coplanar outside
                              sets for qh_outcoplanar */
  ...
  int num_outside;         /* number of points in outsidesets (for tracing and
                              RANDOMoutside)         includes coplanar outsideset points
                              for         NARROWhull/qh_outcoplanar() */
...
#define FORALLpoint_(points, num)                                    \
  for (point = (points), pointtemp = (points) + qh hull_dim * (num); \
       point < pointtemp; point += qh hull_dim)
...
#define FOREACHneighbor_(facet) \
  FOREACHsetelement_(facetT, facet->neighbors, neighbor)
...
#define FOREACHneighbor_i_(facet) \
  FOREACHsetelement_i_(facetT, facet->neighbors, neighbor)
...
#define FOREACHvertex_i_(vertices) \
  FOREACHsetelement_i_(vertexT, vertices, vertex)
