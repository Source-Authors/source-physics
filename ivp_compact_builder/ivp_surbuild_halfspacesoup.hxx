 ...
 *		    between all supplied halfspace planes while dropping from
 *the resulting set of points any vertices outside of the enclosed volume. It
 *then will return the convex set of points or, alternatively, further convert
 *it to a compact ledge or compact surface (by internally using the
 *IVP_SurfaceBuilder_Pointsoup class). Note:	    Points that are closer to
 *each other than a certain threshold will be merged to avoid numerical
 *problems. Note:	    The resulting set of points might well be empty
 *(depending, of course, on the input data)! Note:	    Be careful not to
 *provide a set of halfspace planes that defines an infinite volume!
 *******************************************************************************/
...
class IVP_SurfaceBuilder_Halfspacesoup {
 ...
 private:
  // internal methods
  static IVP_U_Point *insert_point_into_list(IVP_U_Point *point,
