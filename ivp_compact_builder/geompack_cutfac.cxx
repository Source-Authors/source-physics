...
void IVP_Geompack::cutfac_(int *p, double *nrmlc, double *dtol, int *nedgc,
                           int *pedge, int *nce, int *cedge, double *cdang,
                           long int *rflag) {
  ...
  /*              containing CEDGE(1,1) if CEDGE(1,1) >
   * ADDR_OF_N_ORIGINAL_VERTICES; */
  /*              PEDGE(1,I), PEDGE(2,I) are indices of FACEVERTICESLIST; if
   * PEDGE(1,I) */
  /*              = A and B = FACEVERTICESLIST(SUCC,A) then PEDGE(3,I) =
   * 10*CA+CB where */
  ...
  /*              called by RESEDG, LU < LV <= ADDR_OF_N_ORIGINAL_VERTICES are
   * on reflex edge and */
  /*              CEDGE(2,1) = U is index of FACEVERTICESLIST indicating reflex
   * edge; */
  /*              LV may be ADDR_OF_N_ORIGINAL_VERTICES+1 and LU may be
   * ADDR_OF_N_ORIGINAL_VERTICES+1 or ADDR_OF_N_ORIGINAL_VERTICES+2 to indicate
   */
  /*              a point on interior of an edge; CEDGE(2,1) is specified */
  /*              as described for output below; if LV >
   * ADDR_OF_N_ORIGINAL_VERTICES, CEDGE(2,0) */
...
L10:
  if (lw1 > this->n_original_vertices) {
    ...
    dof = (i__1 = facesdata[fl * 3 + 2], abs(i__1)) ==
  ...
  L20:
    if (eflag || e != estrt && e != estop) {
      ...
      if (abs(dist) <= *dtol) {
        ...
        d__3 = abs(dir[2]), d__2 = max(d__2, d__3), d__3 = abs(dir1[0]),
        d__2 = max(d__2, d__3), d__3 = abs(dir1[1]), d__2 = max(d__2, d__3),
        d__3 = abs(dir1[2]);
        if ((d__1 = cp[kmax - 1], abs(d__1)) <=
            this->tolerance * max(d__2, d__3)) {
          intang = IVP_PI;
    ...
    dof = (i__1 = facesdata[f * 3 + 2], abs(i__1)) ==
    ...
    /* Computing MAX */
    d__1 = abs(dir1[0]), d__2 = abs(dir1[1]), d__1 = max(d__1, d__2),
    d__2 = abs(dir1[2]);
    ...
    k = 1;
    if ((d__1 = normals[f * 3 + 2], abs(d__1)) >
        (d__2 = normals[f * 3 + 1], abs(d__2))) {
      k = 2;
    }
    if ((d__1 = normals[f * 3 + 3], abs(d__1)) >
        (d__2 = normals[k + f * 3], abs(d__2))) {
      k = 3;
    }
    ...
    /* Computing MAX */
    d__1 = abs(de[0]), d__2 = abs(de[1]), d__1 = max(d__1, d__2),
    d__2 = abs(de[2]), d__1 = max(d__1, d__2), d__2 = abs(dee[0]),
    d__1 = max(d__1, d__2), d__2 = abs(dee[1]), d__1 = max(d__1, d__2),
    d__2 = abs(dee[2]);
...
L70:
  ...
  recheck_size_1:
    if ((2 + n) * 1 > this->size_vcl) {
      int res = increase_memory((void **)&this->g_vcl, &this->size_vcl,
                                3 * sizeof(double));
      if (res == 0) {
        this->ierr = 500;
        return;
...
L110:
        ...
        IVP_IF(1) {
          IVP_IFDEBUG(IVP_DM_GEOMPACK_LEVEL3, {
            ivp_debugmanager.dprint(
                ...
                "Rejected due to simply-connected polygon: case 1\n");
          })
        }
        return;
      }
  ...
  recheck_size_2:
    if (nev >= this->size_ev) {  // orig. 20
      int res =
          increase_memory((void **)&this->g_ev, &this->size_ev, sizeof(int));
      if (res == 0) {
        this->ierr = 500;
        return;
      }
    ...
    for (i__ = this->n_original_vertices + 1; i__ <= i__1; ++i__) {
      for (j = 1; j <= 3; ++j) {
        /* Computing MAX */
        d__3 = (d__1 = vcl[j + i__ * 3], abs(d__1)),
        d__4 = (d__2 = vcl[j + n * 3], abs(d__2));
        cmax = max(d__3, d__4);
      ...
      IVP_IF(1) {
        IVP_IFDEBUG(IVP_DM_GEOMPACK_LEVEL3, {
          ivp_debugmanager.dprint(
              IVP_DM_GEOMPACK_LEVEL3,
              "Rejected due to simply-connected polygon: case 2\n");
        })
      }
      return;
    L140:;
...
L150:
  if (fr == 0) {
    ...
    dof = (i__1 = facesdata[f * 3 + 2], abs(i__1)) ==
          (i__2 = facesdata[f * 3 + 3], abs(i__2));
    eflag = (i__1 = facesdata[f * 3 + 2], abs(i__1)) != *p || dof && ccwfl < 0;
  } else {
    f = fr;
    dof = (i__1 = facesdata[f * 3 + 2], abs(i__1)) ==
          (i__2 = facesdata[f * 3 + 3], abs(i__2));
  ...
  dotp = -(nrmlc[1] * normals[f * 3 + 1] + nrmlc[2] * normals[f * 3 + 2] +
           nrmlc[3] * normals[f * 3 + 3]);
  if (abs(dotp) > 1. - this->tolerance) {
    dotp = d_sign(1.0, dotp);
  }
  if (eflag) {
  ...
  dotp = dir1[0] * normals[f * 3 + 1] + dir1[1] * normals[f * 3 + 2] +
         dir1[2] * normals[f * 3 + 3];
  if (eflag) {
    dotp = -dotp;
  }
  if (dotp > 0.) {
  ...
  if (s < 0.) {
    IVP_IF(1) {
      IVP_IFDEBUG(IVP_DM_GEOMPACK_LEVEL3, {
        ivp_debugmanager.dprint(IVP_DM_GEOMPACK_LEVEL3,
                                "Rejected due to inner boundary\n");
      })
    }
    return;
  }

  /*     Move edges incident on LV (if <= this->N_ORIGINAL_VERTICES), EV(1:NEV)
   * to end of PEDGE. */
  ...
  for (i__ = 1; i__ <= i__1; ++i__) {
    ...
    if (ca == 2) {
      ...
      t = (nrmlc[4] - nrmlc[1] * vcl[la * 3 + 1] - nrmlc[2] * vcl[la * 3 + 2] -
           nrmlc[3] * vcl[la * 3 + 3]) /
          (nrmlc[1] * dir[0] + nrmlc[2] * dir[1] + nrmlc[3] * dir[2]);
    ...
    // *******************************************************************
    ptpolg_(3, 3, nce, 2, &cedge[1], &vcl[4], cp, &nrmlc[1], dtol, &inout);

    if (inout == 1) {
      IVP_IF(1) {
        IVP_IFDEBUG(IVP_DM_GEOMPACK_LEVEL3, {
          ivp_debugmanager.dprint(IVP_DM_GEOMPACK_LEVEL3,
                                  "Rejected due to hole polygon\n");
        })
      }
      return;
    }
    ...
    IVP_IFDEBUG(IVP_DM_GEOMPACK_LEVEL3, {
      ...
      for (i__ = 1; i__ <= i__1; ++i__) {
        ivp_debugmanager.dprint(IVP_DM_GEOMPACK_LEVEL3, "%d %d %d %f\n", i__,
                                cedge[(i__ << 1) + 1], cedge[(i__ << 1) + 2],
                                cdang[i__] * 180.0 / IVP_PI);
      }
    })
  }
