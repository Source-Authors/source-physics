...
unsigned long qh_clock(void) {
  ...
  if (!clktck) {
    if ((clktck = sysconf(_SC_CLK_TCK)) < 0) {
      ivp_message(
          "qhull internal error (qh_clock): sysconf() failed.  Use "
          "qh_CLOCKtype 1 in user.h\n");
      qh_errexit(qh_ERRqhull, NULL, NULL);
    }
  }
  if (times(&time) == -1) {
    ivp_message(
        "qhull internal error (qh_clock): times() failed.  Use qh_CLOCKtype 1 "
        "in user.h\n");
    qh_errexit(qh_ERRqhull, NULL, NULL);
  }
...
void qh_freebuffers(void) {
  ...
  if (qh line) /* allocated by qh_readinput, freed if no error */
    P_FREE(qh line);
  if (qh half_space) P_FREE(qh half_space);
  if (qh temp_malloc) P_FREE(qh temp_malloc);
...
void qh_freebuild(boolT allmem) {
  ...
  if (qh del_vertices) qh_settruncate(qh del_vertices, 0);
  if (allmem) {
...
void qh_init_B(coordT *points, int numpoints, int dim, boolT ismalloc) {
  ...
  if (qhmem.LASTsize == 0) qh_initqhull_mem();
  /* mem.c and qset.c are initialized */
  qh_initqhull_buffers();
  qh_initthresholds(qh qhull_command);
  if (qh PROJECTinput || (qh DELAUNAY && qh PROJECTdelaunay)) qh_projectinput();
  if (qh SCALEinput) qh_scaleinput();
...
void qh_initflags(char *command) {
  ...
  while (*s) {
    ...
    switch (*s++) {
      ...
      case 'A':
        if (!isdigit(*s) && *s != '.' && *s != '-')
          fprintf(qh ferr,
                  "qhull warning: no maximum cosine angle given for option "
                  "'An'.  Ignored.\n");
        else {
          if (*s == '-') {
      ...
      case 'C':
        if (!isdigit(*s) && *s != '.' && *s != '-')
          fprintf(qh ferr,
                  "qhull warning: no centrum radius given for option 'Cn'.  "
                  "Ignored.\n");
        else {
          if (*s == '-') {
      ...
      case 'E':
        if (*s == '-')
          fprintf(qh ferr,
                  "qhull warning: negative maximum roundoff given for option "
                  "'An'.  Ignored.\n");
        else if (!isdigit(*s))
          fprintf(qh ferr,
                  "qhull warning: no maximum roundoff given for option 'En'.  "
                  "Ignored.\n");
        else {
        ...
        while (t > s) {
          if (*t && !isspace(*t)) {
            if (*t == ',')
              ...
              ivp_message(
                  "qhull warning: origin for Halfspace intersection should be "
                  "'Hn,n,n,...'\n");
          }
          s = t;
          qh_strtod(s, &t);
      ...
      case 'R':
        if (!isdigit(*s))
          fprintf(qh ferr,
                  "qhull warning: missing random perturbation for option 'Rn'. "
                  " Ignored\n");
        else {
      ...
      case 'V':
        if (!isdigit(*s) && *s != '-')
          fprintf(qh ferr,
                  "qhull warning: missing visible distance for option 'Vn'.  "
                  "Ignored\n");
        else {
      ...
      case 'U':
        if (!isdigit(*s) && *s != '-')
          fprintf(qh ferr,
                  "qhull warning: missing coplanar distance for option 'Un'.  "
                  "Ignored\n");
        else {
      ...
      case 'W':
        if (*s == '-')
          fprintf(qh ferr,
                  "qhull warning: negative outside width for option 'Wn'.  "
                  "Ignored.\n");
        else if (!isdigit(*s))
          fprintf(qh ferr,
                  "qhull warning: missing outside width for option 'Wn'.  "
                  "Ignored\n");
        else {
        ...
        while (*s && !isspace(*s)) {
          switch (*s++) {
            ...
            case 'A':
              if (!isdigit(*s))
                ivp_message(
                    "qhull input error: missing facet count for keep area "
                    "option 'PAn'\n");
              else {
            ...
            case 'M':
              if (!isdigit(*s))
                ivp_message(
                    "qhull input error: missing merge count for option "
                    "'PMn'\n");
              else {
        ...
        while (*s && !isspace(*s)) {
          switch (*s++) {
            ...
            case 'G':
              ...
              if (qh GOODpoint)
                ivp_message(
                    "qhull warning: good point already defined for option "
                    "'QGn'.  Ignored\n");
              else if (s == t)
                ivp_message(
                    "qhull warning: missing good point id for option 'QGn'.  "
                    "Ignored\n");
              else if (i < 0 || *s == '-') {
                qh GOODpoint = i - 1;
            ...
            case 'R':
              if (!isdigit(*s) && *s != '-')
                ivp_message(
                    "qhull warning: missing random seed for option 'QRn'.  "
                    "Ignored\n");
              else {
            ...
            case 'V':
              ...
              if (qh GOODvertex)
                ivp_message(
                    "qhull warning: good vertex already defined for option "
                    "'QVn'.  Ignored\n");
              else if (s == t)
                ivp_message(
                    "qhull warning: no good point id given for option 'QVn'.  "
                    "Ignored\n");
              else if (i < 0) {
                qh GOODvertex = i - 1;
          ...
          if (isdigit(*s) || *s == '-')
            ...
            switch (*s++) {
              ...
              case 'z':
                if (!qh fout)
                  ivp_message(
                      "qhull warning: output file undefined (stdout).  Option "
                      "'Tz' ignored.\n");
                else {
              ...
              case 'C':
                if (!isdigit(*s))
                  ivp_message(
                      "qhull warning: missing point id for cone for trace "
                      "option 'TCn'.  Ignored\n");
                else {
                  i = qh_strtol(s, &s);
              ...
              case 'F':
                if (!isdigit(*s))
                  ivp_message(
                      "qhull warning: missing frequency count for trace option "
                      "'TFn'.  Ignored\n");
                else {
              ...
              case 'O':
                if (s[0] != ' ' || s[1] == '\"' || isspace(s[1])) {
                  ...
                  ivp_message(
                      ...
                      "space, file name, and space or end-of-line.\nThe file "
                      "name may be enclosed in single quotes.\nDo not use "
                      "double quotes.  Option 'FO' ignored.\n");
                } else { /* not a procedure because of qh_option (filename,
                            NULL, NULL); */
                  ...
                  *tt = '\0';
                  ...
                  if (isquote)
                    ivp_message(
                        "qhull error: missing end quote for option 'TO'.  Rest "
                        "of line ignored.\n");
                  else if (!freopen(filename, "w", stdout /*stdout*/)) {
                    ivp_message("qhull error: could not open file \"%s\".",
                                filename);
                    qh_errexit(qh_ERRinput, NULL, NULL);
                  } else {
              ...
              case 'P':
                if (!isdigit(*s))
                  ivp_message(
                      "qhull warning: missing point id for trace option 'TPn'. "
                      " Ignored\n");
                else {
              ...
              case 'M':
                if (!isdigit(*s))
                  ivp_message(
                      "qhull warning: missing merge id for trace option 'TMn'. "
                      " Ignored\n");
                else {
              ...
              case 'R':
                if (!isdigit(*s))
                  ivp_message(
                      "qhull warning: missing rerun count for trace option "
                      "'TRn'.  Ignored\n");
                else {
              ...
              case 'V':
                ...
                if (s == t)
                  ivp_message(
                      "qhull warning: missing furthest point id for trace "
                      "option 'TVn'.  Ignored\n");
                else if (i < 0) {
                  qh STOPpoint = i - 1;
              ...
              case 'W':
                if (!isdigit(*s))
                  ivp_message(
                      "qhull warning: missing max width for trace option "
                      "'TWn'.  Ignored\n");
                else {
              ...
              default:
                s--;
                ivp_message(
                    "qhull warning: unknown 'T' trace option %c, rest "
                    "ignored\n",
                    (int)s[0]);
                while (*++s && !isspace(*s));
                break;
    ...
    if (s - 1 == prev_s && *s && !isspace(*s)) {
      ivp_message(
          "qhull warning: missing space after flag %c (%x); reserved for menu. "
          "Skipped.\n",
  ...
  if (isgeom && !qh FORCEoutput && qh PRINTout[1])
    ivp_message(
        "qhull warning: additional output formats are not compatible with "
        "Geomview\n");
...
/*-<a                             href="qh-c.htm#global"
  ...
  returns:
    ...
    modifies qh.hull_dim if ((qh.DELAUNAY and qh.PROJECTdelaunay) or
  qh.PROJECTinput) adjust user flags as needed also checks DIM3 dependencies and
  constants
...
void qh_initqhull_globals(coordT *points, int numpoints, int dim,
  ...
  if (qh JOGGLEmax < REALmax / 2 && qh DELAUNAY && !qh SCALEinput &&
      !qh SCALElast) {
    qh SCALElast = True;
  ...
  if (qh JOGGLEmax < REALmax / 2 && qh PRINTprecision)
    if constexpr (REALepsilon > 2e-8)
      fprintf(qh ferr,
              "qhull warning: real epsilon, %2.2g, is probably too large for "
              "joggle ('QJn')\nRecompile with double precision reals (see "
              "user.h).\n",
              REALepsilon);
#ifdef qh_NOmerge
  if (qh MERGING) {
    ivp_message(
        "qhull input error: merging not installed (qh_NOmerge + 'Qx', 'Cn' or "
        "'An')\n");
    qh_errexit(qh_ERRinput, NULL, NULL);
  }
#endif
  ...
  if (qh DELAUNAY && qh HALFspace) {
    ivp_message(
        "qhull input error: can not use Delaunay ('d') or Voronoi ('v') with "
        "halfspace intersection ('H')\n");
    qh_errexit(qh_ERRinput, NULL, NULL);
  }
  if (!qh DELAUNAY && (qh UPPERdelaunay || qh ATinfinity)) {
    ivp_message(
        "qhull input error: use upper-Delaunay ('Qu') or infinity-point ('Qz') "
        "with Delaunay ('d') or Voronoi ('v')\n");
    qh_errexit(qh_ERRinput, NULL, NULL);
  }
  if (qh UPPERdelaunay && qh ATinfinity) {
    ivp_message(
        "qhull input error: can not use infinity-point ('Qz') with "
        "upper-Delaunay ('Qu')\n");
    qh_errexit(qh_ERRinput, NULL, NULL);
  }
  if (qh SCALElast && !qh DELAUNAY)
    ivp_message(
        "qhull input warning: option 'Qbb' (scale-last-coordinate) is normally "
        "used with 'd' or 'v'\n");
  ...
  if (qh TESTvneighbors && !qh MERGING) {
    fprintf(qh ferr,
            "qhull input error: test vertex neighbors ('Qv') needs a merge "
            "option\n");
    qh_errexit(qh_ERRinput, NULL, NULL);
  }
  ...
  for (k = 2, factorial = 1.0; k < qh hull_dim; k++) factorial *= k;
  ...
  if (qh RERUN > 1) {
    qh TRACElastrun =
        qh IStracing; /* qh_build_withrestart duplicates next conditional */
    if (qh IStracing != -1) qh IStracing = 0;
...
qhull configuration error (qh_RANDOMmax in user.h):\n\
  ...
  qh_RANDOMseed_(seed);
  randr = randr / 1000;
  if (randr < qh_RANDOMmax / 10 || randr > qh_RANDOMmax * 5)
    ivp_message(
        "\
qhull configuration warning (qh_RANDOMmax in user.h):\n\
  ...
  if (qh PRINTtransparent) {
    if (qh hull_dim != 4 || !qh DELAUNAY || qh VORONOI ||
        qh DROPdim >= 0) {  //-V112
      fprintf(qh ferr,
              "qhull input error: transparent Delaunay ('Gt') needs 3-d "
              "Delaunay ('d') w/o 'GDn'\n");
      qh_errexit(qh_ERRinput, NULL, NULL);
    }
    qh DROPdim = 3;
  ...
  for (i = qh_PRINTEND; i--;) {
    if (qh PRINTout[i] == qh_PRINTgeom)
      ...
      ivp_message(
          "qhull input error: option 'Ft' is not available for Voronoi "
          "vertices or halfspace intersection\n");
      qh_errexit(qh_ERRinput, NULL, NULL);
    } else if (qh PRINTout[i] == qh_PRINTcentrums && qh VORONOI) {
      ivp_message(
          "qhull input error: option 'Fc' is not available for Voronoi "
          "vertices ('v')\n");
      qh_errexit(qh_ERRinput, NULL, NULL);
  ...
  if (!qh KEEPcoplanar && !qh KEEPinside && !qh ONLYgood) {
    if ((qh PRINTcoplanar && qh PRINTspheres) || printcoplanar)
      ivp_message(
          "qhull input warning: options 'Fc', 'FP', and 'Gp' need option 'Qc' "
          "or 'Qi' to record coplanar/inside points\n");
  }
  if (printmath && (qh hull_dim > 3 || qh VORONOI || qh HALFspace)) {
    ivp_message(
        "qhull input error: Mathematica output is only available for 2-d and "
        "3-d convex hulls and Delaunay triangulations\n");
    qh_errexit(qh_ERRinput, NULL, NULL);
  }
  if (printgeom) {
    if (qh hull_dim > 4) {  //-V112
      ivp_message(
          "qhull input error: Geomview output is only available for 2-d, 3-d "
          "and 4-d\n");
      qh_errexit(qh_ERRinput, NULL, NULL);
    }
    if (qh PRINTnoplanes &&
    ...
    if (qh VORONOI && (qh hull_dim > 3 || qh DROPdim >= 0)) {
      ivp_message(
          "qhull input error: Geomview output for Voronoi diagrams only for "
          "2-d\n");
      qh_errexit(qh_ERRinput, NULL, NULL);
    }
  ...
  if (qh DROPdim >= 0) { /* after Geomview checks */
    if (qh DROPdim < qh hull_dim) {
      ...
      if (!printgeom || qh hull_dim < 3)
        ivp_message(
            "qhull input warning: drop dimension 'GD%d' is only available for "
            "3-d/4-d Geomview\n",
            qh DROPdim);
    } else
      qh DROPdim = -1;
...
/*-<a                             href="qh-c.htm#global"
  ...
  qh_initqhull_mem(  )
    ...
    if qh.MERGING,
      includes ridgeT
    calls qh_user_memsizes() to add up to 10 additional sizes for quick
  allocation (see numsizes below)
...
void qh_initthresholds(char *command) {
  ...
  maxdim = qh input_dim;
  if (qh DELAUNAY && (qh PROJECTdelaunay || qh PROJECTinput)) maxdim++;
  while (*s) {
    if (*s == '-') s++;
    if (*s == 'P') {
      s++;
...
void qh_option(const char *option, int *i, realT *r) {
  ...
  if (i) snprintf(buf + strlen(buf), sizeof(buf) - strlen(buf), " %d", *i);
  if (r) snprintf(buf + strlen(buf), sizeof(buf) - strlen(buf), " %2.2g", *r);
  len = strlen(buf);
...
void qh_restore_qhull(qhT **oldqh) {
  ...
  if (qh_qh) {
    fprintf(qhmem.ferr,
            "qhull internal error (qh_restore_qhull): did not save or free "
            "existing qhull\n");
    qh_errexit(qh_ERRqhull, NULL, NULL);
  }
