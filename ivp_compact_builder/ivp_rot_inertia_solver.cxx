 ...
 *... transform:	transformation matrix of the original points Out:
 *		center:		the center of mass x-axis
 *		mass:		the volume of the object
 *		inertia:	the inertia given that the object is centered
 ********************************************************************************/

IVP_Compact_Ledge_Mass_Center_Solver::IVP_Compact_Ledge_Mass_Center_Solver(
...
void IVP_Compact_Ledge_Mass_Center_Solver::integrate_triangle(
    const IVP_Compact_Ledge *ledge, const IVP_Compact_Triangle *triangle, int x,
    int y, int z) {
  IVP_U_Point hesse2;
  ...
  if (IVP_DOUBLE(IVP_Inline_Math::fabsd(hesse->k[z])) <
      IVP_DOUBLE(IVP_Inline_Math::fabsd(hesse->k[y]))) {
    ...
    if (IVP_Inline_Math::fabsd(hesse->k[z]) > P_DOUBLE_RES) {
      fzdy = -0.5f * hesse->k[y] /
             hesse->k[z];  // hesse form is vertical to area !!
    } else {
      fzdy = 0.0f;
    }
    fydz = 0.0f;
  ...
  for (l = 0; l < 3; l++) {
    ...
    {  // get the point coords
      ...
      const IVP_U_Float_Point *p1_object =
          IVP_CLS.give_object_coords(en, ledge);
    ...
    IVP_DOUBLE ly0 =
        p0.k[y] - lydx * p0.k[x];  // intersection point of line with x-axis

    IVP_DOUBLE ka, kb, kc;
    if (three_edge_area) {
      // loese Integrale im Bereich [x0 x1]: (y0 + x lydx)(y0 + x lydx) fzdy
      // [x][x] dx
      ka = fzdy * ly0 * ly0;
...
void IVP_Compact_Ledge_Find_Mass_Center::integrate_triangle(
    const IVP_Compact_Ledge *ledge, const IVP_Compact_Triangle *triangle) {
  const IVP_Compact_Edge *edge = triangle->get_first_edge();
  ...
  IVP_U_Float_Point surface_normal;
  surface_normal.inline_set_vert_to_area_defined_by_three_points(p0, p2, p1);
...
void IVP_Compact_Ledge_Find_Mass_Center::integrate_ledge(
    const IVP_Compact_Ledge *ledge) {
  const IVP_Compact_Triangle *tri;
  int t;
  for (t = 0, tri = ledge->get_first_triangle(); t < ledge->get_n_triangles();
       t++, tri = tri->get_next_tri()) {
    integrate_triangle(ledge, tri);
  }
}

void IVP_Compact_Ledge_Find_Mass_Center::integrate_ledges(
...
void IVP_Rot_Inertia_Solver::find_center_given_xyz(
    IVP_U_BigVector<IVP_Compact_Ledge> *v_ledges, int x, int y, int z,
    const IVP_U_Matrix *transform, IVP_DOUBLE *center, IVP_DOUBLE *mass,
    IVP_DOUBLE *inertia) {
...
static IVP_RETURN_TYPE localCalcMassCenterAndRotInertia(
  ...
  if (fmc.sum_det > surface * surface * surface * 1e-9f) {
    ...
    IVP_Rot_Inertia_Solver::find_center_given_xyz(
        all_ledges, 0, 1, 2, &transform, &check_center.k[0], &volume,
        &rot_inertia_2d.k[0]);
    IVP_Rot_Inertia_Solver::find_center_given_xyz(
        all_ledges, 1, 2, 0, &transform, &check_center.k[1], &volume,
        &rot_inertia_2d.k[1]);
    IVP_Rot_Inertia_Solver::find_center_given_xyz(
        all_ledges, 2, 0, 1, &transform, &check_center.k[2], &volume,
        &rot_inertia_2d.k[2]);
    ...
    IVP_CLS.calc_bounding_box(all_ledges->element_at(0), &min_extents,
                              &max_extents);
    ...
    radius = 0.5f *
             IVP_Inline_Math::sqrtd(min_extents.quad_distance_to(&max_extents));
    IVP_DOUBLE estimate_inertia = radius * radius * 0.5f;
    rotation_inertia_out->set(estimate_inertia, estimate_inertia,
                              estimate_inertia);
    return IVP_FAULT;
  }
}
...
IVP_RETURN_TYPE IVP_Rot_Inertia_Solver::calc_mass_center_and_rotation_inertia(
    const IVP_Compact_Surface *c_surface_in, IVP_U_Point *mass_center_out,
  ...
  return localCalcMassCenterAndRotInertia(&all_ledges, mass_center_out,
                                          rotation_inertia_out);
}

#ifdef HAVOK_MOPP
IVP_RETURN_TYPE IVP_Rot_Inertia_Solver::calc_mass_center_and_rotation_inertia(
    const IVP_Compact_Mopp *c_mopp_in, IVP_U_Point *mass_center_out,
    IVP_U_Point *rotation_inertia_out) {
  IVP_U_BigVector<IVP_Compact_Ledge> all_ledges(128);
