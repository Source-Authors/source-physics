...
int IVP_SurfaceBuilder_Pointsoup::get_offset_from_pointlist(
    IVP_Template_Point *points, int length, IVP_U_Point *point) {
  int i;

  for (i = 0; i < length; i++) {
    if (hk_Math::almost_equal(points[i].k[0], point->k[0]) &&
...
int IVP_SurfaceBuilder_Pointsoup::get_offset_from_lineslist(
    IVP_Template_Line *lines, int length, int pointnr1, int pointnr2,
    char *reverse) {
  int x;

  for (x = 0; x < length; x++) {
    if ((pointnr1 == lines[x].p[0]) && (pointnr2 == lines[x].p[1])) {
      *reverse = 0;
      return ((ushort)x);
    }
    if ((pointnr1 == lines[x].p[1]) && (pointnr2 == lines[x].p[0])) {
      *reverse = 1;
      return (x);
    }
  }
...
IVP_Template_Polygon *IVP_SurfaceBuilder_Pointsoup::planes_to_template(
  ...
  // At this point we have the pointlist completed (still missing are the
  // linelist and the surfacelist).
  ...
  for (x = 0; x < face_count; x++) {
    ...
    for (int pointnr = 0; pointnr < face_vertex_count; pointnr++) {
      ...
      offset_point1 =
          get_offset_from_pointlist(templ->points, templ->n_points, point);
      ...
      // in 'offset_point1/2' we now have the points that build the current
      // line...

      // construct a unique hash key for the line (independent of pointoffset
      // order)
      ...
      if (lines_hash->find((char *)&phk) == 0) {
        ...
        struct point_hash_key2 *nphk = (struct point_hash_key2 *)p_calloc(
            1, sizeof(struct point_hash_key2));
  ...
  // now we have all lines (without duplicates) in our 'lines_vector' list and
  // just need to copy the data to the IVP_Template_Line structure.
  ...
  for (x = 0; x < face_count; x++) {
    ...
    templ->surfaces[x].lines =
        (ushort *)p_calloc(templ->surfaces[x].n_lines, sizeof(ushort));
    templ->surfaces[x].revert_line =
        new char[templ->surfaces[x].n_lines];  //-V121
    ...
    for (int pointnr = 0; pointnr < punktezahl; pointnr++) {
      char reverse;
      int line_index = get_offset_from_lineslist(
          templ->lines, templ->n_lines, offset_points[pointnr],
          offset_points[pointnr + 1], &reverse);
...
IVP_DOUBLE IVP_SurMan_PS_Plane::get_area_size() {
  ...
  for (int i = 0; i < points.len(); i++) {
    ...
    diff1.subtract(a, p0);
    IVP_U_Point cross;
    cross.calc_cross_product(&diff0, &diff1);
...
void IVP_SurfaceBuilder_Pointsoup::error_output(IVP_Template_Polygon *templ) {
  IVP_DEBUGCODE(
      ...
      //    "IVP_SurfaceBuilder_Pointsoup::convert_pointsoup_to_compact_ledge_internal()
      //    - failed to triangulize object!\n");
      ivp_debugmanager.dprint(IVP_DM_SURBUILD_POINTSOUP, "Object points:\n");
      int i; for (i = 0; i < templ->n_points; i++) {
      ...
      for (i = 0; i < templ->n_lines; i++) {
        ...
        ivp_debugmanager.dprint(
            IVP_DM_SURBUILD_POINTSOUP, "Distance [%d] - [%d] : %f\n", p0, p1,
            templ->points[p0].quad_distance_to(&templ->points[p1]));
      } ivp_debugmanager.dprint(IVP_DM_SURBUILD_POINTSOUP, "\n");)
  return;
}
...
IVP_Compact_Ledge *
IVP_SurfaceBuilder_Pointsoup::try_to_build_convex_ledge_from_qhull_result(
  ...
  *skip_point = IVP_FALSE;
  ...
  IVP_IF(1){IVP_IFDEBUG(IVP_DM_SURBUILD_POINTSOUP,
                        {
                          ...
                          FORALLvertices {
                            ...
                            ivp_debugmanager.dprint(
                                ...
                                "Point in convex hull: %f %f %f\n",
                                points->element_at(point_index)->k[0],
                                points->element_at(point_index)->k[1],
                                points->element_at(point_index)->k[2]);
                          }
                        })}

  IVP_U_Vector<IVP_SurMan_PS_Plane>
      planes;
  ...
  FORALLfacets {
    ...
    FOREACHvertex_(vertices) {
      int point_index = qh_pointid(vertex->point);
      point_indizes.add(
          reinterpret_cast<int *>(static_cast<hk_intp>(point_index)));
    ...
    if (area_sum <
        0.5 * P_MIN_EDGE_LEN * IVP_Inline_Math::ivp_sqrtf(qedge_sum)) {
      ...
      max_dist = -1.0;
      {
        for (int i1 = 0; i1 < point_indizes.len(); i1++) {
          *skip_point = IVP_TRUE;
          ...
          IVP_DOUBLE dist =
              plane->points.element_at(max_index)->quad_distance_to(p2);
          if (dist > max_dist) {
            max_index2 = i1;
            max_dist = dist;
          }
    ...
    IVP_IF(0) {
      IVP_IFDEBUG(IVP_DM_SURBUILD_POINTSOUP, {
        ...
        if (plane->points.len() > 3) {  //@@@SF: Debug [14.12.1999]
          ivp_debugmanager.dprint(
              IVP_DM_SURBUILD_POINTSOUP,
              "[Testing/Debug] Qhull : more than 3 points on plane!\n");
        }
      })
    }
  }
...
IVP_SurfaceBuilder_Pointsoup::convert_pointsoup_to_compact_ledge_internal(
  ...
  IVP_IF(1) {
    IVP_IFDEBUG(IVP_DM_SURBUILD_POINTSOUP, {
      ivp_debugmanager.dprint(
          IVP_DM_SURBUILD_POINTSOUP,
          "*** Starting building new convex hull from pointsoup.\n");
    })
  }
  ...
  coordT *points2 =
      (coordT *)p_calloc(points_in->len() * (dim + 1), sizeof(coordT));
  ...
  FILE *outfile = NULL;  // output from qh_produce_output() use NULL to skip
                         // qh_produce_output()
  FILE *errfile = NULL;  // error messages from qhull code
#else
  FILE *outfile = stderr;  // stdout // output from qh_produce_output() use NULL
                           // to skip qh_produce_output()
  ...
  for (IVP_DOUBLE random_eps = 1e-12f; random_eps < 0.02f;) {
    snprintf(flags, sizeof(flags),
             "qhull Qs Pp C-0 W1e-14 E1.0e-18");  // "qhull QbB Pp"
    int exitcode = 0;
    if (try_unjumbled) {
      if (qhull_free_flag) qh_freeqhull(!qh_ALL); /* free long memory  */
      exitcode = qh_new_qhull(dim, numpoints, points2, ismalloc, flags, outfile,
                              errfile);
    ...
    if (exitcode || !try_unjumbled) {
      ...
      snprintf(flags, sizeof(flags), "qhull Qs QJ%G C-0 Pp W1e-14 E1.0e-18",
               random_eps);                       // "qhull QbB Pp"
      if (qhull_free_flag) qh_freeqhull(!qh_ALL); /* free long memory  */
      exitcode = qh_new_qhull(dim, numpoints, points2, ismalloc, flags, outfile,
                              errfile);
      qhull_free_flag = IVP_TRUE;
    ...
    res = try_to_build_convex_ledge_from_qhull_result(&points, &skip_point,
                                                      skip_list, use_list);
    ...
    if (res)  //@@CB
    {
      ...
      int compact_ledge_num_points =
          (res->get_size() / 16) - (res->get_n_triangles()) - 1;

      {
        ...
        for (int i = 0; i < compact_ledge_num_points; i++) {
          ...
          IVP_U_Point *associated_point =
              point_hash.find_point(reinterpret_cast<IVP_U_Point *>(cpp));
  ...
  if (!res) {
    IVP_IFDEBUG(IVP_DM_SURBUILD_POINTSOUP, {
      ivp_debugmanager.dprint(
          IVP_DM_SURBUILD_POINTSOUP,
          "*** "
          "IVP_SurfaceBuilder_Pointsoup::convert_pointsoup_to_template_polygon "
          "- couldn't build convex hull! Skipping object...\n");
      int i;
...
IVP_SurfaceBuilder_Pointsoup::convert_triangle_to_compace_ledge(
    IVP_U_Point *p0, IVP_U_Point *p1, IVP_U_Point *p2) {
  if (!single_tri_ledge) {
    ...
    single_tri_ledge = IVP_SurfaceBuilder_Polygon_Convex::
        convert_templateledgepolygonsoup_to_ledge(&ledge_templ);
  }

  IVP_U_Hesse th;
  th.inline_set_vert_to_area_defined_by_three_points(p0, p1, p2);
  if (th.quad_length() < P_DOUBLE_RES) return NULL;

  IVP_Compact_Ledge *cl =
      (IVP_Compact_Ledge *)ivp_malloc_aligned(single_tri_ledge->get_size(), 16);
...
IVP_SurfaceBuilder_Pointsoup::convert_pointsoup_to_compact_ledge(
  ...
  if (n_points == 3) {
    ...
    return convert_triangle_to_compace_ledge(
        points->element_at(0), points->element_at(1), points->element_at(2));
  }

  // use QHULL to convert pointsoup
  return IVP_SurfaceBuilder_Pointsoup::
      convert_pointsoup_to_compact_ledge_internal(points);
}
    ...
    IVP_IF(1) {
      IVP_IFDEBUG(IVP_DM_SURBUILD_POINTSOUP, {
        ivp_debugmanager.dprint(
            IVP_DM_SURBUILD_POINTSOUP,
            "*** "
            "IVP_SurfaceBuilder_Pointsoup::convert_pointsoup_to_compact_"
            "surface - skipping ledge due to invalid topology\n");
      })
    }
  }
