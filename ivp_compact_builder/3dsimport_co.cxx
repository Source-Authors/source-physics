  ...
  for (int currvtx = 0; currvtx < verts; currvtx++) {
    ...
    if (vrtmap[currvtx].marked == 0) {
      ...
      for (int runvtx = currvtx + 1; runvtx < verts; runvtx++) {
        ...
        if (vrtmap[runvtx].marked == 1) continue;
...
IVP_DOUBLE ipoe_calc_s_val(const IVP_U_Point *p_world, const IVP_U_Point *vec,
                           const IVP_U_Point *tp, const IVP_U_Point *tp_next) {
  // calcs intersect pos
  // von lot von p auf this (rel. zu this)
  //    IVP_U_Point *tp      = give_world_coords_AT(edge, m_cache_edge);
  //    IVP_U_Point *tp_next = give_world_coords_AT(edge->get_next(),
  //    m_cache_edge);
...
IVP_BOOL is_point_on_edge(IVP_U_Point *point, IVP_U_Point *edge_startpoint,
                          IVP_U_Point *edge_endpoint) {
  ...
  if (edge.get_quad_dist_to_point(point) < IVP_MERGE_EPS * IVP_MERGE_EPS) {
    IVP_DOUBLE s_val =
        ipoe_calc_s_val(point, &vec, edge_startpoint, edge_endpoint);
  ...
  for (x = 0; x < concave_polyhedron->points.len(); x++) {
    ...
    for (y = 0; y < concave_polyhedron->faces.len(); y++) {
      IVP_Concave_Polyhedron_Face *face =
          concave_polyhedron->faces.element_at(y);
      ...
      for (z = 0; z < n_points; z++) {
        ...
        if (is_point_on_edge(p, concave_polyhedron->points.element_at(start),
                             concave_polyhedron->points.element_at(end))) {
          ivp_message("T-Junction found for point with offset <%d>\n",
                      concave_polyhedron->points.index_of(p));
          IVP_Concave_Polyhedron_Face_Pointoffset *new_offset =
...
IVP_Concave_Polyhedron *IVP_SurfaceBuilder_3ds::convert_3ds_to_concave(
    const char *filename, IVP_Template_SurfaceBuilder_3ds *params) {
//****
#ifndef GEKKO
  //****
...
//	ConvertFloatsToInts(scene);
  ...
  if (!(flags & NORMDUP)) {
    if (flags & VERBOSE) ivp_message("Removing duplicated vertices ");
...
class P_Hardware;
int p_graphlib_robust_convert_3dmax_object_to_compact_ledges(
