...
/*-<a                             href="qh-c.htm#qhull"
  ...
  returns:
    ...
    returns global variables
      qh.hulltime, qh.max_outside, qh.interior_point, qh.max_vertex,
  qh.min_vertex
...
void qh_qhull(void) {
  ...
  if (!qh STOPpoint && !qh STOPcone) {
    ...
    if (qh ZEROall_ok && !qh TESTvneighbors && !qh WAScoplanar) {
      trace2((qh ferr,
              "qh_qhull: all facets are clearly convex and no coplanar points. "
              " Post-merging and check of maxout not needed.\n"));
    } else {
  ...
  if (qh KEEPnearinside && !qh maxoutdone) qh_nearcoplanar();
...
/*-<a                             href="qh-c.htm#qhull"
  ...
  returns:
    ...
    clear qh.maxoutdone (will need to call qh_check_maxout() for
  facet->maxoutside) if unknown point, adds a pointer to qh.other_points do not
  deallocate the point's coordinates
...
boolT qh_addpoint(pointT *furthest, facetT *facet, boolT checkdist) {
  ...
  qh maxoutdone = False;
  if (qh_pointid(furthest) == -1) qh_setappend(&qh other_points, furthest);
  if (!facet) {
  ...
  if (qh ONLYgood) qh_attachnewfacets(/*visible_list*/);
  qh_matchnewfacets();
  qh_updatevertices();
  ...
  if (qh STOPpoint > 0 && qh furthest_id == qh STOPpoint - 1) return False;
  qh_resetlists(True /*qh visible_list newvertex_list newfacet_list */);
  trace2((qh ferr,
...
qhull precision error: %d attempts to construct a convex hull\n\
    ...
    if (qh JOGGLEmax < REALmax / 2) qh_joggleinput();
    qh_initbuild();
    qh_buildhull();
...
void qh_buildhull(void) {
  ...
  FORALLfacets {
    if (facet->visible || facet->newfacet) {
      ivp_message(
          "qhull internal error (qh_buildhull): visible or new facet f%d in "
          "facet list\n",
          facet->id);
  ...
  while ((furthest = qh_nextfurthest(&facet))) {
    ...
    if (!qh_addpoint(furthest, facet, qh ONLYmax)) break;
  }
...
At %02d:%02d:%02d & %2.5g CPU secs, qhull has created %d facets and merged %d.\n\
 The current hull contains %d facets and %d vertices.  Last point was p%d\n",
        ...
        0, 0, 0,
#endif
        cpu, qh facet_id - 1, total, qh num_facets, qh num_vertices,
        qh furthest_id);
    return;
  }
...
#endif
        cpu, qh facet_id - 1, total, qh num_facets, qh num_vertices,
        qh num_outside + 1, furthestid, qh vertex_id, dist, getid_(facet));
  } else if (qh IStracing >= 1) {
    cpu = qh_CPUclock - qh hulltime;
    cpu /= qh_SECticks;
    qh_distplane(furthest, facet, &dist);
    ivp_message(
        "qh_addpoint: add p%d (v%d) to hull of %d facets (%2.2g above f%d) and "
        "%d outside at %4.4g CPU secs.  Previous was p%d.\n",
...
*/
void qh_errexit2
    [[noreturn]] (int exitcode, facetT *facet, facetT *otherfacet) {
    ...
    FOREACHneighbor_(visible) {
      ...
      if (dist > qh MINvisible) {
        ...
        if (dist > -qh MAXcoplanar) {
          ...
          trace2((qh ferr,
                  "qh_findhorizon: point p%d is coplanar to horizon f%u, "
...
pointT *qh_nextfurthest(facetT **visible) {
  ...
  while ((facet = qh facet_next) != qh facet_tail) {
    ...
    if (qh NARROWhull) {
      if (facet->notfurthest) qh_furthestout(facet);
    ...
    if (qh RANDOMoutside) {
      ...
      if (qh NARROWhull) {
        FORALLfacets {
          if (facet == qh facet_next) break;
          if (facet->outsideset) outcoplanar += qh_setsize(facet->outsideset);
        }
      }
      randr = qh_RANDOMint;
      ...
      ivp_message(
          "qhull internal error (qh_nextfurthest): num_outside %d is too "
...
void qh_partitionall(setT *vertices, pointT *points, int numpoints) {
  ...
  FOREACHvertex_(vertices) {
    if ((id = qh_pointid(vertex->point)) >= 0) SETelem_(pointset, id) = NULL;
  }
  id = qh_pointid(qh GOODpointp);
  if (id >= 0 && qh STOPcone - 1 != id && -qh STOPpoint - 1 != id)
    SETelem_(pointset, id) = NULL;
  if (qh GOODvertexp && qh ONLYgood && !qh MERGING) { /* matches qhull()*/
    if ((id = qh_pointid(qh GOODvertexp)) >= 0) SETelem_(pointset, id) = NULL;
  }
  ...
  if (qh BESToutside || qh MERGING || qh KEEPcoplanar || qh KEEPinside) {
    qh findbestnew = True;
    FOREACHpoint_i_(pointset) {
      if (point) qh_partitionpoint(point, qh facet_list);
    }
    qh findbestnew = False;
  }
...
/*-<a                             href="qh-c.htm#qhull"
  ...
  design:
    if dist undefined
      ...
      if point sufficiently below facet (depends on qh.NEARinside and
  qh.KEEPinside) exit if keeping coplanar/nearinside/inside points if point is
  above furthest coplanar point append point to coplanar set (it is the new
  furthest) update qh.max_outside else append point one before end of coplanar
  set else update qh.max_outside
*/
void qh_partitioncoplanar(pointT *point, facetT *facet, realT *dist) {
  ...
  if (qh KEEPcoplanar + qh KEEPinside + qh KEEPnearinside) {
    ...
    if (!oldfurthest || dist2 < bestdist) {
      ...
      if (bestdist > qh max_outside) {
        ...
        if (bestdist > qh TRACEdist) istrace = True;
      }
    } else
  ...
  if (istrace) {
    ivp_message(
        "qh_partitioncoplanar: ====== p%d increases max_outside to %2.2g of "
        "f%d last p%d\n",
...
void qh_partitionpoint(pointT *point, facetT *facet) {
  ...
  if (qh NARROWhull) {
    ...
    if (qh KEEPnearinside) {
      if (bestdist >= -qh NEARinside) isoutside = True;
    } else if (bestdist >= -qh MAXcoplanar)
      isoutside = True;
  }
  ...
  if (isoutside) {
    ...
    if (qh DELAUNAY) qh_precision("nearly incident point");
    ...
    trace4((qh ferr,
            ...
            "dist %2.2g\n",
            qh_pointid(point), bestfacet->id, bestdist));
    if (qh KEEPinside) qh_partitioncoplanar(point, bestfacet, &bestdist);
  }
} /* partitionpoint */
...
void qh_partitionvisible(/*visible_list*/ boolT allpoints, int *numoutside) {
  ...
  if (qh ONLYmax) maximize_(qh MINoutside, qh max_vertex);
  *numoutside = 0;
  FORALLvisible_facets {
    if (!visible->outsideset && !visible->coplanarset) continue;
    newfacet = visible->f.replace;
    count = 0;
    while (newfacet && newfacet->visible) {
      newfacet = newfacet->f.replace;
      if (count++ > qh facet_id) qh_infiniteloop(visible);
    }
    if (!newfacet) newfacet = qh newfacet_list;
...
void qh_printsummary(FILE *fp) {
  ...
  if (id >= 0 && qh STOPcone - 1 != id && -qh STOPpoint - 1 != id) size--;
...
Convex hull of %d points in %d-d:\n\n",
  ...
  if (qh MERGING) {
    ...
    if (outerplane > 2 * qh DISTround) {
      ...
      if (ratio > 0.05 && qh ONEmerge > qh MINoutside &&
