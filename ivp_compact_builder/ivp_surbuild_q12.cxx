...
// This file contains software technology by Id Software, Inc. ("Id
// Technology"). This data has been taken from the file 'bspfile.h' which is
// part of the archive 'qutils.zip'. The archive is freely available at ID's own
// ftp server 'ftp.id-software.com'.
//
// Ipion, June 8th 1999
...
IVP_FLOAT BigFloat(IVP_FLOAT l) { return l; }
...
IVP_FLOAT LittleFloat(IVP_FLOAT l) { return l; }
...
void IVP_SurfaceBuilder_Q12::swap_bsp_data() {
  ...
  for (i = 0; i < this->n_clipnodes; i++) {
    ...
    this->dclipnodes[i].children[0] =
        LittleShort(this->dclipnodes[i].children[0]);
    this->dclipnodes[i].children[1] =
        LittleShort(this->dclipnodes[i].children[1]);
  }
...
// ****
  ...
  this->n_models = this->init_bsp_data(LUMP_MODELS, (void **)&this->dmodels,
                                       sizeof(dmodel_t));
  // numvertexes     = init_bsp_data(LUMP_VERTEXES    , dvertexes    ,
  // sizeof(dvertex_t));
  this->n_planes = this->init_bsp_data(LUMP_PLANES, (void **)&this->dplanes,
                                       sizeof(dplane_t));
  // numleafs        = init_bsp_data(LUMP_LEAFS       , dleafs       ,
  // sizeof(dleaf_t));
  this->n_nodes =
      this->init_bsp_data(LUMP_NODES, (void **)&this->dnodes, sizeof(dnode_t));
  // numtexinfo      = init_bsp_data(LUMP_TEXINFO     , texinfo      ,
  // sizeof(texinfo_t));
  this->n_clipnodes = this->init_bsp_data(
      LUMP_CLIPNODES, (void **)&this->dclipnodes, sizeof(dclipnode_t));
  // numfaces        = this->init_bsp_data(LUMP_FACES       , dfaces       ,
  // sizeof(dface_t)); nummarksurfaces = this->init_bsp_data(LUMP_MARKSURFACES,
  // dmarksurfaces, sizeof(dmarksurfaces[0])); numsurfedges    =
  // this->init_bsp_data(LUMP_SURFEDGES   , dsurfedges   ,
  // sizeof(dsurfedges[0])); numedges        = this->init_bsp_data(LUMP_EDGES ,
  // dedges       , sizeof(dedge_t));
  ...
  IVP_IF(1) { ivp_message("Number of models in bsp file: %d\n", n_models); }
...
void IVP_SurfaceBuilder_Q12::init_q12bsp_from_memory(
    int version, int n_models_in, dmodel_t *dmodels_in, int n_planes_in,
    dplane_t *dplanes_in, int n_nodes_in, dnode_t *dnodes_in,
...
void IVP_SurfaceBuilder_Q12::convert_model(int model) {
  this->min_x =
      (IVP_FLOAT)(dmodels[model].mins[0] -
                  (1.0f / this->scale));  // blow-up (1m) necessary to avoid
                                          // 2-dimensional objects on the
                                          // level's boundaries
...
void IVP_SurfaceBuilder_Q12::convert_solid_node() {
  ...
  new_ledge =
      IVP_SurfaceBuilder_Halfspacesoup::convert_halfspacesoup_to_compact_ledge(
          this->halfspaces, this->pointmerge_threshold);
  if (new_ledge) {
...
void IVP_SurfaceBuilder_Q12::nodes_to_planes() {
  ...
  for (i = 0; i < this->nodes.len(); i++) {
    dplane_t *bsp_plane =
        &dplanes[dnodes[(hk_intp)nodes.element_at(i)].planenum];
    if (i == this->nodes.len() - 1) {
      if (dnodes[(hk_intp)this->nodes.element_at(i)].children[0] == -1) {
        create_and_insert_plane(bsp_plane->normal[0], bsp_plane->normal[1],
                                bsp_plane->normal[2], bsp_plane->dist);
      } else {
        create_and_insert_plane(-bsp_plane->normal[0], -bsp_plane->normal[1],
                                -bsp_plane->normal[2], -bsp_plane->dist);
      }
    } else {
      if (dnodes[(hk_intp)this->nodes.element_at(i)].children[0] ==
          (hk_intp)this->nodes.element_at(i + 1)) {
        create_and_insert_plane(bsp_plane->normal[0], bsp_plane->normal[1],
                                bsp_plane->normal[2], bsp_plane->dist);
      } else {
        create_and_insert_plane(-bsp_plane->normal[0], -bsp_plane->normal[1],
...
// ------------------------------------------------------------------------
void IVP_SurfaceBuilder_Q12::create_and_insert_plane(IVP_FLOAT nx, IVP_FLOAT ny,
                                                     IVP_FLOAT nz,
                                                     IVP_FLOAT dist) {
  ...
  IVP_U_Hesse plane;
  plane.set(nx, -nz, ny);
  plane.hesse_val = -(dist + this->shrink_value) *
                    this->scale;  // scale to a basis of 1 unit = 1 meter
...
void IVP_SurfaceBuilder_Q12::convert_q12bsp_model_to_compact_ledges(
    int model, IVP_DOUBLE scaling_factor, IVP_DOUBLE shrink_value_in,
    IVP_FLOAT pointmerge_threshold_in,
    IVP_U_Vector<IVP_Compact_Ledge> *ledges_out) {
  if (!this->dmodels)
    return;  // something went wrong while loading/initializing the bsp data.
             // aborting...
...
IVP_Compact_Surface *
IVP_SurfaceBuilder_Q12::convert_q12bsp_model_to_single_compact_surface(
    int model, IVP_DOUBLE scaling_factor, IVP_DOUBLE shrink_value_in,
    IVP_FLOAT pointmerge_threshold_in) {
  int i;
  IVP_U_Vector<IVP_Compact_Ledge> ledges_local;

  IVP_SurfaceBuilder_Q12::convert_q12bsp_model_to_compact_ledges(
      model, scaling_factor, shrink_value_in, pointmerge_threshold_in,
      &ledges_local);
