...
void debug_sphere_output(IVV_Sphere *sphere) {
  ...
  if (ivp_debug_sf_max_treedepth < ivp_debug_sf_treedepth)
    ivp_debug_sf_max_treedepth = ivp_debug_sf_treedepth;

  // for (int x=0; x<ivp_debug_sf_indent; x++) {
  //	ivp_message("  ");
  // }
  // ivp_message("center: %.6f/%.6f/%.6f --- Radius: %.6f\n",
  // sphere->center.k[0], sphere->center.k[1], sphere->center.k[2],
  // sphere->radius); ivp_debug_sf_indent++;
  if (sphere->child_1) {
...
void IVP_SurfaceBuilder_Ledge_Soup::insert_ledge(IVP_Compact_Ledge *c_ledge) {
  if (!c_ledge) {
    IVP_IFDEBUG(IVP_DM_SURBUILD_LEDGESOUP, {
      ivp_debugmanager.dprint(
          IVP_DM_SURBUILD_LEDGESOUP,
          "warning: tried to add NULL ledge in "
...
IVP_Compact_Surface *IVP_SurfaceBuilder_Ledge_Soup::compile(
    IVP_Template_Surbuild_LedgeSoup *templ) {
  IVP_Template_Surbuild_LedgeSoup t2;
  if (!templ) templ = &t2;
  this->parameters = templ;

  if (this->c_ledge_vec.len() == 0)
    return NULL;  // invalid (empty) ledge list. aborting...
  ...
  if (number_of_terminal_spheres > 1) {
    ...
    ivp_debug_sf_last_node_was_terminal = IVP_FALSE;
    debug_sphere_output(
        this->spheres_cluster[this->spheres_cluster[0].next].sphere);
    ivp_message("Tree depth: %d\n", ivp_debug_sf_max_treedepth);
    ivp_message("# of terminals: %d\n", this->number_of_terminal_spheres);
  }
  // ivp_message("# of nodes with single terminal child: %d\n",
  ...
  if (this->number_of_terminal_spheres > 1 &&
      !parameters->link_to_input_compact_ledges) {
    if (parameters->merge_points == IVP_SLMP_MERGE_AND_REALLOCATE) {
      res = (IVP_Compact_Surface *)ivp_malloc_aligned(
          compact_surface->byte_size, 16);
...
void IVP_SurfaceBuilder_Ledge_Soup::add_ledge_tree_to_convex_hull(
    class IVP_Compact_Recursive &cr, IVV_Sphere *node) {
  if (!node) return;
...
IVP_Compact_Ledge * __thiscall
build_root_convex_hull(IVP_SurfaceBuilder_Ledge_Soup *this)

{
  ...
  if (local_30 != (IVP_Compact_Ledge *)0x0) {
    ...
    *(IVP_Compact_Ledge **)((this->c_ledge_vec).super_IVP_U_Vector_Base.elems +
uVar4) = local_30; puVar1 =
&(this->c_ledge_vec).super_IVP_U_Vector_Base.n_elems; *puVar1 = *puVar1 + 1;
    ...
    *(IVV_Sphere **)((this->rec_spheres).super_IVP_U_Vector_Base.elems + uVar4)
= pIVar2; puVar1 = &(this->rec_spheres).super_IVP_U_Vector_Base.n_elems; *puVar1
= *puVar1 + 1;
  }
...
void IVP_SurfaceBuilder_Ledge_Soup::ledges_to_spheres() {
  // -----------------------------------------------------------------------------------
  // create a minimal sphere around each ledge and save it into a fix-sized
  // array...
  // -----------------------------------------------------------------------------------
  ...
  // allocate array: element #0 (dummy) always indicates the first valid element
  // through its 'next' pointer!
  this->spheres_cluster = (struct IVV_Sphere_Cluster *)p_calloc(
      this->number_of_terminal_spheres + 1, sizeof(IVV_Sphere_Cluster));
  ...
  for (ledge_cnt = 0; ledge_cnt < n_ledges; ledge_cnt++) {
    ...
    if (center.k[0] - radius < this->extents_min.k[0])
      this->extents_min.k[0] = center.k[0] - radius;
    if (center.k[0] + radius > this->extents_max.k[0])
      this->extents_max.k[0] = center.k[0] + radius;
    if (center.k[1] - radius < this->extents_min.k[1])
      this->extents_min.k[1] = center.k[1] - radius;
    if (center.k[1] + radius > this->extents_max.k[1])
      this->extents_max.k[1] = center.k[1] + radius;
    if (center.k[2] - radius < this->extents_min.k[2])
      this->extents_min.k[2] = center.k[2] - radius;
    if (center.k[2] + radius > this->extents_max.k[2])
      this->extents_max.k[2] = center.k[2] + radius;
  ...
  // ivp_message("Radius of smallest sphere: %f\n", this->smallest_radius);

  // @@@SF: for speedup of single-convex objects, move this into
  // sphere-clustering!
  if (ledge_cnt > 1) {
    ...
    // ivp_message("Extension in x-direction: %f\n", this->extents_max_x -
    // this->extents_min_x); ivp_message("Extension in y-direction: %f\n",
    // this->extents_max_y - this->extents_min_y); ivp_message("Extension in
...
void IVP_SurfaceBuilder_Ledge_Soup::ledges_to_boxes_and_spheres() {
  ...
  for (ledge_cnt = 0; ledge_cnt < n_ledges; ledge_cnt++) {
    ...
    sphere->center.set_interpolate(
        &max, &min, 0.5f);  // use center of bounding box as center of sphere
    rad.subtract(&max, &sphere->center);
    sphere->radius =
        rad.real_length();  // calculate radius of box-enclosing sphere

    if (sphere->center.k[0] - sphere->radius < this->extents_min.k[0])
      this->extents_min.k[0] = sphere->center.k[0] - sphere->radius;
    if (sphere->center.k[0] + sphere->radius > this->extents_max.k[0])
      this->extents_max.k[0] = sphere->center.k[0] + sphere->radius;
    if (sphere->center.k[1] - sphere->radius < this->extents_min.k[1])
      this->extents_min.k[1] = sphere->center.k[1] - sphere->radius;
    if (sphere->center.k[1] + sphere->radius > this->extents_max.k[1])
      this->extents_max.k[1] = sphere->center.k[1] + sphere->radius;
    if (sphere->center.k[2] - sphere->radius < this->extents_min.k[2])
      this->extents_min.k[2] = sphere->center.k[2] - sphere->radius;
    if (sphere->center.k[2] + sphere->radius > this->extents_max.k[2])
      this->extents_max.k[2] = sphere->center.k[2] + sphere->radius;

    // @@CB - scaling
    IVP_DOUBLE work =
        sphere->radius *
        dimension_steps;  // dimension_steps is constanct, 1/250 @@CB
    sphere->box_sizes[0] = int((max.k[0] - sphere->center.k[0]) / work) +
                           1;  // translate to origin, scale by 250, divide
    sphere->box_sizes[1] =
        int((max.k[1] - sphere->center.k[1]) / work) + 1;  // by radius, add 1
...
class IVP_Clustering_Shortrange_Interval_Min_Hash_Entry {
 public:
  ...
  IVP_Clustering_Shortrange_Interval_Min_Hash_Entry(IVP_BOOL start_in,
                                                    IVV_Sphere *sphere_in) {
    this->start = start_in;
...
void IVP_SurfaceBuilder_Ledge_Soup::cluster_spheres_bottomup(
    IVP_DOUBLE threshold_increase) {
  ...
  this->number_of_nodes =
      this->number_of_terminal_spheres;  // [# of nodes] = [# of term.nodes] +
                                         // [# of internal nodes]; needed for
                                         // calculation of compact_surface size!

  this->number_of_unclustered_spheres = this->number_of_terminal_spheres;
  while (this->number_of_unclustered_spheres > 1) {
    IVV_Cluster_Min_Hash cluster_min_hash(
        256);  //@@@SF: start value still has to be optimized!
    ...
    while (this->interval_minhash->counter > 0) {
      ...
      IVP_Clustering_Shortrange_Interval_Min_Hash_Entry *entry =
          (IVP_Clustering_Shortrange_Interval_Min_Hash_Entry *)this
              ->interval_minhash->find_min_elem();
      this->interval_minhash->remove((void *)entry);

      // value is left interval border (i.e. the smallest point occupied by
      // sphere)
      if (entry->start) {
        this->overlapping_spheres.add(entry->sphere);
        this->combine_spheres_in_vector(
            &cluster_min_hash);  // calculate all possible combinations of all
                                 // overlapping spheres
        ...
        if (key.key !=
            0) {  // verify that there were at least 2 spheres in vector and a
                  // new minimal sphere could be generated!
          // IVP_DOUBLE radius = cluster_min_hash->find_min_value(); // radius
          // of minimal sphere
          int sphere_1_number = key.spheres.s1;
          int sphere_2_number = key.spheres.s2;

          // remove all combinations from hash that contain the (soon to be)
          // clustered child spheres...
          this->remove_all_further_spherecombinations_from_hash(
              &cluster_min_hash, sphere_1_number);
          this->remove_all_further_spherecombinations_from_hash(
              &cluster_min_hash, sphere_2_number);
      ...
      // value is right interval border (i.e. the largest point occupied by
      // sphere)
      else {
        ...
        this->remove_all_further_spherecombinations_from_hash(
            &cluster_min_hash, entry->sphere->number);
      }
    ...
    if (this->built_spheres.len() == 0) {
      // ivp_message("No spheres could be combined. Increasing radius threshold
      // from %f to %f.\n", fixed_max_radius_for_new_sphere,
...
void IVP_SurfaceBuilder_Ledge_Soup::generate_interval_minhash(
    float fixed_max_radius) {
  IVV_Cluster_Min_Hash_Key new_key;

  int next_sphere;
  for (next_sphere = this->spheres_cluster[0].next; next_sphere != 0;
       next_sphere = this->spheres_cluster[next_sphere].next) {
    ...
    IVP_Clustering_Shortrange_Interval_Min_Hash_Entry
        *new_entry_interval_start =
            new IVP_Clustering_Shortrange_Interval_Min_Hash_Entry(IVP_TRUE,
                                                                  sphere);
    new_key.key = (hk_uintp)new_entry_interval_start;
    this->interval_minhash->add((void *)new_key.key,
                                sphere->center.k[this->longest_axis] - radius);

    // insert interval end into interval MinHash
    IVP_Clustering_Shortrange_Interval_Min_Hash_Entry *new_entry_interval_end =
        new IVP_Clustering_Shortrange_Interval_Min_Hash_Entry(IVP_FALSE,
                                                              sphere);
...
void IVP_SurfaceBuilder_Ledge_Soup::combine_spheres_in_vector(
    IVV_Cluster_Min_Hash *cluster_min_hash) {
  ...
  for (x = 0; x < this->overlapping_spheres.len(); x++) {
    ...
    for (y = 0; y < this->overlapping_spheres.len(); y++) {
      ...
      IVP_DOUBLE max1_x =
          sphere_1->center.k[0] + (sphere_1->box_sizes[0] * work1);
      IVP_DOUBLE max1_y =
          sphere_1->center.k[1] + (sphere_1->box_sizes[1] * work1);
      IVP_DOUBLE max1_z =
          sphere_1->center.k[2] + (sphere_1->box_sizes[2] * work1);
      IVP_DOUBLE min1_x =
          sphere_1->center.k[0] - (sphere_1->box_sizes[0] * work1);
      IVP_DOUBLE min1_y =
          sphere_1->center.k[1] - (sphere_1->box_sizes[1] * work1);
      ...
      IVP_DOUBLE max2_x =
          sphere_2->center.k[0] + (sphere_2->box_sizes[0] * work2);
      IVP_DOUBLE max2_y =
          sphere_2->center.k[1] + (sphere_2->box_sizes[1] * work2);
      IVP_DOUBLE max2_z =
          sphere_2->center.k[2] + (sphere_2->box_sizes[2] * work2);
      IVP_DOUBLE min2_x =
          sphere_2->center.k[0] - (sphere_2->box_sizes[0] * work2);
      IVP_DOUBLE min2_y =
          sphere_2->center.k[1] - (sphere_2->box_sizes[1] * work2);
...
void IVP_SurfaceBuilder_Ledge_Soup::
    remove_all_further_spherecombinations_from_hash(
        IVV_Cluster_Min_Hash *cluster_min_hash, int spherenumber) {
  // ----------------------------------------------------------------------
...
IVV_Sphere *IVP_SurfaceBuilder_Ledge_Soup::build_minimal_sphere(
    IVV_Sphere *sphere_1, IVV_Sphere *sphere_2) {
  ...
  new_sphere->number =
      sphere_1->number;  // replace sphere_1 (left sphere) with mothersphere
...
void IVP_SurfaceBuilder_Ledge_Soup::
    replace_childspheres_in_spherelist_with_motherspheres() {
  int x;
  for (x = 0; x < this->built_spheres.len(); x++) {
    ...
    // replace old left sphere with new mothersphere, remove old right sphere
    // completely...
    this->spheres_cluster[child_sphere_1->number].sphere = mothersphere;

    if (child_sphere_2->number == this->spheres_cluster[0].next) {
      this->spheres_cluster[0].next =
          this->spheres_cluster[child_sphere_2->number].next;
    }
    this->spheres_cluster[child_sphere_2->number].sphere = NULL;
    this->spheres_cluster[this->spheres_cluster[child_sphere_2->number]
                              .previous]
        .next = this->spheres_cluster[child_sphere_2->number].next;
    this->spheres_cluster[this->spheres_cluster[child_sphere_2->number].next]
        .previous = this->spheres_cluster[child_sphere_2->number].previous;

    this->number_of_nodes++;  // [# of nodes] = [# of term.nodes] + [# of
                              // internal nodes]
...
void IVP_SurfaceBuilder_Ledge_Soup::cluster_spheres_topdown_mediancut(
    IVP_DOUBLE /*threshold_increase*/) {
  IVP_U_Vector<IVV_Sphere> terminals;

  for (int next_sphere = this->spheres_cluster[0].next; next_sphere != 0;
       next_sphere = this->spheres_cluster[next_sphere].next) {
  ...
  this->spheres_cluster[this->spheres_cluster[0].next].sphere =
      this->cluster_spheres_topdown_mediancut_recursively(&terminals);

  return;
}

void IVP_SurfaceBuilder_Ledge_Soup::calculate_boundingbox(
    IVP_U_Vector<IVV_Sphere> *terminals, IVP_U_Float_Point *ext_min,
...
IVV_Sphere *
IVP_SurfaceBuilder_Ledge_Soup::cluster_spheres_topdown_mediancut_recursively(
    IVP_U_Vector<IVV_Sphere> *terminals) {
  // Proceedings:
  // 1. calculate bounding box (and smallest enclosing sphere) for set of
  // terminals
  // 2. find longest axis
  // 3. calculate median
  // 4. assign terminals to left- and righthand vectors according to their
  // centers' positions
  ...
  if (terminals->len() == 1)
    return (terminals->element_at(0));  // only one terminal left: return it

  // calculate the bounding box for the supplied set of terminals
  this->calculate_boundingbox(terminals, &this->extents_min,
                              &this->extents_max);
  ...
  IVP_DOUBLE new_radius;
  new_center.set_interpolate(
      &this->extents_max, &this->extents_min,
      0.5f);  // use center of bounding box as center of sphere
  ...
  new_sphere->center = new_center;
  new_sphere->compact_ledge = NULL;
  new_sphere->box_sizes[0] =
      int((this->extents_max.k[0] - new_center.k[0]) / work) + 1;
  new_sphere->box_sizes[1] =
      int((this->extents_max.k[1] - new_center.k[1]) / work) + 1;
  new_sphere->box_sizes[2] =
      int((this->extents_max.k[2] - new_center.k[2]) / work) + 1;

  // special case: "two spheres left in branch"
  // independent of the spheres' position we simply assign one of them to the
  // left branch and one to the right branch.
  if (terminals->len() == 2) {
    ...
    new_sphere->child_1 = this->cluster_spheres_topdown_mediancut_recursively(
        &lefthand_terminals);
    new_sphere->child_2 = this->cluster_spheres_topdown_mediancut_recursively(
        &righthand_terminals);
...
#endif
  {
    ...
    for (int median_average = 0; median_average < terminals->len();
         median_average++) {
    ...
    // (this is a bit tricky as we have to carefully handle terminals lying on
    // the median itself;
    //  to avoid ending up with an empty subbox by assigning a median-terminal
    //  randomly to the same side as the non-median-terminal, we examine the
    //  non-median-terminal and choose the opposite side; in case of more than
    //  one median-terminal we alternate between left and right side)
    ...
    for (int x = 0; x < terminals->len(); x++) {
      ...
      if (terminal_sphere->center.k[axis] < (median - P_FLOAT_RES)) {
        lefthand_terminals[axis]->add(
            terminal_sphere);  // sphere center left of the median

      } else if (terminal_sphere->center.k[axis] > (median + P_FLOAT_RES)) {
        righthand_terminals[axis]->add(
            terminal_sphere);  // sphere center right of the median
    ...
    // calculate children's boundingboxes for all three possible splits and
    // choose the split with optimal balanced boundingbox volume/surface
    ...
    IVP_U_Float_Point dim1(
        IVP_Inline_Math::fabsd(max[0].k[0] - min[0].k[0]),
        IVP_Inline_Math::fabsd(max[0].k[1] - min[0].k[1]),
        IVP_Inline_Math::fabsd(max[0].k[2] - min[0].k[2]));  // slower!
    IVP_U_Float_Point dim2(
        IVP_Inline_Math::fabsd(max[1].k[0] - min[1].k[0]),
    ...
    // IVP_FLOAT volume1 = fabs(max[0].k[0]-min[0].k[0]) *
    // fabs(max[0].k[1]-min[0].k[1]) * fabs(max[0].k[2]-min[0].k[2]); // faster!
    // IVP_FLOAT volume2 = fabs(max[1].k[0]-min[1].k[0]) *
    // fabs(max[1].k[1]-min[1].k[1]) * fabs(max[1].k[2]-min[1].k[2]); // faster!
    difference_in_boundingbox_volumes[axis] =
        IVP_Inline_Math::fabsd(volume2 - volume1);
#else

    // difference_in_boundingbox_volumes[axis] = max[left].k[axis] -
    // min[right].k[axis];
    IVP_U_Float_Point dim1(
        IVP_Inline_Math::fabsd(max[0].k[0] - min[0].k[0]),
        IVP_Inline_Math::fabsd(max[0].k[1] - min[0].k[1]),
  ...
  {  // find optimum axis
    ...
    for (int i = 0; i < 3; i++) {
      if (difference_in_boundingbox_volumes[i] < max_diff) {
        if ((lefthand_terminals[i]->len() != 0) &&
            (righthand_terminals[i]->len() != 0))
          chosen_axis = i;
  ...
  // recursively process subsets
  new_sphere->child_1 = this->cluster_spheres_topdown_mediancut_recursively(
      lefthand_terminals[chosen_axis]);
  new_sphere->child_2 = this->cluster_spheres_topdown_mediancut_recursively(
      righthand_terminals[chosen_axis]);
  ...
  this->number_of_nodes++;  // [# of nodes] = [# of term.nodes] + [# of internal
                            // nodes]
...
IVP_Compact_Surface *IVP_SurfaceBuilder_Ledge_Soup::allocate_compact_surface() {
  ...
  if (parameters->link_to_input_compact_ledges == IVP_FALSE) {
    for (int i = 0; i < this->terminal_spheres.len(); i++) {
      ...
      int ledge_size =
          source
              ->get_size();  // 'IVP_Compact_Ledge's have to be 16bytes-aligned!
  ...
  for (int j = 0; j < this->rec_spheres.len(); j++) {
    ...
    IVP_Compact_Ledge *source = sphere->compact_ledge;
    int ledge_size =
        source->get_size();  // 'IVP_Compact_Ledge's have to be 16bytes-aligned!
    cs_estimated_ledglelist_size += ledge_size;
    number_of_triangles_compiled += source->get_n_triangles();
    number_of_ledges_compiled++;
  }

  int cs_ledgetree_size =
      this->number_of_nodes * sizeof(IVP_Compact_Ledgetree_Node);
  // complete size of compact surface
  int cs_size =
      cs_header_size + cs_estimated_ledglelist_size + cs_ledgetree_size;

  cs_size = (cs_size + 15) & ~0xf;

  IVP_IF(1) { /*ivp_message("Compact surface size : %d\n", cs_size);*/ }

  this->compact_surface =
      (IVP_Compact_Surface *)ivp_malloc_aligned(cs_size * sizeof(char), 16);
  if (this->compact_surface == 0) {
    return (0);
  }
  if (number_of_ledges_compiled > 1 &&
      !parameters->link_to_input_compact_ledges) {
    if (parameters->merge_points == IVP_SLMP_MERGE_AND_REALLOCATE ||
        parameters->merge_points == IVP_SLMP_MERGE_NO_REALLOCATE) {
      int a = number_of_triangles_compiled;
      int b = 2;
      while (a) {
        a = a >> 1;
        b = b << 1;
      ...
      this->first_poly_point =
          (IVP_Compact_Poly_Point *)((char *)this->compact_surface +
                                     cs_header_size +
                                     number_of_triangles_compiled *
                                         sizeof(IVP_Compact_Triangle) +
                                     number_of_ledges_compiled *
                                         sizeof(IVP_Compact_Ledge));
    }
  } else {
  ...
  {
    this->first_compact_ledge =
        (IVP_Compact_Ledge *)((char *)this->compact_surface + cs_header_size);
  ...
  if (this->point_hash) {
    ...
    cs_ledglelist_size =
        number_of_triangles_compiled * sizeof(IVP_Compact_Triangle) +
  ...
  this->compact_surface->byte_size =
      cs_real_size;  // remember size of whole compact surface; useful for e.g.
                     // saving to disk
  this->compact_surface->offset_ledgetree_root =
      cs_header_size + cs_ledglelist_size;
  ...
  this->ledgetree_work =
      (IVP_Compact_Ledgetree_Node *)((char *)this->compact_surface +
                                     this->compact_surface
...
int IVP_SurfaceBuilder_Ledge_Soup::recompile_point_indizes_of_compact_ledge(
    IVP_Compact_Ledge *ledge_source, char *dest) {
  if (!point_hash) {
  ...
  int size = sizeof(IVP_Compact_Ledge) +
             sizeof(IVP_Compact_Triangle) * ledge_source->get_n_triangles();
 ...
 ********************************************************************************/
IVP_Compact_Ledgetree_Node *IVP_SurfaceBuilder_Ledge_Soup::build_ledgetree(
    IVV_Sphere *node) {
  IVP_Compact_Ledgetree_Node *current_node = this->ledgetree_work;

  IVP_ASSERT((hk_intp)current_node <
             (hk_intp)this->clt_highmem);  // ledgetree memory overwrite!
  IVP_ASSERT((hk_intp)current_node >=
             (hk_intp)this->clt_lowmem);  // ledgetree memory underwrite!
  ...
  if (node->child_1) {
    ...
    if (node->compact_ledge) {
      current_node->offset_compact_ledge =
          (char *)(node->compact_ledge) - (char *)current_node;
      node->compact_ledge->ledgetree_node_offset =
          (char *)current_node - (char *)node->compact_ledge;
    ...
    IVP_Compact_Ledgetree_Node *address_of_right_branch =
        build_ledgetree(node->child_2);

    // fill in specific internal ledgetree_node data
    current_node->offset_right_node =
        (char *)address_of_right_branch -
        (char *)current_node;  // calulcate offset to right branch
  } else {
    IVP_ASSERT(!node->child_2);
    ...
    current_node->offset_compact_ledge =
        (char *)node->compact_ledge -
        (char *)current_node;  // calculate backward offset to compact ledge
 ...
 ********************************************************************************/
void IVP_SurfaceBuilder_Ledge_Soup::ledgetree_debug_output(
    const IVP_Compact_Ledgetree_Node *node) const {
  // *** debugging START ******************************************************
  IVP_IF(1) {
    IVP_ASSERT((hk_intp)node <
               (hk_intp)this->clt_highmem);  // ledgetree memory overread!
    IVP_ASSERT((hk_intp)node >=
               (hk_intp)this->clt_lowmem);  // ledgetree memory underread!
  }
...
void IVP_SurfaceBuilder_Ledge_Soup::ledgetree_array_debug_output() {
  ...
  const IVP_Compact_Ledgetree_Node *nodes =
      compact_surface->get_compact_ledge_tree_root();

  int i;
  for (i = 0; i < this->number_of_nodes; i++) {
    node = &nodes[i];
    ivp_message("Node %d (address: 0x%zx / %zd)\n", i, (size_t)node,
                (size_t)node);
    // node->center.print("     center ");
    ivp_message("        radius %.6f)\n", node->radius);
    ivp_message("         left branch offset: %zd (address: 0x%zx / %zd)\n",
                sizeof(*node), (size_t)(node + 1), (size_t)(node + 1));
    ivp_message("        right branch offset: %zd (address: 0x%zx / %zd)\n",
                (size_t)node->offset_right_node,
                (size_t)node + node->offset_right_node,
                (size_t)node + node->offset_right_node);
    ivp_message("\n");
  }
  // *** debugging END ********************************************************
...
void IVP_SurfaceBuilder_Ledge_Soup::insert_radius_in_compact_surface() {
  ...
  IVP_Rot_Inertia_Solver::calc_mass_center_and_rotation_inertia(
      this->compact_surface, &mass_center, &rotation_inertia);
  ...
  this->compact_surface->max_factor_surface_deviation =
      int(1.0f + mass_radius_dev /
                     (mass_radius * IVP_COMPACT_SURFACE_DEVIATION_STEP_SIZE));
}
...
IVP_RETURN_TYPE IVP_SurfaceBuilder_Ledge_Soup::create_compact_ledgetree() {
  ...
  IVP_IF(1) {
    // ivp_debug_sf_indent=0;
    ledgetree_debug_output(
        this->compact_surface->get_compact_ledge_tree_root());
...
void IVP_SurfaceBuilder_Ledge_Soup::convert_ledges_to_templates(
  ...
  for (i = 0; i < ledges.len(); i++) {
    ...
    // copy all points of ledge (i.e. all points of all triangles) into
    // pointlist
    int j;
  ...
  // --------------------------------------------------
  //    *template_out =
  //    IVP_SurfaceBuilder_Pointsoup::planes_to_template(&points, &planes);
  return;
}
