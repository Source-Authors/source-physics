...
void qh_appendfacet(facetT *facet) {
  ...
  if (tail == qh newfacet_list) qh newfacet_list = facet;
  if (tail == qh facet_next) qh facet_next = facet;
...
void qh_appendvertex(vertexT *vertex) {
  ...
  if (tail == qh newvertex_list) qh newvertex_list = vertex;
  vertex->newlist = True;
...
void qh_attachnewfacets(void) {
  ...
  trace1(
      (qh ferr, "qh_attachnewfacets: attach horizon facets to new facets\n"));
  FORALLnew_facets {
    ...
    if (horizon->simplicial) {
      ...
      if (visible) {
        ...
        ivp_message(
            "qhull internal error (qh_attachnewfacets): couldn't find visible "
            "facet for horizon f%d of newfacet f%d\n",
  ...
  if (qh PRINTstatistics) {
    FORALLvisible_facets {
      if (!visible->f.replace) zinc_(Zinsidevisible);
    }
  }
} /* attachnewfacets */
...
boolT qh_checkflipped(facetT *facet, realT *distp, boolT allerror) {
  ...
  if (facet->flipped && !distp) return False;
  zzinc_(Zdistcheck);
...
void qh_delfacet(facetT *facet) {
  ...
  if (facet == qh tracefacet) qh tracefacet = NULL;
  if (facet == qh GOODclosest) qh GOODclosest = NULL;
  ...
  if (facet->ridges) qh_setfree(&(facet->ridges));
  qh_setfree(&(facet->vertices));
  if (facet->outsideset) qh_setfree(&(facet->outsideset));
...
void qh_deletevisible(void /*qh visible_list*/) {
  ...
  if (numvisible != qh num_visible) {
    ivp_message(
        "qhull internal error (qh_deletevisible): qh num_visible %d is not "
        "number of visible facets %d\n",
  ...
  FOREACHvertex_(qh del_vertices) qh_delvertex(vertex);
...
*/
setT *qh_facetintersect(facetT *facetA, facetT *facetB, int *skipA, int *skipB,
                        int prepend) {
  setT *intersect;
  ...
  if (i >= dim || j >= dim) {
    ivp_message(
        "qhull internal error (qh_facetintersect): f%d or f%d not in others "
        "neighbors\n",
        facetA->id, facetB->id);
    qh_errexit2(qh_ERRqhull, facetA, facetB);
  }
  intersect =
      qh_setnew_delnthsorted(facetA->vertices, qh hull_dim, *skipA, prepend);
...
/*-<a                             href="qh-c.htm#poly"
  ...
  notes:
    ...
    hashes memory addresses which may change over different runs of the same
  data using sum for hash does badly in high d
*/
unsigned qh_gethash(int hashsize, setT *set, int size, int firstindex,
                    void *skipelem) {
...
void qh_makenewplanes(void /* newfacet_list */) {
  ...
  FORALLnew_facets {
    if (!newfacet->mergehorizon) qh_setfacetplane(newfacet);
  }
    ...
    for each ridge of visible facet
      ...
      if neighbor is a horizon facet
        ...
        if neighbor coplanar
          ...
          (checks for non-simplicial facet with multiple ridges to visible
  facet) updates neighbor's ridge set (checks for simplicial neighbor to
  non-simplicial visible facet)
...
facetT *qh_makenew_nonsimplicial(facetT *visible, vertexT *apex, int *numnew) {
  ...
  FOREACHridge_(visible->ridges) {
    ...
    if (neighbor->visible) {
      ...
      if (qh ONLYgood) {
        if (!neighbor->simplicial) qh_setappend(&(newfacet->ridges), ridge);
      } else { /* qh_attachnewfacets */
        if (neighbor->seen) {
          if (neighbor->simplicial) {
            ivp_message(
                "qhull internal error (qh_makenew_nonsimplicial): simplicial "
                "f%d sharing two ridges with f%d\n",
        ...
        trace4((qh ferr,
                "qh_makenew_nonsimplicial: created facet f%d from v%d and r%d "
                "of horizon f%d\n",
  ...
  if (!qh ONLYgood) SETfirst_(visible->ridges) = NULL;
  return newfacet;
...
facetT *qh_makenew_simplicial(facetT *visible, vertexT *apex, int *numnew) {
  ...
  FOREACHneighbor_(visible) {
    if (!neighbor->seen && !neighbor->visible) {
      vertices =
          qh_facetintersect(neighbor, visible, &horizonskip, &visibleskip, 1);
      ...
      if (!qh ONLYgood) SETelem_(neighbor->neighbors, horizonskip) = newfacet;
      trace4((qh ferr,
              "qh_makenew_simplicial: create facet f%d top %d from v%d and "
              "horizon f%d skip %d top %d and visible f%d skip %d, flip? %d\n",
...
void qh_matchneighbor(facetT *newfacet, int newskip, int hashsize,
  ...
  for (scan = hash; (facet = SETelemt_(qh hash_table, scan, facetT));
    ...
    zinc_(Zhashtests);
    if (qh_matchvertices(1, newfacet->vertices, newskip, facet->vertices, &skip,
                         &same)) {
      if (SETelem_(newfacet->vertices, newskip) ==
          SETelem_(facet->vertices, skip)) {
        qh_precision("two facets with the same vertices");
        ivp_message(
            "qhull precision error: Vertex sets are the same for f%d and f%d.  "
            "Can not force output.\n",
      ...
      if (!qh PREmerge && !qh MERGEexact) {
        ...
        ivp_message(
            "qhull precision error: facets f%d, f%d and f%d meet at a ridge "
            "with more than 2 neighbors.  Can not continue.\n",
            facet->id, newfacet->id, getid_(matchfacet));
        qh_errexit2(qh_ERRprec, facet, newfacet);
      }
      SETelem_(newfacet->neighbors, newskip) = qh_DUPLICATEridge;
      newfacet->dupridge = True;
      if (!newfacet->normal) qh_setfacetplane(newfacet);
      qh_addhash(newfacet, qh hash_table, hashsize, hash);
      (*hashcount)++;
      if (!facet->normal) qh_setfacetplane(facet);
      if (matchfacet != qh_DUPLICATEridge) {
        ...
        if (matchfacet) {
          ...
          if (!matchfacet->normal) qh_setfacetplane(matchfacet);
          qh_addhash(matchfacet, qh hash_table, hashsize, hash);
          *hashcount += 2;
        }
      }
      trace4((qh ferr,
              "qh_matchneighbor: new f%d skip %d duplicates ridge for f%d skip "
              "%d matching f%d ismatch %d at hash %d\n",
...
void qh_matchnewfacets(void) {
  ...
  qh_newhashtable(numnew *
                  (qh hull_dim - 1)); /* twice what is normally needed,
                               but every ridge could be DUPLICATEridge */
  hashsize = qh_setsize(qh hash_table);
  FORALLnew_facets {
    for (newskip = 1; newskip < qh hull_dim;
         newskip++) /* furthest/horizon already matched */
      qh_matchneighbor(newfacet, newskip, hashsize, &hashcount);
  ...
  if (hashcount) {
    ivp_message(
        "qhull internal error (qh_matchnewfacets): %d neighbors did not match "
        "up\n",
        hashcount);
  ...
  if (qh IStracing >= 2) {
    ...
    ivp_message(
        "qh_matchnewfacets: %d new facets, %d unused hash entries .  hashsize "
        "%d\n",
  ...
  if (qh PREmerge || qh MERGEexact) {
    ...
    FORALLnew_facets {
      if (newfacet->normal) qh_checkflipped(newfacet, NULL, qh_ALL);
    }
...
boolT qh_matchvertices(int firstindex, setT *verticesA, int skipA,
  ...
  do
    if (elemAp != skipAp) {
      while (*elemAp != *elemBp++) {
        if (skipBp) return False;
        skipBp = elemBp; /* one extra like FOREACH */
      }
    }
  while (*(++elemAp));
  if (!skipBp) skipBp = ++elemBp;
...
facetT *qh_newfacet(void) {
  ...
  if (qh facet_id == qh tracefacet_id) qh tracefacet = facet;
...
void qh_removefacet(facetT *facet) {
  ...
  if (facet == qh newfacet_list) qh newfacet_list = next;
  if (facet == qh facet_next) qh facet_next = next;
  if (facet == qh visible_list) qh visible_list = next;
  if (previous) {
    previous->next = next;
...
void qh_removevertex(vertexT *vertex) {
  ...
  if (vertex == qh newvertex_list) qh newvertex_list = next;
  if (previous) {
    previous->next = next;
    next->previous = previous;
  ...
  qh num_vertices--;
  trace4(
      (qh ferr, "qh_removevertex: remove v%d from vertex_list\n", vertex->id));
} /* removevertex */
...
void qh_updatevertices(void) {
  ...
  trace3((qh ferr,
          "qh_updatevertices: delete interior vertices and update "
          "vertex->neighbors\n"));
  if (qh VERTEXneighbors) {
    FORALLvertex_(qh newvertex_list) {
      FOREACHneighbor_(vertex) {
        if (neighbor->visible) SETref_(neighbor) = NULL;
      }
