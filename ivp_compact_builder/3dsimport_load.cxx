...
static dword MemGetpos(void) { return MemBufferIndex; }
#endif
...
static void ReadTriMeshBlocks(dword p, char *name) {
  ...
  while ((pc = dgetpos()) < p) {
    ...
    switch ((int)id) {
      case CHUNK_VERTLIST:
        ReadVertList(pc + len, meshobj);
        break;
      case CHUNK_FACELIST:
        ReadFaceList(pc + len, meshobj);
        break;
      case CHUNK_MAPLIST:
        ReadMapList(pc + len, meshobj);
        break;
      case CHUNK_TRMATRIX:
        ReadTraMatrix(pc + len, meshobj);
        break;
...
static void ReadObjBlocks(dword p) {
  ...
  do {
    dread(&name[n++], 1);
    // dimhotepus: Check n in range before access.
...
H3dsScene *HRead3dsScene(void *ptr, int what, dword size) {
  ...
  if ((Scene = (H3dsScene *)p_malloc(sizeof(H3dsScene))) == 0) return 0;
