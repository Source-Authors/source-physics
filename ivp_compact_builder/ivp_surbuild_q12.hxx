...
// This header file contains software technology by Id Software, Inc. ("Id
...
class IVP_SurfaceBuilder_Q12 {
 private:
  ...
  void create_and_insert_plane(IVP_FLOAT nx, IVP_FLOAT ny, IVP_FLOAT nz,
                               IVP_FLOAT dist);

  // void clipnodes_to_planes();          // not used right now, but don't
  // delete! void convert_clipnode(int clipnode); // not used right now, but
  // don't delete! void convert_solid_clipnode();       // not used right now,
  // but don't delete!
 ...
 public:
   ...
   *			<scaling_factor>       factor by which the contents of
   *the bsptree will be scaled; <shrink_value>         all convex subparts of
   *the level will be shrunk by this value. (unit: m) [should be 0]
   ...
   *	Note:		The IPION engine uses meters as basic scale (so to
   *convert e.g. a bsptree using inches as basic scale you would provide a
   *factor of 0.0254f).
   *****************************************************************************/
  void convert_q12bsp_model_to_compact_ledges(
      int model, IVP_DOUBLE scaling_factor, IVP_DOUBLE shrink_value,
      IVP_FLOAT pointmerge_threshold,
   ...
   *
   * - 'load_q12bsp_file()'        : will load the supplied bspfile from disk
   *and import its data into the physical world
   ...
   *****************************************************************************/
  void init_q12bsp_from_memory(int version, int n_models_in,
                               dmodel_t *dmodels_in, int n_planes_in,
                               dplane_t *dplanes_in, int n_nodes_in,
   ...
   *			memory passed to the class by the
   *"init_q12bsp_from_memory" method)!
   *****************************************************************************/
  void unload_q12bsp();
