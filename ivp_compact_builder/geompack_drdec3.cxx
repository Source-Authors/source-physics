...
int IVP_Geompack::increase_memory(void **mem_block, int *mem_size,
                                  int size_of_element) {
////    void *new_mem = p_realloc(*mem_block, (*mem_size) * 2 *
/// size_of_element);
#ifndef GEKKO
...
void IVP_Geompack::decompose(struct geompack_parameters *params) {
  ...
  this->size_intworkarray =
      12;  // orig: maxiw [5000]. Should be divisible by 3 & 4!;
  this->size_doubleworkarray =
      12;  // orig: maxwk [5000]. Should be divisible by 3 & 4!;
  this->size_vcl =
      params->nvc + 2;  // number of vertices (NOT bytesize of array!)
  this->size_polyhedronfirstfaceoffset =
      2;  // Leave this to "2" if there will never be more than one polyhedron
          // to decompose. <orig. maxhf [200]>
  this->size_polyhedronfaceindices =
      1;  // number of entries (NOT bytesize of array!) <orig: maxpf [2000]>
  this->size_facearrays = params->nface + 2;  // <orig. maxfp [800]>
  this->size_facevertexarrays =
      params->facep[params->nface * 3] + 2;  // <orig. maxfv>
  this->size_ev = 200;
  ...
  *npolh_out = 0;
  ...
  this->g_intworkarray =
      (int *)p_calloc(this->size_intworkarray, sizeof(int));  // [5000]
  this->g_doubleworkarray =
      (double *)p_calloc(this->size_doubleworkarray, sizeof(double));  // [5000]
  this->g_polyhedronfirstfaceoffset = (int *)p_calloc(
      this->size_polyhedronfirstfaceoffset, sizeof(int));  // [200]
  this->g_polyhedronfaceindices = (int *)p_calloc(
      this->size_polyhedronfaceindices, 2 * sizeof(int));  // [4000]
  this->g_normals =
      (double *)p_calloc(this->size_facearrays, 3 * sizeof(double));  // [2400]
  this->g_facesdata =
      (int *)p_calloc(this->size_facearrays, 3 * sizeof(int));  // [2400]
  this->g_facestype = (int *)p_calloc(this->size_facearrays, sizeof(int));
  this->g_faceverticeslist =
      (int *)p_calloc(this->size_facevertexarrays, 6 * sizeof(int));  // [21000]
  this->g_edge_angles = (double *)p_calloc(this->size_facevertexarrays,
                                           sizeof(double));  // [3500]
  this->g_ev = (int *)p_calloc(this->size_ev, sizeof(int));

  if (!this->g_facesdata || !this->g_facestype || !this->g_hashtable ||
      !this->g_polyhedronfirstfaceoffset || !this->g_polyhedronfaceindices ||
      !this->g_faceverticeslist || !this->g_intworkarray ||
      !this->g_doubleworkarray || !this->g_edge_angles || !this->g_normals ||
      !this->g_ev) {
    IVP_IF(1) {
      IVP_IFDEBUG(IVP_DM_GEOMPACK_LEVEL1, {
        ivp_debugmanager.dprint(IVP_DM_GEOMPACK_LEVEL1,
                                "*** GEOMPACK: Out of memory!\n\n");
      })
    }
    goto out_of_memory;
  }

  // N_ORIGINAL_VERTICES - number of original vertex coordinates (i.e. total
  // number of polyhedron points without any duplicated points for different
  // polygons!) NFACE               - number of faces in decomposition NPOLH -
  // number of polyhedra in decomposition
  this->n_original_vertices = params->nvc;
  this->nface = params->nface;
  this->npolh =
      1;  // if changed to anything higher than 1 you will have to adjust the
          // variable 'this->size_polyhedronfirstfaceoffset' accordingly!
  ...
  // offsets into face list for each polyhedron (we will only process ONE
  // polyhedron!)
  ...
  while (((2 + this->n_polyhedronfaces) * 1) >
         this->size_polyhedronfaceindices) {  // 2000
    int res =
        increase_memory((void **)&this->g_polyhedronfaceindices,
                        &this->size_polyhedronfaceindices, 2 * sizeof(int));
    if (res == 0) {
      this->ierr = 500;
      goto GEOMPACK_abort;
    }
  ...
  // init 'this->g_polyhedronfaceindices' (?)... we will simply set these values
  // to the number of the corresponding face.
  for (i = 1; i <= this->n_polyhedronfaces; i++) {
    this->g_polyhedronfaceindices[(i << 1) - 2] = i;
  }

  this->hashtable_size =
      min(prime_(this->n_original_vertices + 2), this->hashtable_maxsize);
  ...
  if (this->ierr != 0) {
    IVP_IF(1) {
      IVP_IFDEBUG(IVP_DM_GEOMPACK_LEVEL1, {
        ...
        ivp_debugmanager.dprint(IVP_DM_GEOMPACK_LEVEL1,
                                "Premature abort due to above error.\n\n");
      })
    }
    goto GEOMPACK_abort;
  }

  IVP_IF(1){IVP_IFDEBUG(
      IVP_DM_GEOMPACK_LEVEL1,
      {
        ...
        ivp_debugmanager.dprint(IVP_DM_GEOMPACK_LEVEL1,
                                "Statistics after initializing polyhedral "
        ...
        mem_fl = this->size_facearrays * sizeof(int) +
                 this->size_facearrays * 3 * sizeof(int) +
                 this->size_facearrays * 3 * sizeof(double);
        mem_fvl = this->size_facevertexarrays * 6 * sizeof(int) +
                  this->size_facevertexarrays * sizeof(double);
        mem_total = mem_iwa + mem_dwa + mem_vcl + mem_pffl + mem_pfil + mem_fl +
                    mem_fvl;
        IVP_IFDEBUG(IVP_DM_GEOMPACK_LEVEL2, {
          ivp_debugmanager.dprint(
              IVP_DM_GEOMPACK_LEVEL2,
              "Memory allocated for int WORK ARRAY: %d bytes\n", mem_iwa);
          ivp_debugmanager.dprint(
              IVP_DM_GEOMPACK_LEVEL2,
              "Memory allocated for DOUBLE WORK ARRAY: %d bytes\n", mem_dwa);
          ivp_debugmanager.dprint(
              IVP_DM_GEOMPACK_LEVEL2,
              "Memory allocated for VERTEX COORDINATE LIST: %d bytes\n",
              mem_vcl);
          ivp_debugmanager.dprint(
              IVP_DM_GEOMPACK_LEVEL2,
              "Memory allocated for POLYHEDRON FIRST FACE LIST: %d bytes\n",
              mem_pffl);
          ivp_debugmanager.dprint(
              IVP_DM_GEOMPACK_LEVEL2,
              "Memory allocated for POLYHEDRON FACE INDEX LIST: %d bytes\n",
              mem_pfil);
          ivp_debugmanager.dprint(IVP_DM_GEOMPACK_LEVEL2,
                                  "Memory allocated for FACE LISTS: %d bytes\n",
                                  mem_fl);
          ivp_debugmanager.dprint(
              IVP_DM_GEOMPACK_LEVEL2,
              "Memory allocated for FACE VERTICES LISTS: %d bytes\n", mem_fvl);
        })
        else {
          ivp_debugmanager.dprint(IVP_DM_GEOMPACK_LEVEL1,
...
Retry_convex_decomposition:
  ...
  if ((this->ierr != 0) &&
      (this->ierr !=
       327)) {  // abort on error but skip "reflex edge" resolving problems
    IVP_IF(1) {
      IVP_IFDEBUG(IVP_DM_GEOMPACK_LEVEL1, {
        if (this->ierr == 500) {
          ivp_debugmanager.dprint(IVP_DM_GEOMPACK_LEVEL1,
  ...
  IVP_IF(1) {
    IVP_IFDEBUG(IVP_DM_GEOMPACK_LEVEL1, {
      ...
      ivp_debugmanager.dprint(IVP_DM_GEOMPACK_LEVEL1,
                              "Intermediate decomposition statistics:\n");
      mem_iwa = this->size_intworkarray * sizeof(int);
      mem_dwa = this->size_doubleworkarray * sizeof(double);
      mem_vcl = this->size_vcl * 3 * sizeof(double);
      mem_pffl = this->size_polyhedronfirstfaceoffset * sizeof(int);
      ...
      mem_fvl = this->size_facevertexarrays * 6 * sizeof(int) +
                this->size_facevertexarrays * sizeof(double);
      mem_total =
          mem_iwa + mem_dwa + mem_vcl + mem_pffl + mem_pfil + mem_fl + mem_fvl;
      IVP_IFDEBUG(IVP_DM_GEOMPACK_LEVEL2, {
        ivp_debugmanager.dprint(
            IVP_DM_GEOMPACK_LEVEL2,
            "Memory allocated for int WORK ARRAY: %d bytes\n", mem_iwa);
  ...
  if (this->ierr == 327) {            // unresolved reflex edge
      ...
      if (this->angacc > 0.0) {
        ...
        IVP_IF(1) {
          IVP_IFDEBUG(IVP_DM_GEOMPACK_LEVEL1, {
            ivp_debugmanager.dprint(
                IVP_DM_GEOMPACK_LEVEL1,
                "Retrying with <angacc=%f> and <rdacc=%f>\n\n", this->angacc,
                this->rdacc);
          })
        }
    ...
    IVP_IF(1) {
      IVP_IFDEBUG(IVP_DM_GEOMPACK_LEVEL1, {
        ivp_debugmanager.dprint(IVP_DM_GEOMPACK_LEVEL1,
                                "Premature abort due to difficulties in "
                                "resolving some reflex edge(s).\n\n");
      })
    }
    goto GEOMPACK_abort;
  }
  ...
  IVP_IF(1) {
    IVP_IFDEBUG(IVP_DM_GEOMPACK_LEVEL1, {
      ivp_debugmanager.dprint(IVP_DM_GEOMPACK_LEVEL1,
                              "+++ Convex decomposition successful!\n\n");
    })
    ...
    IVP_IFDEBUG(IVP_DM_GEOMPACK_LEVEL2, {
      ...
      for (i = 1; i <= this->n_original_vertices; i++) {
        ...
        ivp_message("#%d : \t", i);
        for (j = 1; j <= 3; j++) {
          ivp_debugmanager.dprint(IVP_DM_GEOMPACK_LEVEL2, "%f   ",
                                  this->g_vcl[j + (i * 3) - 4]);
        }
        ivp_debugmanager.dprint(IVP_DM_GEOMPACK_LEVEL2, "\n");
      }
      ...
      for (i = 1; i <= this->nface; i++) {
        ...
        for (j = 1; j <= 3; j++) {
          ivp_debugmanager.dprint(IVP_DM_GEOMPACK_LEVEL2, "%d ",
                                  this->g_facesdata[j + (i * 3) - 4]);
        }
        ivp_debugmanager.dprint(IVP_DM_GEOMPACK_LEVEL2,
                                "}  facestype[%d]=%d\tnormals={", i - 1,
                                this->g_facestype[i - 1]);
        for (j = 1; j <= 3; j++) {
          ivp_debugmanager.dprint(IVP_DM_GEOMPACK_LEVEL2, "%f  ",
                                  this->g_normals[j + (i * 3) - 4]);
        }
        ivp_debugmanager.dprint(IVP_DM_GEOMPACK_LEVEL2, "\n");
      }
      ivp_debugmanager.dprint(IVP_DM_GEOMPACK_LEVEL2, "\n\n");
      ...
      for (i = 1; i <= this->n_work_vertices; i++) {
        ...
        for (j = 1; j <= 6; j++) {
          ...
          ivp_debugmanager.dprint(IVP_DM_GEOMPACK_LEVEL2, "%d\t",
                                  this->g_faceverticeslist[j + (i * 6) - 7]);
        }
...
GEOMPACK_abort:
  ...
  IVP_IF(1){IVP_IFDEBUG(
      ...
      {
        ivp_debugmanager.dprint(IVP_DM_GEOMPACK_LEVEL1,
                                "Final GEOMPACK statistics:\n");
        ivp_debugmanager.dprint(
            IVP_DM_GEOMPACK_LEVEL1,
            "N_ORIGINAL_VERTICES   \t= %d (number of vertex coordinates)\n",
            this->n_original_vertices);
        ivp_debugmanager.dprint(
            IVP_DM_GEOMPACK_LEVEL1,
            "NFACE \t= %d (number of faces in polyhedral decomposition)\n",
            this->nface);
        ivp_debugmanager.dprint(
            IVP_DM_GEOMPACK_LEVEL1,
            "N_WORK_VERTICES \t= %d (number of positions used in FVL array)\n",
            this->n_work_vertices);
        ivp_debugmanager.dprint(
            IVP_DM_GEOMPACK_LEVEL1,
            "NPOLH \t= %d (number of polyhedra in decomposition)\n",
            this->npolh);
        ivp_debugmanager.dprint(IVP_DM_GEOMPACK_LEVEL1, "\n\n");
      })}
