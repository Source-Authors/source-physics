...
void IVP_Geompack::resedg_(int u_in, long int *rflag) {
  ...
  /*        ADDR_OF_N_ORIGINAL_VERTICES - number of vertex coordinates or
   ...
   * POLYHEDRONFACEINDICES array */
  /*        ADDR_OF_SIZE_intWORKARRAY - maximum size available for intWORKARRAY
   * array; should be about */
  /*              5/3*NEDG where NEDG is number of edges in polyhedron */
  /*              containing reflex edge */
  /*        ADDR_OF_SIZE_DOUBLEWORKARRAY - maximum size available for
   * DOUBLEWORKARRAY array; should be >= NEDG */
  ...
  /*        VCL,FACESDATA,FACESTYPE,NORMALS,FACEVERTICESLIST,EDGE_ANGLES,POLYHEDRONFIRSTFACEOFFSET,POLYHEDRONFACEINDICES
   * - updated by cut face */
  ...
  /*        DOUBLEWORKARRAY(1:ADDR_OF_SIZE_DOUBLEWORKARRAY) - double precision
   * work array */
  ...
  *rflag = 0;
  ...
  if ((i__1 = facesdata[fl * 3 + 2], abs(i__1)) ==
      (i__2 = facesdata[fl * 3 + 3], abs(i__2))) {
    return;
  }
  if ((i__1 = facesdata[fr * 3 + 2], abs(i__1)) ==
      (i__2 = facesdata[fr * 3 + 3], abs(i__2))) {
    return;
  }
  if (lu < lv) {
      ...
      IVP_IFDEBUG(IVP_DM_GEOMPACK_LEVEL3,
                  {
                    ivp_debugmanager.dprint(
                        ...
                        "RESEDG: U,V,LU,LV,FL,FR,P = %d %d %d %d %d %d %d\n",
  ...
  if (la < lb) {
    ...
    recheck_size_1:
      if ((2 + n_edges) * 3 > (this->size_intworkarray)) {
        int res = increase_memory((void **)&this->g_intworkarray,
                                  &this->size_intworkarray, sizeof(int));
        if (res == 0) {
          this->ierr = 500;
          return;
        }
...
recheck_size_2:
  if (((2 + n_edges) * 5) > (this->size_intworkarray)) {
    ...
    int res = increase_memory((void **)&this->g_doubleworkarray,
                              &this->size_doubleworkarray, sizeof(double));
    if (res == 0) {
      this->ierr = 500;
      return;
    }
  ...
  L30:
    ...
    if ((faceverticeslist[c__ * 6 + 1] - faceverticeslist[b * 6 + 1]) * sp >
        0) {
    ...
    if (first) {
      ...
      leng = sqrt(d__1 * d__1 + d__2 * d__2 + d__3 * d__3);
      /* Computing MAX */
      d__1 = abs(ep[0]), d__2 = abs(ep[1]), d__1 = max(d__1, d__2),
      d__2 = abs(ep[2]);
...
L80:
  IVP_IF(1){IVP_IFDEBUG(
      IVP_DM_GEOMPACK_LEVEL3,
      {
        ivp_debugmanager.dprint(IVP_DM_GEOMPACK_LEVEL3,
                                "Candidate angles (dihedral angle = %f ",
                                edge_angles[u_in] * 180.0 / IVP_PI);
        for (i__ = 1; i__ <= nang; ++i__) {
          ivp_debugmanager.dprint(IVP_DM_GEOMPACK_LEVEL3, "%f ",
                                  ang[i__ - 1] * 180.0 / IVP_PI);
        }
      })}
  ...
  L90:
    ...
    da = nrmlc[0] * vcl[la * 3 + 1] + nrmlc[1] * vcl[la * 3 + 2] +
         nrmlc[2] * vcl[la * 3 + 3] - nrmlc[3];
    if (abs(da) <= dtol) {
      ca = 0;
    ...
    lb = faceverticeslist[b * 6 + 1];
    db = nrmlc[0] * vcl[lb * 3 + 1] + nrmlc[1] * vcl[lb * 3 + 2] +
         nrmlc[2] * vcl[lb * 3 + 3] - nrmlc[3];
    if (abs(db) <= dtol) {
      cb = 0;
    ...
    IVP_IF(1){IVP_IFDEBUG(IVP_DM_GEOMPACK_LEVEL3,
                          {
                            ivp_debugmanager.dprint(
                                IVP_DM_GEOMPACK_LEVEL3,
                                "Trying angle %d   NEDGC = %d\n", i__, nedgc);
                          })}

    // *******************************************************************
    cutfac_(&p, nrmlc, &dtol, &nedgc, intworkarray, &nce,
            &intworkarray[((n_edges + 1) * 3)], &doubleworkarray[1], rflag);
    ...
    if (*rflag) {
      // ***************************************************************
      insfac_(&p, nrmlc, &nce, &intworkarray[(n_edges + 1) * 3],
              &doubleworkarray[1]);
