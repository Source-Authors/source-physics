 ...
 *representation to the physics engine Attention:	All IVP_*_Template
 *structures are used to transport information and therefore are only needed for
 *a limited period of time: after usage, the user should free the memory of the
 *structures. Attention:	All surfaces have to share the same lines! All
 *lines have to share the same points! This means that no two point instances
 *may exist with the same coordinates. Attention:	Filling the structures
 *incorrectly may result in program hangup or crash (in the current version
 *there is no plausible check). Attention:     	The physical simulation is
 *optimized to handle normal sized ojects (0.3f - 50 meter diameter) Objects
 *which are much larger may result in unnececarry CPU usage. Smaller objects
 *cause simulation inaccuracies to become visible. Therefore, you should rescale
 *your world according to whether you want to simulate colliding planets or
 *jumping insects (this may change in the future versions). Attention:	All
 *values are: Meters, Kilo, Newtons
 ********************************************************************************/
...
class IVP_Template_Line {
 public:
  ushort
      p[2];  // end points of a line stored as a point index (see point array)
  void set(ushort a, ushort b) {
    p[0] = a;
    p[1] = b;
  }
};
...
class IVP_Template_Polygon {
 public:
  ...
  IVP_Template_Polygon(
      int point_count, int line_count,
...
class IVP_Template_Ledge_Polygon_Soup {
 public:
  IVP_BOOL
  ledge_is_open;  // IVP_FALSE means: ledge is already closed (and convex!)
 ...
 *			The material referenced by the surface is assumed to be
 *long term. Each surface uses a line index array which stores the line indices
 *			(see IVP_Template_Object::lines) and a revert_line array
 *which stores flags about the direction of the lines. The order of the lines is
 *not important.
 *
 *	Attention:	When walking from the start_point of a line to the
 *end_point, the surface has to be on the left side (in a right hand coordinate
 *system). Technically: for surface line number i (0<i<n_lines): walking from
 *point nr templ_poly->lines[lines[i]].p[revert_line[i]] to
...
class IVP_Template_Surface {
 public:
  ...
  ushort *lines;  // all lines of this surfaces: Stored in an array of indices
                  // which refer to elements in the templ_poly->lines array
