...
class IVP_Compact_Modify {
 public:
   ...
   *			<chop_vector>  halfspace plane defining the orientation
   *of the cut <chop_depth>   the 'thickness' of the slice to be chopped off
   *(i.e. the distance between the slicing plane and the furthest chopped-off
   *point orthogonal to this plane) Output:		Pointer to the new
   *compact surface. Note:		The halfspace plane's normal vector has
   *to point AT the object. Note:		For now this method only
   *supports chopping of 'one-ledged' compact surfaces (i.e. convex objects)!
   *	Warning:	Function may fail due to numerical errors (unlikely but
   *			possible)
   *	Example:	If you have a pole (pointing in x-direction) and you
   *want to slice off 1 cm, you should call: "chop(pole_cs,
   *IVP_U_Float_Point(1.0 ,0.0, 0.0), 0.01);"
   *****************************************************************************/
