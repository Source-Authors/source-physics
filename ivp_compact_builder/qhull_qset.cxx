...
void qh_setaddnth(setT **setp, int nth, void *newelem) {
  ...
  if (nth < 0 || nth > oldsize) {
    ivp_message(
        "qhull internal error (qh_setaddnth): nth %d is out-of-bounds for "
        "set:\n",
        nth);
...
void qh_setappend_set(setT **setp, setT *setA) {
  ...
  *sizep = size + sizeA + 1; /* memcpy may overwrite */
  if (sizeA > 0)
    memcpy((char *)&((*setp)->e[size].p), (char *)&(setA->e[0].p),
           SETelemsize * (sizeA + 1));
} /* setappend_set */
...
void qh_setcheck(setT *set, const char *tname, int id) {
  ...
  if (size > maxsize || !maxsize) {
    ivp_message(
        "qhull internal error (qh_setcheck): actual size %d of %s%d is greater "
        "than max size %d\n",
        size, tname, id, maxsize);
    waserr = 1;
  } else if (set->e[size].p) {
    ivp_message(
        "qhull internal error (qh_setcheck): %s%d (size %d max %d) is not null "
        "terminated.\n",
...
setT *qh_setcopy(setT *set, int extra) {
  ...
  *SETsizeaddr_(newset) = size + 1; /* memcpy may overwrite */
  memcpy((char *)&(newset->e[0].p), (char *)&(set->e[0].p),
         SETelemsize * (size + 1));
  return (newset);
} /* setcopy */
...
void *qh_setdel(setT *set, void *oldelem) {
  ...
  while (*elemp != oldelem && *elemp) elemp++;
  if (*elemp) {
...
void *qh_setdelnth(setT *set, int nth) {
  ...
  // dimhotepus: Check nth in range before access.
...
void *qh_setdelnthsorted(setT *set, int nth) {
  ...
  elem = *newp;
  oldp = newp + 1;
  while ((*(newp++) = *(oldp++))); /* copy remaining elements and NULL */
  if (!(*sizep)--)                 /*  if was a full set */
...
void *qh_setdelsorted(setT *set, void *oldelem) {
  ...
  while (*newp != oldelem && *newp) newp++;
  if (*newp) {
    oldp = newp + 1;
    while ((*(newp++) = *(oldp++))); /* copy remaining elements */
    sizep = SETsizeaddr_(set);
    if (!(*sizep)--)         /*  if was a full set */
      *sizep = set->maxsize; /*     *sizep= (max size-1)+ 1 */
    return oldelem;
...
setT *qh_setduplicate(setT *set, int elemsize) {
  ...
  if (!(size = qh_setsize(set))) return NULL;
...
int qh_setequal(setT *setA, setT *setB) {
  ...
  if (!memcmp((char *)elemAp, (char *)elemBp, sizeA * SETelemsize)) return 1;
  return 0;
} /* setequal */
...
/*-<a                             href="qh-c.htm#set"
  ...
  design:
    ...
    check results
*/
int qh_setequal_except(setT *setA, void *skipelemA, setT *setB,
                       void *skipelemB) {
  void **elemA, **elemB;
  int skip = 0;
...
int qh_setequal_skip(setT *setA, int skipA, setT *setB, int skipB) {
  ...
  while (1) {
    if (elemA == skipAp) elemA++;
    if (elemB == skipBp) elemB++;
    if (!*elemA) break;
    if (*elemA++ != *elemB++) return 0;
  }
  if (*elemB) return 0;
...
void qh_setfree2(setT **setp, int elemsize) {
  ...
  FOREACHelem_(*setp) qh_memfree(elem, elemsize);
  qh_setfree(setp);
} /* setfree2 */
...
int qh_setindex(setT *set, void *atelem) {
  ...
  SETreturnsize_(set, size);
  if (size > set->maxsize) return -1;
  elem = SETaddr_(set, void);
  for (i = 0; i < size; i++) {
    if (*elem++ == atelem) return i;
  }
  return -1;
} /* setindex */
...
setT *qh_setnew(int setsize) {
  ...
  if ((unsigned)size <= (unsigned)qhmem.LASTsize) {
    ...
    if (sizereceived > size) setsize += (sizereceived - size) / SETelemsize;
#endif
  } else
...
void qh_setprint(FILE *fp, const char *string, setT *set) {
  ...
  if (!set)
    ...
    SETreturnsize_(set, size);
    fprintf(fp, "%s set=%p maxsize=%d size=%d elems=", string, (void *)set,
            set->maxsize, size);
    if (size > set->maxsize) size = set->maxsize + 1;
    for (k = 0; k < size; k++) fprintf(fp, " %p", set->e[k].p);
    fprintf(fp, "\n");
  }
} /* setprint */
...
int qh_setsize(setT *set) {
  ...
  if ((size = *sizep)) {
    ...
    if (size > set->maxsize) {
      ivp_message(
          "qhull internal error (qh_setsize): current set size %d is greater "
          "than maximum size %d\n",
          size, set->maxsize);
...
void qh_settempfree(setT **set) {
  ...
  if (stackedset != *set) {
    ...
    ivp_message(
        "qhull internal error (qh_settempfree): set %p (size %d) was not last "
        "temporary allocated (depth %d, set %p, size %d)\n",
...
void qh_settempfree_all(void) {
  ...
  FOREACHset_((setT *)qhmem.tempstack) qh_setfree(&set);
...
setT *qh_settemppop(void) {
  ...
  if (!stackedset) {
    ivp_message(
        "qhull internal error (qh_settemppop): pop from empty temporary "
        "stack\n");
    qh_errexit(qhmem_ERRqhull, NULL, NULL);
  }
  if (qhmem.IStracing >= 5)
...
void qh_settruncate(setT *set, int size) {
  if (size < 0 || size > set->maxsize) {
    ivp_message(
        "qhull internal error (qh_settruncate): size %d out of bounds for "
        "set:\n",
        size);
    qh_setprint(qhmem.ferr, "", set);
    qh_errexit(qhmem_ERRqhull, NULL, NULL);
...
void qh_setzero(setT *set, int index, int size) {
  ...
  if (index < 0 || index >= size || size > set->maxsize) {
    ivp_message(
        "qhull internal error (qh_setzero): index %d or size %d out of bounds "
        "for set:\n",
        index, size);
    qh_setprint(qhmem.ferr, "", set);
    qh_errexit(qhmem_ERRqhull, NULL, NULL);
