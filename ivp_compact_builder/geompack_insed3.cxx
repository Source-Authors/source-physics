...
void IVP_Geompack::insed3_(int *a, int *b, int *hfl) {
  ...
  /*     Input parameters: */
  /*        A,B - indices of FACEVERTICESLIST for nonadjacent vertices on same
   * face */
  /*        NFACE - number of faces or positions used in FACESDATA array */
  /*        NVERT - number of positions used in FACEVERTICESLIST, EDGE_ANGLES
   * arrays */
  ...
  /*        HFL(1:*) - head pointer to face indices in POLYHEDRONFACEINDICES for
   * each polyh */
  /*        POLYHEDRONFACEINDICES(1:2,1:ADDR_OF_N_POLYHEDRONFACES) - list of
   * signed face indices for each polyh */

  /*     Updated parameters: */
  /*        A - index in FACEVERTICESLIST of new edge; LOC field of output A
   * same as */
  /*              input A */
  /*        NFACE - increased by 1 */
  /*        NVERT - increased by 2 */
  /*        ADDR_OF_N_POLYHEDRONFACES - increased by 1 or 2 */
  /*        FACESDATA,FACTYP,NORMALS,FACEVERTICESLIST,EDGE_ANGLES,POLYHEDRONFACEINDICES
   * - updated by new edge */
...
recheck_size_3:
  ...
  polyhedronfaceindices[(this->n_polyhedronfaces << 1) + 1] =
      i_sign(this->nface, sp);
  polyhedronfaceindices[(this->n_polyhedronfaces << 1) + 2] =
      polyhedronfaceindices[(g << 1) + 2];
  ...
  recheck_size_4:
    ...
    polyhedronfaceindices[(this->n_polyhedronfaces << 1) + 1] =
        i_sign(this->nface, sq);
    polyhedronfaceindices[(this->n_polyhedronfaces << 1) + 2] =
        polyhedronfaceindices[(g << 1) + 2];
    polyhedronfaceindices[(g << 1) + 2] = this->n_polyhedronfaces;
  } else {
    if ((faceverticeslist[*b * 6 + 1] - faceverticeslist[*a * 6 + 1]) * sp >
        0) {
