...
IVP_Tri_Edge *IVP_Tri_Edge::search_nearest_edge_to(IVP_U_Point *reference,
                                                   IVP_DOUBLE *quad_dist) {
...
void IVP_Tetra_Point::print(const char *text) {
  if (!text) text = "";
  ivp_message("%s	Tetra-Point: Mask %X  Coverbits %X %X %X  pntnum=%i\n",
              text, this->tmp_side_of_triangle_bits, cover_area_bits[0],
              cover_area_bits[1], cover_area_bits[2], opoint->point_num());
}
...
void IVP_Tetra_Edge::print(const char *text) {
  if (!text) text = "";
  ivp_message("%s	Tetra_Edge: Bits %X %X %X\n", text, cover_area_bits[0],
              cover_area_bits[1], cover_area_bits[2]);
...
void IVP_Intrusion_Included_Points::print(const char *text) {
  if (!text) text = "";
  ivp_message("%s	Intrusion_Point", text);
  tetra_point->print("	");
}
...
void IVP_Intrusion_Intersection::print(const char *text) {
  ...
  ivp_message("%s %s Intersection pnt %i - %i", text,
              ...
              line_endpoints[0]->point_num(), line_endpoints[1]->point_num());
...
void IVP_Intrusion_Status::print(const char *text) {
  if (!text) text = "";
  ivp_message("%s	Intrusion Status\n", text);
...
IVP_Tetra_Intrude::IVP_Tetra_Intrude(IVP_Tetra_Point *i_tetra_points,
                                     int orig_pnt_anz) {
  twop_2_tetra_edge_hash = new IVP_Hash(
      orig_pnt_anz * 2, sizeof(IVP_Tetra_Edge *) * 2, P_INVALID_TETRA_EDGE);
  tetra_points = i_tetra_points;
  n_tetra_points = orig_pnt_anz;
  memsize_of_tetra_edges = n_tetra_points * 10;
  n_tetra_edges = 0;
  tetra_edges = (IVP_Tetra_Edge *)p_calloc(memsize_of_tetra_edges,
                                           sizeof(IVP_Tetra_Edge));
...
int IVP_Tetra_Intrude::pe(
    const int pnt_num) {  // print all edges including pnt_num
  int i;
...
inline IVP_DOUBLE p_quad_distance_between_two_straights(IVP_U_Point *linea_0,
                                                        IVP_U_Point *linea_1,
                                                        IVP_U_Point *lineb_0,
                                                        IVP_U_Point *lineb_1) {
  IVP_U_Hesse hesse;
  IVP_U_Point linea_2;
  linea_2.subtract(lineb_1, lineb_0);
...
void IVP_Tetra_Intrude::init_tetra_edge(IVP_Tetra_Edge *edge,
                                        IVP_Tetra_Point *p0,
...
void IVP_Tetra_Intrude::point_2_bits(IVP_U_Point *lpos, IVP_U_Point *rpos,
                                     int *result_bitmasp) {
  int k;
...
void IVP_Tetra_Intrude::checkin_edge(IVP_Tri_Edge *edge) {
  ...
  if (te == P_INVALID_TETRA_EDGE) {
    if (n_tetra_edges == memsize_of_tetra_edges) {
      ...
      // ivp_message("*********** Increasing tetra_edge memory *****\n");
      IVP_Tetra_Edge *newmem = (IVP_Tetra_Edge *)p_calloc(
          memsize_of_tetra_edges, sizeof(IVP_Tetra_Edge));
      memcpy((char *)newmem, (char *)tetra_edges,
             sizeof(IVP_Tetra_Edge) * n_tetra_edges);
      delete tetra_edges;
      tetra_edges = newmem;
    }
    t_edge = &tetra_edges[n_tetra_edges++];
    twop_2_tetra_edge_hash->add(
        (char *)&buffer[0], (void *)(((char *)t_edge) - ((char *)tetra_edges)));

    this->init_tetra_edge(t_edge, edge->tmp.gen.tetra_point,
                          edge->next->tmp.gen.tetra_point, edge);
  } else {
...
IVP_INTRUSION_CHECK_RESULTS P_THREE_CHECK_INTRUDE(
    int me, int other0, int other1, int other2, IVP_Tetra_Point *t0,
    IVP_Tetra_Point *t1, IVP_Tri_Edge *tri_edges[4], IVP_U_Hesse hesse_of_t[4],
    IVP_U_Hesse edge_hesses[6], IVP_Tri_Edge *edge_edges[6],
    IVP_DOUBLE pop_eps_of_t[4], IVP_U_Point &intrude_position,
  ...
  while (1) {
    if (((bitsand & 0xf & ~(bitsand >> 8)) & (1 << me)) != 0)
      return IVP_INTRUSION_CHECK_NONE;  // both both inside and not on the
                                        // surface

    if ((bor & (0x1111 << me)) ==
        (0x11 << me)) { /* real intrusion, no epsilon, no identic to point ->
                  one inside, one outside*/
      IVP_DOUBLE d0 = hesse_of_t[me].get_dist(t0->opoint);
      IVP_DOUBLE d1 = hesse_of_t[me].get_dist(t1->opoint);
      IVP_U_Point vec;
      vec.subtract(t1->opoint, t0->opoint);
      intrude_position.add_multiple(t0->opoint, &vec, -d0 / (d1 - d0));
      break;
    }

    if ((t0->tmp_side_of_triangle_bits & (1 << (me + 12))) !=
        0) {  // identic to point
      /* continue checking only if t1 in same plane */
      if ((t1->tmp_side_of_triangle_bits & (1 << (me + 8))) == 0)
    ...
    if ((t1->tmp_side_of_triangle_bits & (1 << (me + 12))) !=
        0) {  // identic to point
      /* continue checking only if in same plane */
      if ((t0->tmp_side_of_triangle_bits & (1 << (me + 8))) == 0)
        return IVP_INTRUSION_CHECK_NONE;
      intrude_position.set(t0->opoint);
      type = IVP_INTRUSION_CHECK_OVERLAP;
      break;
    }
    if ((t0->tmp_side_of_triangle_bits & (1 << (8 + me))) !=
        0) {  // mid of a surface
      intrude_position.set(t0->opoint);
      break;
    }
    if ((t1->tmp_side_of_triangle_bits & (1 << (8 + me))) !=
        0) {  // mid of a surface
  ...
  if (dist0 < -pop_eps_of_t[other0])
    return IVP_INTRUSION_CHECK_NONE;  // outside P_Pop_Eps
...

  {
    ...
    for (int i = 5; i >= 0; i--) {
      ...
      if (dist < meps) {  // now collision point is outside eps of edges
        IVP_DOUBLE d0 = edge_hesses[i].get_dist(
            t0->opoint);  // check whether points are outside two
        IVP_DOUBLE d1 = edge_hesses[i].get_dist(t1->opoint);
        if (d0 < meps || d1 < meps) {
          return IVP_INTRUSION_CHECK_NONE;  // both points outside-> no problem
        }
        IVP_Tri_Edge *e =
            edge_edges[i];  // get the distances between the two straights
        IVP_DOUBLE eps2 = meps * meps;
        IVP_DOUBLE qd2 = p_quad_distance_between_two_straights(
            t0->opoint, t1->opoint, e->start_point, e->next->start_point);
        ivp_message("qd2 = %f\n", sqrt(qd2));
        if (qd2 > eps2) {
          return IVP_INTRUSION_CHECK_NONE;  // outside edge hesses
        }
        ivp_message(
            "Warning: two straights are closer than eps, but no collision by "
            "normal test\n");
        t0->print("to test");
        t1->print("to test");
        e->print("triangle\n");
  ...
  if (dist0 < -P_RES_EPS)
    return IVP_INTRUSION_CHECK_EPSILON;  // outside DOUBLE Eps
  ...
  {
    auto *is = new IVP_Intrusion_Intersection();
    P_MEM_CLEAR(is);
...
inline IVP_INTRUSION_CHECK_RESULTS IVP_Tetra_Intrude::check_point_intrusion(
    int bitmask[3], IVP_Tetra_Point *t_points[4], IVP_U_Hesse hesse_of_t[4],
    IVP_DOUBLE pop_eps_of_t[4], IVP_U_Hesse edge_hesses[6], IVP_Tetra_Point *tp,
    IVP_Intrusion_Status *status) {
  ...
  for (tpnr = 0x8; tpnr > 0; pop_eps--, h--, tpnr >>= 1) {
    ...
    if (dist < P_RES_EPS) {                        // eps area (surface)
      ...
      if (tp == t_points[0] || tp == t_points[1] || tp == t_points[2] ||
          tp == t_points[3]) {
        tp->tmp_side_of_triangle_bits |=
            (1 << 16) | (tpnr << 12);  // identic to a point
        if (tp == t_points[2] || tp == t_points[3]) {
          tp->tmp_side_of_triangle_bits |=
              (1 << 17);  // identic to new_pop_edge
        }
        continue;
      } else {
  ...
  if ((tp->tmp_side_of_triangle_bits & (1 << 16)) != 0)
    return IVP_INTRUSION_CHECK_NONE;  // is a point

  if ((tp->tmp_side_of_triangle_bits & (0xf << 20)) !=
      0) {  // one side really outside
    return IVP_INTRUSION_CHECK_NONE;
  }
  {
    for (int i = 5; i >= 0; i--) {
  ...
  if ((tp->tmp_side_of_triangle_bits & 0xf) !=
      0xf) {  // not within DOUBLE EPS -> on the surface
...
IVP_INTRUSION_CHECK_RESULTS IVP_Tetra_Intrude::check_intrusion(
    ...
    for (a = 0; a < 4; a++) {        //-V112
      for (b = a + 1; b < 4; b++) {  //-V112
        ...
        for (e = tri_edges[a];
             ...
             e = e->next) {
        }  // search point not on both triangles
        e = e->next;
        edge_edges[c] = e;
     ...
     ok -> check hesse to 4 planes ***** */
  {
    ...
    for (pnr = n_tetra_points - 1; pnr >= 0; pnr--, tp++) {
      IVP_INTRUSION_CHECK_RESULTS s = check_point_intrusion(
          bitmask, t_points, hesse_of_t, pop_eps_of_t, edge_hesses, tp, status);
    ...
    }
    {
      ...
      for (op = first_extra_point; op; op = op->next) {
        ...
        IVP_INTRUSION_CHECK_RESULTS s =
            check_point_intrusion(bitmask, t_points, hesse_of_t, pop_eps_of_t,
                                  edge_hesses, tp, status);
        if (s > result) result = s;
        if (result > IVP_INTRUSION_CHECK_EPSILON) return result;
      }
    }
     ...
     tmp.tetra_point bits, then real check ****/
  {
    ...
    for (tnr--; tnr >= 0; te++, tnr--) {
      ...
      int bitsand =
          t0->tmp_side_of_triangle_bits & t1->tmp_side_of_triangle_bits;
      ...
      {                                // check 3
        ...
        if (n_new_triangles >= 1) {
          IVP_INTRUSION_CHECK_RESULTS s = P_THREE_CHECK_INTRUDE(
              3, 2, 1, 0, t0, t1, tri_edges, hesse_of_t, edge_hesses,
              edge_edges, pop_eps_of_t, intrude_position, status);
          if (s > result) result = s;
          if (result > IVP_INTRUSION_CHECK_EPSILON) return result;
        }
        if (n_new_triangles >= 2) {
          IVP_INTRUSION_CHECK_RESULTS s = P_THREE_CHECK_INTRUDE(
              2, 3, 1, 0, t0, t1, tri_edges, hesse_of_t, edge_hesses,
              edge_edges, pop_eps_of_t, intrude_position, status);
          if (s > result) result = s;
          if (result > IVP_INTRUSION_CHECK_EPSILON) return result;
        }
        if (n_new_triangles >= 3) {
          IVP_INTRUSION_CHECK_RESULTS s = P_THREE_CHECK_INTRUDE(
              1, 3, 2, 0, t0, t1, tri_edges, hesse_of_t, edge_hesses,
              edge_edges, pop_eps_of_t, intrude_position, status);
          if (s > result) result = s;
          if (result > IVP_INTRUSION_CHECK_EPSILON) return result;
        }
...
IVP_Intrusion_Status *IVP_Tetra_Intrude::calc_intrusion_status(
  ...
  this->check_intrusion(old_pop_edge_a, old_pop_edge_b, pop_edge_a, pop_edge_b,
