...
IVP_SurfaceBuilder_Polygon_Convex::IVP_SurfaceBuilder_Polygon_Convex(
    IVP_Template_Ledge_Polygon_Soup *templ_ledge)  // more of a hack :)
{
  this->tetras = NULL;
  this->c_ledge = NULL;
  this->poly_point_hash = new IVP_point_hash(8);
  ...
  for (int i = 0; i < 1 /*templ_ledge->n_templ_triangles*/; i++, templ_tri++) {
    ...
    IVP_Triangle *double_tri =
        IVP_Object_Polygon_Tetra::generate_double_triangle(
  ...
  uchar *mem;
  {
    ...
    // ivp_message("Compact ledge size: '%d'\n", size);
    mem =
        (uchar *)ivp_malloc_aligned(size, 16);  // 16 should be enough, but ...
    memset(mem, 0, size);
    ledge_gen.generate_compact_ledge(mem);
#ifdef DEBUG
    if (ledge_gen.validate() != IVP_OK) {
      ivp_message("Compact ledge generation fizzled :-(\n");
    }
#endif
  }
  {
...
void IVP_SurfaceBuilder_Polygon_Convex::init_surface_manager_polygon() {
  tetras->points_to_edge_hash =
      new IVP_Hash(500, 2 * sizeof(void *), (void *)0);
  ...
  uchar *mem;
  {
    ...
    this->fill_list_with_all_triangles(&tri_vec);
    {
      ...
      // ivp_message("Compact ledge size: '%d'\n", size);
      mem = (uchar *)ivp_malloc_aligned(
          size, 16);  // @@@ 16 should be enough, but ...
      ledge_gen.generate_compact_ledge(mem);
#ifdef DEBUG
      if (ledge_gen.validate() != IVP_OK) {
        ivp_message("Compact ledge generation fizzled :-(\n");
...
void IVP_SurfaceBuilder_Polygon_Convex::fill_list_with_all_triangles(
    IVP_U_Vector<IVP_Triangle> *tri_vec) {
  IVP_Triangle *tri;
  for (tri = this->tetras->triangles.first; tri; tri = tri->next) {
...
IVP_Compact_Ledge *IVP_SurfaceBuilder_Polygon_Convex::convert_template_to_ledge(
    IVP_Template_Polygon *templat) {
...
IVP_Compact_Ledge *
IVP_SurfaceBuilder_Polygon_Convex::convert_templateledgepolygonsoup_to_ledge(
    IVP_Template_Ledge_Polygon_Soup *templat) {
  if (!templat) return (NULL);

  IVP_SurfaceBuilder_Polygon_Convex surface_builder(templat);
  IVP_Compact_Ledge *compact_ledge =
      surface_builder.get_and_remove_compact_ledge();
