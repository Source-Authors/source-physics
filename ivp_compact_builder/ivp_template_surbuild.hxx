...
class IVP_Template_Surbuild_LedgeSoup {
 public:
   ...
   *  Default:      IVP_FALSE
   *	Description:  if true than an extra convex hull around the whole object
   *is generated
   *******************************************************************************/
   ...
   *  Default:      IVP_TRUE
   *	Description:  if true input data compact_ledges are freed using
   *ivp_free_aligned()
   *******************************************************************************/
   ...
   *  Note:	      Only input ledges which are copied ( and not linked to )
   *are taken into account Note:	      For reasons of speed Attention:
   *The current implementation limits the number of points to 65k If your
   *resulting ledge contains more points, the engine will crash Values:
   *IVP_SLMP_NO_MERGE	    result needs more memory but compile is fast
   *		    IVP_SLMP_MERGE_AND_REALLOCATE results needs less memory
   *		    IVP_SLMP_MERGE_NO_REALLOCATE result is placed in a chunk of
   *memory same size as IVP_SLMP_NO_MERGE, but actually needs only same size as
   *IVP_SLMP_MERGE_AND_REALLOCATE good for creating intermediate results
   *******************************************************************************/
