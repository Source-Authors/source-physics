...
void qh_addhash(void *newelem, setT *hashtable, int hashsize, unsigned hash) {
  ...
  for (scan = (int)hash; (elem = SETelem_(hashtable, scan));
    ...
    if (elem == newelem) break;
  }
  /* loop terminates because qh_HASHfactor >= 1.1 by qh_initbuffers */
  if (!elem) SETelem_(hashtable, scan) = newelem;
} /* addhash */
...
void qh_check_bestdist(void) {
  ...
  trace1((qh ferr,
          "qh_check_bestdist: check that all points are within %2.2g of best "
          "facet\n",
          maxoutside));
...
below %2.2g of the nearest %sfacet.\n",
  ...
  FOREACHfacet_i_(facets) { /* for each point with facet assignment */
    ...
    if (point == qh GOODpointp) continue;
    bestfacet = qh_findbest(point, facet, qh_ALL, False, !qh_NOupper, &dist,
                            &isoutside, &numpart);
    ...
    if (dist > maxoutside) {
      if (qh ONLYgood && !bestfacet->good &&
          !((bestfacet =
                 qh_findgooddist(point, bestfacet, &dist, &facetlist)) &&
            dist > maxoutside))
      // notgood++;
      {
      } else {
        waserror = True;
        ivp_message(
            "qhull precision error: point p%d is outside facet f%d, distance= "
            "%6.8g maxoutside= %6.8g\n",
...
options 'Qci Tv' to verify all points.\n",
  ...
  if (maxdist > qh outside_err) {
    ivp_message(
        "qhull precision error (qh_check_bestdist): a coplanar point is %6.2g "
        "from convex hull.  The maximum value (qh.outside_err) is %6.2g\n",
    ...
    trace0(
        (qh ferr, "qh_check_bestdist: max distance outside %2.2g\n", maxdist));
    int x = 0;
    x = 2 * x;
  }
} /* check_bestdist */
...
void qh_check_maxout(void) {
  ...
  if (qh VERTEXneighbors &&
      (qh PRINTsummary || qh KEEPinside || qh KEEPcoplanar || qh TRACElevel ||
       qh PRINTstatistics || qh PRINTout[0] == qh_PRINTsummary ||
       qh PRINTout[0] == qh_PRINTnone)) {
    trace1((qh ferr,
      ...
      FOREACHneighbor_(vertex) {
        ...
        if (-dist > qh TRACEdist || dist > qh TRACEdist ||
          ...
          ivp_message("qh_check_maxout: p%d (v%d) is %.2g from f%d\n",
                      qh_pointid(vertex->point), vertex->id, dist,
                      neighbor->id);
      }
    }
    if (qh MERGING) {
  ...
  qh maxoutdone = True;
  trace1((qh ferr,
          "qh_check_maxout: maxoutside %2.2g, min_vertex %2.2g, outside of not "
          "good %d\n",
...
*/
void qh_check_point(pointT *point, facetT *facet, realT *maxoutside,
                    realT *maxdist, facetT **errfacet1, facetT **errfacet2) {
  realT dist;
...
all %sfacets.  Will make %2.0f distance computations.\n",
    ...
    FORALLfacets {
      if (!facet->good && qh ONLYgood) continue;
      if (facet->flipped) continue;
      if (testouter) {
#if qh_MAXoutside
      ...
      FORALLpoints {
        if (point != qh GOODpointp)
          qh_check_point(point, facet, &maxoutside, &maxdist, &errfacet1,
                         &errfacet2);
      }
    ...
    if (maxdist > qh outside_err) {
      ivp_message(
          "qhull precision error (qh_check_points): a coplanar point is %6.2g "
          "from convex hull.  The maximum value (qh.outside_err) is %6.2g\n",
          maxdist, qh outside_err);
      qh_errexit2(qh_ERRprec, errfacet1, errfacet2);
      ...
      trace0(
          (qh ferr, "qh_check_points: max distance outside %2.2g\n", maxdist));
    }
  }
} /* check_points */
      ...
      FOREACHneighbor_(facet) {
        ...
        if (dist > -qh DISTround) {
          ...
          if (dist > qh DISTround) {
            ...
            if (qh ferr) {
              ivp_message(
                  "qhull precision error: f%d is concave to f%d, since p%d "
                  "(v%d) is %6.4g above\n",
                  facet->id, neighbor->id, qh_pointid(vertex->point),
                  vertex->id, dist);
            }
            errfacet1 = facet;
            errfacet2 = neighbor;
            ...
            if (dist > 0) { /* qh_checkzero checks that dist < - qh DISTround */
              ...
              if (qh ferr) {
                ivp_message(
                    "qhull precision error: f%d is clearly not convex to f%d, "
                    "since p%d (v%d) is %6.4g above\n",
                    facet->id, neighbor->id, qh_pointid(vertex->point),
                    vertex->id, dist);
              }
              errfacet1 = facet;
            ...
            qh_precision("coplanar ridge");
            trace0((qh ferr,
                    "qhull precision error: f%d may be coplanar to f%d, since "
                    "p%d (v%d) is within %6.4g during p%d\n",
                    facet->id, neighbor->id, qh_pointid(vertex->point),
                    vertex->id, dist, qh furthest_id));
          }
        }
      }
    }
    if (!allsimplicial) {
      ...
      FOREACHneighbor_(facet) {
        ...
        if (dist > qh DISTround) {
          ...
          if (qh ferr) {
            ivp_message(
                "qhull precision error: f%d is concave to f%d.  Centrum of f%d "
                "is %6.4g above f%d\n",
          ...
          if (qh ferr) {
            ivp_message(
                "qhull precision error: f%d is coplanar or concave to f%d.  "
                "Centrum of f%d is %6.4g above f%d\n",
                facet->id, neighbor->id, facet->id, dist, neighbor->id);
          }
          errfacet1 = facet;
          errfacet2 = neighbor;
...
/*-<a                             href="qh-c.htm#poly"
  ...
  checks:
    ...
    unless newmerge, at least hull_dim neighbors and vertices (exactly if
  simplicial) if non-simplicial, at least as many ridges as neighbors neighbors
  are not duplicated ridges are not duplicated in 3-d, ridges=verticies
...
void qh_checkfacet(facetT *facet, boolT newmerge, boolT *waserrorp) {
  ...
  if (facet->visible) {
    ivp_message(
        "qhull internal error (qh_checkfacet): facet f%d is on the "
        "visible_list\n",
        facet->id);
  ...
  if (!facet->normal) {
    if (qh ferr) {
      ivp_message(
          "qhull internal error (qh_checkfacet): facet f%d does not have  a "
          "normal\n",
          facet->id);
    }
    waserror = True;
  ...
  FOREACHvertex_(facet->vertices) {
    ...
    if (vertex->id >= previousid) {
      ivp_message(
          "qhull internal error (qh_checkfacet): vertices of f%d are not in "
          "descending id order at v%d\n",
          facet->id, vertex->id);
      waserror = True;
      break;
    }
  ...
  if (facet->simplicial) {
    if (numvertices + numneighbors != 2 * qh hull_dim && !facet->degenerate &&
        !facet->redundant) {
      ivp_message(
          "qhull internal error (qh_checkfacet): for simplicial facet f%d, "
          "#vertices %d + #neighbors %d != 2*qh hull_dim\n",
    ...
    if (!newmerge &&
      ...
      ivp_message(
          "qhull internal error (qh_checkfacet): for facet f%d, #vertices %d "
          "or #neighbors %d < qh hull_dim\n",
          facet->id, numvertices, numneighbors);
      waserror = True;
    }
  ...
  FOREACHneighbor_(facet) {
    if (neighbor == qh_MERGEridge || neighbor == qh_DUPLICATEridge) {
      ivp_message(
          "qhull internal error (qh_checkfacet): facet f%d still has a MERGE "
          "or DUP neighbor\n",
          facet->id);
      qh_errexit(qh_ERRqhull, facet, NULL);
    }
  ...
  FOREACHneighbor_(facet) {
    if (!qh_setin(neighbor->neighbors, facet)) {
      ivp_message(
          "qhull internal error (qh_checkfacet): facet f%d has neighbor f%d, "
          "but f%d does not have neighbor f%d\n",
    ...
    if (!neighbor->seen) {
      ivp_message(
          "qhull internal error (qh_checkfacet): facet f%d has a duplicate "
          "neighbor f%d\n",
  ...
  FOREACHridge_(facet->ridges) {
    ...
    if (numRvertices != qh hull_dim - 1) {
      ivp_message(
          "qhull internal error (qh_checkfacet): ridge between f%d and f%d has "
          "%d vertices\n",
    ...
    if (!qh_setin(facet->neighbors, neighbor)) {
      ivp_message(
          "qhull internal error (qh_checkfacet): for facet f%d, neighbor f%d "
          "of ridge r%d not in facet\n",
  ...
  if (!facet->simplicial) {
    FOREACHneighbor_(facet) {
      if (!neighbor->seen) {
        ivp_message(
            "qhull internal error (qh_checkfacet): facet f%d does not have a "
            "ridge for neighbor f%d\n",
            facet->id, neighbor->id);
        errother = neighbor;
        waserror = True;
      }
      intersection =
          qh_vertexintersect_new(facet->vertices, neighbor->vertices);
      qh_settemppush(intersection);
      FOREACHvertex_(facet->vertices) {
        vertex->seen = False;
        vertex->seen2 = False;
      }
      FOREACHvertex_(intersection) vertex->seen = True;
      FOREACHridge_(facet->ridges) {
        if (neighbor != otherfacet_(ridge, facet)) continue;
        FOREACHvertex_(ridge->vertices) {
          if (!vertex->seen) {
            ivp_message(
                "qhull internal error (qh_checkfacet): vertex v%d in r%d not "
                "in f%d intersect f%d\n",
 ...
 not in a ridge.  This is ok under merging.  Last point was p%d\n",
              ...
              if (!qh FORCEoutput && !qh MERGING) {
                ...
                if (!qh MERGING) qh_errexit(qh_ERRqhull, NULL, NULL);
              }
            }
          }
        }
    ...
    FOREACHneighbor_(facet) {
      if (neighbor->simplicial) {
        ...
        if (!qh_setequal_skip(facet->vertices, skipA, neighbor->vertices,
                              skipB)) {
          ivp_message(
              "qhull internal error (qh_checkfacet): facet f%d skip %d and "
              "neighbor f%d skip %d do not match \n",
...
void qh_checkflipped_all(facetT *facetlist) {
  ...
  if (facetlist == qh facet_list) zzval_(Zflippedfacets) = 0;
  FORALLfacet_(facetlist) {
...
void qh_checkpolygon(facetT *facetlist) {
  ...
  trace1(
      (qh ferr, "qh_checkpolygon: check all facets from f%d\n", facetlist->id));
  if (facetlist != qh facet_list || qh ONLYgood) nextseen = True;
  FORALLfacet_(facetlist) {
    if (facet == qh visible_list) visibleseen = True;
    if (!facet->visible) {
      if (!nextseen) {
        if (facet == qh facet_next)
          ...
          ivp_message(
              "qhull internal error (qh_checkpolygon): f%d has outside set "
              "before qh facet_next\n",
              facet->id);
          qh_errexit(qh_ERRqhull, facet, NULL);
        }
      }
  ...
  if (qh visible_list && !visibleseen && facetlist == qh facet_list) {
    ivp_message(
        "qhull internal error (qh_checkpolygon): visible list f%d no longer on "
        "facet list\n",
        qh visible_list->id);
    qh_printlists();
    ...
    FOREACHvertex_(facet->vertices) {
      ...
      if (!vertex->seen) {
        ...
        if (qh_pointid(vertex->point) == -1) {
          ivp_message(
              "qhull internal error (qh_checkpolygon): unknown point %p for "
              "vertex v%d first_point %p\n",
  ...
  if (facetlist == qh facet_list) {
    if (numfacets != qh num_facets - qh num_visible) {
      ivp_message(
          "qhull internal error (qh_checkpolygon): actual number of facets is "
          "%d, cumulative facet count is %d\n",
    ...
    if (qh VERTEXneighbors) {
      ...
      FORALLfacet_(facetlist) totvertices += qh_setsize(facet->vertices);
      if (totvneighbors != totvertices) {
        ivp_message(
            "qhull internal error (qh_checkpolygon): vertex neighbors "
            "inconsistent.  Totvneighbors %d, totvertices %d\n",
    ...
    if (numvertices != qh num_vertices - qh_setsize(qh del_vertices)) {
      ivp_message(
          "qhull internal error (qh_checkpolygon): actual number of vertices "
          "is %d, cumulative vertex count is %d\n",
          numvertices, qh num_vertices - qh_setsize(qh del_vertices));
      waserror = True;
    }
    if (qh hull_dim == 2 && numvertices != numfacets) {
      ivp_message(
          "qhull internal error (qh_checkpolygon): #vertices %d != #facets "
          "%d\n",
    ...
    if (qh_setsize(vertex->neighbors)) {
      FOREACHneighbor_(vertex) {
        if (!qh_setin(neighbor->vertices, vertex)) {
          ivp_message(
              "qhull internal error (qh_checkvertex): neighbor f%d does not "
              "contain v%d\n",
...
void qh_createsimplex(setT *vertices) {
  ...
  FOREACHvertex_i_(vertices) {
    ...
    newfacet->vertices =
        qh_setnew_delnthsorted(vertices, vertex_n, vertex_i, 0);
  ...
  FORALLnew_facets {
    ...
    FORALLfacet_(qh newfacet_list) {
      if (facet != newfacet) SETelem_(newfacet->neighbors, nth++) = facet;
    }
...
void qh_delvertex(vertexT *vertex) {
  if (vertex == qh tracevertex) qh tracevertex = NULL;
...
setT *qh_facet3vertex(facetT *facet) {
  ...
  if (facet->simplicial) {
    if (cntvertices != 3) {
      ivp_message(
          "qhull internal error (qh_facet3vertex): only %d vertices for "
          "simplicial facet f%d\n",
    ...
    while ((ridge = qh_nextridge3d(ridge, facet, &vertex))) {
      ...
      if (++cntprojected > cntvertices || ridge == firstridge) break;
    }
    if (!ridge || cntprojected != cntvertices) {
      ivp_message(
          "qhull internal error (qh_facet3vertex): ridges for facet %d don't "
...
/*-<a                             href="qh-c.htm#poly"
  ...
  qh_findbestfacet( point, bestoutside, bestdist, isoutside )
    ...
    for Delaunay triangulations,
      Use qh_setdelaunay() to lift point to paraboloid and scale by 'Qbb' if
  needed Do not use options 'Qbk', 'QBk', or 'QbB' since they scale the
  coordinates.
...
*/
facetT *qh_findbestfacet(pointT *point, boolT bestoutside, realT *bestdist,
                         boolT *isoutside) {
  facetT *bestfacet = NULL;
  int numpart, totpart = 0;

  bestfacet = qh_findbest(point, qh facet_list, bestoutside, False, bestoutside,
  ...
  if (*bestdist < -qh DISTround) {
    ...
    if ((isoutside && bestoutside) ||
      ...
      bestfacet = qh_findbest(point, bestfacet, bestoutside, False, bestoutside,
                              bestdist, isoutside, &totpart);
      totpart += numpart;
    }
  }
...
facetT *qh_findfacet_all(pointT *point, realT *bestdist, boolT *isoutside,
  ...
  *isoutside = False;
  FORALLfacets {
    if (facet->flipped || !facet->normal) continue;
    totpart++;
    qh_distplane(point, facet, &dist);
    if (dist > *bestdist) {
      *bestdist = dist;
...
int qh_findgood(facetT *facetlist, int goodhorizon) {
    ...
    if (!numgood && (!goodhorizon || qh GOODclosest)) {
      if (qh GOODclosest) {
        if (qh GOODclosest->visible)
          ...
          if (angle < bestangle) bestfacet = qh GOODclosest;
        }
      }
      if (bestfacet && bestfacet != qh GOODclosest) {
        if (qh GOODclosest) qh GOODclosest->good = False;
  ...
  trace2((qh ferr, "qh_findgood: found %d good facets with %d good horizon\n",
          numgood, goodhorizon));
  if (!numgood && qh GOODvertex > 0 && !qh MERGING) return goodhorizon;
  return numgood;
} /* findgood */
...
void qh_findgood_all(facetT *facetlist) {
  ...
  if (!qh GOODvertex && !qh GOODthreshold && !qh GOODpoint &&
      !qh SPLITthresholds)
    return;
  if (!qh ONLYgood) qh_findgood(qh facet_list, 0);
  FORALLfacet_(facetlist) {
    if (facet->good) numgood++;
  }
  if (qh GOODvertex < 0 || (qh GOODvertex > 0 && qh MERGING)) {
    FORALLfacet_(facetlist) {
      if (facet->good && ((qh GOODvertex > 0) ^
                          !!qh_isvertex(qh GOODvertexp, facet->vertices))) {
        if (!--numgood) {
          ivp_message(
              "qhull warning: good vertex p%d does not match last good facet "
              "f%d.  Ignored.\n",
...
void qh_infiniteloop [[noreturn]] (facetT *facet) {
  ivp_message(
      "qhull internal error (qh_infiniteloop): potential infinite loop "
      "detected\n");
  qh_errexit(qh_ERRqhull, facet, NULL);
} /* qh_infiniteloop */
...
void qh_initbuild(void) {
  ...
  if (qh SCALElast)
    qh_scalelast(qh first_point, qh num_points, qh hull_dim, qh MINlastcoord,
                 qh MAXlastcoord, qh MAXwidth);
  qh_detroundoff();
  ...
  if (qh PRINToptions1st || qh TRACElevel || qh IStracing) {
    if (qh TRACElevel || qh IStracing)
      ivp_message("\nTrace level %d for %s | %s\n",
                  qh IStracing ? qh IStracing : qh TRACElevel, qh rbox_command,
                  qh qhull_command);
  ...
  if (qh ONLYgood) {
    if (qh GOODvertex > 0 && qh MERGING) {
      ivp_message(
          "qhull input error: 'Qg QVn' (only good vertex) does not work with "
          "merging.\nUse 'QJ' to joggle the input or 'Q0' to turn off "
          "merging.\n");
      qh_errexit(qh_ERRinput, NULL, NULL);
    }
...
good point (QGn or QG-n), or a good vertex with 'QJ' or 'Q0' (QVn).\n");
    ...
    if (qh GOODvertex > 0 && !qh MERGING /* matches qh_partitionall */
      ...
      if (!isoutside) {
        ivp_message(
            "qhull input error: point for QV%d is inside initial simplex.  It "
            "can not be made a vertex.\n",
  ...
  qh_settempfree(&maxpoints);
  trace1(
      (qh ferr, "qh_initbuild: initial hull created and points partitioned\n"));
} /* initbuild */
...
/*-<a                             href="qh-c.htm#poly"
  ...
  design:
    ...
    doubles checks orientation (in case of axis-parallel facets with Gaussian
  elimination) checks for flipped facets and qh.NARROWhull checks the result
*/
void qh_initialhull(setT *vertices) {
  ...
  if (dist > 0) {
    FORALLfacets facet->toporient ^= True;
  }
  FORALLfacets qh_setfacetplane(facet);
  FORALLfacets {
    if (!qh_checkflipped(facet, NULL,
                         qh_ALL)) { /* due to axis-parallel facet */
      trace1((qh ferr,
              "qh_initialhull: initial orientation incorrect.  Correct all "
              "facets\n"));
      facet->flipped = False;
      FORALLfacets {
  ...
  FORALLfacets {
    if (!qh_checkflipped(facet, NULL, !qh_ALL)) { /* can happen with 'R0.1' */
      ...
      if (qh ferr) {
        ivp_message(
            "qhull precision error: initial facet %d is coplanar with the "
            "interior point\n",
            facet->id);
      }
...
*/
setT *qh_initialvertices(int dim, setT *maxpoints, pointT *points,
                         int numpoints) {
  pointT *point, **pointp;
  ...
  if (qh ALLpoints)
    ...
    qh_maxsimplex(fmin_(qh_INITIALsearch, dim), maxpoints, points, numpoints,
                  &simplex);
...
vertexT *qh_makenewfacets(pointT *point /*visible_list*/) {
  ...
  qh visit_id++;
  if (!qh ONLYgood) qh NEWfacets = True;
  FORALLvisible_facets {
    FOREACHneighbor_(visible) neighbor->seen = False;
    if (visible->ridges) {
...
*/
#ifndef qh_NOmerge
void qh_matchduplicates(facetT *atfacet, int atskip, int hashsize,
                        int *hashcount) {
  boolT same, ismatch;
  int hash, scan;
  ...
  trace2((qh ferr,
          "qh_matchduplicates: find duplicate matches for f%d skip %d hash %d "
          "hashcount %d\n",
          atfacet->id, atskip, hash, *hashcount));
  for (makematch = 0; makematch < 2; makematch++) {
    qh visit_id++;
    for (newfacet = atfacet, newskip = atskip; newfacet;
         newfacet = nextfacet, newskip = nextskip) {
      zinc_(Zhashlookup);
      nextfacet = NULL;
      newfacet->visitid = qh visit_id;
      for (scan = hash; (facet = SETelemt_(qh hash_table, scan, facetT));
           scan = (++scan >= hashsize ? 0 : scan)) {
        if (!facet->dupridge || facet->visitid == qh visit_id) continue;
        zinc_(Zhashtests);
        if (qh_matchvertices(1, newfacet->vertices, newskip, facet->vertices,
                             &skip, &same)) {
          ...
          if (SETelemt_(facet->neighbors, skip, facetT) != qh_DUPLICATEridge) {
            ...
            if (SETelemt_(newfacet->neighbors, newskip, facetT) ==
                qh_DUPLICATEridge) {
              SETelem_(facet->neighbors, skip) = newfacet;
              SETelem_(newfacet->neighbors, newskip) = qh_MERGEridge;
              *hashcount -= 2; /* removed two unmatched facets */
              trace4((qh ferr,
                      "qh_matchduplicates: duplicate f%d skip %d matched with "
            ...
            trace3((qh ferr,
                    "qh_matchduplicates: duplicate f%d skip %d new f%d skip %d "
                    "at dist %2.2g, max is now f%d f%d\n",
        ...
        if (makematch && !facet &&
          ...
          ivp_message(
              "qhull internal error (qh_matchduplicates): no MERGEridge match "
              "for duplicate f%d skip %d at hash %d\n",
    ...
    if (!makematch) {
      if (!maxmatch) {
        ivp_message(
            "qhull internal error (qh_matchduplicates): no maximum match at "
            "duplicate f%d skip %d at hash %d\n",
      ...
      *hashcount -= 2; /* removed two unmatched facets */
      zzinc_(Zmultiridge);
      trace0((qh ferr,
              "qh_matchduplicates: duplicate f%d skip %d matched with new f%d "
              "skip %d keep\n",
...
void qh_nearcoplanar(void /* qh.facet_list */) {
  ...
  if (!qh KEEPcoplanar && !qh KEEPinside) {
    FORALLfacets {
      if (facet->coplanarset) qh_setfree(&facet->coplanarset);
    }
    ...
    FORALLfacets {
      if (facet->coplanarset) {
        FOREACHpoint_(facet->coplanarset) {
          ...
          qh_distplane(point, facet, &dist);
          if (dist < innerplane) {
            if (!qh KEEPinside) SETref_(point) = NULL;
...
may have the same identifier.  Vertices not sorted correctly.\n",
  ...
  if (qh vertex_id == qh tracevertex_id) qh tracevertex = vertex;
...
ridgeT *qh_nextridge3d(ridgeT *atridge, facetT *facet, vertexT **vertexp) {
  ...
  FOREACHridge_(facet->ridges) {
    ...
    if (vertex == atvertex) {
      if (vertexp) *vertexp = othervertex;
      return ridge;
    }
  }
  return NULL;
...
pointT *qh_point(int id) {
  if (id < 0) return NULL;
  if (id < qh num_points) return qh first_point + id * qh hull_dim;
  id -= qh num_points;
...
setT *qh_pointfacet(void /*qh facet_list*/) {
  ...
  FORALLfacets {
    ...
    FOREACHpoint_(facet->coplanarset) qh_point_add(facets, point, facet);
    FOREACHpoint_(facet->outsideset) qh_point_add(facets, point, facet);
  }
  return facets;
} /* pointfacet */
...
setT *qh_pointvertex(void /*qh facet_list*/) {
  ...
  FORALLvertices qh_point_add(vertices, vertex->point, vertex);
  return vertices;
} /* pointvertex */
...
void qh_prependfacet(facetT *facet, facetT **facetlist) {
  ...
  if (prevfacet) prevfacet->next = facet;
  list->previous = facet;
  facet->next = *facetlist;
  if (qh facet_list == list) /* this may change *facetlist */
    qh facet_list = facet;
  if (qh facet_next == list) qh facet_next = facet;
  *facetlist = facet;
  qh num_facets++;
} /* prependfacet */
  ...
  FOREACHfacet_i_(qh hash_table) {
    if (facet) {
      FOREACHneighbor_i_(facet) {
        if (!neighbor || neighbor == qh_MERGEridge ||
            neighbor == qh_DUPLICATEridge)
          break;
      }
      if (neighbor_i == neighbor_n) continue;
      fprintf(fp, "hash %d f%d ", facet_i, facet->id);
      FOREACHvertex_(facet->vertices) fprintf(fp, "v%d ", vertex->id);
      fprintf(fp, "\n neighbors:");
      FOREACHneighbor_i_(facet) {
...
void qh_printlists(void) {
  ...
  FORALLfacets ivp_message(" %d", facet->id);
  ivp_message(
  ...
  FORALLvertices ivp_message(" %d", vertex->id);
  ivp_message("\n");
} /* printlists */
...
*/
void qh_resetlists(
    boolT stats /*qh newvertex_list newfacet_list visible_list*/) {
  vertexT *vertex;
  facetT *newfacet, *visible;
  int totnew = 0, totver = 0;

  if (stats) {
    FORALLvertex_(qh newvertex_list) totver++;
    FORALLnew_facets totnew++;
    zadd_(Zvisvertextot, totver);
    zmax_(Zvisvertexmax, totver);
    zadd_(Znewfacettot, totnew);
    zmax_(Znewfacetmax, totnew);
  }
  FORALLvertex_(qh newvertex_list) vertex->newlist = False;
  qh newvertex_list = NULL;
  FORALLnew_facets newfacet->newfacet = False;
...
/*-<a                             href="qh-c.htm"
  ...
  use:
    ...
    FOREACHneighbor_(vertex) {...} to visit the Voronoi centers for a Voronoi
  cell.
*/
...
boolT qh_vertexsubset(setT *vertexsetA, setT *vertexsetB) {
  ...
  while (True) {
    if (!*vertexA) return True;
    if (!*vertexB) return False;
    if ((*vertexA)->id > (*vertexB)->id) return False;
    if (*vertexA == *vertexB) vertexA++;
    vertexB++;
  }
