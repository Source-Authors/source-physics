...
IVV_Cluster_Min_Hash_Elem::~IVV_Cluster_Min_Hash_Elem() { delete next; }
 ...
 *	min_per_array_pos	parallel array of minimal elements per linked
 list
 *
 ...
 *			stadel[size - 2size-1] == min_per_array_pos[0 .. size-1]
 (same memory)

 */

// a new minimum element at a given hash index was set, so update stadel
void IVV_Cluster_Min_Hash::min_added_at_index(IVV_Cluster_Min_Hash_Elem *elem,
                                              int i) {
  unsigned int height;
...
void IVV_Cluster_Min_Hash::min_removed_at_index(IVV_Cluster_Min_Hash_Elem *elem,
                                                int i) {
    ...
    if (el) {
      for (el = el->next; el; el = el->next) {
        ...
        if (el->value < new_min->value ||
            (hk_Math::almost_equal(el->value, new_min->value) &&
             el->cmp_index < new_min->cmp_index)) {
          new_min = el;
        }
#else
        if (el->value < new_min->value) {
  ...
  for (height = size >> 1; height > 0;
    ...
    if (!stadel[j]) {
      ...
      if (stadel[j]->value < stadel[j + 1]->value ||
          (hk_Math::almost_equal(stadel[j]->value, stadel[j + 1]->value) &&
           stadel[j]->cmp_index < stadel[j + 1]->cmp_index))
#else
      if (stadel[j]->value < stadel[j + 1]->value)
#endif
...
void IVV_Cluster_Min_Hash::add(void *elem, IVP_DOUBLE val) {
  ...
  IVV_Cluster_Min_Hash_Elem *el =
      new IVV_Cluster_Min_Hash_Elem();  //(IVV_Cluster_Min_Hash_Elem
                                        //*)p_malloc(sizeof(IVV_Cluster_Min_Hash_Elem));
#if defined(SORT_MINDIST_ELEMENTS)
...
void IVV_Cluster_Min_Hash::remove_min() {
  IVV_Cluster_Min_Hash_Elem *min =
      (IVV_Cluster_Min_Hash_Elem *)this->find_min_elem();
  if (min) {
    this->remove(min);
  }
}
