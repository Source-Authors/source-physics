 ...
 *	Description:	collection of functions that affect the shape of a
 *compact surface
 ********************************************************************************/
 ...
 *	chop off a slice
 ********************************************************************************/
IVP_Compact_Surface *IVP_Compact_Modify::chop(
    const IVP_Compact_Surface *c_surface_in,
    const IVP_U_Float_Point *chop_vector, IVP_FLOAT chop_depth) {
  ...
  IVP_U_BigVector<IVP_Compact_Ledge> ledges;
  {
    ...
    IVP_ASSERT(ledges.len() == 1);  // currently, we just want to chop objects
                                    // that consist of just ONE convex ledge
  }
  ...
  distant_point.mult(
      -1e6f);  // set him far way. in future: check for far enough point.
  test_plane.calc_hesse_val(&distant_point);

  const IVP_Compact_Triangle *tri = the_ledge->get_first_triangle();
  IVP_DOUBLE min_d = -1.0f;
  for (int i = 0; i < the_ledge->get_n_triangles();
       i++, tri = tri->get_next_tri())  // needn't to be super-fast
  {
    for (int e = 0; e < 3; e++) {
      const IVP_Compact_Edge *edge = tri->get_edge(e);
      const IVP_Compact_Poly_Point *start_point =
          edge->get_start_point(the_ledge);
  ...
  chopped_compact_surface = IVP_SurfaceBuilder_Halfspacesoup::
      convert_halfspacesoup_to_compact_surface(&half_spaces, merge_eps);

  return chopped_compact_surface;
}

IVP_Compact_Ledge *IVP_Compact_Modify::shrink(const IVP_Compact_Ledge *ledge_in,
  ...
  IVP_Compact_Ledge *ledge =
      IVP_SurfaceBuilder_Halfspacesoup::convert_halfspacesoup_to_compact_ledge(
          &half_spaces, pointmerge_threshold);
  return ledge;
}

IVP_Compact_Surface *IVP_Compact_Modify::shrink(
    const IVP_Compact_Surface *c_surface_in, IVP_FLOAT shrink_value,
    IVP_DOUBLE pointmerge_threshold) {
  IVP_U_BigVector<IVP_Compact_Ledge> ledges;
  // get all the ledges
  {
  ...
  for (int l = ledges.len() - 1; l >= 0; l--) {
    IVP_Compact_Ledge *cl =
        shrink(ledges.element_at(l), shrink_value, pointmerge_threshold);
    if (cl) {
      ledges_dest.add(cl);
