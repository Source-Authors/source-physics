...
void IVP_Geompack::insfac_(int *p, double *nrmlc, int *nce, int *cedge,
                           double *cdang) {
  ...
  /*        CEDGE(1:2,0:NCE) - CEDGE(1,I) is an index of VCL, indices >
   * ADDR_OF_N_ORIGINAL_VERTICES */
  /*              are new points; CEDGE(2,I) = J indicates that edge of cut */
  /*              face ending at CEDGE(1,I) is edge from J to
   * FACEVERTICESLIST(SUCC,J) */
  /*              if J > 0; else if J < 0 then edge of cut face ending at */
  /*              CEDGE(1,I) is a new edge and CEDGE(1,I) lies on edge from */
  /*              -J to FACEVERTICESLIST(SUC,-J) and new edge lies in face
   * FACEVERTICESLIST(FACN,-J); */
  ...
  /*        ADDR_OF_N_WORK_VERTICES - number of positions used in
   * FACEVERTICESLIST, EDGE_ANGLES arrays */
  /*        NPOLH - number of polyhedra or positions used in
   * POLYHEDRONFIRSTFACEOFFSET array */
  /*        ADDR_OF_N_POLYHEDRONFACES - number of positions used in
   * POLYHEDRONFACEINDICES array */
  /*        ADDR_OF_SIZE_FACEARRAYS - maximum size available for FACESDATA,
   * FACESTYPE, NORMALS arrays */
  /*        ADDR_OF_SIZE_FACEVERTEXARRAYS - maximum size available for
   * FACEVERTICESLIST, EDGE_ANGLES arrays */
  /*        ADDR_OF_SIZE_POLYHEDRONFIRSTFACEOFFSET - maximum size available for
   * POLYHEDRONFIRSTFACEOFFSET array */
  /*        ADDR_OF_SIZE_POLYHEDRONFACEINDICES - maximum size available for
   * POLYHEDRONFACEINDICES array */
  /*        VCL(1:3,1:ADDR_OF_N_ORIGINAL_VERTICES+?) - vertex coordinate list;
   * the new vertices to */
  /*           be inserted as indicated by CEDGE are after column
   * ADDR_OF_N_ORIGINAL_VERTICES */
  ...
  /*        POLYHEDRONFIRSTFACEOFFSET(1:NPOLH) - head ptr to face indices in
   * POLYHEDRONFACEINDICES for each polyh */
  /*        POLYHEDRONFACEINDICES(1:2,1:ADDR_OF_N_POLYHEDRONFACES) - list of
   * signed face indices for each polyh */

  /*     Updated parameters: */
  /*        CEDGE(1:1,1:NCE) - updated to edges of cut face wrt positive */
  /*              half-space */
  /*        CEDGE(2:2,1:NCE) - neg. entries updated to index of new edge */
  /*        ADDR_OF_N_ORIGINAL_VERTICES,NFACE,ADDR_OF_N_WORK_VERTICES,NPOLH,ADDR_OF_N_POLYHEDRONFACES
   * - sizes updated due to cut face */
  /*        FACESDATA,FACESTYPE,NORMALS,FACEVERTICESLIST,EDGE_ANGLES,POLYHEDRONFIRSTFACEOFFSET,POLYHEDRONFACEINDICES
   * - updated by cut face */
  ...
  L20:
    ...
    // *******************************************************************
    insed3_(&a, &b, &polyhedronfirstfaceoffset[1]);
...
recheck_size:
  if ((this->n_work_vertices + *nce + 2) > this->size_facevertexarrays) {
    int res = 0;
    res = res | increase_memory((void **)&this->g_faceverticeslist,
                                &this->size_facevertexarrays, 6 * sizeof(int));
    res = res | increase_memory((void **)&this->g_edge_angles,
    ...
    int res = 0;
    res = res | increase_memory((void **)&this->g_normals,
                                &this->size_facearrays, 3 * sizeof(double));
    res = res | increase_memory((void **)&this->g_facesdata,
                                &this->size_facearrays, 3 * sizeof(int));
    res = res | increase_memory((void **)&this->g_facestype,
                                &this->size_facearrays, sizeof(int));
    if (res == 0) {
      this->ierr = 500;
      return;
    }
    ...
    int res =
        increase_memory((void **)&this->g_polyhedronfirstfaceoffset,
                        &this->size_polyhedronfirstfaceoffset, sizeof(int));
    if (res == 0) {
      this->ierr = 500;
      return;
    }
  ...
  faceverticeslist[this->n_work_vertices * 6 + 3] =
      facesdata[this->nface * 3 + 1];
  faceverticeslist[facesdata[this->nface * 3 + 1] * 6 + 4] =
      this->n_work_vertices;
  ...
  /*     and update POLYHEDRONFIRSTFACEOFFSET, POLYHEDRONFACEINDICES. NP1 is
   * used as a temporary polyhedron index. */
...
L60:
  ...
  if ((i__1 = facesdata[f * 3 + 2], abs(i__1)) ==
      (i__2 = facesdata[f * 3 + 3], abs(i__2))) {
    facesdata[f * 3 + 2] = np1;
    facesdata[f * 3 + 3] = -np1;
    facestype[f] = -1;
    dof = 1;
  ...
  for (i__ = 1; i__ <= i__1; ++i__) {
    ...
    if (facestype[f] == -1) {
      if (faceverticeslist[cedge[(i__ << 1) + 1] * 6 + 1] !=
          faceverticeslist[(nv + i__) * 6 + 1]) {
        j = 3;
      }
    }
...
L170:
  ...
  polyhedronfaceindices[((this->n_polyhedronfaces - 1) << 1) + 2] =
  ...
  IVP_IF(1) {
    IVP_IFDEBUG(IVP_DM_GEOMPACK_LEVEL2, {
      ivp_debugmanager.dprint(IVP_DM_GEOMPACK_LEVEL2,
                              "Cut face: #edges, polyh(1:2) = %d %d %d\n",
                              (*nce), facesdata[this->nface * 3 + 2],
                              facesdata[this->nface * 3 + 3]);
      i__1 = *nce;
      for (i__ = 1; i__ <= i__1; ++i__) {
