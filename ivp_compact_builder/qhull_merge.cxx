/*<html><pre>  -<a                             href="qh-c.htm#merge"
  >-------------------------------</a><a name="TOP">-</a>
   ...
   To remove deleted facets and vertices (qhull() in qhull.c):
     ...
     qh_resetlists (False);       // qh.visible_list newvertex_list
  newfacet_list
...
void qh_premerge(vertexT *apex, realT maxcentrum, realT maxangle) {
  ...
  if (qh ZEROcentrum && qh_checkzero(!qh_ALL)) return;
  trace2((qh ferr,
          "qh_premerge: premerge centrum %2.2g angle %2.2g for apex v%d "
          "facetlist f%d\n",
  ...
  if (qh hull_dim >= 3) {
    ...
    if (qh_merge_degenredundant()) othermerge = True;
  } else /* qh hull_dim == 2 */
    qh_mergecycle_all(qh newfacet_list, &othermerge);
...
void qh_postmerge(const char *reason, realT maxcentrum, realT maxangle,
  ...
  if (qh REPORTfreq || qh IStracing) {
    ...
    if (qh PRINTstatistics) qh_printallstatistics(qh ferr, "reason");
    ivp_message("\n%s with 'C%.2g' and 'A%.2g'\n", reason, maxcentrum,
                maxangle);
  }
  trace2(
      (qh ferr, "qh_postmerge: postmerge.  test vneighbors? %d\n", vneighbors));
  qh centrum_radius = maxcentrum;
  qh cos_max = maxangle;
  qh POSTmerging = True;
  qh degen_mergeset = qh_settemp(qh TEMPsize);
  ...
  if (qh visible_list != qh facet_list) { /* first call */
    ...
    FORALLnew_facets {
      ...
      if (!newfacet->simplicial) newfacet->newmerge = True;
      zinc_(Zpostfacets);
    }
    qh newvertex_list = qh vertex_list;
    FORALLvertices vertex->newlist = True;
    if (qh VERTEXneighbors) {                 /* a merge has occurred */
      FORALLvertices vertex->delridge = True; /* test for redundant, needed? */
      if (qh MERGEexact) {
...
void qh_all_merges(boolT othermerge, boolT vneighbors) {
  ...
  while (True) {
    ...
    if (qh VERTEXneighbors) {
      ...
      if (qh hull_dim >= 4 && qh POSTmerging) {  //-V112
        FORALLvertices vertex->delridge = True;
        isreduce = True;
      }
    ...
    if (vneighbors && qh_test_vneighbors(/* qh newfacet_list */)) continue;
    break;
  } /* while (True) */
  ...
  trace1((qh ferr,
          "qh_all_merges: merged %d coplanar facets %d concave facets and %d "
          "degen or redundant facets.\n",
...
/*-<a                             href="qh-c.htm#merge"
  ...
  design:
    ...
    if regular merge
      ...
      append to qh.degen_mergeset
*/
void qh_appendmergeset(facetT *facet, facetT *neighbor, mergeType mergetype,
                       realT *angle) {
  mergeT *merge, *lastmerge;
  void **freelistp; /* used !qh_NOmem */

  if (facet->redundant) return;
  if (facet->degenerate && mergetype == MRGdegen) return;
  qh_memalloc_(sizeof(mergeT), freelistp, merge, mergeT);
  merge->facet1 = facet;
  merge->facet2 = neighbor;
  merge->type = mergetype;
  if (angle && qh ANGLEmerge) merge->angle = *angle;
...
setT *qh_basevertices(facetT *samecycle) {
  ...
  trace4(
      (qh ferr, "qh_basevertices: found %d vertices\n", qh_setsize(vertices)));
  return vertices;
} /* basevertices */
...
void qh_checkconnect(void /* qh newfacet_list */) {
  ...
  FORALLnew_facets {
    if (newfacet->visitid == qh visit_id) break;
    ivp_message("qhull error: f%d is not attached to the new facets\n",
                newfacet->id);
    errfacet = newfacet;
  }
  if (errfacet) qh_errexit(qh_ERRqhull, errfacet, NULL);
} /* checkconnect */
...
boolT qh_checkzero(boolT testall) {
  ...
  if (testall)
    ...
    FORALLfacet_(facetlist) {
      ...
      if (!horizon->simplicial) goto LABELproblem;
    ...
    if (!testall) {
      FOREACHvertex_(horizon->vertices) {
        if (vertex->visitid != qh vertex_visit) {
          ...
          if (dist >= -qh DISTround) {
            qh ZEROall_ok = False;
            if (!qh MERGEexact || dist > qh DISTround) goto LABELnonconvex;
          }
          break;
        }
      }
...
LABELnonconvex:
  trace2((qh ferr,
          "qh_checkzero: facet f%d and f%d are not clearly convex.  v%d dist "
          "%.2g\n",
...
void qh_degen_redundant_facet(facetT *facet) {
  ...
  FOREACHneighbor_(facet) {
    qh vertex_visit++;
    FOREACHvertex_(neighbor->vertices) vertex->visitid = qh vertex_visit;
    FOREACHvertex_(facet->vertices) {
      if (vertex->visitid != qh vertex_visit) break;
    }
    if (!vertex) {
...
void qh_degen_redundant_neighbors(facetT *facet, facetT *delfacet) {
  ...
  if (!delfacet) delfacet = facet;
  qh vertex_visit++;
  FOREACHvertex_(facet->vertices) vertex->visitid = qh vertex_visit;
  FOREACHneighbor_(delfacet) {
    /* uses early out instead of checking vertex count */
    if (neighbor == facet) continue;
  ...
  FOREACHneighbor_(delfacet) { /* redundant merges occur first */
    ...
    if ((size = qh_setsize(neighbor->neighbors)) < qh hull_dim) {
      ...
      trace2((qh ferr,
              "qh_degen_redundant_neighbors: f%d is degenerate with %d "
              "neighbors.  Neighbor of f%d.\n",
...
vertexT *qh_find_newvertex(vertexT *oldvertex, setT *vertices, setT *ridges) {
  ...
  if (qh IStracing >= 4) {  //-V112
    ivp_message("qh_find_newvertex: find new vertex for v%d from ",
                oldvertex->id);
    FOREACHvertex_(vertices) ivp_message("v%d ", vertex->id);
    FOREACHridge_(ridges) ivp_message("r%d ", ridge->id);
    ivp_message("\n");
  }
#endif
  FOREACHvertex_(vertices) vertex->visitid = 0;
  FOREACHridge_(ridges) { FOREACHvertex_(ridge->vertices) vertex->visitid++; }
  ...
  qsort(SETaddr_(vertices, vertexT), qh_setsize(vertices), sizeof(vertexT *),
        qh_comparevisit);
  ...
  FOREACHridge_(ridges) qh_hashridge(qh hash_table, hashsize, ridge, oldvertex);
  FOREACHvertex_(vertices) {
    newridges = qh_vertexridges(vertex);
    FOREACHridge_(newridges) {
      if (qh_hashridge_find(qh hash_table, hashsize, ridge, vertex, oldvertex,
                            &hash)) {
        zinc_(Zdupridge);
        break;
      }
    ...
    if (!ridge) break; /* found a rename */
  }
  if (vertex) {
    /* counted in qh_renamevertex */
    trace2((qh ferr,
            "qh_find_newvertex: found v%d for old v%d from %d vertices and %d "
            "ridges.\n",
            vertex->id, oldvertex->id, qh_setsize(vertices),
            qh_setsize(ridges)));
  } else {
    zinc_(Zfindfail);
    trace0((qh ferr,
            "qh_find_newvertex: no vertex for renaming v%d (all duplicated "
            "ridges) during p%d\n",
...
/*-<a                             href="qh-c.htm#merge"
  ...
  qh_findbest_test( testcentrum, facet, neighbor, bestfacet, dist, mindist,
  maxdist ) test neighbor of facet for qh_findbestneighbor() if testcentrum,
...
void qh_findbest_test(boolT testcentrum, facetT *facet, facetT *neighbor,
                      facetT **bestfacet, realT *distp, realT *mindistp,
                      realT *maxdistp) {
  realT dist, mindist = FLT_MIN, maxdist;
...
*/
facetT *qh_findbestneighbor(facetT *facet, realT *distp, realT *mindistp,
                            realT *maxdistp) {
  ...
  *distp = REALmax;
  ...
  if (size > qh hull_dim + qh_BESTnonconvex) {
    FOREACHridge_(facet->ridges) {
      if (ridge->nonconvex) {
        ...
        qh_findbest_test(testcentrum, facet, neighbor, &bestfacet, distp,
                         mindistp, maxdistp);
      }
    }
  }
  if (!bestfacet) {
    nonconvex = False;
    FOREACHneighbor_(facet) qh_findbest_test(
        testcentrum, facet, neighbor, &bestfacet, distp, mindistp, maxdistp);
  }
  if (!bestfacet) {
    ivp_message(
  ...
  if (testcentrum) qh_getdistance(facet, bestfacet, mindistp, maxdistp);
  trace3((qh ferr,
          "qh_findbestneighbor: f%d is best neighbor for f%d testcentrum? %d "
          "nonconvex? %d dist %2.2g min %2.2g max %2.2g\n",
...
void qh_flippedmerges(facetT *facetlist, boolT *wasmerge) {
  ...
  FOREACHmerge_(othermerges) {
    facet1 = merge->facet1;
    if (merge->type != MRGflip || facet1->visible) continue;
...
void qh_forcedmerges(boolT *wasmerge) {
  ...
  qh_settemppush(othermerges);
  FOREACHmerge_(othermerges) {
    if (merge->type != MRGridge) continue;
    facet1 = merge->facet1;
    facet2 = merge->facet2;
    ...
    if (facet1 == facet2) continue;
    if (!qh_setin(facet2->neighbors, facet1)) {
      ivp_message(
          "qhull internal error (qh_forcedmerges): f%d and f%d had a duplicate "
    ...
    trace0((qh ferr,
            "qh_forcedmerges: duplicate ridge between f%d and f%d, dist %2.2g "
            "and reverse dist %2.2g during p%d\n",
  ...
  if (nummerge) *wasmerge = True;
  trace1((qh ferr,
          "qh_forcedmerges: merged %d facets and %d flipped facets across "
          "duplicated ridges\n",
          nummerge, numflip));
} /* forcedmerges */
...
void qh_getmergeset(facetT *facetlist) {
  ...
  qh visit_id++;
  FORALLfacet_(facetlist) {
    if (facet->tested) continue;
    facet->visitid = qh visit_id;
    facet->tested = True; /* must be non-simplicial due to merge */
    FOREACHneighbor_(facet) neighbor->seen = False;
    FOREACHridge_(facet->ridges) {
      if (ridge->tested && !ridge->nonconvex) continue;
      ...
      if (neighbor->seen) {
        ...
        if (qh_test_appendmerge(facet, neighbor)) ridge->nonconvex = True;
      }
    }
  }
  nummerges = qh_setsize(qh facet_mergeset);
  if (qh ANGLEmerge)
    qsort(SETaddr_(qh facet_mergeset, mergeT), nummerges, sizeof(mergeT *),
          qh_compareangle);
  else
    qsort(SETaddr_(qh facet_mergeset, mergeT), nummerges, sizeof(mergeT *),
          qh_comparemerge);
...
void qh_getmergeset_initial(facetT *facetlist) {
  ...
  FORALLfacet_(facetlist) {
    ...
    FOREACHridge_(facet->ridges) ridge->tested = True;
  }
  nummerges = qh_setsize(qh facet_mergeset);
  if (qh ANGLEmerge)
    qsort(SETaddr_(qh facet_mergeset, mergeT), nummerges, sizeof(mergeT *),
...
*/
void qh_hashridge(setT *hashtable, int hashsize, ridgeT *ridge,
                  vertexT *oldvertex) {
  int hash;
  ridgeT *ridgeA;

  hash =
      (int)qh_gethash(hashsize, ridge->vertices, qh hull_dim - 1, 0, oldvertex);
  while (True) {
  ...
  while ((ridgeA = SETelemt_(hashtable, hash, ridgeT))) {
    if (ridgeA == ridge)
      *hashslot = -1;
      ...
      if (qh_setequal_except(ridge->vertices, vertex, ridgeA->vertices,
                             oldvertex))
        return ridgeA;
    }
    if (++hash == hashsize) hash = 0;
  }
  if (!*hashslot) *hashslot = hash;
  return NULL;
} /* hashridge_find */
...
void qh_makeridges(facetT *facet) {
  ...
  FOREACHridge_(facet->ridges) otherfacet_(ridge, facet)->seen = True;
  FOREACHneighbor_i_(facet) {
    if (neighbor == qh_MERGEridge)
      ...
      ridge = qh_newridge();
      ridge->vertices =
          qh_setnew_delnthsorted(facet->vertices, qh hull_dim, neighbor_i, 0);
  ...
  if (mergeridge) {
    while (qh_setdel(facet->neighbors, qh_MERGEridge)); /* delete each one */
  }
} /* makeridges */
...
void qh_mark_dupridges(facetT *facetlist) {
  ...
  FORALLfacet_(facetlist) { /* gets rid of qh_MERGEridge */
    if (facet->mergeridge && !facet->mergeridge2) qh_makeridges(facet);
  }
  ...
  trace1(
      (qh ferr, "qh_mark_dupridges: found %d duplicated ridges\n", nummerge));
} /* mark_dupridges */
...
void qh_maydropneighbor(facetT *facet) {
  ...
  FOREACHneighbor_(facet) {
    if (neighbor->visitid != qh visit_id) {
      trace0((qh ferr,
              "qh_maydropneighbor: facets f%d and f%d are no longer neighbors "
              "during p%d\n",
  ...
  while ((merge = (mergeT *)qh_setdellast(qh degen_mergeset))) {
    ...
    if (mergetype == MRGredundant) {
      ...
      while (facet2->visible) {
        if (!facet2->f.replace) {
          ivp_message(
              "qhull internal error (qh_merge_degenredunant): f%d redundant "
              "but f%d has no replacement\n",
              facet1->id, facet2->id);
          qh_errexit2(qh_ERRqhull, facet1, facet2);
        }
      ...
      trace2((qh ferr,
              "qh_merge_degenredundant: facet f%d is contained in f%d, will "
              "merge\n",
              facet1->id, facet2->id));
      ...
      if (!(size = qh_setsize(facet1->neighbors))) {
        ...
        FOREACHvertex_(facet1->vertices) {
          ...
          if (!SETfirst_(vertex->neighbors)) {
            zinc_(Zdegenvertex);
            trace2((qh ferr,
                    "qh_merge_degenredundant: deleted v%d because f%d has no "
                    "neighbors\n",
                    vertex->id, facet1->id));
            vertex->deleted = True;
        ...
        trace2((qh ferr,
                "qh_merge_degenredundant: facet f%d has %d neighbors, merge "
                "into f%d dist %2.2g\n",
...
void qh_merge_nonconvex(facetT *facet1, facetT *facet2, mergeType mergetype) {
  ...
  if (dist < dist2) {
    ...
    trace2((qh ferr,
            "qh_merge_nonconvex: avoid merging old facet f%d dist %2.2g.  Use "
            "f%d dist %2.2g instead\n",
...
void qh_mergecycle(facetT *samecycle, facetT *newfacet) {
  ...
  if (!qh VERTEXneighbors) qh_vertexneighbors();
  zzinc_(Ztotmerge);
  if (qh REPORTfreq2 && qh POSTmerging) {
    if (zzval_(Ztotmerge) > qh mergereport + qh REPORTfreq2) qh_tracemerging();
  }
#ifndef qh_NOtrace
  if (qh TRACEmerge == zzval_(Ztotmerge))
    qhmem.IStracing = qh IStracing = qh TRACElevel;
  trace2((qh ferr,
          "qh_mergecycle: merge #%d for facets from cycle f%d into coplanar "
          "horizon f%d\n",
  ...
  if (newfacet == qh tracefacet) {
    ...
    qh IStracing = 4;  //-V112
    ivp_message(
        "qh_mergecycle: ========= trace merge %d of samecycle %d into trace "
        "f%d, furthest is p%d\n",
  ...
  if (qh IStracing >= 4) {  //-V112
    ...
    FORALLsame_cycle_(samecycle) ivp_message(" f%d", same->id);
    ivp_message("\n");
  }
  if (qh IStracing >= 4)  //-V112
  ...
  if (!newfacet->newfacet) qh_newvertices(newfacet->vertices);
...
void qh_mergecycle_all(facetT *facetlist, boolT *wasmerge) {
  ...
  for (facet = facetlist; facet && (nextfacet = facet->next);
       facet = nextfacet) {
    ...
    if (facet->f.samecycle == facet) {
      ...
      FOREACHvertex_(facet->vertices) {
        if (vertex != apex) vertex->delridge = True;
      }
      ...
      for (same = facet->f.samecycle; same; /* FORALLsame_cycle_(facet) */
        ...
        if (same->cycledone || same->visible) qh_infiniteloop(same);
        same->cycledone = True;
        if (same->normal) {
  ...
  if (cycles) *wasmerge = True;
  trace1((qh ferr,
          "qh_mergecycle_all: merged %d same cycles or facets into coplanar "
          "horizons\n",
          cycles));
} /* mergecycle_all */
...
void qh_mergecycle_facets(facetT *samecycle, facetT *newfacet) {
  ...
  for (same = samecycle->f.samecycle; same;
       same = (same == samecycle ? 0 : next)) {
...
void qh_mergecycle_neighbors(facetT *samecycle, facetT *newfacet) {
  ...
  FORALLsame_cycle_(samecycle) {
    FOREACHneighbor_(same) {
      if (neighbor->visitid == samevisitid) continue;
      if (neighbor->simplicial) {
        if (neighbor->visitid != qh visit_id) {
          ...
          neighbor->visitid = qh visit_id;
          FOREACHridge_(
              neighbor->ridges) { /* update ridge in case of qh_makeridges */
            if (ridge->top == same) {
              ridge->top = newfacet;
              break;
...
void qh_mergecycle_ridges(facetT *samecycle, facetT *newfacet) {
  ...
  trace4(
      (qh ferr, "qh_mergecycle_ridges: delete shared ridges from newfacet\n"));
  ...
  FORALLsame_cycle_(samecycle) {
    ...
    if (same->ridges) qh_settruncate(same->ridges, 0);
    if (!same->simplicial) continue;
    FOREACHneighbor_i_(same) { /* note: !newfact->simplicial */
      if (neighbor->visitid != samevisitid && neighbor->simplicial) {
        ridge = qh_newridge();
        ridge->vertices =
            qh_setnew_delnthsorted(same->vertices, qh hull_dim, neighbor_i, 0);
...
void qh_mergecycle_vneighbors(facetT *samecycle, facetT *newfacet) {
  ...
  FOREACHvertex_(vertices) {
    ...
    FOREACHneighbor_(vertex) {
      if (neighbor->visitid == mergeid) SETref_(neighbor) = NULL;
    }
    ...
    if (!SETsecond_(vertex->neighbors)) {
      zinc_(Zcyclevertex);
      trace2((qh ferr,
              "qh_mergecycle_vneighbors: deleted v%d when merging cycle f%d "
              "into f%d\n",
...
*/
void qh_mergefacet(facetT *facet1, facetT *facet2, realT *mindist,
                   realT *maxdist, boolT mergeapex) {
  ...
  if (qh build_cnt >= qh RERUN) {
    if (mindist && (-*mindist > qh TRACEdist || *maxdist > qh TRACEdist)) {
      ...
      traceonce = True;
      ivp_message(
          "qh_mergefacet: ========= trace wide merge #%d (%2.2g) for f%d into "
          "f%d, last point was p%d\n",
          zzval_(Ztotmerge), fmax_(-*mindist, *maxdist), facet1->id, facet2->id,
          qh furthest_id);
      ...
      traceonce = True;
      ivp_message(
          "qh_mergefacet: ========= trace merge #%d involving f%d, furthest is "
          "p%d\n",
  ...
  if (qh IStracing >= 2) {
    ...
    ivp_message(
        "qh_mergefacet: #%d merge f%d into f%d, mindist= %2.2g, maxdist= "
        "%2.2g\n",
  ...
  if (facet1 == facet2 || facet1->visible || facet2->visible) {
    ivp_message(
        "qhull internal error (qh_mergefacet): either f%d and f%d are the same "
        "or one is a visible facet\n",
        facet1->id, facet2->id);
    qh_errexit2(qh_ERRqhull, facet1, facet2);
  }
...
try 'C-0.001' instead of 'C-0.1' or 'A-0.999' instead of 'A-0.9'.\n",
  ...
  if (qh hull_dim > 2 && qh_setsize(facet1->vertices) == qh hull_dim)
    ...
    qh vertex_visit++;
    FOREACHvertex_(facet2->vertices) vertex->visitid = qh vertex_visit;
    if (qh hull_dim == 2)
    ...
    if (!facet2->newfacet) qh_newvertices(facet2->vertices);
  }
  if (!mergeapex) qh_degen_redundant_neighbors(facet2, facet1);
...
void qh_mergefacet2d(facetT *facet1, facetT *facet2) {
  ...
  facetT *neighbor1A, *neighbor1B, *neighbor2A, *neighbor2B, *neighborA,
      *neighborB;
  ...
  if (vertexA->id > vertexB->id) {
    ...
    if (vertexB == vertex2A) facet2->toporient = !facet2->toporient;
    ...
    if (vertexB == vertex2B) facet2->toporient = !facet2->toporient;
...
void qh_mergeneighbors(facetT *facet1, facetT *facet2) {
  ...
  trace4((qh ferr, "qh_mergeneighbors: merge neighbors of f%d and f%d\n",
          facet1->id, facet2->id));
  qh visit_id++;
  FOREACHneighbor_(facet2) { neighbor->visitid = qh visit_id; }
  FOREACHneighbor_(facet1) {
    if (neighbor->visitid == qh visit_id) {
      if (neighbor->simplicial) /* is degen, needs ridges */
        qh_makeridges(neighbor);
      if (SETfirstt_(neighbor->neighbors, facetT) !=
          facet1) /*keep newfacet->horizon*/
...
void qh_mergeridges(facetT *facet1, facetT *facet2) {
  ...
  FOREACHridge_(facet2->ridges) {
    if ((ridge->top == facet1) || (ridge->bottom == facet1)) {
      FOREACHvertex_(ridge->vertices) vertex->delridge = True;
...
void qh_mergesimplex(facetT *facet1, facetT *facet2, boolT mergeapex) {
  ...
  if (mergeapex) {
    if (!facet2->newfacet) qh_newvertices(facet2->vertices); /* apex is new */
    ...
    zinc_(Zmergesimplex);
    FOREACHvertex_(facet1->vertices) vertex->seen = False;
    FOREACHridge_(facet1->ridges) {
    ...
    FOREACHvertex_(facet1->vertices) {
      if (!vertex->seen) break; /* must occur */
    }
    apex = vertex;
    ...
    if (!issubset) qh_setaddnth(&facet2->vertices, vertex_i, apex);
    if (!facet2->newfacet)
...
void qh_mergevertex_neighbors(facetT *facet1, facetT *facet2) {
  ...
  if (qh tracevertex) qh_errprint("TRACE", NULL, NULL, NULL, qh tracevertex);
...
void qh_mergevertices(setT *vertices1, setT **vertices2) {
  int newsize =
      qh_setsize(vertices1) + qh_setsize(*vertices2) - qh hull_dim + 1;
  setT *mergedvertices;
  ...
  while (*vertex2) qh_setappend(&mergedvertices, *vertex2++);
  if (newsize < qh_setsize(mergedvertices)) {
    ivp_message(
        "qhull internal error (qh_mergevertices): facets did not share a "
        "ridge\n");
...
setT *qh_neighbor_intersections(vertexT *vertex) {
  ...
  trace3((qh ferr,
          "qh_neighbor_intersections: %d vertices in neighbor intersection of "
          "v%d\n",
...
boolT qh_reducevertices(void) {
  ...
  if (qh hull_dim == 2) return False;
  if (qh_merge_degenredundant()) degenredun = True;
LABELrestart:
  FORALLnew_facets {
    if (newfacet->newmerge) {
      if (!qh MERGEvertices) newfacet->newmerge = False;
      qh_remove_extravertices(newfacet);
    }
  }
  if (!qh MERGEvertices) return False;
  FORALLnew_facets {
    if (newfacet->newmerge) {
  ...
  trace1((qh ferr,
          "qh_reducevertices: renamed %d shared vertices and %d redundant "
          "vertices. Degen? %d\n",
...
boolT qh_remove_extravertices(facetT *facet) {
  ...
  trace4((qh ferr, "qh_remove_extravertices: test f%d for extra vertices\n",
          facet->id));
  FOREACHvertex_(facet->vertices) vertex->seen = False;
  FOREACHridge_(facet->ridges) {
  ...
  FOREACHvertex_(facet->vertices) {
    if (!vertex->seen) {
      ...
      if (!qh_setsize(vertex->neighbors)) {
        ...
        zinc_(Zremvertexdel);
        trace2((qh ferr,
                "qh_remove_extravertices: v%d deleted because it's lost all "
                "ridges\n",
                vertex->id));
      } else
        trace3((qh ferr,
                "qh_remove_extravertices: v%d removed from f%d because it's "
                "lost all ridges\n",
                vertex->id, facet->id));
      vertexp--; /*repeat*/
    }
...
vertexT *qh_rename_sharedvertex(vertexT *vertex, facetT *facet) {
  ...
  if (qh_setsize(vertex->neighbors) == 2) {
    neighborA = SETfirstt_(vertex->neighbors, facetT);
    if (neighborA == facet) neighborA = SETsecondt_(vertex->neighbors, facetT);
  } else if (qh hull_dim == 3)
    return NULL;
  else {
    qh visit_id++;
    FOREACHneighbor_(facet) neighbor->visitid = qh visit_id;
    FOREACHneighbor_(vertex) {
      if (neighbor->visitid == qh visit_id) {
        if (neighborA) return NULL;
        neighborA = neighbor;
    ...
    if (!neighborA) {
      ivp_message(
          "qhull internal error (qh_rename_sharedvertex): v%d's neighbors not "
          "in f%d\n",
          vertex->id, facet->id);
  ...
  trace2((qh ferr,
          "qh_rename_sharedvertex: p%d (v%d) is shared by f%d (%d ridges) and "
          "f%d\n",
          qh_pointid(vertex->point), vertex->id, facet->id, qh_setsize(ridges),
          neighborA->id));
  zinc_(Zintersectnum);
...
*/
void qh_renameridgevertex(ridgeT *ridge, vertexT *oldvertex,
                          vertexT *newvertex) {
  int nth = 0, oldnth;
  facetT *temp;
  vertexT *vertex, **vertexp;
  ...
  FOREACHvertex_(ridge->vertices) {
    if (vertex == newvertex) {
      ...
      qh_delridge(ridge);
      trace2((qh ferr,
              "qh_renameridgevertex: ridge r%d deleted.  It contained both v%d "
              "and v%d\n",
              ridge->id, oldvertex->id, newvertex->id));
      return;
    }
    if (vertex->id < newvertex->id) break;
    nth++;
  }
...
void qh_renamevertex(vertexT *oldvertex, vertexT *newvertex, setT *ridges,
  ...
  if (qh IStracing >= 2 || oldvertex->id == qh tracevertex_id ||
    ...
    istrace = True;
  FOREACHridge_(ridges) qh_renameridgevertex(ridge, oldvertex, newvertex);
  if (!oldfacet) {
    zinc_(Zrenameall);
    if (istrace)
    ...
    FOREACHneighbor_(oldvertex) qh_setdelsorted(neighbor->vertices, oldvertex);
    oldvertex->deleted = True;
    qh_setappend(&qh del_vertices, oldvertex);
  } else {
    zinc_(Zrenamepinch);
...
boolT qh_test_appendmerge(facetT *facet, facetT *neighbor) {
  ...
  if (qh SKIPconvex && !qh POSTmerging) return False;
  ...
  if (dist > qh centrum_radius)
    ...
    if (dist > -qh centrum_radius) iscoplanar = True;
    if (!neighbor->center) neighbor->center = qh_getcentrum(neighbor);
    zzinc_(Zcentrumtests);
  ...
  if (isconcave) {
    zinc_(Zconcaveridge);
    if (qh ANGLEmerge) angle += qh_ANGLEconcave + 0.5;
    qh_appendmergeset(facet, neighbor, MRGconcave, &angle);
    trace0((qh ferr,
            "qh_test_appendmerge: concave f%d to f%d dist %4.4g and reverse "
            "dist %4.4g angle %4.4g during p%d\n",
    ...
    trace2((qh ferr,
            "qh_test_appendmerge: coplanar f%d to f%d dist %4.4g, reverse dist "
            "%4.4g angle %4.4g\n",
...
boolT qh_test_vneighbors(void /* qh newfacet_list */) {
  ...
  if (!qh VERTEXneighbors) qh_vertexneighbors();
  FORALLnew_facets newfacet->seen = False;
  FORALLnew_facets {
    newfacet->seen = True;
    newfacet->visitid = qh visit_id++;
    FOREACHneighbor_(newfacet) newfacet->visitid = qh visit_id;
    FOREACHvertex_(newfacet->vertices) {
      FOREACHneighbor_(vertex) {
        if (neighbor->seen || neighbor->visitid == qh visit_id) continue;
        if (qh_test_appendmerge(newfacet, neighbor)) nummerges++;
      }
    }
  }
...
void qh_tracemerge(facetT *facet1, facetT *facet2) {
  ...
  if (facet2 == qh tracefacet || (qh tracevertex && qh tracevertex->newlist)) {
    ivp_message(
        "qh_tracemerge: trace facet and vertex after merge of f%d and f%d, "
        "furthest p%d\n",
        facet1->id, facet2->id, qh furthest_id);
    if (facet2 != qh tracefacet)
      qh_errprint("TRACE", qh tracefacet,
                  (qh tracevertex && qh tracevertex->neighbors)
                      ? SETfirstt_(qh tracevertex->neighbors, facetT)
                      : 0,
                  0, qh tracevertex);
  }
  if (qh tracevertex) {
  ...
  if (qh tracefacet) {
    ...
    if (waserror) qh_errexit(qh_ERRqhull, qh tracefacet, NULL);
  }
#endif /* !qh_NOtrace */
  if (qh CHECKfrequently ||
      qh IStracing >= 4) { /* can't check polygon here */  //-V112
    qh_checkfacet(facet2, True, &waserror);
    if (waserror) qh_errexit(qh_ERRqhull, NULL, NULL);
  }
} /* tracemerge */
...
/*-<a                             href="qh-c.htm#merge"
  ...
  design:
    ...
    if facet2 has a centrum
      ...
      else if facet2 has 3 vertices due to many merges, or not large and post
  merging clear facet2->keepcentrum unless facet2->keepcentrum clear
  facet2->center to recompute centrum later clear ridge->tested for facet2's
  ridges
*/
void qh_updatetested(facetT *facet1, facetT *facet2) {
  ...
  facet2->tested = False;
  FOREACHridge_(facet1->ridges) ridge->tested = False;
  if (!facet2->center) return;
  ...
  if (!facet2->keepcentrum) {
    ...
    if (size == qh hull_dim || qh POSTmerging)
      facet2->keepcentrum =
          False; /* if many merges need to recompute centrum */
  }
  if (!facet2->keepcentrum) {
    qh_memfree(facet2->center, qh normal_size);
    facet2->center = NULL;
    FOREACHridge_(facet2->ridges) ridge->tested = False;
  }
} /* updatetested */
...
setT *qh_vertexridges(vertexT *vertex) {
  ...
  qh visit_id++;
  FOREACHneighbor_(vertex) neighbor->visitid = qh visit_id;
  FOREACHneighbor_(vertex) {
...
void qh_vertexridges_facet(vertexT *vertex, facetT *facet, setT **ridges) {
  ...
  FOREACHridge_(facet->ridges) {
    neighbor = otherfacet_(ridge, facet);
    if (neighbor->visitid == qh visit_id && qh_setin(ridge->vertices, vertex))
