...
/*-<a                             href="qh-c.htm#geom"
  ...
  returns:
     ...
     if will not be able to divzero() when normalized (qh.MINdenom_2 and
  qh.MINdenom_1_2), if fails on last row this means that the hyperplane
  intersects [0,..,1] sets last coordinate of normal to sign otherwise sets tail
  of normal to [...,sign,0,...], i.e., solves for b= [0...0] sets nearzero
  ...
  *normalp-- = (sign ? -1.0 : 1.0);
  for (i = numrow; i--;) {
    *normalp = 0.0;
    ai = rows[i] + i + 1;
    ak = normalp + 1;
    for (j = i + 1; j < numcol; j++) *normalp -= *ai++ * *ak++;
    diagonal = (rows[i])[i];
    if (fabs_(diagonal) > qh MINdenom_2)
      ...
      *normalp = qh_divzero(*normalp, diagonal, qh MINdenom_1_2, &waszero);
      if (waszero) {
        zerocol = i;
        *(normalp--) = (sign ? -1.0 : 1.0);
        for (normal_tail = normalp + 2; normal_tail < normal + numcol;
             normal_tail++)
          *normal_tail = 0.0;
      } else
        normalp--;
...
void qh_distplane(pointT *point, facetT *facet, realT *dist) {
  ...
  switch (qh hull_dim) {
    ...
    case 6:
      *dist = facet->offset + point[0] * normal[0] + point[1] * normal[1] +
              point[2] * normal[2] + point[3] * normal[3] +
              point[4] * normal[4] + point[5] * normal[5];
      break;
    case 7:
      *dist = facet->offset + point[0] * normal[0] + point[1] * normal[1] +
              point[2] * normal[2] + point[3] * normal[3] +
    ...
    case 8:
      *dist = facet->offset + point[0] * normal[0] + point[1] * normal[1] +
              point[2] * normal[2] + point[3] * normal[3] +
              point[4] * normal[4] + point[5] * normal[5] +
              point[6] * normal[6] + point[7] * normal[7];
      break;
    default:
      *dist = facet->offset;
      coordp = point;
      for (k = qh hull_dim; k--;) *dist += *coordp++ * *normal++;
      break;
  }
  zinc_(Zdistplane);
  ...
  if (qh RANDOMdist) {
    randr = qh_RANDOMint;
    *dist +=
        (2.0 * randr / qh_RANDOMmax - 1.0) * qh RANDOMfactor * qh MAXabs_coord;
  }
...
/*-<a                             href="qh-c.htm#geom"
  ...
  qh_findbest( point, startfacet, bestoutside, newfacts, dist, isoutside,
  numpart ) find facet that is furthest below a point searches neighbors of
  coplanar and most flipped facets for upperDelaunay facets searches if coplanar
  (within searchdist) returns facet only if !noupper and clearly above
...
*/
facetT *qh_findbest(pointT *point, facetT *startfacet, boolT bestoutside,
                    boolT newfacets, boolT noupper, realT *dist,
                    boolT *isoutside, int *numpart) {
  ...
  boolT testhorizon =
      ispartition && (bestoutside || qh APPROXhull || qh MERGING);

  if (!ischeckmax && !ispartition && !isfindfacet) {
    ivp_message(
        "qhull internal error (qh_findbest): unknown combination of "
        "arguments\n");
    qh_errexit(qh_ERRqhull, startfacet, NULL);
  }
  if (qh TRACElevel && qh TRACEpoint >= 0 &&
      qh TRACEpoint == qh_pointid(point)) {
  ...
  if (!startfacet->flipped) {
    *numpart = 1;
    ...
    if (!startfacet->upperdelaunay || (!noupper && *dist >= qh MINoutside)) {
      ...
      if (!bestoutside && *dist >= qh MINoutside) goto LABELreturn_best;
    }
#if qh_MAXoutside
    if (ischeckmax && (!qh ONLYgood || startfacet->good) &&
        *dist > startfacet->maxoutside)
  ...
  do { /* search neighbors of coplanar, upperdelaunay, and flipped facets */
    ...
    LABELrestart: /* directed search whenever improvement > searchdist */
      newbest = False;
      trace4((qh ferr,
              "qh_findbest: neighbors of f%d, bestdist %2.2g cutoff %2.2g "
              "searchdist %2.2g\n",
              facet->id, bestdist, cutoff, searchdist));
      FOREACHneighbor_(facet) {
        if (ispartition && !neighbor->newfacet) continue;
        if (!neighbor->flipped) {
...
#endif /* dangling else! */
            if (bestfacet && *dist < cutoff) continue;
          if (*dist > bestdist) {
  ...
  } while (searchsize && (facet = (facetT *)qh_setdellast(qh searchset)));
  if (!ischeckmax) {
    if (!bestfacet) {
      ivp_message(
          "qh_findbest: point p%d starting at f%d bestoutside? %d newfacets "
          "%d\n",
          qh TRACEpoint, startfacet->id, bestoutside, newfacets);
      fprintf(qh ferr,
              "\n\
...
Please report this error to qhull_bug@geom.umn.edu with the input and all of the output.\n",
  ...
  *dist = bestdist;
  if (isoutside && bestdist < qh MINoutside) *isoutside = False;
LABELreturn_best:
...
facetT *qh_findbestnew(pointT *point, facetT *startfacet, realT *dist,
  ...
  if (!startfacet) {
    if (qh MERGING)
      fprintf(qh ferr,
              "qhull precision error (qh_findbestnew): merging has formed and "
              "deleted an independent cycle of facets.  Can not continue.\n");
    else
      fprintf(qh ferr,
              "qhull internal error (qh_findbestnew): no new facets for point "
              "p%d\n",
              qh furthest_id);
  ...
  *numpart = 0;
  ...
  for (i = 0, facet = startfacet; i < 2; i++, facet = qh newfacet_list) {
    FORALLfacet_(facet) {
      ...
      if (facet->upperdelaunay) {
        ...
        bestdist = *dist;
        bestfacet = facet;
        if (*dist >= distoutside) goto LABELreturn_bestnew;
      }
    }
  }
  ...
  if (!bestfacet ||
    ...
    *dist = bestdist;
  if (isoutside && *dist < qh MINoutside) *isoutside = False;
LABELreturn_bestnew:
  qh IStracing = oldtrace;
  return bestfacet;
} /* findbestnew */
...
*/
void qh_gausselim(realT **rows, int numrow, int numcol, boolT *sign,
                  boolT *nearzero) {
...
realT qh_getangle(pointT *vect1, pointT *vect2) {
  ...
  for (k = qh hull_dim; k--;) angle += *vect1++ * *vect2++;
  if (qh RANDOMdist) {
    randr = qh_RANDOMint;
    angle += (2.0 * randr / qh_RANDOMmax - 1.0) * qh RANDOMfactor;
  }
...
pointT *qh_getcenter(setT *vertices) {
  ...
  for (k = 0; k < qh hull_dim; k++) {
    coord = center + k;
    *coord = 0.0;
    FOREACHvertex_(vertices) *coord += vertex->point[k];
    *coord /= count;
  }
  return (center);
} /* getcenter */
...
*/
realT qh_getdistance(facetT *facet, facetT *neighbor, realT *mindist,
                     realT *maxdist) {
  vertexT *vertex, **vertexp;
  realT dist, maxd, mind;

  FOREACHvertex_(facet->vertices) vertex->seen = False;
  FOREACHvertex_(neighbor->vertices) vertex->seen = True;
  mind = 0.0;
  maxd = 0.0;
...
*/
void qh_normalize2(coordT *normal, int dim, boolT toporient, realT *minnorm,
                   boolT *ismin) {
  int k;
  ...
  if (dim == 2)
    ...
    norm =
        sqrt((*normal) * (*normal) + (*norm1) * (*norm1) + (*norm2) * (*norm2));
  else if (dim == 4) {  //-V112
    norm = sqrt((*normal) * (*normal) + (*norm1) * (*norm1) +
  ...
  if (norm > qh MINdenom) {
    ...
    *norm1 /= norm;
    ...
    temp = sqrt(1.0 / dim);
    for (k = dim, colp = normal; k--;) *colp++ = temp;
  } else {
    if (!toporient) norm = -norm;
    for (k = dim, colp = normal; k--; colp++) { /* k used below */
      ...
      if (!zerodiv)
        *colp = temp;
        ...
        for (k = dim, colp = normal; k--; colp++) *colp = 0.0;
        *maxp = temp;
...
pointT *qh_projectpoint(pointT *point, facetT *facet, realT dist) {
  ...
  np = newpoint;
  normal = facet->normal;
  for (k = qh hull_dim; k--;) *(np++) = *point++ - dist * *normal++;
  return (newpoint);
} /* projectpoint */
...
void qh_setfacetplane(facetT *facet) {
  ...
  zzinc_(Zsetplane);
  if (!facet->normal) qh_memalloc_(normsize, freelistp, facet->normal, coordT);
  ...
  if (qh hull_dim <= 4) {  //-V112
    ...
    if (qh RANDOMdist) {
      ...
      FOREACHvertex_(facet->vertices) {
        qh gm_row[i++] = gmcoord;
        coord = vertex->point;
        for (k = qh hull_dim; k--;) *(gmcoord++) = *coord++ * qh_randomfactor();
      }
    } else {
      FOREACHvertex_(facet->vertices) qh gm_row[i++] = vertex->point;
    }
  ...
  if (qh hull_dim > 4 || nearzero) {  //-V112
    ...
    FOREACHvertex_(facet->vertices) {
      if (vertex->point != point0) {
        ...
        coord = vertex->point;
        point = point0;
        for (k = qh hull_dim; k--;) *(gmcoord++) = *coord++ - *point++;
      }
    }
    ...
    if (qh RANDOMdist) {
      ...
      for (i = qh hull_dim - 1; i--;) {
        for (k = qh hull_dim; k--;) *(gmcoord++) *= qh_randomfactor();
      }
    }
    ...
    if (nearzero) {
      if (qh_orientoutside(facet)) {
        trace0((qh ferr,
                "qh_setfacetplane: flipped orientation after testing "
                "interior_point during p%d\n",
                qh furthest_id));
      ...
      if (vertex->point != point0) {
        ...
        if (dist > wwval_(Wnewvertexmax)) {
          ...
          if (dist > qh max_outside) {
            ...
            if (dist > qh TRACEdist) istrace = True;
          }
        } else if (-dist > qh TRACEdist)
          istrace = True;
        if (istrace) {
          ivp_message(
              "qh_setfacetplane: ====== vertex p%d (v%d) increases max_outside "
              "to %2.2g for new facet f%d last p%d\n",
              qh_pointid(vertex->point), vertex->id, dist, facet->id,
              qh furthest_id);
  ...
  if (qh IStracing >= 3) {
    ivp_message("qh_setfacetplane: f%d offset %2.2g normal: ", facet->id,
                facet->offset);
    for (k = 0; k < qh hull_dim; k++) ivp_message("%2.2g ", facet->normal[k]);
    ivp_message("\n");
  }
  if (facet == qh tracefacet) qh IStracing = oldtrace;
} /* setfacetplane */
...
/*-<a                             href="qh-c.htm#geom"
  ...
  notes:
    ...
    solves det(P-V_0, V_n-V_0, ..., V_1-V_0)=0, i.e. every point is on
  hyperplane see Bower & Woodworth, A programmer's geometry, Butterworths 1983.
  ...
  derivation of 3-d minnorm
    ...
    Let u be machine roundoff and distround be max error for distance
  computation The max error for n_0 is sqrt(3) u M_a M_d / norm.  n_1 is approx.
  1 and n_2 is approx. 0 The max error for distance of V_1 is sqrt(3) u M_a M_d
  M_d / norm.  Offset=0 at origin Then minnorm = 1.8 u M_a M_d M_d / qh.ONEmerge
    Note that qh.one_merge is approx. 45.5 u M_a and norm is usually about M_d
  M_d

  derivation of 4-d minnorm
    same as above except rotate the facet so that V_1 on x-axis and w_2, y_3,
  w_3=0 [if two vertices fixed on x-axis, can rotate the other two in yzw.] n_0
  = det3_(...) = y_2 det2_(z_1, w_1, z_3, w_3) = - y_2 w_1 z_3 [all other terms
  contain at least two factors nearly zero.] The max error for n_0 is sqrt(4) u
  M_a M_d M_d / norm Then minnorm = 2 u M_a M_d M_d M_d / qh.ONEmerge Note that
  qh.one_merge is approx. 82 u M_a and norm is usually about M_d M_d M_d
*/
void qh_sethyperplane_det(int dim, coordT **rows, coordT *point0,
  ...
  if (dim == 2) {
    ...
    *offset = -(point0[0] * normal[0] + point0[1] * normal[1] +
    ...
    normal[0] = -det3_(dY(2, 0), dZ(2, 0), dW(2, 0), dY(1, 0), dZ(1, 0),
                       dW(1, 0), dY(3, 0), dZ(3, 0), dW(3, 0));
    normal[1] = det3_(dX(2, 0), dZ(2, 0), dW(2, 0), dX(1, 0), dZ(1, 0),
                      dW(1, 0), dX(3, 0), dZ(3, 0), dW(3, 0));
    normal[2] = -det3_(dX(2, 0), dY(2, 0), dW(2, 0), dX(1, 0), dY(1, 0),
                       dW(1, 0), dX(3, 0), dY(3, 0), dW(3, 0));
    normal[3] = det3_(dX(2, 0), dY(2, 0), dZ(2, 0), dX(1, 0), dY(1, 0),
                      dZ(1, 0), dX(3, 0), dY(3, 0), dZ(3, 0));
    qh_normalize2(normal, dim, toporient, NULL, NULL);
    *offset = -(point0[0] * normal[0] + point0[1] * normal[1] +
...
/*-<a                             href="qh-c.htm#geom"
  ...
  ) given (dim-1) X dim array of rows[i]= V_{i+1} - V_0 (point0) set normalized
  hyperplane equation from oriented simplex
...
void qh_sethyperplane_gauss(int dim, coordT **rows, pointT *point0,
                            boolT toporient, coordT *normal, coordT *offset,
                            boolT *nearzero) {
  ...
  for (k = dim - 1; k--;) {
    if ((rows[k])[k] < 0) sign ^= 1;
  }
  if (*nearzero) {
    zzinc_(Znearlysingular);
    trace0((qh ferr,
            "qh_sethyperplane_gauss: nearly singular or axis parallel "
            "hyperplane during p%d.\n",
            qh furthest_id));
    ...
    if (nearzero2) {
      zzinc_(Znearlysingular);
      trace0((qh ferr,
              "qh_sethyperplane_gauss: singular or axis parallel hyperplane at "
              "normalization during p%d.\n",
              qh furthest_id));
    }
  }
  if (nearzero2) *nearzero = True;
  qh_normalize2(normal, dim, True, NULL, NULL);
  pointcoord = point0;
  normalcoef = normal;
  *offset = -(*pointcoord++ * *normalcoef++);
  for (k = dim - 1; k--;) *offset -= *pointcoord++ * *normalcoef++;
} /* sethyperplane_gauss */
