...
void IVP_Halfspacesoup::add_halfspace(const IVP_U_Hesse *plane) {
  ...
  for (i = 0; i < planes->len(); i++) {
    ...
    if (dp >
        1 - HALFSPACESOUP_TOLERANCE) {  // plane is almost identical (position,
                                        // orientation and direction) to a
                                        // previous one in list
      if (plane_old->hesse_val <
          plane->hesse_val) {  // drop plane that is further away
        return;
      }
...
IVP_Halfspacesoup::IVP_Halfspacesoup(const IVP_Compact_Ledge *ledge) {
  ...
  for (int i = 0; i < ledge->get_n_triangles();
       tri = tri->get_next_tri(), i++) {
    IVP_U_Hesse hesse;
