...
void IVP_Geompack::edght_(int *a, int *b, int *v, int *hdfree, int *last,
                          int *w) {
  int bptr, newp, k, aa, bb, ptr;

  int *edge = this->g_intworkarray;
  int max_n_edges = (int)(this->size_intworkarray / 4) -
                    1;  // max_n_edges = size of int work array / 4 because we
                        // have to store 4 values for each edge! //-V112
  ...
  /*        HASHTABLE(0:HASHTABLE_SIZE-1) - hash table of head pointers (direct
   * chaining */
  ...
  /*        HASHTABLE,EDGE - if key with A,B is found then this record is
   * deleted */
