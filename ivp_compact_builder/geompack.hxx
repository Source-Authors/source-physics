...
class IVP_Geompack {
 private:
  ...
  int size_doubleworkarray;  // orig: maxwk [5000]. Should be divisible by 3 &
                             // 4!;
  ...
  int size_polyhedronfirstfaceoffset;  // Leave this to "2" if there will never
                                       // be more than one polyhedron to
                                       // decompose. <orig. maxhf [200]>
  int *g_polyhedronfirstfaceoffset;    // 1 int for each polyhedron <orig. hfl>

  int size_polyhedronfaceindices;  // number of entries (NOT bytesize of array!)
                                   // <orig: maxpf [2000]>
 ...
 public:
  ...
  void cutfac_(int *p, double *nrmlc, double *dtol, int *nedgc, int *pedge,
               int *nce, int *cedge, double *cdang, long int *rflag);

  void dsphdc_();

  void edght_(int *a, int *b, int *v, int *hdfree, int *last, int *w);

  void insed3_(int *a, int *b, int *hfl);

  void insfac_(int *p, double *nrmlc, int *nce, int *cedge, double *cdang);

  void insvr3_(int a);

  int prime_(int k);

  void ptpolg_(int dim, int ldv, int *nv, int inc, int *pgind, double *vcl,
               double *pt, double *nrml, double *dtol, int *inout);

  void resedg_(int u, long int *rflag);
