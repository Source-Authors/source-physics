...
#define P_OBJECT_BLUR \
  0.01  // some other eps depend on it
...
class IVP_Tri_Edge  // Triangle Edge
{
 public:
  ...
  IVP_Tri_Edge *behind;  // falls kein blatt die direkt dahinterliegende Kannte
                         // des darunterliegenden Dreiecks
  ...
  IVP_Tri_Edge *search_nearest_edge_to(IVP_U_Point *reference,
                                       IVP_DOUBLE *quad_old_dist);
...
class IVP_Object_Polygon_Tetra {
 ...
 private:
  IVP_U_Min_Hash *min_hash[P_HASH_CLASS_MAX];  // needed to find the best
                                               // element for tetraification
 public:
 ...
 private:
  ...
  IVP_ERROR_STRING breakivp_triangle(IVP_Triangle *triangle,
                                     IVP_Triangle *no_break_a = NULL,
                                     IVP_Triangle *no_break_b = NULL,
                                     IVP_Triangle *no_break_c = NULL);
  ...
  void add_edge_to_min_list(IVP_Tri_Edge *edge, P_HASH_CLASS hash_class,
                            IVP_DOUBLE rating);
  ...
  IVP_BOOL p_link_edge(IVP_Tri_Edge *edge,
                       IVP_Tri_Edge *neighbor);  // returns true if problems
  void pop_concav_edge(IVP_Tri_Edge *edge);
  void pop_problematic_edge(IVP_Tri_Edge *edge);
  int link_triangle_couple(IVP_Triangle *tri_a, IVP_Tri_Edge *neighbor_0,
                           IVP_Tri_Edge *neighbor_1, IVP_Tri_Edge *neighbor_2);
  ...
  IVP_DOUBLE calc_rot_inertia_for_optimization(IVP_Template_Polygon *tpop,
                                               IVP_FLOAT angle, short rot_axis,
                                               short value);
  void maximize_one_physic_axis(IVP_Template_Polygon *tpop,
                                IVP_U_Matrix *m_axis, short max, short rot_axis,
                                short start);
 ...
 public:
  ...
  void calc_main_physic_axis(IVP_Template_Polygon *tpop,
                             IVP_U_Matrix *m_core_f_pop_out);
  ...
  static IVP_Triangle *generate_double_triangle(IVP_Poly_Point *p1,
                                                IVP_Poly_Point *p2,
                                                IVP_Poly_Point *p3);
};
