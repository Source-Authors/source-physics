...
void IVP_Concave_Polyhedron_Face::add_offset(hk_intp offset_in) {
  IVP_Concave_Polyhedron_Face_Pointoffset *p_offset =
...
IVP_Concave_Polyhedron_Face::~IVP_Concave_Polyhedron_Face() {
  ...
  for (i = 0; i < this->point_offset.len(); i++) {
    IVP_Concave_Polyhedron_Face_Pointoffset *offset =
        this->point_offset.element_at(i);
    P_DELETE(offset);
  }
  return;
}
...
int IVP_Convex_Decompositor::perform_convex_decomposition_on_concave_polyhedron(
  ...
  int *fvl;    // face vertex list: stores the offsets (into vcl) of points for
               // each face
    ...
    if (!vcl) {
      IVP_IFDEBUG(IVP_DEBUG_IPION_ERROR_MSG, {
        ivp_debugmanager.dprint(
            IVP_DEBUG_IPION_ERROR_MSG,
            "IVP_Convex_Decompositor::perform_convex_decomposition_on_concave_"
            "polyhedron()\n"
    ...
    for (i = 0; i < n_faces; i++) {
      ...
      // fill 'face vertex list' with offsets of all the faces' points
      {
        ...
        for (j = 0; j < n_points_of_face; j++) {
          IVP_Concave_Polyhedron_Face_Pointoffset *point_offset =
              face->point_offset.element_at(j);
  ...
  // IMPORTANT: DO NOT free the above allocated memory yourself! GEOMPACK might
  // reallocate it!
...
fclose(fp);
    ...
    for (i = 0; i < nvert_out; i++) {
      ...
      IVP_Convex_Subpart_Work *subpart_work =
          subparts_offsets.element_at(subpart_number);
    ...
    for (i = 0; i < npolh_out; i++) {
      ...
      IVP_IF(1) {
        IVP_IFDEBUG(IVP_DM_CONVEX_DECOMPOSITOR, {
          ivp_debugmanager.dprint(IVP_DM_CONVEX_DECOMPOSITOR,
                                  "Points of convex subpart:\n");
        })
      }
      int j;
      for (j = 0; j < subpart_work->offsets.len(); j++) {
        ...
        IVP_IF(1){IVP_IFDEBUG(IVP_DM_CONVEX_DECOMPOSITOR, {
          ivp_debugmanager.dprint(IVP_DM_CONVEX_DECOMPOSITOR,
                                  "   %.6f %.6f %.6f\n", point->k[0],
                                  point->k[1], point->k[2]);
