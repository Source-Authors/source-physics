...
void qh_crossproduct(int dim, realT vecA[3], realT vecB[3], realT vecC[3]) {
  if (dim == 3) {
    vecC[0] = det2_(vecA[1], vecA[2], vecB[1], vecB[2]);
    vecC[1] = -det2_(vecA[0], vecA[2], vecB[0], vecB[2]);
    vecC[2] = det2_(vecA[0], vecA[1], vecB[0], vecB[1]);
  }
} /* vcross */
...
realT qh_determinant(realT **rows, int dim, boolT *nearzero) {
  ...
  *nearzero = False;
  if (dim < 2) {
    ...
    det = det2_(rows[0][0], rows[0][1], rows[1][0], rows[1][1]);
    if (fabs_(det) <
        qh NEARzero[1]) /* not really correct, what should this be? */
      *nearzero = True;
  } else if (dim == 3) {
    det = det3_(rows[0][0], rows[0][1], rows[0][2], rows[1][0], rows[1][1],
                rows[1][2], rows[2][0], rows[2][1], rows[2][2]);
    if (fabs_(det) <
        qh NEARzero[2]) /* not really correct, what should this be? */
      *nearzero = True;
  } else {
    qh_gausselim(rows, dim, dim, &sign,
                 nearzero); /* if nearzero, diagonal still ok*/
    det = 1.0;
    for (i = dim; i--;) det *= (rows[i])[i];
    if (sign) det = -det;
  }
  return det;
} /* determinant */
...
void qh_detroundoff(void) {
  ...
  if (!qh SETroundoff) {
    ...
    if (qh RANDOMdist) qh DISTround += qh RANDOMfactor * qh MAXabs_coord;
    qh_option("Error-roundoff", NULL, &qh DISTround);
  }
  qh MINdenom_1 = fmax_(1.0 / REALmax, REALmin);
  qh MINdenom = qh MINdenom_1 * qh MAXabs_coord;
  qh MINdenom_1_2 =
      sqrt(qh MINdenom_1 * qh hull_dim); /* if will be normalized */
  qh MINdenom_2 = qh MINdenom_1_2 * qh MAXabs_coord;
  /* for inner product */
  qh ANGLEround = 1.01 * qh hull_dim * REALepsilon;
  if (qh RANDOMdist) qh ANGLEround += qh RANDOMfactor;
  ...
  { /* compute ONEmerge, max vertex offset for merging simplicial facets */
    ...
    qh ONEmerge =
        sqrt(qh hull_dim) * qh MAXwidth * sqrt(1.0 - maxangle * maxangle) +
        qh DISTround;
    maxrho = qh hull_dim * qh premerge_centrum + qh DISTround;
    maximize_(qh ONEmerge, maxrho);
    maxrho = qh hull_dim * qh postmerge_centrum + qh DISTround;
    maximize_(qh ONEmerge, maxrho);
    if (qh MERGING) qh_option("_one-merge", NULL, &qh ONEmerge);
  }
  qh NEARinside =
      qh ONEmerge * qh_RATIOnearinside; /* only used if qh KEEPnearinside */
  if (qh JOGGLEmax < REALmax / 2 && (qh KEEPcoplanar || qh KEEPinside)) {
    realT maxdist;

    qh KEEPnearinside = True;
    maxdist = sqrt(qh hull_dim) * qh JOGGLEmax + qh DISTround;
    maxdist = 2 * maxdist; /* vertex and coplanar point can joggle in opposite
                              directions */
    maximize_(qh NEARinside, maxdist); /* must agree with qh_nearcoplanar() */
  }
  if (qh KEEPnearinside) qh_option("_near-inside", NULL, &qh NEARinside);
  if (qh JOGGLEmax < qh DISTround) {
    ivp_message(
        "qhull error: the joggle for 'QJn', %.2g, is below roundoff for "
        "distance computations, %.2g\n",
...
realT qh_detsimplex(pointT *apex, setT *points, int dim, boolT *nearzero) {
  ...
  FOREACHpoint_(points) {
    ...
    coordp = point;
    coorda = apex;
    for (k = dim; k--;) *(gmcoord++) = *coordp++ - *coorda++;
  }
  if (i < dim) {
    ivp_message(
...
realT qh_distnorm(int dim, pointT *point, pointT *normal, realT *offsetp) {
  ...
  dist = *offsetp;
  for (k = dim; k--;) dist += *(coordp++) * *(normalp++);
  return dist;
} /* distnorm */
...
realT qh_facetarea(facetT *facet) {
  ...
  if (facet->simplicial) {
    ...
    area =
        qh_facetarea_simplex(qh hull_dim, apex->point, facet->vertices, apex,
                             facet->toporient, facet->normal, &facet->offset);
  } else {
    if (qh CENTERtype == qh_AScentrum)
      centrum = facet->center;
    else
      centrum = qh_getcentrum(facet);
    FOREACHridge_(facet->ridges) area += qh_facetarea_simplex(
        qh hull_dim, centrum, ridge->vertices, NULL, (ridge->top == facet),
        facet->normal, &facet->offset);
    if (qh CENTERtype != qh_AScentrum) qh_memfree(centrum, qh normal_size);
  }
...
/*-<a                             href="qh-c.htm#geom"
  ...
  qh_facetarea_simplex( dim, apex, vertices, notvertex, toporient, normal,
  offset ) return area for a simplex defined by an apex, a base of vertices, an
  orientation, and a unit normal if simplicial facet, notvertex is defined and
  it is skipped in vertices
...
realT qh_facetarea_simplex(int dim, coordT *apex, setT *vertices,
                           vertexT *notvertex, boolT toporient, coordT *normal,
                           realT *offset) {
  ...
  if (i != dim - 1) {
    ivp_message(
        "qhull internal error (qh_facetarea_simplex): #points %d != dim %d "
        "-1\n",
        i, dim);
    qh_errexit(qh_ERRqhull, NULL, NULL);
  }
  rows[i] = gmcoord;
  if (qh DELAUNAY) {
    for (i = 0; i < dim - 1; i++) rows[i][dim - 1] = 0.0;
    for (k = dim; k--;) *(gmcoord++) = 0.0;
    rows[dim - 1][dim - 1] = -1.0;
  } else {
    normalp = normal;
    for (k = dim; k--;) *(gmcoord++) = *normalp++;
  }
  zinc_(Zdetsimplex);
  area = qh_determinant(rows, dim, &nearzero);
  if (toporient) area = -area;
  area *= qh AREAfactor;
  trace4((qh ferr,
          "qh_facetarea_simplex: area=%2.2g for point p%d, toporient %d, "
          "nearzero? %d\n",
...
/*-<a                             href="qh-c.htm#geom"
  ...
  design:
    for all facets on qh.newfacet_list
      ...
      update best facet
*/
boolT qh_findbestsharp(pointT *point, facetT **bestfacet, realT *bestdist,
                       int *numpart) {
  facetT *facet;
  realT dist;
  boolT issharp = False;
  ...
  FORALLfacet_(qh newfacet_list) {
    if (facet == qh newfacet_list) {
      for (k = qh hull_dim; k--;) quadrant[k] = (facet->normal[k] > 0);
    } else if (!issharp) {
      for (k = qh hull_dim; k--;) {
  ...
  *facetlist = facetA;
  ...
  FORALLfacet_(*facetlist) {
    FOREACHneighbor_(facet) {
      ...
      if (goodseen && !neighbor->good) continue;
      zinc_(Zcheckpart);
...
/*-<a                             href="qh-c.htm#geom"
  ...
  returns:
    ...
    for Delaunay triangulation, computes projected area of the lower or upper
  hull ignores upper hull if qh ATinfinity
...
void qh_getarea(facetT *facetlist) {
  ...
  FORALLfacet_(facetlist) {
    ...
    if (facet->upperdelaunay && qh ATinfinity) continue;
    facet->f.area = area = qh_facetarea(facet);
    facet->isarea = True;
    if (qh DELAUNAY) {
      if (facet->upperdelaunay == qh UPPERdelaunay) qh totarea += area;
    } else {
      qh totarea += area;
...
boolT qh_gram_schmidt(int dim, realT **row) {
  ...
  for (i = 0; i < dim; i++) {
    rowi = row[i];
    for (norm = 0.0, k = dim; k--; rowi++) norm += *rowi * *rowi;
    norm = sqrt(norm);
    wmin_(Wmindenom, norm);
    if (norm == 0.0) /* either 0 or overflow due to sqrt */
      return False;
    for (k = dim; k--;) *(--rowi) /= norm;
    for (j = i + 1; j < dim; j++) {
      rowj = row[j];
      for (norm = 0.0, k = dim; k--;) norm += *rowi++ * *rowj++;
      for (k = dim; k--;) *(--rowj) -= *(--rowi) * norm;
    }
  }
  return True;
} /* gram_schmidt */
...
boolT qh_inthresholds(coordT *normal, realT *angle) {
  ...
  for (k = 0; k < qh hull_dim; k++) {
    ...
    if (threshold > -REALmax / 2) {
      if (normal[k] < threshold) within = False;
      if (angle) {
        threshold -= normal[k];
...
void qh_joggleinput(void) {
  ...
  if (qh build_cnt > 1 && qh JOGGLEmax > fmax_(qh MAXwidth / 4, 0.1)) {
    ivp_message(
        "qhull error: the current joggle for 'QJn', %.2g, is too large for the "
        "width\nof the input.  If possible, recompile Qhull with "
...
setT *qh_maxmin(pointT *points, int numpoints, int dimension) {
  ...
  if (qh ZEROcentrum) qh ZEROall_ok = True;
  if constexpr (REALmin < REALepsilon && REALmin < REALmax &&
                REALmin > -REALmax && REALmax > 0.0 && -REALmax < 0.0)
    ; /* all ok */
  else {
    ivp_message(
...
/*-<a                             href="qh-c.htm#geom"
  ...
  for joggle:
    qh_setfacetplane() updated qh.max_outer for Wnewvertexmax (max distance to
  vertex) need to use Wnewvertexmax since could have a coplanar point for a high
...
realT qh_maxouter(void) {
  ...
  dist += qh DISTround;
  trace4((qh ferr,
          "qh_maxouter: max distance from facet to outer plane is %2.2g "
          "max_outside is %2.2g\n",
          dist, qh max_outside));
  return dist;
} /* maxouter */
...
*/
void qh_maxsimplex(int dim, setT *maxpoints, pointT *points, int numpoints,
                   setT **simplex) {
  ...
  if (sizinit < 2) {
    ...
    if (qh_setsize(*simplex) < 2) qh_setunique(simplex, maxx);
    sizinit = qh_setsize(*simplex);
    if (sizinit < 2) {
      ...
      if (zzval_(Zsetplane) > qh hull_dim + 1) {
        ...
        if (qh ferr) {
          ivp_message(
              "qhull input error: input is less than %d-dimensional since it "
  ...
  for (k = sizinit; k < dim + 1; k++) {
    ...
    if (!maxpoint) {
      ivp_message(
          "qhull internal error (qh_maxsimplex): not enough points "
          "available\n");
      qh_errexit(qh_ERRqhull, NULL, NULL);
    }
    qh_setappend(simplex, maxpoint);
    trace1((qh ferr,
            "qh_maxsimplex: selected point p%d for %d`th initial vertex, "
            "det=%2.2g\n",
...
boolT qh_orientoutside(facetT *facet) {
  ...
  qh_distplane(qh interior_point, facet, &dist);
  if (dist > 0) {
    for (k = qh hull_dim; k--;) facet->normal[k] = -facet->normal[k];
...
*/
void qh_printmatrix(FILE *fp, const char *string, realT **rows, int numrow,
                    int numcol) {
  realT *rowp;
  realT r; /*bug fix*/
  int i, k;
...
void qh_printpoints(FILE *fp, const char *string, setT *points) {
  ...
  if (string) {
    fprintf(fp, "%s", string);
    FOREACHpoint_(points) fprintf(fp, " p%d", qh_pointid(point));
    fprintf(fp, "\n");
  } else {
    FOREACHpoint_(points) fprintf(fp, " %d", qh_pointid(point));
    fprintf(fp, "\n");
  }
} /* printpoints */
...
void qh_projectinput(void) {
  ...
  for (k = 0; k < qh input_dim; k++) { /* skip Delaunay bound */
    if (hk_Math::almost_zero(qh lower_bound[k]) &&
        hk_Math::almost_zero(qh upper_bound[k])) {
      project[k] = -1;
      newdim--;
    }
  }
  if (qh DELAUNAY) {
    project[k] = 1;
    newdim++;
    if (qh ATinfinity) newnum++;
  }
  if (newdim != qh hull_dim) {
    fprintf(qh ferr,
            "qhull internal error (qh_projectinput): dimension after "
            "projection %d != hull_dim %d\n",
            newdim, qh hull_dim);
    qh_errexit(qh_ERRqhull, NULL, NULL);
  }
  ...
  qh_projectpoints(project, qh input_dim + 1, qh first_point, qh num_points,
                   qh input_dim, newpoints, newdim);
  trace1((qh ferr, "qh_projectinput: updating lower and upper_bound\n"));
  qh_projectpoints(project, qh input_dim + 1, qh lower_bound, 1,
                   qh input_dim + 1, qh lower_bound, newdim + 1);
  qh_projectpoints(project, qh input_dim + 1, qh upper_bound, 1,
                   qh input_dim + 1, qh upper_bound, newdim + 1);
  qh_memfree(project, ((qh input_dim + 1) * sizeof(*project)));
  if (qh POINTSmalloc) P_FREE(qh first_point);
  qh first_point = newpoints;
  qh POINTSmalloc = True;
  if (qh DELAUNAY && qh ATinfinity) {
    coord = qh first_point;
    infinity = qh first_point + qh hull_dim * qh num_points;
    for (k = qh hull_dim - 1; k--;) infinity[k] = 0.0;
    ...
    /* coord == infinity */
    for (k = qh hull_dim - 1; k--;) *(coord++) /= qh num_points;
    *(coord++) = maxboloid * 1.1;
    qh num_points++;
    trace0((qh ferr,
...
*/
void qh_projectpoints(signed char *project, int n, realT *points, int numpoints,
                      int dim, realT *newpoints, int newdim) {
  int testdim = dim, oldk = 0, newk = 0, i, j = 0, k;
  realT *newp, *oldp;

  for (k = 0; k < n; k++) testdim += project[k];
  if (testdim != newdim) {
    ivp_message(
        "qhull internal error (qh_projectpoints): newdim %d should be %d after "
        "projection\n",
        newdim, testdim);
    qh_errexit(qh_ERRqhull, NULL, NULL);
  }
...
void qh_rotatepoints(realT *points, int numpoints, int dim, realT **row) {
  ...
  for (point = points, j = numpoints; j--; point += dim) {
    ...
    for (i = 0; i < dim; i++) {
      rowi = row[i];
      coord = point;
      for (sum = 0.0, k = dim; k--;) sum += *rowi++ * *coord++;
      *(newval++) = sum;
    }
    for (k = dim; k--;) *(--coord) = *(--newval);
  }
} /* rotatepoints */
...
void qh_scaleinput(void) {
  ...
  qh_scalepoints(qh first_point, qh num_points, qh hull_dim, qh lower_bound,
                 qh upper_bound);
} /* scaleinput */
...
void qh_scalelast(coordT *points, int numpoints, int dim, coordT low,
  ...
  scale = qh_divzero(newhigh, high - low, qh MINdenom_1, &nearzero);
  if (nearzero) {
    ivp_message(
        "qhull input error: last coordinate's new bounds [0, %2.2g] too wide "
  ...
  coord = points + dim - 1;
  for (i = numpoints; i--; coord += dim) *coord = *coord * scale + shift;
} /* scalelast */
...
*/
void qh_scalepoints(pointT *points, int numpoints, int dim, realT *newlows,
                    realT *newhighs) {
  int i, k;
  ...
  for (k = 0; k < dim; k++) {
    newhigh = newhighs[k];
    newlow = newlows[k];
    if (newhigh > REALmax / 2 && newlow < -REALmax / 2) continue;
    low = REALmax;
    high = -REALmax;
    ...
    if (newhigh > REALmax / 2) newhigh = high;
    if (newlow < -REALmax / 2) newlow = low;
    if (qh DELAUNAY && k == dim - 1 && newhigh < newlow) {
      ivp_message(
          "qhull input error: 'Qb%d' or 'QB%d' inverts paraboloid since high "
          "bound %.2g < low bound %.2g\n",
          k, k, newhigh, newlow);
      qh_errexit(qh_ERRinput, NULL, NULL);
    }
    scale = qh_divzero(newhigh - newlow, high - low, qh MINdenom_1, &nearzero);
    if (nearzero) {
      ivp_message(
          "qhull input error: %d'th dimension's new bounds [%2.2g, %2.2g] too "
    ...
    trace0((qh ferr,
            "qh_scalepoints: scaled %d'th coordinate [%2.2g, %2.2g] to [%.2g, "
            "%.2g] for %d points by %2.2g and shifted %2.2g\n",
...
/*-<a                             href="qh-c.htm#geom"
  ...
  qh_setdelaunay( dim, count, points )
    ...
    if qh.last_low defined (i.e., 'Qbb' called qh_scalelast)
      calls qh_scalelast to scale the last coordinate the same as the other
  points
...
void qh_setdelaunay(int dim, int count, pointT *points) {
  ...
  trace0((qh ferr,
          "qh_setdelaunay: project %d points to paraboloid for Delaunay "
          "triangulation\n",
          count));
  coordp = points;
  for (i = 0; i < count; i++) {
  ...
  if (qh last_low < REALmax / 2)
    qh_scalelast(points, count, dim, qh last_low, qh last_high,
                 qh last_newhigh);
} /* setdelaunay */
...
*/
boolT qh_sethalfspace(int dim, coordT *coords, coordT **nextp, coordT *normal,
                      coordT *offset, coordT *feasible) {
  ...
  dist = *offset;
  for (k = dim; k--;) dist += *(normp++) * *(feasiblep++);
  if (dist > 0) goto LABELerroroutside;
  normp = normal;
  if (dist < -qh MINdenom) {
    for (k = dim; k--;) *(coordp++) = *(normp++) / -dist;
  } else {
    for (k = dim; k--;) {
      *(coordp++) = qh_divzero(*(normp++), -dist, qh MINdenom_1, &zerodiv);
      if (zerodiv) goto LABELerroroutside;
    }
  }
  *nextp = coordp;
  if (qh IStracing >= 4) {  //-V112
    ivp_message("qh_sethalfspace: halfspace at offset %6.2g to point: ",
                *offset);
...
LABELerroroutside:
  ...
  normp = normal;
  fprintf(qh ferr,
          "qhull input error: feasible point is not clearly inside "
          "halfspace\nfeasible point: ");
  for (k = dim; k--;) ivp_message(qh_REAL_1, r = *(feasiblep++));
  ivp_message("\n     halfspace: ");
  for (k = dim; k--;) ivp_message(qh_REAL_1, r = *(normp++));
  ivp_message("\n     at offset: ");
...
/*-<a                             href="qh-c.htm#geom"
  ...
  design:
    see qh_sethalfspace
*/
coordT *qh_sethalfspace_all(int dim, int count, coordT *halfspaces,
                            pointT *feasible) {
  int i, newdim;
  pointT *newpoints;
...
pointT *qh_voronoi_center(int dim, setT *points) {
  ...
  for (k = 0; k < dim; k++) {
    ...
    FOREACHpoint_(simplex) {
      if (point != point0) *(gmcoord++) = point[k] - point0[k];
    }
  }
  sum2row = gmcoord;
  for (i = 0; i < dim; i++) {
  ...
  if (infinite) {
    for (k = dim; k--;) center[k] = qh_INFINITE;
    if (qh IStracing)
    ...
    for (i = 0; i < dim; i++) {
      ...
      center[i] =
          qh_determinant(qh gm_row, dim, &nearzero) * factor + point0[i];
    }
#ifndef qh_NOtrace
    if (qh IStracing >= 3) {
      ...
      if (qh IStracing >= 5) {
        ...
        FOREACHpoint_(simplex) ivp_message("p%d dist %.2g, ", qh_pointid(point),
