...
// ------------------------------------------------------------------------
IVP_U_Point *IVP_SurfaceBuilder_Halfspacesoup::insert_point_into_list(
    IVP_U_Point *point, IVP_U_Vector<IVP_U_Point> *points,
    IVP_DOUBLE quad_threshold) {
  int i;
  for (i = 0; i < points->len(); i++) {
    ...
    if (point->quad_distance_to(old_point) < quad_threshold) {
      ...
      ivp_message(
          "  +++ Dropping almost similar point {%f, %f, %f} in favour of {%f, "
          "%f, %f}\n",
          point->k[0], point->k[1], point->k[2], old_point->k[0],
          old_point->k[1], old_point->k[2]);
#endif
      P_DELETE(point);
      return (old_point);
    }
...
// ------------------------------------------------------------------------
int IVP_SurfaceBuilder_Halfspacesoup::convert_halfspacesoup_to_points(
    IVP_Halfspacesoup *halfspaces, IVP_DOUBLE pointmerge_threshold,
    ...
    for (j = i + 1; j < halfspaces->len(); j++) {
      ...
      for (k = j + 1; k < halfspaces->len(); k++) {
        ...
        if (debug_plane) {
          ...
          ivp_message("  {%f,%f,%f}, %f\n", plane1->k[0], plane1->k[1],
                      plane1->k[2], plane1->hesse_val);
          ivp_message("  {%f,%f,%f}, %f\n", plane2->k[0], plane2->k[1],
                      plane2->k[2], plane2->hesse_val);
          ivp_message("  {%f,%f,%f}, %f\n", plane3->k[0], plane3->k[1],
                      plane3->k[2], plane3->hesse_val);
        }
#endif
              ...
              if (debug_plane) {
                if (debug_plane == plane) {
                  ...
                  ivp_message("  {%f,%f,%f}, %f\n", plane3->k[0], plane3->k[1],
                              plane3->k[2], plane3->hesse_val);
                  ivp_message(
                      "  +++ Dropping point {%f, %f, %f}\n      because "
                      "outside of plane {%f, %f, %f}, %f\n      Distance = "
                      "%f\n",
                      point.k[0], point.k[1], point.k[2], plane->k[0],
                      plane->k[1], plane->k[2], plane->hesse_val,
  ...
  // -------------------------------------------------------------------------------
  // to avoid numerical problems when triangularizing the object we merge any
  // points that are closer than a user-defined threshold
  // -------------------------------------------------------------------------------
  for (i = 0; i < points->len(); i++) {
    ...
    for (j = points->len() - 1; j > i; j--) {
      ...
      if (p1->quad_distance_to(p2) < threshold) {
        IVP_IF(1) {
          ivp_message(
              "*** "
              "IVP_SurfaceBuilder_Halfspacesoup::convert_halfspacesoup_to_"
              "points - removing ");
          p2->print();
        }
        points->remove(p2);
...
IVP_SurfaceBuilder_Halfspacesoup::convert_halfspacesoup_to_compact_ledge(
  ...
  IVP_SurfaceBuilder_Halfspacesoup::convert_halfspacesoup_to_points(
      halfspaces, pointmerge_threshold, &points);
...
IVP_Compact_Surface *
IVP_SurfaceBuilder_Halfspacesoup::convert_halfspacesoup_to_compact_surface(
    IVP_Halfspacesoup *halfspaces, IVP_DOUBLE pointmerge_threshold) {
  ...
  if (ledge) {
    ...
    ivp_message(
        "*** "
        "IVP_SurfaceBuilder_Halfspacesoup::convert_halfspacesoup_to_compact_"
