...
IVP_Compact_Recursive::IVP_Compact_Recursive() : ledges(128), hull(nullptr) {}
...
void IVP_Compact_Recursive::add_compact_ledge_treenode(
...
void IVP_Compact_Recursive::add_compact_surface(
  ...
  for (int k = ledges.len() - 1; k >= 0; k--) {
    ...
    for (int t = ledge->get_n_triangles() - 1; t >= 0; t--) {
      for (int e = 0; e < 3; e++) {
        ...
        IVP_U_Float_Point *p =
            &point_array[edge->get_start_point_index()];
...
void IVP_Compact_Recursive::set_rekursive_convex_hull() {
  ...
  for (int ledge_i = ledges.len() - 1; ledge_i >= 0; ledge_i--) {
    ...
    for (int t = ledge->get_n_triangles() - 1; t >= 0;
         tri = tri->get_next_tri(), t--) {
      ...
      for (int i = 0; i < 3; i++) {
        ...
        if (ind < 0) {
          point_hash.add((const char *)p[i],
                         (void *)static_cast<hk_intp>(n_points_in_hash));
