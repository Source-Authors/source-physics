...
void IVP_Geompack::ptpolg_(int dim, int ldv, int *nv, int inc, int *pgind,
                           double *vcl, double *pt, double *nrml, double *dtol,
                           int *inout) {
  ...
  *inout = -2;
  ...
  if (dim == 2) {
    ...
    dist =
        nr[0] * vcl[lb * vcl_dim1 + 1] + nr[1] * vcl[lb * vcl_dim1 + 2] - nr[3];
  } else {
    dir[2] = (vcl[la * vcl_dim1 + 3] + vcl[lb * vcl_dim1 + 3]) * .5 - pt[3];
    /* Computing 2nd power */
    d__1 = dir[0];
    /* Computing 2nd power */
    d__2 = dir[1];
...
L40:
  ...
  if (s < 0) {
    if (dim == 2) {
      ...
      t = (rhs[0] * da[1] - rhs[1] * da[0]) / (dir[0] * da[1] - dir[1] * da[0]);
    } else {
      da[0] = vcl[la * vcl_dim1 + 1] - vcl[lb * vcl_dim1 + 1];
      da[1] = vcl[la * vcl_dim1 + 2] - vcl[lb * vcl_dim1 + 2];
