  ...
  for (int i = sim_unit_cores.len() - 1; i >= 0; i--) {
    ...
    for (int k = my_core->controllers_of_core.len() - 1; k >= 0; k--) {
      IVP_Controller *my_controller =
          my_core->controllers_of_core.element_at(k);
...
IVP_BOOL IVP_Simulation_Unit::controller_is_known_to_sim_unit(
    IVP_Controller *my_controller) {
  int i;
...
void IVP_Simulation_Unit::add_controlled_core_for_controller(
    IVP_Controller *cntrl, IVP_Core *my_core) {
  int i;
  for (i = controller_cores.len() - 1; i >= 0; i--) {
...
void IVP_Simulation_Unit::add_controller_unit_sim(IVP_Controller *new_cntrl) {
  ...
  IVP_Sim_Unit_Controller_Core_List *core_list =
      new IVP_Sim_Unit_Controller_Core_List();
...
void IVP_Simulation_Unit::split_sim_unit(IVP_Core *split_father) {
  ...
  // faster: operate directly on list, build two new lists -> only linear not
  // quadratic
  int i;
...
#ifdef DEBUG
  IVP_IF(1) { split_new_unit->sim_unit_debug_consistency(); }
#endif
...
IVP_Core *IVP_Simulation_Unit::sim_unit_union_find_test() {
  ...
  IVP_Core *first_father =
      this->sim_unit_cores.element_at(0)->union_find_get_father();
...
void IVP_Simulation_Unit::throw_cores_into_my_sim_unit(
...
void IVP_Simulation_Unit::fusion_simulation_unities(
    IVP_Simulation_Unit *second_unit) {
  this->clean_sim_unit();
...
void IVP_Simulation_Unit::sim_unit_remove_core(IVP_Core *del_core) {
  ...
  for (i = del_core->controllers_of_core.len() - 1; i >= 0; i--) {
    ...
    IVP_ASSERT(pos >= 0);
    IVP_Sim_Unit_Controller_Core_List *c_info =
        controller_cores.element_at(pos);
  ...
  if (sim_unit_cores.len() == 0) {
    del_core->environment->get_sim_units_manager()->rem_sim_unit_from_manager(
        this);
    P_DELETE_THIS(this);
  }
}
...
IVP_Simulation_Unit::~IVP_Simulation_Unit() {
  this->clean_sim_unit();
  ;  // ivp_message("delete_simu %lx\n",(long)this&0x0000ffff);
}

IVP_Simulation_Unit::IVP_Simulation_Unit() {
  // ivp_message("create_simu %lx\n",(long)this&0x0000ffff);
...
void IVP_Simulation_Unit::rem_sim_unit_controller(
    IVP_Controller *rem_controller) {
...
void IVP_Simulation_Unit::sim_unit_debug_out() {
  ...
  for (i = sim_unit_cores.len() - 1; i >= 0; i--) {
    ...
    for (j = my_core->controllers_of_core.len() - 1; j >= 0; j--) {
      ...
      ivp_message("%p ", my_cnt);
    }
    ivp_message("\n");
  }

  for (i = controller_cores.len() - 1; i >= 0; i--) {
    ivp_message("    controlr %p: ",
                controller_cores.element_at(i)->l_controller);
    int j;
    IVP_Sim_Unit_Controller_Core_List *c_info = controller_cores.element_at(i);
    for (j = c_info->cores_controlled_by.len() - 1; j >= 0; j--) {
      ivp_message("%p ", c_info->cores_controlled_by.element_at(j));
    }
    ivp_message("\n");
  }
}
...
void IVP_Simulation_Unit::sim_unit_debug_consistency() {
  ...
  for (i = controller_cores.len() - 1; i >= 0; i--) {
    IVP_Controller *search_controller =
        controller_cores.element_at(i)->l_controller;
    for (j = i - 1; j >= 0; j--) {
      IVP_ASSERT(controller_cores.element_at(j)->l_controller !=
                 search_controller);
    }
  }
  ...
  for (int i1 = sim_unit_cores.len() - 1; i1 >= 0; i1--) {
    ...
    for (int i2 = my_core->controllers_of_core.len() - 1; i2 >= 0; i2--) {
      IVP_Controller *my_controller =
          my_core->controllers_of_core.element_at(i2);
      ...
      IVP_U_Vector<IVP_Core> *core_list =
          my_controller->get_associated_controlled_cores();
...
void IVP_Controller_Manager::remove_controller_from_environment(
    IVP_Controller_Dependent *cntrl, IVP_BOOL silently) {
  IVP_U_Vector<IVP_Core> *controlled_cores =
      cntrl->get_associated_controlled_cores();
...
void IVP_Controller_Manager::ensure_controller_in_simulation(
    IVP_Controller_Dependent *cntrl) {
  IVP_U_Vector<IVP_Core> *controlled_cores =
      cntrl->get_associated_controlled_cores();
  IVP_ASSERT(controlled_cores->len() > 0);
  if (controlled_cores->len() > 0) {  // #+# is this to avoid an assert ???
    controlled_cores->element_at(0)
        ->sim_unit_of_core->sim_unit_ensure_in_simulation();
  }
}

void IVP_Controller_Manager::add_controller_to_core(
    IVP_Controller_Independent *cntrl, IVP_Core *core) {
  core->add_core_controller(cntrl);
}

void IVP_Controller_Manager::remove_controller_from_core(
    IVP_Controller_Independent *cntrl, IVP_Core *core) {
  core->rem_core_controller(cntrl);
}

// only for Controllers that have all cores computed at the same time
void IVP_Controller_Manager::announce_controller_to_environment(
    IVP_Controller_Dependent *cntrl) {
  IVP_U_Vector<IVP_Core> *controlled_cores =
      cntrl->get_associated_controlled_cores();
  ...
  if (mtype < IVP_MT_NOT_SIM) {
    // ensure in simulation is not enough (cannot handle mixture of simulated
    // and not simulated objects)
...
void IVP_Simulation_Unit::add_controller_of_core(IVP_Core *my_core,
...
void IVP_Simulation_Unit::remove_controller_of_core(IVP_Core *my_core,
                                                    IVP_Controller *cntrl) {
  ...
  if (c_info->cores_controlled_by.len() <= 0) {
    P_DELETE(c_info);
    controller_cores.remove_at(
        pos);  // this controller no longer controlls any cores
  }
  // sim_unit_controllers.delete_at( pos );
}
...
void IVP_Simulation_Unit::sim_unit_sort_controllers() {
  ...
  while (1) {
    ...
    IVP_CONTROLLER_PRIORITY second_prio =
        controller_cores.element_at(second)
            ->l_controller->get_controller_priority();
    if (controller_cores.element_at(first)
            ->l_controller->get_controller_priority() > second_prio) {
      sim_unit_exchange_controllers(first, second);
      int test_pos = first;
      while (
          (test_pos > 0) &&
          (controller_cores.element_at(test_pos - 1)
               ->l_controller->get_controller_priority() >
  ...
  IVP_IF(1) {
    for (int i = 0; i < contr_num - 1; i++) {
      IVP_ASSERT(!(controller_cores.element_at(i)
                       ->l_controller->get_controller_priority() >
                   controller_cores.element_at(i + 1)
                       ->l_controller->get_controller_priority()));
    }
  }
}
...
void IVP_Sim_Units_Manager::add_unit_to_slot(IVP_Simulation_Unit *sim_u,
...
void IVP_Sim_Units_Manager::add_sim_unit_to_manager(
...
void IVP_Sim_Units_Manager::rem_unit_from_slot(IVP_Simulation_Unit *sim_u,
                                               IVP_Simulation_Unit **slot) {
...
void IVP_Sim_Units_Manager::rem_sim_unit_from_manager(
    IVP_Simulation_Unit *sim_u) {
  IVP_Simulation_Unit **slot;
  if (sim_u->get_unit_movement_type() < IVP_MT_NOT_SIM) {
    slot = &sim_units_slots[0];
...
// after a fusion is done, I have to restart the reviving of cores
...
void IVP_Standard_Gravity_Controller::set_standard_gravity(
    IVP_U_Point *gravity) {
  grav_vec.set(gravity);
}

void IVP_Standard_Gravity_Controller::do_simulation_controller(
...
IVP_BOOL IVP_Simulation_Unit::sim_unit_calc_movement_state(
    IVP_Environment *env) {
...
void IVP_Simulation_Unit::init_moving_core_for_psi(IVP_Core *core,
                                                   const IVP_Time &c_time) {
  ...
  core->m_world_f_core_last_psi.vv.add_multiple(
      &core->pos_world_f_core_last_psi, &core->delta_world_f_core_psis, d_time);
}

void IVP_Simulation_Unit::simulate_single_sim_unit_psi(
    IVP_Event_Sim *es, IVP_U_Vector<IVP_Core> *touched_cores) {
#ifdef DEBUG
  IVP_IF(1) { this->sim_unit_debug_consistency(); }
#endif
  es->sim_unit = this;
  ...
  };  // warning: 4 Byte of memory are used to store an integer and a float
      // value to get access to sign bit
  ...
  for (int d = sim_unit_cores.len() - 1; d >= 1; d--) {
    ...
    IVP_IF(1) {
      for (int k = my_core->objects.len() - 1; k >= 0; k--) {
        IVP_ASSERT(my_core->objects.element_at(k)->get_movement_state() <=
                   IVP_MT_NOT_SIM);
      }
    }

    my_core->commit_all_async_pushes();  // @@@OS this happens very seldomly
                                         // !!!!, remove !!!!this necessary as
                                         // it may happen that core was
                                         // temporarily_unmovable TL: it is also
                                         // used for delayed pushes and
                                         // async_pushes
    fast_moving_core_p = IVP_OBJECT_MOVING_FAST * IVP_OBJECT_MOVING_FAST -
                         my_core->speed.quad_length();
  ...
  if (fast_moving_flag < 0) {
    ...
    check_movement_state =
        es->environment->must_perform_movement_check();  // do not always make
                                                         // movement check
    //	check_movement_state = IVP_FALSE;   // this might be wrong if flag is
    // set to IVP_MT_SLOW
  } else {
  ...
  for (int j = controller_num - 1; j >= 0; j--) {
    ...
    IVP_Controller *my_controller =
        controller_cores.element_at(j)->l_controller;
    IVP_IF(1) { debug_contr_prio = my_controller->get_controller_priority(); }
    my_controller->do_simulation_controller(
        es, &controller_cores.element_at(j)
                 ->cores_controlled_by);  // speed dependent, real speed
    IVP_IF(1) {
      for (int c = sim_unit_cores.len() - 1; c >= 0; c--) {
  ...
  for (int c = sim_unit_cores.len() - 1; c >= 0; c--) {
    ...
    IVP_IF(0) { core->debug_vec_movement_state(); }
  }
  ...
  for (int k = sim_unit_cores.len() - 1; k >= 0; k--) {
    ...
    for (int c = my_core->objects.len() - 1; c >= 0; c--) {
      ...
      obj->recalc_invalid_mindists_of_object();  // maybe a more lazy approach
...
#define prefetch0_simulate_single_sim_unit_psi(this) \
  IVP_PREFETCH_BLOCK(this, sizeof(*this));

#define prefetch1_simulate_single_sim_unit_psi(this) \
  IVP_IF_PREFETCH_ENABLED(IVP_TRUE) {                \
    IVP_PREFETCH(this->controller_cores.elems, 0);   \
    IVP_PREFETCH(this->sim_unit_cores.elems, 0);     \
  }

// dimhotepus: prefetch only when have controller cores.
#define prefetch2_simulate_single_sim_unit_psi(this)                           \
  IVP_IF_PREFETCH_ENABLED(IVP_TRUE) {                                          \
    if (this->controller_cores.n_elems) {                                      \
      IVP_PREFETCH(                                                            \
          this->controller_cores.element_at(this->controller_cores.len() - 1), \
          0);                                                                  \
      IVP_PREFETCH(this->controller_cores.element_at(0), 0);                   \
    }                                                                          \
    IVP_PREFETCH(                                                              \
        this->sim_unit_cores.element_at(this->sim_unit_cores.len() - 1), 0);   \
    this->prefetch0_init_moving_core_for_psi(                                  \
        this->sim_unit_cores.element_at(this->sim_unit_cores.len() - 1));      \
  }

void IVP_Sim_Units_Manager::simulate_sim_units_psi(
    IVP_Environment *env, IVP_U_Vector<IVP_Core> *touched_cores) {
...
void IVP_Sim_Units_Manager::reset_time(IVP_Time offset) {
  for (IVP_Simulation_Unit *s = this->sim_units_slots[0]; s;
       s = s->next_sim_unit) {
    s->reset_time(offset);
  }
}
