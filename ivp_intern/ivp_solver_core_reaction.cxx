...
void IVP_Solver_Core_Reaction::init_trans_ws(
    const IVP_U_Point *pos_ws, IVP_Core *core,
    IVP_U_Point_4 cross_direction_position_cs[3], IVP_U_Point_4 cr_mult_inv[3],
    IVP_FLOAT sign) {
  ...
  // direction_ws[0]
  {
    ...
    IVP_DOUBLE val =
        m_velocity_ds_f_impulse_ds.get_elem(0, 0) +
        cr_mult_inv[0].dot_product4(cross_direction_position_cs[0]);
    m_velocity_ds_f_impulse_ds.set_elem(0, 0, val);

    IVP_DOUBLE a =
        core->rot_speed.dot_product(&cross_direction_position_cs[0]) +
        core->speed.dot_product(direction_ws[0]);
  ...
  if (direction_ws[1]) {
    ...
    IVP_DOUBLE val =
        m_velocity_ds_f_impulse_ds.get_elem(1, 1) +
        cr_mult_inv[1].dot_product4(cross_direction_position_cs[1]);
    m_velocity_ds_f_impulse_ds.set_elem(1, 1, val);

    val = m_velocity_ds_f_impulse_ds.get_elem(0, 1) +
          cr_mult_inv[1].dot_product(&cross_direction_position_cs[0]);
    m_velocity_ds_f_impulse_ds.set_elem(0, 1, val);

    IVP_DOUBLE a =
        core->rot_speed.dot_product(&cross_direction_position_cs[1]) +
        core->speed.dot_product(direction_ws[1]);
  ...
  if (direction_ws[2]) {
    ...
    IVP_DOUBLE val =
        m_velocity_ds_f_impulse_ds.get_elem(2, 2) +
        cr_mult_inv[2].dot_product4(cross_direction_position_cs[2]);
    m_velocity_ds_f_impulse_ds.set_elem(2, 2, val);

    val = m_velocity_ds_f_impulse_ds.get_elem(0, 2) +
          cr_mult_inv[2].dot_product(&cross_direction_position_cs[0]);
    m_velocity_ds_f_impulse_ds.set_elem(0, 2, val);

    val = m_velocity_ds_f_impulse_ds.get_elem(1, 2) +
          cr_mult_inv[2].dot_product(&cross_direction_position_cs[1]);
    m_velocity_ds_f_impulse_ds.set_elem(1, 2, val);

    IVP_DOUBLE a =
        core->rot_speed.dot_product(&cross_direction_position_cs[2]) +
        core->speed.dot_product(direction_ws[2]);
    delta_velocity_ds.k[2] += a * sign;
  }
}

void IVP_Solver_Core_Reaction::init_rot_ws(
    IVP_Core *core, IVP_U_Point_4 cross_direction_position_cs[3],
  ...
  // direciton_ws[0]
  {
    m_world_f_core->inline_vimult3(direction_ws[0],
                                   &cross_direction_position_cs[0]);
    ...
    IVP_DOUBLE val =
        m_velocity_ds_f_impulse_ds.get_elem(0, 0) +
        cr_mult_inv[0].dot_product(&cross_direction_position_cs[0]);
    m_velocity_ds_f_impulse_ds.set_elem(0, 0, val);
  ...
  if (direction_ws[1]) {
    m_world_f_core->inline_vimult3(direction_ws[1],
                                   &cross_direction_position_cs[1]);
    ...
    IVP_DOUBLE val =
        m_velocity_ds_f_impulse_ds.get_elem(1, 1) +
        cr_mult_inv[1].dot_product(&cross_direction_position_cs[1]);
    m_velocity_ds_f_impulse_ds.set_elem(1, 1, val);
  ...
  if (direction_ws[2]) {
    ...
    IVP_DOUBLE val =
        m_velocity_ds_f_impulse_ds.get_elem(2, 2) +
        cr_mult_inv[2].dot_product(&cross_direction_position_cs[2]);
    m_velocity_ds_f_impulse_ds.set_elem(2, 2, val);
...
void IVP_Solver_Core_Reaction::init_reaction_solver_translation_ws(
    IVP_Core *core_0, IVP_Core *core_1, IVP_U_Point &pos_ws,
    IVP_U_Float_Point
        *direction_0_ws,  // the directions define the d_space (_ds)
    IVP_U_Float_Point *direction_1_ws, IVP_U_Float_Point *direction_2_ws) {
  ...
  if (core_0) {
    init_trans_ws(&pos_ws, core_0, cross_direction_position_cs0, cr_mult_inv0,
                  1.0f);
  }

  if (core_1) {
    init_trans_ws(&pos_ws, core_1, cross_direction_position_cs1, cr_mult_inv1,
                  -1.0f);
  }
}

void IVP_Solver_Core_Reaction::init_reaction_solver_rotation_ws(
    IVP_Core *core_0, IVP_Core *core_1,
    IVP_U_Float_Point
        *direction_0_ws,  // the directions define the d_space (_ds)
    IVP_U_Float_Point *direction_1_ws, IVP_U_Float_Point *direction_2_ws) {
  // if <3, m_velocity_ds_f_impulse_ds is not filled completely
...
void IVP_Solver_Core_Reaction::exert_impulse_dim3(
    IVP_Core *core_0, IVP_Core *core_1, IVP_U_Float_Point &impulse_ds) {
  if (core_0) {
...
void IVP_Solver_Core_Reaction::exert_impulse_dim2(
    IVP_Core *core_0, IVP_Core *core_1, IVP_U_Float_Point &impulse_ds) {
  if (core_0) {
    IVP_DOUBLE inv_m = core_0->get_inv_mass();
    core_0->speed.add_multiple(direction_ws[0], impulse_ds.k[0] * inv_m);
...
void IVP_Solver_Core_Reaction::exert_impulse_dim1(
    IVP_Core *core_0, IVP_Core *core_1, IVP_U_Float_Point &impulse_ds) {
  if (core_0) {
    core_0->speed.add_multiple(direction_ws[0],
                               impulse_ds.k[0] * core_0->get_inv_mass());
    core_0->rot_speed.add_multiple(&cr_mult_inv0[0], impulse_ds.k[0]);
  }

  if (core_1) {
    core_1->speed.add_multiple(direction_ws[0],
                               -impulse_ds.k[0] * core_1->get_inv_mass());
    core_1->rot_speed.add_multiple(&cr_mult_inv1[0], -impulse_ds.k[0]);
  }
}

void IVP_Solver_Core_Reaction::exert_angular_impulse_dim3(
    IVP_Core *core_0, IVP_Core *core_1, IVP_U_Float_Point &impulse_ds) {
  if (core_0) {
    core_0->rot_speed.add_multiple(&cr_mult_inv0[0], impulse_ds.k[0]);
    core_0->rot_speed.add_multiple(&cr_mult_inv0[1], impulse_ds.k[1]);
...
void IVP_Solver_Core_Reaction::exert_angular_impulse_dim2(
    IVP_Core *core_0, IVP_Core *core_1, IVP_U_Float_Point &impulse_ds) {
  if (core_0) {
    core_0->rot_speed.add_multiple(&cr_mult_inv0[0], impulse_ds.k[0]);
    core_0->rot_speed.add_multiple(&cr_mult_inv0[1], impulse_ds.k[1]);
...
void IVP_Solver_Core_Reaction::exert_angular_impulse_dim1(
    IVP_Core *core_0, IVP_Core *core_1, IVP_U_Float_Point &impulse_ds) {
  if (core_0) {
    core_0->rot_speed.add_multiple(&cr_mult_inv0[0], impulse_ds.k[0]);
  }
