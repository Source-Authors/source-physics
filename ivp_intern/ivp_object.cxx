...
void IVP_Real_Object::change_unmovable_flag(IVP_BOOL flag) {
  // ivp_message("changing_unmov_flag to %d at time %f  csimfl %d  osimfl
  // %d\n",flag,get_core()->environment->get_current_time(),this->get_core()->movement_state,this->object_movement_state);

  // friction info is stored differently in movables and unmovables, so destroy
  // first
  ...
  if (my_core->physical_unmoveable) {
    ...
    my_core->core_friction_info.for_moveables.moveable_core_friction_info =
        NULL;
  }
  ...
  if (flag == IVP_TRUE) {
    ...
    // ivp_message("switch_to_unmovable: controllers:
    ...
    if (my_core->sim_unit_of_core) {
      ...
      // clear all controllers. CORRECT would be: give all controllers signal
      // that core switched to unmovable
      int ic;
      for (ic = my_core->controllers_of_core.len() - 1; ic >= 0; ic--) {
        my_core->controllers_of_core.remove_at(ic);
      }

      my_core->environment->get_sim_units_manager()->add_sim_unit_to_manager(
          my_core->sim_unit_of_core);
      my_core->add_core_controller(
          my_core->environment->standard_gravity_controller);
    }
  } else {
...
void IVP_Real_Object::enable_collision_detection(IVP_BOOL enable) {
  if (enable) {
    if (!this->flags.collision_detection_enabled) {
      get_environment()
          ->get_mindist_manager()
          ->enable_collision_detection_for_object(this);
    ...
    if (this->flags.collision_detection_enabled) {
      ...
      unlink_contact_points(
          IVP_TRUE);  // do it silently (do not wake up objects)
    }
  }
}
...
void IVP_Real_Object::set_new_m_object_f_core(
    const IVP_U_Matrix *new_m_object_f_core) {
  IVP_U_Matrix new_m_core_f_object;
  new_m_core_f_object.set_transpose(new_m_object_f_core);
  IVP_Anchor *a;
  ...
  if (m->get_elem(0, 0) == 1.0f && m->get_elem(1, 1) == 1.0f &&
      m->get_elem(2, 2) == 1.0f) {
  ...
  // invalid caches !!
  if (cache_object) {
    get_environment()->get_cache_object_manager()->invalid_cache_object(
        this);  // removes cache_ledge_point too
  }
}

void IVP_Real_Object::set_new_quat_object_f_core(
    const IVP_U_Quat *new_quat_object_f_core,
...
void IVP_Real_Object::remove_listener_collision(
...
// update all mindists between this objects and other objects whose core has
...
void IVP_Real_Object::update_exact_mindist_events_of_object() {
  ...
  for (syn = this->get_first_exact_synapse(); syn; syn = syn_next) {
    ...
    if (core0->mindist_event_already_done !=
        core1->mindist_event_already_done) {  /// #+# OS: vector for recalc
                                              /// mindists
      mindist->recalc_mindist();  // should not be needed as the current
                                  // position has not changed
      if (mindist->recalc_result == IVP_MDRR_OK) {
        mindist->update_exact_mindist_events(
            IVP_FALSE, IVP_EH_BIG_DELAY);  // check length, do not check for
                                           // hull, update time manager ...
      }
    }
  }
}
...
IVP_BOOL IVP_Real_Object::disable_simulation() {
  ...
  if (core->movement_state < IVP_MT_NOT_SIM) {
    for (int c = core->objects.len() - 1; c >= 0; c--) {
      ...
      r_obj->unlink_contact_points(
          IVP_TRUE);  // trick to prevent any friction systems to wake up my
                      // object again
    }
    ...
    IVP_Time ref_time =
        now_time -
        20;  // bad hack: this ensures that 'sim_unit_calc_movement_state'
             // switches off simulation
    core->time_of_calm_reference[0] = ref_time;

    core->sim_unit_of_core->do_sim_unit_union_find();

    return core->sim_unit_of_core->sim_unit_calc_movement_state(
        core->environment);
  }
  return IVP_TRUE;
}
...
// called at AT-Time
// normally wake up at PSI-time not allowed (not allowed in simulate_sim_units
// loop)
void IVP_Real_Object::revive_object_for_simulation() {
  if (this->get_movement_state() ==
      IVP_MT_NOT_SIM) {  // STATICS won't be revived too
                         // IVP_Environment *env=this->get_environment();
    // IVP_ASSERT( env->state == IVP_ES_AT ); //TL: without this assert no
    // controll
    //	this->friction_core->init_core_for_simulation();

    this->friction_core->sim_unit_of_core->sim_unit_revive_for_simulation(
        this->friction_core->environment);
  }
}

IVP_Real_Object::IVP_Real_Object(IVP_Cluster *cluster,
  ...
  anchors = NULL;
  ov_element =
      NULL;  // will be set by IVP_Mindist_Manager::enable_collision_detection
  ((int *)&flags)[0] = 0;
  ...
  strncpy(nocoll_group_ident, templ_obj->get_nocoll_group_ident(),
          IVP_NO_COLL_GROUP_STRING_LEN);
  nocoll_group_ident[IVP_NO_COLL_GROUP_STRING_LEN - 1] = '\0';

  physical_core = new IVP_Core(this, q_world_f_object, position,
                               templ_obj->physical_unmoveable,
                               templ_obj->enable_piling_optimization);
...
void IVP_Real_Object::set_new_surface_manager(IVP_SurfaceManager *new_sm) {
  ...
  switch (type) {
    case IVP_POLYGON: {
      ...
      surface_manager->get_radius_and_radius_dev_to_given_center(&center, &rad,
                                                                 &rad_dev);
    } break;
    case IVP_BALL:
      rad = 0.0f;
...
void IVP_Real_Object::set_extra_radius(IVP_DOUBLE new_radius) {
  if (new_radius > this->extra_radius) {
    ...
    this->extra_radius = new_radius;
    // dimhotepus: Cache core for performance.
...
IVP_Real_Object::~IVP_Real_Object() {
  ...
  if (this->flags.collision_listener_exists) {  // local friction listener
    environment->get_cluster_manager()->fire_event_collision_object_deleted(
        this);
  }

  if (this->flags.object_listener_exists) {  // fire object-dependant 'object
                                             // deleted' event
    IVP_Event_Object event_deleted;
    event_deleted.real_object = this;
    event_deleted.environment = environment;
    environment->get_cluster_manager()->fire_event_object_deleted(
        &event_deleted);
  }
...
void IVP_Real_Object::get_all_near_mindists() {
  IVP_Movement_Type temp_movement =
      (IVP_Movement_Type)physical_core->movement_state;
...
IVP_Object::IVP_Object(IVP_Environment *env) { this->init(env); }
...
void IVP_Real_Object::get_m_world_f_object_AT(
    IVP_U_Matrix *m_world_f_object_out) const {
  calc_at_matrix(get_environment()->get_current_time(), m_world_f_object_out);
};

void IVP_Real_Object::get_quat_world_f_object_AT(
    IVP_U_Quat *quat_world_f_object,
    IVP_U_Point *position) const  // ditto (using quaternions)
{
  calc_at_quaternion(get_environment()->get_current_time(), quat_world_f_object,
                     position);
}

IVP_Cluster::IVP_Cluster(IVP_Cluster *father, IVP_Template_Cluster *templ)
...
void IVP_Real_Object::calc_at_matrix(IVP_Time current_time,
                                     IVP_U_Matrix *m_world_f_object_out) const {
  ...
  if (!flags.shift_core_f_object_is_zero) {
    m_world_f_object_out->vmult4(&this->shift_core_f_object,
                                 &m_world_f_object_out->vv);
  }

  if (this->q_core_f_object) {
    q_world_f_object.inline_set_mult_quat(&q_world_f_object,
                                          this->q_core_f_object);
    q_world_f_object.set_matrix(m_world_f_object_out);
  }
}
...
void IVP_Real_Object::calc_at_quaternion(IVP_Time current_time,
  ...
  if (this->q_core_f_object) {
    q_world_f_object_out->inline_set_mult_quat(q_world_f_object_out,
                                               this->q_core_f_object);
  }
}

void IVP_Real_Object::init_object_core(IVP_Environment *i_environment,
  ...
  IVP_U_Float_Hesse &rot_inertia =
      (IVP_U_Float_Hesse &)*core->get_rot_inertia();
  ...
  if (templ->auto_check_rot_inertia != 0.0f) {
    IVP_FLOAT min_inertia_value =
        rot_inertia.real_length() * templ->auto_check_rot_inertia;
    int hh;
    for (hh = 0; hh < 3; hh++) {
  ...
  IVP_Event_Sim es(i_environment, i_environment->get_next_PSI_time() -
                                      i_environment->get_current_time());
  core->calc_next_PSI_matrix_zero_speed(&es);
}

void IVP_Real_Object::unlink_contact_points_for_object(
...
int IVP_Real_Object::
    get_collision_check_reference_count() {  // see
...
void IVP_Real_Object::get_geom_center_world_space(
    IVP_U_Point *geom_center_out) const {
...
void IVP_Real_Object::get_geom_center_speed_vec(
    IVP_U_Point *speed_ws_out) const {
...
IVP_BOOL IVP_Controller_VHash::compare(const void *elem0,
...
void IVP_Real_Object::do_radar_checking(IVP_Radar *radar) {
  ...
  while ((supers = (IVP_Listener_Hull *)mindists.get_next_element_lt(
              max_range)) != NULL) {
...
void IVP_Real_Object::beam_object_to_new_position(
    const IVP_U_Quat *rotation_world_f_object,
    const IVP_U_Point *position_w_f_o, IVP_BOOL optimize_for_repeated_calls) {
  IVP_Core *core = this->get_core();
  ...
  if (this->q_core_f_object) {
    rotation_world_f_core.set_div_unit_quat(rotation_world_f_object,
                                            this->q_core_f_object);
  }

  IVP_Calc_Next_PSI_Solver nps(core);
  nps.set_transformation(&rotation_world_f_core, &position_w_f_c,
                         optimize_for_repeated_calls);
}

void IVP_Real_Object::async_add_speed_object_ws(
...
void IVP_Real_Object::async_add_rot_speed_object_cs(
...
void IVP_Real_Object::async_push_object_ws(
    const IVP_U_Point *position_ws_, const IVP_U_Float_Point *impulse_ws_) {
  this->ensure_in_simulation();
  IVP_Core *core = this->get_core();
