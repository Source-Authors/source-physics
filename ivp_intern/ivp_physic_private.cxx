...
void IVP_Listener_Collision::event_pre_collision(IVP_Event_Collision *) {
}  // the user app sould override this
void IVP_Listener_Collision::event_post_collision(IVP_Event_Collision *) {
}  // the user app sould override this
void IVP_Listener_Collision::event_collision_object_deleted(IVP_Real_Object *) {
}  // only object private callbacks are called

// friction
// set the IVP_LISTENER_COLLISION_CALLBACK_FRICTION bit in the constructor if
// you want to use this
void IVP_Listener_Collision::event_friction_created(IVP_Event_Friction *) {
}  // the user app sould override this
void IVP_Listener_Collision::event_friction_deleted(IVP_Event_Friction *) {
}  // the user app sould override this

IVP_Cluster *IVP_Cluster_Manager::get_root_cluster() { return root_cluster; }
...
void IVP_Cluster_Manager::fire_event_object_deleted(
    IVP_Event_Object *event_obj) {
...
void IVP_Cluster_Manager::fire_event_object_frozen(
    IVP_Event_Object *event_obj) {
  IVP_Object_Callback_Table *obj_table;
  IVP_Real_Object *ro = event_obj->real_object;
  obj_table = this->obj_callback_hash->find_table(ro);
...
void IVP_Cluster_Manager::fire_event_object_created(
    IVP_Event_Object *event_obj) {
  IVP_Object_Callback_Table *obj_table;
  IVP_Real_Object *ro = event_obj->real_object;
  obj_table = this->obj_callback_hash->find_table(ro);
...
void IVP_Cluster_Manager::fire_event_object_revived(
    IVP_Event_Object *event_obj) {
  IVP_Object_Callback_Table *obj_table;
  IVP_Real_Object *ro = event_obj->real_object;
  obj_table = this->obj_callback_hash->find_table(ro);
...
void IVP_Cluster_Manager::fire_event_pre_collision(
    IVP_Real_Object *real_object, IVP_Event_Collision *event_obj) {
  ...
  if (obj_table != NULL) {
    ...
    for (i = obj_table->listeners.len() - 1; i >= 0; i--) {
      ...
      if (!(lis->get_enabled_callbacks() &
            IVP_LISTENER_COLLISION_CALLBACK_PRE_COLLISION))
        continue;

      lis->event_pre_collision(event_obj);
      if (i > 0 && !collision_callback_hash->find_table(real_object))
        break;  // object deleted
    }
  }
}

void IVP_Cluster_Manager::fire_event_post_collision(
    IVP_Real_Object *real_object, IVP_Event_Collision *event_obj) {
  IVP_Collision_Callback_Table *obj_table;
  obj_table = this->collision_callback_hash->find_table(real_object);
  if (obj_table != NULL) {
...
void IVP_Cluster_Manager::fire_event_collision_object_deleted(
...
void IVP_Cluster_Manager::fire_event_friction_created(
    IVP_Real_Object *real_object, IVP_Event_Friction *event_friction) {
  IVP_Collision_Callback_Table *obj_table;
  obj_table = this->collision_callback_hash->find_table(real_object);
  if (obj_table != NULL) {
    int i;
...
void IVP_Cluster_Manager::fire_event_friction_deleted(
    IVP_Real_Object *real_object, IVP_Event_Friction *event_friction) {
  IVP_Collision_Callback_Table *obj_table;
  obj_table = this->collision_callback_hash->find_table(real_object);
  if (obj_table != NULL) {
...
IVP_Real_Object *IVP_Cluster_Manager::get_next_real_object_in_cluster_tree(
    IVP_Object *object) {
  ...
  if (object == NULL) {
    ...
    next_object = object;  // no object supplied: we get the root cluster and we
                           // want the first object in root cluster!
    if (!object) return NULL;
  } else {
    next_object = object->next_in_cluster;  // object supplied: just get the
                                            // next object in cluster
  }
  ...
  for (next = next_object; !next; next = object->next_in_cluster) {
    ...
    if (!object) {
      IVP_Cluster *root = get_root_cluster();
      if (root == old_father)
        return (NULL); /* necessary to avoid endless loop caused by the last
                        *object (ball's father is cluster; cluster has no
                        *father, therefore we would "get_root_cluster()" which
                        *is the same as the old cluster and whose first object
                        *is the "static ball"
...
void IVP_Cluster_Manager::check_for_unused_objects(IVP_Universe_Manager *um) {
  ...
  if (number_of_real_objects < ums->num_objects_in_environment_threshold_0)
    return;
  {
    int i = int(ums->check_objects_per_second_threshold_0 *
                environment->get_delta_PSI_time());
    for (; i >= 0; i--) {
      IVP_Real_Object *an_object = this->an_object_to_be_checked;
      this->an_object_to_be_checked =
          get_next_real_object_in_cluster_tree(this->an_object_to_be_checked);
  ...
  if (number_of_real_objects < ums->num_objects_in_environment_threshold_1)
    return;
  {
    int i = int(ums->check_objects_per_second_threshold_1 *
                environment->get_delta_PSI_time());
    for (; i >= 0; i--) {
      IVP_Real_Object *an_object = this->an_object_to_be_checked;
      if (!an_object) return;  // no objects available
...
IVP_Object_Callback_Table::~IVP_Object_Callback_Table() { return; }

IVP_Collision_Callback_Table::~IVP_Collision_Callback_Table() { return; }
...
void IVP_Cluster_Manager::add_listener_object(IVP_Real_Object *real_object,
...
void IVP_Cluster_Manager::add_listener_collision(
    IVP_Real_Object *real_object, IVP_Listener_Collision *listener) {
  IVP_Collision_Callback_Table *obj_table;
  obj_table = this->collision_callback_hash->find_table(real_object);
  if (obj_table != NULL) {
    obj_table->listeners.add(listener);
...
void IVP_Cluster_Manager::remove_listener_object(
    IVP_Real_Object *real_object, IVP_Listener_Object *listener) {
  IVP_Object_Callback_Table *obj_table;
  obj_table = this->obj_callback_hash->find_table(real_object);
  if (obj_table != NULL) {
...
void IVP_Cluster_Manager::remove_listener_collision(
    IVP_Real_Object *real_object, IVP_Listener_Collision *listener) {
  IVP_Collision_Callback_Table *obj_table;
  obj_table = this->collision_callback_hash->find_table(real_object);
  if (obj_table != NULL) {
...
void IVP_Cluster_Manager::remove_object(IVP_Real_Object *real_object) {
  ...
  if (real_object == an_object_to_be_checked) {
    an_object_to_be_checked =
        get_next_real_object_in_cluster_tree(an_object_to_be_checked);
    if (real_object == an_object_to_be_checked) {
...
// returns IVP_TRUE when a friction system was grown -> needed for sim_unit
// revival algorithm
IVP_BOOL IVP_Core::revive_simulation_core() {
  ...
  for (int c = core->objects.len() - 1; c >= 0; c--) {
    ...
    IVP_IF(1) { IVP_ASSERT(r_obj->get_movement_state() >= IVP_MT_NOT_SIM); }
    r_obj->set_movement_state(IVP_MT_MOVING);
  }

  core->init_core_for_simulation();
  IVP_Event_Sim es(environment, environment->get_next_PSI_time() -
  ...
  // *my_info=core->moveable_core_has_friction_info(); if(my_info==NULL) {
  //  grow a friction system (search for near objects and put them into one
  //  friction system )
  ...
  for (int d = core->objects.len() - 1; d >= 0; d--) {
    ...
    {  // fire object-dependant 'object revived' event
      ...
      core->environment->get_cluster_manager()->fire_event_object_revived(
          &event_revived);
    }
...
void IVP_Core::fire_event_object_frozen() {
  ...
  for (int i = this->objects.len() - 1; i >= 0; i--) {
    ...
    {  // fire object-dependant 'object frozen' event
      ...
      this->environment->get_cluster_manager()->fire_event_object_frozen(
          &event_frozen);
    }
...
void IVP_Core::freeze_simulation_core() {
  ...
  IVP_IF(IVP_DEBUG_OBJECT0) {
    ...
    if (!P_String::string_cmp(name0, search0, IVP_FALSE)) {
      if (r_core->environment->get_current_time().get_time() >=
          IVP_DEBUG_TIME) {
...
void IVP_Core::debug_out_movement_vars() {
  ivp_message("core_status %zi  trans %f %f %f  rot %f %f %f\n",
              (hk_intp)this & 0x0000ffff, speed.k[0], speed.k[1], speed.k[2],
              rot_speed.k[0], rot_speed.k[1], rot_speed.k[2]);
}
...
void IVP_Core::debug_vec_movement_state() {
  IVP_IF(0) {
    ...
    out_text = p_make_string(
        "oob%zi_sp%.3f", (hk_intp)one_object & 0x0000ffff,
        one_object->speed
            .real_length());  //,one_object->get_energy_on_test(&one_object->speed,&one_object->rot_speed));
    ...
    this->environment->add_draw_vector(&ivp_start, &ivp_pointer, out_text,
                                       v_color);
    P_FREE(out_text);
  }
}
...
// object is removed from simulation if not moving for 1 sec or slowling moving
// for 10 sec exception: objects belonging to friction systems friction systems
// are tested separately, all objects of fr_system are removed at a time
...
IVP_Friction_System::IVP_Friction_System(IVP_Environment *env) {
  ...
  // ivp_message("creating_new_fs %lx at time
...
IVP_Friction_System::~IVP_Friction_System() {
  // ivp_message("deleting_of_fs %lx at time
  ...
  // deleteing of real friction systems filled with information is not yet
  // implemented (not trivial : unlink whole friction infos)
...
IVP_BOOL IVP_Friction_System::dist_removed_update_pair_info(
  ...
  IVP_Friction_Core_Pair *my_pair_info =
      this->get_pair_info_for_objs(core0, core1);
...
void IVP_Friction_System::remove_dist_from_system(IVP_Contact_Point *old_dist) {
  IVP_IFDEBUG(
      l_environment->get_debug_manager()->check_fs,
      fprintf(l_environment->get_debug_manager()->out_deb_file,
              "rem_dist_from_fs %f %p from %p\n",
...
void IVP_Friction_System::dist_added_update_pair_info(
...
void IVP_Friction_System::add_dist_to_system(IVP_Contact_Point *new_dist) {
  ...
  // new_dist->number_in_friction = this->friction_dist_number; //dists are
  // numbered the wrong way: last has number 0
  IVP_IF(l_environment->debug_information->debug_mindist) {
    ivp_message("added_dist %d\n", (int)friction_dist_number);
  }
  friction_dist_number++;

  // fr_solver.calc_calc_solver(this); //solver matrix must be greater and
  // temporary results also
}
...
void IVP_Friction_System::add_core_to_system(IVP_Core *new_obj) {
  ...
  if (!new_obj->physical_unmoveable) {
    ...
    // new_obj->sim_unit_of_core->add_controller_of_core( new_obj,
    // &this->static_fs_handle );
    // new_obj->sim_unit_of_core->add_controller_of_core( new_obj, this );
    // new_obj->sim_unit_of_core->add_controller_of_core( new_obj,
    // &this->energy_fs_handle );
  }
...
void IVP_Friction_System::remove_core_from_system(IVP_Core *old_obj) {
  IVP_IFDEBUG(
      l_environment->get_debug_manager()->check_fs,
      fprintf(l_environment->get_debug_manager()->out_deb_file,
              "remove_core_from_fs %f %p from %p\n",
...
void IVP_Environment::add_revive_core(IVP_Core *c) {
  ...
  IVP_IF(1) { IVP_ASSERT(this->core_revive_list.index_of(c) < 0); }
...
void IVP_Universe_Manager::ensure_objects_in_environment(
    IVP_Real_Object * /*object*/, IVP_U_Float_Point * /*sphere_center*/,
...
IVP_Universe_Manager::provide_universe_settings() {
  return NULL;
}
