...
// different simulation rates: recalculate rate after fusion (like in
// try_to_generate_managed_friction)
...
class IVP_Simulation_Unit {
 ...
 public:
  ...
  IVP_BOOL
  sim_unit_has_fast_objects : 2;  // fast moving objects: some optimizations
                                  // like: no energy controll ...
  IVP_BOOL sim_unit_just_slowed_down : 2;  // changing from fast moving to
                                           // slowly moving
  ...
  IVP_BOOL sim_unit_calc_movement_state(
      IVP_Environment *
          env);  // return TRUE when Simulation_Unit was removed from Simulation
  void sim_unit_clear_movement_check_values();
  void sim_unit_revive_for_simulation(
      IVP_Environment
          *env);  // can handle mixture of simulated objs and not simulated objs
  inline void sim_unit_ensure_in_simulation();  // can not ...
  void sim_unit_ensure_cores_movement();        // reset_freeze_check_values
  IVP_Core *
  sim_unit_union_find_test();  // problem: quadratic search, because every
                               // controller gives list of associated cores (but
                               // this list depends on core)
  void sim_unit_calc_redundants();
  IVP_BOOL controller_is_known_to_sim_unit(IVP_Controller *my_controller);
  void add_controlled_core_for_controller(IVP_Controller *cntrl,
                                          IVP_Core *my_core);
  ...
  IVP_Movement_Type get_unit_movement_type() {
    return (IVP_Movement_Type)sim_unit_movement_type;
  };
  void set_unit_movement_type(IVP_Movement_Type mm_type) {
    sim_unit_movement_type = mm_type;
  };
  ...
  static inline void init_moving_core_for_psi(IVP_Core *core,
  ...
  void simulate_single_sim_unit_psi(class IVP_Event_Sim *es,
                                    IVP_U_Vector<IVP_Core> *touched_cores_out);

//    inline void prefetch0_simulate_single_sim_unit_psi();   // first level
//    prefetch inline void prefetch1_simulate_single_sim_unit_psi();   // second
//    level prefetch inline void prefetch2_simulate_single_sim_unit_psi();   //
...
class IVP_Sim_Units_Manager {
 ...
 public:
  ...
  void rem_unit_from_slot(IVP_Simulation_Unit *sim_u,
                          IVP_Simulation_Unit **slot);

  void simulate_sim_units_psi(IVP_Environment *env,
                              IVP_U_Vector<IVP_Core> *touched_cores_out);
...
void IVP_Simulation_Unit::sim_unit_ensure_in_simulation() {
  if (sim_unit_movement_type >= IVP_MT_NOT_SIM) {
    ...
    this->sim_unit_revive_for_simulation(
...
inline void IVP_Simulation_Unit::prefetch0_init_moving_core_for_psi(
    IVP_Core *core) {
  IVP_IF_PREFETCH_ENABLED(IVP_TRUE) {
    IVP_PREFETCH_BLOCK(
        (char *)core + sizeof(IVP_Core_Fast_Static),
        sizeof(IVP_Core_Fast_PSI) - sizeof(IVP_Core_Fast_Static));
  }
}
