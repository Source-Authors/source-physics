...
IVP_BOOL IVP_Hull_Manager::are_events_in_hull() {
  if (sorted_synapses.find_min_value() - hull_value_next_psi <
      0.0f) {  // no other synapses need to be checked
    return IVP_TRUE;
  } else {
    return IVP_FALSE;
  }
...
void IVP_Hull_Manager::check_hull_synapses(IVP_Environment *pEnv) {
  ...
  while ((syn_val = sorted_synapses.find_min_value() - hull_value_next_psi) <
         0.0f) {  // no other synapses need to be checked
    // syn hull event is to be fired
    IVP_Listener_Hull *syn =
        (IVP_Listener_Hull *)
            sorted_synapses.find_min_elem();  // corresponding synapse
    syn->hull_limit_exceeded_event(
        this,
        syn_val);  // ATT: syn MUST update/remove itself/its peer from min_hash!
    if (maxcnt-- < 0) break;
  }
}

void IVP_Hull_Manager::increase_hull_by_x(IVP_Time t_now, IVP_FLOAT delta_time,
                                          IVP_FLOAT gradient_,
