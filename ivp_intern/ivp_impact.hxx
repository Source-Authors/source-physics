...
class IVP_Impact_System {
  ...
  void impact_system_check_start_pair(IVP_Friction_Core_Pair *start_pair,
  ...
  IVP_BOOL pair_is_already_in_system(
      IVP_Friction_Core_Pair
          *test_pair);  // place info in pair to avoid linear search
  void add_pushed_core_with_pairs_except(IVP_Core *new_core,
 ...
 public:
  ...
  void init_and_solve_impact_system(IVP_Mindist *mindist,
                                    IVP_Friction_System *fs_system,
                                    IVP_Friction_Core_Pair *start_pair,
                                    IVP_Contact_Point *start_mdf);
};
...
void IVP_Impact_System::add_pushed_core_to_impact_system(IVP_Core *new_core) {
  ...
  IVP_ASSERT(new_core->tmp_null.old_sync_info->was_pushed_during_i_s ==
             IVP_FALSE);
  new_core->tmp_null.old_sync_info->was_pushed_during_i_s = IVP_TRUE;
}

IVP_BOOL IVP_Impact_System::core_was_already_pushed_in_system(
    IVP_Core *test_core) {
  return test_core->tmp_null.old_sync_info->was_pushed_during_i_s;
}

IVP_BOOL IVP_Impact_System::core_is_already_known_to_system(
    IVP_Core *test_core) {
...
class IVP_Impact_Solver {
  ...
  int turnaround_next_time;  // lowest energy point is traversed (but because of
                             // calculation faults objects may still be
                             // closing!)
  // IVP_DOUBLE percent_deformation; //for each impact step the percent energy
  // that is reversible deformation
  IVP_DOUBLE
  energy_deformation;  // whole reversible deformation energy stored so far
  IVP_U_Matrix *m_world_f_core[2];
  IVP_U_Float_Point
      rot_speed[2];  // rotation speed (changes with several impacts)
  IVP_U_Float_Point
      trans_speed[2];  // center speed (changes with several impacts)
  IVP_U_Float_Point rot_speed_change[2];  // change within one push
  IVP_U_Float_Point trans_speed_change[2];
  IVP_U_Float_Point
      relative_world_speed;  // seen from object 0 (that is virtually still)
  IVP_U_Float_Point world_push_direction;   // normized relative_world_speed and
                                            // changed according to friction
                                            // (which makes vector smaller)
  ...
  void calc_virt_masses_impact_solver(
  ...
  void get_cos_sin_for_impact(IVP_FLOAT friction_val,
 ...
 public:
  ...
  // IVP_U_Point world_direction_first_friction; //normized, orthogonal to
  // surface normal
  IVP_U_Float_Point
      world_direction_second_friction;  // normized, orthogonal to
                                        // first_friction and surface_normal
  ...
  IVP_FLOAT cos_friction;  // angle how far friction is valid seen form surface
                           // normal 90 degree
  ...
  void do_impact(
      IVP_Core *pushed_cores[2], IVP_BOOL allow_delaying,
      int pushes_while_system,
      IVP_FLOAT rescue_speed_val);  // comunicates what cores where pushed
