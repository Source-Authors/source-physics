...
void IVP_Contact_Point::p_calc_friction_qr_PF(
    const IVP_U_Point *pp, const IVP_Compact_Edge *F,
    IVP_Cache_Ledge_Point *m_cache_F, IVP_Impact_Solver_Long_Term *info,
  ...
  IVP_U_Point pp_Fos;
  m_cache_F->get_object_cache()->transform_position_to_object_coords(pp,
                                                                     &pp_Fos);
  ...
  if (cp_status == IVP_CPBS_NEEDS_RECHECK) {  // do checking
    ...
    IVP_CLS.calc_unscaled_qr_vals_F_space(m_cache_F->get_compact_ledge(), F,
                                          &pp_Fos, &qr);
    if (qr.is_outside()) {
  ...
  {  // calc surface normal
    ...
    IVP_CLS.calc_hesse_vec_object_not_normized(
        F, m_cache_F->get_compact_ledge(), &wHesse_vecF_Fos);
    ...
    const IVP_U_Float_Point *F_1_Fos =
        IVP_CLS.give_object_coords(F->get_next(), m_cache_F);
    last_gap_len = wHesse_vecF_Fos.dot_product(&pp_Fos) -
    ...
    m_cache_F->get_object_cache()->transform_vector_to_world_coords(
        &qvec_Fos, &info->span_friction_v[0]);
  }

#endif
#ifdef IVP_USE_S_VALS_FOR_PRETENSION
  IVP_U_Point tp;
  IVP_CLS.give_world_coords_AT(F, m_cache_F, &tp);
  IVP_U_Point tp_next;
  IVP_CLS.give_world_coords_AT(F->get_next(), m_cache_F, &tp_next);
  IVP_U_Point tp_prev;
  IVP_CLS.give_world_coords_AT(F->get_prev(), m_cache_F, &tp_prev);
  ...
  info->surf_normal.calc_cross_product(
      &Q, &R);  // @@@OS optimization: store surf_normal in CS coords, or at
                // least 1.0f/sqrt(len)
  ...
  info->gap_distance =
      info->surf_normal.dot_product(&tp) - info->surf_normal.dot_product(pp);
  ...
  IVP_IF(1) {
    ...
    if (test_val < 0.0f) {
      ivp_message(
          "recalc_friction_s_point_surface wrong surf_normal %f should be >0\n",
          test_val);
    }
  }
...
void IVP_Contact_Point::p_calc_friction_s_PP(
    const IVP_U_Point *pp0, const IVP_U_Point *pp1,
    IVP_Impact_Solver_Long_Term *info, IVP_U_Float_Point *diff_contact_vec) {
  IVP_USE(diff_contact_vec);
  ...
  info->span_friction_v[0].inline_calc_cross_product_and_normize(
      &info->surf_normal, &magic_world_random_vec);
...
void IVP_Contact_Point::p_calc_friction_s_PK(
    const IVP_U_Point *pp, const IVP_Compact_Edge *K,
    IVP_Cache_Ledge_Point *m_cache_K, IVP_Impact_Solver_Long_Term *info,
    IVP_U_Float_Point *diff_contact_vec) {
  // if status indicates friction mode, point is always kept the same
  IVP_USE(diff_contact_vec);
  IVP_U_Point tp;
  IVP_CLS.give_world_coords_AT(K, m_cache_K, &tp);
  IVP_U_Point tp_next;
  IVP_CLS.give_world_coords_AT(K->get_next(), m_cache_K, &tp_next);
...
void IVP_Contact_Point::p_calc_friction_ss_KK(
  ...
  // very bad code down there #+# (this is a copy paste from an ugly old OS
  // version
  IVP_U_Point Kp;
  IVP_CLS.give_world_coords_AT(K, m_cache_K, &Kp);
  IVP_U_Point Kp_next;
  IVP_CLS.give_world_coords_AT(K->get_next(), m_cache_K, &Kp_next);
  IVP_U_Point Lp;
  IVP_CLS.give_world_coords_AT(L, m_cache_L, &Lp);
  IVP_U_Point Lp_next;
  IVP_CLS.give_world_coords_AT(L->get_next(), m_cache_L, &Lp_next);
  ...
  if (quad_dist > quad_res) {  // not parallel
    ...
    // TL -- modify if necessary  #+#  very bad code down there, vec0 won't be
    // normized soon
    ...
    if (length > P_DOUBLE_EPS) {
      ...
      last_gap_len = 0.0f;
      info->surf_normal.set_multiple(
          &norm, 1.0f / IVP_Inline_Math::ivp_sqrtf(quad_dist));
    }
...
void IVP_Contact_Point::recalc_friction_s_vals() {
  ...
  IVP_Impact_Solver_Long_Term *info =
      (IVP_Impact_Solver_Long_Term *)env->get_sim_unit_mem()
          ->get_mem_transaction(sizeof(IVP_Impact_Solver_Long_Term));
  ...
  // set: 	this->info.gap_distance, info->surf_normal,
  // info->contact_points_ws[0],
  //		this->span_friction_v[0]
  //          [and add to diff_vec_contact]
  {
    IVP_U_Float_Point diff_vec_contact;
    diff_vec_contact.set_to_zero();
    ...
    switch (stat0) {
      case IVP_ST_BALL: {
        point0_position_ws.set(
            m_cache_0.get_object_cache()->m_world_f_object.get_position());
      ...
      ball_same_as_point:
        switch (stat1) {
          case IVP_ST_BALL: {
            IVP_U_Point *tp2 =
                m_cache_1.get_object_cache()->m_world_f_object.get_position();
            p_calc_friction_s_PP(&point0_position_ws, tp2, info,
                                 &diff_vec_contact);
            break;
          }
          ...
          case IVP_ST_POINT: {
            IVP_U_Point tp2;
            IVP_CLS.give_world_coords_AT(e1, &m_cache_1, &tp2);
            p_calc_friction_s_PP(&point0_position_ws, &tp2, info,
                                 &diff_vec_contact);
            break;
          }
          case IVP_ST_EDGE: {
            p_calc_friction_s_PK(&point0_position_ws, e1, &m_cache_1, info,
                                 &diff_vec_contact);
            break;
          }
          case IVP_ST_TRIANGLE: {
            p_calc_friction_qr_PF(&point0_position_ws, e1, &m_cache_1, info,
                                  &diff_vec_contact);
            break;
          }
          default:
      ...
      case IVP_ST_EDGE: {
        ...
        p_calc_friction_ss_KK(e0, e1, &m_cache_0, &m_cache_1, info,
                              &diff_vec_contact);
        break;
      };
      default:
  ...
  info->span_friction_v[1].inline_calc_cross_product(&info->surf_normal,
                                                     &info->span_friction_v[0]);
  ...
  {  // calculate some values plus pretension for core 0
    ...
    if (!core0->physical_unmoveable) {      // && !core0->pinned){ //@@CBPIN
      ...
      m_world_f_core->inline_vimult4(&info->contact_point_ws,
                                     &info->contact_point_cs[0]);
      m_world_f_core->inline_vimult3(&info->surf_normal, &surface_normal_cs);

      info->contact_cross_nomal_cs[0].inline_calc_cross_product(
          &info->contact_point_cs[0], &surface_normal_cs);
      core0->get_surface_speed_on_test(&info->contact_point_cs[0],
                                       &core0->speed, &core0->rot_speed,
                                       &delta_velocity_ws);

      IVP_U_Float_Point hp;
      hp.set_pairwise_mult(&info->contact_cross_nomal_cs[0],
                           core0->get_inv_rot_inertia());
      info->inv_virtual_mass =
          info->contact_cross_nomal_cs[0].dot_product(&hp) +
          core0->get_inv_mass();
    } else {
      core0 = NULL;
  ...
  {  // core 1
    ...
    if (!core1->physical_unmoveable) {      // && !core1->pinned){ //@@CBPIN
      ...
      m_world_f_core->inline_vimult4(&info->contact_point_ws,
                                     &info->contact_point_cs[1]);
      m_world_f_core->inline_vimult3(&info->surf_normal, &surface_normal_cs);

      info->contact_cross_nomal_cs[1].inline_calc_cross_product(
          &info->contact_point_cs[1], &surface_normal_cs);

      IVP_U_Float_Point dh;
      core1->get_surface_speed_on_test(&info->contact_point_cs[1],
                                       &core1->speed, &core1->rot_speed, &dh);
      delta_velocity_ws.subtract(&dh);

      IVP_U_Float_Point hp;
      hp.set_pairwise_mult(&info->contact_cross_nomal_cs[1],
                           core1->get_inv_rot_inertia());
      info->inv_virtual_mass +=
          info->contact_cross_nomal_cs[1].dot_product(&hp) +
          core1->get_inv_mass();
    } else {
  ...
  info->virtual_mass = 1.0f / info->inv_virtual_mass;
  {
    ...
    this->span_friction_s[0] -=
        delta_velocity_ws.dot_product(&info->span_friction_v[0]) * dt;
    this->span_friction_s[1] -=
        delta_velocity_ws.dot_product(&info->span_friction_v[1]) * dt;
...
IVP_Contact_Point *IVP_Friction_Manager::get_associated_contact_point(
    IVP_Mindist *mindist) {
  // does a contact_point with same objs and position_status already exist?
  {
    ...
    for (syn0 = obj0->get_first_friction_synapse(); syn0;
         syn0 = syn0->get_next()) {
      IVP_Contact_Point *md = syn0->get_contact_point();
      if (md->get_synapse(0)->get_object() == obj1 ||
          md->get_synapse(1)->get_object() == obj1) {
...
void IVP_Friction_Manager::delete_all_contact_points_of_object(
...
// generates a friction mindist when no one exists. Is there already a friction
// dist, the old one is returned
IVP_Contact_Point *IVP_Friction_Manager::generate_contact_point(
    IVP_Mindist *mindist, IVP_BOOL *successful) {
  //
  // Valid call with non-exact and Ipion forces a write to null to crash in its
  // IVP_ASSERT!
  //
