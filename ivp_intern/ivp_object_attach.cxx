...
void IVP_Object_Attach::attach_object(
    IVP_Real_Object *parent, IVP_Real_Object *attached_object,
  ...
  if (max_distance_attached_object_to_parent < 0.0f) {
    ...
    attached_object->get_core()->inline_calc_at_position(time,
                                                         &attached_mass_center);

    max_distance_attached_object_to_parent = IVP_Inline_Math::sqrtd(
        parents_mass_center.quad_distance_to(&attached_mass_center));
  }
  ...
  // update radius
  IVP_DOUBLE radius =
      max_distance_attached_object_to_parent + old_core->upper_limit_radius;
  if (radius > ncore->upper_limit_radius) {
    ncore->upper_limit_radius = radius;
  }

  IVP_DOUBLE max_surface_deviation =
      max_distance_attached_object_to_parent + old_core->max_surface_deviation;
  ...
  // position object
  reposition_object_Ros(NULL, attached_object, &q_world_f_object,
                        &shift_world_f_object, IVP_FALSE);
...
void IVP_Object_Attach::detach_object(IVP_Real_Object *attached_object,
  ...
  IVP_DOUBLE moved_distance_val =
      attached_object->get_shift_core_f_object()->real_length();

  // reset object values
  if (t->get_nocoll_group_ident()[0]) {
    strncpy(attached_object->nocoll_group_ident, t->get_nocoll_group_ident(),
            IVP_NO_COLL_GROUP_STRING_LEN);
    attached_object->nocoll_group_ident[IVP_NO_COLL_GROUP_STRING_LEN - 1] =
        '\0';
  }
  {  // get rid of old core
  ...
  {  // build new core
    ...
    attached_object->physical_core = new IVP_Core(
        attached_object, &q_world_f_object, &position_ws,
        IVP_FALSE /*physical_unmoveable*/, t->enable_piling_optimization);
  ...
  if (old_core && IVP_MTIS_SIMULATED(old_core->movement_state)) {
    attached_object->get_core()
        ->fire_event_object_frozen();  // tell them that we went to sleep
    ...
    IVP_DOUBLE delta_time_till_next_PSI =
        env->get_next_PSI_time() - env->get_current_time();
    ...
    if (!my_core->physical_unmoveable) {
      ...
      IVP_Calc_Next_PSI_Solver::commit_all_hull_managers(env,
 ...
 *	Parameter:	q_Ros_f_Aos is a quat that transforms from Attached
 *object space into Reference object space == matrix defining the attached
 *object in Ros space Parameter:      check_before_moving if set movement will
 *be limited if there might be penetration problems
 ********************************************************************************/
IVP_RETURN_TYPE IVP_Object_Attach::reposition_object_Ros(
    IVP_Real_Object *parent, IVP_Real_Object *attached_object,
    const IVP_U_Quat *q_Ros_f_Aos, const IVP_U_Point *shift_Ros_f_Aos,
  ...
  {  // get current m_world_f_core
    IVP_Time current_time =
        attached_object->get_environment()->get_current_time();
    IVP_U_Quat q_world_f_core;
    attached_object->get_core()->inline_calc_at_quaternion(current_time,
                                                           &q_world_f_core);
    q_world_f_core.set_matrix(&m_world_f_core);
    attached_object->get_core()->inline_calc_at_position(current_time,
                                                         &m_world_f_core.vv);
  }

  IVP_U_Matrix m_Aos_f_Rcs;
  m_ws_f_Aos.mimult4(&m_world_f_core, &m_Aos_f_Rcs);
