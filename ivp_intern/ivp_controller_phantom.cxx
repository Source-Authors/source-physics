...
void IVP_Controller_Phantom::add_listener_phantom(
    IVP_Listener_Phantom *listener) {
  listeners.add(listener);
}

void IVP_Controller_Phantom::remove_listener_phantom(
    IVP_Listener_Phantom *listener) {
...
void IVP_Controller_Phantom::wake_all_sleeping_objects() {
  ...
  while (IVP_Core *core =
             all_sleeping_cores.get_next_element(set_of_sleeping_cores)) {
...
void IVP_Controller_Phantom::mindist_entered_volume(
    class IVP_Mindist *mindist) {
...
IVP_Controller_Phantom::IVP_Controller_Phantom(
    IVP_Real_Object *object_in, const IVP_Template_Phantom *templat)
  ...
  if (templat->dont_check_for_unmoveables) {
    ...
    if (object->get_movement_state() == IVP_MT_STATIC) {
      ...
      object->get_environment()->get_mindist_manager()->recheck_ov_element(
          object);
    }
  }
  ...
  // loop through all hull mindists and mark them as phantom mindists
  {
    ...
    while ((supers = (IVP_Listener_Hull *)mindists.get_next_element()) !=
           NULL) {
