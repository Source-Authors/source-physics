...
//     Vector of all objects (IVP_Real_Object) belonging to fr sys, objects are
//     counted: friction_obj_number
//         every object has Vector of IVP_Friction_Info_For_Obj for every
//         friction system it belongs to (one object can have more then one fr
//         sys,but now used only for fixed objects)
//             in one IVP_Friction_Info_For_Obj is Vector of IVP_Contact_Point
//             and a backlink to friction system
//     Vector IVP_Friction_Core_Pair for every pair of objects that share
//     mindists (for easing friction forces and speedup of union find)
//         every pair has backlinks to objects and Vector of IVP_Contact_Point
//         that have that object pair as anchors
//     double linked list of all IVP_Contact_Point belonging to fr sys, dists
//     are counted: friction_dist_number
//         IVP_Contact_Point has backlink to both objects (via snapses ...)
//         IVP_Contact_Point are numbered (but counted oppositely the way they
//         occur in the list)
//     function test_hole_fr_system_data checks consistency of most of this
//     infos
 ...
 public:
  ...
  union {
    struct {  // impact data
      ...
      IVP_FLOAT
      percent_energy_conservation;  // #+# maybe move to IVP_Contact_Point,
                                    // maybe short 1.0f subtract energy
                                    // destroyed
    } impact;
    // next block is used for friction and set by
    // IVP_Friction_Solver::setup_coords_mindists
    struct {  // friction data
      IVP_Friction_Info_For_Core
          *friction_infos[2];  // two friction infos for one distance, to get
                               // all distances of a object  x2
      int has_negative_pull_since;  // copy of contact point to avoid cache
                                    // misses
  ...
  // vector of v[0]*s[0] + v[1]*s[1] is real world force vector of force that
  // has to be done on first obj (is pointing to second obj)

  IVP_U_Float_Point
      contact_point_cs[2];  // impact/friction position in core coordinates
  IVP_U_Float_Point
      contact_cross_nomal_cs[2];  // crossproduct of contact_point_cs and
                                  // normal_cs
  ...
  void do_impact_long_term(
      IVP_Core *pushed_cores[2], IVP_FLOAT rescue_speed_val,
      IVP_Contact_Point *cp);  // returns 0 if O.K. , returns 1 if false impact
  static IVP_Core *find_second_critical_impact_core(IVP_Core *core1,
                                                    IVP_Core *core0);

  // the real start of an impact
  static void do_impact_of_two_objects(IVP_Mindist *mindist,
                                       IVP_Real_Object *obj0,
                                       IVP_Real_Object *obj1);
  static IVP_Core *get_best_merge_core(IVP_Core *core0, IVP_Core *core1,
                                       IVP_Core *core2);
};
...
class IVP_Synapse_Friction {  // sizeof() == 32
 ...
 public:
  ...
  IVP_Contact_Point *get_contact_point() const {
    return (IVP_Contact_Point *)(contact_point_offset + (char *)this);
  }
  void set_contact_point(IVP_Contact_Point *cp) {
    contact_point_offset = ((char *)cp) - (char *)this;
  }
...
//     Real friction force done on objects horizontal: (now_friction_pressure *
//     inv_max_len * friction_factor)  *  real_fr_now_len    .SI:  t*m / s*s
//     Corresponding D of horizontal spring: now_friction_pressure * inv_max_len
//     * friction_factor     .SI:  t / s*s Energy stored in horizontal spring:
//     0.5f * D * real_fr_now_len*real_fr_now_len   . SI:  t*m*m / s*s
//  Pot Energy won by complex: now_friction_pressure * delta Distance.    SI:
//  t*m*m / s*s
...
class IVP_Contact_Point_Fast_Static {
 protected:
  IVP_Contact_Point
      *next_dist_in_friction;  // all friction springs in a friction system
...
class IVP_Contact_Point_Fast : public IVP_Contact_Point_Fast_Static {
 protected:
  ...
  IVP_FLOAT
  integrated_destroyed_energy;  // for watching a contact point over time,
                                // e.g. to drive the sound engine
  IVP_FLOAT inv_triangle_det;   // to optimize pF cases
  // for energy difference calculations
  IVP_FLOAT old_energy_dynamic_fr;
  IVP_FLOAT now_friction_pressure;  // needed to add values  distance_keeper
                                    // pressure + static friction
  ...
  int has_negative_pull_since;  // 0 means inactive, n>1 means negative since
                                // n-1 PSIs, -n>0 means has positive value since
                                // n PSIs
  ...
  IVP_U_Float_Point
      surf_normal_ws;  // the normized normal of the contact surface (world
                       // space), pointing to second obj
};
...
class IVP_Contact_Point : public IVP_Contact_Point_Fast {
  ...
  // impact system
  void get_cos_sin_for_impact(IVP_FLOAT friction_val, IVP_FLOAT *cos_val,
                              IVP_FLOAT *sin_val);
  void read_materials_for_contact_situation(
      IVP_Impact_Solver_Long_Term
          *info);  // #+# check calls to this function when material is changed
                   // on the fly -> recalc some values

  void calc_coll_distance();  // returns the distance reached in one PSI and the
                              // rescue speed val and stores result in
                              // impact.distance_reached_in_time
  ...
  void ease_the_friction_force(
      IVP_U_Float_Point
          *diff_vec);  // #+#, call only randomly, create flag for sliding
  ...
  bool friction_force_local_constraint_2d_wheel(
      IVP_Core *core_a, IVP_Impact_Solver_Long_Term *info,
      const IVP_Event_Sim *es, IVP_FLOAT &flEnergy);
  void static_friction_single(const IVP_Event_Sim *es, IVP_FLOAT dg,
                              IVP_FLOAT speedup_factor);
  IVP_BOOL get_world_direction_second_friction(
      IVP_U_Float_Point *ret_second_friction_vec,
      IVP_FLOAT *sin_second_friction);
  IVP_DOUBLE two_values_friction(IVP_U_Float_Point *real_world_friction_vec);
  IVP_DOUBLE get_and_set_real_friction_len(
  ...
  void p_calc_friction_s_PP(const IVP_U_Point *pp0, const IVP_U_Point *pp1,
                            IVP_Impact_Solver_Long_Term *info,
                            IVP_U_Float_Point *diff_contact_vec);
  void p_calc_friction_qr_PF(const IVP_U_Point *pp, const IVP_Compact_Edge *F,
                             IVP_Cache_Ledge_Point *m_cache_F,
                             IVP_Impact_Solver_Long_Term *info,
                             IVP_U_Float_Point *diff_contact_vec);
  void p_calc_friction_ss_KK(const IVP_Compact_Edge *K,
                             const IVP_Compact_Edge *L,
                             IVP_Cache_Ledge_Point *m_cache_K,
                             IVP_Cache_Ledge_Point *m_cache_L,
                             IVP_Impact_Solver_Long_Term *info,
                             IVP_U_Float_Point *diff_contact_vec);
  void p_calc_friction_s_PK(const IVP_U_Point *pp, const IVP_Compact_Edge *K,
                            IVP_Cache_Ledge_Point *m_cache_K,
                            IVP_Impact_Solver_Long_Term *info,
                            IVP_U_Float_Point *diff_contact_vec);

 public:
  const IVP_U_Float_Point *get_contact_point_ws() const {
    return &last_contact_point_ws;
  }
...
class IVP_Friction_Sys_Energy : public IVP_Controller_Independent {
 public:
  ...
  void do_simulation_controller(IVP_Event_Sim *,
                                IVP_U_Vector<IVP_Core> *core_list) override;
  ...
  virtual const char *get_controller_name() override {
    return "ivp:friction_sys_energy";
  };
};
...
class IVP_Friction_Sys_Static : public IVP_Controller_Independent {
 ...
 public:
  ...
  virtual const char *get_controller_name() override {
    return "ivp:friction_sys_static";
  };
};

// a physical unmovable object can have more than one friction system and for
// every friction systems several distances
class IVP_Friction_Info_For_Core {
 public:
...
class IVP_Friction_System : public IVP_Controller_Dependent {
 ...
 private:
  ...
  short complex_not_necessary_number;  // the first of this number mindists are
                                       // not recognized in complex
 public:
  // some flags/constants
  IVP_BOOL union_find_necessary : 8;  // flag set when mindist is removed and a
                                      // split into two systems may be possible
  ...
  IVP_Friction_Core_Pair *get_pair_info_for_objs(IVP_Core *core0,
                                                 IVP_Core *core1);
  ...
  IVP_Contact_Point *get_next_friction_dist(IVP_Contact_Point *old) {
    return old->next_dist_in_friction;
  }
  IVP_Contact_Point *get_prev_friction_dist(IVP_Contact_Point *old) {
    return old->prev_dist_in_friction;
  }
  ...
  void delete_friction_distance(
      IVP_Contact_Point *old_dist);  // has a call to remove_dist_from_system
  IVP_BOOL dist_removed_update_pair_info(
      IVP_Contact_Point
          *old_dist);  // returns IVP_TRUE if union find has to be called
  ...
  void apply_real_friction(
      const IVP_Event_Sim
          *es);  // does real friction according to pressure calculated by
                 // 'do_friction_system' in last PSI
  ...
  virtual const char *get_controller_name() override {
    return "ivp:friction_system";
  };
};

class IVP_Mutual_Energizer {
  IVP_U_Float_Point trans_vec_world;  // direction of push done to core0, to
                                      // core1 negative push is given
  IVP_U_Float_Point rot_vec_obj[2];   // direction of pushes done to core0 and
                                      // core1 in obj coords
  ...
  IVP_DOUBLE rot_inertia[2];      // resistence against rot pushes; resistance
                                  // against trans pushes are masses
  IVP_DOUBLE inv_rot_inertia[2];  // inverses

  IVP_DOUBLE rot_energy_potential;
  IVP_DOUBLE trans_energy_potential;

  static inline void get_rot_inertia(IVP_Core *core,
                                     IVP_U_Float_Point *rot_vec_obj,
                                     IVP_DOUBLE *rot_inertia,
                                     IVP_DOUBLE *inv_rot_inertia);
  IVP_DOUBLE calc_impulse_to_reduce_energy_level(IVP_DOUBLE speed_pot,
 ...
 public:
  ...
  static IVP_DOUBLE calc_energy_potential(IVP_DOUBLE speed_pot,
                                          IVP_DOUBLE mass0, IVP_DOUBLE mass1,
                                          IVP_DOUBLE inv_mass0,
                                          IVP_DOUBLE inv_mass1);
...
class IVP_Friction_Manager {
 public:
  static IVP_Contact_Point *generate_contact_point(IVP_Mindist *orig_mindist,
                                                   IVP_BOOL *was_success);
