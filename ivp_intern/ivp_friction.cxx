...
IVP_Contact_Point::IVP_Contact_Point(IVP_Mindist *md) {
  ...
  syn0->get_object()->get_surface_manager()->add_reference_to_ledge(
  ...
  synapse[1].init_synapse_friction(this, syn1->get_object(), syn1->edge,
                                   syn1->get_status());
  // dimhotepus: Add reference to used ledge.  See ~IVP_Contact_Point();
  syn1->get_object()->get_surface_manager()->add_reference_to_ledge(
  ...
  IVP_IF(1) {
    IVP_IFDEBUG(
        ...
        fprintf(env->get_debug_manager()->out_deb_file,
                "create_mindist %f %p cores %p %p\n",
                env->get_current_time().get_time(), this,
                syn0->l_obj->physical_core, syn0->l_obj->physical_core);)
  }
  ...
  this->last_gap_len =
      ivp_mindist_settings
          .friction_dist;  // at first PSI-iteration there is no energy from
                           // complex //maybe drop it
...
bool IVP_Contact_Point::friction_force_local_constraint_2d_wheel(
  ...
  if (span_v_0.quad_length() < 0.001f) return false;
  ...
  IVP_RETURN_TYPE ret = IVP_Inline_Math::invert_2x2_matrix(
      tpm00, tpm01, tpm10, tpm11, &inv_mat2x2[0], &inv_mat2x2[1],
...
IVP_FLOAT IVP_Contact_Point::friction_force_local_constraint_2d(
    const IVP_Event_Sim *es) {
  IVP_Impact_Solver_Long_Term *info = this->tmp_contact_info;
  IVP_DOUBLE maximum_impulse_force =
      this->now_friction_pressure * this->real_friction_factor * es->delta_time;
  if (maximum_impulse_force < P_FLOAT_RES) return 0.0f;
...
IVP_FLOAT IVP_Contact_Point::get_possible_friction_slide_way(
    const IVP_Event_Sim *) {
    ...
    if ((fr_dist->now_friction_pressure > P_DOUBLE_EPS) || 1) {
      for (int j = i + 1; j < total_n; j++) {
        ...
        if ((fr_dist2->now_friction_pressure > P_DOUBLE_EPS) || 1) {
          ...
          // ivp_message("dirconform %f\n",dir_conform);
          if ((dir_conform <
               1E-3f))  // due to random stray in distances the vectors between
                        // point-surface and edge-edge are not really parallel
          {
            // ivp_message("eaase ");
...
IVP_FLOAT IVP_Contact_Point_API::get_vert_force(
    IVP_Contact_Point *friction_handle) {
  return friction_handle->now_friction_pressure;
};

void IVP_Contact_Point_API::get_surface_normal_ws(
    const IVP_Contact_Point *friction_handle, IVP_U_Float_Point *normal) {
...
void IVP_Friction_System::delete_friction_distance(
  ...
  // dimhotepus: Done in old_dist destructor via
  // remove_friction_synapse_from_object
  // for(int i=0;i<0;i++) {
  ...
  // ivp_message("deleting_frdist %lx o %lx c %lx  o %lx c
...
void IVP_Friction_System::fusion_friction_systems(
  ...
  IVP_IF(1) {
    ...
    IVP_IFDEBUG(
        env->get_debug_manager()->check_fs,
        ivp_message("fusion_fs %f %p %p  cores ",
                    env->get_current_time().get_time(), first_sys, second_sys);
        for (int k = first_sys->cores_of_friction_system.len() - 1; k >= 0;
             k--) {
        ...
        for (int l = second_sys->cores_of_friction_system.len() - 1; l >= 0;
             l--) {
          IVP_Core *my_core =
              second_sys->cores_of_friction_system.element_at(l);
          ivp_message("%p ", my_core);
        } ivp_message("\n");)
  }
...
IVP_Friction_Core_Pair *IVP_Friction_System::get_pair_info_for_objs(
...
IVP_Core *IVP_Friction_System::union_find_fr_sys() {
  ...
  IVP_IF(0) {
    for (int k = fr_sys->cores_of_friction_system.len() - 1; k >= 0; k--) {
      ...
      ivp_message("uff of %zi : %zi\n", (hk_intp)objj & 0x0000ffff,
                  (hk_intp)of & 0x0000ffff);
    }
  }
...
void IVP_Friction_System::split_friction_system(IVP_Core *split_father) {
  ...
  IVP_IFDEBUG(
      fr_sys->l_environment->get_debug_manager()->check_fs,
      ivp_message("split_fs %f %p %p  cores ",
                  fr_sys->l_environment->get_current_time().get_time(), fr_sys,
                  new_fr_sys);
...

  {
    ...
    for (int n = fr_sys->fr_pairs_of_objs.len() - 1; n >= 0; n--) {
      ...
      // ivp_message("reassignpair %lx infoc
      ...
      if (robj->union_find_get_father() == split_father) {
        ...
        for (int k = fr_pair->fr_dists.len() - 1; k >= 0; k--) {
          ...
          if (!found_mine) {
            IVP_IF(1) {
              ivp_message("removing_dist that doesnt belong to sys\n");
            }
            CORE;
          }
          fr_sys->remove_dist_from_system(fr_dist);
          new_fr_sys->add_dist_to_system(fr_dist);
          // ivp_message("add_to_new %lx fs_num_now
    ...
    for (int l = fr_sys->cores_of_friction_system.len() - 1; l >= 0; l--) {
      ...
      if (obj->physical_unmoveable) {
        ...
        if (fr_i_new->dist_number() == 0) {
          // ivp_message("uf_rem %lx from new
        ...
        if (fr_i_old->dist_number() == 0) {
          // ivp_message("uf_rem %lx from old
...
void IVP_Friction_System::print_all_dists() {
  IVP_IF(1) {
    ...
    for (int i = fr_pairs_of_objs.len() - 1; i >= 0; i--) {
      ...
      ivp_message("p %zi %zi  ", (hk_intp)fr_pair->objs[0] & 0x0000ffff,
                  (hk_intp)fr_pair->objs[1] & 0x0000ffff);
...
IVP_Friction_Core_Pair *IVP_Friction_System::find_pair_of_cores(
    IVP_Core *core0, IVP_Core *core1) {
  for (int m = fr_pairs_of_objs.len() - 1; m >= 0; m--) {
...
IVP_BOOL IVP_Synapse_Friction::is_same_as(const IVP_Synapse_Real *syn1) const {
  ...
  switch (syn_stat) {
    ...
    case IVP_ST_EDGE: {
      ...
      if (syn0->edge == syn1->edge ||
          syn0->edge->get_opposite() == syn1->edge) {
        return IVP_TRUE;
      } else {
        return IVP_FALSE;
      }
