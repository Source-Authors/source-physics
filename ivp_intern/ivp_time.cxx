...
void IVP_Event_Manager_Standard::simulate_time_events(IVP_Time_Manager *tman,
                                                      IVP_Environment *env,
                                                      IVP_Time time) {
  IVP_FLOAT event_time;
  while ((event_time = tman->min_hash->find_min_value()) <
         time - tman->base_time) {
...
void IVP_Event_Manager_D::simulate_time_events(IVP_Time_Manager *tman,
                                               IVP_Environment *env,
                                               IVP_Time time) {
  IVP_FLOAT event_time;
  event_time = tman->min_hash->find_min_value();
  if (!hk_Math::almost_zero(time - tman->base_time)) {
    // while ( (event_time = tman->min_hash->find_min_value()) < time -
    // tman->base_time ){
    IVP_Time_Event *event = (IVP_Time_Event *)tman->min_hash->find_min_elem();
    tman->min_hash->remove_minlist_elem(event->index);
    event->index = IVP_U_MINLIST_UNUSED;
...
void IVP_Event_Manager::simulate_variable_time_step(
    IVP_Time_Manager *tman, IVP_Environment *env, IVP_Time_Event_PSI *psi_event,
    IVP_FLOAT delta) {
...
void IVP_Time_Manager::simulate_variable_time_step(IVP_Environment *env,
                                                   IVP_FLOAT delta_time) {
  /************************************************
   * FPU mode
   ************************************************/
#if defined(WIN32) && defined(_M_IX86)
...
void IVP_Time_Manager::env_set_current_time(IVP_Environment *env,
                                            IVP_Time time) {
