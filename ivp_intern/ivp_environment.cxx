...
IVP_Freeze_Manager::IVP_Freeze_Manager() { init_freeze_manager(); }

void IVP_Freeze_Manager::init_freeze_manager() { freeze_check_dtime = 0.3f; }

IVP_Environment::IVP_Environment(IVP_Environment_Manager *manager,
...

  {
    ...
    short_term_mem->init_mem_transaction_usage(appl_env->scratchpad_addr,
                                               ssizeh);
    sim_unit_mem->init_mem_transaction_usage(ssizeh + appl_env->scratchpad_addr,
                                             ssizeh);
  }
  ...
  integrated_energy_damp =
      IVP_Inline_Math::ivp_expf(IVP_FLOAT(log(0.9f)) * get_delta_PSI_time());
...
void IVP_Environment::fire_object_is_removed_from_collision_detection(
    IVP_Real_Object *obj) {
  for (int k = collision_delegator_roots.len() - 1; k >= 0; k--) {
    IVP_Collision_Delegator_Root *cdr = collision_delegator_roots.element_at(k);
...
IVP_Actuator_Rot_Mot *IVP_Environment::create_rotmot(
  ...
  if (templ->active_float_max_torque ||
      templ->active_float_max_rotation_speed || templ->active_float_power) {
    return new IVP_Actuator_Rot_Mot_Active(this, templ);
  }
  return new IVP_Actuator_Rot_Mot(this, templ);
}

IVP_Actuator_Torque *IVP_Environment::create_torque(
...
IVP_Controller_Motion *IVP_Environment::create_controller_motion(
    IVP_Real_Object *obj, const IVP_Template_Controller_Motion *templ) {
...
IVP_Actuator_Spring *IVP_Environment::create_spring(
    IVP_Template_Spring *templ) {
  if (templ->active_float_spring_len || templ->active_float_spring_constant ||
...
IVP_Actuator_Suspension *IVP_Environment::create_suspension(
    IVP_Template_Suspension *templ) {
  return new IVP_Actuator_Suspension(this, templ);
}

IVP_Actuator_Stabilizer *IVP_Environment::create_stabilizer(
    IVP_Template_Stabilizer *templ) {
  return new IVP_Actuator_Stabilizer(this, templ);
}

IVP_Actuator_Check_Dist *IVP_Environment::create_check_dist(
    IVP_Template_Check_Dist *templ) {
...
IVP_Environment *IVP_Environment_Manager::create_environment(
    IVP_Application_Environment *appl_env, const char *costumer_name,
    unsigned int auth_code) {
#ifdef IVP_ENCRYPT_EXISTS
  IVP_UINT32 crypt_val =
      IVP_Encrypt::encrypt_strings(costumer_name, IVP_IPION_AUTH_CHECK);
  if (auth_code != crypt_val) {
    return NULL;
  }
#endif
  IVP_Environment *new_en =
      new IVP_Environment(this, appl_env, costumer_name, auth_code);
  return new_en;
}
...
void IVP_Environment::simulate_until(IVP_Time until_time) {
  this->get_betterstatisticsmanager()->set_simulation_time(
      until_time.get_time());
  time_manager->event_loop(this, until_time);
  // time_manager->simulate_variable_time_step(this,until_time -
  // get_current_time());
}

void IVP_Environment::simulate_variable_time_step(IVP_FLOAT delta_time) {
  this->get_betterstatisticsmanager()->set_simulation_time(
      get_current_time().get_time() + delta_time);
...
IVP_Debug_Manager::~IVP_Debug_Manager() { clear_debug_manager(); }
...
IVP_Debug_Manager::IVP_Debug_Manager() { init_debug_manager(); }
...
IVP_Polygon *IVP_Environment::create_polygon(
    IVP_SurfaceManager *vic, const IVP_Template_Real_Object *templ,
    IVP_U_Quat const *rotation, const IVP_U_Point *position) {
  return new IVP_Polygon(get_root_cluster(), vic, templ, rotation, position);
}

IVP_Ball *IVP_Environment::create_ball(const IVP_Template_Ball *templ_ball,
                                       const IVP_Template_Real_Object *templ,
                                       IVP_U_Quat const *rotation,
                                       const IVP_U_Point *position) {
  return new IVP_Ball(get_root_cluster(), templ_ball, templ, rotation,
                      position);
}

void IVP_Environment::add_listener_object_private(IVP_Real_Object *ro,
                                                  IVP_Listener_Object *lo) {
  this->get_cluster_manager()->add_listener_object(ro, lo);
}

void IVP_Environment::remove_listener_object_private(IVP_Real_Object *ro,
                                                     IVP_Listener_Object *lo) {
  this->get_cluster_manager()->remove_listener_object(ro, lo);
}

void IVP_Environment::add_listener_collision_private(
    IVP_Real_Object *ro, IVP_Listener_Collision *lo) {
  this->get_cluster_manager()->add_listener_collision(ro, lo);
}

void IVP_Environment::remove_listener_collision_private(
    IVP_Real_Object *ro, IVP_Listener_Collision *lo) {
  this->get_cluster_manager()->remove_listener_collision(ro, lo);
}

void IVP_Environment::add_listener_object_global(
    IVP_Listener_Object *listener) {
  this->global_object_listeners.add(listener);
}

void IVP_Environment::install_listener_object_global(
    IVP_Listener_Object *listener) {
  this->global_object_listeners.install(listener);
}

void IVP_Environment::remove_listener_object_global(
    IVP_Listener_Object *listener) {
...
void IVP_Environment::fire_event_friction_pair_created(
    class IVP_Friction_Core_Pair *pair) {
  int i;
  for (i = collision_listeners.len() - 1; i >= 0; i--) {
    IVP_Listener_Collision *lis = collision_listeners.element_at(i);
    if (!(lis->get_enabled_callbacks() &
...
void IVP_Environment::fire_event_friction_pair_deleted(
    class IVP_Friction_Core_Pair *pair) {
  int i;
  for (i = collision_listeners.len() - 1; i >= 0; i--) {
    IVP_Listener_Collision *lis = collision_listeners.element_at(i);
...
void IVP_Environment::fire_event_constraint_broken(
...
void IVP_Environment::add_listener_collision_global(
    IVP_Listener_Collision *listener) {
  collision_listeners.add(listener);
}

void IVP_Environment::remove_listener_collision_global(
    IVP_Listener_Collision *listener) {
...
// Constraint listener
void IVP_Environment::add_listener_constraint_global(
    IVP_Listener_Constraint *listener) {
  constraint_listeners.add(listener);
}
void IVP_Environment::remove_listener_constraint_global(
    IVP_Listener_Constraint *listener) {
  constraint_listeners.remove(listener);
}

void IVP_Environment::merge_objects(
    IVP_U_Vector<IVP_Real_Object> *objs_to_merge) {
...
void IVP_Time_Event_D::simulate_time_event(IVP_Environment *env) {
  ...
  int i;
  for (i = 0; i <= nr; i++) {
    IVP_Time_Event_D *delay_event =
        new IVP_Time_Event_D(env->get_current_time());
...
void IVP_Time_Event_N::simulate_time_event(IVP_Environment *env) {
  ...
  // dimhotepus: Setup manager before assignment, not after the one.
  auto *manager = new IVP_Event_Manager_Standard();
  manager->mode = 0;
...
void IVP_Environment::do_d_events() {
  ...
  for (int i = 0; i < 20; i++) {
    IVP_Time_Event_D *delay_event =
        new IVP_Time_Event_D(this->get_current_time());
    delay_event->number_of_sons = 7;
    this->get_time_manager()->insert_event(delay_event,
...
IVP_Time_Event_N::IVP_Time_Event_N(IVP_Time time) { IVP_USE(time); }

void IVP_Environment::add_draw_vector(const IVP_U_Point *start_p,
                                      const IVP_U_Float_Point *dir_vec,
                                      const char *debug_text_in, int v_color) {
  IVP_IF(1) {
...
void IVP_Environment::simulate_psi(IVP_Time /*psi_time*/) {
  IVP_IF(1 || this->debug_information->display_statistic) {
    if (get_current_time() - get_statistic_manager()->last_statistic_output >=
        1.0f) {
      get_statistic_manager()->output_statistic();
      get_statistic_manager()->last_statistic_output = get_current_time();
    }
  }
  IVP_DEBUGCODE(get_debug_manager()->psi_counter += 1.0f;
  ...
  l_active_value_manager->refresh_psi_active_values(
      this);         // lots of side effects, bombs ....
  ...
  IVP_Calc_Next_PSI_Solver::commit_all_calc_next_PSI_matrix(
      this, &touched_cores, &active_hulls);
...
#if (defined(WIN32) || defined(PSXII)) &&                  \
    defined(IVP_VERSION_EVAL) /* blocking only if original \
                                 ivp_authenticity.hxx is used */
  {
...
void IVP_Environment::set_global_collision_tolerance(
    IVP_DOUBLE tolerance, [[maybe_unused]] IVP_DOUBLE gravity_length) {
  ivp_mindist_settings.set_collision_tolerance(tolerance);
  // CRACK - lol. according to both IDA and Ghidra, nothing is done with
  // gravity_length
}
