...
class IVP_Controller_VHash : protected IVP_VHash {
 ...
 public:
  ...
  IVP_Controller *remove_controller(IVP_Controller *controller) {
    return (IVP_Controller *)remove_elem(controller,
                                         controller_to_index(controller));
  };

  IVP_Controller *find_controller(IVP_Controller *controller) {
    return (IVP_Controller *)find_elem(controller,
                                       controller_to_index(controller));
  };

  ~IVP_Controller_VHash();
  IVP_Controller_VHash(IVP_Real_Object *object, int init_size)
      : IVP_VHash(init_size) {
    real_object = object;
  };
};
