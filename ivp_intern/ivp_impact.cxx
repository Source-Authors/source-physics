...
#define IVP_ROT_MAX_UNCERTAINTY \
  0.5f  // max rot degree speed uncertainty for rescue_speed
#define IVP_COLL_DETECT_MIN_INV_TIME \
  200.0f  // inverse time we have to make sure no collision occurs
#define IVP_COLL_DETECT_MIN_TIME 1.0f / IVP_COLL_DETECT_MIN_INV_TIME
#define IVP_SPEED_ADDON_SYSTEM_IMPACT 0.01f
#define IVP_INV_HALF_CONSERVATION_STEPS \
  0.5f  // elasticity is raised with number of impacts in system,
        // at 1.0f/IVP_INV_HALF_CONSERVATION_STEPS the elasticity is at least
        // 0.5
...
// #define EASEONIMPACT //doesnt work with balls // #+# kill and remove
// dependent functions
...
IVP_Contact_Point *IVP_Mindist::try_to_generate_managed_friction(
    IVP_Friction_System **associated_fs, IVP_BOOL *having_new,
    IVP_Simulation_Unit *sim_unit_not_destroy, IVP_BOOL call_recalc_svals) {
  ...
  friction_dist =
      IVP_Friction_Manager::generate_contact_point(my_dist, &gen_success);
  {
    if (gen_success != IVP_TRUE) {
      ...
      *having_new = IVP_FALSE;
      if (call_recalc_svals) {
        ...
        friction_dist->read_materials_for_contact_situation(
            friction_dist->tmp_contact_info);
      }
  ...
  if (call_recalc_svals) {
    friction_dist
        ->recalc_friction_s_vals();  // this is overhead, because nearly the
                                     // same function was called for the
                                     // associated mindist
    friction_dist->read_materials_for_contact_situation(
        friction_dist->tmp_contact_info);
  }

  {
    ...
    event_friction.contact_situation =
        friction_dist->tmp_contact_info;  // is inherited
  ...
  *having_new = IVP_TRUE;
  ...
  if (fr_info0) {
    ...
    if (fr_info1) {
      ...
      if (core1->physical_unmoveable) {
        // obj1 may have other friction systems. Doesnt matter systems act
        // independently.
        goto add_second_to_first;
      } else {
        fr_info1 =
            core1->moveable_core_has_friction_info();  // obj1 is movable and so
                                                       // has only one fr_sys
        if (fr_info1) {
  ...
  // ivp_message("add_fri_dist %lx to sys
  ...
  *associated_fs = affected_fs;
  ...
  IVP_IF(0) {
    ...
    if (!core1->physical_unmoveable) {
      IVP_Friction_Info_For_Core *info_for_core =
          core1->moveable_core_has_friction_info();
      if (info_for_core) {
        fs1 = info_for_core->l_friction_system;
      }
    }
    ivp_message(
        "\nmaking_new_frdist for cores %p %d  %p %d  fss %p %p  with "
        "sim_units: %p %p\n",
        core0, core0->physical_unmoveable, core1, core1->physical_unmoveable,
...
void IVP_Impact_Solver::get_relative_speed_vector() {
  ...
  core[0]->get_surface_speed_on_test(obj_point[0], &trans_speed[0],
                                     &rot_speed[0], &world_speed0);
  core[1]->get_surface_speed_on_test(obj_point[1], &trans_speed[1],
                                     &rot_speed[1], &world_speed1);
...
void IVP_Impact_Solver::get_world_push_direction() {
  ...
  // world_push_direction differs from relative_world_speed if allowed friction
  // is exceeded. then length is also smaller than 1.0f

  IVP_U_Float_Point
      part_in_direction_surf;  // decomposition of velo vec in direction normal
                               // and rest (this is the rest)

  world_push_direction.set(&relative_world_speed);
  world_push_direction.fast_normize();

  IVP_DOUBLE angle = world_push_direction.dot_product(
      surf_normal);  // is negativ. the greater (absolut) the steeper is angle
  // angle is cos alpha . alpha seen from surface normal

  IVP_IF(core[0]->environment->debug_information->debug_impact) {
    ivp_message("wpd %.2f %.2f %.2f  ", world_push_direction.k[0],
                world_push_direction.k[1], world_push_direction.k[2]);
  }
  if ((angle > 0.0f)) {
    // turnaround_next_time: objects may still be closing, but is only rest
    // speed that can be neglected, as lowest energy point of impact was already
    // traversed
    IVP_IF(core[0]->environment->debug_information->debug_impact) {
    ...
    world_push_direction.fast_normize();
    // angle=-angle;
    // world_push_direction.subtract(surf_normal); // impact direction is
    // mirrored relative to surface
    return;
  } else {
    IVP_IF(core[0]->environment->debug_information->debug_impact) {
  ...
  if (two_friction_values == IVP_FALSE) {
    ...
    if ((angle > -cos_friction))  // negativ value
    {
      ...
      // devide world_push_direction in parts in direction of surface and
      // surface normal direction in surface normal is kept. direction in
      // surface is shortened in a way the angle is border-angle
#if 0
//not used at the moment -> needed by single_impact
		percent_deformation=angle/-cos_friction;
#endif
      IVP_DOUBLE mangle = -angle;
      part_in_direction_surf.add_multiple(&world_push_direction, surf_normal,
                                          mangle);
      part_in_direction_surf.normize();
      part_in_direction_surf.mult(
          sin_friction);  // part in direction of surface is shortend to be
                          // length of sin

      // part_in_direction_norm.set_multiple(surf_normal,-cos_friction);
      world_push_direction.add_multiple(
          &part_in_direction_surf, surf_normal,
          -cos_friction);  // recombine; vector has exactly length 1.0

      // ivp_message("recomb2 %f %f
...
// when friction values are low, the part in direction surface of world push
// direction is not allowed to be big we calculate the length of the part in
// direction surface of world push direction then we calculate the allowed
// length therefore we calculate the angle in 2D (spanned by the two friction
// vectors) and the allowed length is a position on a 2D ellipse when allowed
// length is shorter, we have to take the shorter value
void IVP_Impact_Solver::get_world_push_direction_two_friction(
    IVP_DOUBLE part_direction_surf_normal) {
  IVP_U_Float_Point
      part_in_direction_surf;  // decomposition of velo vec in direction normal
                               // and rest (this is the rest)
#if 0	
  ...
  part_in_direction_surf.add_multiple(&world_push_direction, surf_normal,
                                      mangle);
  // existing length
  IVP_FLOAT length_direction_surf =
      part_in_direction_surf.real_length_plus_normize();

  // now calculate allowed length
  // we have an ellipse with first radius sin_friction and second radius
  // sin_second_friction second axis is world_direction_second_friction
  IVP_DOUBLE part_second_friction =
      part_in_direction_surf.dot_product(&world_direction_second_friction);
  ...
  IVP_FLOAT quad_len_allowed =
      axis_first_friction * axis_first_friction * sin_friction * sin_friction +
      axis_second_friction * axis_second_friction * sin_second_friction *
          sin_second_friction;
  ...
  if (length_direction_surf * length_direction_surf > quad_len_allowed) {
    ...
    world_push_direction.add_multiple(
        &part_in_direction_surf, allowed_len);  // length is now exactly 1.0
  }

  // IVP_DOUBLE
  // part_second_friction=world_push_direction.dot_product(&world_direction_second_friction);
}
...
void IVP_Impact_Solver::confirm_impact(int core_nr) {
  ...
  if (c->impacts_since_last_PSI > al->get_max_collisions_per_psi()) {
    ...
    c->temporarily_unmovable =
        am->max_collisions_exceeded_check_freezing(al, c);
  }
}
...
void IVP_Impact_Solver::undo_push() {
  ...
  if (!core[1]->physical_unmoveable) {
    ...
    IVP_IF(core[0]->environment->debug_information->debug_impact) {
      ivp_message("undoing trans %.3f %.3f %.3f\n", trans_speed_change[1].k[0],
                  trans_speed_change[1].k[1], trans_speed_change[1].k[2]);
    }
  }
}

// push_dir_norm: normized push pointer, points to first core
// rescue push gives rotation free pushes when needed, to assure that objects
// are moving away from from each other
void IVP_Impact_Solver::do_rescue_push(IVP_U_Float_Point *push_dir_norm,
                                       IVP_BOOL panic_mode) {
  ...
  IVP_U_Float_Point diff_vec_world;  // vector of position difference. points
                                     // from first synapse to second
  ...
  diff_vec_world.set_multiple(
      push_dir_norm, -1.0f);  // diff_vec_world must point from first to second
  ...
  core[0]->get_surface_speed_on_test(obj_point[0], &translation, &rotation,
                                     &world_speed0);
  ...
  core[1]->get_surface_speed_on_test(obj_point[1], &translation, &rotation,
                                     &world_speed1);

  IVP_U_Float_Point rel_speed_world;  // speed of second synapse against first,
                                      // in world coords
  rel_speed_world.subtract(&world_speed1, &world_speed0);

  IVP_DOUBLE dist_velocity = rel_speed_world.dot_product(
      &diff_vec_world);  // velocity of distance change. negative means getting
                         // closer.
  ...
  IVP_DOUBLE
  desired_velo_change =  //(ivp_mindist_settings.coll_dist + 2 *
                         // ivp_mindist_settings.mindist_change_force_dist -
                         // dist_len_now) / env->get_delta_PSI_time() +
  ...
  if (!core[1]->physical_unmoveable) {
    ...
    core[1]->test_push_core(obj_point[1], &push_vec_obj, &push_vec_world,
                            &translation_vec, &rotation_vec);
    core[1]->get_surface_speed_on_test(obj_point[1], &translation_vec,
                                       &rotation_vec, &speed_change_vec);
  ...
  if (!core[0]->physical_unmoveable) {
    ...
    core[0]->test_push_core(obj_point[0], &push_vec_obj, &push_vec_world,
                            &translation_vec, &rotation_vec);
    core[0]->get_surface_speed_on_test(obj_point[0], &translation_vec,
                                       &rotation_vec, &speed_change_vec);
    speed_change_scalar += push_vec_world.dot_product(&speed_change_vec);
  }

  IVP_DOUBLE resulting_push =
      desired_velo_change / (speed_change_scalar + 1e-15f);
  ...
  IVP_IF(1) {
    if (panic_mode) {
      core[0]
          ->environment->get_statistic_manager()
          ->impact_hard_rescue_counter++;
    } else {
      core[0]
          ->environment->get_statistic_manager()
  ...
  if (!core[0]->physical_unmoveable) {
    mat = m_world_f_core[0];
    world_push.set_multiple(
        &diff_vec_world,
        -resulting_push);  // negativ: dist_vector_world points to second core
    ...
    core[0]->test_push_core(obj_point[0], &obj_push, &world_push,
                            &trans_speed_change[0], &rot_speed_change[0]);
    rot_speed[0].add(&rot_speed_change[0]);  // ...speed0 has sum of pushes and
                                             // beginning speed before impact
    trans_speed[0].add(&trans_speed_change[0]);  // if push has to be undone,
                                                 // values are subtracted later
  }
  ...
  if (!core[1]->physical_unmoveable) {
    ...
    // optimization, but debugging impossible, instead:
    core[1]->test_push_core(obj_point[1], &obj_push, &world_push,
                            &trans_speed_change[1], &rot_speed_change[1]);
    rot_speed[1].add(&rot_speed_change[1]);  // ...speed0 has sum of pushes and
                                             // beginning speed before impact
    trans_speed[1].add(&trans_speed_change[1]);  // if push has to be undone,
                                                 // values are subtracted later
  }
#endif
}

void IVP_Impact_Solver::do_push_on_core(IVP_U_Float_Point *push_vec_world,
                                        int num_core) {
  IVP_U_Float_Point push_vec_obj;
  IVP_U_Matrix *mat = m_world_f_core[num_core];
  mat->vimult3(push_vec_world, &push_vec_obj);

  core[num_core]->test_push_core(obj_point[num_core], &push_vec_obj,
                                 push_vec_world, &trans_speed_change[num_core],
                                 &rot_speed_change[num_core]);
  rot_speed[num_core].add(
      &rot_speed_change[num_core]);  // ...speed0 has sum of pushes and
                                     // beginning speed before impact
  trans_speed[num_core].add(
      &trans_speed_change[num_core]);  // if push has to be undone, values are
                                       // subtracted later
}

// first_push and second_push are pointers to cores that are pushed instantly
// (if not set to NULL) when allow_delaying is TRUE, one core may rest untouched
// and is changed in next PSI
void IVP_Impact_Solver::do_impact(IVP_Core *pushed_cores[2],
                                  IVP_BOOL allow_delaying,
                                  int pushes_while_system,
                                  IVP_FLOAT rescue_speed_addon) {
  // impact of surface (obj0) and point (obj1)
  // impulse is estimated, but problem cant be solved in one step -> iteration
  // of pushes there are done several pushes with push direction changing, when
  // desired energy level interval is reached : Stop there may be situations
  // where energy level cant be reached ( very flat impact angle). Then do a
  // single short push to avoid (hopefully) collision: do_rescue_push
  ...
  IVP_DEBUGCODE(core[0]->environment->get_debug_manager()->all_time_impacts +=
                1.0f;)

  used_conservation =
      1.0f -
      (1.0f / (pushes_while_system * IVP_INV_HALF_CONSERVATION_STEPS + 1.0f)) *
          (1.0f - percent_energy_conservation);

  this->rescue_speed_impact_solver =
      (IVP_FLOAT)(ivp_mindist_settings.min_vertical_speed_at_collision +
                  rescue_speed_addon) *
      IVP_SAFETY_FACTOR_FOR_DELAY;
  ...
  IVP_IF(0) {
    ...
    IVP_DOUBLE speed_parallel_mindist = relative_world_speed.dot_product(
        surf_normal);  // normally negative value
  ...
  IVP_DOUBLE impulse =
      this->estimate_push_impulse() * 0.1f;  // do only 10 percent pushes

  IVP_DOUBLE speed_parallel_mindist =
      relative_world_speed.dot_product(surf_normal);  // normally negative value

  if (speed_parallel_mindist > -IMPACT_EPS) {
    // something went wrong, do a rescue push
    // ivp_message("bad_rescue_push\n");
    IVP_U_Float_Point push_dir;
    push_dir.set_negative(surf_normal);
  ...
  // maybe do an extra rescue_push when speed is very low, this time with
  // friction

  IVP_IF(core[0]->environment->debug_information->debug_impact) {
    ivp_message("speed %.4f %.4f %.4f rot %.4f %.4f %.4f\n",
                trans_speed[1].k[0], trans_speed[1].k[1], trans_speed[1].k[2],
                rot_speed[1].k[0], rot_speed[1].k[1], rot_speed[1].k[2]);
  }
  ...
  virtual_speed = -surf_normal->dot_product(
      &relative_world_speed);  // alternative: world_push_direction->dot_pro..
  speed_before = virtual_speed;
  IVP_ASSERT(virtual_speed > 0.0f);

  give_back_speed =
      virtual_speed * IVP_Inline_Math::ivp_sqrtf(used_conservation);
  give_back_speed +=
      IVP_SPEED_ADDON_SYSTEM_IMPACT *
      IVP_Inline_Math::ivp_sqrtf((IVP_DOUBLE)pushes_while_system);
  ...
  give_back_speed += virtual_speed;  // virtual_speed is negative, and that
                                     // speed is already given back
  ...
  do_rescue_push(
      ...
      IVP_FALSE);  // to be sure that minimal velocity is reached, not panic

  IVP_IF(1 == 1) {
    IVP_U_Float_Point rel_speed_now;
    IVP_Core::get_diff_surface_speed_of_two_cores_on_test(
        core[0], core[1], obj_point[0], obj_point[1], &trans_speed[0],
        &rot_speed[0], &trans_speed[1], &rot_speed[1], &rel_speed_now);
    // ivp_message("world_speed_after_imp %.3f %.3f %.3f
    // ",rel_speed_now.k[0],rel_speed_now.k[1],rel_speed_now.k[2]);
    // ivp_message("rel_sppeed %.4f\n",rel_speed_now.dot_product(surf_normal));
    if (rel_speed_now.dot_product(surf_normal) > 0.0f) {
      IVP_IF(1) {
        ivp_message("impact_objects_have_illegal_speed_afterwards\n");
      }
      // CORE;
    }
  }
...
delaying_test:
  ...
  IVP_IF(0) {
    ...
    IVP_USE(testtest);
    IVP_DOUBLE speed_gain =
        testtest -
        speed_before;  // speed before not initialized when hard rescue
    if (speed_gain > 0.0f) {
      if (speed_gain >
          core[0]->environment->get_statistic_manager()->max_speed_gain) {
        core[0]->environment->get_statistic_manager()->max_speed_gain =
            speed_gain;
      }
    }
  ...
  // ivp_message("speed_after_imp %f %f  %f
  ...
  if (core[0]->temporarily_unmovable | core[1]->temporarily_unmovable) {
    IVP_IF(0) {
      ivp_message("switched_to_temporarily_unmov %p %p %f\n", core[0], core[1],
                  core[0]->environment->get_current_time().get_time());
    }
...
void IVP_Impact_Solver::delay_decision(IVP_Core *pushed_cores[2]) {
  ...
  core[0]->impacts_since_last_PSI +=
      static_cast<short>(1 - (bool)core[0]->physical_unmoveable);
  core[1]->impacts_since_last_PSI +=
      static_cast<short>(1 - (bool)core[1]->physical_unmoveable);
  ...
  if (delaying_is_allowed) {
    ...
    delayer_rot_speed.set(
        &core[try_to_delay_core]->rot_speed);  // use original speeds
    ...
    IVP_DOUBLE closing_speed;
    {
      ...
      IVP_Core::get_diff_surface_speed_of_two_cores_on_test(
          core[try_to_delay_core], core[other_core],
          obj_point[try_to_delay_core], obj_point[other_core],
          &delayer_trans_speed, &delayer_rot_speed, &other_trans_speed,
          &other_rot_speed, &rel_world_speed);
    ...
    if (closing_speed <
        -this->rescue_speed_impact_solver * IVP_INV_SAFETY_FACTOR_FOR_DELAY) {
...
void IVP_Impact_Solver::do_push(IVP_DOUBLE impulse_val) {
  ...
  if (!core[0]->physical_unmoveable) {
    IVP_IF(core[0]->environment->debug_information->debug_impact) {
      // ivp_message("p %.2f %.2f %.2f
      // ",world_push_direction.k[0],world_push_direction.k[1],world_push_direction.k[2]);
    }
    m_world_f_core[0]->inline_vimult3(
        &world_push, &obj_push);  // obj_push now in object coords
    core[0]->test_push_core(obj_point[0], &obj_push, &world_push,
                            &trans_speed_change[0], &rot_speed_change[0]);
    rot_speed[0].add(&rot_speed_change[0]);  // ...speed0 has sum of pushes and
                                             // beginning speed before impact
  ...
  if (!core[1]->physical_unmoveable) {
    ...
    core[1]->test_push_core(obj_point[1], &obj_push, &world_push,
                            &trans_speed_change[1], &rot_speed_change[1]);
    IVP_IF(core[0]->environment->debug_information->debug_impact) {
      // ivp_message("did_push trans %.3f %.3f %.3f add %.3f %.3f
...
void IVP_Impact_Solver::calc_virt_masses_impact_solver(
  ...
  for (i = 1; i >= 0; i--) {
    ...
    this->virt_mass[i] = core[i]->calc_correct_virt_mass(
        obj_point[i], (IVP_Vec_PCore *)&direction_obj, &direction_world);
...
IVP_DOUBLE IVP_Impact_Solver::estimate_push_impulse() {
  ...
  // virtual masses are along relative_world_speed , alternative would be to
  // change relative_world_speed according (not existing) friction and then calc
  // virtual mass no longer!
  ...
  // not maximal fast
  u0 =
      (2.0f * virt_mass[1] * push_v1 + push_v0 * (virt_mass[0] - virt_mass[1]));
  u1 =
      (2.0f * virt_mass[0] * push_v0 + push_v1 * (virt_mass[1] - virt_mass[0]));
  u0 *= div_masses;
  u1 *= div_masses;
  ...
  IVP_IF(core[0]->environment->debug_information->debug_impact) {
    ...
    const IVP_DOUBLE diff_v1 =
        u1 - push_v1;  // difference of velocity before/after impact
...
void IVP_Impact_Solver_Long_Term::do_impact_of_two_objects(
    IVP_Mindist *mindist, IVP_Real_Object *obj0, IVP_Real_Object *obj1) {
  ...
  IVP_Contact_Point *my_fr_dist = mindist->try_to_generate_managed_friction(
      &affected_friction_system, &having_new_dist, sim_u,
      IVP_TRUE);  // with recalc_friction_s_vals
  IVP_Impact_Solver_Long_Term *imp_solve = my_fr_dist->tmp_contact_info;

  IVP_Time now_time, last_time;
  IVP_Friction_Core_Pair *pair =
      affected_friction_system->find_pair_of_cores(core0, core1);
  ...
  IVP_FLOAT rescue_val_addon =
      my_fr_dist->get_rescue_speed_impact(obj0->get_environment());
  ...
  if (core1->physical_unmoveable) {
    speed_for_callback.mult(
        -1.0f);  // IVP_Impact_Solver_Long_Term changes order when core1 is
                 // unmovable @@CB fptodp
  }
  ...
  //	if((having_new_dist==IVP_FALSE)||1)
  {
    ...
    imp_sys.init_and_solve_impact_system(mindist, affected_friction_system,
                                         pair, my_fr_dist);
  }
...
void IVP_Contact_Point::get_material_info(IVP_Material *(&mtl)[2]) {
  for (int k = 0; k < 2; k++) {
    ...
    if (mat_index == 0) {
      ...
      mtl[k] = env->get_material_manager()->get_material_by_index(
          0, mat_index);  // @@@@@
    }
    IVP_ASSERT(mtl[k]);
  }
...
void IVP_Impact_Solver::get_cos_sin_for_impact(
    IVP_FLOAT friction_val, IVP_FLOAT percent_energy_conservation_,
    IVP_FLOAT *cos_val, IVP_FLOAT *sin_val) {
  IVP_DOUBLE impact_fri_fact =
      friction_val *
      (1.0f + IVP_Inline_Math::ivp_sqrtf(percent_energy_conservation_));
  IVP_DOUBLE fri_angle =
      IVP_Inline_Math::atand(impact_fri_fact);  // #+# bitter, kill,
  IVP_DOUBLE c = IVP_Inline_Math::approx5_cos(fri_angle);
  *cos_val = c;
  *sin_val = c * impact_fri_fact;
}

void IVP_Contact_Point::read_materials_for_contact_situation(
...
void IVP_Mindist::do_impact() {
  ...
  // revive_object_core is calling start_memory_transaction and
  // end_memory_transaction, so we cannot start memory transaction earlier
  env->sim_unit_mem->start_memory_transaction();
  ...
  IVP_Impact_Solver_Long_Term::do_impact_of_two_objects(this, objects[0],
                                                        objects[1]);
  env->sim_unit_mem->end_memory_transaction();
}

// find second critical core for core0 that is not core1
IVP_Core *IVP_Impact_Solver_Long_Term::find_second_critical_impact_core(
    IVP_Core *core0, IVP_Core *core1) {
  for (int c = core0->objects.len() - 1; c >= 0; c--) {
    IVP_Real_Object *r_obj = core0->objects.element_at(c);
    for (IVP_Synapse_Real *synr = r_obj->get_first_exact_synapse(); synr;
         synr = synr->get_next()) {
...
IVP_Core *IVP_Impact_Solver_Long_Term::get_best_merge_core(IVP_Core *core0,
                                                           IVP_Core *core1,
                                                           IVP_Core *core2) {
...
void IVP_Impact_Solver_Long_Term::do_impact_long_term(
    IVP_Core *pushed_cores[2], IVP_FLOAT rescue_speed_val,
  ...
  imp_solver_stack.percent_energy_conservation =
      this->impact.percent_energy_conservation;
  imp_solver_stack.two_friction_values = IVP_FALSE;

  if (imp_solver_stack.core[0]->car_wheel ||
      imp_solver_stack.core[1]->car_wheel) {
    imp_solver_stack.get_cos_sin_for_impact(
        0.0f, this->impact.percent_energy_conservation,
        &imp_solver_stack.cos_friction, &imp_solver_stack.sin_friction);
  } else {
    imp_solver_stack.get_cos_sin_for_impact(
        cp->real_friction_factor, this->impact.percent_energy_conservation,
        &imp_solver_stack.cos_friction, &imp_solver_stack.sin_friction);
    if (cp->two_friction_values) {
      imp_solver_stack.get_world_direction_second_friction(
          cp);  // imp_solver_stack.two_friction_values=cp->get_world_direction_second_friction(&imp_solver_stack.world_direction_second_friction,&imp_solver_stack.sin_second_friction);
    }
  }
  IVP_IF(1) {
  ...
  imp_solver_stack.do_impact(pushed_cores, IVP_TRUE, impacts_while_system,
                             rescue_speed_val);
  IVP_IF(1) {
    if (contact_core[0]) contact_core[0]->core_plausible_check();
    if (contact_core[1]) contact_core[1]->core_plausible_check();
...
//->make redesign !!!!!
void IVP_Impact_System::impact_system_check_start_pair(
    IVP_Friction_Core_Pair *start_pair, IVP_Contact_Point *mdfr) {
...
// optimize: first (initial) friction mindist is checked although it is known
// that it is valid

// Impact System:
// An Impact System is oriented to the Friction System. Friction Mindists are
// checked if they are still valid and then if they may collide in the near
// future. If they do, the impact is done instantly for speedup.
//
// There are three lists. One list with interesting Mindists. One list with
// Cores that are known (means they are synchronized for that point in time) and
// one list with Cores that have already impacted (they are synchronized and
// their speed has changed, meaning their Exact_Mindists and their
// Next_PSI_Matrix have to be updated. More speedup is gained with delaying
// technique. A core that is pushed only very weak doesnt change its speed. The
// speed is added in next PSI. Delayed cores need no update in Exact_Mindists
// and Next_PSI_Matrix. But the synchronize has to be undone. Delayed cores are
// found in list with known cores, but not necessarily in list with pushed
// cores. When a push is done in Impact System, the pushed cores (not a delayed
// one) are taken and their Mindists are rechecked (calc_coll_distance). A Core
// that was pushed is put into pushed list. A delayed push means that Mindists
// dont need call to calc_coll_distance. When everything is over, the cores
// beeing in known list and NOT beeing in pushed list are unsynchronized. All
// Cores in pushed list are updated in Exact_Mindists and Next_PSI_Matrix.

// #+# these things with start_pair and except_pair are weird and not necessary
// any longer: remove
void IVP_Impact_System::init_and_solve_impact_system(
    IVP_Mindist *mindist, IVP_Friction_System *fs_system,
    IVP_Friction_Core_Pair *start_pair, IVP_Contact_Point *start_fr_dist) {
  IVP_Core *start_core0 =
      start_fr_dist->get_synapse(0)->get_object()->get_core();
  IVP_Core *start_core1 =
      start_fr_dist->get_synapse(1)->get_object()->get_core();
  ...
  if (start_core0 && !start_core0->physical_unmoveable &&
      !start_core0->pinned) {  //@@CBPIN
  ...
  if (start_core1 && !start_core1->physical_unmoveable &&
      !start_core1->pinned) {  //@@CBPIN
  ...
  IVP_DEBUGCODE(l_environment->get_debug_manager()->debug_imp_sys = IVP_FALSE;)

  IVP_IFDEBUG(
      l_environment->get_debug_manager()->file_out_impacts,
      if (l_environment->get_current_time().get_time() > 10000.86f) {
        ...
        l_environment->get_debug_manager()->debug_imp_sys = IVP_TRUE;
                ...
                "starting_impact_system at time %f\n",
  ...
  while (test_loop_all_pairs() == IVP_TRUE) {
    ...
    if (sum_of_pushes > MAXIMUM_SYSTEM_PUSH_NUMBER) {
      IVP_IF(1) { ivp_message("system_push_number exceeded\n"); }
      P_DELETE(mindist);
      break;
      // put some mindists into hull!
    }
  }
  l_environment->get_statistic_manager()->impact_sum_sys +=
      impact_sys_counter + 1;

  IVP_IF(sum_of_pushes > 1) {
    // ivp_message("did_impact_sys %d pushes at
  ...
  IVP_IF(1) { debug_check_all_dists_at_end(); }

  recalc_all_affected_cores();
  IVP_IF(1) { associated_fs_system->debug_clean_tmp_info(); }
  IVP_IFDEBUG(l_environment->get_debug_manager()->file_out_impacts,
              l_environment->get_debug_manager()->debug_imp_sys = IVP_FALSE;
              fprintf(l_environment->get_debug_manager()->out_deb_file, "\n");)
}

void IVP_Impact_System::add_pair_to_impact_system(
    IVP_Friction_Core_Pair *new_pair) {
...
void IVP_Impact_System::add_pushed_core_with_pairs_except(
    IVP_Core *new_core, IVP_Friction_Core_Pair *start_pair) {
  add_pushed_core_to_impact_system(new_core);

  // connection of core to all its pairs is missing -> loop is quite big, but
  // add_core is not too often done
  for (int i = associated_fs_system->fr_pairs_of_objs.len() - 1; i >= 0; i--) {
    IVP_Friction_Core_Pair *fr_pair =
        associated_fs_system->fr_pairs_of_objs.element_at(i);
    if ((fr_pair->objs[0] == new_core) || (fr_pair->objs[1] == new_core)) {
      if (fr_pair != start_pair) {
        if (pair_is_already_in_system(fr_pair) == IVP_FALSE) {
          if (fr_pair->check_all_fr_mindists_to_be_valid(
                  this->associated_fs_system) > 0) {
...
IVP_BOOL IVP_Impact_System::pair_is_already_in_system(
    IVP_Friction_Core_Pair *test_pair) {
...
void IVP_Friction_System::debug_clean_tmp_info() {
  ...
  for (my_dist = this->get_first_friction_dist(); my_dist;
       my_dist = this->get_next_friction_dist(my_dist)) {
...
void IVP_Impact_System::debug_check_all_dists_at_end() {
  IVP_Contact_Point *my_dist;
  for (my_dist = associated_fs_system->get_first_friction_dist(); my_dist;
       my_dist = associated_fs_system->get_next_friction_dist(my_dist)) {
    my_dist->recalc_friction_s_vals();
    my_dist->calc_coll_distance();
    my_dist->tmp_contact_info->coll_time_is_valid = IVP_FALSE;
    if (my_dist->tmp_contact_info->impact.distance_reached_in_time <
        ivp_mindist_settings.minimum_friction_dist) {
      IVP_Core *core0, *core1;
      ...
      int unmov0, unmov1;
      unmov0 =
          (int)core0->physical_unmoveable | (int)core0->temporarily_unmovable;
      unmov1 =
          (int)core1->physical_unmoveable | (int)core1->temporarily_unmovable;
      if (unmov0 & unmov1) {
        continue;
      }
      if (core_is_already_known_to_system(core0) &&
          core_is_already_known_to_system(core1)) {
...
IVP_BOOL IVP_Impact_System::test_loop_all_pairs() {
  ...
  IVP_IFDEBUG(l_environment->get_debug_manager()->file_out_impacts,
              fprintf(l_environment->get_debug_manager()->out_deb_file,
                      "calc_new_coll_distances\n");)
  for (int i = i_s_pairs.len() - 1; i >= 0; i--) {
    IVP_Friction_Core_Pair *my_pair = i_s_pairs.element_at(i);
    int unmov0, unmov1;
    unmov0 = (int)my_pair->objs[0]->physical_unmoveable |
             (int)my_pair->objs[0]->temporarily_unmovable;  //@@CBPIN
    unmov1 = (int)my_pair->objs[1]->physical_unmoveable |
             (int)my_pair->objs[1]->temporarily_unmovable;  //@@CBPIN
    if (unmov0 & unmov1) {
      continue;
    }
    for (int k = my_pair->fr_dists.len() - 1; k >= 0; k--) {
      ...
      if (my_fr->tmp_contact_info->coll_time_is_valid == IVP_TRUE) {
        IVP_IFDEBUG(
            l_environment->get_debug_manager()->debug_imp_sys,
      ...
      if (my_fr->tmp_contact_info->impact.distance_reached_in_time <
          smallest_distance) {
        rescue_speed_addon = my_fr->tmp_contact_info->impact.rescue_speed_addon;
        smallest_distance =
            my_fr->tmp_contact_info->impact.distance_reached_in_time;
  ...
  if (worst_impact_dist) {
    IVP_ASSERT(
        worst_impact_dist->tmp_contact_info->impact.distance_reached_in_time <
        ivp_mindist_settings.minimum_friction_dist);
    IVP_Core *core;
    for (int i = 1; i >= 0; i--) {
    ...
    worst_impact_dist->tmp_contact_info->do_impact_long_term(
        impacting_cores, rescue_speed_addon, worst_impact_dist);

    // ivp_message("did_sys_imp
...
void IVP_Impact_System::invalidate_impact_mindists(
    IVP_Core *my_core) {  // there is a cheaper way: via synapses
  IVP_Friction_Info_For_Core *my_info =
      my_core->get_friction_info(associated_fs_system);
...
void IVP_Impact_Solver::get_world_direction_second_friction(
    IVP_Contact_Point
        *cp) {  // IVP_U_Float_Point *ret_second_friction_vec,IVP_FLOAT
                // *sin_second_friction) {
  IVP_Material *mtl[2];
  ...
  for (i = 0; i < 2; i++) {
    if (mtl[i]->second_friction_x_enabled) {
      ...
      IVP_DOUBLE difference_in_friction =
          cp->real_friction_factor - using_friction;
      IVP_FLOAT effective_friction_second =
          cp->real_friction_factor - difference_in_friction * relevance_factor;
#endif
      ...
      IVP_FLOAT dummy_cos;
      get_cos_sin_for_impact(effective_friction_second,
                             (IVP_FLOAT)percent_energy_conservation, &dummy_cos,
                             &sin_second_friction);
    }
  }
  ...
  for (i = 0; i < 2; i++) {
    ...
    if (my_core) {
      ...
      if (my_syn->get_status() != IVP_ST_BALL) {
        // IVP_DOUBLE rota_way=my_core->rot_speed.real_length() *
        // IVP_COLL_DETECT_MIN_TIME; if( rota_way > IVP_PI_2) {
        // ivp_message("warning_pi\n"); }

        // IVP_DOUBLE tan_quad = tan ( rota_way );
        // tan_quad*=tan_quad;
        IVP_DOUBLE tan_quad =
            my_core->rot_speed.quad_length() * IVP_COLL_DETECT_MIN_TIME *
            IVP_COLL_DETECT_MIN_TIME;  // approximate tan quad alpha with
                                       // alpha*alpha

        IVP_DOUBLE max_tan_quad =
            IVP_ROT_MAX_UNCERTAINTY * IVP_ROT_MAX_UNCERTAINTY;
        if (tan_quad > max_tan_quad) {
          // ivp_message("clippingrottan %f to
        ...
        IVP_DOUBLE uncertain_way =
            (1.0f - IVP_Inline_Math::cosd(alpha)) * my_core->upper_limit_radius;

        // test only
        // IVP_DOUBLE alpha = my_core->rot_speed.real_length() *
        // IVP_COLL_DETECT_MIN_TIME; IVP_DOUBLE sina = sin(alpha);
        // IVP_ASSERT(alpha >= sina);
        // IVP_DOUBLE uncertain_way2 = (alpha - sina) *
        // this->long_term_impact_info.contact_point_cs[i].real_length();

        IVP_DOUBLE uncertain_speed =
            uncertain_way *
            my_core->environment
                ->get_inv_delta_PSI_time();  // later speed is transformed back
                                             // to way with get_delta_PSI_time

        // IVP_FLOAT
        // rot_speed_quad=my_core->get_quad_surface_speed_orth_to_normal(&this->long_term_impact_info.contact_point_cs[i],&this->get_lt()->surf_normal);
        // IVP_FLOAT
        // upper_bound=IVP_ROT_MAX_UNCERTAINTY*IVP_ROT_MAX_UNCERTAINTY*my_core->environment->get_inv_delta_PSI_time()*my_core->environment->get_inv_delta_PSI_time();
        // if(rot_speed_quad > upper_bound ) {
        //   rot_speed_quad = upper_bound;
        // }
        //  rotate surface and take worst case way passed -> get uncertainty
        //  speed in formula tan alpha is approximated through alpha
        ...
        // IVP_FLOAT
        // obj_length_factor=long_term_impact_info.contact_point_cs[i].real_length();
        // IVP_ASSERT( (obj_length_factor < my_core->upper_limit_radius +
        // P_DOUBLE_EPS) );
...
IVP_FLOAT IVP_Contact_Point::get_rescue_speed_impact(IVP_Environment *env) {
  ...
  if (penetration_len < ivp_mindist_settings.min_coll_dists) {
    resc_val = (ivp_mindist_settings.min_coll_dists - penetration_len) *
               env->get_inv_delta_PSI_time();
  } else {
  ...
  return 2.0f * resc_val;  // 2.0f: use half factor when testing afterwards to
                           // assure convergence
}
...
void IVP_Contact_Point::calc_coll_distance() {
  ...
  info->impact.distance_reached_in_time =
      ...
      (closing_speed + info->impact.rescue_speed_addon * 0.5f) *
          env->get_delta_PSI_time();
  // rescue speed * 0.5f : at impact demand full rescue_speed; at testing,
  // demand only half (to assure termination)

  IVP_DEBUGCODE(
      IVP_IFDEBUG(
          env->get_debug_manager()->file_out_impacts,
          ...
          fprintf(fp, "  %zi %zi-%zi: ", 0x0000ffff & (hk_intp)this,
                  0x0000ffff & (hk_intp)core0, 0x0000ffff & (hk_intp)core1);
          ...
          IVP_IFDEBUG(env->get_debug_manager()->debug_imp_sys,
                      ivp_message("tested_frdist %zi di %.4f cs %.4f dr %.4f\n",
                                  0x0000ffff & (hk_intp)this, get_gap_length(),
                                  closing_speed +
                                      info->impact.rescue_speed_addon * 0.5f,
                                  info->impact.distance_reached_in_time);))
};

class IVP_Vector_of_Hull_Managers_256
...
void IVP_Impact_System::recalc_all_affected_cores() {
  IVP_IF(1) {
    for (int k = i_s_pushed_cores.len() - 1; k >= 0; k--) {
      ...
      IVP_ASSERT(my_core->tmp_null.old_sync_info->was_pushed_during_i_s ==
                 IVP_TRUE);
    }
  }
  {
  ...
  {
    IVP_DOUBLE delta_time_till_next_PSI =
        l_environment->get_next_PSI_time() - l_environment->get_current_time();
    ...
    for (int k = i_s_pushed_cores.len() - 1; k >= 0; k--) {
      ...
      if (!my_core->physical_unmoveable) {
        // ivp_message("calcing_next_psi_matrix %lx
    ...
    IVP_Calc_Next_PSI_Solver::commit_all_hull_managers(l_environment,
                                                       &active_hull_managers);
  }
  {
    for (int k = i_s_pushed_cores.len() - 1; k >= 0; k--) {
      IVP_Core *my_core = i_s_pushed_cores.element_at(k);
