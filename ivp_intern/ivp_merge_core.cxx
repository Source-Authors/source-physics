...
void IVP_Merge_Core::find_mass_center() {
  if (unmovable_core) {
    mcore->m_world_f_core_last_psi.vv.set(
        &unmovable_core->m_world_f_core_last_psi.vv);
  } else {
    IVP_U_Point center;
    center.set_to_zero();
...
void IVP_Merge_Core::find_damp_factors() {
  if (!unmovable_core) {
    ...
    for (int i = 0; i < n_cores; i++) {
      ...
      mcore->rot_speed_damp_factor.add_multiple(&core->rot_speed_damp_factor,
                                                mass);
...
void IVP_Merge_Core::find_main_axis() {
  if (unmovable_core) {
    ...
    mcore->pos_world_f_core_last_psi =
        unmovable_core->pos_world_f_core_last_psi;
  ...
  for (int i = 0; i < n_cores; i++) {
...

    {
      ...
      for (int s = 0; s < 3; s++) {  // s for source
        ...
        IVP_U_Float_Point
            projected_achse;  // spin of old axis projected to new axis
        // new axis are columns in m_world_f_mcore
        mcore->m_world_f_core_last_psi.vimult3(&world_achse, &projected_achse);
        sum_rot_inertia_speed.add_multiple(
            &projected_achse,
            core->get_rot_inertia()->k[s] * core->rot_speed.k[s]);
        ...
        quad_sum_rot_inertia.add_multiple(
            &projected_achse,
            rot_inertia_for_this_axis * rot_inertia_for_this_axis);
      }
    ...
    // add spin momentum by different mass centers
    {
      ...
      mcore->m_world_f_core_last_psi.vimult4(&core->m_world_f_core_last_psi.vv,
                                             &m_center_in_mcore);
      mcore->m_world_f_core_last_psi.vimult3(&core->speed, &speed_in_mcore);

      // see test_push_core for details
      distances_2d.inline_calc_cross_product(&m_center_in_mcore,
                                             &speed_in_mcore);
      ...
      sum_rot_inertia.k[1] +=
          ...
          core->get_mass();
      sum_rot_inertia.k[2] +=
          IVP_Inline_Math::sqrtd(quad_center.k[0] + quad_center.k[1]) *
          core->get_mass();
    }
  }
...
void IVP_Merge_Core::synchronize_motion() {
  IVP_DOUBLE d_time = mcore->environment->get_current_time() -
                      mcore->environment->get_old_time_of_last_PSI();
...
void IVP_Merge_Core::place_objects() {
  for (int i = 0; i < n_cores; i++) {
    ...
    mcore->m_world_f_core_last_psi.mimult4(&core->m_world_f_core_last_psi,
                                           &m_ncore_f_core);
    int o;
