 ...
 *************/
inline void IVP_Calc_Next_PSI_Solver::calc_rotation_matrix(
    IVP_FLOAT delta_sim_time, IVP_U_Quat *q_core_f_core) {
  IVP_Core *pc = core;

  // collision events do not want to calculate gyro effects because
  // they are based on synchronize_with_rot_z
  if (pc->rot_inertias_are_equal ||
      pc->environment->get_env_state() == IVP_ES_AT) {
    ...
    if (pc->car_wheel && (pc->max_surface_deviation == 0.0f)) {
      ...
      p.set_to_zero();
      IVP_DOUBLE sinus =
          IVP_Inline_Math::sind(rot_speed_wheel_axis * 0.5f * delta_sim_time);
      p.k[axis] = sinus;
...
#endif
    {
      q_core_f_core->set_fast_multiple_with_clip(&pc->rot_speed,
                                                 delta_sim_time);
    }
  ...
  const IVP_DOUBLE max_angle =
      5.0f * 6.0f / 360.0f;  // ten degrees per simulation step
  const IVP_DOUBLE qmax_angle = max_angle * max_angle;

  IVP_DOUBLE square_rot_speed =
      pc->rot_speed.quad_length();  // was already calculated in clip rot speed
  ...
  IVP_U_Point diff_other_rot_inertia_div_this(
      (ri->k[1] - ri->k[2]) * iri->k[0], (ri->k[2] - ri->k[0]) * iri->k[1],
      (ri->k[0] - ri->k[1]) * iri->k[2]);
  for (int i = 1;; i++) {
    IVP_U_Float_Point hp;
    hp.set(pc->rot_speed.k[1] * pc->rot_speed.k[2] *
               diff_other_rot_inertia_div_this.k[0],
           pc->rot_speed.k[2] * pc->rot_speed.k[0] *
               diff_other_rot_inertia_div_this.k[1],
           pc->rot_speed.k[0] * pc->rot_speed.k[1] *
               diff_other_rot_inertia_div_this.k[2]);
...
// returns the next simulation slot and the extra time step for faster
// simulation ( 0 means no extra time step )
#if 0
...
void IVP_Calc_Next_PSI_Solver::calc_next_PSI_matrix(
  ...
  IVP_IFDEBUG(
      ...
      if (dm->file_out_impacts) {
        fprintf(dm->out_deb_file, "making_calc_next_psi %zi at %f\n",
                ...
                core->environment->get_current_time().get_time());
      })
  ...
  {  // update AT positions !!!!
    IVP_DOUBLE d_last_time =
        core->environment->get_current_time() - core->time_of_last_psi;
    core->time_of_last_psi = core->environment->get_current_time();
    core->pos_world_f_core_last_psi.add_multiple(&core->delta_world_f_core_psis,
                                                 d_last_time);
    core->delta_world_f_core_psis.set(&core->speed);

    core->q_world_f_core_last_psi = core->q_world_f_core_next_psi;
    core->q_world_f_core_next_psi.inline_set_mult_quat(
        &core->q_world_f_core_next_psi, &q_core_f_core);
  ...
  for (int c = core->objects.len() - 1; c >= 0; c--) {
    ...
    IVP_Hull_Manager *h_manager = r_obj->get_hull_manager();
    h_manager->increase_hull_by_x(event_sim->environment->get_current_time(),
                                  event_sim->delta_time, new_speed,
                                  core->current_speed);
...
void IVP_Calc_Next_PSI_Solver::calc_psi_rotation_axis(
    const IVP_U_Quat *q_core_f_core) {
  ...
  if (qlen > P_DOUBLE_EPS) {
    ...
    IVP_IF(1) {
#if defined(DEBUG) && 0
      IVP_DOUBLE ref_angle =
          asin(sqrt(qlen)) -
          1E9f;  //@@@ was P_DOUBLE_RES; difference between old and new (fast)
                 // calculation should not be too great;
    ...
    core->m_world_f_core_last_psi.inline_vmult3((IVP_U_Point *)q_core_f_core,
                                                &rot_axis_ws);
...
void IVP_Calc_Next_PSI_Solver::set_transformation(
  ...
  IVP_U_Quat q_core_f_core;
  q_core_f_core.set_invert_mult(&core->q_world_f_core_last_psi,
  ...
  // set some speed values so there will be some speed values for hull
  // conversion
  if (optimize_for_repeated_calls) {
    core->current_speed =
        moved_distance_val * core->i_delta_time;  // simulate speed
  ...
  // calc rotation axis in world space and rotation speed. using quaternions
  IVP_DOUBLE diff = core->max_surface_deviation * core->max_surface_rot_speed *
...
void IVP_Calc_Next_PSI_Solver::commit_all_calc_next_PSI_matrix(
    IVP_Environment *env,
...
void IVP_Calc_Next_PSI_Solver::commit_one_hull_manager(
  ...
  if (active_hull_managers->len()) {
    ...
    IVP_Hull_Manager *hm =
        (IVP_Hull_Manager *)active_hull_managers->element_at(0);
...
void IVP_Calc_Next_PSI_Solver::commit_all_hull_managers(
  ...
  for (i = active_hull_managers->len() - 1; i > 1; i--) {
    IVP_IF_PREFETCH_ENABLED(IVP_TRUE) {
      IVP_Hull_Manager *hm2 =
          (IVP_Hull_Manager *)active_hull_managers->element_at(i - 2);
      hm2->prefetch0_hull();
      IVP_Hull_Manager *hm1 =
          (IVP_Hull_Manager *)active_hull_managers->element_at(i - 1);
      hm1->prefetch1_hull();
    }
    IVP_Hull_Manager *hm =
        (IVP_Hull_Manager *)active_hull_managers->element_at(i);
    hm->check_hull_synapses(env);
    hm->check_for_reset();
  }
  if (i == 1) {
    IVP_Hull_Manager *hm =
        (IVP_Hull_Manager *)active_hull_managers->element_at(1);
    hm->check_hull_synapses(env);
    hm->check_for_reset();
    i--;
  }
