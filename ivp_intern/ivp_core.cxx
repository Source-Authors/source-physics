...
#define MAX_PLAUSIBLE_LEN \
  100000.0f  // slower than meter/sec  //XXX 0 added by chris!

// get surface speed; take rotation speed and center speed as input
void IVP_Core::get_surface_speed_on_test(
    const IVP_U_Float_Point *point_cs, const IVP_U_Float_Point *center_speed_ws,
  ...
  this->m_world_f_core_last_psi.inline_vmult3(
      &speed_cs, speed_out_ws);  // transform to world coords
  speed_out_ws->add(center_speed_ws);
}

void IVP_Core::get_surface_speed_ws(const IVP_U_Point *position_ws_in,
  ...
  pos_rel.subtract(position_ws_in, get_position_PSI());
  IVP_U_Float_Point cross;
  cross.inline_calc_cross_product(&av_ws, &pos_rel);
  speed_ws_out->add(&speed, &cross);
}

void IVP_Core::get_diff_surface_speed_of_two_cores(
    const IVP_Core *this_core, const IVP_Core *other_core,
  ...
  if (this_core && !this_core->physical_unmoveable) {
    this_core->get_surface_speed_on_test(obj_point_this, &this_core->speed,
                                         &this_core->rot_speed,
                                         delta_velocity_ws);
  } else {
  ...
  if (other_core && !other_core->physical_unmoveable) {
    ...
    other_core->get_surface_speed_on_test(obj_point_other, &other_core->speed,
                                          &other_core->rot_speed,
                                          &velocity_ws_2);
    delta_velocity_ws->subtract(&velocity_ws_2);
  }
}

void IVP_Core::get_diff_surface_speed_of_two_cores_on_test(
    const IVP_Core *this_core, const IVP_Core *other_core,
    const IVP_U_Float_Point *obj_point_this,
    const IVP_U_Float_Point *obj_point_other,
  ...
  if (!this_core->physical_unmoveable) {
    this_core->get_surface_speed_on_test(obj_point_this, trans_speed0,
                                         rot_speed0, &world_this);
  } else {
    world_this.set_to_zero();
  }

  if (!other_core->physical_unmoveable) {
    other_core->get_surface_speed_on_test(obj_point_other, trans_speed1,
                                          rot_speed1, &world_other);
  } else {
...
IVP_DOUBLE IVP_Core::calc_virt_mass_worst_case(
    const IVP_U_Float_Point *core_point) const {
  IVP_U_Float_Point worst_direction;
  worst_direction.k[0] =
      core_point->k[1] * core_point->k[1] + core_point->k[2] * core_point->k[2];
  worst_direction.k[1] =
      core_point->k[0] * core_point->k[0] + core_point->k[2] * core_point->k[2];
  worst_direction.k[2] =
      core_point->k[0] * core_point->k[0] + core_point->k[1] * core_point->k[1];
  worst_direction.set_pairwise_mult(&worst_direction,
                                    this->get_inv_rot_inertia());
  ...
  for (d = this->objects.len() - 1; d >= 0; d--) {
    ...
    while (fr_synapse) {
      ...
      if (other_core->physical_unmoveable) {
        ...
        fr_mindist->read_materials_for_contact_situation(
            &info);  // this is done only to get friction_factor ; maybe make
                     // extra function for friction_factor
...
void IVP_Core::values_changed_recalc_redundants() {
  ...
  if (this->physical_unmoveable) {
    ...
    this->movement_state =
        IVP_MT_NOT_SIM;  // flag changed from moveable to unmoveable
  } else {
    if (this->movement_state > IVP_MT_NOT_SIM) {
      this->movement_state =
          IVP_MT_NOT_SIM;  // flag changed from unmoveable to moveable
    }
  ...
  if (!this->physical_unmoveable) {
    ...
    for (d = this->objects.len() - 1; d >= 0; d--) {
      ...
      IVP_Friction_System *fs = HK_NULL;
      fr_synapse = r_obj->get_first_friction_synapse();
      if (fr_synapse) {
        fr_mindist = fr_synapse->get_contact_point();
        fs = fr_mindist->l_friction_system;
      ...
      for (; fr_synapse; fr_synapse = fr_synapse->get_next()) {
        ...
        fr_mindist->recalc_friction_s_vals();
        fr_mindist->read_materials_for_contact_situation(
            &info);  // this is only done to calculate friction_factor ; maybe
                     // make extra function for friction_factor
        fr_mindist->calc_virtual_mass_of_mindist();
      }
    }
    // EMAURER
...
IVP_DOUBLE IVP_Core::calc_correct_virt_mass(
    const IVP_U_Float_Point *core_point,
    const IVP_U_Float_Point *direction_core,
    const IVP_U_Float_Point *direction_world) const {
  if (!pinned)  //@@CBPIN
  {
    IVP_U_Float_Point ret_center_speed, ret_rot_speed;
    this->test_push_core(core_point, direction_core, direction_world,
                         &ret_center_speed, &ret_rot_speed);

    IVP_U_Float_Point ret_world_speed;
    this->get_surface_speed_on_test(core_point, &ret_center_speed,
                                    &ret_rot_speed, &ret_world_speed);
...
void IVP_Core::apply_velocity_limit() {
  ...
  IVP_FLOAT max_angular_velocity_per_psi =
      al->get_max_angular_velocity_per_psi();
  if (max_angular_velocity_per_psi > 0.0) {
    IVP_DOUBLE max_rot_speed =
        environment->get_inv_delta_PSI_time() * max_angular_velocity_per_psi;
...
void p_calc_2d_distances_to_axis(const IVP_U_Float_Point *point,
                                 const IVP_U_Float_Point *direction,
                                 IVP_U_Float_Point *distances) {
...
IVP_DOUBLE IVP_Core::calc_virt_mass(const IVP_U_Float_Point *core_point,
  ...
  if (direction) {
    ...
    distances_2d.set_pairwise_mult(
        &distances_2d,
        &distances_2d);  // use ^2 because speed = rot_speed * radius
...
IVP_DOUBLE IVP_Core::get_energy_on_test(
...
void IVP_Core::calc_at_matrix(IVP_Time current_time,
                              IVP_U_Matrix *m_world_f_core_out) const {
  while (IVP_MTIS_SIMULATED(movement_state)) {
    ...
    {
      IVP_U_Quat res;
      res.set_interpolate_smoothly(&q_world_f_core_last_psi,
                                   &q_world_f_core_next_psi,
                                   d_time * i_delta_time);
      res.set_matrix(m_world_f_core_out);
      m_world_f_core_out->get_position()->add_multiple(
          &pos_world_f_core_last_psi, &delta_world_f_core_psis, d_time);
    }
    return;
  }
...
void IVP_Core::global_damp_core(IVP_DOUBLE d_time) {
  ...
  if (my_core->movement_state >= IVP_MT_SLOW) {
    ...
    IVP_U_Float_Point add_on(
        s_add_on, s_add_on,
        s_add_on);  // somewhat more dampening for slow objects
    IVP_U_Float_Point rot_damp_factor;
    rot_damp_factor.add(&my_core->rot_speed_damp_factor, &add_on);

    my_core->damp_object(d_time, &rot_damp_factor,
                         my_core->speed_damp_factor + s_add_on);  // 60 1.5
  } else {
    my_core->damp_object(d_time, &my_core->rot_speed_damp_factor,
                         my_core->speed_damp_factor);  // 60 1.5
  }
}

// object is affected after commit_all_async_pushes
void IVP_Core::center_push_core_multiple_ws(
    const IVP_U_Float_Point *delta_speed, IVP_DOUBLE factor) {
  this->speed.add_multiple(delta_speed, factor * get_inv_mass());
}

void IVP_Core::async_center_push_core_multiple_ws(
    const IVP_U_Float_Point *delta_speed, IVP_DOUBLE factor) {
  this->speed_change.add_multiple(delta_speed, factor * get_inv_mass());
}

void IVP_Core::damp_object(IVP_DOUBLE delta_time_,
                           const IVP_U_Float_Point *rotation_factor,
                           IVP_DOUBLE speed_factor) {
...
void IVP_Core::set_rotation_inertia(const IVP_U_Float_Point *r) {
  rot_inertia.set(r->k[0] > P_FLOAT_MAX ? P_FLOAT_MAX : r->k[0],
...
void IVP_Core::get_surface_speed(const IVP_U_Float_Point *point_core,
                                 IVP_U_Float_Point *speed_world_out) const {
  this->get_surface_speed_on_test(point_core, &this->speed, &this->rot_speed,
                                  speed_world_out);
}

// change rot_speed_change and speed_change, needs movement_transaction // #+#
// to be killed for (springs)
void IVP_Core::async_push_core(const IVP_U_Float_Point *point_cs,
  ...
  this->test_push_core(
      point_cs, impulse_in_core, impulse_in_world, &center_change,
      &rot_change);  // stores results in variables rot_change and center_change
...
void IVP_Core::push_core_ws(const IVP_U_Point *world_point,
  ...
  point_d_ws.subtract(world_point, m_world_f_core->get_position());
...
void IVP_Core::async_rot_push_core_multiple_ws(
    const IVP_U_Float_Point *angular_impulse_ws, IVP_DOUBLE factor) {
  const IVP_U_Matrix *m_world_f_core = get_m_world_f_core_PSI();
...
void IVP_Core::rot_push_core_multiple_ws(
    const IVP_U_Float_Point *angular_impulse_ws, IVP_DOUBLE factor) {
  const IVP_U_Matrix *m_world_f_core = get_m_world_f_core_PSI();
  IVP_U_Float_Point angular_impulse_cs;
  m_world_f_core->vimult3(angular_impulse_ws, &angular_impulse_cs);
...
// don't store result of push directly in core variables but in temporary
// variables
void IVP_Core::test_push_core(const IVP_U_Float_Point *point_cs,
                              const IVP_U_Float_Point *impulse_in_core,
                              const IVP_U_Float_Point *impulse_in_world,
                              IVP_U_Float_Point *speed_out,
                              IVP_U_Float_Point *rot_out) const {
  IVP_U_Float_Point distances_2d;
...
// don't store result of rot push directly in core variables but in temporary
// variables
void IVP_Core::test_rot_push_core_multiple_cs(
    const IVP_U_Float_Point *normized_core_axis, IVP_DOUBLE rot_impulse,
...
IVP_DOUBLE IVP_Core::get_rot_inertia_cs(
...
IVP_DOUBLE IVP_Core::get_rot_speed_cs(
    const IVP_U_Float_Point *normized_core_axis) {
  return rot_speed.dot_product(normized_core_axis);
}

void IVP_Core::async_rot_push_core_multiple_cs(
    const IVP_U_Float_Point *normized_core_axis, IVP_DOUBLE rot_impulse) {
  IVP_U_Float_Point h;
  h.set_pairwise_mult(normized_core_axis, get_inv_rot_inertia());
...
void IVP_Core::rot_push_core_multiple_cs(
    const IVP_U_Float_Point *normized_core_axis, IVP_DOUBLE rot_impulse) {
  IVP_U_Float_Point h;
  h.set_pairwise_mult(normized_core_axis, get_inv_rot_inertia());
...
void IVP_Core::unlink_obj_from_core_and_maybe_destroy(
    IVP_Real_Object *remove_obj) {
...
void IVP_Core::synchronize_with_rot_z() {
  ...
  IVP_IFDEBUG(
      IVP_TRUE, IVP_Debug_Manager *dm = environment->get_debug_manager();
  ...
  tmp_null.old_sync_info->old_sync_q_world_f_core_next_psi =
      q_world_f_core_next_psi;

  IVP_U_Quat q_core_f_core;
  q_core_f_core.set_invert_mult(&q_world_f_core_last_psi,
                                &q_world_f_core_next_psi);
...
void IVP_Core::undo_synchronize_rot_z() {
  IVP_IFDEBUG(
      IVP_TRUE, IVP_Debug_Manager *dm = environment->get_debug_manager();
      if (dm->file_out_impacts) {
        fprintf(dm->out_deb_file, "undoing_synchro %p at %f\n",
                this,
...
void IVP_Core::calc_calc() {
  ...
  IVP_U_Float_Point diff_vec(iri->k[1] - iri->k[2], iri->k[2] - iri->k[0],
                             iri->k[0] - iri->k[1]);
...
void IVP_Core::init(IVP_Real_Object *io) {
  ...
  // please change: unmoveable and not sim objs get no simulation unit -> no sim
  // unit per default
...
IVP_Core::IVP_Core(IVP_Real_Object *io) {
  init(io);
  environment->get_sim_units_manager()->add_sim_unit_to_manager(
      sim_unit_of_core);
}

IVP_Core::IVP_Core(IVP_Real_Object *io, const IVP_U_Quat *q_world_f_object_init,
                   const IVP_U_Point *position, IVP_BOOL physical_unmoveable_,
...
IVP_Core_Merged::IVP_Core_Merged(IVP_Real_Object *real_obj)
    : IVP_Core(real_obj) {
...
IVP_Core_Merged::IVP_Core_Merged(IVP_Core *core0, IVP_Core *core1)
  ...
  P_MEM_CLEAR(this);  // warning: this is not so good (old values are destroyed,
                      // e.g. a vector object in IVP_Core)
  physical_unmoveable =
      (IVP_BOOL)(core0->physical_unmoveable | core1->physical_unmoveable);
...
IVP_Movement_Type IVP_Core::calc_movement_state(IVP_Time psi_time) {
  ...
  while (1) {  // break if object has moved
    IVP_DOUBLE qdist = pos_world_f_core_last_psi.quad_distance_to(
        &position_world_f_core_calm_reference[0]);
    if (qdist > IVP_MINIMAL_TRANS_SPEED_PER_PSI_SHORT *
                    IVP_MINIMAL_TRANS_SPEED_PER_PSI_SHORT)
      break;

    IVP_DOUBLE qrot_sum = q_world_f_core_next_psi.inline_estimate_q_diff_to(
        &q_world_f_core_calm_reference[0]);
    if (qrot_sum * upper_limit_radius * upper_limit_radius >
        IVP_MINIMAL_ROT_SPEED_PER_PSI_SHORT *
            IVP_MINIMAL_ROT_SPEED_PER_PSI_SHORT)
      break;

    // don't check long term for slow objects
    if (psi_time - time_of_calm_reference[0] >
        environment->get_freeze_manager()->freeze_check_dtime) {
      return IVP_MT_CALM;
    }
    return IVP_MT_SLOW;
  }
  ...
  while (1) {  // break if object has moved
    IVP_DOUBLE q_dist = pos_world_f_core_last_psi.quad_distance_to(
        &position_world_f_core_calm_reference[1]);
    if (q_dist > IVP_MINIMAL_TRANS_SPEED_PER_PSI_LONG *
                     IVP_MINIMAL_TRANS_SPEED_PER_PSI_LONG)
      break;

    IVP_DOUBLE qrot_sum = q_world_f_core_last_psi.inline_estimate_q_diff_to(
        &q_world_f_core_calm_reference[1]);
    if (qrot_sum * upper_limit_radius * upper_limit_radius >
        IVP_MINIMAL_ROT_SPEED_PER_PSI_LONG * IVP_MINIMAL_ROT_SPEED_PER_PSI_LONG)
      break;
...
void IVP_Core::create_collision_merged_core_with(IVP_Core *other_core) {
  ...
  // update coordinates of all real objects
  {
    ...
    if (this->m_world_f_core_last_psi.quad_distance_to(m_world_f_core_merged) >
        P_DOUBLE_EPS) {
      IVP_Core *core = this;
      IVP_U_Matrix new_m_core_f_core;
      m_world_f_core_merged->mimult4(&core->m_world_f_core_last_psi,
                                     &new_m_core_f_core);
      for (int c = objects.len() - 1; c >= 0; c--) {
    ...
    if (other_core->m_world_f_core_last_psi.quad_distance_to(
            m_world_f_core_merged) > P_DOUBLE_EPS) {
...
void IVP_Core::set_matrizes_and_speed(IVP_Core_Merged *template_core,
                                      IVP_U_Matrix *m_CORE_f_core_out) {
  ...
  if (template_core == this->merged_core_which_replace_this_core) {
    template_core->m_world_f_core_last_psi.mi2mult4(
        &template_core->m_world_f_core_when_created, &m_WORLD_f_world);
  } else {
    m_WORLD_f_world.init();
    IVP_Core_Merged *core;
    for (core = this->merged_core_which_replace_this_core; core;
      ...
      m_WORLD_f_world_one_core.mmult4(&m_WORLD_f_world, &m_WORLD_f_world);
    }
  }
  ...
  IVP_U_Matrix m_CORE_f_core;
  {
    ...
    m_WORLD_f_world.mmult4(&this->m_world_f_core_last_psi, &m_WORLD_f_core);
    m_world_f_core_last_psi.mimult4(&m_WORLD_f_core, &m_CORE_f_core);
  }
  *m_CORE_f_core_out = m_CORE_f_core;
  ...
  template_core->m_world_f_core_last_psi.vmult3(&template_core->speed,
                                                &rot_speed_world);
...
void IVP_Core::update_exact_mindist_events_of_core() {
  this->mindist_event_already_done =
      environment->mindist_event_timestamp_reference;  // flag in core to avoid
                                                       // IVP_DOUBLE calculating
  for (int c = objects.len() - 1; c >= 0; c--) {
    IVP_Real_Object *obj = this->objects.element_at(c);
...
void IVP_Core::rot_speed_plausible_check(
    const IVP_U_Float_Point *rot_speed_test) {
  IVP_DOUBLE rot_len;
...
IVP_Friction_Info_For_Core *IVP_Core::get_friction_info(
    IVP_Friction_System *my_fr_system) {
  if (this->physical_unmoveable == IVP_TRUE) {
    IVP_Friction_Hash *my_hash =
        this->core_friction_info.for_unmoveables.l_friction_info_hash;
    if (my_hash) {
      IVP_Friction_Info_For_Core *fr_info;
      fr_info = my_hash->find_friction_info(my_fr_system);
#ifdef DEBUG_FRICTION_CONSISTENCY
      IVP_IFDEBUG(
          my_fr_system->l_environment->get_debug_manager()->check_fs,
          IVP_Friction_Info_For_Core * test_info;
          test_info = NULL; int i; for (i = 0; i < list_debug_hash.len(); i++) {
    ...
    IVP_Friction_Info_For_Core *my_info =
        this->core_friction_info.for_moveables.moveable_core_friction_info;
    if (my_info) {
...
void IVP_Core::unmovable_core_debug_friction_hash() {
  for (int c = objects.len() - 1; c >= 0; c--) {
    ...
    IVP_Synapse_Friction *fr_synapse;
    for (fr_synapse = obj->get_first_friction_synapse(); fr_synapse;
         fr_synapse = fr_synapse->get_next()) {
      [[maybe_unused]] IVP_Contact_Point *fr_mindist =
          fr_synapse->get_contact_point();

      IVP_ASSERT(
          get_friction_info(fr_mindist->l_friction_system)->l_friction_system ==
          fr_mindist->l_friction_system);  // error after deleting hash entry
    }
  }
...
void IVP_Core::unlink_friction_info(IVP_Friction_Info_For_Core *my_fr_info) {
  IVP_Friction_Info_For_Core *my_info;
  if (this->physical_unmoveable == IVP_TRUE) {
    my_info = this->core_friction_info.for_unmoveables.l_friction_info_hash
                  ->remove_friction_info(my_fr_info);
    IVP_ASSERT(my_info);
#ifdef DEBUG_FRICTION_CONSISTENCY
    IVP_IFDEBUG(
        environment->get_debug_manager()->check_fs, int i = 0;
