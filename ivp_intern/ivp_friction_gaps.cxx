  ...
  int gauss_failed = 0;
  {
    for (int i = 0; i < total_actives; i++) {
      ...
      IVP_Impact_Solver_Long_Term *info =
          contact_info_vector.element_at(orig_index);

      IVP_DOUBLE push_val = push_results[i];
      // ivp_message("gauss_value %f  ",push_val);

      IVP_DOUBLE allowed_negative =
          IVP_MAX_ADHESION_GAUSS * l_environment->gravity_scalar;

      // ivp_message("allowed_negative %f\n",allowed_negative);
      if (allowed_negative >
          push_val * correct_x_factor * get_inv_virtual_mass(info)) {
...
void IVP_Friction_Solver::normize_constraint_equ() {
  ...
  for (i = dist_change_mat.columns - 1; i >= 0; i--) {
    IVP_DOUBLE test_val =
        dist_change_mat.matrix_values[i * dist_change_mat.aligned_row_len + i];
    if (test_val > max_val) {
      max_val = test_val;
    }
  }
  ...
  for (i = dist_change_mat.columns - 1; i >= 0; i--) {
    IVP_DOUBLE test_val =
        IVP_Inline_Math::fabsd(dist_change_mat.desired_vector[i]);
    if (test_val > max_val) {
      max_val = test_val;
    }
  }
  ...
  for (i = 0; i < dist_change_mat.columns; i++) {
    IVP_DOUBLE *base =
        &dist_change_mat.matrix_values[i * dist_change_mat.aligned_row_len];
...
int IVP_Friction_System::get_num_supposed_active_frdists() {
  ...
  for (fr_dist = this->get_first_friction_dist(); fr_dist;
...
// array 'active_is_at_pos' has 'total_actives' entries and gives original pos
// of an active
void IVP_Friction_Solver::solve_linear_equation_and_push(
    IVP_Friction_System *my_fs, int *active_is_at_pos, int total_actives,
    IVP_U_Memory *mem_friction) {
  ...
  memset((char *)dist_change_mat.result_vector, 0,
         dist_change_mat.aligned_row_len * sizeof(IVP_DOUBLE));

  actives_matrix.fill_from_bigger_matrix(&dist_change_mat, active_is_at_pos,
                                         total_actives);

  IVP_Linear_Constraint_Solver constraint_unilateral;

  if (actives_matrix.solve_great_matrix_many_zero() == IVP_OK) {
    if ((test_gauss_solution_suggestion(actives_matrix.result_vector,
                                        active_is_at_pos, total_actives,
                                        mem_friction) == IVP_OK)) {
      IVP_IF(1) { this->gauss_succed++; }
      goto first_try;
    }
  ...
  if (constraint_unilateral.init_and_solve_lc(
          dist_change_mat.matrix_values, dist_change_mat.desired_vector,
          dist_change_mat.result_vector, dist_change_mat.columns,
          supposed_actives, mem_friction) == IVP_OK) {
  first_try:
    ...
    if (e_after <= e_before + e_max_grown) {
      ...
      IVP_IF(1) { ivp_message("too_much_energy_gained\n"); }
      my_fs->undo_complex_pushes();
    }
  } else {
    IVP_IF(1) { ivp_message("static_friction_unequation failed\n"); }
  }
...
void IVP_Friction_System::do_pushes_distance_keepers(
    const IVP_Event_Sim *es_in) {
  IVP_Friction_System *fs = this;

  for (IVP_Contact_Point *fr_dist = fs->get_first_friction_dist(); fr_dist;
       fr_dist = this->get_next_friction_dist(fr_dist)) {
    ...
    IVP_FLOAT speedup;
    speedup =
        ((IVP_FLOAT)(IVP_SLOWLY_TURN_ON_KEEPER -
    ...
    if (impulse > 0.0f) {
      ...
      // ivp_message("%x gap %f single old %f new %f  impulse %f\n", this,
      // gap_diff, closing_speed, closing_speed2, impulse);
    } else {
      fr_dist->now_friction_pressure = 0.0f;
    }
...
IVP_FLOAT IVP_Friction_Solver::do_penalty_step(IVP_FLOAT *impulses,
                                               IVP_FLOAT *pretension,
  ...
  // forward step
  {
    for (int i = contact_info_vector.len() - 1; i >= 0; i--) {
      ...
      IVP_DOUBLE a =
          delta_velocity * damp_factor + pretension[i] * force_factor;
      IVP_DOUBLE b = info->virtual_mass;
      IVP_DOUBLE impulse = a * b;
...
int IVP_Friction_Solver::do_resulting_pushes(IVP_Friction_System *fs) {
  int m_index = 0;
  for (IVP_Contact_Point *my_dist = fs->get_first_friction_dist(); my_dist;
    ...
    if (impulse_now > 0.0f) {
      ...
      if (impulse_now == 0.0f) {
        ...
        impulse_now =
            my_dist->now_friction_pressure * 0.0f * this->es->delta_time;
        goto cont_for_next;
      } else {
        // negative push
        if (my_dist->has_negative_pull_since < 0) {
        ...
        my_dist->has_negative_pull_since++;
        if (my_dist->has_negative_pull_since >
            9) {  // allow only 9 negative pushes in following PSIs
...
#else
    my_dist->now_friction_pressure +=
        impulse_now * this->es->i_delta_time;  // keine Glaettung der Reibung
#endif
  }
  IVP_IF(l_environment->debug_information->debug_friction) {
...
#ifdef DEBUG
  IVP_IF(l_environment->debug_information->debug_friction) {
    ivp_message("pushes_ok. resulting_distance_velos:\n");
...
void IVP_Friction_Solver::print_dist_velocity(IVP_Friction_System *fs) {
  ...
  while (my_dist) {
    ...
    core0->get_surface_speed_on_test(
        &my_dist->tmp_contact_info->contact_point_cs[0], &speed_var_trans,
        &speed_var_rot, &w_speed0);
    speed_var_trans.add(&core1->speed, &core1->speed_change);
    speed_var_rot.add(&core1->rot_speed, &core1->rot_speed_change);
    core1->get_surface_speed_on_test(
        &my_dist->tmp_contact_info->contact_point_cs[1], &speed_var_trans,
        &speed_var_rot, &w_speed1);

    IVP_U_Float_Point rel_world_speed;
    rel_world_speed.subtract(&w_speed1, &w_speed0);

    IVP_IF(1) {
      ivp_message("dvelo_ %.4f   ",
                  rel_world_speed.dot_product(&my_dist->get_lt()->surf_normal));
    }
...
void IVP_Friction_System::test_hole_fr_system_data() {
  IVP_IF(1) {
	...
	{
	    ivp_message("test_fr dist_number wrong\n");
	    CORE;
	}
    }
#endif
	...
	if(startnumber<0)
	{
	    ivp_message("test_fr number <0");
	    CORE;
	}

	if(startnumber!=mindist->number_in_friction)
	{
	    ivp_message("test_fr wrong number");
	    CORE;
	}
#endif
      for (int i = 0; i < 2; i++) {
        ...
        if (fr_info->l_friction_system != this) {
          ivp_message("test_fr l_friction_system %f %p\n", time.get_time(),
                      obj_obj);
          CORE;
        }
        ...
        for (int j = fr_info->friction_springs.len() - 1; j >= 0; j--) {
          IVP_Contact_Point *all_dists =
              fr_info->friction_springs.element_at(j);
          if (all_dists == mindist) {
            ...
            if (!is_in_collection) {
              ivp_message(
                  "test_fr there was mindist %zi in obj which is not in "
                  "system\n",
                  (hk_intp)all_dists & 0x0000ffff);
              CORE;
            }
          }
        }
        if (!found_mine) {
          ivp_message("test_fr mindist %p missing in obj\n", mindist);
          CORE;
        }
      }

      {
        ...
        if (!found_mine) {
          ivp_message("test_fr dist %p not found in pairs\n", mindist);
          CORE;
        }
      }
...

    {
      ...
      for (int n = fr_pairs_of_objs.len() - 1; n >= 0; n--) {
        ...
        for (int m = fr_pair->fr_dists.len() - 1; m >= 0; m--) {
          ...
          int found_mine = 0;
          for (IVP_Contact_Point *mindist2 = fs->get_first_friction_dist();
               mindist2; mindist2 = fs->get_next_friction_dist(mindist2)) {
            if (mindist2 == fr_dist) {
              found_mine = 1;
            }
          }
          if (!found_mine) {
            ivp_message("test_fr pair %p containing dist %p not in system\n",
                        fr_pair, fr_dist);
            CORE;
          }
        }
...
void check_fr_info_mindist(IVP_Friction_Solver *fsolv,IVP_Contact_Point *my_dist,int dist_counter_two)
{
    ...
    {
	ivp_message("fr_info_damaged obj %lx\n",(long)obj0&0x0000ffff);
	CORE;
    }
  ...
  for (IVP_Contact_Point *my_dist = fs->get_first_friction_dist(); my_dist;
    ...
    info->friction.friction_infos[0] =
        my_dist->get_synapse(0)->get_object()->get_core()->get_friction_info(
            fs);  // #+# bad bad
    info->friction.friction_infos[1] =
        my_dist->get_synapse(1)->get_object()->get_core()->get_friction_info(
            fs);
    info->friction.dist_len = my_dist->get_gap_length();
    info->friction.has_negative_pull_since =
        my_dist->has_negative_pull_since;  // copy to avoid later cache misses
	...
	{
	    //ivp_message("dist_vec_world %.3f %.3f %.3f\n",dist_vector_world[dist_counter].k[0],dist_vector_world[dist_counter].k[1],dist_vector_world[dist_counter].k[2]);
	}
#endif

/* friction distance with number 'current_contact_point_index' is tested.
 * Therefore both objects of distance get a test push Velocity changes are
 * represented with 'rotation_vec' (obj coords) and 'translation_vec' (world
 * coords) then this funcion is called for both objects for one object is looked
 * if other distances are inflicted (speed of distance changes) if yes, speed
 * change is represented in a line of the matrix, with line number = number of
 * the distance values are added because a column of the matrix is built with
 * testing of two objects (example: diagonal elements, but not only)
 */
void IVP_Friction_Solver::calc_distance_matrix_column(
    int current_contact_point_index, IVP_Core *core,
    IVP_Friction_Info_For_Core *fr_info, IVP_U_Float_Point *rotation_vec,
    IVP_U_Float_Point *translation_vec) {
  // dist_counter is number of distance that is tested. It is represented with a
  ...
  for (int col = 0; col < fr_info->friction_springs.n_elems; col++) {
    ...
    int which_obj = 1;  // a distance is between two objects, which one is mine
                        // ? - 0 first, 1 second
    IVP_DOUBLE factor;

    IVP_Impact_Solver_Long_Term *info =
        contact_info_vector.element_at(line_num);
    ...
    if (info->contact_core[which_obj]) {
      IVP_DOUBLE speed_change_scalar =
          factor * get_closing_speed_core_i(info, which_obj, rotation_vec,
                                            translation_vec);
      dist_change_mat.matrix_values[line_num * dist_change_mat.aligned_row_len +
                                    current_contact_point_index] +=
          speed_change_scalar;
      // if ( line_num != current_contact_point_index){
      // dist_change_mat.matrix_values[current_contact_point_index *
      // dist_change_mat.columns + line_num] += speed_change_scalar * (
      // ivp_rand() * 0.001f + 1.0f);
      // }
    }
  }
}

// returns number of active mindists (that had in last PSI push_values different
// to 0.0f) array 'active_is_at_pos' is filled up to position
// 'active_is_at_pos'-1 main task is to fill 'dist_change_mat'
int IVP_Friction_Solver::calc_solver_PSI(IVP_Friction_System *fs,
  ...
  for (int i = 0; i < contact_info_vector.len(); i++) {
    ...
    IVP_DOUBLE distance_off =
        ivp_mindist_settings.friction_dist -
        info->friction.dist_len;  // distance to my optimal position
    dist_change_mat.desired_vector[i] =
        calc_desired_gap_speed(dist_velocity, distance_off, 1.0f);

    // mark contact point as active if positive pressure
    {
      // ivp_message("has_neg_since %d\n",my_dist->has_negative_pull_since);
    ...
    if (info->contact_core[0]) {
      ...
      calc_distance_matrix_column(i, info->contact_core[0],
                                  info->friction.friction_infos[0],
    ...
    if (info->contact_core[1]) {
      ...
      calc_distance_matrix_column(i, info->contact_core[1],
                                  info->friction.friction_infos[1],
                                  &rotation_vec, &translation_vec);
    }
  }
...
void IVP_Friction_System::reorder_mindists_for_complex() {
  ...
  while (my_frdist) {
    ...
    if (my_frdist->get_gap_length() >
        ivp_mindist_settings.friction_dist +
            ivp_mindist_settings.distance_keepers_safety) {
  ...
  // ivp_message("not_necessary_nr %ld\n",not_in_complex);
}

// first_frd and second_frd are neighbours in queue, first is before second,
// exchange them
void IVP_Friction_System::exchange_friction_dists(
    IVP_Contact_Point *first_frd, IVP_Contact_Point *second_frd) {
...
void IVP_Friction_System::ivp_debug_fs_pointers() {
  IVP_IF(1) {
    ...
    for (IVP_Contact_Point *fr_d = get_first_friction_dist(); fr_d;
      ...
      ivp_message("%zi %zi %d %zi  ",
                  (hk_intp)fr_d->prev_dist_in_friction & 0x0000ffff,
                  (hk_intp)fr_d & 0x0000ffff, fr_d->has_negative_pull_since,
                  (hk_intp)fr_d->next_dist_in_friction & 0x0000ffff);
    }
    ivp_message("\n");
  }
}
...
void IVP_Friction_System::bubble_sort_dists_importance() {
  ...
  while (1) {
    ...
    if (first_dist->has_negative_pull_since >
        second_dist->has_negative_pull_since) {
      exchange_friction_dists(first_dist, second_dist);
      IVP_Contact_Point *test_dist;
      while ((test_dist = get_prev_friction_dist(second_dist)) &&
             (test_dist->has_negative_pull_since >
              second_dist->has_negative_pull_since)) {
  ...
  IVP_IF(1) {
    for (first_dist = get_first_friction_dist(); first_dist;
         first_dist = get_next_friction_dist(first_dist)) {
      second_dist = get_next_friction_dist(first_dist);
      if (second_dist) {
        IVP_ASSERT(!(first_dist->has_negative_pull_since >
                     second_dist->has_negative_pull_since));
      }
    }
  }
...
void IVP_Friction_System::bubble_sort_dists_importance()
{
	...
	IVP_Contact_Point *test_dist;
	//ivp_message("tlp_for %lx  ",(long)moving_dist&0x0000ffff);
	...
	while((test_dist=this->get_next_friction_dist(moving_dist))!=NULL) {
		...
		did_exchange=1;
		//ivp_message("exchgg %lx %lx  ",(long)moving_dist&0x0000ffff,(long)test_dist&0x0000ffff);
		//ivp_debug_fs_pointers();
	    } else {
	...
	for(IVP_Contact_Point *fr_d=get_first_friction_dist();fr_d;fr_d=get_next_friction_dist(fr_d))
	{
		...
		if(fr_d->has_negative_pull_since<test_d->has_negative_pull_since) {
		    ivp_message("bubble_sort_error\n");
		    CORE;
		}
	    }
	}
    }
    //ivp_message("after_bs  ");
    //ivp_debug_fs_pointers();
    IVP_IF(0)
    {
        ivp_message("sort_dists ");
	for(IVP_Contact_Point *fr_d=get_first_friction_dist();fr_d;fr_d=get_next_friction_dist(fr_d))
	{
	    ivp_message("%d ",fr_d->has_negative_pull_since);
	}
	ivp_message("\n");
    }
}
...
IVP_BOOL IVP_Friction_System::core_is_terminal_in_fs(IVP_Core *test_core) {
  ...
  for (int i = fr_pairs_of_objs.len() - 1; i >= 0; i--) {
    ...
    if ((fcp->objs[0] == test_core) || (fcp->objs[1] == test_core)) {
      int unmov =
          fcp->objs[0]->physical_unmoveable | fcp->objs[1]->physical_unmoveable;
      if (!unmov) {
  ...
  // ivp_message("found_terminal %lx\n",(long)test_core&0x0000ffff);
  return IVP_TRUE;
}

// fallback for oversized matrizes:
// for terminal objects: turn on fast piling technique and hope they will be
// pushed away for nonterminal objects: turn on fast piling but delete whole
// energy (with delayed pushes the energy would be accumulated)
...
void IVP_Friction_System::do_friction_system(const IVP_Event_Sim *es_in) {
  ...
  IVP_IF(l_environment->debug_information->debug_friction) {
    ivp_message("doing_friction_nr %d\n", friction_global_counter);
    if (friction_global_counter == 14) {
      ivp_message("losgehts\n");
    }
    ivp_message("friction_system_simulating %d distances\n",
                this->friction_dist_number);
  }

  this->bubble_sort_dists_importance();  // to to put heavy pushed mindists
                                         // ontop of the list
  this->reorder_mindists_for_complex();  // get rid of mindist already heaving a
                                         // big gap @+@ ->check speed after
                                         // do_pushes_distance_keepers also

#if defined(IVP_DISTANCE_KEEPERS) && !defined(IVP_PANELTY_SOLVER)
  this->do_pushes_distance_keepers(es_in);  // do local constraint pushes
#endif

  if ((this->friction_dist_number - this->complex_not_necessary_number) >
      IVP_MAX_FRICTION_MATRIX) {
  ...
  while (IVP_TRUE) {
    ...
    IVP_IF(l_environment->debug_information->disable_friction) { break; }
    ...
    fr_solver.solve_linear_equation_and_push(this, original_pos_of_active,
                                             num_actives, my_mem);
    break;
  }
  my_mem->end_memory_transaction();
...
void IVP_Friction_System::confirm_complex_pushes() {
  for (int k = moveable_cores_of_friction_system.len() - 1; k >= 0; k--) {
    ...
    my_core->commit_all_async_pushes();  // not too nice: no
                                         // clear_all_async_pushes done before
  }
}
