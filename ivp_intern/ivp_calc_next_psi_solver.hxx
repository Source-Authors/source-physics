...
class IVP_Vector_of_Hull_Managers_1
...
class IVP_Calc_Next_PSI_Solver {
  ...
  inline void calc_rotation_matrix(
      IVP_FLOAT delta_sim_time,
      IVP_U_Quat *q_core_f_core_out);  // calc q_core_f_core
  IVP_DOUBLE get_longest_time_step_dependent_on_rot();

  // void reduce_energy_by_damping(IVP_DOUBLE remove_energy); //not used at
  // moment void get_next_simulation_slot(IVP_DOUBLE next_lowest_psi,int
  // *next_used_slot,int *time_step);

  void calc_psi_rotation_axis(
      const IVP_U_Quat *q_core_f_core);  // calc the rotation matrix
 ...
 public:
  ...
  static void commit_one_hull_manager(
      ...
      IVP_U_Vector<IVP_Hull_Manager_Base>
          *active_hull_managers);  // if you now there is only one hull_manager
